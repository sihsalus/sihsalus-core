...
/**
 * CustomReflectionConverter and IgnoreUnknownElementMapper together implement the function that
 * ignoring unknown elements when xstream deserialize from a xml string
 *
 * <p>You can see the source code of "ReflectionConverter" to understand the logic of this class.
 * Because the difference between this class and "ReflectionConverter" is only in line 165 of this
 * class comparing with "ReflectionConverter"
 *
 * @see ReflectionConverter.java -
 *     http://svn.codehaus.org/xstream/trunk/xstream/src/java/com/thoughtworks/xstream/converters/reflection/ReflectionConverter.java
 * @see AbstractReflectionConverter.java -
 *     http://svn.codehaus.org/xstream/trunk/xstream/src/java/com/thoughtworks/xstream/converters/reflection/AbstractReflectionConverter.java
  ...
  private Class determineType(
    ...
    if (classAttribute != null) {
      ...
      if (itemType != null) {
        ...
        if (definedInCls == null) {
          for (definedInCls = result.getClass();
              definedInCls != null;
              definedInCls = definedInCls.getSuperclass()) {
      ...
      return mapper.defaultImplementationOf(
          reflectionProvider.getFieldType(result, fieldName, definedInCls));
    }
  }
  ...
  public Object doUnmarshal(
    ...
    while (it.hasNext()) {
      ...
      boolean fieldExistsInClass =
          reflectionProvider.fieldDefinedInClass(attrName, result.getClass());
    ...
    while (reader.hasMoreChildren()) {
      ...
      Mapper.ImplicitCollectionMapping implicitCollectionMapping =
          mapper.getImplicitCollectionDefForFieldName(result.getClass(), fieldName);

      Class classDefiningField = determineWhichClassDefinesField(reader);

      boolean fieldExistsInClass =
          implicitCollectionMapping == null
              && reflectionProvider.fieldDefinedInClass(fieldName, result.getClass());
      ...
      if (fieldExistsInClass) {
        Field field =
            reflectionProvider.getField(
                classDefiningField != null ? classDefiningField : result.getClass(), fieldName);
        if (Modifier.isTransient(field.getModifiers()) && !shouldUnmarshalTransientFields()) {
          reader.moveUp();
          continue;
        }
      ...
      if (value != null && targetType != null && !targetType.isAssignableFrom(value.getClass())) {
        ...
        // throw new ConversionException("Cannot convert type " + value.getClass().getName() + " to
        // type " + type.getName());
      }
      ...
      // here add a condition "implicitCollectionMapping != null" to judge whether need to serialize
      // a implicitCollection
      else if (type != null && implicitCollectionMapping != null) {
        implicitCollectionsForCurrentObject =
            writeValueToImplicitCollection(
                context, value, implicitCollectionsForCurrentObject, result, originalNodeName);
      }
