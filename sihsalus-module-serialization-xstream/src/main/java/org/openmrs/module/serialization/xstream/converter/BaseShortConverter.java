...
public abstract class BaseShortConverter implements Converter {
   ...
   * @param converterLookup - used to look up a converter for fully serializing the short-serialized
   *     object
   */
   ...
   *
   * @param context - xstrema can know how many objects are serialized with a "id" attribute through
   *     this param
   ...
   * Note: this method will be used in every short converter's "unmarshal(HierarchicalStreamReader,
  ...
  /**
   * If this object is an OpenMRS object, if it is not the base node in the serialization (eg. it is
   * a property of another object that is being serialized), and if it has been previously saved to
   * the database, this will serialize the object by uuid. Otherwise, it will perform the default
   * full serialization of all of it's underlying properties
   *
  ...
  public void marshal(Object obj, HierarchicalStreamWriter writer, MarshallingContext context) {
    ...
    if (needsFullSeralization(context) || uuid == null) {
      if (isCGLibProxy(obj.getClass())) {
        CustomCGLIBEnhancedConverter converter =
            new CustomCGLIBEnhancedConverter(getMapper(), getConverterLookup());
        converter.marshal(obj, writer, context);
      } else if (isJavassistProxy(obj.getClass())) {
        CustomJavassistEnhancedConverter converter =
            new CustomJavassistEnhancedConverter(getMapper(), getConverterLookup());
        converter.marshal(obj, writer, context);
      } else {
  ...
  /**
   * Every short converter should override this method, so that it can correctly get object from its
   * uuid.
   *
   * <pre>
