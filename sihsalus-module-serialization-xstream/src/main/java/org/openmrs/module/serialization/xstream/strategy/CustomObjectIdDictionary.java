...
public class CustomObjectIdDictionary {
  ...
  /**
   * Iterate each entry of map to find whether it has a key equals with "obj" <br>
   *
   ...
   * <br>
   * Note: Here, the mechanism of comparison is equals with {@link
   * CustomObjectIdDictionary#lookupId(Object)}
   *
   * @param obj - the object need to be compared with keys of map
  ...
  /**
   * Judge whether obj is a Collection or Map And its size is zero <br>
   *
  ...
  private boolean isCollectionAndNoItems(Object obj) {
    ...
    if (Collection.class.isAssignableFrom(cls) && ((Collection) obj).size() == 0) return true;
    else if (Map.class.isAssignableFrom(cls) && ((Map) obj).size() == 0) return true;
    else return false;
  }
}
