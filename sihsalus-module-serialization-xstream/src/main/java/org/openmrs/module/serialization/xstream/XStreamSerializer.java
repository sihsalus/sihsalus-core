...
public class XStreamSerializer implements OpenmrsSerializer {
  ...
  @Autowired private HibernateCollectionConverter collectionConverter;
  ...
  public XStreamSerializer(XStream customXstream) throws SerializationException {
    ...
    xstream.registerConverter(
        new CustomCGLIBEnhancedConverter(xstream.getMapper(), xstream.getConverterLookup()));
    xstream.registerConverter(
        new CustomJavassistEnhancedConverter(xstream.getMapper(), xstream.getConverterLookup()));
  ...
  private List<String> getAllSerializedPackages() {
    ...
    packageNames.add(
        "org.openmrs"); // do serialization/deserialization for all classes in package "org.openmrs"
          ...
          if (dir.exists() && dir.isDirectory()) {
            ...
            for (File f : fileList) {
              ...
              if (f.isFile() && f.getName().endsWith(".class") && !f.getName().contains("$")) {
                String className =
                    packageName + "." + f.getName().substring(0, f.getName().indexOf(".class"));
                loadedClass = OpenmrsClassLoader.getInstance().loadClass(className);
                list.add(loadedClass);
              }
            }
   ...
   * "org.openmrs.User.class". The alias principle is to low the first letter of the simple name of
   * class "c" Note: if in module have a few new classes need to be serialize, call this method for
   * each new class will automatically alias className for them
   *
   * @param c - the class need to alias its className
   */
  private void aliasClassName(Class<?> c) {
    // through Class.getSimpleName(), we get the short name of a class, such as get "User" for class
    // "org.openmrs.User"
