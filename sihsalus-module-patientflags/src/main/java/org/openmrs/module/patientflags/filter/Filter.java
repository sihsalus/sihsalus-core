...
public class Filter {
  ...
  public Filter() {
    type =
        FilterType
            .ANYTAG; // default the Filter	to match all Flags that contain one or more of the tags
    // in the filter
  }
    ...
    if (this.type == FilterType.ANYTAG || this.type == FilterType.ANYTAG_OR_NOTAG) {
      for (Flag flag : flags) {
        ...
        if (flagTags != null && !flagTags.isEmpty()) {
          ...
          // if the flag has no tags associated it it and this is an ANYTAG_OR_NOTAG filter, add the
          // flag to the result list
      ...
      for (Flag flag : flags) {
        if (flag.getTags() != null && flag.getTags().containsAll(this.tags)) results.add(flag);
      }
    } else {
