...
public class SQLFlagEvaluator implements FlagEvaluator {
  ...
  public Boolean eval(Flag flag, Patient patient, Map<Object, Object> context) {
    ...
    if (patient.getVoided())
      throw new APIException(
          "Unable to evaluate SQL flag " + flag.getName() + " against voided patient");
    ...
    if (patientIdColumn == null) {
      throw new APIException(
          "Unable to evaluate SQL flag " + flag.getName() + " without patient_id criteria");
    }

    // since we are going to append a where/and to the end of this sql statement, we need to trim
    // off trailing ";" and any trailing whitespace
    ...
    String toEval =
        criteria
            + (containsIgnoreCase(criteria, "where") ? " and " : " where ")
            + patientIdColumn
            + " = "
    ...
    try {
      ...
      throw new APIException(
          "Unable to evaluate SQL Flag " + flag.getName() + ", " + e.getLocalizedMessage(), e);
    } finally {
  ...
  public String evalMessage(Flag flag, int patientId) {
    ...
    if (p.getVoided()) throw new APIException("VOIDED PATIENT");
    ...
    if (patientIdColumn == null) {
      throw new APIException(
          "Unable to evaluate SQL flag message " + flag.getName() + " without patient_id criteria");
    }
    ...
    try {
      ...
      throw new APIException(
          "Unable to evaluate SQL Flag Message" + flag.getName() + ", " + e.getLocalizedMessage(),
          e);
    } finally {
      Context.removeProxyPrivilege("SQL Level Access");
    }
  ...
  String findPatientIdColumn(String criteria) {
    ...
    while (fromIndex <= criteria.length() - search.length()) {
      ...
      boolean hasColumnBoundary =
          columnEnd == criteria.length() || !isSqlWordChar(criteria.charAt(columnEnd));
