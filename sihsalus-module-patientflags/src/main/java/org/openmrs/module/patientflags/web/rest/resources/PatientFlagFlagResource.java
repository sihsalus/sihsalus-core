...
@Resource(
    name =
        RestConstants.VERSION_1 + PatientFlagsRestController.PATIENT_FLAGS_REST_NAMESPACE + "/flag",
    supportedClass = Flag.class,
...
public class PatientFlagFlagResource extends MetadataDelegatingCrudResource<Flag> {
  ...
  protected PageableResult doSearch(RequestContext context) {
    ...
    if (StringUtils.isNotBlank(patientUuid)) {
      ...
      if (patient != null) {
        return new NeedsPaging<Flag>(
            Context.getService(FlagService.class).getFlagsForPatient(patient), context);
      }
      ...
      return new NeedsPaging<Flag>(
          Context.getService(FlagService.class).searchFlags(q, evaluator, enabled, tags), context);
    }
  }
