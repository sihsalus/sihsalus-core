...
public class GroovyFlagEvaluator implements FlagEvaluator {
  ...
  public Boolean eval(Flag flag, Patient patient, Map<Object, Object> context) {
    ...
    if (patient.getVoided())
      throw new APIException(
          "Unable to evaluate Groovy flag " + flag.getName() + " against voided patient");
  ...
  public Cohort evalCohort(Flag flag, Cohort cohort, Map<Object, Object> context) {
    ...
    try {
      ...
      // (note that we pass 'null' as the user parameter because we don't want to restrict flag
      ...
      // if we do this, then we must also comment out the wait() in fetchResultCohort() in the
      // GroovyFlagEvaluator Thread
      // evaluatorThread.run();
      ...
      throw new APIException(
          "Unable to evaluate Groovy Flag " + flag.getName() + ", " + e.getLocalizedMessage(), e);
    }
  }
  ...
  public FlagValidationResult validate(Flag flag) {
    try {
      ...
      // (note that in this case we pass the current user to the thread, because we want to restrict
      // flag validation based on the user-
      //  a user can evaluate a flag even if she doesn't have all the required privileges, but
      // shouldn't be able to create/update a flag
      //  the does something she doesn't have privileges to do herself)
      GroovyFlagEvaluatorThread evaluatorThread =
          new GroovyFlagEvaluatorThread(flag, new Cohort(), Context.getAuthenticatedUser());
      new Thread(evaluatorThread).start();
