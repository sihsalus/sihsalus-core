...
public class SihsalusPatientFlagsConfiguration {
    ...
    @Bean
    SmartInitializingSingleton patientFlagsStaticInitializer(AdministrationService administrationService) {
    ...
    private static void ensureGlobalProperty(
            AdministrationService administrationService, String property, String defaultValue, String description) {
        if (administrationService.getGlobalPropertyObject(property) == null) {
            administrationService.saveGlobalProperty(new GlobalProperty(property, defaultValue, description));
        }
    }
}
