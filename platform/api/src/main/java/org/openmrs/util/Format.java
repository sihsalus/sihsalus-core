...
public class Format {
  ...
  /**
   * This method formats a date object according to a particular locale and returns the date as a
   * string. The string can contain only the date (month, day and year), only the time (hours,
   * minutes, seconds) or as a timestamp (both date and time).
   *
   * <p><strong>Should</strong> not fail when only date is null<br>
   * <strong>Should</strong> not fail when only locale is null<br>
   * <strong>Should</strong> not fail when only type is null<br>
   * <strong>Should</strong> not fail when date and locale is null<br>
   * <strong>Should</strong> not fail when date and type is null<br>
   * <strong>Should</strong> not fail when locale and type is null<br>
   * <strong>Should</strong> not fail when all parameters are null<br>
   ...
   * @return empty string if one of the parameters is null. Otherwise a string object for the date
   *     such that it is formatted according to locale and the amount of information it contains is
   *     determined by type.
   */
