...
public class ConvertOrderersToProviders implements CustomTaskChange {
  ...
  private List<List<Object>> getUsersAndProviders(JdbcConnection connection)
    ...
    final String query =
        "SELECT u.user_id AS userId, p.provider_id AS providerId FROM users u, provider p"
  ...
  private void convertOrdererToProvider(
      JdbcConnection connection, List<List<Object>> usersAndProviders)
    ...
    try {
      ...
      updateStatement =
          connection.prepareStatement("UPDATE orders SET orderer = ? WHERE orderer = ?");
