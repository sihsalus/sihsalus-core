...
public class ValidateUtil {
   ...
   * class
   *
   * <p><strong>Should</strong> throw APIException if errors occur during validation<br>
    ...
    for (Object objerr : errors.getAllErrors()) {
      ...
      String message =
          Context.getMessageSourceService()
              .getMessage(error.getCode(), error.getArguments(), Context.getLocale());
   ...
   * class
   *
   * <p><strong>Should</strong> populate errors if object invalid<br>
   ...
   *
   * <p><strong>Should</strong> pass validation if regEx field length is not too long<br>
  ...
  /**
   * Used to indicate that validation should be re-enabled for the current thread Typically this
   * would be placed in a `finally` block after the disableValidationForThread method is used
   *
   * @since 2.5.8
   */
