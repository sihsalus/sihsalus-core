...
public class HL7Util {
   ...
   * varying levels of precision &mdash; e.g., just the year or just the year and month, etc. Since
   * java.util.Date cannot store a partial value, we fill in defaults like January, 01 at midnight
   * within the current timezone.
   *
   * <p><strong>Should</strong> fail on 19784 <strong>Should</strong> handle 197804<br>
   * <strong>Should</strong> fail on 197841 <strong>Should</strong> handle 19780411<br>
   * <strong>Should</strong> fail on 197804116 <strong>Should</strong> handle<br>
   * <strong>Should</strong> handle 19780411061538 <strong>Should</strong> handle<br>
   * <strong>Should</strong> handle 19780411061538.123 <strong>Should</strong> handle<br>
   ...
   *     1978041106 <strong>Should</strong> fail on 19780411065 <strong>Should</strong> handle
   *     197804110615 <strong>Should</strong> fail on 1978041106153 19780411061538.1
   *     <strong>Should</strong> handle 19780411061538.12 19780411061538.1234
   *     <strong>Should</strong> fail on 197804110615-5 197804110615-0200 <strong>Should</strong>
   *     not flub dst with 20091225123000
   */
   ...
   *     string if exists in given string <strong>Should</strong> return timezone for givenDate and
  ...
  /**
   * Convenience method for parsing HL7 dates (treated just like a timestamp with only year, month,
   * and day specified)
   *
  ...
  /**
   * Converts an HL7 time into a java.util.Date object. Since the java.util.Date object cannot store
   * just the time, the date will remain at the epoch (e.g., January 1, 1970). Time more precise
   * than microseconds is ignored.
   *
   ...
   *     handle 0615 <strong>Should</strong> handle 061538 <strong>Should</strong> handle 061538.1
   *     <strong>Should</strong> handle 061538.12 <strong>Should</strong> handle 061538.123
   *     <strong>Should</strong> handle 061538.1234 <strong>Should</strong> handle 061538-0300
   */
  ...
  public static File getHl7ArchivesDirectory() throws APIException {
    ...
    if (StringUtils.isBlank(archiveDir)) {
      log.warn(
          "Invalid value for global property '"
