...
public interface HL7Service extends OpenmrsService {
   ...
   *
   * <p><strong>Should</strong> add generated uuid if uuid is null
   *
  ...
  @Authorized(
      value = {
        PrivilegeConstants.PRIV_UPDATE_HL7_IN_QUEUE,
  ...
  public List<HL7InArchive> getHL7InArchiveBatch(
      int start, int length, int messageState, String query) throws APIException;
  ...
  @Authorized(
      value = {
        PrivilegeConstants.PRIV_UPDATE_HL7_IN_ARCHIVE,
        PrivilegeConstants.PRIV_ADD_HL7_IN_ARCHIVE
      },
      requireAll = false)
   ...
   * can't do a look up by hl7ArchiveId, instead call {@link
  ...
  /**
   * If hl7 migration has been run and the state matches that of processed items, the method returns
   * a list of all archives in the file system, for any other state , it returns an empty list, this
   * is because all archives would have a status of 'processed' after migration and all deleted
   * archives moved back into the queue with a status of 'deleted' otherwise it returns archives
   * with a matching state if migration hasn't yet been run.
   *
  ...
  @Authorized(
      value = {
        PrivilegeConstants.PRIV_UPDATE_HL7_IN_EXCEPTION,
        PrivilegeConstants.PRIV_ADD_HL7_IN_EXCEPTION
      },
      requireAll = false)
   ...
   *
   * @return a List&lt;HL7InError&gt; object with all <code>HL7InError</code> items from the
   *     database
   * @throws APIException
   */
  @Authorized(PrivilegeConstants.GET_HL7_IN_EXCEPTION)
   ...
   *
   * <p><strong>Should</strong> return internal identifier of location if only location name is
   * specified<br>
  ...
  /**
   * determines a person (or patient) based on identifiers from a CX array, as found in a PID or NK1
   * segment; the first resolving identifier in the list wins
   *
   * <p><strong>Should</strong> find a person based on a patient identifier<br>
   * <strong>Should</strong> find a person based on a UUID<br>
   * <strong>Should</strong> find a person based on the internal person ID<br>
   ...
   *
   * <p><strong>Should</strong> create HL7InArchive after successful parsing<br>
   * <strong>Should</strong> create HL7InError after failed parsing<br>
   * <strong>Should</strong> fail if given inQueue is already marked as processing<br>
   ...
   *
   * <p><strong>Should</strong> parse the given string into Message
   *
   ...
   *
   * <p><strong>Should</strong> save hl7Message to the database<br>
  ...
  @Authorized(
      requireAll = true,
      value = {
        PrivilegeConstants.GET_HL7_IN_ARCHIVE,
        PrivilegeConstants.PRIV_PURGE_HL7_IN_ARCHIVE,
        PrivilegeConstants.PRIV_ADD_HL7_IN_QUEUE
      })
  public void migrateHl7InArchivesToFileSystem(Map<String, Integer> progressStatusMap)
      throws APIException;

  /**
   * finds a UUID from an array of identifiers
   *
   * <p><strong>Should</strong> return null if no UUID found<br>
   * <strong>Should</strong> find a UUID in any position of the array<br>
   * <strong>Should</strong> not fail if multiple similar UUIDs exist in identifiers<br>
   * <strong>Should</strong> not fail if no assigning authority is found<br>
  ...
  /**
   * creates a Person from information held in an NK1 segment; if valid PatientIdentifiers exist, a
   * Patient will be created and returned
   *
   * <p><strong>Should</strong> return a saved new person<br>
   * <strong>Should</strong> return a Patient if valid patient identifiers exist<br>
   * <strong>Should</strong> fail if a person with the same UUID exists<br>
   * <strong>Should</strong> fail on an invalid gender<br>
   * <strong>Should</strong> fail if no gender specified<br>
   ...
   *
   * <p>It calls {@link #getHL7InQueueByUuid(String)}, {@link #getHL7InArchiveByUuid(String)} and
