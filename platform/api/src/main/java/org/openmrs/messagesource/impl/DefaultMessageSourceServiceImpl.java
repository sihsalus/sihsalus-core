...
public class DefaultMessageSourceServiceImpl implements MessageSourceService {
  ...
  /**
   * Private class to hold the one instance. This is an alternative to storing the instance object
   * on {@link DefaultMessageSourceServiceImpl} itself so that garbage collection can happen
   * correctly.
   */
  ...
  private DefaultMessageSourceServiceImpl() {
    InputStream stream =
        OpenmrsClassLoader.getInstance().getResourceAsStream("messages.properties");
    if (stream != null) {
