...
public class Condition extends BaseFormRecordableOpenmrsData {
  ...
  @AssociationOverrides({
    ...
    @AssociationOverride(
        name = "specificName",
        joinColumns = @JoinColumn(name = "condition_coded_name"))
  })
  ...
  @Transient private String endReason;
   ...
   * @param verificationStatus - the verification status of the condition, describing if the
   *     condition is confirmed or not
  ...
  /**
   * Creates a new Condition instance from the passed condition such that the newly created
   * Condition matches the passed Condition @see Condition#matches, but does not equal the passed
   * Condition (uuid, id differ)
   *
  ...
  /**
   * Compares properties with those in the given Condition to determine if they have the same
   * meaning This method will return true immediately following the creation of a Condition from
   * another Condition
   *
   * @see Condition#newInstance(Condition) This method will return false if any value is different,
   *     excepting identity data (id, uuid) If the given instance is null, this will return false
  ...
  public boolean matches(Condition c) {
    ...
    ret =
        ret && (OpenmrsUtil.nullSafeEquals(getFormNamespaceAndPath(), c.getFormNamespaceAndPath()));
