 ...
 *
 * <p>This is used by the PersonService. This should not be used directly, but rather used through
 * the methods on the PersonService.
 *
 * <p>Use case: <code>
...
public interface PersonDAO {
  ...
  public List<Relationship> getRelationships(
      Person fromPerson, Person toPerson, RelationshipType relType) throws DAOException;
   ...
   * change the vlaue of any global property which is in {@link
   * OpenmrsConstants#GLOBAL_PROPERTIES_OF_PERSON_ATTRIBUTES} and reference the given
   * personAttributeType. <br>
   *
   * <p><strong>Should</strong> get saved personAttributeType name from database
   *
  ...
  /**
   * Gets the value of the searchable property currently saved in the database for the given
   * personAttributeType, bypassing any caches. This is used when saving an personAttributeType, so
   * that we can determine if the searchable property has changed, which we use to determine whether
   * we need to update the Lucene index
   *
