 ...
 *       the database
 *   <li>Call {@link Context#openSession()} to start a "unit of work".
 *   <li>Call {@link Context#authenticate(String, String)} to authenticate the current user on the
 *       current thread
 *   <li>Call {@link Context#closeSession()} to end your "unit of work" and commit all changes to
 *       the database.
 * </ol>
 *
 * <br>
...
public class Context {
  ...
  // bug in Java 1.5
  private static final ThreadLocal<Object[] /* UserContext */> userContextHolder =
      new ThreadLocal<>();
   ...
   *
   * <p><strong>Should</strong> load class with the OpenmrsClassLoader
   *
   ...
   * same time.
   *
   * <p><strong>Should</strong> fail if session hasn't been opened
   *
  ...
  public static UserContext getUserContext() {
    ...
    log.trace(
        "Getting user context {} from userContextHolder {}",
        Arrays.toString(arr),
        userContextHolder);
   ...
   *
   * @deprecated as of 2.3.0, replaced by {@link #authenticate(Credentials)} Used to authenticate
   *     user within the context
   ...
   * @since 2.3.0
   */
  public static Authenticated authenticate(Credentials credentials)
      throws ContextAuthenticationException {

    if (Daemon.isDaemonThread()) {
      log.error(
          "Authentication attempted while operating on a "
              + "daemon thread, authenticating is not necessary or allowed");
      return new BasicAuthenticated(
    ...
    if (credentials == null) {
      throw new ContextAuthenticationException(
          "Context cannot authenticate with null credentials.");
    }
   ...
   *
   * <p><strong>Should</strong> get fresh values from the database
   *
   * @since 1.5
   */
   ...
   *
   * <p><strong>Should</strong> change locale when become another user
   *
   * @param systemId
   * @throws ContextAuthenticationException
   */
   ...
   *     properties, and/or system properties, with system properties overriding runtime properties
  ...
  public static Properties getMailProperties() {
    ...
    for (GlobalProperty gp : getAdministrationService().getGlobalPropertiesByPrefix(prefix)) {
      // Historically, some mail properties defined with underscores, support these for legacy
      // compatibility
              ...
              new Authenticator() {
                ...
                public PasswordAuthentication getPasswordAuthentication() {
                  return new PasswordAuthentication(
                      ConfigUtil.getProperty("mail.user"), ConfigUtil.getProperty("mail.password"));
                }
              };
  ...
  /**
   * Convenience method to allow us to change the configuration more easily. TODO Ideally, we would
   * be using Spring's method injection to set the dependencies for the message service.
   *
   ...
   *
   * <p><strong>Should</strong> not fail if session hasn't been opened yet
   *
   * @see #authenticate
   */
      ...
      if (StringUtils.isNotBlank(privilege)) {
        errorMessage =
            Context.getMessageSourceService()
                .getMessage(
                    "error.privilegesRequired", new Object[] {privilege}, Locale.getDefault());
      } else {
        // Should we even be here if the privilege is blank?
        errorMessage =
            Context.getMessageSourceService().getMessage("error.privilegesRequiredNoArgs");
      }
   ...
   *
   * <p>This is useful for situations where a system process may need access to some piece of data
   * that the user would not otherwise have access to, like a GlobalProperty. <strong>This facility
   * should not be used to return data to the user that they otherwise would be unable to
   * see.</strong>
   *
   * <p>The expected usage is:
   *
   * <p>
   *
   * <pre>{@code
  ...
  /**
   * Removes one or more privileges from the list of privileges that that {@link
   * #hasPrivilege(String)} will regard as available regardless of whether the user would otherwise
   * have the privilege.
   *
   * <p>This is the compliment for {@link #addProxyPrivilege(String...)} to clean-up the context.
   *
   * <p>
   *
   ...
   *
   * <p><strong>Should</strong> not fail if session hasn't been opened
   */
   ...
   * database. If you call this method, and later call closeSession() or flushSession() your changes
   * are still lost.
   */
   ...
   *
   * <p><strong>Should</strong> return true if session is closed
   *
  ...
  /**
   * Used to clear a cached object out of a session in the middle of a unit of work. Future updates
   * to this object will not be saved. Future gets of this object will not fetch this cached copy
   *
   ...
   * @throws DatabaseUpdateException if database updates are required, see {@link
   ...
   * <br>
   * <b>Note:</b> This method calls {@link Context#openSession()}, so you must call {@link
   * Context#closeSession()} somewhere on the same thread of this application so as to not leak
   * memory.
   *
  ...
  public static synchronized void startup(
      String url, String username, String password, Properties properties)
      throws DatabaseUpdateException, InputRequiredException, ModuleMustStartException {
    if (properties == null) {
   ...
   *
   * <p><strong>Should</strong> return the same object when called multiple times for the same class
   *
   ...
   *
   * <p>Advisors can wrap around a method and effect the method before or after
   *
   * @param cls
   * @param advisor
   ...
   *
   * <p>Advice comes in the form of before or afterReturning methods
   *
   * @param cls
   * @param advice
   * @deprecated since 3.0.0 use {@link AOPConfig}
  ...
  public static void checkCoreDataset() {
    ...
    Boolean disableValidation =
        Boolean.valueOf(as.getGlobalProperty(OpenmrsConstants.GP_DISABLE_VALIDATION, "false"));
    ...
    Allergen.setOtherNonCodedConceptUuid(
        Context.getAdministrationService()
            .getGlobalProperty(OpenmrsConstants.GP_ALLERGEN_OTHER_NON_CODED_UUID));
  }
   ...
   * to mm/dd/yyyy
   *
   * <p><strong>Should</strong> return a pattern with four y characters in it
   *
  ...
  /**
   * Gets the simple time format for the current user's locale. The format will be similar to hh:mm
   * a
   *
   * <p><strong>Should</strong> return a pattern with two h characters in it
   *
   * @return SimpleDateFormat for the user's current locale
   ...
   *
   * <p>There is no need to call this method in normal usage since the index is automatically
   * updated whenever DB transactions are committed.
   *
   * <p>The method is designated to be used in tests, which rollback transactions. Note that if the
   ...
   *
   * <p>Updates the search index for objects of the given type.
   *
   ...
   * procedure without optimization, please set the <code>optimized.startup</code> runtime property
   * to <code>false</code>. (<code>true</code> by default)
   *
   * <p>See <a href="https://issues.openmrs.org/browse/TRUNK-6417">TRUNK-6417</a>
   *
  ...
  public static boolean isOptimizedStartup() {
    return Boolean.parseBoolean(
        Context.getRuntimeProperties().getProperty("optimized.startup", "true"));
  }
}
