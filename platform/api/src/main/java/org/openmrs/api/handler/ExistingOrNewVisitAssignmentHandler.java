...
public class ExistingOrNewVisitAssignmentHandler extends ExistingVisitAssignmentHandler
  ...
  private static void setEncounterVisitMapping(
      Map<EncounterType, VisitType> encounterVisitMapping) {
  ...
  public String getDisplayName(Locale locale) {
    return Context.getMessageSourceService()
        .getMessage("visit.assignmentHandler.assignToExistingVisitOrNew", null, locale);
  }

  /**
   * <strong>Should</strong> assign existing visit if match found<br>
   * <strong>Should</strong> assign new visit if no match found<br>
   * <strong>Should</strong> resolve encounter and visit type uuids as global property values
   *
   * @see
   *     org.openmrs.api.handler.ExistingVisitAssignmentHandler#beforeCreateEncounter(org.openmrs.Encounter)
   */
  @Override
  ...
  private static VisitType loadVisitType(EncounterType encounterType) throws APIException {
    ...
    if (!StringUtils.isBlank(value)) {
      ...
      throw new APIException(
          "GlobalProperty.error.loadVisitType", new Object[] {encounterType.getName()});
    }
