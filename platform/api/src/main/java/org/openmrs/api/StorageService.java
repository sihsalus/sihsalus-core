 ...
 *
 * <p>Data objects are stored under generated or provided unique keys. Data and keys are immutable.
 *
 * <p>{@link ObjectMetadata} can be passed in save methods. It is implementation dependent on how
 * the metadata is used or persisted.
 *
...
public interface StorageService extends OpenmrsService {
   ...
   *
   * <p>Remember to close the stream when done iterating.
   *
  ...
  /**
   * Saves the given InputStream with auto-generated key (should be preferred to avoid key collision
   * and use the best key structure).
   *
  ...
  /**
   * Saves the given InputStream as a temporary file. Temporary files are kept, if possible, in
   * local storage for fast IO. You shall assume that they are not accessible between HTTP requests
   * and different replicas.
   *
   * @param inputStream data
   * @param metadata object metadata or null
   * @return unique key
  ...
  /**
   * Saves data as a temporary file. Temporary files are kept, if possible, in local storage for
   * fast IO. You shall assume that they are not accessible between HTTP requests and different
   * replicas.
   *
   * <p>It uses pipes internally to avoid copying data into memory and writes directly to storage in
   * a separate thread.
   *
   ...
   *
   * <p>{@link #saveData(InputStream, ObjectMetadata, String)} should be preferred to avoid key
   * collision and use the best key structure.
   *
   * @param inputStream data
   * @param metadata object metadata or null
   * @param moduleIdOrGroup module id or group (allowed characters <code>[\\w-./]+</code>)
   ...
   */
  String saveData(
      InputStream inputStream, ObjectMetadata metadata, String moduleIdOrGroup, String keySuffix)
      throws IOException;

  /**
   * Saves the data under the given key suffix.
   *
   * <p>{@link #saveData(StreamDataWriter, ObjectMetadata, String)} should be preferred to avoid key
   * collision and use the best key structure.
   *
   * <p>It uses pipes internally to avoid copying data into memory and writes directly to storage in
   * a separate thread.
   ...
   */
  String saveData(
      StreamDataWriter writer, ObjectMetadata metadata, String moduleIdOrGroup, String keySuffix)
      throws IOException;

  /**
   * Saves the data with auto-generated key (should be preferred to avoid key collision and use the
   * best key structure).
   *
   * <p>It uses pipes internally to avoid copying data into memory and writes directly to storage in
   * a separate thread.
   ...
   * @throws IOException if key exists or IO error
   */
  String saveData(StreamDataWriter writer, ObjectMetadata metadata, String moduleIdOrGroup)
      throws IOException;
