 ...
 * <br>
 * This class should not be used directly. All calls should go through the {@link
 * org.openmrs.api.ProgramWorkflowService} methods.
 *
...
public class HibernateProgramWorkflowDAO implements ProgramWorkflowDAO {
  ...
  public List<Program> findPrograms(String nameFragment) throws DAOException {
    ...
    Predicate nameLike =
        cb.like(cb.lower(root.get("name")), MatchMode.ANYWHERE.toLowerCasePattern(nameFragment));
  ...
  /**
   * @see
   *     org.openmrs.api.db.ProgramWorkflowDAO#saveConceptStateConversion(org.openmrs.ConceptStateConversion)
   */
  @Override
  ...
  public ConceptStateConversion getConceptStateConversion(Integer conceptStateConversionId) {
    return sessionFactory
        .getCurrentSession()
        .get(ConceptStateConversion.class, conceptStateConversionId);
  }

  /**
   * @see
   *     org.openmrs.api.db.ProgramWorkflowDAO#deleteConceptStateConversion(org.openmrs.ConceptStateConversion)
   */
  @Override
  ...
  /**
   * @see
   *     org.openmrs.api.db.ProgramWorkflowDAO#getConceptStateConversion(org.openmrs.ProgramWorkflow,
   *     org.openmrs.Concept)
   */
  @Override
  ...
  /**
   * @see
   *     org.openmrs.api.db.ProgramWorkflowDAO#getProgramWorkflowStatesByConcept(org.openmrs.Concept)
   */
  @Override
  ...
  public Map<Object, Object> getPatientProgramAttributeByAttributeName(
      List<Integer> patientIds, String attributeName) {
    ...
    List<Object> list =
        sessionFactory
            .getCurrentSession()
            .createNativeQuery(
                "SELECT p.patient_id as person_id,  concat('{',group_concat(DISTINCT"
                    + " (coalesce(concat('\"',ppt.name,'\":\"', COALESCE (cn.name,"
                    + " ppa.value_reference),'\"'))) SEPARATOR ','),'}') AS"
                    + " patientProgramAttributeValue   from patient p  join patient_program pp on"
                    + " p.patient_id = pp.patient_id and p.patient_id in ("
                    + commaSeperatedPatientIds
                    + ") join patient_program_attribute ppa on pp.patient_program_id ="
                    + " ppa.patient_program_id and ppa.voided=0 join program_attribute_type ppt on"
                    + " ppa.attribute_type_id = ppt.program_attribute_type_id and ppt.name ='"
                    + attributeName
                    + "'  LEFT OUTER JOIN concept_name cn on ppa.value_reference = cn.concept_id"
                    + " and cn.concept_name_type= 'FULLY_SPECIFIED' and cn.voided=0 and"
                    + " ppt.datatype like '%ConceptDataType%' group by p.patient_id",
                Object.class)
