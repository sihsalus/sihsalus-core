...
public final class Daemon {
  ...
  public static Module startModule(
      final Module module,
      final boolean isOpenmrsStartup,
      final AbstractRefreshableApplicationContext applicationContext)
      throws ModuleException {
    var possibleFrame =
        STACK_WALKER.walk(
            s -> s.skip(1).limit(1).map(StackWalker.StackFrame::getDeclaringClass).findFirst());
    ...
    Future<Module> moduleStartFuture =
        runInDaemonThreadInternal(
  ...
  /**
   * This method should not be called directly, only {@link ContextDAO#createUser(User, String,
   * List)} can legally invoke this method.
   *
   * <p><strong>Should</strong> only allow the creation of new users, not the edition of existing
   * ones
   *
   ...
   * @since 2.3.0
   */
  public static User createUser(User user, String password, List<String> roleNames)
      throws Exception {
        ...
        runInDaemonThreadInternal(
            () -> {
              if ((user.getId() != null && Context.getUserService().getUser(user.getId()) != null)
                  ...
                  || (user.getEmail() != null
                      && Context.getUserService().getUserByUsernameOrEmail(user.getEmail())
                          != null)) {
                throw new APIException(
                    "User.creating.already.exists", new Object[] {user.getDisplayString()});
              }

              if (!CollectionUtils.isEmpty(roleNames)) {
                List<Role> roles =
                    roleNames.stream()
                        .map(roleName -> Context.getUserService().getRole(roleName))
  ...
  public static Thread runInNewDaemonThread(final Runnable runnable) {
    ...
    // the previous implementation ensured that Thread.start() was called before this function
    // returned
    // since we cannot guarantee that the executor will run the thread when `execute()` is called,
    // we need another
    // mechanism to ensure the submitted Runnable was actually started.
    final CountDownLatch countDownLatch = new CountDownLatch(1);

    // we should consider making DaemonThread public, so the caller can access returnedObject and
    // exceptionThrown
    DaemonThread thread =
        new DaemonThread() {
          ...
          public void run() {
            ...
            try {
              ...
              // We intentionally do not start a new thread yet, rather wrap the run call in a
              // session.
              runnable.run();
            } finally {
              try {
  ...
  public static boolean isDaemonThread() {
    ...
    if (b == null || !b) {
      ...
      var possibleFrame =
          STACK_WALKER.walk(
              s -> s.skip(2).limit(1).map(StackWalker.StackFrame::getDeclaringClass).findFirst());

      if (possibleFrame.isEmpty()) {
        throw new APIException("Could not determine where isDaemonThread() was called from");
      } else {
        var callerClass = possibleFrame.get();
        return Daemon.class.equals(callerClass)
            || "org.openmrs.web.WebDaemon".equals(callerClass.getName());
      }
    } else {
      return true;
    }
    ...
    try {
      ...
      if (e.getCause() instanceof ModuleException) {
        ...
        throw new ModuleException(
            "Unable to run onStartup() method of service {}",
  ...
  public static Future<?> runInDaemonThreadWithoutResult(
      final Runnable runnable, DaemonToken token) {
    if (!ModuleFactory.isTokenValid(token)) {
      throw new ContextAuthenticationException("Invalid token");
    }
  ...
  private static Future<?> runInDaemonThreadInternal(Runnable runnable) {
    ...
    // since we cannot guarantee that the executor actually started executing the thread, we use a
    // CountDownLatch
   ...
   *
   * <p><strong>Should</strong> not be called from other methods other than JobRequestHandlerAdapter
   ...
   * @since 2.9.x
   */
  public static void executeScheduledTaskAsUser(String userSystemId, DaemonTask runnable)
      throws Exception {
    var possibleFrame =
        STACK_WALKER.walk(
            s -> s.skip(1).limit(1).map(StackWalker.StackFrame::getDeclaringClass).findFirst());
    ...
    if (possibleFrame.isEmpty()) {
      throw new APIException(
          "Could not determine where executeScheduledTaskAsUser was called from");
    } else {
      var callerClass = possibleFrame.get();
      if (!JobRequestHandlerAdapter.class.isAssignableFrom(callerClass)) {
        throw new APIException(
            "executeScheduledTaskAsUser can only be called from JobRequestHandlerAdapter",
