 ...
 *
 * <ul>
 *   <li>A Cohort is a list of patient ids.
 *   <li>A CohortDefinition is a search strategy which can be used to arrive at a cohort. Therefore,
 *       the patients returned by running a CohortDefinition can be different depending on the data
 *       that is stored elsewhere in the database.
 * </ul>
 *
...
public interface CohortService extends OpenmrsService {
   ...
   *
   * <p><strong>Should</strong> create new cohorts<br>
   ...
   *
   * <p><strong>Should</strong> fail if reason is null<br>
   * <strong>Should</strong> fail if reason is empty<br>
   * <strong>Should</strong> void cohort<br>
   ...
   *
   * <p><strong>Should</strong> delete cohort from database
   *
   ...
   *
   * <p><strong>Should</strong> get cohort by id
   *
   * @param id
   ...
   *
   * <p><strong>Should</strong> get cohort given a name<br>
   * <strong>Should</strong> get the nonvoided cohort if two exist with same name<br>
   ...
   *
   * <p><strong>Should</strong> get all nonvoided cohorts in database<br>
   ...
   *
   * <p><strong>Should</strong> return all cohorts and voided
   *
   ...
   *
   * <p><strong>Should</strong> never return null<br>
  ...
  /**
   * <strong>Should</strong> not return voided cohorts<br>
   * <strong>Should</strong> return cohorts that have given patient
   *
   * @deprecated use {@link #getCohortsContainingPatientId(Integer)} Find all Cohorts that contain
   *     the given patient. (Not including voided Cohorts)
  ...
  /**
   * Find all Cohorts that contain the given patientId right now. (Not including voided Cohorts, or
   * ended memberships)
   *
   ...
   *
   * <p><strong>Should</strong> add a patient and save the cohort<br>
   * <strong>Should</strong> add a patient and insert the cohort to database<br>
  ...
  /**
   * Removes a patient from a Cohort, by voiding their membership. (Has no effect if the patient is
   * not in the cohort.) (This behavior is provided for consistency with the pre-2.1.0 API, which
   * didn't track cohort membership dates.)
   *
   * <p><strong>Should</strong> not fail if cohort doesn't contain patient<br>
   * <strong>Should</strong> save cohort after removing patient
   *
   * @deprecated since 2.1.0 you should explicitly call either {@link
   *     #endCohortMembership(CohortMembership, Date)} or {@link
   *     #voidCohortMembership(CohortMembership, String)}
  ...
  /**
   * NOTE: CLIENT CODE SHOULD NEVER CALL THIS METHOD. TREAT THIS AS AN INTERNAL METHOD WHICH MAY
   * CHANGE WITHOUT WARNING. Used to notify this service that a patient has been voided, and
   * therefore we should void all cohort memberships that refer to that patient
   *
   * <p><strong>Should</strong> void the membership for the patient that was passed in
   *
   * @since 2.1.0
  ...
  /**
   * NOTE: CLIENT CODE SHOULD NEVER CALL THIS METHOD. TREAT THIS AS AN INTERNAL METHOD WHICH MAY
   * CHANGE WITHOUT WARNING. Used to notify this service that a patient has been unvoided, and
   * therefore we should unvoid all cohort memberships that were automatically voided with the
   * patient
   *
   * <p><strong>Should</strong> unvoid the membership for the patient that was passed in
   *
   * @since 2.1.0
   ...
   */
  @Authorized({PrivilegeConstants.GET_PATIENT_COHORTS})
  List<CohortMembership> getCohortMemberships(
      Integer patientId, Date activeOnDate, boolean includeVoided);
}
