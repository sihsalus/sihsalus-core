...
public class NameSupport extends LayoutSupport<NameTemplate> implements GlobalPropertyListener {
  ...
  /**
   * Initializes layout templates with a custom template configured via the "layout.name.template"
   * GP.
   */
  private void init() {
    if (initialized) {
  ...
  private void updateLayoutTemplates(NameTemplate nameTemplate) {
    ...
    list.addAll(
        getLayoutTemplates().stream()
            .filter(
                existingTemplate -> existingTemplate.getCodeName() != nameTemplate.getCodeName())
