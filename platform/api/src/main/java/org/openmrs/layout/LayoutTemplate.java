...
public abstract class LayoutTemplate {
  ...
  /**
   * Very crude way of setting just one line of template. This just puts something on {@link
   * #setLineByLineFormat(List)} with this string
   *
    ...
    if (line != null && nonTokens != null && nonTokens.length > 0) {
      ...
      for (int i = 0; i < nonTokens.length; i++) {
        ...
        if (i < nonTokens.length - 1) {
          // this means we are still not at the last non-token, so let's add this non-token AND this
          // token
