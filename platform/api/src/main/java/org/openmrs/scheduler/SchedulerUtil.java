...
public class SchedulerUtil {
   ...
   * 04/01/2006 12:00am). If we scheduled the task using the JDK Timer scheduleAtFixedRate(TimerTask
   * task, Date startDate, int interval) method and passed in the start date above, the JDK Timer
   * would execute this task once for every day between the start date and today, which would lead
   * to hundreds of unnecessary (and likely expensive) executions.
   *
   * @see java.util.Timer
  ...
  public static Date getNextExecution(TaskDefinition taskDefinition) {
    ...
    try {
      ...
      if (firstTime != null) {
        ...
        // Calculate time between the first time the process was run and right now (e.g. 3 days, 15
        // hours)
