 ...
 *
 * <p>Since 2.9.x the default implementation switched to <a
 * href="https://github.com/jobrunr/jobrunr">JobRunr</a>. Please see its documentation <a
 * href="ttps://www.jobrunr.io/en/documentation/">here</a>.
 *
 * <p>The most significant change is that tasks are now run as a user who scheduled the task.
 *
 * <p><s>{@link TaskDefinition}</s> and <s>{@link Task}</s> are deprecated, but still run on the new
 ...
 *
 * <p>Tasks scheduled via SchedulerService are persisted in the database. They can be viewed by a
 * system administrator or presented to users.
 *
 * <p>For more lightweight tasks that are not persisted in the database, please use {@link
 * ScheduledWithLock}.
 */
public interface SchedulerService extends OpenmrsService {
   ...
   *
   * <p><strong>Should</strong> save task to the database
   *
   * @deprecated since 2.9.x use {@link #schedule(TaskData)}
   */
  @Deprecated
   ...
   *
   * <p>User can only get its own tasks unless has Manage Scheduler privilege.
   *
   * @param uuid
   * @return TaskDetails
   * @since 2.9.x
   ...
   *
   * <p>Please note that if a task runs, it won't be stopped, unless it implements the stop logic.
   * The deletion will prevent the task from running in the future.
   *
   * <p>User can only delete its own tasks unless has Manage Scheduler privilege.
   *
   * @param uuid
   * @since 2.9.x
   */
   ...
   *
   * <p>Please note that if a task runs, it won't be stopped. The deletion will prevent the task
   ...
   *
   * <p>It fetches tasks in batches of 100 by default.
   *
   * <p>User can only get its own tasks unless has Manage Scheduler privilege.
   *
   ...
   *
   * <p>In order to schedule a task, you need to implement {@link TaskData} and a corresponding
   * {@link TaskHandler}.
   *
   * <p>{@link TaskData} is persisted and then used by a corresponding {@link TaskHandler} to
   * process the data.
   *
   * <p>Please remember that for performance and consistency it is always better pass simple values
   * in {@link TaskData} (e.g. DB ids) that can be quickly serialized/deserialized and if needed
   * fetch objects from DB inside your {@link TaskHandler}.
   *
   * @param taskData
   * @return TaskDetails
   * @since 2.9.x
  ...
  /**
   * It schedules a one-off task with a user-friendly name, that will be put in queue and executed
   * in background.
   *
   * <p>See {@link #schedule(TaskData)} for more details.
   *
   * @param name
   * @param taskData
   * @return TaskDetails
  ...
  /**
   * It schedules a task with a user-friendly name to run once at the given time respecting time
   * zone.
   *
   * <p>See {@link #schedule(TaskData)} for more details.
   *
  ...
  /**
   * It schedules a task to run recurrently at the given cron schedule respecting the given time
   * zone.
   *
   * <p>See {@link #schedule(TaskData)} for more details.
   *
   ...
   */
  @Authorized
  RecurringTaskDetails scheduleRecurrently(
      String name, TaskData taskData, String cron, ZoneId zoneId);
