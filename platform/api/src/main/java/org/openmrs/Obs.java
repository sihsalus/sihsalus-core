 ...
 * <br>
 *
 * <p>The {@link #getObsGroup()} method returns an optional parent. That parent object is also an
 * Obs. The parent Obs object knows about its child objects through the {@link #getGroupMembers()}
 * method.
 *
 * <p>(Multi-level hierarchies are achieved by an Obs parent object being a member of another Obs
 ...
 *
 * <p>For example, a site may decide to replace a concept in the dictionary with a more specific set
 * of concepts. An observation is part of the official record of an encounter. There may be legal,
 * ethical, and auditing consequences from altering a record. It is recommended that you create a
 * new Obs and void the old one: Obs newObs = Obs.newInstance(oldObs); //copies values from oldObs
...
public class Obs extends BaseFormRecordableOpenmrsData {
   ...
   * obs; logic is that while a obs that has only voided group members should be seen as "having no
   * group members" it still should be considered an "obs grouping"
   *
   * <p>NOTE: This method could also be called "isObsGroup" for a little less confusion on names.
   * However, jstl in a web layer (or any psuedo-getter) access isn't good with both an "isObsGroup"
   * method and a "getObsGroup" method. Which one should be returned with a simplified jstl call
   * like ${obs.obsGroup} ? With this setup, ${obs.obsGrouping} returns a boolean of whether this
   * obs is a parent and has members. ${obs.obsGroup} returns the parent object to this obs if this
   * obs is a group member of some other group.
   *
   ...
   * members. By default, this ignores voided-objects. To include voided, use {@link
   * #hasGroupMembers(boolean)} with value true.
   *
   * <p><strong>Should</strong> not include voided obs
   *
   * @return true if this is the parent group of other obs
   */
   ...
   * members. The parameter specifies if this method whether or not voided obs should be considered.
   *
   * <p><strong>Should</strong> return true if this obs has group members based on parameter
   *
   ...
   * only returns non-voided group members. To get all group members, use {@link
   * #getGroupMembers(boolean)} with value true.
   *
   * <p>If it's not a group (i.e. {@link #getConcept()}.{@link org.openmrs.Concept#getSet()} is not
  ...
  /**
   * Get the group members of this obs group, if this obs is a group. This method will either return
   * all group members, or only non-voided group members, depending on if the argument is set to be
   * true or false respectively.
   *
   * <p><strong>Should</strong> Get all group members if passed true, and non-voided if passed false
   *
   * @param includeVoided
   ...
   * true, then this returns null.
   *
   * <p><strong>Should</strong> mark the obs as dirty when the set is changed from null to a non
   * empty one<br>
   * <strong>Should</strong> not mark the obs as dirty when the set is changed from null to an empty
   * one<br>
   * <strong>Should</strong> mark the obs as dirty when the set is replaced with another with
   * different members<br>
   * <strong>Should</strong> not mark the obs as dirty when the set is replaced with another with
   * same members
   *
   ...
   *
   * <p><strong>Should</strong> return true when a new obs is added as a member<br>
  ...
  public void addGroupMember(Obs member) {
    ...
    if (member.equals(this)) {
      throw new APIException(
          "Obs.error.groupCannotHaveItselfAsAMentor", new Object[] {this, member});
    }
  ...
  /**
   * Convenience method to remove an Obs from this grouping This also removes the link in the given
   * <code>obs</code>object to this obs grouper
   *
   * <p><strong>Should</strong> return true when an obs is removed<br>
   ...
   * ObsGroups themselves. This will include this Obs by default, unless getObsGroup() returns null,
   * in which case an empty set is returned. If the Obs argument is an ObsGroup: a Set&lt;Obs&gt;
   * will be returned containing 1. all of this Obs' group members, and 2. all ancestor Obs that are
   * not themselves obsGroups.
   *
   * @return Set&lt;Obs&gt;
   */
  ...
  /**
   * The person id of the person on this object. This should be the same as <code>
   * {@link #getPerson()}.getPersonId()</code>. It is duplicated here for speed and simplicity
   * reasons
   *
   ...
   * {@link #setPerson(Person)} method should be used like <code>setPerson(new Person(personId))
   * </code>
   *
   ...
   *
   * <p><strong>Should</strong> return true for value_numeric concepts if value is 1<br>
   * <strong>Should</strong> return false for value_numeric concepts if value is 0<br>
   ...
   *
   * <p><strong>Should</strong> return true if value coded answer concept is true concept<br>
  ...
  /**
   * @return the value of this obs as a Date. Note that this uses a java.util.Date, so it includes a
   *     time component, that should be ignored.
   * @since 1.9
   */
  ...
  /**
   * @return the time value of this obs. Note that this uses a java.util.Date, so it includes a date
   *     component, that should be ignored.
   * @since 1.9
   */
   ...
   * valueComplex has two parts separated by a bar '|' character: part A) the title; and part B) the
   * URI. The title is the readable description of the valueComplex that is returned by {@link
   * Obs#getValueAsString(java.util.Locale)}. The URI is the location where the ComplexData is
   * stored.
   *
  ...
  /**
   * Get the ComplexData. This is retrieved by the {@link ComplexObsHandler} from the file system or
   * another location, not from the database. <br>
   * <br>
   ...
   * missing.
   *
   * <p><strong>Should</strong> return first part of valueComplex for complex obs<br>
   * <strong>Should</strong> return first part of valueComplex for non null valueComplexes<br>
   * <strong>Should</strong> return non precise values for NumericConcepts<br>
   * <strong>Should</strong> return date in correct format<br>
   * <strong>Should</strong> not return long decimal numbers as scientific notation<br>
   * <strong>Should</strong> use commas or decimal places depending on locale<br>
   * <strong>Should</strong> not use thousand separator<br>
  ...
  public String getValueAsString(Locale locale) {
    ...
    if (getConcept() != null) {
      ...
      if ("BIT".equals(abbrev)) {
        ...
        if (getValueNumeric() == null) {
          ...
          if (deproxiedConcept instanceof ConceptNumeric) {
            ...
            if (!cn.getAllowDecimal()) {
              return BigDecimal.valueOf(getValueNumeric())
        ...
        return (getValueDatetime() == null
            ? ""
            : Format.format(getValueDatetime(), locale, FORMAT_TYPE.TIME));
      } else if ("TS".equals(abbrev)) {
        return (getValueDatetime() == null
            ? ""
            : Format.format(getValueDatetime(), locale, FORMAT_TYPE.TIMESTAMP));
   ...
   *
   * <p><strong>Should</strong> set value as boolean if the datatype of the question concept is
   * boolean<br>
   * <strong>Should</strong> fail if the value of the string is null<br>
   ...
   * and adds a reference to the previousVersion in the new Obs. getPreviousVersion returns the last
   * version of this Obs.
   */
   ...
   * isDirty() is going to return false is when a new Obs has just been instantiated or loaded from
   * the database and no method that modifies it internally has been invoked.
   *
   * <p><strong>Should</strong> return false when no change has been made<br>
   * <strong>Should</strong> return true when any immutable field has been changed<br>
   * <strong>Should</strong> return false when only mutable fields are changed<br>
   * <strong>Should</strong> return true when an immutable field is changed from a null to a non
   * null value<br>
   * <strong>Should</strong> return true when an immutable field is changed from a non null to a
   * null value
   *
  ...
  /**
   * Similar to FHIR's Observation.status. Supports a subset of FHIR's ObservationStatus values. At
   * present OpenMRS does not support FHIR's REGISTERED and CANCELLED statuses, because we don't
   * support obs with null values. See: https://www.hl7.org/fhir/valueset-observation-status.html
   *
   * @since 2.1.0
   */
