...
public interface O3FormsService extends OpenmrsService {
   ...
   *
   * <p>In O3 forms, a reference is an element like this:
   *
   * <p>
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>References the section labelled "Pre-Clinical Review" in the page labelled "Pre-Clinical
   * Review" in the form with the alias "pcr". After compilation, this element will be replaced with
   * the contents of the appropriate section.
   *
   ...
   *
   * @param compiledForm A {@link SimpleObject} that represents the compiled form, such as returned
   *     by {@link #compileFormSchema(Form)}
   * @return A simple object of key-value pairs where the keys are the concepts as referred to in
   *     the form and the values are values extracted from those concepts, using the default
   *     representation defined in {@link
   *     org.openmrs.module.o3forms.O3FormsConstants#DEFAULT_FORMAT}.
   */
   ...
   * @return A simple object of key-value pairs where the keys are the concepts as referred to in
   *     the form and the values are values extracted from those concepts, using the requested
   *     representation.
   */
  SimpleObject getConceptReferences(
   ...
   *
   * <p>For each form, only a single translation file will be created, the first to match the
   * appropriate locale as determined by {@link org.openmrs.util.LocaleUtility#getLocalesInOrder()}.
   * If multiple forms are referenced, the translations will be merged into a single JSON object,
   * with keys from the explicitly loaded form overwriting keys from referenced forms.
   *
   * @param formNameOrUuid The name or UUID of the form
