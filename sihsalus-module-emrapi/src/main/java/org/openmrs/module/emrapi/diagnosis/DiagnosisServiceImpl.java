...
public class DiagnosisServiceImpl extends BaseOpenmrsService implements DiagnosisService {
  ...
  protected boolean useDiagnosesAsObs() {
    return adminService
        .getGlobalProperty(EmrApiConstants.GP_USE_LEGACY_DIAGNOSIS_SERVICE, "false")
  ...
  public List<Obs> getDiagnosesAsObs(
    ...
    if (useDiagnosesAsObs()) {
      return obsGroupDiagnosisService.getDiagnosesAsObs(
          visit, diagnosisMetadata, primaryOnly, confirmedOnly);
    } else {
      return coreDiagnosisService.getDiagnosesAsObs(
          visit, diagnosisMetadata, primaryOnly, confirmedOnly);
    }
  }
}
