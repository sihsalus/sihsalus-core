...
public class EmrDiagnosisDAOImpl implements EmrDiagnosisDAO {
   ...
   * @param confirmedOnly whether to fetch only confirmed diagnosis or both confirmed and
   *     provisional
   * @return list of diagnoses for a visit
   */
  public List<org.openmrs.Diagnosis> getDiagnoses(
      Visit visit, boolean primaryOnly, boolean confirmedOnly) {
    String queryString =
        "from Diagnosis d where d.encounter.visit.visitId = :visitId and d.voided = false";
    if (primaryOnly) {
