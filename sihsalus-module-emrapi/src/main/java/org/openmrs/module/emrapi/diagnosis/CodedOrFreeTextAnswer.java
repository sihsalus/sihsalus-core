...
/**
 * Class representing a value that can be either coded (as a Concept or a more specific ConceptName)
 * or non-coded.
 */
public class CodedOrFreeTextAnswer {
  ...
  public CodedOrFreeTextAnswer(Obs codedOrNonCodedValue) {
    if (codedOrNonCodedValue.getValueCodedName() != null) {
      ...
      throw new IllegalArgumentException(
          "codedOrNonCodedValue must have one of valueCodedName, valueCoded, or valueText");
    }
  }
   ...
   */
  public CodedOrFreeTextAnswer(
      Concept codedAnswer, ConceptName specificCodedAnswer, String nonCodedAnswer) {
    this.nonCodedAnswer = nonCodedAnswer;
    this.codedAnswer = codedAnswer;
  }
  ...
  public int hashCode() {
    return new HashCodeBuilder(17, 37)
        .append(codedAnswer)
        .append(specificCodedAnswer)
        .append(nonCodedAnswer)
        .toHashCode();
  }
   ...
   * <ul>
   *   <li>non-coded value
   *   <li>coded value's preferred name in the current locale
   * </ul>
   *
   ...
   *   <li>\"non-coded value\"
   *   <li>coded value's preferred name in the current locale
   *   <li>specific coded value → coded value's preferred name in the current locale
   * </ul>
   *
   * @param locale
   * @return
  ...
  public String format(Locale locale) {
    if (nonCodedAnswer != null) {
      ...
      if (specificCodedAnswer.isLocalePreferred()
          && specificCodedAnswer.getLocale().equals(locale)) {
  ...
  /**
   * Like {link #format(Locale)} but if this is a concept, and is mapped to a term in the given
   * fromSources, append that term's code
   *
   * @param locale
    ...
    for (ConceptMap candidate : concept.getConceptMappings()) {
      if (fromSources.contains(candidate.getConceptReferenceTerm().getConceptSource())) {
        if (candidate
            .getConceptMapType()
            .getUuid()
            .equals(EmrApiConstants.SAME_AS_CONCEPT_MAP_TYPE_UUID)) {
          return candidate.getConceptReferenceTerm();
        } else if (candidate
            .getConceptMapType()
            .getUuid()
            .equals(EmrApiConstants.NARROWER_THAN_CONCEPT_MAP_TYPE_UUID)) {
