...
public interface DispositionService {
  ...
  /**
   * Fetch a copy of the Disposition Descriptor, which describes the concepts necessary (and
   * optional, like admission location, transfer location, and date of death) for recording a
  ...
  /**
   * Gets only dispositions that are valid in the specified visit context Currently, logic works as
   * follows: 1) if visit.isActive() = false, then return all dispositions, else 2) if
   * visit.isAdmitted() = true, then return only those whore careSettingTypes contains INPATIENT (or
   * any where careSettingType = null) 3) if visit.isAdmitted() = false, then return only those
   * whore careSettingTypes contains OUTPATIENT (or any where careSettingType = null)
   */
  List<Disposition> getValidDispositions(VisitDomainWrapper visitDomainWrapper);

  /**
   * Gets only dispositions that are valid in the specified visit and encounter context Currently,
   * this follows the logic of {@link #getValidDispositions(VisitDomainWrapper)}, but also filters
   * dispositions which define an encounterType to only those encounterTypes.
   */
  List<Disposition> getValidDispositions(
      VisitDomainWrapper visitDomainWrapper, EncounterType encounterType);
