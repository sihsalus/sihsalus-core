 ...
 * However if the EmrApiConstants#GP_VISIT_ASSIGNMENT_HANDLER_ENCOUNTER_TYPE_TO_VISIT_TYPE_MAP
 * property that provides a mapping between encounter types and visit types is set then a new visit
 * is created using the visit type from the saved mapping
 */
public class EmrApiVisitAssignmentHandler extends BaseEncounterVisitHandler implements EncounterVisitHandler {
	...
	public void beforeCreateEncounter(Encounter encounter) {
		...
		if ("true".equalsIgnoreCase(
		    administrationService.getGlobalProperty(EmrApiConstants.GP_DISABLE_VISIT_ASSIGMENT_HANDLER))) {
			return;
		}
				...
				if (emrApiProperties.getVisitAssignmentHandlerAdjustEncounterTimeOfDayIfNecessary()) {
					if (adtService.isSuitableVisitIgnoringTime(candidate, encounter.getLocation(), when)) {
						if (when.before(candidate.getStartDatetime())) {
							updateDateActivatedOfOrdersIfNecessary(encounter.getOrders(), when,
	 ...
	 * encounter's datetime, then if the encounter's datetime is changed, the order dateActivated values
	...
	public void setEncounterTypetoVisitTypeMapper(EncounterTypetoVisitTypeMapper encounterTypetoVisitTypeMapper) {
