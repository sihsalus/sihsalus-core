...
/**
 * Will actually discharge the patient from inpatient care (using {@link
 * org.openmrs.module.emrapi.adt.AdtService#admitPatient(org.openmrs.module.emrapi.adt.Admission)})
 * if they are currently admitted. This may not be the recommended workflow -- you might expect a
 * disposition of Discharge to be an order for discharge, not the official discharge itself, which
 * might be done by a specific employee at the inpatient ward where the patient is currently
 * admitted. However in a hacky shortcut workflow you can use this action to have a Discharge
 * disposition immediately discharge a patient.
 */
...
public class DischargeIfAdmittedDispositionAction implements DispositionAction {
  ...
  public void action(
    ...
    // datetime, but we have to test against visitDomainWrapper.isAdmitted()
    // TODO for now because the "createAdtEncounterFor" method will throw an exception if
    // isAdmitted() returns false; see
    // https://minglehosting.thoughtworks.com/unicef/projects/pih_mirebalais/cards/938
    if (visitDomainWrapper.isAdmitted()) {
      // if
      // (visitDomainWrapper.isAdmitted(encounterDomainWrapper.getEncounter().getEncounterDatetime())) {
      AdtAction discharge =
          new AdtAction(
              visit,
              encounterDomainWrapper.getLocation(),
