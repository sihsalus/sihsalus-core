...
public class ProcedureValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    ...
    if (!(target instanceof Procedure)) {
      ...
      if (procedure.getProcedureCoded() != null
          && StringUtils.isNotBlank(procedure.getProcedureNonCoded())) {
      ...
      if (procedure.getProcedureId() == null) {
        ...
        if (procedure.getEstimatedStartDate() != null && procedure.getStartDateTime() != null) {
          errors.reject(
              "Procedure.error.startDateTimeAndEstimatedDateMutuallyExclusiveForNewProcedures");
        }
      }

      if (errors.hasErrors()) {
        log.warn(
            "Validation failed for procedure {}: {}", procedure.getUuid(), errors.getAllErrors());
      }
    }
  }
}
