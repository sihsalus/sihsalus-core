...
public abstract class ConceptSetDescriptor {
   ...
   * @param memberConceptFields Fields for member concepts of primary concept. These concepts can be
   *     mandatory or optional.
   */
  protected void setup(
      ConceptService conceptService,
      String conceptSourceName,
      ConceptSetDescriptorField primaryConceptField,
      ConceptSetDescriptorField... memberConceptFields) {
    try {
      String primaryConceptCode = primaryConceptField.getConceptCode();
      Concept primaryConcept =
          conceptService.getConceptByMapping(primaryConceptCode, conceptSourceName);
      if (primaryConcept == null) {
        throw new MissingConceptException(
            "Couldn't find primary concept for "
                + getClass().getSimpleName()
                + " which should be mapped as "
                + conceptSourceName
                + ":"
                + primaryConceptCode);
      }
          ...
          if (!primaryConcept.getSetMembers().contains(childConcept)) {
            throw new IllegalStateException(
                "Concept mapped as "
                    ...
                    + ":"
                    + mappingCode
                    + " needs to be a set member of concept "
                    + primaryConcept.getConceptId()
                    + " which is mapped as "
                    + conceptSourceName
                    + ":"
                    + primaryConceptCode);
  ...
  protected void setCodedOrFreeTextMember(
      Obs obsGroup,
      CodedOrFreeTextAnswer answer,
      Concept questionIfCoded,
    ...
    if (answer.getNonCodedAnswer() != null) {
      ...
      setCodedMember(
          obsGroup, questionIfCoded, answer.getCodedAnswer(), answer.getSpecificCodedAnswer());
    }
  }
  ...
  private void setFreeTextMember(Obs obsGroup, Concept memberConcept, String memberAnswer) {
    ...
    boolean needToVoid =
        member != null && !OpenmrsUtil.nullSafeEquals(memberAnswer, member.getValueText());
  ...
  /**
   * @return text to put in the voidReason field of an obs that gets voided by an implementation of
   *     this class.
   */
  ...
  protected void setCodedMember(
      Obs obsGroup, Concept memberConcept, Concept memberAnswer, ConceptName specificAnswer) {
    Obs member = findMember(obsGroup, memberConcept);
    boolean needToVoid =
        member != null
  ...
  protected Obs buildObsFor(
      CodedOrFreeTextAnswer codedOrFreeTextAnswer,
      Concept questionIfCoded,
      Concept questionIfNonCoded) {
    Obs obs = new Obs();
  ...
  protected Concept findAnswer(Concept concept, String conceptSource, String codeForAnswer) {
    ...
    throw new IllegalStateException(
        "Cannot find answer mapped with "
            + conceptSource
            + ":"
            + codeForAnswer
            + " in the concept "
