...
public class DefaultOrderMapper implements OrderMapper {
  ...
  private void sortByOrderNumber(List<EncounterTransaction.DrugOrder> orders) {
    Collections.sort(
        orders,
        new Comparator<EncounterTransaction.DrugOrder>() {
  ...
  public EncounterTransaction.DrugOrder mapDrugOrder(DrugOrder openMRSDrugOrder) {
    ...
    if (openMRSDrugOrder.getCareSetting() != null) {
      drugOrder.setCareSetting(
          CareSetting.CareSettingType.valueOf(
    ...
    if (openMRSDrugOrder.getDrug() != null) {
      EncounterTransaction.Drug encounterTransactionDrug =
          new DefaultDrugMapper().map(openMRSDrugOrder.getDrug());
    ...
    if (openMRSOrderGroup != null) {
      EncounterTransaction.OrderGroup orderGroup =
          new EncounterTransaction.OrderGroup(openMRSOrderGroup.getUuid());
      EncounterTransaction.OrderSet orderSet =
          new EncounterTransaction.OrderSet(openMRSOrderGroup.getOrderSet().getUuid());
