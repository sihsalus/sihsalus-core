...
public class AccountServiceImpl extends BaseOpenmrsService implements AccountService {
  ...
  /**
   * @see
   *     org.openmrs.module.emrapi.account.AccountService#getAccountsByCriteria(AccountSearchCriteria)
   */
  public AccountSearchResult getAccountsByCriteria(AccountSearchCriteria criteria) {
    ...
    if (StringUtils.isNotBlank(criteria.getNameOrIdentifier())) {
      ...
      providers =
          emrApiDAO.executeHqlFromResource("hql/provider_search.hql", searchParams, Provider.class);
      log.debug(
          "Retrieved {} providers with search {}",
          providers.size(),
          criteria.getNameOrIdentifier());
    } else {
    ...
    for (User user : users) {
      if (user.getPerson() == null) {
        log.warn(
            "Users not associated to a person are not supported.  Excluding {}", user.getUuid());
      } else {
    ...
    for (Provider provider : providers) {
      if (provider.getPerson() == null) {
        log.warn(
            "Providers not associated to a person are not supported.  Excluding {}",
            provider.getUuid());
      } else {
    ...
    for (Provider provider : providers) {
      ...
      if (Boolean.TRUE.equals(criteria.getHasProviderRole())
          && provider.getProviderRole() == null) {
        persons.remove(provider.getPerson());
      }
      if (Boolean.FALSE.equals(criteria.getHasProviderRole())
          && provider.getProviderRole() != null) {
        persons.remove(provider.getPerson());
      }
    ...
    sortedPersons.sort(
        (o1, o2) ->
            o1.getPersonName().getFullName().compareToIgnoreCase(o2.getPersonName().getFullName()));
    ...
    int endIndex =
        criteria.getLimit() == null
            ? sortedPersons.size()
            : Math.min(startIndex + criteria.getLimit(), persons.size());
    sortedPersons = sortedPersons.subList(startIndex, endIndex);
    log.debug(
        "Limited results to those from {} to {}, total results: {}",
        startIndex,
        endIndex,
        sortedPersons.size());
  ...
  /**
   * @see
   *     org.openmrs.module.emrapi.account.AccountService#saveAccount(org.openmrs.module.emrapi.account.AccountDomainWrapper)
   */
  @Override
  @Transactional
