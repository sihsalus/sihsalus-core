...
/**
 * Wrapper around a Visit, that provides convenience methods to find particular encounters of
 * interest.
 */
public class VisitDomainWrapper implements DomainWrapper {
  ...
  @Autowired protected AdtService adtService;
  ...
  @Deprecated // use new VisitDomainWrapperFactory instead to instantiate a visit domain wrapper
  public VisitDomainWrapper(
      Visit visit, EmrApiProperties emrApiProperties, DispositionService dispositionService) {
    this.visit = visit;
    this.emrApiProperties = emrApiProperties;
  ...
  public Encounter getAdmissionEncounter() {
    return (Encounter)
        find(
            getSortedEncounters(SortOrder.MOST_RECENT_FIRST),
            new EncounterTypePredicate(emrApiProperties.getAdmissionEncounterType()));
  }
  ...
  public Encounter getMostRecentCheckInEncounter() {
    return (Encounter)
        find(
            getSortedEncounters(SortOrder.MOST_RECENT_FIRST),
            new EncounterTypePredicate(emrApiProperties.getCheckInEncounterType()));
  }
  ...
  public Encounter getEarliestCheckInEncounter() {
    return (Encounter)
        find(
            getSortedEncounters(SortOrder.EARLIEST_FIRST),
            new EncounterTypePredicate(emrApiProperties.getCheckInEncounterType()));
  }
  ...
  public Encounter getMostRecentEncounter() {
    ...
    if (encounters.size() > 0) return encounters.get(0);
    return null;
  }
  ...
  public Encounter getEarliestEncounter() {
    List<Encounter> encounters = getSortedEncounters(SortOrder.MOST_RECENT_FIRST);
    if (encounters.size() != 0) return encounters.get(encounters.size() - 1);
    return null;
  }
  ...
  public Encounter getMostRecentVisitNote() {
    return (Encounter)
        find(
            getSortedEncounters(SortOrder.MOST_RECENT_FIRST),
            new EncounterTypePredicate(emrApiProperties.getVisitNoteEncounterType()));
  }

  /**
   * Fetches the most recent (non-voided) visit note encounter from this visit at the specified
   * location
   *
   * @return
   */
  public Encounter getMostRecentVisitNoteAtLocation(Location location) {
    return (Encounter)
        find(
            getSortedEncounters(SortOrder.MOST_RECENT_FIRST),
            new EncounterTypeAndLocationPredicate(
                emrApiProperties.getVisitNoteEncounterType(), location));
  }
  ...
  /**
   * True/false whether there is a non-voided visit encounter associated with this visit at the
   * specified location
   *
   * @return
   */
  ...
  public List<Encounter> getSortedEncounters(SortOrder order) {
    ...
    if (visit.getEncounters() != null) {
      List<Encounter> nonVoidedEncounters =
          (List<Encounter>)
              select(visit.getEncounters(), EncounterDomainWrapper.NON_VOIDED_PREDICATE);
  ...
  // also, if encounter has multiple disposition (is this possible?) it just returns the first one
  // it finds
  public Disposition getMostRecentDisposition() {

    if (dispositionService
        .dispositionsSupported()) { // prevents against stace trace if dispositions are supported
  ...
  /**
   * Finds the most recent encounter in the visit with a disposition of a specific type, and
   * retrieves all diagnoses stored on that encounter. Can be used to prepopulate an admission,
   * transfer, or discharge note; example workflow: a doctor writes a visit note and on that note
   * sets a disposition of admission; later on, another doctor writes the actual admission note;
   * that doctor may want a way to prepopulate this note with the dispositions from the visit note
   * recommending admission; see the EncounterDisposition HFE tag in CoreApps for an example of how
   * this method is used
   */
  public List<Diagnosis> getDiagnosesFromMostRecentDispositionByType(DispositionType type) {

    if (dispositionService
        .dispositionsSupported()) { // prevents against stack trace if dispositions not configured
      DispositionDescriptor dispositionDescriptor = dispositionService.getDispositionDescriptor();

      for (Encounter encounter :
          getSortedEncounters(
              SortOrder
                  .MOST_RECENT_FIRST)) { // getSortedEncounters already excludes voided encounters
        for (Obs obs : encounter.getObsAtTopLevel(false)) {
          if (dispositionDescriptor.isDisposition(obs)
  ...
  public List<Diagnosis> getPrimaryDiagnoses() {
    ...
    for (Encounter encounter : getSortedEncounters(SortOrder.MOST_RECENT_FIRST)) {
      diagnoses.addAll(
          getDiagnosesFromEncounter(encounter, Collections.singletonList(Diagnosis.Order.PRIMARY)));
    }
    return diagnoses;
  }
   ...
   *     whether the CodedOrNonCoded diagnosis is the same, not on whether the order or certainty
   *     are the same if the primaryOnly flag is true, return only primary diagnoses if the
   *     confirmedOnly flag is true, return only confirmed diagnoses will return null if diagnosis
   *     support not currently configured
   */
  public List<Diagnosis> getUniqueDiagnoses(Boolean primaryOnly, Boolean confirmedOnly) {
    ...
    try {
      ...
      // this isn't a hard error, because some implementations will not be using diagnoses
      // functionality
    ...
    List<Obs> obsList =
        diagnosisService.getDiagnosesAsObs(
            getVisit(), diagnosisMetadata, primaryOnly, confirmedOnly);

    Map<CodedOrFreeTextAnswer, Diagnosis> diagnoses =
        new LinkedHashMap<CodedOrFreeTextAnswer, Diagnosis>();
    for (Obs obs : obsList) {
  ...
  private List<Diagnosis> getDiagnosesFromEncounter(
      Encounter encounter, List<Diagnosis.Order> diagnosisOrders) {
  ...
  public boolean hasEncounterWithoutSubsequentEncounter(
      EncounterType lookForEncounterType, EncounterType withoutSubsequentEncounterType) {
    return hasEncounterWithoutSubsequentEncounter(
        lookForEncounterType, withoutSubsequentEncounterType, null);
  }

  private boolean hasEncounterWithoutSubsequentEncounter(
      EncounterType lookForEncounterType,
      EncounterType withoutSubsequentEncounterType,
      Date onDate) {
  ...
  public boolean isAdmitted(Date onDate) {
    ...
    if (visit.getStartDatetime().after(onDate)
        || (visit.getStopDatetime() != null && visit.getStopDatetime().before(onDate))) {
    ...
    return hasEncounterWithoutSubsequentEncounter(
        admissionEncounterType, dischargeEncounterType, onDate);
  }
  ...
  public boolean isAwaitingAdmission() {
    ...
    if (!isActive()
        || !dispositionService
            .dispositionsSupported()) { // prevents a stack trace if dispositions are supported
      return false;
    }
    List<Integer> visitIds = Collections.singletonList(getVisitId());
    List<Visit> matches =
        adtService.getVisitsAwaitingAdmission(visit.getLocation(), null, visitIds);
  ...
  public Location getInpatientLocation(Date onDate) {
    ...
    for (Encounter encounter : getSortedEncounters(SortOrder.MOST_RECENT_FIRST)) {
      if (onDate == null
          || encounter.getEncounterDatetime().before(onDate)
          || encounter.getEncounterDatetime().equals(onDate)) {
  ...
  public Integer getTimeSinceAdmissionInMinutes() {
    if (!isAdmitted() || getAdmissionEncounter() == null) {
      ...
      return Minutes.minutesBetween(
              new DateTime(getAdmissionEncounter().getEncounterDatetime()), new DateTime())
          .getMinutes();
    }
  }
  ...
  public Integer getTimeAtCurrentInpatientLocationInMinutes() {
    ...
    for (Encounter encounter : getSortedEncounters(SortOrder.MOST_RECENT_FIRST)) {
      if (encounter.getEncounterType().equals(admissionEncounterType)
        ...
        time =
            Minutes.minutesBetween(new DateTime(encounter.getEncounterDatetime()), new DateTime())
                .getMinutes();
      }
    }
    return time;
  ...
  public Date getStartDate() {
    return visit.getStartDatetime() != null
        ? new DateMidnight(visit.getStartDatetime()).toDate()
        : null;
  }

  public Date getStopDate() {
    return visit.getStopDatetime() != null
        ? new DateMidnight(visit.getStopDatetime()).toDate()
        : null;
  }
  ...
  /**
   * Throws an {@link IllegalArgumentException} if checkDatetime is not within the start/stop date
   * bounds of this visit
   *
   * @param checkDatetime
   * @param errorMessage base of the error message to throw (some details may be added)
   */
  public void errorIfOutsideVisit(Date checkDatetime, String errorMessage)
      throws IllegalArgumentException {
    if (visit.getStartDatetime() != null
        && OpenmrsUtil.compare(checkDatetime, visit.getStartDatetime()) < 0) {
      throw new IllegalArgumentException(
          errorMessage
              + ": visit started at "
              + visit.getStartDatetime()
              + " but testing an earlier date");
    }
    if (visit.getStopDatetime() != null
        && OpenmrsUtil.compare(visit.getStopDatetime(), checkDatetime) < 0) {
      throw new IllegalArgumentException(
          errorMessage
              + ": visit stopped at "
              + visit.getStopDatetime()
              + " but testing a later date");
    }
  }
