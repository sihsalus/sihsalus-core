...
public class HibernateEmrPatientDAO implements EmrPatientDAO {
	...
	public List<Visit> getVisitsForPatient(Patient patient, Integer startIndex, Integer limit) {
		jakarta.persistence.Query query = sessionFactory.getCurrentSession()
		        .createQuery("from Visit visit where visit.patient = :patient and visit.voided = false order by visit.startDatetime desc");
	...
	public List<Obs> getVisitNoteObservations(Collection<Visit> visits) {
		...
		jakarta.persistence.Query query = sessionFactory.getCurrentSession().createQuery(
