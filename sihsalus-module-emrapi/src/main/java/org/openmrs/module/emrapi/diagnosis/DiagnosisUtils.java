...
public class DiagnosisUtils {
  ...
  /**
   * Method to convert the core diagnosis object into a list of diagnoses compatible with the
   * diagnosis object in the emrapi module
   *
   * @return diagnoses
   */
  ...
  public static Diagnosis convert(org.openmrs.Diagnosis coreDiagnosis) {
    ...
    if (coded != null) {
      diagnosis.setDiagnosis(
          new CodedOrFreeTextAnswer(
              coded.getCoded(), coded.getSpecificName(), coded.getNonCoded()));
    }
    diagnosis.setCertainty(
        coreDiagnosis.getCertainty() == ConditionVerificationStatus.CONFIRMED
            ? Diagnosis.Certainty.CONFIRMED
            : Diagnosis.Certainty.PRESUMED);
    diagnosis.setOrder(
        coreDiagnosis.getRank() == 1 ? Diagnosis.Order.PRIMARY : Diagnosis.Order.SECONDARY);
    return diagnosis;
  }
