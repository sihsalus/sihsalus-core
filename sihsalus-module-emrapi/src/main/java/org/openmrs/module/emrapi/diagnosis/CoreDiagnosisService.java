...
public class CoreDiagnosisService {
  ...
  public List<Diagnosis> getUniqueDiagnoses(Patient patient, Date fromDate) {
    return DiagnosisUtils.convert(
        Context.getDiagnosisService().getUniqueDiagnoses(patient, fromDate));
  }
  ...
  public boolean hasDiagnosis(Encounter encounter, Diagnosis diagnosis) {
    ...
    for (org.openmrs.Diagnosis existingDiagnosis :
        Context.getDiagnosisService().getDiagnosesByEncounter(encounter, false, false)) {
      CodedOrFreeTextAnswer existingAnswer =
          DiagnosisUtils.convert(existingDiagnosis).getDiagnosis();
  ...
  public List<Obs> getDiagnosesAsObs(
    ...
    List<org.openmrs.Diagnosis> diagnoses =
        emrDiagnosisDAO.getDiagnoses(visit, primaryOnly, confirmedOnly);
  ...
  public Map<Visit, List<org.openmrs.Diagnosis>> getDiagnoses(Collection<Visit> visits) {
    ...
    List<org.openmrs.Diagnosis> diagnoses =
        emrApiDAO.executeHql(query, parameters, org.openmrs.Diagnosis.class);
