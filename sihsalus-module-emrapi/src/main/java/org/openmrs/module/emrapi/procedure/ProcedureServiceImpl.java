...
public class ProcedureServiceImpl extends BaseOpenmrsService implements ProcedureService {
  ...
  @Authorized(PrivilegeConstants.GET_PROCEDURES)
  @Transactional(readOnly = true)
  public List<Procedure> getProceduresByPatient(
      Patient patient, boolean includeAll, Integer firstResult, Integer maxResults) {
    log.debug(
        "Getting procedures for patient: {}, includeAll: {}, firstResult: {}, maxResults: {}",
        patient,
        includeAll,
        firstResult,
        maxResults);
  ...
  public Procedure saveProcedure(Procedure procedure) throws APIException {
    ...
    if (procedure.getEstimatedStartDate() != null) {
      log.debug(
          "Calculating startDateTime from estimatedStartDate: {}",
          procedure.getEstimatedStartDate());
      Date calculatedStartDateTime =
          ProcedureUtil.getDateTimeFromEstimatedDate(procedure.getEstimatedStartDate());
