...
@Handler(
    supports = {AccountDomainWrapper.class},
    order = 50)
public class AccountValidator implements Validator {
  ...
  public void validate(Object obj, Errors errors) {
    if (obj == null || !(obj instanceof AccountDomainWrapper))
      throw new IllegalArgumentException(
          "The parameter obj should not be null and must be of type" + AccountDomainWrapper.class);
    ...
    // TODO: we are (probably just temporarily) insisting that all accounts have an associated
    // provider
    ...
    if (checkIfUserWasCreated(user)
        ...
        || StringUtils.isNotBlank(account.getConfirmPassword())) {
  ...
  private void checkIfPrivilegeLevelIsCorrect(Errors errors, AccountDomainWrapper account) {
    if (account.getPrivilegeLevel() == null) {
      errors.rejectValue(
          "privilegeLevel",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.user.privilegeLevel")},
          null);
    }
  }
  ...
  private void checkIfPasswordIsCorrect(Errors errors, AccountDomainWrapper account) {
    ...
    if (checkIfPasswordWasCreated(password, confirmPassword)) {
      ...
      errors.rejectValue(
          "password",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.user.password")},
          null);
      errors.rejectValue(
          "confirmPassword",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.user.confirmPassword")},
          null);
    }
  }

  private void validatePassword(
      Errors errors, AccountDomainWrapper account, String password, String confirmPassword) {
    if (password.equals(confirmPassword)) {
      getErrorInPassword(errors, account);
    } else {
      errors.rejectValue(
          "password",
          "emr.account.error.passwordDontMatch",
          new Object[] {messageSourceService.getMessage("emr.user.password")},
          null);
    }
  }
  ...
  private void getErrorInPassword(Errors errors, AccountDomainWrapper account) {
    try {
      OpenmrsUtil.validatePassword(
          account.getUsername(), account.getPassword(), account.getUser().getSystemId());
    } catch (PasswordException e) {
      errors.rejectValue(
          "password",
          "emr.account.error.passwordError",
          new Object[] {messageSourceService.getMessage("emr.account.error.passwordError")},
          null);
    }
  }
  ...
  private void checkIfUserNameIsCorrect(Errors errors, String username) {
    if (StringUtils.isNotBlank(username)) {
      if (!username.matches(
        ...
        errors.rejectValue(
            "username",
            "emr.user.username.error",
            new Object[] {messageSourceService.getMessage("emr.user.username.error")},
            null);
      }
      ...
      errors.rejectValue(
          "username",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.user.username")},
          null);
    }
  }
  ...
  private void checkIfDuplicateUsername(Errors errors, User user) {
    if (userService.hasDuplicateUsername(user)) {
      errors.rejectValue(
          "username",
          "emr.user.duplicateUsername",
          new Object[] {messageSourceService.getMessage("emr.user.duplicateUsername")},
          null);
    }
  }
  ...
  private void checkIfGivenAndFamilyNameAreNotNull(Errors errors, AccountDomainWrapper account) {
    if (StringUtils.isBlank(account.getGivenName())) {
      errors.rejectValue(
          "givenName",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.person.givenName")},
          null);
    }
    if (StringUtils.isBlank(account.getFamilyName())) {
      errors.rejectValue(
          "familyName",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.person.familyName")},
          null);
    }
  }
  ...
  private void checkIfGenderIsNull(Errors errors, AccountDomainWrapper account) {
    if (StringUtils.isBlank(account.getGender())) {
      errors.rejectValue(
          "gender",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.gender")},
          null);
    }
  }
  ...
  private void checkIfNoCapabilities(Errors errors, AccountDomainWrapper account) {
    if (account.getCapabilities() == null || account.getCapabilities().isEmpty()) {
      errors.rejectValue(
          "capabilities",
          "emr.user.Capabilities.required",
          new Object[] {messageSourceService.getMessage("emr.user.Capabilities.required")},
          null);
    }
  }

  private void checkIfUserAndProviderRoleAreNull(
      Errors errors, AccountDomainWrapper accountDomainWrapper) {
  ...
  private void checkIfProviderRoleIsNull(Errors errors, AccountDomainWrapper accountDomainWrapper) {
    if (accountDomainWrapper.getProviderRole() == null) {
      errors.rejectValue(
          "providerRole",
          "error.required",
          new Object[] {messageSourceService.getMessage("emr.account.providerRole.label")},
          null);
    }
  }
