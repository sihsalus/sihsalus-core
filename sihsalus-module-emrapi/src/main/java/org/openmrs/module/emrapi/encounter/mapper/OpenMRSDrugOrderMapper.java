...
public class OpenMRSDrugOrderMapper {
  ...
  public DrugOrder map(EncounterTransaction.DrugOrder drugOrder, Encounter encounter) {
    ...
    openMRSDrugOrder.setUrgency(
        drugOrder.getScheduledDate() != null ? Order.Urgency.ON_SCHEDULED_DATE : DEFAULT_URGENCY);
    ...
    Concept durationUnitsConcept =
        orderMetadataService.getDurationUnitsConceptByName(drugOrder.getDurationUnits());
    if (durationUnitsConcept != null) {
      openMRSDrugOrder.setDurationUnits(durationUnitsConcept);
    } else {
      List<Concept> conceptList =
          conceptService.getConceptsByName(
              drugOrder.getDurationUnits(), LocaleUtility.getDefaultLocale(), null);
      ...
      if (drugOrder.getDosingInstructionType() != null) {
        openMRSDrugOrder.setDosingType(
            (Class<? extends DosingInstructions>)
                Context.loadClass(drugOrder.getDosingInstructionType()));
      }
    } catch (ClassNotFoundException e) {
      throw new APIException(
          "Class not found for : DosingInstructionType " + drugOrder.getDosingInstructionType(), e);
    }
  ...
  private boolean isDiscontinuationDrugOrder(EncounterTransaction.DrugOrder drugOrder) {
    return drugOrder.getAction() != null
        && Order.Action.valueOf(drugOrder.getAction()) == Order.Action.DISCONTINUE;
  }
  ...
  private DrugOrder createDrugOrder(EncounterTransaction.DrugOrder drugOrder) {
    if (isNewDrugOrder(drugOrder)) {
      ...
      return (DrugOrder)
          orderService.getOrderByUuid(drugOrder.getPreviousOrderUuid()).cloneForDiscontinuing();
    } else {
      return (DrugOrder)
          orderService.getOrderByUuid(drugOrder.getPreviousOrderUuid()).cloneForRevision();
    }
  }

  private CareSetting getCareSettingFrom(
      EncounterTransaction.DrugOrder drugOrder, DrugOrder openMRSDrugOrder) {
    if (!isNewDrugOrder(drugOrder)) {
