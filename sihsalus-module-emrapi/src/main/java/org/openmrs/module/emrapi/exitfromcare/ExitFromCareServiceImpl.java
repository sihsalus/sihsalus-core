...
public class ExitFromCareServiceImpl extends BaseOpenmrsService implements ExitFromCareService {
    ...
    for (PatientProgram patientProgram :
      ...
      if (patientProgram.getActive()
          && programWorkflowService
              .getPossibleOutcomes(patientProgram.getProgram().getProgramId())
              .contains(outcome)) {
        patientProgram.setOutcome(outcome);
        patientProgram.setDateCompleted(
            completionDate.after(patientProgram.getDateEnrolled())
                ? completionDate
                : patientProgram
                    .getDateEnrolled()); // if completion date is before date enrolled, just use
        // completion date
  ...
  @Override
  public void updatePatientProgramCompletionDate(
      Patient patient, Concept outcome, Date completionDate) {
    for (PatientProgram patientProgram :
        programWorkflowService.getPatientPrograms(patient, null, null, null, null, null, false)) {
  ...
  public void markPatientDead(Patient patient, Concept causeOfDeath, Date deathDate) {
    ...
    // we intentionally bypass the PatientService exitFromCare and processDeath methods as we are
    ...
    if (patientDied != null) {
      // update any programs that had previously been closed with outcome=patient died with matching
      // death date
