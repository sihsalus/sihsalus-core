...
public class EncounterDomainWrapper implements DomainWrapper {
  ...
  public static final Predicate NON_VOIDED_PREDICATE =
      new Predicate() {
  ...
  public static final Comparator<Encounter> DATETIME_COMPARATOR =
      new Comparator<Encounter>() {

        @Override
        public int compare(Encounter encounter, Encounter encounter2) {
          return DateTimeComparator.getInstance()
              .compare(encounter.getEncounterDatetime(), encounter2.getEncounterDatetime());
        }
      };
  ...
  /**
   * Associates the encounter with the specified visit If the encounterDatetime has only a Date
   * component, adds a time component (if necessary) based on our business logic: if this is an open
   * visit and encounter date = today, consider a real-time transaction and we stamp with the
   * current time, otherwise we add a time component (if necessary) to make sure the encounter falls
   * within the specified visit
   *
   * @param visit
  ...
  public void attachToVisit(VisitDomainWrapper visit)
    ...
    // if time component already exists, for now we just verify that the encounter falls within the
    // visit
    ...
    // otherwise, properly set the time component
      ...
      DateMidnight visitStopDate =
          visit.getStopDatetime() != null ? new DateMidnight(visit.getStopDatetime()) : null;
      ...
      // if encounter date = today and open visit, consider this a real-time transaction and
      ...
      // otherwise, consider a retrospective encounter, and so we want an encounter date to have no
      // time component, EXCEPT
      // if this encounter is on the first day of the visit, the encounter datetime cannot be before
  ...
  /**
   * Sets encounter.encounterDatetime, and if any obs in encounter.allObs had an obsDatetime equal
   * to the original encounterDatetime, those obsDatetimes are also set. (I.e. this will change the
   * obsDatetime of obs who "inherit" datetime from the encounter, but not touch obs who have an
   * different obsDatetime.
   *
   * @param encounter
   * @param datetime
   */
  ...
  /**
   * If an encounter has not been persisted yet, we cannot be sure that Encounter.getAllObs() will
   * actually return all obs (because some obs group members may not yet have been attached to the
   * encounter by the service), so we use this helper method to ensure we recursively get all obs in
   * an encounter
   *
   * @param encounter
   * @return
