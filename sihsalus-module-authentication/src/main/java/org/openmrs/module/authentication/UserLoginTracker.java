...
/**
 * This class maintains a UserLogins that are tracked across the application. The primary purpose is
 * to maintain a collection of active UserLogin instances that represent the currently logged-in
 * users, and to maintain a ThreadLocal of UserLogin instances to enable tracking a particular
 * UserLogin throughout the lifetime of a thread
 */
public class UserLoginTracker {

  private static final ThreadLocal<UserLogin> threadLogins = new ThreadLocal<>();
  private static final Map<String, UserLogin> activeLogins =
      Collections.synchronizedMap(new LinkedHashMap<>());

  /**
   * This method should be called in order to register the given UserLogin on the current thread To
   * guard against memory leaks, this should be paired with remove()
   *
  ...
  /**
   * This method should be called in order to remove the given UserLogin from the set of logged-in
   * users To guard against memory leaks, this should pair with add(UserLogin)
   *
  ...
  /**
   * @return a Collection of UserLogins, defined as those that have been logged in and not logged
   *     out or expired The Map returned is keyed on the loginId of the UserLogin
   */
