...
public class WeeklyRecurringAppointmentDate {
  ...
  static List<Pair<Date, Date>> getAppointmentDates(
      Date endDate,
    ...
    while (!currentAppointmentDate.after(endDate)) {
      appointmentDates.addAll(
          getAppointmentDatesForAWeek(endDate, startCalendar, endCalendar, selectedDayCodes));
  ...
  public static Date getEndDate(RecurringPattern recurringPattern, Date startDateTime) {
    ...
    List<Integer> selectedDaysPerWeek =
        getSortedSelectedDayCodes(
            calendar.get(DAY_OF_WEEK), getSelectedDayCodes(recurringPattern.getDaysOfWeek()));
  ...
  private static void moveCalendarToRemainingFrequency(
      Integer remainingFrequency, Calendar calendar, List<Integer> selectedDaysPerWeek) {
    final Integer dayCodeOfLastOccurrence =
        getDayCodeOfLastOccurrence(remainingFrequency, selectedDaysPerWeek);
    while (calendar.get(DAY_OF_WEEK) != dayCodeOfLastOccurrence) {
      calendar.add(DATE, 1);
    }
  }

  private static Integer getDayCodeOfLastOccurrence(
      Integer currentFrequency, List<Integer> selectedDaysPerWeek) {
  ...
  static List<Integer> getSelectedDayCodes(List<String> selectedDays) {
    return selectedDays.stream()
        .map(WeeklyRecurringAppointmentDate::getDayCodeFromDay)
        .sorted()
        .collect(Collectors.toList());
  }

  static List<Pair<Date, Date>> getNewAppointmentDates(
      List<Appointment> activeAppointments, List<Pair<Date, Date>> appointmentStartAndEndDates) {
        ...
        activeAppointment -> {
          for (Pair<Date, Date> appointmentStartAndEndDate : appointmentStartAndEndDates) {
            ...
            if (relatedAppointment != null
                && areAppointmentDatesSame(relatedAppointment, appointmentStartAndEndDate)) {
  ...
  private static boolean areAppointmentDatesSame(
      Appointment appointment, Pair<Date, Date> appointmentStartAndEndDate) {
    return new Date(appointment.getStartDateTime().getTime())
            .equals(appointmentStartAndEndDate.getLeft())
        && new Date(appointment.getEndDateTime().getTime())
            .equals(appointmentStartAndEndDate.getRight());
  }
  ...
  private static List<Pair<Date, Date>> getAppointmentDatesForAWeek(
      Date endDate, Calendar startCalendar, Calendar endCalendar, List<Integer> selectedDayCodes) {
    List<Pair<Date, Date>> appointmentDates = new ArrayList<>();
    for (int dayCode : selectedDayCodes) {
      ...
      appointmentDates.add(
          new ImmutablePair<>(startCalenderInstance.getTime(), endCalenderInstance.getTime()));
    }
    return appointmentDates;
  }

  private static void updateCalendarInstancesToNextSelectedDay(
      Calendar startCalender, Calendar endCalender, int dayCode) {
    int startDayCode = startCalender.get(DAY_OF_WEEK);
    int daysToBeAdded = dayCode - startDayCode;
    if (dayCode < startDayCode) daysToBeAdded += DAY_OF_WEEK;
    startCalender.add(DAY_OF_WEEK, daysToBeAdded);
    endCalender.add(DAY_OF_WEEK, daysToBeAdded);
  }

  private static List<Integer> getSortedSelectedDayCodes(
      int startDayCode, List<Integer> selectedDayCodes) {
  ...
  static Appointment getFirstOriginalAppointmentInThePattern(
      List<Appointment> activeAppointments, List<Appointment> removedAppointments) {
    ...
    activeAndRemovedAppointments.forEach(
        appointment -> {
          final Appointment relatedAppointment = appointment.getRelatedAppointment();
          originalAppointmentsInThePattern.add(
              relatedAppointment == null ? appointment : relatedAppointment);
        });

    originalAppointmentsInThePattern.sort(
        Comparator.comparing(Appointment::getDateFromStartDateTime));
