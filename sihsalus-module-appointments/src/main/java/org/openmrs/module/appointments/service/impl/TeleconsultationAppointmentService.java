...
public class TeleconsultationAppointmentService {
  ...
  private static final String CREATE_TELECONSULTATION_PRIVILEGE = "Create Teleconsultation";
  ...
  public void setPatientAppointmentNotifierService(
  ...
  public AdhocTeleconsultationResponse generateAdhocTeleconsultationLink(
      String patientUuid, String provider) {
    ...
    if (StringUtils.isNotBlank(identifierType)) {
      identifier =
          patient.getIdentifiers().stream()
              .filter(pi -> identifierType.equals(pi.getIdentifierType().getName()))
              .findAny()
              .orElse(null);
    }
    String teleConsultationId =
        (identifier != null) ? identifier.getIdentifier() : generateRandomID();
  ...
  private void notifyUpdates(
      AdhocTeleconsultationResponse response, Patient patient, String provider, String link) {
    List<NotificationResult> notificationResults =
        patientAppointmentNotifierService.notifyAll(patient, provider, link);
