...
public class WeeklyRecurringAppointmentsGenerationService
  ...
  public List<Appointment> generateAppointments(
    ...
    List<Pair<Date, Date>> appointmentDates =
        getAppointmentDates(
            ...
            selectedDayCodes,
            recurringAppointmentRequest.getRecurringPattern().getPeriod());
  ...
  public List<Appointment> addAppointments(
    ...
    if (appointmentRecurringPattern.getEndDate() == null) {
      appointmentRecurringPattern.setFrequency(
          recurringPattern.getFrequency() - appointmentRecurringPattern.getFrequency() + 1);
    ...
    final Appointment originalFirstAppointmentInThePattern =
        getFirstOriginalAppointmentInThePattern(
            activeAppointments,
    ...
    List<String> daysOfWeek =
        stream(appointmentRecurringPattern.getDaysOfWeek().split(",")).collect(toList());
    List<Integer> selectedDayCodes = getSelectedDayCodes(daysOfWeek);

    final List<Pair<Date, Date>> appointmentStartAndEndDates =
        getAppointmentDates(
            endDate, startDateTime, endDateTime, selectedDayCodes, recurringPattern.getPeriod());
    ...
    final List<Appointment> newAppointments =
        createAppointments(
            newAppointmentDates, recurringAppointmentRequest.getAppointmentRequest());
