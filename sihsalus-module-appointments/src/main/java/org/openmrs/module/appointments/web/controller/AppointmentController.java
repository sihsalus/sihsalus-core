...
public class AppointmentController extends BaseRestController {
  ...
  @ResponseBody
  public List<AppointmentDefaultResponse> searchAppointments(
      @Valid @RequestBody AppointmentSearchRequestModel searchQuery) throws IOException {
    if (searchQuery.isWithoutDates()) {
      List<Appointment> appointmentsWithoutDates =
  ...
  public ResponseEntity<Object> saveAppointment(
    ...
    try {
      /**
       * The above code has been done so because to make appointment save within a transaction
       * boundary. calling appointmentMapper starts a transaction and persistent object is modified,
       * which when appointmentService.validateAndSave() is called was being written to DB, as
       * calling validateAndSave() will start another transaction, and before that the dirty
       * persistent entity will be flushed to DB. appointmentMapper should be fixed and
       * validateAndSave() signature should be changed.
       */
      Appointment appointment =
          appointmentsService.validateAndSave(
  ...
  public List<AppointmentDefaultResponse> getAllFututreAppointmentsForGivenServiceType(
    ...
    AppointmentServiceType appointmentServiceType =
        appointmentServiceDefinitionService.getAppointmentServiceTypeByUuid(serviceTypeUuid);
    List<Appointment> appointments =
  ...
  public List<AppointmentsSummary> getAllAppointmentsSummary(
    ...
    for (AppointmentServiceDefinition appointmentServiceDefinition :
      ...
      for (Map.Entry<Date, List<Appointment>> appointmentDateMap :
        ...
        Long missedAppointmentsCount =
            appointments.stream()
                ...
                .count();
        DailyAppointmentServiceSummary dailyAppointmentServiceSummary =
            new DailyAppointmentServiceSummary(
                appointmentDateMap.getKey(),
                appointmentServiceDefinition.getUuid(),
                appointments.size(),
                Math.toIntExact(missedAppointmentsCount));
        appointmentCountMap.put(
            simpleDateFormat.format(appointmentDateMap.getKey()), dailyAppointmentServiceSummary);
      }

      AppointmentsSummary appointmentsSummary =
          new AppointmentsSummary(
  ...
  @ResponseBody
  public ResponseEntity<Object> undoStatusChange(
      @PathVariable("appointmentUuid") String appointmentUuid) throws ParseException {
    try {
  ...
  @ResponseBody
  public AppointmentDefaultResponse getAppointmentByUuid(
  ...
  public ResponseEntity<Object> updateAppointmentProviderResponse(
      ...
      @RequestBody AppointmentProviderDetail providerResponse)
      throws ParseException {
    try {
      Appointment appointment = appointmentsService.getAppointmentByUuid(appointmentUuid);
      if (appointment == null) {
      ...
      AppointmentProvider appointmentProviderProvider =
          appointmentMapper.mapAppointmentProvider(providerResponse);
  ...
  public ResponseEntity<Object> rescheduleAppointment(
      @PathVariable("uuid") String prevAppointmentUuid,
      @Valid @RequestBody AppointmentRequest appointmentRequest,
      @RequestParam(value = "retainNumber", required = false, defaultValue = "false")
          boolean retainAppointmentNumber)
      throws ParseException {
    try {
      appointmentRequest.setUuid(null);
      Appointment appointment = appointmentMapper.fromRequest(appointmentRequest);
      Appointment rescheduledAppointment =
          appointmentsService.reschedule(prevAppointmentUuid, appointment, retainAppointmentNumber);
      return new ResponseEntity<>(
          appointmentMapper.constructResponse(rescheduledAppointment), HttpStatus.OK);
    } catch (APIAuthenticationException | ContextAuthenticationException e) {
      return new ResponseEntity<>(
          RestUtil.wrapErrorResponse(e, e.getMessage()), HttpStatus.FORBIDDEN);
    } catch (RuntimeException e) {
