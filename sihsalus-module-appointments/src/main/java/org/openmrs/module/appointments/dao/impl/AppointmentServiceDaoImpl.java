...
public class AppointmentServiceDaoImpl implements AppointmentServiceDao {
  ...
  public List<AppointmentServiceDefinition> getAllAppointmentServices(boolean includeVoided) {
    String hql =
        "select asd from AppointmentServiceDefinition asd"
  ...
  public List<AppointmentServiceDefinition> search(AppointmentServiceSearchParams searchParams) {
    ...
    CriteriaQuery<AppointmentServiceDefinition> criteriaQuery =
        criteriaBuilder.createQuery(AppointmentServiceDefinition.class);
    Root<AppointmentServiceDefinition> root =
        criteriaQuery.from(AppointmentServiceDefinition.class);
    ...
    if (StringUtils.isNotBlank(searchParams.getLocationUuid())) {
      ...
      predicates.add(
          criteriaBuilder.equal(locationJoin.get("uuid"), searchParams.getLocationUuid()));
    }
    ...
    if (StringUtils.isNotBlank(searchParams.getSpecialityUuid())) {
      ...
      predicates.add(
          criteriaBuilder.equal(specialityJoin.get("uuid"), searchParams.getSpecialityUuid()));
    }
    ...
    TypedQuery<AppointmentServiceDefinition> query =
  ...
  private void evictObjectFromSession(
      Session currentSession, AppointmentServiceDefinition appointmentServiceDefinition) {
