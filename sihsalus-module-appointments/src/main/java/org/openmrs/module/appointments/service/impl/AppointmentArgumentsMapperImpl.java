...
public class AppointmentArgumentsMapperImpl implements AppointmentArgumentsMapper {
  ...
  @Override
  public Map<String, String> createArgumentsMapForRecurringAppointmentBooking(
      Appointment appointment) {
    Map<String, String> arguments = createArgumentsMapForAppointmentBooking(appointment);
    arguments.put(
        "recurringperiod", appointment.getAppointmentRecurringPattern().getPeriod().toString());
    arguments.put(
        "recurringtype", appointment.getAppointmentRecurringPattern().getType().toString());
    String daysOfWeek =
        appointment.getAppointmentRecurringPattern().getDaysOfWeek() != null
            ? appointment.getAppointmentRecurringPattern().getDaysOfWeek()
            : "";
    arguments.put("recurringdays", daysOfWeek);
    if (appointment.getAppointmentRecurringPattern().getFrequency() != null)
      arguments.put(
          "recurringfrequency",
          appointment.getAppointmentRecurringPattern().getFrequency().toString());
  ...
  public Map<String, String> createArgumentsMapForAppointmentBooking(Appointment appointment) {
    ...
    Patient patient =
        Context.getPatientService().getPatientByUuid(appointment.getPatient().getUuid());
    ...
    String helpdeskNumber =
        Context.getAdministrationService()
            .getGlobalPropertyObject("clinic.helpDeskNumber")
            .getPropertyValue();
    ...
    if (location == null) {
      ...
      List<Location> locations =
          Context.getLocationService()
              .getLocationsHavingAnyTag(Collections.singletonList(visitLocationTag));
      return (visitLocationTag != null && !locations.isEmpty())
  ...
  private Location getParentVisitLocationUuid(Location location) {
    if (isVisitLocation(location)) {
      return location.getParentLocation() != null
          ? getParentVisitLocationUuid(location.getParentLocation())
          : location;
    } else {
      return location.getParentLocation() != null
          ? getParentVisitLocationUuid(location.getParentLocation())
  ...
  private Boolean isVisitLocation(Location location) {
    return (location.getTags().size() > 0
        && location.getTags().stream()
                .filter(tag -> tag.getName().equalsIgnoreCase("Visit Location"))
                .count()
            != 0);
  }
}
