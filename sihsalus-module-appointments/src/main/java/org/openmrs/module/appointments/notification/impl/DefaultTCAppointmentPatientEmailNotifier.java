...
public class DefaultTCAppointmentPatientEmailNotifier implements AppointmentEventNotifier {
  private static final String PROP_PATIENT_EMAIL_SUBJECT =
      "bahmni.appointment.teleConsultation.patientEmailNotificationSubject";
  private static final String PROP_PATIENT_EMAIL_TEMPLATE =
      "bahmni.appointment.teleConsultation.patientEmailNotificationTemplate";
  private static final String PROP_SEND_TC_APPT_EMAIL =
      "bahmni.appointment.teleConsultation.sendEmail";

  private static final String PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_SUBJECT =
      "bahmni.appointment.adhocTeleConsultation.patientEmailNotificationSubject";
  private static final String PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_TEMPLATE =
      "bahmni.appointment.adhocTeleConsultation.patientEmailNotificationTemplate";
  private static final String PROP_ADHOC_TELECONSULTATION_BCC_EMAILS =
      "bahmni.appointment.adhocTeleConsultation.bccEmails";

  private static final String EMAIL_NOT_CONFIGURED =
      "Notification can not be sent to patient. Email address not configured.";
  ...
  private static final String EMAIL_NOT_SENT =
      "Email notification for tele-consultation not configured to be sent to patient.";
  ...
  @Override
  public NotificationResult sendNotification(final Appointment appointment)
    ...
    if (patientEmailAttribute != null) {
      ...
      return new NotificationResult(
          null, "EMAIL", NotificationResult.GENERAL_ERROR, EMAIL_NOT_CONFIGURED);
    }
  }

  public NotificationResult sendNotification(
      final Patient patient, final String provider, final String link)
      throws NotificationException {
    PersonAttribute patientEmailAttribute = patient.getPerson().getAttribute("email");
    if (patientEmailAttribute == null) {
      log.warn(EMAIL_NOT_CONFIGURED);
  ...
  private boolean shouldSendEmailToPatient() {
    String shouldSendEmail =
        Context.getAdministrationService().getGlobalProperty(PROP_SEND_TC_APPT_EMAIL, "false");
  ...
  private String getEmailBody(
    ...
    String emailTemplate =
        Context.getAdministrationService().getGlobalProperty(PROP_PATIENT_EMAIL_TEMPLATE);
    ...
    if (emailTemplate == null || emailTemplate.isEmpty()) {
      return Context.getMessageSourceService()
          .getMessage(PROP_PATIENT_EMAIL_TEMPLATE, arguments, LocaleUtility.getDefaultLocale());
    } else {
  ...
  private String getEmailSubject() {
    String emailSubject =
        Context.getAdministrationService().getGlobalProperty(PROP_PATIENT_EMAIL_SUBJECT);
    if (emailSubject == null || emailSubject.isEmpty()) {
      emailSubject =
          Context.getMessageSourceService()
              .getMessage(PROP_PATIENT_EMAIL_SUBJECT, null, LocaleUtility.getDefaultLocale());
    }
    return emailSubject;
  }
  ...
  private String getAdhocTeleconsultationEmailSubject() {
    String emailSubject =
        Context.getAdministrationService()
            .getGlobalProperty(PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_SUBJECT);
    if (StringUtils.isBlank(emailSubject)) {
      emailSubject =
          Context.getMessageSourceService()
              .getMessage(
                  PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_SUBJECT,
                  null,
  ...
  private String getAdhocTeleconsultationEmailBody(
      String patientName, String provider, String link) {
    String emailTemplate =
        Context.getAdministrationService()
            .getGlobalProperty(PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_TEMPLATE);
    Object[] arguments = {patientName, provider, link};
    if (StringUtils.isBlank(emailTemplate)) {
      return Context.getMessageSourceService()
          .getMessage(
              PROP_ADHOC_TELECONSULTATION_PATIENT_EMAIL_TEMPLATE,
              arguments,
              LocaleUtility.getDefaultLocale());
    } else {
      return new MessageFormat(emailTemplate).format(arguments);
  ...
  private String getAdhocTeleconsultationBCCEmails() {
    String bccEmails =
        Context.getAdministrationService()
            .getGlobalProperty(PROP_ADHOC_TELECONSULTATION_BCC_EMAILS);
    if (bccEmails == null || bccEmails.isEmpty()) {
      bccEmails =
          Context.getMessageSourceService()
              .getMessage(
                  PROP_ADHOC_TELECONSULTATION_BCC_EMAILS, null, LocaleUtility.getDefaultLocale());
    }
    return bccEmails;
  }
