...
public class MarkAppointmentAsMissedTask extends AbstractTask {
  ...
  public void execute() {
    ...
    GlobalProperty schedulerMarksMissedProperty =
        administrationService.getGlobalPropertyObject("SchedulerMarksMissed");
    Boolean schedulerMarksMissed = Boolean.valueOf(schedulerMarksMissedProperty.getPropertyValue());
    if (!schedulerMarksMissed) {
      return;
    }
    GlobalProperty schedulerMarksCompleteProperty =
        administrationService.getGlobalPropertyObject("SchedulerMarksComplete");
    Boolean schedulerMarksComplete =
        Boolean.valueOf(schedulerMarksCompleteProperty.getPropertyValue());
    Date today = new Date();
    List<Appointment> appointments = appointmentsService.getAllAppointmentsInDateRange(null, today);
    List<Appointment> scheduledAndCheckedInAppointments =
        appointments.stream()
            .filter(
                appointment ->
                    isAppointmentScheduled(appointment) || isAppointmentCheckedIn(appointment))
            .collect(Collectors.toList());
    for (Appointment appointment : scheduledAndCheckedInAppointments) {
      String status = AppointmentStatus.Missed.toString();
      if ((!schedulerMarksComplete && appointment.getStatus().equals(AppointmentStatus.CheckedIn))
