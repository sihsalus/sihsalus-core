...
public class RecurringAppointmentEventsAdvice implements AfterReturningAdvice, MethodBeforeAdvice {
    ...
    if (adviceMethodNames.contains(method.getName())) {
      ...
      if (appointmentInfo != null) {
        AppointmentEventType eventType =
            appointmentInfo.get(RECURRING_APPOINTMENT_ID_KEY) == null
                ? BAHMNI_RECURRING_APPOINTMENT_CREATED
                : BAHMNI_RECURRING_APPOINTMENT_UPDATED;
        threadLocal.remove();
        AppointmentRecurringPattern appointmentRecurringPattern =
            (AppointmentRecurringPattern) returnValue;
        RecurringAppointmentEvent recurringAppointmentEvent =
            new RecurringAppointmentEvent(eventType, appointmentRecurringPattern);
  ...
  public void before(Method method, Object[] objects, Object o) {
    if (adviceMethodNames.contains(method.getName())) {
      if (objects == null
          || objects.length == 0
          || !(objects[0] instanceof AppointmentRecurringPattern)) {
        return;
      }
      AppointmentRecurringPattern appointmentRecurringPattern =
          (AppointmentRecurringPattern) objects[0];
