...
public class ConceptHandler extends CodedHandler {
  ...
  protected void setDefaults(WidgetConfig config) {
    ...
    if (defaultValue != null
        && StringUtils.isNotEmpty(defaultValue.toString())
        && !(defaultValue instanceof Concept)) {
      ...
      try {
        ...
        convertedDefaultValue =
            Context.getConceptService().getConcept(Integer.parseInt(defaultValue.toString()));
      } catch (Exception e) {
        // And if that fails, then by uuid
        convertedDefaultValue =
            Context.getConceptService().getConceptByUuid(defaultValue.toString());
      }
      if (convertedDefaultValue != null) {
        defaultValue = convertedDefaultValue;
      } else {
        throw new IllegalArgumentException(
            "Default value of " + defaultValue + " is not able to be converted to a Concept");
      }
    }
  }
