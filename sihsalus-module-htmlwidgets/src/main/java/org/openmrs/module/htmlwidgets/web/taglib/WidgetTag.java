 ...
 */
package org.openmrs.module.htmlwidgets.web.taglib;
...
public class WidgetTag extends TagSupport {
  ...
  private String genericTypes; // Comma separated generic types
  ...
  private String
      attributes; // Pipe-separated list of attributes to provide flexible control of Widgets as
  ...
  public int doStartTag() throws JspException {
    ...
    if (getObject() != null && getProperty() != null) {
      ...
      genericFieldTypes = ReflectionUtil.getGenericTypes(f);
      propertyValue = ReflectionUtil.getPropertyValue(object, property);

      if (fieldType == null) {
        throw new IllegalArgumentException(
            "Property <" + property + "> is invalid for object <" + object + ">");
      }
      if (getType() != null && getType() != fieldType.getName()) {
        throw new IllegalArgumentException(
            "The specified type is not compatible with the specified property.");
      }
    } else if (getType() != null) {
      try {
        fieldType = Context.loadClass(getType());
        if (StringUtils.hasText(getGenericTypes())) {
          List<Type> genericTypeList = new ArrayList<Type>();
          for (String s : getGenericTypes().split(",")) {
            genericTypeList.add(Context.loadClass(s));
          }
          genericFieldTypes = genericTypeList.toArray(new Type[genericTypeList.size()]);
        }
      } catch (Exception e) {
    ...
    try {
      ...
      log.warn("Error handling Widget: " + type, e);
