...
@Handler(
    supports = {ProgramWorkflowState.class},
    order = 50)
public class ProgramWorkflowStateHandler extends OpenmrsMetadataHandler<ProgramWorkflowState> {
    ...
    for (ProgramWorkflow w : workflows) {
      if (!w.isRetired() || includeRetired) {
        ...
        List<ProgramWorkflowState> states =
            new ArrayList<ProgramWorkflowState>(w.getStates(includeRetired));
  ...
  private class ProgramWorkflowComparator implements Comparator<ProgramWorkflow> {
    public int compare(ProgramWorkflow p1, ProgramWorkflow p2) {
      if (p1 != null && p2 != null) {
        String sn1 =
            (p1.getName() == null ? p1.getConcept().getDisplayString() : p1.getName())
                .toUpperCase();
        String sn2 =
            (p2.getName() == null ? p2.getConcept().getDisplayString() : p2.getName())
                .toUpperCase();
