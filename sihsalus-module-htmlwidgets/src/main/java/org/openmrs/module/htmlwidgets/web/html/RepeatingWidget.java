...
public class RepeatingWidget implements Widget {
  ...
  public void render(WidgetConfig config, Writer w) throws IOException {
    ...
    if (type == null) {
      ...
      String genericTypeDisplay =
          configGenericTypes == null ? null : Arrays.toString(configGenericTypes);
      throw new IllegalArgumentException(
          "Invalid CollectionHandler configuration for id="
              + configId
              + ", name="
              + configName
              + ", genericTypes="
    ...
    if (config.getDefaultValue() != null
        && StringUtils.isNotEmpty(config.getDefaultValue().toString())) {
      if (config.getDefaultValue() instanceof Collection) {
        ...
        throw new RuntimeException(
            "Default value for a repeating widget must be an Object[], Collection, or null");
      }
    }
    ...
    for (int i = 0; i <= valuesToRender.size(); i++) {
      ...
      HtmlUtil.renderSimpleTag(
          w,
          "input",
          "type=button|id=removeRowButton|value=X|size=1|"
              + removeStyle
              + "onclick=removeParentWithClass(this,'multiFieldInput');");
