...
public class ConceptSearchController {
  ...
  public void conceptSearch(
      ...
      @RequestParam(required = false, value = "questionConceptId") Concept questionConcept)
      throws Exception {
    ...
    List<ConceptSearchResult> results =
        Context.getConceptService()
            .getConcepts(
                query, l, false, includeClasses, null, null, null, questionConcept, null, null);
    for (Iterator<ConceptSearchResult> i = results.iterator(); i.hasNext(); ) {
      ...
      if (res.getConceptName().isPreferred()
        ...
        out.print(
            res.getConcept().getDisplayString() + " (" + res.getConceptName().getName() + ")");
      }
