...
public class PersonHandler extends CodedHandler {
  ...
  public void populateOptions(WidgetConfig config, CodedWidget widget) {
    ...
    if (StringUtils.isNotEmpty(config.getFormat()) && !"ajax".equals(config.getFormat())) {
      ...
      for (Person p : persons) {
        widget.addOption(
            new Option(p.getId().toString(), getPersonDisplay(p, config), null, p), config);
      }
  ...
  public Object parse(String input, Class<?> type) {
    if (StringUtils.isNotBlank(input)) {
      Integer personId = parseInteger(input);
      return personId == null ? null : Context.getPersonService().getPerson(personId);
    }
    return null;
  }
