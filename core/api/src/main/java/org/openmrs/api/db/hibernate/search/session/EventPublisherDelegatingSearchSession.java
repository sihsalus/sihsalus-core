...
/**
 * Custom implementation of the {@link SearchSession} interface that acts a wrapper around a target
 * SearchSession instance, it actually delegates all the method calls directly to the target except
 * for the {@link SearchSession#search(Collection)} and {@link SearchSession#search(SearchScope)}
 * method where it first notifies registered listeners of the creation event before returning the
 * newly created {@link SearchQuerySelectStep} object. The {@link
 * org.openmrs.api.db.hibernate.search.SearchQueryContributor} object and entity types are passed to
 * the listeners wrapped in a {@link SearchAndEntityCollection} object. <br>
 * <br>
...
public class EventPublisherDelegatingSearchSession implements SearchSession {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(EventPublisherDelegatingSearchSession.class);

  private SearchSession delegate;

  private ApplicationEventPublisher eventPublisher;

  public EventPublisherDelegatingSearchSession(
      SearchSession delegate, ApplicationEventPublisher eventPublisher) {
  ...
  public <T>
      SearchQuerySelectStep<
              NonStaticMetamodelScope, ?, EntityReference, T, SearchLoadingOptionsStep, ?, ?>
          search(Collection<? extends Class<? extends T>> collection) {
    log.debug("Creating new search instance");
    SearchQuerySelectDelegate<NonStaticMetamodelScope, EntityReference, T, SearchLoadingOptionsStep>
        search = new SearchQuerySelectDelegate<>(delegate.search(collection));
    ...
    SearchAndEntityCollection<T> queryAndEntity =
        new SearchAndEntityCollection<>(search, collection);
  ...
  public <T> SearchQuerySelectStep<?, ?, EntityReference, T, SearchLoadingOptionsStep, ?, ?> search(
    ...
    // but has the downside of where a rogue listener can block the entire application.
    SearchAndEntityCollection<T> queryAndEntity =
        new SearchAndEntityCollection<>(
            search,
            searchScope.includedTypes().stream()
  ...
  @Override
  public <SR, T> TypedSearchScope<SR, T> typedScope(
      Class<SR> srClass, Collection<? extends Class<? extends T>> collection) {
    return delegate.typedScope(srClass, collection);
  }

  @Override
  public <SR, T>
      SearchQuerySelectStep<SR, ?, EntityReference, T, SearchLoadingOptionsStep, ?, ?> search(
          TypedSearchScope<SR, T> typedSearchScope) {
