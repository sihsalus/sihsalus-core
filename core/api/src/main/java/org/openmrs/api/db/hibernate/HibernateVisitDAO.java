...
public class HibernateVisitDAO implements VisitDAO {
  ...
  public List<VisitType> getVisitTypes(String fuzzySearchPhrase) {
    ...
    cq.where(
        cb.like(
            cb.lower(root.get("name")), MatchMode.ANYWHERE.toLowerCasePattern(fuzzySearchPhrase)));
   ...
   *     java.util.Date, java.util.Map, boolean, boolean)
   */
  @Override
  @Transactional(readOnly = true)
  public List<Visit> getVisits(
      ...
      Date minEndDatetime,
      Date maxEndDatetime,
      final Map<VisitAttributeType, String> serializedAttributeValues,
    ...
    // active visits have null end date, so it doesn't make sense to search against it if include
    // inactive is set to false
    if (!includeInactive) {
      // the user only asked for currently active visits, so stop time needs to be null or after
      // right now
      predicates.add(
          cb.or(
              cb.isNull(root.get("stopDatetime")),
              cb.greaterThan(root.get("stopDatetime"), new Date())));
    } else {
      if (minEndDatetime != null) {
        predicates.add(
            cb.or(
                cb.isNull(root.get("stopDatetime")),
                cb.greaterThanOrEqualTo(root.get("stopDatetime"), minEndDatetime)));
      }
  ...
  public List<Visit> getVisits(VisitSearchCriteria criteria) throws DAOException {
    ...
    if (criteria.getMinStartDatetime() != null) {
      predicates.add(
          cb.greaterThanOrEqualTo(root.get("startDatetime"), criteria.getMinStartDatetime()));
    }
    if (criteria.getMaxStartDatetime() != null) {
      predicates.add(
          cb.lessThanOrEqualTo(root.get("startDatetime"), criteria.getMaxStartDatetime()));
    }
    ...
    if (!criteria.isIncludeInactive()) {
      ...
      if (criteria.getMinEndDatetime() != null) {
        predicates.add(
            cb.or(
                cb.isNull(root.get("stopDatetime")),
                cb.greaterThanOrEqualTo(root.get("stopDatetime"), criteria.getMinEndDatetime())));
      }
      if (criteria.getMaxEndDatetime() != null) {
        predicates.add(
            cb.lessThanOrEqualTo(root.get("stopDatetime"), criteria.getMaxEndDatetime()));
      }
    }
    ...
    if (criteria.getSerializedAttributeValues() != null) {
      CollectionUtils.filter(
          visits,
          new AttributeMatcherPredicate<Visit, VisitAttributeType>(
  ...
  public Visit getNextVisit(
      Visit previousVisit, Collection<VisitType> visitTypes, Date maximumStartDate) {
    Session session = sessionFactory.getCurrentSession();
    CriteriaBuilder cb = session.getCriteriaBuilder();
    CriteriaQuery<Visit> cq = cb.createQuery(Visit.class);
    Root<Visit> root = cq.from(Visit.class);
    ...
    predicates.add(
        cb.greaterThan(
            root.get("visitId"), (previousVisit != null) ? previousVisit.getVisitId() : 0));
    ...
    cq.where(predicates.toArray(new Predicate[] {})).orderBy(cb.asc(root.get("visitId")));
