...
public class ServiceContext implements ApplicationContextAware {
  ...
  /**
   * Static variable holding whether or not to use the system classloader. By default this is false
   * so the openmrs classloader is used instead
   */
  ...
  /**
   * There should only be one ServiceContext per openmrs (java virtual machine). This method should
   * be used when wanting to fetch the service context Note: The ServiceContext shouldn't be used
  ...
  public static void destroyInstance() {
    ...
    int serviceCount =
        instance == null || instance.services == null ? -1 : instance.services.size();
    int moduleServiceCount =
        instance == null || instance.moduleOpenmrsServices == null
            ? -1
            : instance.moduleOpenmrsServices.size();
    log.debug(
        "Destroying ServiceContext instance [serviceCount={}, moduleServiceCount={}]",
        serviceCount,
        moduleServiceCount);
    ...
    synchronized (refreshingContextLock) {
      try {
        while (refreshingContext) {
          ...
          log.debug(
              "Finished waiting to get service {} while the context was being refreshed", cls);
        }
    ...
    if (cls != null && classInstance != null) {
      try {
        ...
        if (noExistingService || replacingService) {
          ...
          if (!serviceAdvised) {
            ...
            advisedService =
                (Advised)
                    factory.getProxy(
                        proxyClassLoader != null ? proxyClassLoader : cls.getClassLoader());
          } else {
        ...
        throw new APIException(
            "service.unable.create.proxy.factory",
            new Object[] {classInstance.getClass().getName()},
            e);
      }
    }
  }
  ...
  /**
   * Set this service context to use the system class loader if the <code>useSystemClassLoader
   * </code> is set to true. If false, the openmrs class loader is used to load module services
   *
   ...
   * to doneRefreshingContext() yet. All methods involved in starting/stopping a module should call
   * this if a service method is needed -- otherwise a deadlock will occur.
   *
   ...
   *
   * <p><strong>Should</strong> return a list of all registered beans of the passed type<br>
   * <strong>Should</strong> return beans registered in a module<br>
   * <strong>Should</strong> return an empty list if no beans have been registered of the passed
   * type
   *
  ...
  /**
   * Calls the {@link OpenmrsService#onStartup()} method for an instance implementing the {@link
   ...
   * @since 1.9
   */
  private void runOpenmrsServiceOnStartup(
      final OpenmrsService openmrsService, final String classString) {
    OpenmrsThreadPoolHolder.threadExecutor.execute(
        () -> {
          try {
            synchronized (refreshingContextLock) {
              // Need to wait for application context to finish refreshing otherwise we get into
              // trouble.
              while (refreshingContext) {
                log.debug(
                    "Waiting to get service: {} while the context is being refreshed", classString);

                refreshingContextLock.wait();

                log.debug(
                    "Finished waiting to get service {} while the context was being refreshed",
                    classString);
              }
            }
            ...
            log.warn(
                "Refresh lock was interrupted while waiting to run OpenmrsService.onStartup() for "
                    + classString,
                e);
          }
        });
  ...
  /**
   * Gets a list of services implementing the {@link OpenmrsService} interface, for a given module.
   *
  ...
  public void clearEntireApiCache() {
    ...
    apiCacheManager
        .getCacheNames()
        .forEach(cacheName -> apiCacheManager.getCache(cacheName).invalidate());
  }
}
