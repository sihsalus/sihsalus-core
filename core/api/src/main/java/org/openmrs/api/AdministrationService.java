...
public interface AdministrationService extends OpenmrsService {
   ...
   *
   * <p><strong>Should</strong> return all registered system variables
   *
   ...
   *
   * <p><strong>Should</strong> return all system information
   *
   ...
   *
   * <p>If <code>propertyName</code> is not found in the list of Global Properties currently in the
   * database, a null value is returned. This method should not have any authorization check.
   *
   * <p><strong>Should</strong> not fail with null propertyName<br>
   ...
   * Gets the global property that has the given <code>propertyName</code>
   *
   * <p>If propertyName is not found in the list of Global Properties currently in the database, a
   * <code>defaultValue</code> is returned
   *
   * <p>This method should not have any authorization check
   *
   * <p><strong>Should</strong> return default value if property name does not exist<br>
   ...
   * Gets the global property that has the given <code>propertyName</code>
   *
   * <p><strong>Should</strong> return null when no global property match given property name
   *
   * @param propertyName property key to look for
   ...
   *
   * <p><strong>Should</strong> return all relevant global properties in the database
   *
   ...
   *
   * <p><strong>Should</strong> return all global properties in the database
   *
   ...
   *
   * <p><strong>Should</strong> save all global properties to the database<br>
   ...
   *
   * <p><strong>Should</strong> delete global property from database
   *
   ...
   *
   * <p><strong>Should</strong> delete global properties from database
   *
   ...
   * will be overwritten
   *
   * <p><strong>Should</strong> create global property in database<br>
   ...
   *
   * <p><strong>Should</strong> update global property in database<br>
  ...
  /**
   * Removes a GlobalPropertyListener previously registered by {@link
   ...
   *
   * <p><strong>Should</strong> execute sql containing group by
   *
   * @param sql
   * @param selectOnly
   * @return ResultSet
   ...
   *
   * <p><strong>Should</strong> return null if no implementation id is defined yet
   *
   ...
   *
   * <p><strong>Should</strong> create implementation id in database<br>
   ...
   * specified with a global property named {@link
   * OpenmrsConstants#GLOBAL_PROPERTY_LOCALE_ALLOWED_LIST}.
   *
   * <p><strong>Should</strong> return at least one locale if no locales defined in database yet<br>
   ...
   * <strong>Should</strong> return only country locale if both country locale and language locale
   * are specified in allowed list<br>
   * <strong>Should</strong> return all country locales if language locale and no country locales
   * are specified in allowed list<br>
   * <strong>Should</strong> return language locale if country locale is specified in allowed list
   * but country locale message file is missing<br>
   ...
   *
   * <p><strong>Should</strong> get property value in the proper type specified<br>
   ...
   *
   * <p>Used by {@link ValidateUtil#validate(Object)}.
   *
   * <p><strong>Should</strong> pass for a valid object<br>
   ...
   * Returns a list of locales used by the user when searching.
   *
   * <p>The list is constructed from a currently selected locale and allowed user proficient
   * locales.
   *
   * <p><strong>Should</strong> include currently selected full locale and language<br>
   ...
   *
   * <p>It is an optimisation parameter for MySQL, which can speed up searching if set to
   * <b>false</b>. See http://dev.mysql.com/doc/refman/5.7/en/case-sensitivity.html
   *
   * <p>It is set to <b>true</b> by default.
   *
   ...
   *
   * <ul>
   *   <li>Unlike MySQL which uses identifier strategy, PostgreSQL follows sequence strategy
   *   <li>So as to bridge the gap between these two strategies, this method has been created.
   *   <li>It will perform task of updating the sequence values after insertions are done from core
   *       data or concepts are inserted (present in Reference Metadata Module)
   * </ul>
   *
   * @since 2.4
   */
   ...
   * OpenmmrsMetadata, OpenmrsData and other common OpenMRS classes as well as any whitelists
   * defined through GPs with the '.serializer.whitelist.types' suffix that are considered to be
   * safe for deserializing. It is the responsibility of the serializer to block any unlisted
   * classes from being deserialized and posing security risk. It is especially important for
   * serializers using XStream.
   *
   * <p><strong>Should</strong> return packages and individual classes defined in GPs<br>
   ...
   * @return true if the module setup should be executed because of a version change, false
   *     otherwise
   */
