...
public interface ObsService extends OpenmrsService {
   ...
   * Get an observation
   *
   * <p><strong>Should</strong> get obs matching given obsId
   *
   ...
   *
   * <p><strong>Should</strong> find revision obs for given valid obs<br>
   ...
   *
   * <ul>
   *   <li>the obs you pass in is saved to the database, and its obsId field is filled in
   *   <li>the obs you pass in is returned
   *   <li>the changeMesssage parameter is ignored
   * </ul>
   *
   * <p>When you edit an existing observation:
   *
   * <ul>
   *   <li>the values of the obs you pass to this method are written to the database as <em>new</em>
   *       obs
   *   <li>the newly-created obs is returned (i.e. not the one you passed in)
   *   <li>the obs you passed is marked as voided, with changeMessage as the void reason
   *   <li>the newly-created obs points back to the voided one via its previousVersion field
   * </ul>
   *
   * <p><strong>Should</strong> create new file from complex data for new obs<br>
   * <strong>Should</strong> not overwrite file when updating a complex obs<br>
   * <strong>Should</strong> void the given obs in the database<br>
   * <strong>Should</strong> create very basic obs and add new obsId<br>
   * <strong>Should</strong> allow setting properties on obs<br>
   * <strong>Should</strong> return a different object when updating an obs<br>
   * <strong>Should</strong> set creator and dateCreated on new obs<br>
   * <strong>Should</strong> cascade save to child obs groups<br>
   * <strong>Should</strong> cascade update to new child obs groups<br>
   * <strong>Should</strong> link original and updated obs<br>
   * <strong>Should</strong> set void reason message to changeMessage<br>
   * <strong>Should</strong> not void an Obs with no changes
   *
   * @param obs the Obs to save to the database
   * @param changeMessage String explaining why <code>obs</code> is being changed. If <code>obs
   *     </code> is a new obs, changeMessage is nullable, or if it is being updated, it would be
   *     required
   ...
   *
   * <p><strong>Should</strong> set voided bit on given obs<br>
   ...
   *
   * <p><strong>Should</strong> unset voided bit on given obs<br>
  ...
  /**
   * Completely remove an observation from the database. This should typically not be called because
   * we don't want to ever lose data. The data really <i>should</i> be voided and then it is not
   * seen in interface any longer (see #voidObs(Obs) for that one) If other things link to this obs,
   * an error will be thrown.
   *
   * <p><strong>Should</strong> delete the given obs from the database
   *
   * @param obs
   * @throws APIException
   ...
   * Completely remove an observation from the database. This should typically not be called because
   * we don't want to ever lose data. The data really <i>should</i> be voided and then it is not
   * seen in interface any longer (see #voidObs(Obs) for that one)
   *
   * <p><strong>Should</strong> throw APIException if given true cascade<br>
   * <strong>Should</strong> delete any obsGroupMembers before deleting the obs<br>
   ...
   * voided observations.
   *
   * <p><strong>Should</strong> get all observations assigned to given person
   *
   ...
   * equivalent to an "and"ing of the arguments. (e.g. if both a <code>location</code> and a <code>
   * fromDate</code> are given, only Obs that are <u>both</u> at that Location and after the
   ...
   * @param personTypes List&lt;PERSON_TYPE&gt; objects to restrict this to. Only used if <code>whom
   *     </code> is an empty list (optional)
  ...
  /**
   * <strong>Should</strong> compare dates using lte and gte<br>
   * <strong>Should</strong> get all obs assigned to given encounters<br>
   * <strong>Should</strong> get all obs with question concept in given questions parameter<br>
   * <strong>Should</strong> get all obs with answer concept in given answers parameter<br>
   * <strong>Should</strong> return all obs whose person is a person only<br>
   * <strong>Should</strong> return obs whose person is a patient only<br>
   * <strong>Should</strong> return obs whose person is a user only<br>
   * <strong>Should</strong> return obs with location in given locations parameter<br>
   * <strong>Should</strong> sort returned obs by obsDatetime if sort is empty<br>
   * <strong>Should</strong> sort returned obs by conceptId if sort is concept<br>
   * <strong>Should</strong> limit number of obs returned to mostReturnN parameter<br>
   * <strong>Should</strong> return obs whose groupId is given obsGroupId<br>
   * <strong>Should</strong> not include voided obs<br>
   * <strong>Should</strong> include voided obs if includeVoidedObs is true<br>
   ...
   *     java.lang.Integer, java.util.Date, java.util.Date, boolean) This method works exactly the
   *     same; it only adds accession number search criteria. It effectively surpasses the above
   *     method; the old one is however kept for backward compatibility reasons.
   * @param whom List&lt;Person&gt; to restrict obs to (optional)
   * @param encounters List&lt;Encounter&gt; to restrict obs to (optional)
   * @param questions List&lt;Concept&gt; to restrict the obs to (optional)
   * @param answers List&lt;Concept&gt; to restrict the valueCoded to (optional)
   ...
   * All arguments are optional and nullable. If more than one argument is non-null, the result is
   * equivalent to an "and"ing of the arguments. (e.g. if both a <code>location</code> and a <code>
   * fromDate</code> are given, only Obs that are <u>both</u> at that Location and after the
   * fromDate are returned). <br>
   * <br>
  ...
  /**
   * <strong>Should</strong> compare dates using lte and gte<br>
   * <strong>Should</strong> get the count of all obs assigned to given encounters<br>
   * <strong>Should</strong> get the count of all obs with question concept in given questions
   * parameter<br>
   * <strong>Should</strong> get the count of all obs with answer concept in given answers parameter
   * <br>
   * <strong>Should</strong> return the count of all obs whose person is a person only<br>
   * <strong>Should</strong> return the count of all obs whose person is a patient only<br>
   * <strong>Should</strong> return the count of obs whose person is a user only<br>
   * <strong>Should</strong> return the count of obs with location in given locations parameter<br>
   * <strong>Should</strong> return the count of obs whose groupId is given obsGroupId<br>
   * <strong>Should</strong> not include count of voided obs<br>
   * <strong>Should</strong> include count of voided obs if includeVoidedObs is true<br>
   ...
   *     java.util.List, java.util.List, java.util.List, java.util.List, java.lang.Integer,
   *     java.util.Date, java.util.Date, boolean) This method works exactly the same; it only adds
   *     accession number search criteria. It effectively surpasses the above method; the old one is
   ...
   *
   * <p><strong>Should</strong> get obs matching patient identifier in searchString<br>
   * <strong>Should</strong> get obs matching encounterId in searchString<br>
   ...
   * concept (conceptId)
   *
   * <p><strong>Should</strong> get observations matching person and question<br>
   ...
   *
   * <p><strong>Should</strong> fill in complex data object for complex obs<br>
   * <strong>Should</strong> return normal obs for non complex obs<br>
   ...
   *
   * <p><strong>Should</strong> get handler with matching key<br>
   ...
   * Returns null if the Obs.isComplexObs() is false or there is an error instantiating the handler
   * class.
   *
   * <p><strong>Should</strong> get handler associated with complex observation
   *
   ...
   * ComplexObsHandler to this service. If the given String key exists, that handler is overwritten
   * with the given handler For most situations, this map is set via spring, see the
   * applicationContext-service.xml file to add more handlers.
   *
   * <p><strong>Should</strong> override handlers with same key<br>
   ...
   *
   * <p><strong>Should</strong> never return null
   *
  ...
  /**
   * Registers the given handler with the given key If the given String key exists, that handler is
   * overwritten with the given handler
   *
   * <p><strong>Should</strong> register handler with the given key
   *
   ...
   *
   * <p><strong>Should</strong> load handler and register key
   *
   * @param key the key name to use for this handler
   ...
   *
   * <p><strong>Should</strong> remove handler with matching key<br>
   ...
   *
   * <p><strong>Should</strong> return the count of all observations using the specified
   * conceptNames as answers<br>
   * <strong>Should</strong> include voided observations using the specified conceptNames as answers
   * <br>
   * <strong>Should</strong> return zero if no observation is using any of the concepNames in the
   * list
   *
   ...
   *     same; it only adds visits to the search criteria. It effectively surpasses the above
   *     method; the old one is however kept for backward compatibility reasons.
