...
public class UserContext implements Serializable {
  ...
  /**
   * Authenticate user with the provided credentials. The authentication scheme must be Spring
   * wired, see {@link Context#getAuthenticationScheme()}.
   *
  ...
  public Authenticated authenticate(Credentials credentials) throws ContextAuthenticationException {
    ...
    log.debug(
        "Authenticating client '{}' with scheme '{}'",
  ...
  public void addProxyPrivilege(String... privileges) {
    if (privileges == null || Arrays.stream(privileges).anyMatch(Objects::isNull)) {
      throw new IllegalArgumentException(
          "UserContext.addProxyPrivilege does not accept null privileges");
    }
  ...
  /**
   * Removes one or more privileges to the list of privileges that that {@link
   ...
   *
   * <p><strong>Should</strong> not fail with null user<br>
   ...
   *
   * <p><strong>Should</strong> authorize if authenticated user has specified privilege<br>
  ...
  public boolean hasPrivilege(String privilege) {
    ...
    if (isAuthenticated()
        && (getAuthenticatedUser().hasPrivilege(privilege)
   ...
   *
   * <p><strong>Should</strong> fail if database doesn't contain anonymous role
   *
   * @return Role
   */
   ...
   *
   * <p><strong>Should</strong> fail if database doesn't contain authenticated role
   *
   * @return Role
   */
  ...
  protected Integer getDefaultLocationId(User user) {
    ...
    if (StringUtils.isNotBlank(defaultLocation)) {
      ...
      if (possibleLocation != null
          && (this.locationId == null || !this.locationId.equals(possibleLocation.getId()))) {
        return possibleLocation.getId();
      }

      log.warn(
          "The default location for user '{}' is set to '{}', which is not a valid location",
          user.getUsername(),
          defaultLocation);
    }
   ...
   *
   * <p>It is called by {@link UserContext#hasPrivilege(java.lang.String)}.
   *
  ...
  private void notifyPrivilegeListeners(User user, String privilege, boolean hasPrivilege) {
    for (PrivilegeListener privilegeListener :
        Context.getRegisteredComponents(PrivilegeListener.class)) {
      try {
  ...
  private void notifyUserSessionListener(User user, Event event, Status status) {
    for (UserSessionListener userSessionListener :
        Context.getRegisteredComponents(UserSessionListener.class)) {
