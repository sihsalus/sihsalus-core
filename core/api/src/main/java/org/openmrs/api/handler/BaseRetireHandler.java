...
/**
 * This is the default class for all retire* actions that take place on all services. The {@link
 * RequiredDataAdvice} class uses AOP around each method in every service to check to see if its a
 * retire* method. If it is a retire* method, this class is called to handle setting the {@link
 * Retireable#getRetired()}, {@link Retireable#setRetireReason(String)}, {@link
 * Retireable#setRetiredBy(User)}, and {@link Retireable#setDateRetired(Date)}. <br>
 * <br>
...
public class BaseRetireHandler implements RetireHandler<Retireable> {
   ...
   * are null).<br>
   *
   * <p><strong>Should</strong> set the retired bit<br>
  ...
  @Override
  public void handle(
      Retireable retireableObject, User retiringUser, Date retireDate, String retireReason) {
