...
public class OrderUtil {
   ...
   *
   * <p><strong>Should</strong> return false if they are both null <strong>Should</strong>
   *
   * @since 1.10
   * @param orderType1
   * @param orderType2
   * @return true if orderType2 matches or is a sub type of orderType1 <strong>Should</strong> true
   *     if orderType2 is the same or is a subtype of orderType1 <strong>Should</strong> false if
   *     orderType2 is neither the same nor a subtype of orderType1 return false if any is null and
   *     the other is not
   */
   ...
   * @return true if the schedules overlap otherwise false <strong>Should</strong> return true if
   *     order1 and order2 do not have end date <strong>Should</strong> return true if order1 and
   *     order2 have same start dates <strong>Should</strong> return false if order1 ends before
   *     order2 starts <strong>Should</strong> return false if order1 starts after order2
   *     <strong>Should</strong> return true if order1 stops after the order2 has already been
   *     activated <strong>Should</strong> return true if order1 starts when the order2 is active
  ...
  public static boolean checkScheduleOverlap(Order order1, Order order2) {
    ...
    if (order2.getEffectiveStopDate() == null) {
      return OpenmrsUtil.compare(order1.getEffectiveStopDate(), order2.getEffectiveStartDate())
          > -1;
    }

    if (order1.getEffectiveStopDate() == null) {
      return (OpenmrsUtil.compare(order1.getEffectiveStartDate(), order2.getEffectiveStartDate())
              > -1)
          && (OpenmrsUtil.compare(order1.getEffectiveStartDate(), order2.getEffectiveStopDate())
              < 1);
    }

    return (OpenmrsUtil.compare(order1.getEffectiveStartDate(), order2.getEffectiveStopDate()) < 1)
        && (OpenmrsUtil.compare(order1.getEffectiveStopDate(), order2.getEffectiveStartDate())
            > -1);
  }
}
