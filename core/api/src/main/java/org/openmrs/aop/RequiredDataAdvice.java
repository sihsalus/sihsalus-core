 ...
 * with.<br>
 * <br>
 * To add a new action to happen for a save* method, create a new class that extends {@link
 * RequiredDataHandler}. Add any <b>unique</b> code that needs to be done automatically before the
 * save. See {@link ConceptNameSaveHandler} as an example. (The code should be <b>unique</b> because
 * all other {@link SaveHandler}s will still be called <i>in addition to</i> your new handler.) Be
 * sure to add the {@link org.openmrs.annotation.Handler} annotation (like
 * "@Handler(supports=YourPojoThatHasUniqueSaveNeeds.class)") to your class so that it is picked up
 * by Spring automatically.<br>
 * <br>
 * To add a new action for a void* or retire* method, extend the {@link VoidHandler}/ {@link
 * RetireHandler} class and override the handle method. Do not call super, because that code would
 * then be run twice because both handlers are registered. Be sure to add the {@link
 * org.openmrs.annotation.Handler} annotation (like
 * "@Handler(supports=YourPojoThatHasUniqueSaveNeeds.class)") to your class so that it is picked up
...
public class RequiredDataAdvice implements MethodBeforeAdvice {
  ...
  public void before(Method method, Object[] args, Object target) throws Throwable {
    ...
    // "update" is not an option here because there are multiple methods that start with "update"
    // but is
    ...
    if (methodName.startsWith("save") || methodName.startsWith("create")) {
      ...
      // if the first argument is a list of openmrs objects, handle them all now
        ...
        // ideally we would fail early if the method name is not like
        // savePluralOfXyz(Collection<Xyz>)
      ...
      if (methodName.startsWith("void")) {
        ...
        Date dateRetired =
            retireable.getDateRetired() == null ? new Date() : retireable.getDateRetired();
   ...
   * List)}. Calls that method with the current user and the current Date.
   *
   ...
   */
  public static <H extends RequiredDataHandler<OpenmrsObject>> void recursivelyHandle(
      Class<H> handlerType, OpenmrsObject openmrsObject, String reason) {
    recursivelyHandle(
        handlerType,
        openmrsObject,
        Context.getAuthenticatedUser(),
   ...
   *     void/retire reason)
   * @param alreadyHandled an optional list of objects that have already been handled and should not
   *     be processed again. this is intended to prevent infinite recursion when handling collection
   *     properties.
        ...
        if (childCollection != null) {
          for (OpenmrsObject collectionElement : childCollection) {
            if (!alreadyHandled.contains(collectionElement)) {
              recursivelyHandle(
                  handlerType, collectionElement, currentUser, currentDate, other, alreadyHandled);
            }
          }
        }
      }
  ...
  /**
   * This method gets a child attribute off of an OpenmrsObject. It usually uses the getter for the
   * attribute, but can use the direct field (even if its private) if told to by the {@link
   * AllowDirectAccess} annotation.
   *
   * <p><strong>Should</strong> get value of given child collection on given field<br>
   * <strong>Should</strong> should be able to get annotated private fields<br>
   ...
   */
  @SuppressWarnings("unchecked")
  protected static Collection<OpenmrsObject> getChildCollection(
      OpenmrsObject openmrsObject, Field field) {
    ...
    try {
      ...
      if (field.isAnnotationPresent(AllowDirectAccess.class)) {
        ...
        Collection<OpenmrsObject> childCollection =
            (Collection<OpenmrsObject>) field.get(openmrsObject);
      ...
      if (field.isAnnotationPresent(AllowDirectAccess.class)) {
        throw new APIException(
            "unable.get.field", new Object[] {fieldName, openmrsObject.getClass()});
      } else {
        throw new APIException(
            UNABLE_GETTER_METHOD,
  ...
  /**
   * Checks the given {@link Class} to see if it A) is a {@link Collection}/{@link Set}/ {@link
   * List}, and B) contains {@link OpenmrsObject}s
   *
   * <p><strong>Should</strong> return true if class is openmrsObject list<br>
   * <strong>Should</strong> return true if class is openmrsObject set<br>
   ...
   */
  protected static boolean isHandlerMarkedAsDisabled(
      Class<? extends RequiredDataHandler> handlerType, Field field) {

    // if the annotation isn't present, return false
    if (!field.isAnnotationPresent(DisableHandlers.class)) {
      return false;
    } else {
      // otherwise we need to see if the handler type is one of the types specified in the
      // annotation
      for (Class<? extends RequiredDataHandler> h :
          field.getAnnotation(DisableHandlers.class).handlerTypes()) {
