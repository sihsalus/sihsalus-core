...
public class CustomDatatypeUtil {
  ...
  public static CustomDatatypeHandler getHandler(CustomValueDescriptor descriptor) {
    return getHandler(
        getDatatypeOrDefault(descriptor),
        descriptor.getPreferredHandlerClassname(),
   ...
   */
  public static CustomDatatypeHandler getHandler(
      CustomDatatype<?> dt, String preferredHandlerClassname, String handlerConfig) {
    if (preferredHandlerClassname != null) {
      try {
        Class<? extends CustomDatatypeHandler> clazz =
            (Class<? extends CustomDatatypeHandler>) Context.loadClass(preferredHandlerClassname);
        ...
        log.warn(
            "Failed to instantiate and configure preferred handler with class "
    ...
    // if we couldn't get the preferred handler (or none was specified) we get the default one by
    // datatype
  ...
  /**
   * Deserializes a simple String-based configuration from the serialized form used by {@link
   * #serializeSimpleConfiguration(Map)} Utility method for property-style configuration
   * implementations.
   *
   * <p><strong>Should</strong> deserialize a configuration serialized by the corresponding
   * serialize method
   *
  ...
  public static Map<String, String> deserializeSimpleConfiguration(String serializedConfig) {
    ...
    try {
      return Context.getSerializationService()
          ...
          .deserialize(serializedConfig, Map.class);
    } catch (SerializationException ex) {
      throw new APIException(ex);
    }
  }
   ...
   */
  public static <T extends AttributeType<?>, U> Map<T, String> getValueReferences(
      Map<T, U> datatypeValues) {
      ...
      for (Map.Entry<T, U> e : datatypeValues.entrySet()) {
        ...
        try {
          ...
          throw new APIException(
              "CustomDatatype.error.cannot.search", new Object[] {customDatatype.getClass()});
        }
  ...
  public static void saveAttributesIfNecessary(Customizable<?> customizable) {
    // TODO decide whether we can move this into a SingleCustomValueSaveHandler instead of leaving
    // it here to be called by each Customizable service's save method
  ...
  public static <T, D extends CustomValueDescriptor> boolean validate(SingleCustomValue<D> value) {
    try {
      ...
      log.error(
          "Exception caught while trying to validate the value {} for descriptor {}",
          value.getValue(),
          value.getDescriptor(),
          ex);
      return false;
