...
/**
 * A metadata item describing how a type of custom value is stored. (For example a GlobalProperty is
 * its own {@link CustomValueDescriptor}, and a VisitAttributeTypes is the descriptor for
 * VisitAttribute.)
 *
 * @since 1.9
 */
public interface CustomValueDescriptor {
   ...
   * May be null.
   *
   * @return the configuration to be passed to the datatype. For example if the datatype is
   *     RegexValidatedString the datatypeConfig would be the regular expression
   ...
   * May be null.
   *
   * @return the fully-qualified classname of the preferred {@link CustomDatatypeHandler} chosen by
   *     the system administrator for this attribute type.
   */
