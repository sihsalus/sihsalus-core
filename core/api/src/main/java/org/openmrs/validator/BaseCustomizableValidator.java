...
public abstract class BaseCustomizableValidator implements Validator {
  ...
  public <T extends AttributeType, A extends Attribute> void validateAttributes(
      Customizable<A> customizable, Errors errors, Collection<T> attributeTypes) {
      ...
      if ((at.getMinOccurs() > 0 || at.getMaxOccurs() != null) && !at.getRetired()) {
        ...
        if (at.getMinOccurs() > 0 && numFound < at.getMinOccurs()) {
          ...
          if (at.getMinOccurs() == 1) {
            errors.rejectValue(
                "activeAttributes", "error.required", new Object[] {at.getName()}, null);
          } else {
            errors.rejectValue(
                "activeAttributes",
                "attribute.error.minOccurs",
                new Object[] {at.getName(), at.getMinOccurs()},
                null);
          }
        }
        if (at.getMaxOccurs() != null && numFound > at.getMaxOccurs()) {
          errors.rejectValue(
              "activeAttributes",
              "attribute.error.maxOccurs",
              new Object[] {at.getName(), at.getMaxOccurs()},
              null);
        }
      }
    }

    // validate all non-voided attributes for their values (we already checked minOccurs and
    // maxOccurs for the types)
