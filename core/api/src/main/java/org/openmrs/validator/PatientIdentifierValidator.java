...
@Handler(
    supports = {PatientIdentifier.class},
    order = 50)
public class PatientIdentifierValidator implements Validator {
   ...
   *
   * <p><strong>Should</strong> pass validation if field lengths are correct<br>
   ...
   *
   * <p><strong>Should</strong> fail validation if PatientIdentifier is null<br>
   * <strong>Should</strong> pass validation if PatientIdentifier is voided<br>
   * <strong>Should</strong> fail validation if another patient has a matching identifier of the
   * same type<br>
  ...
  public static void validateIdentifier(PatientIdentifier pi) throws PatientIdentifierException {
    ...
    if (!pi.getVoided()) {
      ...
      if (patient != null) {
        for (PatientIdentifier existingIdentifier :
            patient.getPatientIdentifiers(pi.getIdentifierType())) {

          boolean isDifferentIdentifierObject = !existingIdentifier.getUuid().equals(pi.getUuid());
          boolean hasSameIdentifierValue =
              existingIdentifier.getIdentifier().equals(pi.getIdentifier());
          boolean shouldBeGloballyUnique =
              UniquenessBehavior.UNIQUE.equals(pi.getIdentifierType().getUniquenessBehavior());
          boolean bothLocationsNull =
              existingIdentifier.getLocation() == null && pi.getLocation() == null;
          boolean sameLocation =
              existingIdentifier.getLocation() != null
                  && existingIdentifier.getLocation().equals(pi.getLocation());
          if (isDifferentIdentifierObject
              && hasSameIdentifierValue
              && (shouldBeGloballyUnique || bothLocationsNull || sameLocation)) {
            throw new DuplicateIdentifierException(
                Context.getMessageSourceService()
                    .getMessage(
                        "PatientIdentifier.error.notUniqueSamePatientWithParameter",
                        new Object[] {pi.getIdentifier()},
                        Context.getLocale()),
                pi);
          }
   ...
   *
   * <p><strong>Should</strong> fail validation if PatientIdentifierType is null<br>
   ...
   *
   * <p><strong>Should</strong> fail validation if identifier is blank<br>
   * <strong>Should</strong> fail validation if identifier does not match the format<br>
   * <strong>Should</strong> pass validation if identifier matches the format<br>
   * <strong>Should</strong> pass validation if the format is blank<br>
   ...
   */
  public static void checkIdentifierAgainstFormat(
      String identifier, String format, String formatDescription)
      throws PatientIdentifierException {
    ...
    if (!identifier.matches(format)) {
      ...
      throw new InvalidIdentifierFormatException(
          getMessage(
              "PatientIdentifier.error.invalidFormat",
              identifier,
   ...
   *
   * <p><strong>Should</strong> fail validation if identifier is blank<br>
   * <strong>Should</strong> fail validation if identifier is invalid<br>
   * <strong>Should</strong> pass validation if identifier is valid<br>
   ...
   * @throws PatientIdentifierException if the identifier is does not match the format
   */
  public static void checkIdentifierAgainstValidator(
      String identifier, IdentifierValidator validator) throws PatientIdentifierException {
    ...
    try {
      ...
      throw new InvalidCheckDigitException(
          getMessage(
              "PatientIdentifier.error.unallowedIdentifier", identifier, validator.getName()));
    }
