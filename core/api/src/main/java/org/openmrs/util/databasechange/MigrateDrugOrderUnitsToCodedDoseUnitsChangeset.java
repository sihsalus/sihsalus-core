  ...
  private void migrateUnitsToCodedValue(JdbcConnection connection, Set<String> uniqueUnits)
    ...
    try {
      ...
      updateDrugOrderStatement =
          connection.prepareStatement("update drug_order set dose_units = ? where units = ?");
      ...
      for (String unit : uniqueUnits) {
        if (StringUtils.isBlank(unit)) {
          ...
          String dosingUnitsConceptSetUuid =
              UpgradeUtil.getGlobalProperty(
                  connection.getUnderlyingConnection(),
                  OpenmrsConstants.GP_DRUG_DOSING_UNITS_CONCEPT_UUID);
          List<Integer> dosingUnitsconceptIds =
              UpgradeUtil.getMemberSetIds(
                  connection.getUnderlyingConnection(), dosingUnitsConceptSetUuid);
