...
public class PersonAttribute extends BaseChangeableOpenmrsData
    implements java.io.Serializable, Comparable<PersonAttribute> {
   ...
   * equality. Note: Null/empty fields on <code>otherAttribute</code> /will not/ cause a false value
   * to be returned
   *
   * <p><strong>Should</strong> return true if attributeType value and void status are the same
   *
  ...
  /**
   * Will try to create an object of class 'PersonAttributeType.format'. If that implements <code>
   * Attributable</code>, hydrate(value) is called. Defaults to just returning getValue()
   *
   * <p><strong>Should</strong> load class in format property<br>
   ...
   *
   * <p><strong>Should</strong> set voided bit to true
   *
   * @param reason
   */
  ...
  /**
   * <strong>Should</strong> return negative if other attribute is voided<br>
   * <strong>Should</strong> return negative if other attribute has earlier date created<br>
   * <strong>Should</strong> return negative if this attribute has lower attribute type than
   * argument<br>
   * <strong>Should</strong> return negative if other attribute has lower value<br>
  ...
  public static class DefaultComparator implements Comparator<PersonAttribute>, Serializable {
    ...
    public int compare(PersonAttribute pa1, PersonAttribute pa2) {
      int retValue;
      if ((retValue =
              OpenmrsUtil.compareWithNullAsGreatest(pa1.getAttributeType(), pa2.getAttributeType()))
          != 0) {
        return retValue;
      }
      ...
      if ((retValue =
              OpenmrsUtil.compareWithNullAsLatest(pa1.getDateCreated(), pa2.getDateCreated()))
          != 0) {
        return retValue;
      }
