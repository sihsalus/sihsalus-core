...
@MappedSuperclass
@Audited
public abstract class BaseAttribute<AT extends AttributeType, OwningType extends Customizable<?>>
    extends BaseChangeableOpenmrsData implements Attribute<AT, OwningType>, Comparable<Attribute> {
  ...
  // temporarily holds a typed value, either when getValue() is called the first time (causing
  // valueReference to be converted) or when setValue has been called, but this attribute has not
  // yet been committed to persistent storage
  ...
  public Object getValue() throws InvalidCustomValueException {
    if (value == null) {
      value =
          CustomDatatypeUtil.getDatatype(getAttributeType())
              .fromReferenceString(getValueReference());
    }
    return value;
  }
    ...
    if (retValue == 0) {
      retValue =
          OpenmrsUtil.compareWithNullAsGreatest(
              getAttributeType().getId(), other.getAttributeType().getId());
    }
    if (retValue == 0) {
      retValue =
          OpenmrsUtil.compareWithNullAsGreatest(getValueReference(), other.getValueReference());
    }
    if (retValue == 0) {
