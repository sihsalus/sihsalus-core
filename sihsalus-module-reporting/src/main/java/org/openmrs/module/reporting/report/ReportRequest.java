...
/**
 * Represents a request to run and render a report. (The natural ordering of this class places
 * higher priority requests first, so that it may be used in a PriorityQueue.)
 */
public class ReportRequest extends BaseOpenmrsObject {
  ...
  /** Full Constructor */
  public ReportRequest(
      Mapped<ReportDefinition> reportDefinition,
      Mapped<CohortDefinition> baseCohort,
  ...
  public Mapped<CohortDefinition> getBaseCohort() {
    if (baseCohort == null && StringUtils.isNotBlank(baseCohortUuid)) {
      baseCohort =
          mappedDefinition(CohortDefinition.class, baseCohortUuid, baseCohortParameterMappings);
    }
    return baseCohort;
  }
  ...
  public Mapped<ReportDefinition> getReportDefinition() {
    if (reportDefinition == null && StringUtils.isNotBlank(reportDefinitionUuid)) {
      reportDefinition =
          mappedDefinition(
              ReportDefinition.class, reportDefinitionUuid, reportDefinitionParameterMappings);
    }
  ...
  public RenderingMode getRenderingMode() {
    if (renderingMode == null && rendererType != null) {
      try {
        ...
        renderingMode =
            new RenderingMode(
                renderer, renderer.getClass().getSimpleName(), rendererArgument, null);
      } catch (Exception e) {
  ...
  public void setRenderingMode(RenderingMode renderingMode) {
    ...
    this.rendererType =
        renderingMode == null || renderingMode.getRenderer() == null
            ? null
            : renderingMode.getRenderer().getClass();
  ...
  public String getBaseCohortParameterMappings() {
    return baseCohort == null
        ? baseCohortParameterMappings
        : serializeParameterMappings(baseCohort);
  }
  ...
  public String getReportDefinitionParameterMappings() {
    return reportDefinition == null
        ? reportDefinitionParameterMappings
        : serializeParameterMappings(reportDefinition);
  }
  ...
  public Class<? extends ReportRenderer> getRendererType() {
    return renderingMode == null || renderingMode.getRenderer() == null
        ? rendererType
        : renderingMode.getRenderer().getClass();
  }
   ...
   *       priority
   *   <li>a request created by a demon thread should have LOW or LOWEST priority.
   * </ul>
   */
  ...
  private <T extends Definition> Mapped<T> mappedDefinition(
      Class<T> mappedType, String uuid, String serializedMappings) {
    ...
    if (StringUtils.isNotBlank(serializedMappings)) {
      try {
        ...
        throw new IllegalStateException(
            "Unable to deserialize parameter mappings for " + mappedType.getName(), e);
      }
    }
  ...
  private String serializeParameterMappings(Mapped<? extends Parameterizable> mapped) {
    if (mapped == null
        || mapped.getParameterMappings() == null
        || mapped.getParameterMappings().isEmpty()) {
      return null;
    }
    try {
