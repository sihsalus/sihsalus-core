...
public abstract class CompositionQueryEvaluator<Q extends Query<T>, T extends OpenmrsObject>
  ...
  public static final List<Character> CHARACTER_WORDS =
      Arrays.asList('+', '!', '(', '[', '{', ')', ']', '}');
  public static final List<Class<?>> SUPPORTED_TYPES =
      Arrays.asList(Integer.class, BooleanOperator.class, Query.class, List.class);

  /** Default Constructor */
  public CompositionQueryEvaluator() {}

  /**
   * Implementation classes need to override this method to provide the necessary functionality to
   * evaluate a Query
   */
  ...
  /**
   * Implementation classes need to override this method to return the necessary definition that can
   * be evaluated to all ids of that type
   */
  protected abstract Q getAllIdQuery();

  protected IdSet<T> evaluateToIdSet(Q compositionQuery, EvaluationContext context)
      throws EvaluationException {
  ...
  /**
   * Recursively traverse the List<Object> phrase to produce a (possibly nested) CompositionQuery If
   * another List<Object> is found in the list, recursively evaluate it in place If anything in this
   * list is a key into searches, replace it with the relevant filter from searches
   *
   * @throws EvaluationException
   */
  @SuppressWarnings("unchecked")
  protected IdSet<T> evaluateTokens(
      List<Object> tokens, CompositionQuery<Q, T> composition, EvaluationContext context)
      throws EvaluationException {
   ...
   * BooleanOperator (AND, OR, NOT) a Query, another List of the same form, which indicates a
  ...
  public List<Object> parseIntoTokens(String expression) throws EvaluationException {
    ...
    try {
      ...
      throw new EvaluationException(
          "Unable to parse expression <" + expression + "> into tokens", e);
    }
  }
