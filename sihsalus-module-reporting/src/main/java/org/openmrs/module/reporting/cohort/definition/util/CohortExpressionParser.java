...
/**
 * This class encapsulates the parsing logic necessary to take a String expression and parse it into
 * a List<Object>. The parser
 */
@Deprecated
public class CohortExpressionParser {
  ...
  private static final List<Character> characterWords =
      Arrays.asList('+', '!', '(', '[', '{', ')', ']', '}');
  ...
  /**
   * Recursively traverse the List<Object> phrase to produce a (possibly nested)
   * CompoundCohortDefinition If another List<Object> is found in the list, recursively evaluate it
   * in place If anything in this list is a key into searches, replace it with the relevant filter
   * from searches
   *
   * @throws EvaluationException
   */
  @SuppressWarnings("unchecked")
  public static Cohort evaluate(
      List<Object> tokens, CompositionCohortDefinition composition, EvaluationContext context)
      throws EvaluationException {
    ...
    for (Object o : use) {
      if (o instanceof BooleanOperator) {
        ...
        throw new RuntimeException(
            "Can only handle Cohorts and Operators.  Unable to handle class: " + o.getClass());
      }
    }
   ...
   * BooleanOperator (AND, OR, NOT) a CohortDefinition a PatientSearch another List of the same
   * form, which indicates a parenthetical expression
   */
