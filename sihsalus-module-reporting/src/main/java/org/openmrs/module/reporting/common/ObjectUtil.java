...
public class ObjectUtil {
  ...
  public static String toString(Map<?, ?> m, String keyValueSeparator, String entrySeparator) {
    ...
    for (Map.Entry<?, ?> e : m.entrySet()) {
      ...
      sb.append(
          ObjectUtil.format(e.getKey()) + keyValueSeparator + ObjectUtil.format(e.getValue()));
    }
    return sb.toString();
  }
   ...
   *     "="
   * @param entrySeparator the string that separates each key/value pair in the Map, if null
   *     defaults to ","
   * @return
   */
  public static Map<String, String> toMap(
      String toParse, String keyValueSeparator, String entrySeparator) {
    Map<String, String> ret = new LinkedHashMap<String, String>();
    if (notNull(toParse)) {
      for (String entry : StringUtils.splitByWholeSeparator(toParse, nvlStr(entrySeparator, ","))) {
        String[] keyValue =
            StringUtils.splitByWholeSeparator(entry, nvlStr(keyValueSeparator, "="), 2);
  ...
  /**
   * @param toParse the string to parse into a Map<String, String>. Expected format is
  ...
  /**
   * Checks whether the testIfNull parameter is null, if so returns valueIfNull, otherwise returns
   * valueIfNotNull
   */
  ...
  /**
   * Checks whether the testIfNull parameter is null, if so returns valueIfNull as String, otherwise
   * returns valueIfNotNull as String
   */
  ...
  /**
   * Checks whether two objects are equal. This is null-safe and considers null and empty string to
   * be equal
   */
  ...
  /**
   * Checks whether two objects have equal toString representations. This is null-safe and considers
   * null and empty string to be equal
   */
  ...
  public static <T> void addIfNotNull(
      Collection<T> listToAddTo, T itemToAddToList, Object objectToCheckIfNull) {
  ...
  public static String verifyStringLength(String value, Integer maxLength) {
    if (maxLength != null && value.length() > maxLength) {
      throw new RuntimeException(
          "Maximum width for column with value '"
              + value
  ...
  public static String formatDate(Date d, String format, Locale locale) {
    DateFormat df = null;
    String formatString =
        ObjectUtil.nvl(format, ReportingConstants.GLOBAL_PROPERTY_DEFAULT_DATE_FORMAT());
    ...
    if (ObjectUtil.notNull(format)) {
      ...
      try {
        ...
        while (startIndex != -1 && endIndex != -1) {
          ...
          String newFormat =
              (replacement != null && formatSplit.length > 1 ? formatSplit[1] : null);
  ...
  public static String format(Object o, String format, Locale locale) {
    ...
    try {
      ...
      if (o instanceof LocalDateTime) {
        return formatDate(
            Date.from(((LocalDateTime) o).atZone(ZoneId.systemDefault()).toInstant()),
            format,
            locale);
      }
      if (o instanceof LocalDate) {
        return formatDate(
            Date.from(((LocalDate) o).atStartOfDay(ZoneId.systemDefault()).toInstant()),
            format,
            locale);
      }
      if (o instanceof Map) {
  ...
  @SuppressWarnings({"rawtypes", "unchecked"})
  public static <T extends Object> List<T> sort(Collection<T> collection, String sortSpecification)
      throws Exception {
  ...
  /**
   * @return a Properties object based on a properties file on the classpath at the specified
   *     location
   */
