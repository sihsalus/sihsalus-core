...
/**
 * Evaluator for {@link
 * org.openmrs.module.reporting.data.patient.definition.PatientCalculationDataDefinition}
 */
  ...
  public EvaluatedPatientData evaluate(PatientDataDefinition definition, EvaluationContext context)
    ...
    if (registration == null) {
      throw new EvaluationException(
          "No PatientCalculation found on this PatientCalculationDataDefinition");
    }
    ...
    try {
      provider =
          (CalculationProvider)
              Context.loadClass(registration.getProviderClassName()).newInstance();
    } catch (Exception e) {
      throw new EvaluationException(
    ...
    try {
      calculation =
          (PatientCalculation)
              provider.getCalculation(
                  registration.getCalculationName(), registration.getConfiguration());
    } catch (InvalidCalculationException e) {
      throw new EvaluationException(
          "The provider could not find calculation " + registration.getToken(), e);
    }

    if (calculation instanceof PatientDataCalculation) {
      throw new EvaluationException(
          "You cannot configure a PatientDataCalculation in a PatientCalculationDataDefinition.");
    }
