...
public class EncounterCohortDefinition extends BaseCohortDefinition {
  ...
  public String toString() {
    ...
    if (onOrAfter != null) ret.append(" on or after " + onOrAfter);
    if (onOrBefore != null) ret.append(" on or before " + onOrBefore);
    if (timeQualifier != null && timeQualifier != TimeQualifier.ANY)
      ret.append(" where the " + timeQualifier + " was ");
    if (locationList != null) ret.append(" at " + locationList);
    if (encounterTypeList != null) ret.append(" of type " + encounterTypeList);
    if (formList != null) ret.append(" from form " + formList);
    if (providerList != null) ret.append(" from provider " + providerList);
    if (atLeastCount != null) ret.append(" at least " + atLeastCount);
    if (atMostCount != null) ret.append(" at most " + atMostCount);
    if (returnInverse != null && returnInverse.booleanValue()) ret.append(" AND INVERT THIS");
    if (createdBy != null) ret.append(" created by " + createdBy);
    return ret.toString();
  }
