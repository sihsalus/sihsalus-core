...
@Transactional
public class SerializedDefinitionServiceImpl extends BaseOpenmrsService
    implements SerializedDefinitionService {
   ...
   */
  @Authorized
  public <T extends Definition> List<T> getAllDefinitions(
      Class<T> definitionType, boolean includeRetired) {
   ...
   */
  @Authorized
  public <T extends Definition> List<DefinitionSummary> getAllDefinitionSummaries(
      Class<T> definitionType, boolean includeRetired) {
    ...
    Collections.sort(
        ret,
        new Comparator<DefinitionSummary>() {
   ...
   */
  @Authorized
  public <T extends Definition> List<SerializedObject> getInvalidDefinitions(
      Class<T> definitionType, boolean includeRetired) {
    List<SerializedObject> ret = new ArrayList<SerializedObject>();
    for (SerializedObject so : dao.getAllSerializedObjects(definitionType, includeRetired)) {
      try {
   ...
   */
  @Authorized
  public <T extends Definition> int getNumberOfDefinitions(
      Class<T> definitionType, boolean includeRetired) {
  ...
  public <T extends Definition> List<T> getDefinitions(
      Class<T> definitionType, String name, boolean exactMatchOnly) {
    List<T> ret = new ArrayList<T>();
    for (SerializedObject so :
        dao.getAllSerializedObjectsByName(definitionType, name, exactMatchOnly)) {
      try {
        ret.add(dao.convertSerializedObject(definitionType, so));
      } catch (Exception e) {
        ExceptionUtil.rethrowAuthenticationException(e);
