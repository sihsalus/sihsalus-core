...
public class ParameterizableUtil {
  ...
  public static Parameterizable getParameterizable(
      String uuid, Class<? extends Parameterizable> type) {
  ...
  public static Parameterizable saveParameterizable(Parameterizable parameterizable) {
    if (parameterizable instanceof Definition) {
      ...
      throw new APIException(
          "Unable to save parameterizable of type " + parameterizable.getClass());
    }
  }

  /** Evaluates the given parameterizable. */
  public static Object evaluateParameterizable(
      Parameterizable parameterizable, EvaluationContext context) throws EvaluationException {
    Object result = null;
    if (parameterizable != null) {
      if (parameterizable instanceof Definition) {
        ...
        throw new APIException(
            "Unable to evaluate parameterizable of type <"
                + parameterizable.getClass().getName()
                + ">");
      }
    }
    return result;
  ...
  /**
   * Utility method which will return the underlying Parameterizable type from a class property that
   * is one of the following supported formats:
   *
   * <p>Mapped<Parameterizable> Mapped<? extends Parameterizable>
   * Collection<Mapped<Parameterizable>> Collection<Mapped<? extends Parameterizable>> Map<String,
   * Mapped<Parameterizable>> Map<String, Mapped<? extends Parameterizable>>
   *
      ...
      try {
        ...
        if (Mapped.class.isAssignableFrom(f.getType())) {
          ...
          throw new RuntimeException(
              "Cannot retrieve Mapped type from: " + type.getSimpleName() + "." + property);
        }
        ...
        throw new IllegalArgumentException(
            "Cannot retrieve Mapped type from: " + type.getSimpleName() + "." + property, e);
      }
    }
    return mappedType;
  }
  ...
  /**
   * Utility method which will return the underlying Mapped property of the Parameterizable from a
   * class property that is one of the following supported formats:
   *
   * <p>Mapped<Parameterizable> Mapped<? extends Parameterizable>
   * Collection<Mapped<Parameterizable>> Collection<Mapped<? extends Parameterizable>> Map<String,
   * Mapped<Parameterizable>> Map<String, Mapped<? extends Parameterizable>>
   ...
   */
  @SuppressWarnings("unchecked")
  public static Mapped<Parameterizable> getMappedProperty(
      Parameterizable obj, String property, String collectionKey) {
    if (obj != null) {
      ...
      if (propertyValue != null) {
        try {
          ...
          throw new IllegalArgumentException(
              "Mapped Property Editor cannot handle: " + propertyValue);
        }
        return mapped;
      }
    }
  ...
  /**
   * @return a Map, where the entries are the names of each child parameter, and the values are all
   *     compatible parent parameters. The values are organized as a Map where each key is a parent
   *     parameter name and each value is a parent parameter Label
   */
  public static Map<String, Map<String, String>> getAllowedMappings(
      Parameterizable parent, Parameterizable child) {
  ...
  /**
   * @return a Map, where the entries are the categorizations, and the values are the names and
   *     values of the child parameters per category
   */
  public static Map<String, Map<String, Object>> getCategorizedMappings(
      Parameterizable parent, Parameterizable child, Map<String, Object> mappings) {
  ...
  /**
   * Refreshes this {@link Mapped} with a version of the definition re-fetched from the {@link
   * DefinitionContext} by uuid. (Use this method if you have a Mapped definition from one hibernate
   * session that you need to use in another session.
   *
   * @param mapped
   */
  public static <Def extends Definition> void refreshMappedDefinition(Mapped<Def> mapped) {
    try {
      ...
      // Do nothing.  Sometimes this throws a weird hibernate exception, but this method is a hack
      // anyway
    }
  }

  /**
   * This method takes in a copyFrom Definition that we wish to copy all parameters from, and adds
   * them all to the passed copyTo Definition, before returning a Mapped containing the copyFrom
   * Definition and the parameter mappings that relate it to copyTo
   */
  public static <T extends Definition> Mapped<T> copyAndMap(
      T copyFrom, Definition copyTo, Map<String, String> renamedParameters) {
