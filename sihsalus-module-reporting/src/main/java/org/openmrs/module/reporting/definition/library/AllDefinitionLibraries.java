...
public class AllDefinitionLibraries {
  ...
  public List<LibraryDefinitionSummary> getDefinitionSummaries(
      Class<? extends Definition> returnType) {
    if (returnType == null) {
  ...
  List<DefinitionLibrary<?>> getLibraries() {
    return libraries == null
        ? Collections.<DefinitionLibrary<?>>emptyList()
        : Collections.unmodifiableList(libraries);
  }

  /**
   * Constructs a CohortDefinition that references the given key, with the given specified parameter
   * values. This will cause an underlying library to build the definition (so we can determine its
   * parameters) but the returned CohortDefinition only has a reference to that definition by key
   *
   * @param key
   * @param paramsAndValues must have an even length. Each pair is (String) paramName, (Object)
   *     paramValue
   * @return
   */
  ...
  /**
   * The normal way to do this should be to just define an @Component that implements
   * DefinitionLibrary. This method should only be used for testing -- we may not decide to preserve
   * it.
   *
   * @param library
   */
