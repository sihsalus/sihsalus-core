...
public class CohortUtil {
  ...
  /**
   * @param cohort The cohort to be limited.
   * @param size The desired size of the cohort.
   */
  ...
  public static boolean areEqual(Cohort a, Cohort b) {
    return (a != null
        && b != null
        && a.size() == b.size()
        && a.size() == CohortUtil.intersect(a, b).size());
  }

  /**
   * @return a Composition Cohort Definition with each definition composed with a single type of
   *     operator
   */
  public static CohortDefinition getCompositionCohort(
      String operator, Map<String, Mapped<? extends CohortDefinition>> entries) {
    if (entries.size() == 1) {
    ...
    for (Map.Entry<String, Mapped<? extends CohortDefinition>> cd : entries.entrySet()) {
      d.addSearch(
          cd.getKey(), cd.getValue().getParameterizable(), cd.getValue().getParameterMappings());
      if (s.length() > 0) {
        s.append(" " + operator + " ");
      }
      s.append(cd.getKey());
   ...
   */
  public static CohortDefinition getCompositionCohort(
      String operator, CohortDefinition... definitions) {
  ...
  /**
   * @return a Composition Cohort Definition with a primary cohort which has 1 to n cohorts
   *     subtracted from it
   */
