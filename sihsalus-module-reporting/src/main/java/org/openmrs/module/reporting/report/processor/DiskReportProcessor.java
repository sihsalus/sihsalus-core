...
public class DiskReportProcessor implements ReportProcessor {
    ...
    try {
      ...
      if ("true".equals(configuration.getProperty(COMPRESS_OUTPUT))) {
        ...
        try {
          ...
          ZipEntry zipEntry =
              new ZipEntry(
                  report.getRequest().getReportDefinition().getParameterizable().getName());
        ...
        try {
          ZipInputStream zip =
              new ZipInputStream(new ByteArrayInputStream(report.getRenderedOutput()));
          ...
          log.error(
              "Failed to decompress report: " + report.getReportData().getDefinition().getName(),
              ex);
        }
      }
  ...
  private File resolveZipEntry(String folderName, ZipEntry entry, String extension)
      throws IOException {
    File destinationDirectory = new File(folderName).getCanonicalFile();
    File destinationFile =
        new File(destinationDirectory, entry.getName() + "." + extension).getCanonicalFile();
