...
public interface ReportService extends OpenmrsService {
  ...
  /**
   * Return a list of {@link ReportDesign}s for {@link ReportDefinition} that match the passed
   * parameters Each input parameter can be null, restricting the returned results only if it is not
   * null. This allows you to retrieve all ReportDesigns by ReportDefinition, by RendererType, by
   * retired status, or a combination of these criteria.
   *
   * @param reportDefinition if not null, only {@link ReportDesign}s for this {@link
   *     ReportDefinition} will be returned
   * @param rendererType if not null, only {@link ReportDesign}s for this {@link ReportRenderer}
   *     type will be returned
   * @param includeRetired if true, indicates that retired {@link ReportDesign}s should also be
   *     included
   ...
   * ReportDesign, the returned ReportDesign will have a new {@link ReportDesign#getId()} inserted
   * into it that was generated by the database
   *
  ...
  /**
   * @return a List of {@link RenderingMode}s that the passed {@link ReportDefinition} supports, in
  ...
  /**
   * Get Report Requests by Report Definition, request date, having given status. The list is sorted
   * descending by evaluateCompleteDatetime, evaluateStartDatetime, priority, requestDate.
   *
   * @param reportDefinition a Report Definition filter, nullable
   * @param requestOnOrAfter a Date used to limit result to ReportRequests which ware requested on
   *     or after (greater or equal filter), nullable
   * @param requestOnOrBefore a Date used to limit result to ReportRequests which ware requested on
   *     or before (lower or equal filter), nullable
   * @param statuses an array of Status, used to limit result to ReportRequests with status included
   *     in the array, null or empty array means that all statuses are included, nullable
  ...
  /**
   * Get Report Requests by Report Definition, request date, having given status, limit result to at
   * most {@code mostRecentNum} elements. The list is sorted descending by evaluateCompleteDatetime,
   * evaluateStartDatetime, priority, requestDate.
   *
   * @param reportDefinition a Report Definition filter, nullable
   * @param requestOnOrAfter a Date used to limit result to ReportRequests which ware requested on
   *     or after (greater or equal filter), nullable
   * @param requestOnOrBefore a Date used to limit result to ReportRequests which ware requested on
   *     or before (lower or equal filter), nullable
   * @param mostRecentNum maximum number of results, a null value means all records are to be
   *     returned, nullable
   * @param statuses an array of Status, used to limit result to ReportRequests with status included
   *     in the array, null or empty array means that all statuses are included, nullable
   * @return all {@link ReportRequest} in the system that match the passed parameters
  ...
  /**
   * Gets a count of Report Requests that match by Report Definition, request date limits and
   * status(-es).
   *
   * @param reportDefinition a Report Definition filter, nullable
   * @param requestOnOrAfter a Date used to limit result to ReportRequests which ware requested on
  ...
  /**
   * @return all the {@link ReportProcessorConfiguration}s that aren't associated with a specific
   *     ReportDesign
   * @should retrieve all saved report processor configurations with reportDesign = null, and
   *     retired = false;
   */
  @Transactional(readOnly = true)
   ...
   * @should retrieve all non-retired report processor configurations that are assignable to the
   *     passed type
   */
  @Transactional(readOnly = true)
  ...
  /**
   * @return the File that may contain the serialized {@link ReportData} for a given {@link
   *     ReportRequest}
   */
  @Transactional(readOnly = true)
  ...
  /**
   * @return the File that may contain the rendered output from the evaluation of a {@link
   *     ReportRequest}
   */
  @Transactional(readOnly = true)
  ...
  /**
   * @return the File that may contain any log messages when evaluating a given {@link
   *     ReportRequest}
   */
  @Transactional(readOnly = true)
  ...
  /**
   * Returns the number in the queue for this report request, or null if processing has already
   * started
   */
  ...
  /**
   * Loads the Rendered Output for a previously generated Report for the given ReportRequest, first
   * checking the cache
   */
  @Transactional(readOnly = true)
  ...
  /**
   * Loads the Error message for a previously generated Report for the given ReportRequest, first
   * checking the cache
   */
  @Transactional(readOnly = true)
  ...
  /**
   * Loads the Log messages for a previously generated Report for the given ReportRequest, first
   * checking the cache
   */
  @Transactional(readOnly = true)
  ...
  /**
   * Deletes report requests that are not saved, and are older than the value specified by {@link
  ...
  /**
   * Persists reports that are cached but not on disk Removes from Cache the oldest reports if the
   * max cache size is exceeded {@link ReportingConstants#GLOBAL_PROPERTY_MAX_CACHED_REPORTS()}
   */
  @Transactional(readOnly = true)
  public void persistCachedReports();

  /**
   * Saves the passed message to disk for the given report, in order to have a record of the report
   * generation
   *
