...
/**
 * The logic that evaluates a {@link CohortCrossTabDataSetDefinition} and produces a {@link
 * MapDataSet}
 *
...
public class CohortDataSetEvaluator implements DataSetEvaluator {
  ...
  public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext context)
    ...
    for (CohortDataSetColumn col : dsd.getDataSetColumns()) {
      ...
      try {
        rowCohort =
            (col.getRowDefinition() == null
                ? context.getBaseCohort()
                : cds.evaluate(col.getRowDefinition(), context));
      } catch (Exception ex) {
        throw new EvaluationException(
            "row definition for row=" + col.getRowName() + " , col=" + col.getColumnName(), ex);
      }
      if (rowCohort == null) {
      ...
      try {
        colCohort =
            (col.getColumnDefinition() == null
                ? context.getBaseCohort()
                : cds.evaluate(col.getColumnDefinition(), context));
      } catch (Exception ex) {
        throw new EvaluationException(
            "column definition for row=" + col.getRowName() + " , col=" + col.getColumnName(), ex);
      }
      if (colCohort == null) {
