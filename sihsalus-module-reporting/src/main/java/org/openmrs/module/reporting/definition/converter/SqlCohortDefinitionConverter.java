...
/**
 * Converts legacy SqlCohortDefinitions into the latest supported format Before: ...
 * <queryDefinition class="org.openmrs.module.reporting.query.definition.SqlQueryDefinition" id="3"
 * retired="false"> <parameters id="4"/> <queryString>SELECT distinct patient_id FROM patient WHERE
 * patient_id = :patientId</queryString> </queryDefinition> ...
 *
 * <p>After: ... <query>SELECT distinct patient_id FROM patient WHERE patient_id =
 * :patientId</query> ...
 */
@Handler
public class SqlCohortDefinitionConverter implements DefinitionConverter {
  ...
  public boolean convertDefinition(SerializedObject so) {
    ...
    try {
      ...
      // Confirm this works
      CohortDefinition scd =
          Context.getService(CohortDefinitionService.class).getDefinitionByUuid(so.getUuid());
      log.info(
          "Successfully converted SqlCohortDefinition named '" + scd.getName() + "' to new format");
      return true;
    } catch (Exception e) {
