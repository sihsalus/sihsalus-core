...
public class PatientDataCalculationProvider implements CalculationProvider {
  ...
  public PatientDataCalculation getCalculation(String calculationName, String configuration)
    ...
    try {
      ...
      List<? extends DataDefinition> l =
          Context.getService(PatientDataService.class).getDefinitions(calculationName, true);
      if (l.isEmpty()) {
      ...
      if (l.size() == 1) {
        ...
        throw new InvalidCalculationException(
            "Unable to load Data Definition from calculationName: " + calculationName);
      }
    }
