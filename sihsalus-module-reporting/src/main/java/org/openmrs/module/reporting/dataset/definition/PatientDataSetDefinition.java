...
public class PatientDataSetDefinition extends RowPerObjectDataSetDefinition {
  ...
  @ConfigurationProperty private List<Mapped<? extends CohortDefinition>> rowFilters;
  ...
  public void addColumn(
      String name, DataDefinition dataDefinition, String mappings, DataConverter... converters) {
    addColumn(
        name, dataDefinition, ParameterizableUtil.createParameterMappings(mappings), converters);
  }
  ...
  public void addColumn(
    ...
    if (dataDefinition instanceof PatientDataDefinition) {
      getColumnDefinitions()
          .add(new RowPerObjectColumnDefinition(name, dataDefinition, mappings, converters));
    } else if (dataDefinition instanceof PersonDataDefinition) {
      PatientDataDefinition pdd =
          new PersonToPatientDataDefinition((PersonDataDefinition) dataDefinition);
  ...
  public void addColumns(
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Unable to add columns from " + dataSetDefinition.getClass().getSimpleName(), e);
    }

    addColumn(
        name,
        new PatientDataSetDataDefinition(dataSetDefinition, whichValues, numberOfValues),
        mappings,
        converters);
  }
  ...
  public void addRowFilter(CohortDefinition rowFilter, String parameterMappings) {
    addRowFilter(
        new Mapped<CohortDefinition>(
            rowFilter, ParameterizableUtil.createParameterMappings(parameterMappings)));
  }
}
