 ...
 * and it will generate those rows on demand (by delegating to a {@link IterableDataSetEvaluator}).
 */
public class IterableSqlDataSet implements DataSet {
  ...
  private void mapMetaData() {
    ...
    for (DataSetColumn column : dataSetRowIterator.getColumns()) {
      metadata.addColumn(
          new DataSetColumn(column.getName(), column.getLabel(), column.getDataType()));
    }
  }
}
