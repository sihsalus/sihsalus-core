...
public interface CohortQueryService extends OpenmrsService {
  ...
  /**
   * Executes the given sql query string. This method will substitute parameter values from the
   * given parameter map into the named parameter placeholder in the query.
   *
   * <p>TODO The query string should actually be a bean that encapsulates any type of query (SQL,
   * HQL, MDX). The execute query method would actually delegate to handlers (parser, validator,
   * executor), similar to the way most of our reporting objects are handled by a persister,
   * evaluator handlers.
   *
  ...
  /**
   * Get named parameters from the given sql query. This method will return a list of {@link
   * Parameter}s that have been created based on any named parameters (e.g. "patient_id =
   * :patientId") that have been found within the given sql query.
   *
  ...
  /**
   * Returns a cohort of patients who "pass" a logic expression. (Depends on Logic's
   * Result.toBoolean, which basically means that empty results and the boolean value false are
   * "failure", and anything else "passes".)
   *
  ...
  public Cohort executeLogicQuery(
      String logicExpression, Map<String, Object> parameterValues, Cohort baseCohort);
}
