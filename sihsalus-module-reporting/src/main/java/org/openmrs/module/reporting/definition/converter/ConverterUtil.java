...
public class ConverterUtil {
  ...
  public static synchronized void refreshConversionStatus() {
    numInvalidDefinitions =
        Context.getService(SerializedDefinitionService.class).getInvalidDefinitions(true).size();
    log.warn(
        "Refresh conversion status returned "
            + numInvalidDefinitions
            + " definitions that require conversion.");
  }
}
