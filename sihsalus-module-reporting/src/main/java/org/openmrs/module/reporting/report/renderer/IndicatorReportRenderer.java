    ...
    for (String dataSetKey : results.getDataSets().keySet()) {
      ...
      w.write(
          "<table id=\"indicator-report-dataset-"
              + dataSetKey
              + "\" class=\"display indicator-report-dataset\">");
      for (DataSetColumn column : columns) {
        w.write("<tr>");
        w.write("<td>" + column.getName() + "</td>");
        w.write("<td>" + column.getLabel() + "</td>");

        // Wondering if you can even do this ... iterate over a dataset multiple times (once for
        // each column?)
        ...
        for (DataSetRow row : dataset) {
          ...
          if (cellValue instanceof CohortIndicatorAndDimensionResult) {
            CohortIndicatorAndDimensionResult result =
                (CohortIndicatorAndDimensionResult) cellValue;
