...
public class IndicatorUtil {
  ...
  /**
   * Utility method which does a case-insensitive check on the given Collection of Strings for the
   * passed testString, returning true if the Collection contains the passed string
   *
   ...
   */
  public static List<String> compileColumnDimensionOptions(
      Map<String, List<String>> dimensionsAndOptions) {
    ...
    if (dimensionsAndOptions != null && !dimensionsAndOptions.isEmpty()) {
      ...
      compileDimensionCombinations(
          0, "", new ArrayList<String>(dimensionsAndOptions.keySet()), combinations);
  ...
  private static void compileOptionCombinations(
      int index,
      String prefix,
    ...
    if (index < dimensionKeys.length) {
      ...
      if (currentKeys != null) {
        for (int j = 0; j < currentKeys.size(); j++) {
          ...
          String newPrefix =
              prefix + (ObjectUtil.isNull(prefix) ? "" : ",") + dimensionKeys[index] + "=" + option;
          if (index == dimensionKeys.length - 1) {
            options.add(newPrefix);
          } else {
            compileOptionCombinations(
                index + 1, newPrefix, dimensionKeys, dimensionsAndOptions, options);
          }
        }
      }
    }
