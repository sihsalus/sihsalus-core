...
public class ObsInEncounterCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
  ...
  public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context)
    ...
    // Hibernate doesn't really support limit the way we need, so using this approach of 2
    // queries...
    ...
    if (cd.getWhichEncounter() == TimeQualifier.LAST) {
      encQuery
          ...
          .orderAsc("e.dateCreated"); // Ascending since we take last value found
    } else if (cd.getWhichEncounter() == TimeQualifier.FIRST) {
      encQuery
          .orderDesc("e.encounterDatetime")
          .orderDesc("e.dateCreated"); // Descending since we take last value found
    }
    Set<Integer> encountersToInclude = new HashSet<Integer>();
    if (cd.getWhichEncounter() == TimeQualifier.LAST
        || cd.getWhichEncounter() == TimeQualifier.FIRST) {
      Map<Integer, Integer> encountersByPatient =
          evaluationService.evaluateToMap(encQuery, Integer.class, Integer.class, context);
