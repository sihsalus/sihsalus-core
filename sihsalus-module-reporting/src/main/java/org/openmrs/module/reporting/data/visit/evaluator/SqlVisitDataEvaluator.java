...
/**
 * Expects that the SQL query returns two columns: the first should be an Integer returning the
 * visitId the second should be the data you wish to retrieve for each Visit Expects that you use
 * "visitIds" within your query to limit by the base id set in the evaluation context: eg. "select
 * start_datetime from visit where visit_id in (:visitIds)"
 */
...
public class SqlVisitDataEvaluator implements VisitDataEvaluator {
  ...
  @Override
  public EvaluatedVisitData evaluate(VisitDataDefinition def, EvaluationContext context)
