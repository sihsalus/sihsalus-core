...
public class PersonDataUtil {
  ...
  /**
   * @return the base set of person ids relevant for the passed EvaluationContext or null for all
   *     person ids If returnNullForAllPersonIds is false, then this will return all person ids in
   *     the system if unconstrained by the context
   */
  public static Set<Integer> getPersonIdsForContext(
      EvaluationContext context, boolean returnNullForAllPersonIds) {

    Set<Integer> ret = null;
    Cohort patIds = context.getBaseCohort();
    PersonIdSet personIds =
        (context instanceof PersonEvaluationContext
            ? ((PersonEvaluationContext) context).getBasePersons()
            : null);
    ...
    SqlQueryBuilder qb =
        new SqlQueryBuilder(
            "select person_id from person where voided = 0"); // Seems like a bug in core.  Using
    // HQL here only returns persons who
