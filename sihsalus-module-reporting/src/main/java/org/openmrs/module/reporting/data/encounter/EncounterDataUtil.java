...
public class EncounterDataUtil {
  ...
  /**
   * @return the base set of encounter ids relevant for the passed EvaluationContext or null for all
   *     encounter ids If returnNullForAllEncounterIds is false, then this will return all encounter
   *     ids in the system if unconstrained by the context
   */
  public static Set<Integer> getEncounterIdsForContext(
      EvaluationContext context, boolean returnNullForAllEncounterIds) {

    Cohort patIds = context.getBaseCohort();
    EncounterIdSet encIds =
        (context instanceof EncounterEvaluationContext
            ? ((EncounterEvaluationContext) context).getBaseEncounters()
            : null);
    ...
    if (patIds != null) {
      ...
      qb.select("e.encounterId")
          .from(Encounter.class, "e")
          .wherePatientIn("e.patient.patientId", context);
      List<Integer> encIdsForPatIds =
