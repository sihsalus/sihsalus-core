...
class EvaluationUtilTest {
  ...
  void parseParameterNameFromExpressionReturnsNameBeforeOperations() {
    assertEquals(
        "report.startDate",
        EvaluationUtil.parseParameterNameFromExpression("${report.startDate+5d-1w}"));
  }

  @Test
  void evaluateParameterExpressionAppliesIntegerOperationsInOrder() {
    Object evaluated =
        EvaluationUtil.evaluateParameterExpression("count+5*2-8", Map.of("count", 5));
  ...
  void evaluateParameterExpressionAppliesDateOperations() {
    Object evaluated =
        EvaluationUtil.evaluateParameterExpression("start+1d", Map.of("start", new Date(0)));
