...
public class UpdateUserTask implements Runnable {
  ...
  private static final Logger log = LoggerFactory.getLogger(UpdateUserTask.class);
  ...
  static class NullAwareBeanUtilsBean extends BeanUtilsBean {
    ...
    private void copyCollection(Collection<?> dest, Collection<?> value)
  ...
  private User updated(User user) {
    ...
    try {
      ...
      if (user.getPerson() != null
          && user.getPerson().getPersonName() != null
          && user.getPerson().getPersonName().getUuid() != null) {
      ...
      log.error(
          "Something went wrong when copying attributes from the user info to the OpenMRS user, the OpenMRS user might not have been updated properly.",
          e);
    }

    if (userInfo.getRoleNames() != null) {
      user.setRoles(
          userInfo.getRoleNames().stream()
              .map(roleName -> userService.getRole(roleName))
              .filter(r -> r != null)
