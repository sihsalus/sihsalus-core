...
2. Static modular monolith
   ...
   - `runtime` (`sihsalus-core-boot`) wires OpenMRS services, DAOs, Hibernate mappings, storage, cache, request `Context` sessions, and Liquibase through static Spring configuration. Hibernate mapping discovery no longer asks the OpenMRS module runtime for started modules.
