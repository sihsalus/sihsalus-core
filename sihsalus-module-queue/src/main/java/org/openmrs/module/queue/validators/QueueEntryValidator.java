...
public class QueueEntryValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    if (!(target instanceof QueueEntry)) {
      throw new IllegalArgumentException(
          "the parameter target must be of type " + QueueEntry.class);
    }

    rejectIfEmptyOrWhitespace(
        errors, "queue", "queueEntry.queue.null", "The property queue should not be null");
    rejectIfEmptyOrWhitespace(
        errors, "patient", "queueEntry.patient.null", "The property patient should not be null");
    rejectIfEmptyOrWhitespace(
        errors,
        "startedAt",
    ...
    if (visit != null && queueEntry.getStartedAt() != null) {
      if (queueEntry.getStartedAt().before(visit.getStartDatetime())) {
        errors.rejectValue(
            "startedAt",
            "queue.entry.error.cannotStartBeforeVisitStartDate",
        ...
        if (queueEntry.getStartedAt().after(visit.getStopDatetime())) {
          errors.rejectValue(
              "startedAt",
              "queue.entry.error.cannotStartAfterVisitStopDate",
              "A queue entry cannot start after the associated visit stop date");
        } else if (queueEntry.getEndedAt() == null
            || queueEntry.getEndedAt().after(visit.getStopDatetime())) {
          errors.rejectValue(
              "endedAt",
              "queue.entry.error.cannotEndAfterVisitStopDate",
      ...
      if (queueEntry.getPatient() != null
          && (visit.getPatient() == null
              || !visit
                  .getPatient()
                  .getPatientId()
                  .equals(queueEntry.getPatient().getPatientId()))) {
        errors.rejectValue(
            "visit",
            "queue.entry.error.visit.wrongPatient",
    ...
    if (queueEntry.getStatus() == null) {
      errors.rejectValue(
          "status", "queueEntry.status.null", "The property status should not be null");
    } else if (queue != null) {
      if (!queueServices.getAllowedStatuses(queue).contains(queueEntry.getStatus())) {
        errors.rejectValue(
            "status",
            "queueEntry.status.invalid",
  ...
  private boolean isDuplicate(QueueEntry queueEntry, QueueEntryService queueEntryService) {
    ...
    // if we are checking an existing queue entry then check if there is at least one overlapping
    // entry
