...
public class QueueServicesWrapper {
  ...
  public Concept getConcept(String conceptRef) {
    ...
    // handle name.  this isn't ideal, as core will just log a warning if there are 2 concepts with
    // the same name
    // but there are no alternative suitable methods to get a List of Concepts that exactly match a
    // given name
  ...
  public Location getLocation(String locationRef) {
    ...
    if (locations.size() == 1) {
      ...
      throw new IllegalArgumentException(
          "More than one location is found with name: " + locationRef);
    }
  ...
  public List<Concept> getAllowedServices() {
    ...
    if (conceptSet == null) {
      throw new IllegalStateException(
          "Missing global property: " + QueueModuleConstants.QUEUE_SERVICE);
    }
  ...
  public List<Concept> getAllowedStatuses(Queue queue) {
    ...
    if (conceptSet == null) {
      throw new IllegalStateException(
          "Missing global property: " + QueueModuleConstants.QUEUE_STATUS);
    }
    return conceptSet.getSetMembers();
  }
  ...
  /**
   * @param queue
   * @return the allowed priority concepts for the given queue, either configured directly or from
   *     GP default
   */
  public List<Concept> getAllowedPriorities(Queue queue) {
    ...
    if (conceptSet == null) {
      throw new IllegalStateException(
          "Missing global property: " + QueueModuleConstants.QUEUE_PRIORITY);
    }
    return conceptSet.getSetMembers();
  }
