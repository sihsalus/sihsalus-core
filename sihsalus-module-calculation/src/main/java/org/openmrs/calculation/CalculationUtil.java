...
public class CalculationUtil {
  ...
  public static boolean isPrimitiveWrapperClassName(String className) {
    ...
    if (className != null) {
      for (Class<?> wrapper : primitiveWrappers) {
        if (className.equals(wrapper.getName())) return true;
      }
    }
    return false;
  }
  ...
  public static Calculation getCalculation(
      String providerClassName, String calculationName, String configuration)
  ...
  /**
   * Utility method that casts the specified value to the specified Type and handles conversions to
   * primitive wrapper classes in a better/more lenient way than java's type casting. Note that the
   * method will throw a {@link ConversionException} at runtime if it fails to convert the specified
   * value
   *
   * @see ConversionException
   * @param value the value to be cast
   * @param clazz the class to cast to
   * @return a value of the specified type <strong>Should</strong> fail if the value to convert is
   *     not of a compatible type <strong>Should</strong> convert the value to the specified type if
   *     it is compatible <strong>Should</strong> return null if the passed in value is null
   *     <strong>Should</strong> convert a valid string value to Boolean <strong>Should</strong>
   *     convert a character value to Character <strong>Should</strong> convert a valid string value
   *     to Short <strong>Should</strong> convert a valid string value to Integer
   *     <strong>Should</strong> convert a valid string value to Long <strong>Should</strong>
   *     convert a valid string value to Float <strong>Should</strong> convert a valid string value
   *     to Double <strong>Should</strong> convert a valid string value to Byte
   *     <strong>Should</strong> convert a single character value to Short <strong>Should</strong>
   *     convert a valid single character value to Integer <strong>Should</strong> convert a valid
   *     single character value to Long <strong>Should</strong> convert a result with an number
   *     value in the valid range to byte <strong>Should</strong> convert a valid string to an enum
   *     constant <strong>Should</strong> convert a valid string to a class object
   *     <strong>Should</strong> convert a valid string to a Locale <strong>Should</strong> format a
   *     date object to a string using the default date format
   */
  ...
  public static <T> T cast(Object value, Class<T> clazz) {
    ...
    if (String.class.isAssignableFrom(clazz)) {
      ...
      else castValue = (T) value.toString();
    } else {
      // we should be able to convert strings to simple types like String "2" to integer 2,
      // enums, dates, etc. Java types casting doesn't allow this so we need to transform the value
      // first to
      ...
      if (BeanUtils.isSimpleValueType(clazz) || clazz.isEnum()) {
        try {
          ...
          if (String.class.isAssignableFrom(value.getClass())) stringValue = (String) value;
