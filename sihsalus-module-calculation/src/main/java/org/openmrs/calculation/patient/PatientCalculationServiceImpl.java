...
public class PatientCalculationServiceImpl extends BaseOpenmrsService
    implements PatientCalculationService {
  ...
  public CalculationResult evaluate(
      Integer patientId, PatientCalculation calculation, PatientCalculationContext context)
      throws APIException {
  ...
  public CalculationResult evaluate(
      ...
      throws APIException {
    CalculationResultMap cr =
        evaluate(Arrays.asList(patientId), calculation, parameterValues, context);
  ...
  /**
   * @see org.openmrs.calculation.patient.PatientCalculationService#evaluate(java.util.Collection,
   *     org.openmrs.calculation.patient.PatientCalculation)
   */
  @Override
  public CalculationResultMap evaluate(Collection<Integer> cohort, PatientCalculation calculation)
      throws APIException {
  ...
  public CalculationResultMap evaluate(
      Collection<Integer> cohort, PatientCalculation calculation, PatientCalculationContext context)
      throws APIException {
  ...
  public CalculationResultMap evaluate(
    ...
    if (cohort.isEmpty()) return new CalculationResultMap();
    ...
    if (defs != null) {
      for (ParameterDefinition parameter : defs) {
        ...
        Object value = null;
        if (parameterValues != null) value = parameterValues.get(parameter.getKey());
        ...
        if (parameter.isRequired()) {
          ...
          if (value == null) {
            ...
            if ((CalculationUtil.isPrimitiveWrapperClassName(datatype)
                    || String.class.getName().equals(datatype))
                && StringUtils.isBlank(value.toString())) {
              foundMissingValue = true;
            }
          }

          if (foundMissingValue) throw new MissingParameterException(parameter);
        }
    ...
    if (context == null) context = createCalculationContext();
    ...
    if (cohort.size() <= CalculationConstants.EVALUATION_BATCH_SIZE) {
      ...
      // we take cohort members one by one until we run out or hit the batch size, and evaluate the
      // calculation on those batches
      // We could achieve some negligible performance speedup by special-casing the case where
      // cohort instanceof List and using subList, but this doesn't seem worth it.
    ...
    if (cr == null) cr = new CalculationResultMap();
  ...
  public class SimplePatientCalculationContext implements PatientCalculationContext {
    ...
    /**
     * @see
     *     org.openmrs.calculation.patient.PatientCalculationContext#removeFromCache(java.lang.String)
     */
    @Override
