 ...
 * of it's evaluation should be done within an EvaluationInstanceData object, as these {@link
 * Calculation}s are not thread-safe
 *
...
public abstract class PatientAtATimeCalculation extends BaseCalculation
   ...
   */
  @Override
  public final CalculationResultMap evaluate(
      Collection<Integer> cohort,
      Map<String, Object> parameterValues,
      PatientCalculationContext context) {
    CalculationResultMap result = new CalculationResultMap();
    EvaluationInstanceData instanceData =
        preprocess(cohort, parameterValues, (PatientCalculationContext) context);
    try {
      for (Integer patientId : cohort) {
        result.put(
            patientId,
            evaluateForPatient(
                instanceData, patientId, parameterValues, (PatientCalculationContext) context));
      }
    } finally {
