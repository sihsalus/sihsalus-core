...
public class DyakuConditionMapper {
  ...
  public static final String PROFILE_CONDITION_PE =
      "https://www.gob.pe/minsa/RENHICE/fhir/StructureDefinition/ConditionPe";

  public static final String SYSTEM_CIE10 = "http://hl7.org/fhir/sid/icd-10";

  public static final String VALUE_SET_CIE10 =
      "https://www.gob.pe/minsa/RENHICE/fhir/ValueSet/CIE10VS";

  /** Convierte un Diagnóstico de OpenMRS a Condition FHIR R4 (Perfil ConditionPe) */
  public static org.hl7.fhir.r4.model.Condition toDyakuFhir(
      Diagnosis diagnosis, String patientReference) {
    if (diagnosis == null) {
      throw new IllegalArgumentException("Diagnosis no puede ser nulo");
    }

    log.info(
        "Convirtiendo Diagnosis ["
            + diagnosis.getDiagnosisId()
            + "] a Condition FHIR R4 (Perfil ConditionPe)");
  ...
  private static CodeableConcept mapCie10Code(Diagnosis diagnosis) {
    ...
    if (concept != null) {
      ...
      if (cie10Code != null && !cie10Code.isEmpty()) {
        ...
        code.addCoding()
            .setSystem(SYSTEM_CIE10)
            .setCode(concept.getUuid()) // Fallback
            .setDisplay(concept.getDisplayString());
      }
    } else {
      ...
      String nonCoded =
          diagnosis.getDiagnosis().getNonCoded() != null
  ...
  /**
   * Busca un mapeo CIE-10 en los ConceptMaps de un Concept Mejora: Busca por múltiples variantes de
        ...
        if (isCie10) {
          ...
          if (code != null && !code.isEmpty()) {
            ...
            if (isValidCie10Format(code)) {
              log.debug(
                  "✓ Mapeo CIE-10 encontrado: " + code + " para Concept: " + concept.getUuid());
              return code;
            } else {
              log.warn(
                  "⚠ Código CIE-10 con formato sospechoso: "
                      + code
                      + " (Concept: "
                      + concept.getUuid()
                      + ")");
          ...
          try {
            java.lang.reflect.Method getConceptReferenceTerms =
                conceptService
                    .getClass()
                    .getMethod(
                        "getConceptReferenceTerms", Concept.class, org.openmrs.ConceptSource.class);
