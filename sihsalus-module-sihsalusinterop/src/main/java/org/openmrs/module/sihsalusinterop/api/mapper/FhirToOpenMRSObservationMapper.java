...
public class FhirToOpenMRSObservationMapper {
  ...
  public static Obs mapToOpenMRS(
      org.hl7.fhir.r4.model.Observation fhirObservation, Patient patient, Encounter encounter) {
    Obs obs = new Obs();
  ...
  private static Concept findConceptByName(String conceptName, String loincCode) {
    ...
    try {
      ...
      if (conceptName != null) {
        ...
        if (conceptName.contains("pulse")
            || conceptName.contains("heart rate")
            || conceptName.contains("pulso")) {
          Concept pulse = Context.getConceptService().getConceptByName("Pulse");
          if (pulse != null) return pulse;
        }

        if (conceptName.contains("respiratory rate")
            || conceptName.contains("frecuencia respiratoria")) {
        ...
        if (conceptName.contains("height")
            || conceptName.contains("altura")
            || conceptName.contains("talla")) {
