...
public class DyakuProcedureMapper {
  ...
  public static final String SYSTEM_CPMS =
      "https://www.gob.pe/minsa/RENHICE/fhir/CodeSystem/CPMSCS";
   ...
   */
  public static Procedure toDyakuFhir(
      Order procedureOrder, String patientReference, String encounterReference) {
    if (procedureOrder == null) {
      throw new IllegalArgumentException("ProcedureOrder no puede ser nulo");
    }

    log.info(
        "Convirtiendo ProcedureOrder [" + procedureOrder.getOrderId() + "] a Procedure FHIR R4");
    ...
    if (procedureOrder.getVoided() != null && procedureOrder.getVoided()) {
      ...
      if (procedureOrder.getScheduledDate() != null
          && procedureOrder.getScheduledDate().before(now)) {
        if (procedureOrder.getAutoExpireDate() == null
            || procedureOrder.getAutoExpireDate().after(now)) {
          procedure.setStatus(Procedure.ProcedureStatus.INPROGRESS);
        } else {
  ...
  /**
   * Mapea el código CPMS desde el Concept del procedimiento Busca el mapeo CPMS en los ConceptMaps
   * del Concept
   */
  private static CodeableConcept mapCpmCode(Concept concept) {
    ...
    if (cpmsCode != null && !cpmsCode.isEmpty()) {
      // Agregar código CPMS
      code.addCoding()
          .setSystem(SYSTEM_CPMS)
          .setCode(cpmsCode)
          .setDisplay(concept.getDisplayString());
      code.setText(concept.getDisplayString());
      log.info(
          "✓ Código CPMS mapeado: " + cpmsCode + " para Concept: " + concept.getDisplayString());
    } else {
        ...
        if (isCpms) {
          ...
          if (code != null && !code.isEmpty()) {
            ...
            if (isValidCpmsFormat(code)) {
              ...
              log.warn(
                  "⚠ Código CPMS con formato sospechoso: "
                      + code
                      + " (Concept: "
                      + concept.getUuid()
                      + ")");
