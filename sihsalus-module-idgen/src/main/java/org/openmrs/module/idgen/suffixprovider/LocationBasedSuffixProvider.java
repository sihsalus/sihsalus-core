...
public class LocationBasedSuffixProvider implements GlobalPropertyListener, SuffixProvider {
  ...
  public static final String SUFFIX_LOCATION_ATTRIBUTE_TYPE_GP =
      "idgen.suffixLocationAttributeType";
  ...
  public String getLocationSuffix(Location location) {
    if (location != null) {
      ...
      // This means we either reached the top of the location without a valid suffix found or there
      // is no parent Location
  ...
  public static String getSuffixLocationAttributeType() {
    if (suffixLocationAttributeType == null) {
      suffixLocationAttributeType =
          Context.getAdministrationService().getGlobalProperty(SUFFIX_LOCATION_ATTRIBUTE_TYPE_GP);
    }
