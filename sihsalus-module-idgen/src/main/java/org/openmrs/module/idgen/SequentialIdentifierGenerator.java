...
public class SequentialIdentifierGenerator extends BaseIdentifierSource {
  ...
  // ***** PROPERTIES *****
  private Long
      nextSequenceValue; // not used: declared only so that Hibernate creates the column when

  // running tests

  /**
   * A prefix can either be configurable or static. Configurable prefixes provide the {@link
   * PrefixProvider} bean name which will be used as a prefix source; Example:
   *
   * <blockquote>
   *
   * <pre>
  ...
  /**
   * A suffix can either be configurable or static. Configurable suffixes provide the {@link
   * SuffixProvider} bean name which will be used as a suffix source; Example:
   *
   * <blockquote>
   *
   * <pre>
  ...
  /**
   * A prefix expected in a configured {@code prefix}. If found in the prefix, it means it entails a
   * {@link PrefixProvider} Bean name.
   */
  ...
  public boolean isInitialized() {
    Long nextSequenceValue =
        Context.getService(IdentifierSourceService.class).getSequenceValue(this);
  ...
  public String getIdentifierForSeed(long seed) {
    ...
    if (getIdentifierType() != null && StringUtils.isNotEmpty(getIdentifierType().getValidator())) {
      try {
        ...
        throw new RuntimeException(
            "Error generating check digit with " + getIdentifierType().getValidator(), e);
      }
    }
    ...
    if (this.minLength != null && this.minLength > 0) {
      if (identifier.length() < this.minLength) {
        throw new RuntimeException(
            "Invalid configuration for IdentifierSource. Length minimum set to "
                + this.minLength
                + " but generated "
                + identifier);
      }
    ...
    if (this.maxLength != null && this.maxLength > 0) {
      if (identifier.length() > this.maxLength) {
        throw new RuntimeException(
            "Invalid configuration for IdentifierSource. Length maximum set to "
                + this.maxLength
                + " but generated "
                + identifier);
      }
  ...
  /**
   * @param nextSequenceValue : set the next identifier to be generated for this
   ...
   *
   * @should return {@link ConstantPrefixProvider} based on {@code prefix} if a static prefix is
   *     provided.
   * @should return {@link ConstantPrefixProvider} based on a blank prefix if an null or blank
   *     prefix is provided
   * @should return {@link ConstantPrefixProvider} if the configured prefix provider cannot be
   *     Spring wired.
    ...
    if (prefix.startsWith(CONFIGURATION_PREFIX)) {
      ...
      try {
        ...
        throw new APIException(
            "Invalid prefix configuration. The "
                + PrefixProvider.class.getSimpleName()
                + " bean name ('"
                + beanName
   ...
   *
   * @should return {@link ConstantSuffixProvider} based on {@code suffix} if a static suffix is
   *     provided.
   * @should return {@link ConstantSuffixProvider} based on a blank suffix if an null or blank
   *     suffix is provided
   * @should return {@link ConstantSuffixProvider} if the configured suffix provider cannot be
   *     Spring wired.
  ...
  public SuffixProvider getSuffixProvider(String suffix) throws APIException {
    ...
    if (suffix.startsWith(CONFIGURATION_PREFIX)) {
      ...
      try {
        ...
        throw new APIException(
            "Invalid suffix configuration. The "
                + SuffixProvider.class.getSimpleName()
                + " bean name ('"
                + beanName
                + "') could not be resolved.",
            e);
