...
public interface IdentifierSourceService extends OpenmrsService {
   ...
   */
  @Authorized
  Map<PatientIdentifierType, List<IdentifierSource>> getIdentifierSourcesByType(
  ...
  /**
   * Retires the IdentifierSource, leaving it in the database, but removing it from data entry
   * screens
   *
  ...
  /**
   * Given a PatientIdentifierType, generates an identifier using the proper IdentifierSource for
   * this server Returns null if this PatientIdentifierType is not set to be auto-generated
   */
  ...
  /**
   * Given a PatientIdentifierType and Location, generates an identifier using the proper
   * IdentifierSource Returns null if this PatientIdentifierType is not set to be auto-generated
   */
  @Authorized(PrivilegeConstants.EDIT_PATIENT_IDENTIFIERS)
  ...
  List<String> generateIdentifiers(IdentifierSource source, Integer batchSize, String comment)
      throws APIException;
  ...
  @Authorized
  List<PooledIdentifier> getAvailableIdentifiers(IdentifierPool pool, int quantity)
      throws APIException;

  /** Returns Pooled Identifiers for the given source, with the given status options */
  @Authorized
  int getQuantityInPool(IdentifierPool pool, boolean availableOnly, boolean usedOnly)
      throws APIException;
   ...
   */
  @Authorized
  AutoGenerationOption getAutoGenerationOption(PatientIdentifierType type, Location location)
      throws APIException;
   ...
   */
  @Authorized
  List<AutoGenerationOption> getAutoGenerationOptions(PatientIdentifierType type)
      throws APIException;
  ...
  /**
   * Retrieves the Log Entries that match the supplied parameters. All parameters are optional. The
   * identifier and comment parameters do a "like" match, the date parameters ignore time
   */
  @Authorized
  ...
  /**
   * Retrieves the most recent Log Entry for the given source, based on generation date and auto
   * incremented id
   *
  ...
  /**
   * Convenience method that checks a pool's level and incrementally adds identifiers in batches of
   * 100 until the min available size is reached. Only does something if the pool's source is a
   * remote or sequential generator.
   *
   * @param pool
   */
  @Authorized
  ...
  /**
   * Convenience method that returns the set of Patient Identifier Types that match certain
   * AutoGeneration parameters
   */
  @Authorized
  List<PatientIdentifierType> getPatientIdentifierTypesByAutoGenerationOption(
   ...
   *
   * @param patientIdentifierType the Patient Identifier Type used to retrieve the Identifier
   *     Source(s).
  ...
  /**
   * Internal method for generating identifiers; this should never be called directly--exposing it
   * here is a hack, but because evidently the @Transactional annotation will only be picked up by
   * service methods and we need this method to be transactional
   */
  @Authorized
