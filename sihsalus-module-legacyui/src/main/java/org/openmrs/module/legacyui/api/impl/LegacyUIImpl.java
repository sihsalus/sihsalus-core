...
public class LegacyUIImpl extends BaseOpenmrsService implements LegacyUIService {
   ...
   * main/java/org/openmrs/api/impl/PatientServiceImpl.java#L1092 This is the way to establish that
   * a patient has left the care center. This API call is responsible for:
   *
   * <ol>
  ...
  public void exitFromCare(Patient patient, Date dateExited, Concept reasonForExit)
    ...
    if (dateExited == null) {
      throw new APIException(
          "Must supply a valid dateExited when indicating that a patient has left care");
    }
  ...
  public void triggerStateConversion(Patient patient, Concept trigger, Date dateConverted) {
    ...
    if (trigger == null) {
      throw new APIException(
          "Attempting to convert state for a patient without a valid trigger concept");
    }
  ...
  public Provider getProviderForUser(User user) {
    ...
    if (providers.isEmpty()) {
      throw new IllegalStateException(
          "User " + user + " has no provider accounts, unable to create orders");
    }
   ...
   */
  public void discontinueAllDrugOrders(
      Patient patient, Concept discontinueReason, Date discontinueDate) {
    if (log.isDebugEnabled())
      log.debug(
          "In discontinueAll with patient "
              + patient
              + " and concept "
              + discontinueReason
              + " and date "
    ...
    List<Order> drugOrders =
        orderService.getOrders(
            patient,
            orderService.getCareSetting(2),
            Context.getOrderService().getOrderType(drugOrderType),
            false);
    // loop over all of this patient's drug orders to discontinue each
    if (drugOrders != null) {
      for (Order drugOrder : drugOrders) {
        if (log.isDebugEnabled()) log.debug("discontinuing order: " + drugOrder);
        // do the stuff to the database
        if (drugOrder.isActive() || drugOrder.getEffectiveStopDate() == null) {
          try {
            Provider provider =
                Context.getService(LegacyUIService.class)
