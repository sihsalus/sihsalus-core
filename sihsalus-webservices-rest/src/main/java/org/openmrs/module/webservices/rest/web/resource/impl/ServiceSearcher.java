...
public class ServiceSearcher<T> {
  ...
  public ServiceSearcher(
      Class<? extends OpenmrsService> serviceClass, String searchMethod, String countMethod) {
  ...
  /**
   * Makes service calls to get the count and search results for the given query, and packages those
   * up as an AlreadyPaged search result
   *
   * @param query
   * @param context
   * @return
  ...
  private boolean hasRightParameterTypes(Method method, int expectedStrings, int expectedIntegers) {
    ...
    for (Class<?> clazz : method.getParameterTypes()) {
      if (clazz.equals(String.class)) ++strings;
      else if (clazz.equals(Integer.class)) ++integers;
      else if (!clazz.equals(boolean.class) && !clazz.equals(Boolean.class)) return false;
    }
    return strings == expectedStrings && integers == expectedIntegers;
  }

  /**
   * Invokes a paged search method, using query as its String argument, and the context's startIndex
   * and limit to the first two Integer arguments
   *
   * @param service
   * @param method
   * @param query
  ...
  private List<T> invokePagedSearchMethod(
      OpenmrsService service, Method method, String query, RequestContext context)
      throws Exception {
