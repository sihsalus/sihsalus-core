...
public class RestConstants {
  ...
  /**
   * The absolute number of results to limit lists of objects to, even if the call requests a larger
   * list.
   *
   ...
   */
  public static String MAX_RESULTS_ABSOLUTE_GLOBAL_PROPERTY_NAME =
      MODULE_ID + ".maxResultsAbsolute";

  /**
   * The key of the global property that an admin can set to restrict ws users based on a range of
   * IPs. Should be a comma separated list of IP addresses. "*" in any part of it denotes a wildcard
   * match.
   */
  ...
  /**
   * An optional request parameter used by methods that return lists of patients to cut down on the
  ...
  /**
   * An optional request parameter used by rest methods. Will change the properties on the results.
   * Default is "default"
   *
  ...
  /**
   * An optional request parameter usable with resources that represent class hierarchies. Indicates
   * that you only want results from a specific subclass
   */
  ...
  /**
   * Global property name used to enable or disable the inclusion of stack trace details in the
   * error response.
   *
   * <p>When this property is set to 'true', stack trace details will be included in error
   * responses. When set to 'false', stack trace details will be omitted.
   */
  public static String ENABLE_STACK_TRACE_DETAILS_GLOBAL_PROPERTY_NAME =
      MODULE_ID + ".enableStackTraceDetails";
}
