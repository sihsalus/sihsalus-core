 ...
 * context applied. Typically this will be used by implementations of {@link Searchable} and {@link
 * Listable} that do not have a native query capable of doing a page-limited search
 *
...
public class NeedsPaging<T> extends BasePageableResult<T> {
  ...
  /**
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.impl.BasePageableResult#getPageOfResults()
   */
  @Override
  public List<T> getPageOfResults() {
    if (context.getStartIndex() == 0 && context.getLimit() >= unpagedResults.size()) {
      ...
      if (endIndex > unpagedResults.size()) endIndex = unpagedResults.size();
