...
public class DynamicBeanConfiguration {
   ...
   * configuration method allows the module to run on both pre and post 4.1.0 versions of Spring by
   ...
   * Spring 5 The recommended replacement class ExceptionHandlerExceptionResolver was introduced in
   * Spring 3.1.0 which is not available on OpenMRS platform versions 1.9.x and 1.10.x which run
   * Spring 3.0.5 That's why we can't just statically replace this class in the
  ...
  public AbstractHandlerExceptionResolver getHandlerExceptionResolver() throws Exception {
    ...
    HttpMessageConverter<?> stringHttpMessageConverter =
        Context.getRegisteredComponent("stringHttpMessageConverter", HttpMessageConverter.class);

    HttpMessageConverter<?> jsonHttpMessageConverter =
        Context.getRegisteredComponent("jsonHttpMessageConverter", HttpMessageConverter.class);

    HttpMessageConverter<?> xmlMarshallingHttpMessageConverter =
    ...
    try {
      ...
      Method method =
          bean.getClass().getMethod("setMessageConverters", HttpMessageConverter[].class);
      method.invoke(
          bean,
          new Object[] {
  ...
  /**
   * The DefaultAnnotationHandlerMapping class was deprecated and eventually removed in Spring 5 The
   * recommended replacement class RequestMappingHandlerMapping was introduced in Spring 3.1.0 which
   * is not available on OpenMRS platform versions 1.9.x and 1.10.x which run Spring 3.0.5 That's
   * why we can't just statically replace this class in the webModuleApplicationContext.xml file.
   */
  @Bean
