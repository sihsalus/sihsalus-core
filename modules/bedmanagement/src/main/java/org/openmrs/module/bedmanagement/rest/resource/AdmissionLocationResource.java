...
@Resource(
    name = RestConstants.VERSION_1 + "/admissionLocation",
    supportedClass = AdmissionLocation.class,
...
public class AdmissionLocationResource extends DelegatingCrudResource<AdmissionLocation> {
  ...
  protected PageableResult doGetAll(RequestContext context) throws ResponseException {
    List<AdmissionLocation> admissionLocations =
        Context.getService(BedManagementService.class).getAdmissionLocations();
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation) {
      modelImpl
          .property("ward", new StringProperty())
          .property("totalBeds", new StringProperty())
    ...
    if (rep instanceof FullRepresentation) {
      modelImpl
          ...
          .property("totalBeds", new StringProperty())
          .property("occupiedBeds", new StringProperty())
          .property("bedLayouts", new StringProperty());
    }
    if (rep instanceof NamedRepresentation) {
      modelImpl
          .property("ward", new StringProperty())
          .property("bedLocationMappings", new StringProperty());
    }
  ...
  public Object getBedLocationMappings(AdmissionLocation admissionLocation) throws Exception {
    List<BedLocationMapping> bedLocationMappings =
        Context.getService(BedManagementService.class)
    ...
    for (BedLocationMapping bedLocationMapping : bedLocationMappings) {
      ...
      object.put(
          "bedId",
          bedLocationMapping.getBed() != null ? bedLocationMapping.getBed().getId() : null);
      object.put(
          "bedUuid",
          bedLocationMapping.getBed() != null ? bedLocationMapping.getBed().getUuid() : null);
      object.put(
          "status",
          bedLocationMapping.getBed() != null ? bedLocationMapping.getBed().getStatus() : null);
      object.put(
          "bedType",
  ...
  public Object create(SimpleObject propertiesToCreate, RequestContext context)
    ...
    LocationTag admissionLocationTag =
    ...
    if (propertiesToCreate.get("name") == null
        || StringUtils.isBlank((String) propertiesToCreate.get("name"))) {
    ...
    if (propertiesToCreate.get("parentLocationUuid") != null) {
      ...
      if (parentLocation == null) throw new IllegalPropertyException("Parent location not exist");
  ...
  public Object update(String uuid, SimpleObject propertiesToUpdate, RequestContext context)
      throws ResponseException {
    Location location = Context.getService(LocationService.class).getLocationByUuid(uuid);
    if (location == null) throw new IllegalPropertyException("Location not exist");

    AdmissionLocation admissionLocation =
        Context.getService(BedManagementService.class).getAdmissionLocationByLocation(location);
    ...
    if (propertiesToUpdate.get("bedLayout") != null) {
      Object bedLayoutProperty = propertiesToUpdate.get("bedLayout");
      if (!(bedLayoutProperty instanceof Map)) {
        throw new ConversionException("The bedLayout property must be an object");
      }
      Map<?, ?> bedLayout = (Map<?, ?>) bedLayoutProperty;
      Integer row = toInteger(bedLayout.get("row"), "row");
      Integer column = toInteger(bedLayout.get("column"), "column");
      Context.getService(BedManagementService.class)
          .setBedLayoutForAdmissionLocation(admissionLocation, row, column);
    }
  ...
  protected void delete(
      AdmissionLocation admissionLocation, String reason, RequestContext requestContext)
      throws ResponseException {
  ...
  @Override
  public void purge(AdmissionLocation admissionLocation, RequestContext requestContext)
      throws ResponseException {
  ...
  private Integer toInteger(Object value, String propertyName) {
    if (value == null) {
      throw new ConversionException("The bedLayout." + propertyName + " property is missing");
    }
    ...
    try {
      ...
      throw new ConversionException(
          "The bedLayout." + propertyName + " property must be an integer");
    }
  }
}
