...
@Resource(
    name = RestConstants.VERSION_1 + "/bedtype",
    supportedClass = BedType.class,
...
public class BedTypeResource extends DelegatingCrudResource<BedType> {
  ...
  @Override
  protected void delete(BedType bedType, String reason, RequestContext context)
      throws ResponseException {
  ...
  private BedType constructBedType(String uuid, SimpleObject properties) {
    ...
    if (uuid != null) {
      ...
      if (bedType == null) throw new IllegalPropertyException("Bed Type not exist");

      if (properties.get("name") != null) bedType.setName((String) properties.get("name"));
