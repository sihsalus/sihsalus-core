...
public interface BedManagementDao {
   ...
   * <i>admission</i> locations. This method returns the admission location, if applicable, when
   * given a candidate location.
   *
   ...
   */
  BedLocationMapping getBedLocationMappingByLocationAndRowAndColumn(
   ...
   * Get beds.
   *
   * @param location {@link Location} admission location, if null filter by location criteria will
   *     not applied
   * @param bedType {@link BedType} bed type, if null filter by bed type criteria will not applied
   * @param status {@link BedStatus} bed status, if filter by null bed type criteria will not
   *     applied
