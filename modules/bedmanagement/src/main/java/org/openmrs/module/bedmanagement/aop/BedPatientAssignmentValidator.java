...
public class BedPatientAssignmentValidator implements Validator {
  ...
  @Autowired
  public BedPatientAssignmentValidator(
  ...
  public void validate(Object target, Errors errors) {
    if (!(target instanceof BedPatientAssignment)) {
      throw new IllegalArgumentException(
          "the parameter target must be of type " + BedPatientAssignment.class);
    }
    ...
    if (bpa.getEndDatetime() == null) {
      List<BedPatientAssignment> currentAssignments =
          bedManagementService.getBedPatientAssignmentByPatient(patient.getUuid(), false);
      ...
      if (isAttemptingMultipleAssignments) {
        errors.rejectValue(
            "patient",
            "bedPatientAssignment.patient.alreadyAssigned",
