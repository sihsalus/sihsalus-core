...
public class ImplementerConfiguredCohortDefinitionLibraryTest
  ...
  @Autowired ImplementerConfiguredCohortDefinitionLibrary library;
  ...
  public void testSqlDefinition() throws Exception {
    CohortDefinition definition =
        library.getDefinition("configuration.definitionlibrary.cohort.femalesSql");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlCohortDefinition.class));
    assertThat(
        ((SqlCohortDefinition) definition).getQuery().trim(),
        is("select person_id from person where gender = 'F'"));
  }

  @Test
  public void testSerializedDefinition() throws Exception {
    CohortDefinition definition =
        library.getDefinition("configuration.definitionlibrary.cohort.femalesXml");
    assertThat(definition, notNullValue());
  ...
  public void testGroovyDefinition() throws Exception {
    CohortDefinition definition =
        library.getDefinition("configuration.definitionlibrary.cohort.femalesGroovy");
    assertThat(definition, notNullValue());
