...
public class ImplementerConfiguredVisitDataDefinitionLibraryTest
  ...
  @Autowired ImplementerConfiguredVisitDataDefinitionLibrary library;
  ...
  public void testSqlDefinition() throws Exception {
    VisitDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.visitData.patientIdSql");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlVisitDataDefinition.class));
    assertThat(
        ((SqlVisitDataDefinition) definition).getSql().trim(),
        is("select visit_id, patient_id from visit"));
  }

  @Test
  public void testSerializedDefinition() throws Exception {
    VisitDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.visitData.visitIdXml");
    assertThat(definition, notNullValue());
