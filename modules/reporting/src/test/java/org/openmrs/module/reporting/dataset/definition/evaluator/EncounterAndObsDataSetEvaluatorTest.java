...
public class EncounterAndObsDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should contain all obs values for each encounter",
  ...
  public void evaluate_shouldContainAllObsValuesForEachEncounter() throws Exception {
    ...
    SimpleDataSet result =
        (SimpleDataSet)
            dataSetDefinitionService.evaluate(createEncounterAndObsDataSetDefinition(), null);
    ...
    // There are two weight concept names for locale en { WT, WEIGHT (KG) } in
    ...
    String columnName =
        ObjectUtil.format(wt).replaceAll("\\s", "_").replaceAll("-", "_").toUpperCase();
