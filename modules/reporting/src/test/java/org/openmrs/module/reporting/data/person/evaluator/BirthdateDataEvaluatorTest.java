...
public class BirthdateDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void evaluate_shouldReturnAllBirthDatesForAllPersons() throws Exception {
    ...
    Assert.assertEquals(
        "1975-04-08",
        DateUtil.formatDate(((Birthdate) pd.getData().get(2)).getBirthdate(), "yyyy-MM-dd"));
    Assert.assertEquals(
        "2007-05-27",
        DateUtil.formatDate(((Birthdate) pd.getData().get(6)).getBirthdate(), "yyyy-MM-dd"));
    Assert.assertEquals(
        "1976-08-25",
        DateUtil.formatDate(((Birthdate) pd.getData().get(7)).getBirthdate(), "yyyy-MM-dd"));
