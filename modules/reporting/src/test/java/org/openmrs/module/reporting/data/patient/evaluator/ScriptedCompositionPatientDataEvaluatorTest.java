...
public class ScriptedCompositionPatientDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  /**
   * @see
   *     ScriptedCompositionPatientDataDefinitionEvaluator#evaluate(PatientDataDefinition,EvaluationContext)
   * @verifies return the number of days since last encounter of the specified types from a
   *     specified date parameter
   */
  @Test
  public void evaluate_shouldReturnNumberOfDaysSinceLastEncounterOfTheSpecifiedTypes()
      throws Exception {
    ...
    ScriptedCompositionPatientDataDefinition daysSinceLastVisit =
        new ScriptedCompositionPatientDataDefinition();
    daysSinceLastVisit.setScriptType(new ScriptingLanguage("groovy"));
    daysSinceLastVisit.setScriptCode(script);
    daysSinceLastVisit
        .getContainedDataDefinitions()
        .put("patientLastVisit", new Mapped<PatientDataDefinition>(lastEncounter, null));

    EvaluatedPatientData pd =
        Context.getService(PatientDataService.class).evaluate(daysSinceLastVisit, context);
  ...
  public void evaluate_shouldReturnSpecifiedAlertBasedOnLastWeightValue() throws Exception {
    ...
    EvaluatedPatientData daysSinceLastVisitResult =
        Context.getService(PatientDataService.class).evaluate(daysSinceLastVisit, context);

    Assert.assertEquals("Normal", daysSinceLastVisitResult.getData().get(7));
    Assert.assertEquals(
        "The recorded weight value might be incorrect!",
        daysSinceLastVisitResult.getData().get(20));
    Assert.assertEquals("High", daysSinceLastVisitResult.getData().get(21));
    Assert.assertEquals(
        "The recorded weight value might be incorrect!",
        daysSinceLastVisitResult.getData().get(22));
  }
}
