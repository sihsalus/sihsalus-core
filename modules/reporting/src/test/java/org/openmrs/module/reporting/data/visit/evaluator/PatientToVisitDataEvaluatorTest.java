...
public class PatientToVisitDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Autowired
  @Qualifier("reportingVisitDataService")
  ...
  public void evaluate_shouldReturnPatientDataForEachVisitInThePassedContext() throws Exception {
    ...
    context.setBaseVisits(
        new VisitIdSet(1, 2, 4)); // in our demo set include two visits for same patient
    EvaluatedVisitData ed = Context.getService(VisitDataService.class).evaluate(d, context);

    assertThat(ed.getData().size(), is(3));
    assertThat(
        (PatientIdentifier) ed.getData().get(1),
        is(patientService.getPatient(2).getPatientIdentifier(pit)));
    assertThat(
        (PatientIdentifier) ed.getData().get(2),
        is(patientService.getPatient(2).getPatientIdentifier(pit)));
    assertThat(
        (PatientIdentifier) ed.getData().get(4),
        is(patientService.getPatient(6).getPatientIdentifier(pit)));
  }
