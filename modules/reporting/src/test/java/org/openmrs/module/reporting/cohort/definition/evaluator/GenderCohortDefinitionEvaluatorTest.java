...
public class GenderCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should return all non voided patients when all are included",
  ...
  public void evaluate_shouldReturnAllNonVoidedPatientsWhenAllAreIncluded() throws Exception {
    ...
    Cohort cohort =
        Context.getService(CohortDefinitionService.class).evaluate(genderCohortDefinition, null);
    Assert.assertEquals(9, cohort.getSize());
    Assert.assertTrue(
        "Should not include patient 999 whose record has been voided", !cohort.contains(999));
  }
  ...
  @Test
  @Verifies(
      value = "should return male patients when males are included",
      method = "evaluate(CohortDefinition, EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should return female patients when females are included",
      method = "evaluate(CohortDefinition, EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should return patients with unknown gender when unknown are included",
      method = "evaluate(CohortDefinition, EvaluationContext)")
  public void evaluate_shouldReturnPatientsWithUnknownGenderWhenUnknownAreIncluded()
      throws Exception {
    GenderCohortDefinition genderCohortDefinition = new GenderCohortDefinition();
    genderCohortDefinition.setUnknownGenderIncluded(true);
    Cohort cohort =
  ...
  @Test
  @Verifies(
      value = "@should return no patients when none are included",
      method = "evaluate(CohortDefinition, EvaluationContext)")
