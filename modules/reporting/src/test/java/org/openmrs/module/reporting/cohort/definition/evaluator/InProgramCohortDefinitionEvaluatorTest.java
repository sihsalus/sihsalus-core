...
public class InProgramCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should return patients enrolled in the given programs on or before the given date",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsEnrolledInTheGivenProgramsOnOrBeforeTheGivenDate()
      throws Exception {
  ...
  @Verifies(
      ...
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldFindPatientsInAProgramOnTheOnOrBeforeDateIfPassedInTimeIsAtMidnight()
      throws Exception {
    PatientProgram pp = ps.getPatientProgram(7);
    Assert.assertNull(pp.getDateCompleted());
  ...
  @Test
  @Verifies(
      value = "should return patients enrolled in the given programs on or after the given date",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsEnrolledInTheGivenProgramsOnOrAfterTheGivenDate()
      throws Exception {
    PatientProgram pp = ps.getPatientProgram(7);
  ...
  public void evaluate_shouldReturnPatientsEnrolledAtEvaluationDateIfNoOtherDatesSupplied()
    ...
    Cohort c =
        Context.getService(CohortDefinitionService.class)
            .evaluate(cd, new EvaluationContext(DateUtil.getDateTime(2000, 1, 1)));
    Assert.assertEquals(0, c.getSize());
    c =
        Context.getService(CohortDefinitionService.class)
            .evaluate(cd, new EvaluationContext(DateUtil.getDateTime(2009, 1, 1)));
    Assert.assertTrue(c.contains(2));
