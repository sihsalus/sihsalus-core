...
/**
 * This tests the evaluation of an AgeCohortDefinition Patients in standard test dataset: 2: not
 * voided, birthdate: 1975-04-08 6: not voided, birthdate: 2007-05-27 7: not voided, birthdate:
 * 1976-08-25 8: not voided, birthdate: null 999: voided, birthdate: null
 */
public class AgeCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {

  @Autowired TestDataManager tdf;
  ...
  public void evaluate_shouldReturnOnlyNonVoidedPatients() throws Exception {
    Cohort cohort =
        Context.getService(CohortDefinitionService.class).evaluate(new AgeCohortDefinition(), null);
  ...
  /**
   * Private utility method that contains necessary logic for testing various combinations of age
   * calculations
   *
   * @param numPats
   * @param minAge
   ...
   */
  private void testAgeRange(
      int numPats,
      Integer minAge,
      Integer maxAge,
      boolean unknown,
      String ymdEffectiveDate,
      DurationUnit ageUnits)
      throws EvaluationException {
  ...
  private Cohort evaluate(AgeCohortDefinition acd, EvaluationContext context)
      throws EvaluationException {
