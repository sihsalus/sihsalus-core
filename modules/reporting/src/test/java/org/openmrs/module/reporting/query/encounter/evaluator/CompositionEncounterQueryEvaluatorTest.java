...
public class CompositionEncounterQueryEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public CompositionEncounterQuery getBaseDefinition() {
    ...
    ccd.addSearch(
        "c1",
        Mapped.noMappings(
            new SqlEncounterQuery(
                "select encounter_id from encounter where encounter_id in (3,4,5,6)")));
    ccd.addSearch(
        "c2",
        Mapped.noMappings(
            new SqlEncounterQuery(
                "select encounter_id from encounter where encounter_id in (7,8,9,10)")));
    ccd.addSearch(
        "c3",
        Mapped.noMappings(
            new SqlEncounterQuery(
                "select encounter_id from encounter where encounter_id in (5,6,7,8)")));
    return ccd;
  }
  ...
  public void testComposition(String compositionString, Integer... expectedIds) throws Exception {
    ...
    EncounterQueryResult r =
        Context.getService(EncounterQueryService.class).evaluate(ccd, new EvaluationContext());
    if (expectedIds == null) {
