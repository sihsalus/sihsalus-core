...
public class ObsForPersonDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void evaluate_shouldLimitObsByForm() throws Exception {
    ...
    // By limiting by a first form (with form_id="3") we shouldn't get any Obs because there is no
    // encounter with form_id="3" in our test dataset
    {
      ObsForPersonDataDefinition d = new ObsForPersonDataDefinition();
      d.setQuestion(Context.getConceptService().getConcept(5089));
    ...
    // By limiting by a second form (with form_id="2") we get back 3 Obs because all encounters in
    // our test dataset for the specified Obs question have been entered through this form
    {
      ObsForPersonDataDefinition d = new ObsForPersonDataDefinition();
      d.setQuestion(Context.getConceptService().getConcept(5089));
