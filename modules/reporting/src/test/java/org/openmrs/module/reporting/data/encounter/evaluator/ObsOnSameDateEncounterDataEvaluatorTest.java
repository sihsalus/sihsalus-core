...
public class ObsOnSameDateEncounterDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void testEvaluateForObsOnDifferentEncounters() throws Exception {
    ...
    assertThat(results.getData().size(), is(1));
    // it should return the Weight obs captured on the same date but in different encounter than the
    // context encounter
  ...
  public void testEvaluateForObsWithAnswer() throws Exception {
    ...
    Encounter enc2 =
        data.randomEncounter()
            .encounterDatetime(enc1.getEncounterDatetime())
            .patient(patient)
            .save();
    ...
    // First show that a normal ObsForEncounterDataDefinition would not include the obs from
    // different encounter on same date
