...
public class InverseCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should return all patients who are not in the inner cohort definition",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnAllPatientsWhoAreNotInTheInnerCohortDefinition()
      throws Exception {
    ...
    Cohort nonMaleCohort =
        Context.getService(CohortDefinitionService.class).evaluate(nonMales, null);
    Cohort femaleOrUnknownCohort =
        Context.getService(CohortDefinitionService.class).evaluate(femaleOrUnknown, null);
  ...
  @Test
  @Verifies(
      value = "should successfully use the context base cohort",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldSuccessfullyUseTheContextBaseCohort() throws Exception {
    ...
    Cohort childrenCohort =
        Context.getService(CohortDefinitionService.class).evaluate(children, null);
    ...
    Assert.assertEquals(
        5, Context.getService(CohortDefinitionService.class).evaluate(nonChildren, null).size());

    // Inverse Children, base cohort
    Assert.assertEquals(
        2, Context.getService(CohortDefinitionService.class).evaluate(nonChildren, context).size());
  }
}
