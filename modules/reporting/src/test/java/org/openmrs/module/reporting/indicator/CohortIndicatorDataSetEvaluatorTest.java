...
public class CohortIndicatorDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void shouldEvaluteIndicatorWithNoParameters() throws Exception {
    ...
    CohortDimensionResult results =
        (CohortDimensionResult)
            Context.getService(DimensionService.class).evaluate(gender, new EvaluationContext());

    InProgramCohortDefinition inProgram = new InProgramCohortDefinition();
    inProgram.setPrograms(
        Collections.singletonList(Context.getProgramWorkflowService().getProgram(2)));

    CohortIndicator ind =
        CohortIndicator.newCountIndicator(
            "In HIV Program", new Mapped<InProgramCohortDefinition>(inProgram, null), null);
    ...
    dsd.addColumn(
        "1.b", "Females in program", new Mapped<CohortIndicator>(ind, null), "gender=female");

    MapDataSet ds =
        (MapDataSet) Context.getService(DataSetDefinitionService.class).evaluate(dsd, null);
    DataSetRow row = ds.getData();

    Assert.assertEquals(
        2, ((IndicatorResult) ds.getData().getColumnValue("1")).getValue().intValue());
    Assert.assertEquals(
        1, ((IndicatorResult) ds.getData().getColumnValue("1.a")).getValue().intValue());
    Assert.assertEquals(
        1, ((IndicatorResult) ds.getData().getColumnValue("1.b")).getValue().intValue());
  }
}
