...
public class SimplePatientDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should evaluate a SimplePatientDataSetDefinition",
  ...
  public void evaluate_shouldEvaluateASimplePatientDataSetDefinition() throws Exception {
    ...
    d.addIdentifierType(
        Context.getPatientService().getPatientIdentifierTypeByName("Old Identification Number"));
    ...
    for (DataSetRow row : result.getRows()) {
      ...
      Assert.assertTrue(
          ObjectUtil.areEqualStr(
              p.getPatientIdentifier("Old Identification Number"),
              row.getColumnValue("Old Identification Number")));
      ...
      Object attVal =
          p.getAttribute("Birthplace") == null
              ? null
              : p.getAttribute("Birthplace").getHydratedObject();
