...
public class CohortDetailReportRendererTest extends BaseModuleContextSensitiveTest {
  ...
  public void shouldRenderIndicatorsWithDifferentDatasets() throws Exception {
    ...
    CohortIndicator numberOfMales =
        CohortIndicator.newCountIndicator("Males", new Mapped<CohortDefinition>(males, null), null);
    numberOfMales.setParameters(IndicatorUtil.getDefaultParameters());
    CohortIndicator numberOfFemales =
        CohortIndicator.newCountIndicator(
            "Females", new Mapped<CohortDefinition>(females, null), null);
    ...
    // Then, we construct a Map from indicator number to DataSetDefinition we wish to use to view
    // it's underlying Cohort

    Map<String, Mapped<? extends DataSetDefinition>> m =
        new HashMap<String, Mapped<? extends DataSetDefinition>>();
    ...
    resource.setName(
        "designFile"); // Note: You must name your resource exactly like this for it to work
    ...
    CohortDetailReportRenderer renderer =
        new CohortDetailReportRenderer() {
    ...
    outFile =
        System.getProperty("java.io.tmpdir")
            + File.separator
            + "cohortDetailReportRendererTest.xls";
