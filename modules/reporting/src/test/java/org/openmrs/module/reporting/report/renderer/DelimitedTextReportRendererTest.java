...
public class DelimitedTextReportRendererTest extends BaseModuleContextSensitiveTest {
  ...
  public void getRenderedContentType_shouldBeZipIfMoreThanOneDataSet() throws Exception {
    ...
    assertThat(
        renderer.getRenderedContentType(requestFor(reportDefinitionWithTwoDSDs())),
        is("application/zip"));
  }
  ...
  public void getRenderedContentType_shouldBeCsvIfOneDataSet() throws Exception {
    DelimitedTextReportRenderer renderer = new CsvReportRenderer();
    assertThat(
        renderer.getRenderedContentType(requestFor(reportDefinitionWithOneDSD())), is("text/csv"));
  }
  ...
  public void getFilename_shouldBeZipIfMoreThanOneDataSet() throws Exception {
    ...
    assertTrue(
        Pattern.matches(
            "Testing_.*\\.zip", renderer.getFilename(requestFor(reportDefinitionWithTwoDSDs()))));
  }
  ...
  public void getFilename_shouldBeCsvIfOneDataSet() throws Exception {
    ...
    assertTrue(
        Pattern.matches(
            "Testing_.*\\.csv", renderer.getFilename(requestFor(reportDefinitionWithOneDSD()))));
  }
  ...
  public void render_shouldWritePlainTextIfOneDataSet() throws Exception {
    DelimitedTextReportRenderer renderer = new CsvReportRenderer();
    ReportData data =
        reportDefinitionService.evaluate(reportDefinitionWithOneDSD(), new EvaluationContext());
  ...
  public void render_shouldWriteZipIfMoreThanOneDataSet() throws Exception {
    DelimitedTextReportRenderer renderer = new CsvReportRenderer();
    ReportData data =
        reportDefinitionService.evaluate(reportDefinitionWithTwoDSDs(), new EvaluationContext());
    ...
    // I also did the following, then manually opened the zip file, and opened the CSVs in
    // LibreOffice. It worked
  ...
  public void writeDataSet_shouldFilterBlacklistedCharacters() throws Exception {
    String actual =
        writeSingleColumnWithBlacklist(
            "Yes: ÀÁÂÃÄàáâãä, No: ♪�", "[^\\p{InBasicLatin}\\p{InLatin-1Supplement}]");
  ...
  private String writeSingleColumnWithBlacklist(String value, String blacklistRegex)
      throws IOException {
    SimpleDataSet ds = new SimpleDataSet(null, null);
  ...
  private String renderDataSet(DataSet ds, String blacklistRegex) throws IOException {
    ...
    renderer.writeDataSet(
        ...
        "\n",
        "UTF-8",
        blacklistRegex != null ? Pattern.compile(blacklistRegex) : null,
        null);
  ...
  private ReportDefinition reportDefinitionWithOneDSD() {
    ...
    reportDefinition.addDataSetDefinition(
        "one",
        new SqlDataSetDefinition("one", "description", "select patient_id from patient"),
        null);
  ...
  private ReportDefinition reportDefinitionWithTwoDSDs() {
    ...
    reportDefinition.setName("Testing");
    reportDefinition.addDataSetDefinition(
        "collision with \"tricky!\" tough characters",
        new SqlDataSetDefinition("one", "description", "select patient_id from patient"),
        null);
    reportDefinition.addDataSetDefinition(
        "collision with tricky tough characters",
        new SqlDataSetDefinition("two", "description", "select location_id from location"),
        null);
    return reportDefinition;
  }
