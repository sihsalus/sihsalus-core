...
public class AgeAtEncounterDataEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void testEvaluate() throws Exception {
    ...
    EvaluatedEncounterData result =
        encounterDataService.evaluate(new AgeAtEncounterDataDefinition(), context);
    assertThat(result.getData().size(), is(1));
    assertThat(
        ((Age) result.getData().get(3)).getBirthDate().getTime(),
        is(DateUtil.parseYmd("1976-08-25").getTime()));
    assertThat(
        ((Age) result.getData().get(3)).getCurrentDate().getTime(),
        is(DateUtil.parseYmd("2008-08-01").getTime()));
  }
}
