...
public class ImplementerConfiguredDataSetDefinitionLibraryTest
  ...
  @Autowired ImplementerConfiguredDataSetDefinitionLibrary library;
  ...
  public void testSqlDefinition() throws Exception {
    DataSetDefinition definition =
        library.getDefinition("configuration.definitionlibrary.dataset.patientIdSql");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlDataSetDefinition.class));
    assertThat(
        ((SqlDataSetDefinition) definition).getSqlQuery().trim(),
        is("select patient_id from patient"));
  }

  @Test
  public void testSerializedDefinition() throws Exception {
    DataSetDefinition definition =
        library.getDefinition("configuration.definitionlibrary.dataset.patientIdXml");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlDataSetDefinition.class));
    assertThat(
        ((SqlDataSetDefinition) definition).getSqlQuery().trim(),
  ...
  public void testGroovyDefinition() throws Exception {
    DataSetDefinition definition =
        library.getDefinition("configuration.definitionlibrary.dataset.testGroovy");
    assertThat(definition, notNullValue());
