...
public class InStateCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should return no patients if none have the given state",
    ...
    List<ProgramWorkflowState> states =
        Collections.singletonList(
            Context.getProgramWorkflowService()
                .getStateByUuid("0d5f1bb4-2edb-4dd1-8d9f-34489bb4d9ea"));
    cd.setStates(states);
  ...
  @Test
  @Verifies(
      value = "should return patients in given state on given date",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Verifies(
      ...
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldFindPatientsInAStateOnTheOnOrBeforeDateIfPassedInTimeIsAtMidnight()
      throws Exception {
    InStateCohortDefinition cd = new InStateCohortDefinition();
    cd.addState(
        Context.getProgramWorkflowService().getStateByUuid("e938129e-248a-482a-acea-f85127251472"));
  ...
  @Test
  @Verifies(
      value = "should return patients in the given state on or before the given start date",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsInTheGivenStateOnOrBeforeTheGivenStartDate()
      throws Exception {
    executeDataSet(
        XML_DATASET_PATH + new TestUtil().getTestDatasetFilename(XML_REPORT_TEST_DATASET));
  ...
  @Test
  @Verifies(
      value = "should return patients in the given state on or after the given end date",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsInTheGivenStateOnOrAfterTheGivenEndDate()
      throws Exception {
    executeDataSet(
        XML_DATASET_PATH + new TestUtil().getTestDatasetFilename(XML_REPORT_TEST_DATASET));
  ...
  @Test
  @Verifies(
      value = "should return patients enrolled in the given programs at the given locations",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsEnrolledInTheGivenProgramsAtTheGivenLocations()
      throws Exception {
    InStateCohortDefinition cd = new InStateCohortDefinition();
    cd.addState(
        Context.getProgramWorkflowService().getStateByUuid("e938129e-248a-482a-acea-f85127251472"));
  ...
  @Test
  @Verifies(
      value = "should return patients enrolled at evaluation date if no other dates supplied",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldReturnPatientsEnrolledAtEvaluationDateIfNoOtherDatesSupplied()
      throws Exception {
    InStateCohortDefinition cd = new InStateCohortDefinition();
    cd.addState(
        Context.getProgramWorkflowService().getStateByUuid("e938129e-248a-482a-acea-f85127251472"));
    ...
    Cohort c =
        Context.getService(CohortDefinitionService.class)
            .evaluate(cd, new EvaluationContext(DateUtil.getDateTime(2012, 5, 15)));
    Assert.assertEquals(2, c.getSize());
    Assert.assertTrue(c.contains(2));
    Assert.assertTrue(c.contains(23));
    c =
        Context.getService(CohortDefinitionService.class)
            .evaluate(cd, new EvaluationContext(DateUtil.getDateTime(2008, 1, 1)));
