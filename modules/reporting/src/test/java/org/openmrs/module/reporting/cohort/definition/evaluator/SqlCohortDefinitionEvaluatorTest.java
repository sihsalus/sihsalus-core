...
public class SqlCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should support integer parameter",
  ...
  public void evaluate_shouldSupportIntegerParameter() throws Exception {
    ...
    Cohort cohort =
        Context.getService(CohortDefinitionService.class)
            .evaluate(cohortDefinition, evaluationContext);
  ...
  @Test
  @Verifies(
      value = "should support string parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should support patient parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should support integer list parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldSupportIntegerListParameter() throws Exception {

    String sqlQuery =
        "SELECT distinct patient_id FROM patient WHERE patient_id IN (:patientIdList)";
  ...
  @Test
  @Verifies(
      value = "should support patient list parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should support cohort parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should support date parameter",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  ...
  @Test
  @Verifies(
      value = "should evaluate different results for the same query with different parameters",
      method = "evaluate(CohortDefinition,EvaluationContext)")
  public void evaluate_shouldEvaluateDifferentResultsForTheSameQueryWithDifferentParameters()
      throws Exception {

    SqlCohortDefinition cd =
        new SqlCohortDefinition(
            "SELECT distinct patient_id FROM encounter WHERE encounter_datetime >= :startParam and encounter_datetime <= :endParam");
    cd.addParameter(new Parameter("startParam", "startParam", Date.class));
    cd.addParameter(new Parameter("endParam", "endParam", Date.class));

    CohortIndicator i1 =
        CohortIndicator.newCountIndicator(
            "num",
    ...
    CohortIndicator i2 =
        CohortIndicator.newCountIndicator(
            ...
            new Mapped<CohortDefinition>(
                cd,
                ParameterizableUtil.createParameterMappings(
                    "startParam=${endDate-1m},endParam=${endDate}")),
            null);
    ...
    dsd.addColumn(
        "2",
        "Num at end of period",
        new Mapped(i2, ParameterizableUtil.createParameterMappings("endDate=${endDate}")),
        "");
