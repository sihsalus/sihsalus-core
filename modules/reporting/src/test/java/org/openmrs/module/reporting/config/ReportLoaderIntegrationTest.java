...
public class ReportLoaderIntegrationTest extends BaseModuleContextSensitiveTest {
  ...
  @Autowired
  @Qualifier("reportingEvaluationService")
  EvaluationService evaluationService;

  @Autowired BuiltInCohortDefinitionLibrary cohorts;

  @Override
  public Properties getRuntimeProperties() {
    Properties p = super.getRuntimeProperties();
    String path =
        getClass().getClassLoader().getResource(appDataTestDir).getPath() + File.separator;
  ...
  public void shouldLoadAllReportDescriptorsInReportDescriptorsDirectory() {
    ...
    assertThat(
        names,
        hasItems(
            "sample.export.encounters.name",
            "sample.export.orders.name",
            "sample.export.nested.name"));
  }
  ...
  public void shouldLoadReportsFromConfigAndSave() {
    ...
    ReportDefinition ordersReportDefinition =
        reportDefinitionService.getDefinitionByUuid("9e7dc296-2aad-11e3-a840-5b9e0b589afb");
    ReportDefinition encountersReportDefinition =
        reportDefinitionService.getDefinitionByUuid("752e386d-da67-4e3d-bddc-95157c58c54c");
    ReportDefinition nestedReportDefinition =
        reportDefinitionService.getDefinitionByUuid("c2fb2082-9b36-4398-96af-d20570bacd07");
    ...
    List<ReportDesign> existingOrderReportDesigns =
        Context.getService(ReportService.class)
            .getReportDesigns(ordersReportDefinition, null, true);
  ...
  public void shouldSupportFixedParametersInDataSetDefinitions() throws Exception {
    ReportLoader.loadReportsFromConfig();
    ReportDefinition rd =
        reportDefinitionService.getDefinitionByUuid("0c32f660-c2de-11eb-b5a4-0242ac110002");
    ...
    SqlQueryBuilder maleQuery =
        new SqlQueryBuilder("select person_id from person where gender = 'M'");
    List<Integer> males =
        evaluationService.evaluateToList(maleQuery, Integer.class, new EvaluationContext());

    SqlQueryBuilder femaleQuery =
        new SqlQueryBuilder("select person_id from person where gender = 'F'");
    List<Integer> females =
        evaluationService.evaluateToList(femaleQuery, Integer.class, new EvaluationContext());
  ...
  public void shouldConstructExcelReportDesign() {
    ...
    List<ReportDesign> reportDesigns =
        ReportLoader.constructReportDesigns(reportDefinition, reportDescriptor);
    ...
    ReportDesignResource reportDesignResource =
        reportDesigns.get(0).getResources().iterator().next();
