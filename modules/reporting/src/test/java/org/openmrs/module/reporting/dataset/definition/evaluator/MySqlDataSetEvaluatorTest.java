...
public class MySqlDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public Properties getRuntimeProperties() {
    ...
    p.setProperty(
        "connection.url",
        "jdbc:mysql://localhost:3306/openmrs_mirebalais?autoReconnect=true&useUnicode=true&characterEncoding=UTF-8");
    return p;
  }
  ...
  public void evaluate_shouldAllowVariableInQuery() throws Exception {
    SqlDataSetDefinition dsd = new SqlDataSetDefinition();
    dsd.setSqlQuery(
        "select @numThisYear:=(select count(encounter_datetime) from encounter where voided = 0 and year(encounter_datetime) = :year), (@numThisYear-1000) as numMinus1000");
  ...
  /**
   * This test fails, demonstrating that it is not currently possible to execute modifications to
   * the DB due to the use of "executeQuery" in the SqlQueryBuilder.
   */
  @Test
