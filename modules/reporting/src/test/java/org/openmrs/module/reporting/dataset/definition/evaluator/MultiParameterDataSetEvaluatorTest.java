...
public class MultiParameterDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Before
  // This is needed due to a change to standardTestDataset in the OpenMRS 2.2 release that changed
  // person 6 birth year from 2007 to 1975
  public void setup() {
  ...
  /**
   * @see {@link
   *     org.openmrs.module.reporting.dataset.definition.evaluator.MultiParameterDataSetEvaluator#evaluate(org.openmrs.module.reporting.dataset.definition.DataSetDefinition,
   *     org.openmrs.module.reporting.evaluation.EvaluationContext)}
   */
  @Test
  @Verifies(
      value = "should evaluate a MultiParameterDataSetDefinition",
  ...
  public void evaluate_shouldEvaluateAMultiParameterDataSetDefinition() throws Exception {
    ...
    sqlDataSetDefinition.setSqlQuery(
        "select t.patient_id, p.gender, p.birthdate from patient t inner join person p on t.patient_id = p.person_id where p.birthdate < :maxBirthDate order by patient_id asc");
    sqlDataSetDefinition.addParameter(new Parameter("maxBirthDate", "maxBirthDate", Date.class));

    MultiParameterDataSetDefinition multiParameterDataSetDefinition =
        new MultiParameterDataSetDefinition();
    ...
    SimpleDataSet result =
        (SimpleDataSet)
            dataSetDefinitionService.evaluate(multiParameterDataSetDefinition, evaluationContext);
    ...
    Assert.assertEquals(
        firstIterationParameter, result.getColumnValue(1, "parameter.maxBirthDate"));

    // Asserting result parameters for second iteration
    Assert.assertEquals(
        secondIterationParameter, result.getColumnValue(2, "parameter.maxBirthDate"));
    Assert.assertEquals(
        secondIterationParameter, result.getColumnValue(3, "parameter.maxBirthDate"));
    ...
    // Asserting evaluation results values; first and second dates are the same - both iteration
    // returns them
