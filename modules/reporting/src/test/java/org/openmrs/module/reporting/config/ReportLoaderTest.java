...
public class ReportLoaderTest {
  ...
  public void load_shouldLoadYAMLFile() {
    ...
    assertThat(
        descriptor.getDesigns().get(0).getProperties().get("characterEncoding"), is("ISO-8859-1"));
    assertThat(
        descriptor.getDesigns().get(0).getProperties().get("blacklistRegex"),
        is("[^\\p{InBasicLatin}\\p{L}]"));
  ...
  public void shouldConstructCSVReportDesign() {
    ...
    assertThat(
        reportDesigns.get(0).getPropertyValue(ReportDesignRenderer.FILENAME_BASE_PROPERTY, null),
        startsWith("my.test.report."));
    assertThat(reportDesigns.get(0).getPropertyValue("characterEncoding", null), is("ISO-8859-1"));
    assertThat(
        reportDesigns.get(0).getPropertyValue("blacklistRegex", null),
        is("[^\\p{InBasicLatin}\\p{L}]"));
  }
