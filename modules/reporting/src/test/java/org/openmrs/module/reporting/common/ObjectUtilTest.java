...
public class ObjectUtilTest extends BaseModuleContextSensitiveTest {
  ...
  public void shouldSortObjectThatDontImplementComparable() throws Exception {
    ...
    List<ConceptClass> list =
        Arrays.asList(
            new ConceptClass[] {conceptClass1, conceptClass2, conceptClass3, conceptClass4});
  ...
  @Test
  @Verifies(
      value = "shouldReturnNullIfNoFormatterPresent",
      method = "getLocalization(OpenmrsMetadata md)")
  ...
  @Test
  @Verifies(
      value = "shouldReturnTheDefaultOpenmrsMetadataNames",
      method = "format(OpenmrsMetadata md)")
  public void shouldReturnTheDefaultOpenmrsMetadataNames() throws Exception {
    ...
    PatientIdentifierType patientIdentifierType =
        Context.getPatientService().getPatientIdentifierTypeByName(metadataName);
  ...
  public void shouldFormatMultiplePropertiesOnOpenmrsData() {
    // <encounter encounter_id="3" encounter_type="2" patient_id="7" location_id="1" form_id="1"
    // encounter_datetime="2008-08-01 00:00:00.0" creator="1" date_created="2008-08-18 14:09:05.0"
    // voided="false" void_reason="" uuid="6519d653-393b-4118-9c83-a3715b82d4ac"/>
    Encounter e = Context.getEncounterService().getEncounter(3);
    String s =
        ObjectUtil.format(
            e,
            "Encounter {encounterId} has type {encounterType} and date {encounterDatetime|yyyy-MM-dd}");
  ...
  public void shouldFormatDateWithAppropriateFormat() {
    ...
    String previousFormat =
        TestUtil.getGlobalProperty(ReportingConstants.GLOBAL_PROPERTY_DEFAULT_DATE_FORMAT);
    ...
    TestUtil.updateGlobalProperty(
        ReportingConstants.GLOBAL_PROPERTY_DEFAULT_DATE_FORMAT, "MMMM dd, yyyy");
    Assert.assertEquals("January 14, 2014", ObjectUtil.format(d));
    TestUtil.updateGlobalProperty(ReportingConstants.DEFAULT_LOCALE_GP_NAME, "es");
    Assert.assertEquals("enero 14, 2014", ObjectUtil.format(d));

    TestUtil.updateGlobalProperty(
        ReportingConstants.GLOBAL_PROPERTY_DEFAULT_DATE_FORMAT, previousFormat);
    TestUtil.updateGlobalProperty(ReportingConstants.DEFAULT_LOCALE_GP_NAME, previousLocale);
  }
