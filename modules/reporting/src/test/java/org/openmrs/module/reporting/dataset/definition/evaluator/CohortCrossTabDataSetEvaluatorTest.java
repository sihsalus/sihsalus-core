...
public class CohortCrossTabDataSetEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  @Test
  @Verifies(
      value = "should evaluate a CohortCrossTabDataSetDefinition",
  ...
  public void evaluate_shouldEvaluateACohortIndicatorDataSetDefinition() throws Exception {
    ...
    d.addColumn(
        ...
        new Mapped<CohortDefinition>(
            adultsOnDate, ParameterizableUtil.createParameterMappings("effectiveDate=${endDate}")));
    d.addColumn(
        "child",
        new Mapped<CohortDefinition>(
            childrenOnDate,
    ...
    report.addDataSetDefinition(
        d, ParameterizableUtil.createParameterMappings("endDate=${endDate}"));

    ReportData results =
        Context.getService(ReportDefinitionService.class).evaluate(report, context);
    ...
    Assert.assertEquals(
        3, ((Cohort) ds.getData(ds.getMetaData().getColumn("male.unknown"))).size());
    Assert.assertEquals(
        2, ((Cohort) ds.getData(ds.getMetaData().getColumn("female.adult"))).size());
    Assert.assertEquals(
        1, ((Cohort) ds.getData(ds.getMetaData().getColumn("female.child"))).size());
    Assert.assertEquals(
        5, ((Cohort) ds.getData(ds.getMetaData().getColumn("female.unknown"))).size());
    Assert.assertEquals(
        0, ((Cohort) ds.getData(ds.getMetaData().getColumn("unknown.adult"))).size());
    Assert.assertEquals(
        0, ((Cohort) ds.getData(ds.getMetaData().getColumn("unknown.child"))).size());
    Assert.assertEquals(
        1, ((Cohort) ds.getData(ds.getMetaData().getColumn("unknown.unknown"))).size());
  }
}
