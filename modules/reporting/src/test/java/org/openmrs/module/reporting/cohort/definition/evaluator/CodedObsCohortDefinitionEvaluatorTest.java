...
public class CodedObsCohortDefinitionEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void evaluate_shouldTestAnyWithManyPropertiesSpecified() throws Exception {
    ...
    cd.setQuestion(
        new Concept(21)); // FOOD ASSISTANCE FOR ENTIRE FAMILY, in the reporting test dataset
    cd.setOperator(SetComparator.IN);
    cd.setValueList(
        Collections.singletonList(new Concept(7))); // YES, in the reporting test dataset
  ...
  @Test
  @Verifies(
      value = "should test that obs are retricted by encounter type",
      method = "evaluate(CohortDefinition,EvaluationContext)")
