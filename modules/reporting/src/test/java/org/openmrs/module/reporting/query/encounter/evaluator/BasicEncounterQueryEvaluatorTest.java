...
public class BasicEncounterQueryEvaluatorTest extends BaseModuleContextSensitiveTest {
  ...
  public void testEvaluate() throws Exception {
    ...
    Encounter enc3 =
        data.randomEncounter().patient(patient).encounterDatetime("2013-08-10 09:09:09").save();
    Encounter enc4 =
        data.randomEncounter().patient(patient).encounterDatetime("2013-08-10 10:10:10").save();
    Encounter enc5 =
        data.randomEncounter().patient(patient).encounterDatetime("2013-08-11 10:10:10").save();
    ...
    context.setBaseEncounters(
        new EncounterIdSet(enc1.getId(), enc2.getId(), enc4.getId(), enc5.getId())); // not 3
    EncounterQueryResult result = encounterQueryService.evaluate(query, context);
    assertThat(
        result,
        hasExactlyIds(
            enc2.getId(), enc4.getId())); // 3 is excluded, since it wasn't in base encounters
  }
  ...
  public void testShouldFilterByWhich() throws Exception {
    ...
    Encounter enc1 =
        data.randomEncounter().encounterDatetime("2014-01-01").patient(patient1).save();
    Encounter enc2 =
        data.randomEncounter().encounterDatetime("2014-02-01").patient(patient1).save();
    Encounter enc3 =
        data.randomEncounter().encounterDatetime("2014-03-01").patient(patient1).save();
    Patient patient2 = data.randomPatient().save();
    Encounter enc4 =
        data.randomEncounter().encounterDatetime("2014-04-01").patient(patient2).save();

    EncounterEvaluationContext context = new EncounterEvaluationContext();
    context.setBaseEncounters(
        new EncounterIdSet(enc1.getId(), enc2.getId(), enc3.getId(), enc4.getId()));
