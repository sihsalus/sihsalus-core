...
public class ImplementerConfiguredEncounterDataDefinitionLibraryTest
  ...
  @Autowired ImplementerConfiguredEncounterDataDefinitionLibrary library;
  ...
  public void testSqlDefinition() throws Exception {
    EncounterDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.encounterData.patientIdSql");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlEncounterDataDefinition.class));
    assertThat(
        ((SqlEncounterDataDefinition) definition).getSql().trim(),
        is("select encounter_id, patient_id from encounter"));
  }

  @Test
  public void testSerializedDefinition() throws Exception {
    EncounterDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.encounterData.encounterDatetimeXml");
    assertThat(definition, notNullValue());
