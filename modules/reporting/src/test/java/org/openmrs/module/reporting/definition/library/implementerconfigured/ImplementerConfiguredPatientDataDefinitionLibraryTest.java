...
public class ImplementerConfiguredPatientDataDefinitionLibraryTest
  ...
  @Autowired ImplementerConfiguredPatientDataDefinitionLibrary library;
  ...
  public void testSqlDefinition() throws Exception {
    PatientDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.patientData.patientIdSql");
    assertThat(definition, notNullValue());
    assertThat(definition, instanceOf(SqlPatientDataDefinition.class));
    assertThat(
        ((SqlPatientDataDefinition) definition).getSql().trim(),
        is("select patient_id, patient_id from patient"));
  }

  @Test
  public void testSerializedDefinition() throws Exception {
    PatientDataDefinition definition =
        library.getDefinition("configuration.definitionlibrary.patientData.patientIdXml");
    assertThat(definition, notNullValue());
