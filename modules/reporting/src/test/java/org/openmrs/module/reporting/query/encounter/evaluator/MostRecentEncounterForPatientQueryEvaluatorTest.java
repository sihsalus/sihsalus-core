...
public class MostRecentEncounterForPatientQueryEvaluatorTest
  ...
  /**
   * @see {@link
   *     MostRecentEncounterForPatientQueryEvaluator#evaluate(EncounterQuery,EvaluationContext)}
   */
  @Test
  @Verifies(
      value = "should find an encounter on the onOrBefore date if passed in time is at midnight",
      method = "evaluate(EncounterQuery,EvaluationContext)")
  public void evaluate_shouldFindAnEncounterOnTheOnOrBeforeDateIfPassedInTimeIsAtMidnight()
      throws Exception {

    Date date = new Date();
    Encounter enc =
        tdm.encounter().location(1).encounterType(1).encounterDatetime(date).patient(7).save();
