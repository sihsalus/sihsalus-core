...
public class AgeCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
  ...
  public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) {
    ...
    // If there is a minimum age, then this means patients must have a birthdate earlier than a
    // particular date, so a max birthdate is enforced
    // If none is specified, ensure no patients are included who are born after the effective date
    Date maxBirthdateAllowed =
        getDateForAge(acd.getEffectiveDate(), acd.getMinAge(), acd.getMinAgeUnit());
    q.whereLessOrEqualTo(
        "p.birthdate", ObjectUtil.nvl(maxBirthdateAllowed, acd.getEffectiveDate()));

    if (acd.getMaxAge() != null) {
      // If there is a maximum age, then this means that patients must have a birthdate after a
      // particular date, so a min birthdate is enforced
      // We add one to the max age, and then exclude the boundary, to account for all fractional
      // units
      Date minBirthdateAllowed =
          getDateForAge(acd.getEffectiveDate(), acd.getMaxAge() + 1, acd.getMaxAgeUnit());
  ...
  protected PatientIdSet getPatientsWithUnknownAge(EvaluationContext context) {
    ...
    q.select("p.patientId")
        ...
        .wherePatientIn("p.patientId", context);
    List<Integer> pIds = evaluationService.evaluateToList(q, Integer.class, context);
    return new PatientIdSet(pIds);
  }

  /**
   * @return the date on which a patients birthday would need to fall in order to be on the given
   *     age as of the given effective date
   */
