...
/**
 * Delegates to evaluating a MultiParameterDataSet after producing a specific set of iterations on
 * the fly
 */
...
public class RepeatPerTimePeriodDataSetEvaluator implements DataSetEvaluator {
  ...
  public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
    ...
    RepeatPerTimePeriodDataSetDefinition dsd =
        (RepeatPerTimePeriodDataSetDefinition) dataSetDefinition;

    Mapped<? extends DataSetDefinition> baseMappedDef = dsd.getBaseDefinition();

    MultiParameterDataSetDefinition delegate =
        new MultiParameterDataSetDefinition(baseMappedDef.getParameterizable());
    ...
    DateTime thisPeriodStart =
        new DateTime(((Date) evalContext.getParameterValue("startDate")).getTime());
    DateTime end =
        new DateTime(
            DateUtil.getEndOfDayIfTimeExcluded((Date) evalContext.getParameterValue("endDate"))
                .getTime());
    ...
    while (thisPeriodStart.isBefore(end)) {
      ...
      for (Map.Entry<String, Object> entry : baseMappedDef.getParameterMappings().entrySet()) {
        ...
        if (value instanceof String) {
          ...
          // expressions based on parameters other than startDate/endDate will come out like ${loc}
          // -> "loc"
          if (value.equals(
              EvaluationUtil.EXPRESSION_START + evaluated + EvaluationUtil.EXPRESSION_END)) {
            continue;
          }
          value = evaluated;
        }
