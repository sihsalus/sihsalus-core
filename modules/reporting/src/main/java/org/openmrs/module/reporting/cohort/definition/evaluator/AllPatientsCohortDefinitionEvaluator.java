...
public class AllPatientsCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
  ...
  public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) {
    ...
    if (context.getBaseCohort() == null) {
      // Everywhere else in the reporting module we call Cohorts.allPatients(EvaluationContext) to
      // get the list
      // of all patients, but in this one place we intentionally go directly to the database. This
      // avoids infinite
      // recursion, since the Cohorts.allPatients method delegates to this evaluator.
      //
      // As long as API consumers don't directly call this evaluator, but go through
