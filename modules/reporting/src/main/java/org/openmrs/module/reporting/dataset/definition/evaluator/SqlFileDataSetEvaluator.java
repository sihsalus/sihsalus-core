...
public class SqlFileDataSetEvaluator implements DataSetEvaluator {
    ...
    try {
      ...
      if (StringUtils.isNotBlank(dsd.getSqlFile())) {
        ...
        throw new EvaluationException(
            "A SqlFileDataSetDefinition must define either a SQL File or SQL Resource");
      }
      ...
      throw new EvaluationException(
          "An error occurred while evaluating a SqlFileDataSetDefinition", e);
    } finally {
      try {
        if (connection != null) {
          connection.close();
   ...
   */
  protected Properties getConnectionProperties(String connectionPropertyFile)
      throws EvaluationException {
    ...
    if (StringUtils.isNotBlank(connectionPropertyFile)) {
      ...
      try {
        ...
        throw new EvaluationException(
            "Unable to load connection properties from file <" + connectionPropertyFile + ">", e);
      } finally {
   ...
   */
  protected Connection getConnection(SqlFileDataSetDefinition dsd, EvaluationContext context)
      throws EvaluationException {
  ...
  /**
   * @return parameter values to use within SQL statement, converting object references to metadata
   *     to scalar properties, defaulting to keys
   */
  protected Map<String, Object> constructParameterValues(
      SqlFileDataSetDefinition dsd, EvaluationContext context) {
