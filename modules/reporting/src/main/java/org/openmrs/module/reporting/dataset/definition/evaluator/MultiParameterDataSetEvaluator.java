...
public class MultiParameterDataSetEvaluator implements DataSetEvaluator {
   ...
   * @see
   *     org.openmrs.module.reporting.dataset.definition.evaluator.DataSetEvaluator#evaluate(org.openmrs.module.reporting.dataset.definition.DataSetDefinition,
    ...
    for (Map<String, Object> iteration : dsd.getIterations()) {
      ...
      while (iterator.hasNext()) {
        ...
        for (Parameter p : dsd.getBaseDefinition().getParameters()) {
          ...
          String columnName =
              ReportTemplateRenderer.PARAMETER_PREFIX
                  + ReportTemplateRenderer.SEPARATOR
                  + p.getName();
          row.addColumnValue(
              new DataSetColumn(columnName, p.getLabelOrName(), p.getType()), paramValue);
        }
        for (Map.Entry<DataSetColumn, Object> entry :
            iterator.next().getColumnValues().entrySet()) {
