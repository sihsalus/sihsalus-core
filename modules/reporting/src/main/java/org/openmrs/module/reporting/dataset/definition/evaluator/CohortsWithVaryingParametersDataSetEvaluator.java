...
/**
 * Evaluates each CohortDefinition against each set of parameter options. Produces a MapDataSet
 * whose column names are given by the CohortDefinition's name (which should be a handlebars
 * template that can refer to parameter values, e.g. "at-{{ location.uuid }}". Labels of the columns
 * in the resulting data set are given by the CohortDefinition's description (which should be a
 * handlebars template that can refer to parameter values, e.g. "Registered at {{ location.name
 * }}").
 */
...
public class CohortsWithVaryingParametersDataSetEvaluator implements DataSetEvaluator {
  ...
  @Autowired private CohortDefinitionService cohortDefinitionService;

  @Override
  public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext evalContext)
      throws EvaluationException {
    CohortsWithVaryingParametersDataSetDefinition dsd =
        (CohortsWithVaryingParametersDataSetDefinition) dataSetDefinition;
    SimpleDataSet results = new SimpleDataSet(dsd, evalContext);

    Template rowLabelTemplate =
        templateFactory.compileHandlebarsTemplate(dsd.getRowLabelTemplate());
    ...
    for (Map<String, Object> parameterOption : dsd.getVaryingParameters()) {
      ...
      for (CohortsWithVaryingParametersDataSetDefinition.Column column : dsd.getColumns()) {
        ...
        EvaluatedCohort cohort =
            cohortDefinitionService.evaluate(column.getCohortDefinition(), contextForVariation);
