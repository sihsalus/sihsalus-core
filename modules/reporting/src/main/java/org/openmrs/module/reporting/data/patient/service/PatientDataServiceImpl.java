...
public class PatientDataServiceImpl extends BaseDefinitionService<PatientDataDefinition>
  ...
  public EvaluatedPatientData evaluate(
      Mapped<? extends PatientDataDefinition> mappedDefinition, EvaluationContext context)
      throws EvaluationException {
  ...
  protected Evaluated<PatientDataDefinition> executeEvaluator(
      DefinitionEvaluator<PatientDataDefinition> evaluator,
      PatientDataDefinition definition,
    ...
    if (context.getBaseCohort() == null
        || batchSize <= 0
        || context.getBaseCohort().size() <= batchSize) {
    ...
    if (context.getBaseCohort().size() > 0) {
      ...
      for (Cohort batchCohort : batches) {
        ...
        EvaluatedPatientData batchData =
            (EvaluatedPatientData) super.executeEvaluator(evaluator, definition, batchContext);
