...
/**
 * This is a {@link DataSet} that allows you to request the rows for a subset of the input cohort,
 * and it will generate those rows on demand (by delegating to a {@link
...
public class LazyPageableDataSet implements PageableDataSet {
  ...
  public LazyPageableDataSet(
    ...
    if (evalContext.getBaseCohort() != null) {
      ...
      cohortPatientIds =
          new ArrayList<Integer>(CohortUtil.memberIds(Cohorts.allPatients(evalContext)));
    }
  ...
  /**
   * If the number of rows requested is greater than the maximumPatientsToEvaluate setting then the
   * iterator returned will make multiple calls back to the underlying evaluator over its lifecycle.
   *
   * @see PageableDataSet#rowsForCohortSubset(int, int)
   */
  public Iterator<DataSetRow> rowsForCohortSubset(int start, int size) {
    // TODO add a maximum number of patients to be evaluated at a time, which will keep memory
    // consumption down
    int end = start + size;
    if (size < 0 || end > cohortPatientIds.size()) end = cohortPatientIds.size();
    return createIterator(start, end);
  }

  private Iterator<DataSetRow> createIterator(int start, int end) {
    int MAX =
        (maximumPatientsToEvaluate == null || maximumPatientsToEvaluate <= 0)
            ? Integer.MAX_VALUE
            : maximumPatientsToEvaluate;
    if (end - start <= MAX) {
  ...
  public class BufferedIterator implements Iterator<DataSetRow> {
    ...
    public BufferedIterator(
        LazyPageableDataSetEvaluator evaluator, List<Integer> patientIds, Integer batchSize) {
      this.evaluator = evaluator;
    ...
    private void getNextBatch() {
      if (nextIndex >= patientIds.size()) {
        ...
        if (end > patientIds.size()) end = patientIds.size();
    ...
    public boolean hasNext() {
      if (!currentBatchIterator.hasNext()) getNextBatch();
