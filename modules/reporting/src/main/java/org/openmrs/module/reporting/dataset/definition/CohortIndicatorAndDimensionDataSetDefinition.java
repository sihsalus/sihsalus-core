...
public class CohortIndicatorAndDimensionDataSetDefinition extends BaseDataSetDefinition {
  ...
  @ConfigurationProperty Map<String, Mapped<CohortDefinitionDimension>> dimensions;

  @ConfigurationProperty List<CohortIndicatorAndDimensionSpecification> specifications;
   ...
   */
  public Map<CohortIndicatorAndDimensionSpecification, List<DataSetColumn>>
      getColumnsBySpecification() {
    ...
    for (CohortIndicatorAndDimensionSpecification s : getSpecifications()) {
      ...
      List<String> combinations =
          IndicatorUtil.compileColumnDimensionOptions(s.getDimensionOptions());
      combinations.add(0, null); // Add in the "no dimension" case at the start

      for (String combination : combinations) {
        DataSetColumn column =
            new DataSetColumn(s.getIndicatorNumber(), s.getLabel(), Object.class);
        if (combination != null) {
          for (String option : combination.split(",")) {
            ...
            column.setLabel(
                column.getLabel()
                    + (column.getLabel().equals(s.getLabel()) ? " (" : ", ")
                    + dimOpt[0]
                    + " - "
                    + dimOpt[1]);
          }
  ...
  public void addSpecification(
      ...
      Map<String, List<String>> dimensionOptions) {
