...
public class DrugOrderCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
  ...
  public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context) {
    DrugOrderCohortDefinition drugOrderCohortDefinition =
        (DrugOrderCohortDefinition) cohortDefinition;
    ...
    if (drugOrderCohortDefinition.getDrugSets() != null) {
      ...
      if (drugOrderCohortDefinition.getWhich() == Match.ANY) {
        ...
        query.groupBy(
            "drugOrder.patient.patientId"
                + " having count(distinct drugOrder.concept.conceptId) = "
                + drugOrderCohortDefinition.getDrugSets().size());
    ...
    if (drugOrderCohortDefinition.getDrugConcepts() != null) {
      if (drugOrderCohortDefinition.getWhich() == Match.ANY) {
        query.whereInAny(
            "drugOrder.concept", drugOrderCohortDefinition.getDrugConcepts().toArray());
      } else if (drugOrderCohortDefinition.getWhich() == Match.ALL) {
    ...
    if (drugOrderCohortDefinition.getDrugs() != null) {
      if (drugOrderCohortDefinition.getWhich() == Match.ANY) {
        ...
        query.groupBy(
            "drugOrder.patient.patientId"
                + " having count(distinct drugOrder.drug.drugId) = "
                + drugOrderCohortDefinition.getDrugs().size());
      } else if (drugOrderCohortDefinition.getWhich() == Match.NONE) {
        query.whereNotInAny("drugOrder.drug", drugOrderCohortDefinition.getDrugs());
      }
    }

    query.whereLessOrEqualTo(
        "drugOrder.dateActivated", drugOrderCohortDefinition.getActivatedOnOrBefore());
    query.whereGreaterOrEqualTo(
        "drugOrder.dateActivated", drugOrderCohortDefinition.getActivatedOnOrAfter());
    ...
    if (drugOrderCohortDefinition.getActiveOnOrBefore() != null) {
      query
          .whereNotNull("drugOrder.dateActivated")
          .and()
          .whereLessOrEqualTo(
              "drugOrder.dateStopped", drugOrderCohortDefinition.getActiveOnOrBefore())
          .or()
          .whereLessOrEqualTo(
              "drugOrder.autoExpireDate", drugOrderCohortDefinition.getActiveOnOrBefore());
    }

    query
        .whereNotNull("drugOrder.dateActivated")
        .and()
        .whereGreaterEqualOrNull(
            "drugOrder.dateStopped", drugOrderCohortDefinition.getActiveOnOrAfter())
        .and()
        .whereGreaterEqualOrNull(
            "drugOrder.autoExpireDate", drugOrderCohortDefinition.getActiveOnOrAfter());
    ...
    query
        ...
        .and()
        .whereGreaterOrNull(
            "drugOrder.autoExpireDate", drugOrderCohortDefinition.getActiveOnDate());
