...
public class ProgramStatesForPatientDataEvaluator implements PatientDataEvaluator {
  ...
  private static class PatientStateComparator implements Comparator<PatientState> {
    ...
    public int compare(PatientState s1, PatientState s2) {
      ...
      if (result == 0) {
        result =
            OpenmrsUtil.compareWithNullAsEarliest(
                s1.getPatientProgram().getDateEnrolled(), s2.getPatientProgram().getDateEnrolled());
      }
      if (result == 0) {
        result =
            OpenmrsUtil.compareWithNullAsLatest(
                s1.getPatientProgram().getDateCompleted(),
                s2.getPatientProgram().getDateCompleted());
      }
      if (result == 0) {
