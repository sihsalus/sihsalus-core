...
/**
 * Represents a mode in which a @see org.openmrs.report.ReportRenderer can run. A simple renderer
 * like a CSV renderer can probably only render in one mode. A more sophisticated renderer might be
 * able to render multiple modes, which would show up with different labels, and at different
 * weights in a drop-down list. In this case a renderer would use the String argument of this class
 * to determine which mode was selected by the user. A higher sortWeight (i.e. closer to
 * Integer.MAX_VALUE) will typically appear at the top of a select list.
 */
