...
public class VisitDataUtil {
  ...
  /**
   * @return the base set of visit ids relevant for the passed EvaluationContext or null for all
   *     visit ids If returnNullForAllVisitIds is false, then this will return all visit ids in the
   *     system if unconstrained by the context
   */
  public static Set<Integer> getVisitIdsForContext(
      EvaluationContext context, boolean returnNullForAllVisitIds) {

    Cohort patIds = context.getBaseCohort();
    VisitIdSet visitIds =
        (context instanceof VisitEvaluationContext
            ? ((VisitEvaluationContext) context).getBaseVisits()
            : null);
    ...
    if (patIds != null) {
      ...
      List<Integer> visitIdsForPatients =
