...
/**
 * The logic that evaluates a {@link EncounterAndObsDataSetDefinition} and produces an {@link
 * DataSet}
 *
 * @see EncounterAndObsDataSetDefinition
 */
@Handler(
    supports = {EncounterAndObsDataSetDefinition.class},
    order = 25)
public class EncounterAndObsDataSetEvaluator extends EncounterDataSetEvaluator {
  ...
  @Autowired BuiltInEncounterDataLibrary encounterData;
  ...
  public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext context)
    ...
    // Get the maximum number of occurrences for each Obs column, to determine which need to have
      ...
      if (obsList != null) {
        ...
        for (Obs obs : obsList) {
          ...
          if (key != null) {
            ...
            String columnName =
                ObjectUtil.format(obs.getConcept())
                    .replaceAll("\\s", "_")
                    .replaceAll("-", "_")
                    .toUpperCase();
