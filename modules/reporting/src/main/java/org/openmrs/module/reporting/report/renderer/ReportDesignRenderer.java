...
/**
 * Renderers which use design files to influence the rendered report output should extend this base
 * renderer class
 */
public abstract class ReportDesignRenderer extends AbstractReportRenderer {
   ...
   * of the filename (with no extension) based on a "filenameTemplate" design argument, which
   * defaults to "{{definition.name}}_{{dateYmd}}-{{timeHms}}"
   *
   * @param request
   * @return
   */
  protected String getFilenameBase(ReportRequest request) {
    ...
    try {
      String evaluatedTemplate =
          templateFactory.evaluateHandlebarsTemplate(template, templateModel);
