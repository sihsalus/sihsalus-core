 ...
 * desired mechanism.
 *
 * <p>This code is based loosely on source code from the <i>Apache Ant</i> project at:
 *
 * <p><code>http://ant.apache.org/index.html</code>
 *
 * <p>As mentioned, multiple statements can be provided, separated by semi-colons. Lines within the
...
public class SqlScriptParser {
  ...
  public static String[] parse(Reader sqlScriptReader) {
    ...
    try {
      ...
      while ((line = in.readLine()) != null) {
        ...
        // Check if we have a multiple line comment start delimiter which does not close on same
        // line.
        ...
        if (sql.length() > 0) {
          if (sql.charAt(sql.length() - 1) == STATEMENT_DELIMITER) {
            ...
            String statement =
                sql.toString().substring(0, sql.length() - 1).trim(); // Lose the delimiter;
   ...
   *
   * @param line The line to parse. The string must not have any new line, carriage return, or form
   *     feed characters. It also may not contain a double-quote outside of a string literal, and
   *     the statement delimiter character may only appear at the end of the line.
   * @param sql String buffer that stores the result, which is appended to the end. When the method
   *     returns, it will contain the trimmed line with comments removed. If the line contains no
   *     usable SQL fragment, nothing is appended.
  ...
  private static void parseLine(String line, StringBuffer sql, int lineNumber) {
    ...
    // Parse line looking for single quote string delimiters. Anything that's part of a string just
    // passes through.
      ...
      if (token.equals("'")) {
        ...
        if (token.indexOf("\"") >= 0) {
          throw new APIException(
              "errors.sqlscript.doubleQuotationNotAllowed: lineNumber = " + lineNumber);
        }
        // Thwart any attempt to have the statement delimiter embedded in a line. Not supported at
        // this point.
        int statementEndIndex = token.indexOf(STATEMENT_DELIMITER);
        if (statementEndIndex >= 0 && statementEndIndex != (token.length() - 1)) {
          throw new APIException(
              "errors.sqlscript.noMultipleStatementsPerLine: lineNumber = " + lineNumber);
        }
    ...
    if (inLiteral) {
      ...
      throw new APIException(
          "errors.sqlscript.unterminatedStringLiteral: lineNumber = " + lineNumber);
    }
  }
