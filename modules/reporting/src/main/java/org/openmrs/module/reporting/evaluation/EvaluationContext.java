 ...
 * patients relevant to this context (defaults to all patients) - A Set of base filters, which can
 * be used to retrieve the base set of data to operate upon - An in-memory cache which can be used
 * to persist and retrieve previous Evaluation results. Note that this cache is cleared whenever any
 * changes are made to evaluationDate, limit, baseCohort TODO: We need to be smarter than this. We
...
public class EvaluationContext implements PatientCalculationContext {
  ...
  // An id, similar to a threadId, that is passed from parent to child evaluation context, and can
  // be used to keep track of a particular evaluation
  ...
  // Stores an integer representing the evaluation level of this context.  If this context is
  // evaluating a non-nested definition, this will be 1
  ...
  public EvaluationContext(EvaluationContext context) {
    ...
    this.setCache(
        context.getCache()); // This needs to be the last call, as the above calls clears the cache
  }
  ...
  /**
   * @return a cloned EvaluationContext, replacing the parameters with those in the mapped child
   *     object as appropriate.
   */
  public static EvaluationContext cloneForChild(
      EvaluationContext initialContext, Mapped<? extends Parameterizable> child) {
    ...
    for (String paramName : m.keySet()) {
      ...
      if (paramVal instanceof String) {
        ...
        if (EvaluationUtil.isExpression(paramValStr)
            && EvaluationUtil.stripExpression(paramValStr).equals(paramVal)) {
          paramVal = null;
        }
      }
  ...
  public String toString() {
    ...
    sb.append(
        getClass().getSimpleName() + "-" + evaluationId + "[evaluationDate=" + evaluationDate);
