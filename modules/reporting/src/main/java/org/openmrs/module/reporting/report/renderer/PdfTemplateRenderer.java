...
/**
 * Renders a report to PDF using either an AcroForm PDF template (field fill) or an HTML/CSS
 * template (converted to PDF via openhtmltopdf). Mode is determined by the extension of the
 * resource named "template" in the ReportDesign: .pdf -> AcroForm field-fill mode .html ->
 * HTML-to-PDF mode
 */
@Handler
...
public class PdfTemplateRenderer extends ReportTemplateRenderer {
  ...
  private void renderFromPdfForm(
      ReportData reportData, ReportDesign design, ReportDesignResource template, OutputStream out)
      throws IOException {
      ...
      if (acroForm != null) {
        ...
        while (fieldIterator.hasNext()) {
          ...
          if (replacements.containsKey(fieldName)) {
            ...
            String displayValue =
                format != null ? ObjectUtil.format(value, format) : ObjectUtil.format(value);
            try {
  ...
  private class BarcodeStreamFactory implements FSStreamFactory {
    ...
    public FSStream getUrl(String url) {
      try {
        ...
        BitMatrix matrix =
            new MultiFormatWriter().encode(content, BarcodeFormat.CODE_128, 600, 100, hints);
        ...
        return new FSStream() {
          ...
          public Reader getReader() {
            return new InputStreamReader(
                new ByteArrayInputStream(imageBytes), StandardCharsets.UTF_8);
          }
        };
      } catch (Exception e) {
