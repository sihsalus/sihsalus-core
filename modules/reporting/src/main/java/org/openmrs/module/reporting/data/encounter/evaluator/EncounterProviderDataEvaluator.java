...
/** Evaluates a EncounterProviderDataDefinition (Only works with OpenMRS 1.9 and above) */
...
public class EncounterProviderDataEvaluator implements EncounterDataEvaluator {
  ...
  @Override
  public EvaluatedEncounterData evaluate(
      EncounterDataDefinition definition, EvaluationContext context) throws EvaluationException {

    EncounterProviderDataDefinition def = (EncounterProviderDataDefinition) definition;

    // check to make sure parameter is an encounter role (method signature is type OpenmrsMetadata
    // so it can compile again OpenMRS 1.6.x)
    if (def.getEncounterRole() != null
        && !def.getEncounterRole().getClass().getName().equals("org.openmrs.EncounterRole")) {
    ...
    for (Object[] result : evaluationService.evaluateToList(q, context)) {
      ...
      if (!def.isSingleProvider()) {
        ...
        // If there are multiple matching providers and we are in singleProvider mode then last one
        // wins
