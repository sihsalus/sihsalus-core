...
public class ReportingSerializer extends XStreamShortSerializer {
  ...
  public ReportingSerializer() throws SerializationException {
    ...
    xstream.allowTypes(
        new Class[] {
          Parameter.class,
          Mapped.class,
          CohortIndicatorAndDimensionDataSetDefinition.CohortIndicatorAndDimensionSpecification
              .class
        });
  }

  @Override
  public synchronized <T> T deserialize(String serializedObject, Class<? extends T> clazz)
      throws SerializationException {
    ...
    try {
      ...
      if (cacheOwner) cache.remove();
    }
  }

  /**
   * The OpenMRS serializer interface supports serializing an Object to a String, but when we are
   * serializing a complete ReportData, it may be too memory-intensive to create this String. This
   * method lets you serialize to XML in a streaming fashion.
   *
   * @param object
   * @param out
   */
  ...
  /**
   * Sets up xstream security on the Reporting Serializer to match the OpenMRS core security
   * configuration
   */
  public void setupXStreamSecurity() throws SerializationException {
    ...
    try {
      serializer =
          Context.getRegisteredComponent("simpleXStreamSerializer", SimpleXStreamSerializer.class);
