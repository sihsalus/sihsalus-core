...
public class LogicDataSetEvaluator implements LazyPageableDataSetEvaluator {
  ...
  /**
   * @see LazyPageableDataSetEvaluator#evaluatePartial(PageableDataSetDefinition, EvaluationContext,
   *     List)
   */
    ...
    for (Integer ptId : CohortUtil.memberIds(cohort)) {
      ...
      if (forPatient == null && anyColumns) {
        throw new ReportingException(
            "Logic Module did not return results for all patients in cohort");
      }
      if (forPatient == null) {
        forPatient = new ArrayList<Result>();
      }
