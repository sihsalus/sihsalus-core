...
/**
 * The default implementation of a {@link ReportDefinitionEvaluator} that returns ReportData that
 * contains a single DataSet for each DataSetDefinition that is configured within it
 */
...
public class DefaultReportDefinitionEvaluator implements ReportDefinitionEvaluator {
  ...
  /**
   * Evaluates each DataSetDefinition defined in the ReportDefinition and returns these within
   * ReportData
   *
  ...
  public ReportData evaluate(ReportDefinition reportDefinition, EvaluationContext context)
    ...
    try {
      if (reportDefinition.getBaseCohortDefinition() != null) {
        Cohort newCohort =
            CohortFilter.filter(dataSetContext, reportDefinition.getBaseCohortDefinition());
    ...
    if (dataSetDefinitions != null) {
      for (String key : dataSetDefinitions.keySet()) {
        ...
        EvaluationContext childContext =
            EvaluationContext.cloneForChild(dataSetContext, mappedDataSetDefinition);
        try {
          DataSet ds =
              dataSetDefinitionService.evaluate(
                  mappedDataSetDefinition.getParameterizable(), childContext);
