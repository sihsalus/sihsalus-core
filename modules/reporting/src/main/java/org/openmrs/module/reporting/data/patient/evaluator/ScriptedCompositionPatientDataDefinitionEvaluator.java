...
public class ScriptedCompositionPatientDataDefinitionEvaluator implements PatientDataEvaluator {
  ...
  public EvaluatedPatientData evaluate(PatientDataDefinition definition, EvaluationContext context)
    ...
    ScriptedCompositionPatientDataDefinition pd =
        (ScriptedCompositionPatientDataDefinition) definition;
    Map<String, Mapped<PatientDataDefinition>> containedDataDefintions =
        pd.getContainedDataDefinitions();
    Map<String, EvaluatedPatientData> evaluatedContainedDataDefinitions =
        new HashMap<String, EvaluatedPatientData>();
    ...
    if (pd.getScriptCode() == null) {
      throw new EvaluationException(
          "No script code found on this ScriptedCompositionPatientDataDefinition");
    }

    // fail if passed-in definition has no script type specified
    if (pd.getScriptType() == null) {
      throw new EvaluationException(
          "No script type found on this ScriptedCompositionPatientDataDefinition");
    }
    ...
    for (Entry<String, Mapped<PatientDataDefinition>> d : containedDataDefintions.entrySet()) {
      EvaluatedPatientData patientDataResult =
          Context.getService(PatientDataService.class).evaluate(d.getValue(), context);
    ...
    for (Integer pId : CohortUtil.memberIds(baseCohort)) { // iterate across all patients
      ...
      for (Entry<String, EvaluatedPatientData> dataEntry :
        ...
        scriptEngine.put(
            dataEntry.getKey(),
            o); // put the definition result key and the corresponding actual object directly in the
        // scripting context
      }

      try {
        Object o =
            scriptEngine.eval(pd.getScriptCode()); // execute the script for the current patient.
        evaluationResult.addData(
            pId,
            o); // put the returned object value in the evaluationResult for the current patient
