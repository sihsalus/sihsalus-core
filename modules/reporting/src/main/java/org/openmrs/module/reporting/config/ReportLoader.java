...
public class ReportLoader {
  ...
  public static void saveReportDefinition(ReportDefinition reportDefinition) {
    ...
    ReportDefinition existing =
        Context.getService(ReportDefinitionService.class)
            .getDefinitionByUuid(reportDefinition.getUuid());
    if (existing != null) {
      // we need to overwrite the existing, rather than purge-and-recreate, to avoid deleting old
      // ReportRequests
      reportDefinition.setId(existing.getId());
    } else {
      // incompatible class changes for a serialized object could mean that getting the definition
      // return null
      // and some serialization error gets logged. In that case we want to overwrite the invalid
      // serialized definition
      SerializedObject invalidSerializedObject =
          Context.getRegisteredComponents(SerializedObjectDAO.class)
              .get(0)
              .getSerializedObjectByUuid(reportDefinition.getUuid());
  ...
  public static void saveReportDesigns(
      ReportDefinition reportDefinition, List<ReportDesign> reportDesigns) {
    // purging a ReportDesign doesn't trigger any extra logic, so we can just purge-and-recreate
    // here
    List<ReportDesign> existingDesigns =
        Context.getService(ReportService.class).getReportDesigns(reportDefinition, null, true);
  ...
  public static List<Parameter> constructParameters(
      ...
      for (ParameterDescriptor parameterDescriptor : parameterDescriptors) {
        try {
          ...
          throw new RuntimeException(
              "Unable to configure parameter " + parameterDescriptor.getKey(), e);
        }
      }
    }
    return parameters;
  ...
  public static Object getParameterValue(Class parameterType, String stringVal) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Unable to parse parameter value " + stringVal + " to a " + parameterType);
    }
    return ret;
  }

  public static void addDataSetDefinition(
      ReportDefinition rd, DataSetDescriptor dataSetDescriptor, File path) {
    ...
    if (factory == null) {
      throw new RuntimeException(
          "Unsupported data set descriptor type: " + dataSetDescriptor.getType());
    }
    ...
    // Next, if any data set parameters specify values for report parameters, or are new parameters,
    // add these
  ...
  public static List<ReportDesign> constructReportDesigns(
      ReportDefinition reportDefinition, ReportDescriptor reportDescriptor) {
    ...
    for (DesignDescriptor designDescriptor : reportDescriptor.getDesigns()) {
      ...
      } else if (designDescriptor.getType().equalsIgnoreCase("excel")
          || designDescriptor.getType().equalsIgnoreCase("xls")) {
        design = constructXlsReportDesign(reportDefinition, reportDescriptor, designDescriptor);
      } else {
        try {
          design = new ReportDesign();
          String designName =
              StringUtils.isNotBlank(designDescriptor.getName())
          ...
          if (StringUtils.isNotBlank(designDescriptor.getTemplate())) {
            addReportDesignResource(
                reportDescriptor, design, "template", designDescriptor.getTemplate());
          }
          ...
          throw new RuntimeException(
              "Failed to load report design: "
                  + designDescriptor.getType()
                  + " for report "
      ...
      if (designDescriptor.getProcessors() != null) {
        for (ProcessorDescriptor processorDescriptor : designDescriptor.getProcessors()) {
          ...
          if (processorDescriptor.getProcessorMode() != null) {
            c.setProcessorMode(
                ReportProcessorConfiguration.ProcessorMode.valueOf(
                    processorDescriptor.getProcessorMode()));
          }
          ...
          if (processorDescriptor.getConfiguration() != null) {
            ...
            for (Map.Entry<String, String> config :
                processorDescriptor.getConfiguration().entrySet()) {
              String configValue = config.getValue();
              configValue =
                  configValue.replace(
                      "{{application_data_directory}}", OpenmrsUtil.getApplicationDataDirectory());
  ...
  private static Class<? extends ReportRenderer> getRendererClass(String rendererType)
      throws ClassNotFoundException {
    String fqn =
        rendererType.contains(".")
            ? rendererType
            : "org.openmrs.module.reporting.report.renderer." + rendererType;
    try {
  ...
  private static void addReportDesignResource(
  ...
  public static ReportDesign constructXlsReportDesign(
    ...
    if (StringUtils.isNotBlank(designDescriptor.getTemplate())) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Failed to load XLS template " + designDescriptor.getTemplate(), e);
      }
  ...
  public static List<ReportDescriptor> loadReportDescriptors() {
    ...
    try {
      ...
      if (reportDir.exists()) {
        ...
        files =
            FileUtils.listFiles(
                reportDir, FileFilterUtils.suffixFileFilter("yml"), TrueFileFilter.INSTANCE);
      }
    } catch (Exception e) {
