...
public class VisitDataSetDefinition extends RowPerObjectDataSetDefinition {
  ...
  @ConfigurationProperty private List<Mapped<? extends VisitQuery>> rowFilters;
  ...
  public void addColumn(
    ...
    if (dataDefinition == null) {
      ...
      VisitDataDefinition visitDataDefinition =
          new PatientToVisitDataDefinition((PatientDataDefinition) dataDefinition);
      getColumnDefinitions()
          .add(new RowPerObjectColumnDefinition(name, visitDataDefinition, mappings, converters));
    } else if (dataDefinition instanceof PersonDataDefinition) {
      VisitDataDefinition visitDataDefinition =
          new PersonToVisitDataDefinition((PersonDataDefinition) dataDefinition);
      getColumnDefinitions()
          .add(new RowPerObjectColumnDefinition(name, visitDataDefinition, mappings, converters));
    } else {
  ...
  public void addRowFilter(VisitQuery filter, String mappings) {
    getRowFilters()
        .add(new Mapped<VisitQuery>(filter, ParameterizableUtil.createParameterMappings(mappings)));
  }
