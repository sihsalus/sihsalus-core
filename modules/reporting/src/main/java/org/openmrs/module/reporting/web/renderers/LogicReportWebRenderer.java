...
public class LogicReportWebRenderer extends AbstractWebReportRenderer {
  ...
  public boolean canRender(ReportDefinition reportDefinition) {
    ...
    for (Mapped<? extends DataSetDefinition> def :
        reportDefinition.getDataSetDefinitions().values()) {
      if (def.getParameterizable() instanceof LogicDataSetDefinition) return true;
    }
    return false;
  }
  ...
  public Collection<RenderingMode> getRenderingModes(ReportDefinition definition) {
    ...
    for (Mapped<? extends DataSetDefinition> def : definition.getDataSetDefinitions().values()) {
      if (def.getParameterizable() instanceof LogicDataSetDefinition) ++numMatching;
    }
    List<RenderingMode> ret = new ArrayList<RenderingMode>();
    for (Map.Entry<String, Mapped<? extends DataSetDefinition>> e :
        definition.getDataSetDefinitions().entrySet()) {
      String name = e.getKey();
      DataSetDefinition def = e.getValue().getParameterizable();
      if (def instanceof LogicDataSetDefinition) {
        ret.add(
            new RenderingMode(this, getLabel(def, numMatching == 1), name, Integer.MAX_VALUE - 10));
      }
    }
    return ret;
  }
