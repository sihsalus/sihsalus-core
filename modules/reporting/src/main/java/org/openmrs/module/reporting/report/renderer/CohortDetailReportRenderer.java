...
public class CohortDetailReportRenderer extends ReportDesignRenderer {
  ...
  public Collection<RenderingMode> getRenderingModes(ReportDefinition definition) {
    ...
    List<ReportDesign> designs =
        Context.getService(ReportService.class).getReportDesigns(definition, getClass(), false);
  ...
  /**
   * @see ReportRenderer#render(org.openmrs.module.reporting.report.ReportData, String,
  ...
  public void render(ReportData results, String argument, OutputStream out)
      ...
      if (p != null) {
        String value =
            (e.getValue() instanceof Date
                ? Context.getDateFormat().format((Date) e.getValue())
                : e.getValue().toString());
      ...
      if (ds instanceof MapDataSet) {
        ...
        for (DataSetColumn column : mds.getMetaData().getColumns()) {
          ...
          if (colValue instanceof Cohort) {
            ...
            c =
                ((CohortIndicatorAndDimensionResult) colValue)
      ...
      if (c != null) {
        ...
        try {
          ...
          throw new RenderingException(
              "Error evaluating dataset " + e.getKey(),
              new EvaluationException("dataset: " + e.getKey(), ex));
        }
      }
    }
    ...
    if ("xls".equalsIgnoreCase(args[1])) {
      ...
      w.write("<small>");
      for (Iterator<Map.Entry<String, String>> i = parameterValues.entrySet().iterator();
          i.hasNext(); ) {
      ...
      for (String dataSetKey : datasets.keySet()) {
        ...
        w.write(
            "<table id=\"indicator-report-dataset-"
                + dataSetKey
                + "\" class=\"display indicator-report-dataset\" border=1>");
        w.write("<tr>");
        for (DataSetColumn column : dataset.getMetaData().getColumns()) {
