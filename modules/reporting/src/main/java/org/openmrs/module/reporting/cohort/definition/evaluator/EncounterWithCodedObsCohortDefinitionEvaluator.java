...
public class EncounterWithCodedObsCohortDefinitionEvaluator implements CohortDefinitionEvaluator {
  ...
  public EvaluatedCohort evaluate(CohortDefinition cohortDefinition, EvaluationContext context)
    ...
    EncounterWithCodedObsCohortDefinition cd =
        (EncounterWithCodedObsCohortDefinition) cohortDefinition;
    ...
    if (cd.getEncounterTypeList() != null) {
      q.append("and e.encounter_type in (:etList)")
          .addParameter("etList", cd.getEncounterTypeList());
    }
    if (cd.getLocationList() != null) {
      q.append("and e.location_id in (:locationList)")
          .addParameter("locationList", cd.getLocationList());
    }

    if (cd.isIncludeNoObsValue()
        && cd.getIncludeCodedValues() == null
        && cd.getExcludeCodedValues() == null) {
    ...
    if (cd.getOnOrAfter() != null) {
      q.append("and e.encounter_datetime >= :onOrAfter")
          .addParameter("onOrAfter", cd.getOnOrAfter());
    }
    if (cd.getOnOrBefore() != null) {
      q.append("and e.encounter_datetime <= :onOrBefore")
          .addParameter("onOrBefore", DateUtil.getEndOfDayIfTimeExcluded(cd.getOnOrBefore()));
    }
