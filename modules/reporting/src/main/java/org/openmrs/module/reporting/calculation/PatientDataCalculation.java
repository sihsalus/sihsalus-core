...
public class PatientDataCalculation extends DataCalculation implements PatientCalculation {
  ...
  public CalculationResultMap evaluate(
    ...
    EvaluationContext ec =
        ReportingCalculationUtil.getEvaluationContextForCalculation(
            personIds, parameterValues, context);
    ...
    try {
      ...
      if (MapUtils.isNotEmpty(parameterValues)) {
        for (ParameterDefinition p : getParameterDefinitionSet()) {
          getDataDefinition()
              .getParameter(p.getKey())
              .setDefaultValue(parameterValues.get(p.getKey()));
        }
      }
      ...
      throw new APIException(
          "Evaluation Exception occurred while evaluating " + getDataDefinition(), e);
    }

    if (data == null) {
      throw new IllegalArgumentException(
          "No data generated while evaluating " + getDataDefinition());
    }
