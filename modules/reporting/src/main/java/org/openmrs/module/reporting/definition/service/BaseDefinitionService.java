...
/**
 * Base Implementation of the DefinitionService API. Note that any subclasses that want {@link
 * #getAllDefinitionSummaries(boolean)} to perform well need to override it.
 */
public abstract class BaseDefinitionService<T extends Definition> extends BaseOpenmrsService
    implements DefinitionService<T> {
  ...
  public List<Class<? extends T>> getDefinitionTypes() {
    ...
    for (DefinitionEvaluator<?> e :
        HandlerUtil.getHandlersForType(DefinitionEvaluator.class, null)) {
      ...
      if (type != null) {
        try {
          ...
          throw new IllegalArgumentException(
              "Unable to instantiate a Definition of type: " + type, e);
        }
      } else {
  ...
  /**
   * Note that this base implementation is no more efficient than just calling {@link
   * #getAllDefinitions(boolean)} it should really be overridden in any subclasses that intend to
   * use this functionality.
   *
   * @see
   *     org.openmrs.module.reporting.definition.service.DefinitionService#getAllDefinitionSummaries(boolean)
   */
  @Transactional(readOnly = true)
   ...
   * definition with this configuration exists in the cache (if caching is supported) - returns the
   * cached evaluation result if found - otherwise, delegates to the appropriate Evaluator and
  ...
  public Evaluated<T> evaluate(T definition, EvaluationContext context) throws EvaluationException {
    ...
    // This is a cross-cutting entry point for all evaluations in the reporting module, so doing
    // this here covers
    // all scenarios
  ...
  /**
   * IF YOU ARE A NORMAL CONSUMER OF THE API, DO NOT CALL THIS METHOD! THE CORRECT CONSUMER-FACING
   * METHOD IS: {@link #evaluate(org.openmrs.module.reporting.evaluation.Definition,
   * org.openmrs.module.reporting.evaluation.EvaluationContext)}
   *
   * <p>This method performs the logic of the evaluate method, but it bypasses the logic to exclude
   * test patients. This is used internally by the reporting framework in special cases, e.g. to
   * determine <em>which</em> patients are test patients.
   *
   * <p>(This method will only be visible to an API consumer if a service interface explicitly
   * exposes it, and this should only be done for CohortDefinitionService.)
   *
   * @param definition
   * @param context
   * @return
   ...
   */
  public Evaluated<T> evaluateBypassingExclusionOfTestPatients(
      T definition, EvaluationContext context) throws EvaluationException {
  ...
  /**
   * Ensures that if a "test patients" cohort definition is configured, that no test patients are
   * included in this evaluation context. If the context has a baseCohort defined, we remove those
   * patients ids
   *
   * @param context
   */
  private void ensureNoTestPatients(EvaluationContext context) throws EvaluationException {
    CohortDefinition testPatientsDefinition =
    ...
    if (testPatientsDefinition == null) {
      ...
      EvaluatedCohort testPatients =
          cohortDefinitionService.evaluateBypassingExclusionOfTestPatients(
              testPatientsDefinition, context);
      ...
      if (context.getBaseCohort() == null) {
        ...
        EvaluatedCohort allPatients =
            cohortDefinitionService.evaluateBypassingExclusionOfTestPatients(
                new AllPatientsCohortDefinition(), context);
        ...
        if (!CohortUtil.memberIds(nonTestPatients)
                .containsAll(CohortUtil.memberIds(context.getBaseCohort()))
  ...
  /**
   * Default implementation of how to log status message logging for all services. This can be
   * overridden as desired. By default, this simply logs the status message to the log file
   * associated with a report request. If an implementing service just wants to customize the
   * message, and not where it is logged, they should override the getStatusMessage(Definition,
   * Context) method instead
   */
  protected void logStatusMessage(T definition, EvaluationContext context) {
    try {
      String requestUuid =
          (String) context.getContextValues().get(ReportServiceImpl.REPORT_REQUEST_UUID);
      if (ObjectUtil.notNull(requestUuid)) {
        Context.getService(ReportService.class)
            .logReportMessage(requestUuid, getStatusMessage(definition, context));
      }
    } catch (Exception e) {
  ...
  /**
   * Default implementation of a status message that indicates that this definition is being
   * evaluated
   */
  protected String getStatusMessage(T definition, EvaluationContext context) {
    return "Evaluating "
        + ObjectUtil.nvlStr(definition.getName(), DefinitionUtil.format(definition));
  }

  /**
   * @see DefinitionService#evaluate(Mapped, EvaluationContext)
   */
  public Evaluated<T> evaluate(Mapped<? extends T> definition, EvaluationContext context)
      throws EvaluationException {
  ...
  /**
   * Subclasses should override this method if they need to insert specific behavior around calling
   * the actual evaluators. Examples might include running the evaluators in batches, etc
   */
  protected Evaluated<T> executeEvaluator(
      DefinitionEvaluator<T> evaluator, T definition, EvaluationContext context)
      throws EvaluationException {
  ...
  protected DefinitionPersister<T> getPersister(Class<? extends T> definition) {
    DefinitionPersister<T> persister =
        HandlerUtil.getPreferredHandler(DefinitionPersister.class, definition);
    if (persister == null) {
