...
/**
 * This class provides access to persisted {@link Cohort}s, and exposes them as a {@link
 * CohortDefinition}
 */
@Handler(
    supports = {StaticCohortDefinition.class},
    order = 100)
public class StaticCohortDefinitionPersister implements CohortDefinitionPersister {
  ...
  public List<CohortDefinition> getDefinitions(String name, boolean exactMatchOnly) {
    ...
    if (exactMatchOnly) {
      Cohort c = Context.getCohortService().getCohortByName(name);
      if (c != null) {
        cohorts.add(c);
      }
    } else {
    ...
    if (cohortDefinition != null) {
      if (cohortDefinition instanceof StaticCohortDefinition) {
        ...
        throw new APIException(
            "Unable to save CohortDefinition of type: " + cohortDefinition.getClass());
      }
    }
    return null;
  }
  ...
  public void purgeDefinition(CohortDefinition cohortDefinition) {
    if (cohortDefinition instanceof StaticCohortDefinition) {
      ...
      throw new APIException(
          "Unable to purge CohortDefinition of type: " + cohortDefinition.getClass());
    }
  }
