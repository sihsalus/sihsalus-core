...
public class DefinitionUtil {
      ...
      try {
        for (Property p : getConfigurationProperties(d)) {
          if (ObjectUtil.notNull(p.getValue())) {
            configStr
                .append(configStr.length() > 0 ? "," : "")
                .append(p.getDisplayName())
                .append("=")
                .append(ObjectUtil.format(p.getValue()));
          }
        }
        sb.append(configStr);
      } catch (Exception e) {
  ...
  /**
   * Utility method which takes in an Object and returns a List of {@link Property}s based on the
   * annotated {@link ConfigurationProperty} fields within its class or superclasses.
   *
  ...
  /**
   * Utility method which takes in an Object class and instance and returns a List of {@link
   * Property}s based on the annotated {@link ConfigurationProperty} fields within the classToCheck
   * or its superclasses. This is private as it exists only to support recursion in the above class.
   *
  ...
  private static List<Property> getConfigurationProperties(
      Class<?> classToCheck, Object classInstance) {
  ...
  /**
   * Utility method which takes in a Definition instance and returns a new instance with identical
   * properties for any that are annotated as {@link ConfigurationProperty}
   *
  ...
  public static <T extends Definition> T clone(T instanceToClone) {
    ...
    if (instanceToClone != null) {
      try {
        ...
        Auditable sourceAudit = instanceToClone;
        Auditable newAudit = newInstance;
        newAudit.setChangedBy(sourceAudit.getChangedBy());
        newAudit.setDateChanged(sourceAudit.getDateChanged());
        newInstance.setRetired(instanceToClone.getRetired());
   ...
   */
  @SuppressWarnings("unchecked")
  public static <T extends Definition> DefinitionEvaluator<T> getPreferredEvaluator(T definition)
      throws EvaluationException {
    DefinitionEvaluator<T> evaluator =
        HandlerUtil.getPreferredHandler(DefinitionEvaluator.class, definition.getClass());
    if (evaluator == null) {
      throw new EvaluationException(
          "No Evaluator found for (" + definition.getClass() + ") " + definition.getName());
    }
    return evaluator;
  }
   ...
   */
  public static <T extends Definition> T cloneDefinitionWithContext(
      T definition, EvaluationContext context) {
   ...
   */
  @SuppressWarnings("unchecked")
  public static <P> P getConfiguredProperty(
      Definition d, String name, EvaluationContext context, Class<P> type) {
