...
public class CohortIndicatorResult implements IndicatorResult {
  ...
  private Map<Integer, Number> logicResults =
      new HashMap<Integer, Number>(); // patient id -> logic value
  ...
  public static Number getResultValue(
      CohortIndicatorResult cohortIndicatorResult, Cohort... filters) {
    ...
    Map<Integer, Number> logicVals =
        new HashMap<Integer, Number>(cohortIndicatorResult.getLogicResults());
    ...
    if (type == IndicatorType.FRACTION) {
      ...
      Class<? extends Aggregator> aggregator =
          cohortIndicatorResult.getDefinition().getAggregator();
      if (aggregator == null) {
