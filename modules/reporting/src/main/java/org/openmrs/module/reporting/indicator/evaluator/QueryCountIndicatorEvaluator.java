...
public class QueryCountIndicatorEvaluator implements IndicatorEvaluator {
  ...
  @Autowired PersonQueryService personQueryService;
  ...
  public IndicatorResult evaluate(Indicator indicator, EvaluationContext context)
    ...
    if (q instanceof PersonQuery) {
      ...
      idSet =
          personQueryService.evaluate(
              new Mapped<PersonQuery>(personQuery, qi.getQuery().getParameterMappings()), context);
      ...
      idSet =
          cohortDefinitionService.evaluate(
              new Mapped<CohortDefinition>(cohortDefinition, qi.getQuery().getParameterMappings()),
              context);
    } else if (q instanceof EncounterQuery) {
      EncounterQuery encounterQuery = (EncounterQuery) q;
      idSet =
          encounterQueryService.evaluate(
              new Mapped<EncounterQuery>(encounterQuery, qi.getQuery().getParameterMappings()),
              context);
    } else if (q instanceof ObsQuery) {
      ObsQuery obsQuery = (ObsQuery) q;
      idSet =
          obsQueryService.evaluate(
              new Mapped<ObsQuery>(obsQuery, qi.getQuery().getParameterMappings()), context);
    }

    if (idSet == null) {
      throw new EvaluationException(
          getClass().getSimpleName()
              + " does not support Query instances of type "
              + q.getClass().getName());
    }
