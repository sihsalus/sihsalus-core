...
public class ObsDataUtil {
  ...
  /**
   * @return the base set of obs ids relevant for the passed EvaluationContext or null for all obs
   *     ids If returnNullForAllObsIds is false, then this will return all obs ids in the system if
   *     unconstrained by the context
   */
  public static Set<Integer> getObsIdsForContext(
      EvaluationContext context, boolean returnNullForAllObsIds) {

    Cohort patIds = context.getBaseCohort();
    ObsIdSet obsIds =
        (context instanceof ObsEvaluationContext
            ? ((ObsEvaluationContext) context).getBaseObs()
            : null);
