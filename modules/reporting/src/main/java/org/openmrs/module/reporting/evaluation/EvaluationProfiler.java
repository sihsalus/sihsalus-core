...
/**
 * Class that can be used to profile the time a method takes to execute Outputs TRACE level log
 * messages with timing information. To view those messages ensure that that the EvaluationProfiler
...
public class EvaluationProfiler {
  ...
  public EvaluationProfiler(EvaluationContext context) {
    ...
    this.logFile =
        new File(
            getLogDirectory(),
            EVALUATION_LOG_FILE_PREFIX
                + DateUtil.formatDate(new Date(overallStartTime), "yyyyMMdd")
                + ".log");
  }
