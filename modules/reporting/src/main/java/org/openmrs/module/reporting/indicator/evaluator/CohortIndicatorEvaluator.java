...
public class CohortIndicatorEvaluator implements IndicatorEvaluator {
   ...
   *
   * <p>Note that criteria should be a LogicCriteria. I'm using reflection so this code works on
   * both 1.5 (where LogicCriteria is a class) and 1.6+ (where it's an interface)
   *
   * @param criteria
   * @param context
   */
  private static String[] possibilities =
      new String[] {"indexDate", "date", "endDate", "startDate"};
