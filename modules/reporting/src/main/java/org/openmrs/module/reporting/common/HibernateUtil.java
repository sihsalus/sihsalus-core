...
public class HibernateUtil {
   ...
   * OpenMRS 1.9). See https://hibernate.onjira.com/browse/HHH-5138. For example Hibernate.STRING is
   * now StandardBasicTypes.STRING.
   *
   * @param typeName
   * @return the org.hibernate.type.Type, fetched as a static constant from either the Hibernate
   *     class or the StandardBasicTypes class, depending on which is available.
   */
  ...
  /**
   * Hibernate made a non-backwards-compatible change in version 3.6 (which we use starting in
   * OpenMRS 1.9). See https://hibernate.onjira.com/browse/HHH-5138. TypeFactory.basic no longer
   * exists. (I don't know if we need both this method and {@link #standardType(String)}, but I'm
   * hackily replacing bits of code that I don't understand deeply, and they do two things.)
   *
   * @param name
  ...
  public static Type getBasicType(String name, Properties parameters) {
    try {
      ...
      try {
        Object typeResolver =
            Context.loadClass("org.hibernate.type.TypeResolver")
  ...
  /**
   * Hibernate made a non-backwards-compatible change in version 3.6 (which we use starting in
   * OpenMRS 1.9). See https://hibernate.onjira.com/browse/HHH-5138. There are a few places where in
   * 3.2.5 we'd have a NullableType, but in 3.6 we'll have something like a
   * AbstractSingleColumnStandardBasicType, but either way there's a sqlType() method
   *
   * @param type
