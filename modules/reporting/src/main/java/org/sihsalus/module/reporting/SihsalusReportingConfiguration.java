...
// Static module wiring must still expose deprecated reporting dataset-query services for legacy
// callers.
...
public class SihsalusReportingConfiguration {
  ...
  @Bean
  SmartInitializingSingleton reportingSerializedObjectTypeRegistrar(
  ...
  ReportService reportingReportService(
      ReportDAO reportingReportDao,
      @Qualifier("runQueuedReportsTask") ReportingTimerTask runQueuedReportsTask) {
  ...
  SmartInitializingSingleton reportingGlobalPropertyListenerRegistrar(
    ...
    return () ->
        eventListeners.setGlobalPropertyListeners(new ArrayList<>(List.of(reportingConstants)));
  }
  ...
  SmartInitializingSingleton reportingServiceRegistrar(
    ...
    return () -> {
      serviceContext.setService(
          SerializedDefinitionService.class, reportingSerializedDefinitionService);
