...
public abstract class TransactionEvent extends ApplicationEvent {
  ...
  @Getter private Set<EntityEvent> events;
    ...
    if (incomingEvents != null) {
      ...
      for (EntityEvent incomingEvent : incomingEvents) {
        ...
        if (!hasEvent && incomingEvent.getAction() == Event.Action.UPDATED) {
          hasEvent =
              events.contains(new EntityEvent(incomingEvent.getEntity(), Event.Action.CREATED));
          hasEvent =
              hasEvent
                  || events.contains(
                      new EntityEvent(incomingEvent.getEntity(), Event.Action.PURGED));
        }
