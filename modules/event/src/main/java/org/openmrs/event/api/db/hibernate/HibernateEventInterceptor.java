...
public class HibernateEventInterceptor implements Interceptor, ApplicationEventPublisherAware {
  ...
  private void publishEvent(TransactionEvent event) {
    if (applicationEventPublisher != null) {
      ...
      throw new IllegalStateException(
          "Unable to publish application event, event publisher is null");
    }
  }
  ...
  public boolean onSave(
    ...
    return false; // tells hibernate that there are no changes made here that need to be propagated
    // to the persistent object and DB
  }

  /**
   * This is called only when an entity is updated, not when it is created The voided property is a
    ...
    if (entity instanceof Retireable || entity instanceof Voidable) {
      ...
      for (int i = 0; i < propertyNames.length; i++) {
        ...
        if (propertyName.equals(property)) {
          ...
          currentValue = currentState != null && BooleanUtils.isTrue((Boolean) currentState[i]);
        }
      }
      if (currentValue) {
        if (!previousValue) {
  ...
  /**
   * This is invoked when the collection associated with an object is removed Consider this to be an
  ...
  /**
   * This is invoked when the collection associated with an object is recreated Consider this to be
   * an update of the object containing the collection
   */
  @Override
  ...
  /**
   * This is invoked when the collection associated with an object is updated Consider this to be an
   * update of the object containing the collection
   */
  @Override
