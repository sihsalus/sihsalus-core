...
public class PatientIdStickerPdfReport {
  ...
  @Autowired private PatientIdStickerDataSetEvaluator evaluator;

  @Autowired private InitializerService initializerService;
  ...
  public byte[] generatePdf(Patient patient) throws RuntimeException {
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to generate patient ID sticker for patient: " + patientId, e);
    }
  }
  ...
  private String getStylesheetName() {
    String stylesheetName =
        initializerService.getValueFromKey("report.patientIdSticker.stylesheet");
  ...
  private void writeToOutputStream(
      StreamSource xmlSource, StreamSource xslSource, OutputStream outStream)
    ...
    try (InputStream fopConfigStream =
        OpenmrsClassLoader.getInstance().getResourceAsStream(FOP_CONFIG_PATH)) {
  ...
  private void configureSecureTransformerFactory(TransformerFactory factory)
