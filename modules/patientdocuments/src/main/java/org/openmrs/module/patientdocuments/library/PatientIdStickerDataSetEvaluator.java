...
public class PatientIdStickerDataSetEvaluator implements DataSetEvaluator {
  ...
  private Map<String, Object> convertPatientToMap(Patient patient) {
    ...
    LocalDateTime birthDateTime =
        birthdate.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
    ...
    patientData.put(
        "causeOfDeath",
        patient.getCauseOfDeath() != null ? patient.getCauseOfDeath().getName() : null);
  ...
  private Map<String, Object> convertAddressToMap(PersonAddress address) {
    ...
    // TODO: As future work, we should probably consider making the address as formatted by the
    // address template available
  ...
  private Map<String, Object> convertIdentifierToMap(PatientIdentifier identifier) {
    ...
    identifierData.put(
        "location", identifier.getLocation() != null ? identifier.getLocation().getName() : null);
  ...
  /**
   * Calculates a human-readable age or time duration between a birth date and now. The output
   * format changes based on the duration: - Less than 1 minute: "just now" - 1-119 minutes: "X
   * minute(s)" - 2-47 hours: "X hour(s)" - 2-27 days: "X day(s)" - 4-51 weeks: "X week(s) [Y
   * day(s)]" - 1 year: "X month(s) [Y day(s)]" - 2-17 years: "X year(s) [Y month(s)]" - 18+ years:
   * "X years"
   *
  ...
  private String calculateAge(Date birthDate) {
    ...
    if (weeks.isLessThan(Weeks.weeks(52))) {
      ...
      return remainingDays == 0
          ? formatUnit(weeks.getWeeks(), "week", "weeks")
          : formatUnit(weeks.getWeeks(), "week", "weeks")
              + " "
              + formatUnit(remainingDays, "day", "days");
    }
    ...
    if (months.isLessThan(Months.months(24))) {
      ...
      return remainingDays == 0
          ? formatUnit(months.getMonths(), "month", "months")
          : formatUnit(months.getMonths(), "month", "months")
              + " "
              + formatUnit(remainingDays, "day", "days");
    }
    ...
    if (years.isLessThan(Years.years(18))) {
      ...
      return remainingMonths == 0
          ? formatUnit(years.getYears(), "year", "years")
          : formatUnit(years.getYears(), "year", "years")
              + " "
              + formatUnit(remainingMonths, "month", "months");
    }
