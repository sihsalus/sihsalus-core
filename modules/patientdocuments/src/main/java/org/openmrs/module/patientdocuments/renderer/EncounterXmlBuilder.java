...
public class EncounterXmlBuilder {
  ...
  private String getLogoContent() {
    String logoPath =
        getInitializerService()
            .getValueFromKey(PatientDocumentsConstants.ENCOUNTER_PRINTING_LOGO_PATH_KEY);
  ...
  public String build(EncounterPrintingContext printingContext) {
    ...
    for (Encounter encounter : printingContext.getEncounters()) {
      if (encounter.getForm() != null) {
        xml.append(
            renderSingleEncounter(encounter, printingContext.getLocale(), o3FormsService, user));
      }
    }
  ...
  private String renderSingleEncounter(
      Encounter encounter, Locale locale, O3FormsService o3FormsService, User user) {
    StringBuilder xml = new StringBuilder();
  ...
  private String buildHeaderContent(Encounter encounter, Locale locale) {
    ...
    if (isHeaderFieldEnabled("location")) {
      String locationName =
          encounter.getLocation() != null
              ? encounter.getLocation().getName()
              : PatientDocumentsConstants.MISSING_VALUE_PLACEHOLDER;
      xml.append("<location>").append(escape(locationName)).append("</location>");
    }

    if (isHeaderFieldEnabled("encounterDate")) {
      xml.append("<encounterDate>")
          .append(
              escape(OpenmrsUtil.getDateFormat(locale).format(encounter.getEncounterDatetime())))
          .append("</encounterDate>");
    }

    if (isHeaderFieldEnabled("visitStartDate")) {
      String visitStartDate =
          (visit != null && visit.getStartDatetime() != null)
              ? OpenmrsUtil.getDateFormat(locale).format(visit.getStartDatetime())
              : PatientDocumentsConstants.MISSING_VALUE_PLACEHOLDER;
      xml.append("<visitStartDate>").append(escape(visitStartDate)).append("</visitStartDate>");
    }

    if (isHeaderFieldEnabled("visitEndDate")) {
      String visitEndDate =
          (visit != null && visit.getStopDatetime() != null)
              ? OpenmrsUtil.getDateFormat(locale).format(visit.getStopDatetime())
              : PatientDocumentsConstants.MISSING_VALUE_PLACEHOLDER;
      xml.append("<visitEndDate>").append(escape(visitEndDate)).append("</visitEndDate>");
    }
    ...
    if (isHeaderFieldEnabled("visitType")) {
      String visitTypeName =
          (visit != null && visit.getVisitType() != null)
              ...
              : PatientDocumentsConstants.MISSING_VALUE_PLACEHOLDER;
      xml.append("<visitType>").append(escape(visitTypeName)).append("</visitType>");
    }
  ...
  private String buildFooterContent(User user, Locale locale) {
    ...
    String userName =
        (user != null && user.getPersonName() != null)
    ...
    String printedBy =
        String.format("Printed by %s (%s) at %s", userName, systemId, printTimestamp);
    xml.append("<printedBy>").append(escape(printedBy)).append("</printedBy>");

    String customFooterText =
        getInitializerService()
            .getValueFromKey(
                PatientDocumentsConstants.ENCOUNTER_PRINTING_FOOTER_PREFIX + "customText");
    if (StringUtils.isNotBlank(customFooterText)) {
      xml.append("<customFooterText>")
          .append(escape(customFooterText))
          .append("</customFooterText>");
    }
  ...
  private String buildMainContent(
      Encounter encounter, Locale locale, O3FormsService o3FormsService) {
    StringBuilder xml = new StringBuilder();

    xml.append("<formName>").append(escape(encounter.getForm().getName())).append("</formName>");
  ...
  private String renderSection(
      Map<String, Object> section, Map<String, List<Obs>> obsMap, Locale locale) {
    StringBuilder xml = new StringBuilder();
  ...
  private String renderObsGroupQuestion(
      Map<String, Object> question, Map<String, List<Obs>> obsMap, Locale locale) {
    StringBuilder sb = new StringBuilder();
  ...
  private String renderObsQuestion(
      Map<String, Object> question, Map<String, List<Obs>> obsMap, Locale locale) {
    StringBuilder sb = new StringBuilder();
    String label = getQuestionLabel(question, locale);
  ...
  private String renderPatientIdentifiers(Patient patient) {
    StringBuilder xml = new StringBuilder();
    String configuredPatientIdentifierTypes =
        getInitializerService()
            .getValueFromKey(
                PatientDocumentsConstants.ENCOUNTER_PRINTING_HEADER_PREFIX
    ...
    for (PatientIdentifier identifier : identifiers) {
      ...
      if (identifierTypes.isEmpty() || identifierTypes.contains(identifierTypeName)) {
        ...
        xml.append("<identifier type=\"")
            .append(escape(identifierTypeName))
            .append("\">")
  ...
  private String renderPersonAttributes(Patient patient) {
    StringBuilder xml = new StringBuilder();
    String configuredPersonAttributeTypes =
        getInitializerService()
            .getValueFromKey(
                PatientDocumentsConstants.ENCOUNTER_PRINTING_HEADER_PREFIX
    ...
    for (PersonAttribute attribute : attributes) {
      ...
      if (attributeTypes.isEmpty() || attributeTypes.contains(attributeTypeName)) {
        ...
        xml.append("<attribute type=\"")
            .append(escape(attributeTypeName))
            .append("\">")
  ...
  private String renderVisitAttributes(Visit visit) {
    ...
    String configuredVisitAttributeTypes =
        getInitializerService()
            .getValueFromKey(
                PatientDocumentsConstants.ENCOUNTER_PRINTING_HEADER_PREFIX + "visitAttributeTypes");
