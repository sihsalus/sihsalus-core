...
public class DailyRecurringAppointmentsGenerationService
    extends AbstractRecurringAppointmentsService {
  ...
  public List<Appointment> generateAppointments(
    ...
    Date endDate =
        getEndDate(
            recurringPattern.getPeriod(),
            recurringPattern.getFrequency(),
            recurringPattern.getEndDate(),
            recurringAppointmentRequest);
    List<Pair<Date, Date>> appointmentDates =
        getAppointmentDates(
            endDate,
            DateUtil.getCalendar(
                recurringAppointmentRequest.getAppointmentRequest().getStartDateTime()),
            DateUtil.getCalendar(
                recurringAppointmentRequest.getAppointmentRequest().getEndDateTime()),
            recurringAppointmentRequest);
    return createAppointments(
        appointmentDates, recurringAppointmentRequest.getAppointmentRequest());
  }
  ...
  private List<Pair<Date, Date>> getAppointmentDates(
      Date endDate,
    ...
    while (!currentAppointmentDate.after(endDate)) {
      ...
      startCalendar.add(
          Calendar.DAY_OF_YEAR, recurringAppointmentRequest.getRecurringPattern().getPeriod());
      endCalendar.add(
          Calendar.DAY_OF_YEAR, recurringAppointmentRequest.getRecurringPattern().getPeriod());
  ...
  public List<Appointment> addAppointments(
    ...
    List<Appointment> activeAppointments =
        new ArrayList<>(appointmentRecurringPattern.getActiveAppointments());
    ...
    recurringAppointmentRequest
        ...
        .setStartDateTime(appointments.get(appointments.size() - 1).getStartDateTime());
    recurringAppointmentRequest
        .getAppointmentRequest()
        .setEndDateTime(appointments.get(appointments.size() - 1).getEndDateTime());
    ...
    Date endDate =
        getEndDate(
            appointmentRecurringPattern.getPeriod(),
            appointmentRecurringPattern.getFrequency(),
    ...
    appointments.addAll(
        createAppointments(
            getAppointmentDates(endDate, startCalendar, endCalendar, recurringAppointmentRequest),
