...
public class AppointmentMapper {
  ...
  @Autowired AppointmentServiceMapper appointmentServiceMapper;
  ...
  public List<AppointmentDefaultResponse> constructResponse(List<Appointment> appointments) {
    return appointments.stream()
        .map(as -> this.mapToDefaultResponse(as, new AppointmentDefaultResponse()))
  ...
  public void mapAppointmentRequestToAppointment(
      AppointmentRequest appointmentRequest, Appointment appointment) {
    AppointmentServiceDefinition appointmentServiceDefinition =
        appointmentServiceDefinitionService.getAppointmentServiceByUuid(
            appointmentRequest.getServiceUuid());
    if (appointmentServiceDefinition == null) {
      throw new ConversionException(
          "Bad Request. No appointment service found with UUID: "
              + appointmentRequest.getServiceUuid());
    }
    ...
    if (StringUtils.isNotBlank(appointmentRequest.getServiceTypeUuid())) {
      appointmentServiceType =
          getServiceTypeByUuid(
              ...
              appointmentRequest.getServiceTypeUuid());
    }
    ...
    appointment.setAppointmentKind(
        AppointmentKind.valueOf(appointmentRequest.getAppointmentKind()));
  ...
  private void mapProvidersForAppointment(
      Appointment appointment, List<AppointmentProviderDetail> newProviders) {
    ...
    if (existingProviders != null) {
      for (AppointmentProvider appointmentProvider : existingProviders) {
        boolean exists =
            newProviders != null
                && newProviders.stream()
                    .anyMatch(p -> p.getUuid().equals(appointmentProvider.getProvider().getUuid()));
        if (!exists) {
    ...
    if (newProviders != null && !newProviders.isEmpty()) {
      ...
      for (AppointmentProviderDetail providerDetail : newProviders) {
        List<AppointmentProvider> providers =
            appointment.getProviders().stream()
                .filter(p -> p.getProvider().getUuid().equals(providerDetail.getUuid()))
                .collect(Collectors.toList());
        if (providers.isEmpty()) {
          ...
          providers.forEach(
              existingAppointmentProvider -> {
                ...
                existingAppointmentProvider.setResponse(
                    mapProviderResponse(providerDetail.getResponse()));
  ...
  private void mapReasonsForAppointment(Appointment appointment, List<String> reasonConceptUuids) {
    ...
    for (String conceptUuid : reasonConceptUuids) {
      ...
      if (concept.getRetired()) {
        throw new ConversionException(
            "Bad Request. Concept with UUID: " + conceptUuid + " is retired");
      }

      boolean exists =
          existingReasons.stream()
              .anyMatch(
                  r -> r.getConcept() != null && r.getConcept().getUuid().equals(conceptUuid));
  ...
  private AppointmentProvider createNewAppointmentProvider(
      AppointmentProviderDetail providerDetail) {
  ...
  public AppointmentProviderResponse mapProviderResponse(String response) {
    String namedEnum =
        StringUtils.isEmpty(response)
            ? AppointmentProviderResponse.ACCEPTED.toString()
        ...
        .orElseThrow(
            () ->
                new ConversionException(
                    "Bad Request. No appointment service type found with UUID: "
                        + serviceTypeUuid));
  }
  ...
  private AppointmentDefaultResponse mapToDefaultResponse(
      Appointment a, AppointmentDefaultResponse response) {
    ...
    if (a.getNotificationResults() != null) {
      List<HashMap<String, String>> collect =
          a.getNotificationResults().stream()
              .map(
                  nr -> {
  ...
  private List<AppointmentProviderDetail> mapAppointmentProviders(
  ...
  private List<AppointmentReasonResponse> mapAppointmentReasons(Set<AppointmentReason> reasons) {
    ...
    for (AppointmentReason appointmentReason : reasons) {
      ...
      AppointmentReasonResponse response =
          new AppointmentReasonResponse(concept.getUuid(), conceptName);
  ...
  private Map<String, Object> createPatientMap(Patient p) {
    ...
    map.putAll(
        p.getActiveIdentifiers().stream()
            .filter(e -> e.getIdentifierType() != null)
            .collect(
                Collectors.toMap(
                    e -> e.getIdentifierType().toString().replaceAll("[- ]", ""),
                    e -> e.getIdentifier(),
                    (e1, e2) -> e1 + "," + e2)));
    return map;
  }
  ...
  private static Map<String, String> mapAttributeTypes(Patient p) {
    String customPersonAttributeTypes =
        Context.getAdministrationService()
            .getGlobalProperty("appointments.customPersonAttributeTypes");
    String[] attributeTypes =
        customPersonAttributeTypes != null ? customPersonAttributeTypes.split(",") : new String[0];
