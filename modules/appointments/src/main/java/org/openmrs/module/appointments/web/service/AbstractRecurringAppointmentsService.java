...
public abstract class AbstractRecurringAppointmentsService {
  ...
  @Autowired private TeleconsultationAppointmentService teleconsultationAppointmentService;

  public abstract List<Appointment> generateAppointments(
  ...
  protected List<Appointment> createAppointments(
      List<Pair<Date, Date>> appointmentDates, AppointmentRequest appointmentRequest) {
    List<Appointment> appointments = new ArrayList<>();
    appointmentDates.forEach(
        appointmentDate -> {
  ...
  public List<Appointment> removeRecurringAppointments(
      AppointmentRecurringPattern appointmentRecurringPattern,
      RecurringAppointmentRequest recurringAppointmentRequest) {
    List<Appointment> appointments =
        sort(new ArrayList<>(appointmentRecurringPattern.getActiveAppointments()));
    ...
    if (appointmentRecurringPattern.getEndDate() == null)
      appointmentRecurringPattern.setFrequency(
          appointmentRecurringPattern.getFrequency()
              - recurringAppointmentRequest.getRecurringPattern().getFrequency());
    else
      appointmentRecurringPattern.setEndDate(
          recurringAppointmentRequest.getRecurringPattern().getEndDate());
    recurringAppointmentRequest
        .getAppointmentRequest()
        .setStartDateTime(appointments.get(0).getStartDateTime());
    recurringAppointmentRequest
        .getAppointmentRequest()
        .setEndDateTime(appointments.get(0).getEndDateTime());
    Calendar startCalender = Calendar.getInstance();
    Date currentDate = startCalender.getTime();
    List<Integer> removableAppointments = new ArrayList<>();
    removableAppointments =
        getRemovableAppointments(
  ...
  protected void addRemovedAndRelatedAppointments(
      List<Appointment> appointments, AppointmentRecurringPattern appointmentRecurringPattern) {
  ...
  private void checkAppointmentStatus(
      List<Appointment> appointments, List<Integer> removableAppointments) {
    for (Integer removableAppointment : removableAppointments) {
  ...
  private List<Integer> getRemovableAppointments(
      AppointmentRecurringPattern appointmentRecurringPattern,
      RecurringAppointmentRequest recurringAppointmentRequest,
      List<Appointment> appointments,
      Date currentDate,
    ...
    if (appointmentRecurringPattern.getEndDate() == null) {
      if (appointments
          .get(recurringAppointmentRequest.getRecurringPattern().getFrequency())
          .getStartDateTime()
          .before(currentDate))
        throw new APIException("Changes cannot be made as the appointments are from past date");
      for (int index = recurringAppointmentRequest.getRecurringPattern().getFrequency();
          index < appointments.size();
          index++) removableAppointments.add(index);
    } else {
      ...
      for (int index = 0; index < appointments.size(); index++) {
        if (appointments
            .get(index)
            .getStartDateTime()
            .after(appointmentRecurringPattern.getEndDate())) {
