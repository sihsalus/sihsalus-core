...
public class DefaultAppointmentStatusChangeValidator implements AppointmentStatusChangeValidator {
  ...
  public void validate(Appointment appointment, AppointmentStatus toStatus, List<String> errors) {
    ...
    boolean disableValidation =
        disableDefaultValidationValue != null
            ? Boolean.valueOf(disableDefaultValidationValue)
            : false;
    if (!disableValidation) {
      AppointmentStatus currentStatus = appointment.getStatus();
      if (toStatus.getSequence() <= currentStatus.getSequence()
          && toStatus != AppointmentStatus.Scheduled) {
        errors.add(
            "Appointment status can not be changed from "
