...
public class AppointmentsServiceImpl implements AppointmentsService {
  ...
  public void setStatusChangeValidators(
  ...
  public void setTeleconsultationAppointmentService(
  ...
  public void setAppointmentNotifierService(
      PatientAppointmentNotifierService appointmentNotifierService) {
  ...
  private boolean isCurrentUserSamePersonAsOneOfTheAppointmentProviders(
  ...
  private void checkAndAssignAppointmentNumber(Appointment appointment) {
    if (appointment.getAppointmentNumber() == null) {
      ...
      String generateAppointmentNumber =
          appointmentNumberGenerator.generateAppointmentNumber(appointment);
  ...
  public List<Appointment> searchAppointmentsWithoutDates(
    ...
    String limitString =
        Context.getAdministrationService().getGlobalProperty("webservices.rest.maxResultsDefault");
  ...
  private void setupTeleconsultation(Appointment appointment) {
    if (isVirtual(appointment)) {
      appointment.setTeleHealthVideoLink(
          teleconsultationAppointmentService.generateTeleconsultationLink(appointment.getUuid()));
    }
  }

  private boolean isVirtual(Appointment appointment) {
    return appointment.getAppointmentKind() != null
        && appointment.getAppointmentKind().equals(AppointmentKind.Virtual);
  }

  private void notifyUpdates(Appointment appointment) {
    List<NotificationResult> notificationResults =
        appointmentNotifierService.notifyAll(appointment);
    ...
    if (!validateIfUserHasSelfOrAllAppointmentsAccess(appointment)) {
      throw new APIAuthenticationException(
          Context.getMessageSourceService()
              .getMessage(
                  PRIVILEGES_EXCEPTION_CODE,
                  new Object[] {MANAGE_APPOINTMENTS, MANAGE_OWN_APPOINTMENTS},
                  null));
    }
  ...
  public List<Appointment> search(Appointment appointment) {
    ...
    return appointments.stream()
        .filter(searchedAppointment -> !isServiceOrServiceTypeVoided(searchedAppointment))
        .collect(Collectors.toList());
  }
  ...
  public List<Appointment> getAppointmentsForService(
    ...
    return appointmentDao.getAppointmentsForService(
        appointmentServiceDefinition, startDate, endDate, appointmentStatusList);
  }
  ...
  @Transactional
  @Override
  public void changeStatus(Appointment appointment, String status, Date onDate)
      throws APIException {
    AppointmentStatus appointmentStatus = AppointmentStatus.valueOf(status);
    validateUserPrivilege(appointment, appointmentStatus);
    appointmentServiceHelper.validateStatusChangeAndGetErrors(
        appointment, appointmentStatus, statusChangeValidators);
    validateUserPrivilege(appointment, appointmentStatus);
  ...
  private void validateUserPrivilege(Appointment appointment, AppointmentStatus appointmentStatus) {
    if (!validateIfUserHasSelfOrAllAppointmentsAccess(appointment)) {
      throw new APIAuthenticationException(
          Context.getMessageSourceService()
              .getMessage(PRIVILEGES_EXCEPTION_CODE, new Object[] {MANAGE_APPOINTMENTS}, null));
    }
    if (!isUserAllowedToResetStatus(appointmentStatus, appointment.getStatus())) {
      throw new APIAuthenticationException(
          Context.getMessageSourceService()
              .getMessage(
                  PRIVILEGES_EXCEPTION_CODE, new Object[] {RESET_APPOINTMENT_STATUS}, null));
    }
  }

  private boolean isUserAllowedToResetStatus(
      AppointmentStatus toStatus, AppointmentStatus currentStatus) {
  ...
  public List<Appointment> getAllAppointmentsInDateRange(Date startDate, Date endDate) {
    List<Appointment> appointments =
        appointmentDao.getAllAppointmentsInDateRange(startDate, endDate);
    return appointments.stream()
        .filter(appointment -> !isServiceOrServiceTypeVoided(appointment))
        .collect(Collectors.toList());
  }
  ...
  public void undoStatusChange(Appointment appointment) throws APIException {
    ...
    } else throw new APIException("No status change actions to undo");
  }
    ...
    if (!isNull(appointmentSearchRequest.getLimit())) {
      String limit =
          Context.getAdministrationService()
              .getGlobalProperty("webservices.rest.maxResultsDefault");
      Integer configuredLimit =
          parseIntegerGlobalProperty("webservices.rest.maxResultsDefault", limit);
  ...
  private List<Appointment> getNonVoidedFutureAppointments(List<Appointment> appointments) {
    return appointments.stream()
        .filter(
            appointment -> {
              return !(appointment.getVoided()
                  || appointment.getStartDateTime().before(getStartOfDay()));
            })
        .collect(Collectors.toList());
  }
  ...
  public void updateAppointmentProviderResponse(AppointmentProvider providerWithNewResponse) {
    ...
    AppointmentProvider existingProviderInAppointment =
        findProviderInAppointment(providerWithNewResponse, existingProviders);
  ...
  private AppointmentProvider findProviderInAppointment(
      AppointmentProvider providerWithNewResponse, Set<AppointmentProvider> providers) {
    Optional<AppointmentProvider> providerInAppointment =
        providers.stream()
            .filter(
  ...
  public Appointment reschedule(
      String originalAppointmentUuid, Appointment newAppointment, boolean retainAppointmentNumber) {
    ...
    if (prevAppointment == null) {
      ...
      String msg =
          String.format(
              "Can not identify appointment for rescheduling with %s", originalAppointmentUuid);
      log.error(msg);
    ...
    try {
      ...
      createEventInAppointmentAudit(
          prevAppointment, appointmentServiceHelper.getAppointmentAsJsonString(prevAppointment));
  ...
  private void createEventInAppointmentAudit(Appointment appointment, String notes) {
    AppointmentAudit appointmentAuditEvent =
        appointmentServiceHelper.getAppointmentAuditEvent(appointment, notes);
    appointmentAuditDao.save(appointmentAuditEvent);
  }

  private boolean isFirstAcceptForRequestedAppointment(
      AppointmentProvider providerWithNewResponse, Appointment appointment) {
  ...
  private void createAppointmentAudit(
      AppointmentProvider providerWithNewResponse,
      Appointment appointment,
      AppointmentProvider appointmentProvider) {
    String notes =
        String.format(
            "Changed Provider Response to %s for provider with UUID %s in appointment with UUID %s",
            providerWithNewResponse.getResponse(),
            appointmentProvider.getProvider().getUuid(),
            appointment.getUuid());
    createEventInAppointmentAudit(appointment, notes);
  }
  ...
  public void setAppointmentNumberGeneratorLocator(
  ...
  @Transactional
  @Override
  public List<Appointment> changeStatusForAppointments(
      List<String> appointmentUuids, AppointmentStatus toStatus) {
    ...
    for (Appointment appointment : appointments) {
      ...
      log.debug(
          "Changed status to " + toStatus + " for appointment UUID: " + appointment.getUuid());
    }
