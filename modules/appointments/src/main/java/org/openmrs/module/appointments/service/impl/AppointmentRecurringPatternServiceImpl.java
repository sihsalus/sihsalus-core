...
public class AppointmentRecurringPatternServiceImpl implements AppointmentRecurringPatternService {
  ...
  public void setAppointmentRecurringPatternDao(
  ...
  @Override
  public AppointmentRecurringPattern validateAndSave(
  ...
  @Override
  public AppointmentRecurringPattern update(
  ...
  private void updateAppointmentsDetails(
      AppointmentRecurringPattern appointmentRecurringPattern, List<Appointment> appointments) {
  ...
  private void assignAppointmentNumbers(
      List<Appointment> appointments, AppointmentRecurringPattern pattern) {
    if (appointments.isEmpty()) {
      return;
    }

    RecurringAppointmentNumberGenerator generator =
        appointmentNumberGeneratorLocator.retrieveRecurringAppointmentNumberGenerator();

    if (generator == null) {
      log.warn(
          "Can not apply appointment numbers. No recurring appointment number generator found");
      return;
    }
  ...
  public Appointment update(
    ...
    Appointment editedAppointment =
        updatedAppointments.stream()
            .filter(app -> !app.getVoided())
            .collect(Collectors.toList())
            .get(0);
    updateAppointmentsDetails(appointmentRecurringPattern, updatedAppointments);
    appointmentServiceHelper.validate(
        editedAppointment.getRelatedAppointment(), editAppointmentValidators);
    appointmentRecurringPatternDao.save(appointmentRecurringPattern);
    return editedAppointment;
  }

  @Override
  public List<Appointment> changeStatus(
      Appointment appointment, String status, String clientTimeZone) {
  ...
  private List<Appointment> getPendingOccurrences(
      String appointmentUuid, List<AppointmentStatus> applicableStatusList) {
    Date startOfDay = getStartOfDay();
    Appointment appointment = appointmentDao.getAppointmentByUuid(appointmentUuid);
    return appointment.getAppointmentRecurringPattern().getAppointments().stream()
        .filter(
            appointmentInList ->
                (appointmentInList.getStartDateTime().after(startOfDay)
