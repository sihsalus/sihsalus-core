...
public class AllAppointmentRecurringPatternUpdateService {
  ...
  @Autowired RecurringAppointmentsService recurringAppointmentsService;

  public AppointmentRecurringPattern getUpdatedRecurringPattern(
  ...
  private AppointmentRecurringPattern getAppointmentRecurringPattern(
      RecurringAppointmentRequest recurringAppointmentRequest, boolean forConflicts) {
    ...
    appointmentMapper.mapAppointmentRequestToAppointment(
        recurringAppointmentRequest.getAppointmentRequest(), appointment);
    List<Appointment> newSetOfAppointments =
        recurringAppointmentsService.getUpdatedSetOfAppointments(
            appointment.getAppointmentRecurringPattern(), recurringAppointmentRequest);
    AppointmentRecurringPattern updatedAppointmentRecurringPattern =
        appointment.getAppointmentRecurringPattern();
    updatedAppointmentRecurringPattern.setEndDate(
        recurringAppointmentRequest.getRecurringPattern().getEndDate());
    updatedAppointmentRecurringPattern.setFrequency(
  ...
  public AppointmentRecurringPattern getUpdatedRecurringPatternForConflicts(
      RecurringAppointmentRequest recurringAppointmentRequest) {
  ...
  private Appointment getAppointment(RecurringAppointmentRequest recurringAppointmentRequest) {
    Appointment appointment =
        appointmentsService.getAppointmentByUuid(
            recurringAppointmentRequest.getAppointmentRequest().getUuid());
  ...
  private Appointment cloneAppointmentForConflicts(
      RecurringAppointmentRequest recurringAppointmentRequest) {
    Appointment appointment = getAppointment(recurringAppointmentRequest);
    AppointmentRecurringPattern recurringPatternClone =
        recurringPatternMapper.cloneAppointmentRecurringPattern(
            appointment.getAppointmentRecurringPattern());
  ...
  private void updateMetadataForAllAppointments(Appointment appointment, String clientTimeZone) {
    ...
    appointmentRecurringPattern.setAppointments(
        appointment.getAppointmentRecurringPattern().getAppointments().stream()
            .map(
                app -> {
  ...
  private boolean isRequestedOrScheduledAppointment(Appointment app) {
    return app.getStatus() == AppointmentStatus.Requested
        || app.getStatus() == AppointmentStatus.Scheduled;
  }
  ...
  private void updateMetadata(Appointment pendingAppointment, Appointment appointment) {
    ...
    List<AppointmentProvider> cloneOfProviders =
        getDeepCloneOfProviders(appointment.getProviders());
  ...
  private List<AppointmentProvider> getDeepCloneOfProviders(
      Set<AppointmentProvider> appointmentProviders) {
    if (CollectionUtils.isEmpty(appointmentProviders)) return new ArrayList<>();
    return appointmentProviders.stream()
  ...
  private Predicate<AppointmentProvider> isAcceptedOrAwaitingProvider() {
    return provider ->
        AppointmentProviderResponse.ACCEPTED.equals(provider.getResponse())
            || AppointmentProviderResponse.AWAITING.equals(provider.getResponse());
  }

  private void mapProvidersForAppointment(
      Appointment appointment, List<AppointmentProvider> newProviders) {
  ...
  private void setRemovedProvidersToCancel(
      List<AppointmentProvider> newProviders, Set<AppointmentProvider> existingProviders) {
    if (CollectionUtils.isNotEmpty(existingProviders)) {
      for (AppointmentProvider existingAppointmentProvider : existingProviders) {
        boolean appointmentProviderExists =
            newProviders.stream()
                .anyMatch(
                    newProvider ->
                        newProvider
        ...
        if (!appointmentProviderExists) {
          ...
          existingAppointmentProvider.setVoidReason(
              AppointmentProviderResponse.CANCELLED.toString());
        }
      }
    }
  }

  private void createNewAppointmentProviders(
      Appointment appointment, List<AppointmentProvider> newProviders) {
    if (!newProviders.isEmpty()) {
      ...
      for (AppointmentProvider newAppointmentProvider : newProviders) {
        List<AppointmentProvider> oldProviders =
