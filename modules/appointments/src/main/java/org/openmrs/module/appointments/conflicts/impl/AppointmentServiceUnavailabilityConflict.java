...
public class AppointmentServiceUnavailabilityConflict implements AppointmentConflict {
  ...
  public List<Appointment> getConflicts(List<Appointment> appointments) {
    ...
    for (Appointment appointment : appointments) {
      ...
      if (isConflicting) conflictingAppointments.add(appointment);
    }
    return conflictingAppointments;
  }

  private boolean checkConflicts(
      Appointment appointment, AppointmentServiceDefinition appointmentServiceDefinition) {
    Set<ServiceWeeklyAvailability> weeklyAvailableDays =
    ...
    if (isObjectPresent(weeklyAvailableDays)) {
      ...
      List<ServiceWeeklyAvailability> dayAvailabilities =
          weeklyAvailableDays.stream()
              .filter(day -> day.isSameDay(appointmentDay))
            ...
            .allMatch(
                availableDay ->
                    checkTimeAvailability(
                        appointment,
                        availableDay.getStartTime().getTime(),
                        availableDay.getEndTime().getTime()));
      return true;
    }
    ...
    long serviceStartMillis =
        serviceStartTime != null ? serviceStartTime.getTime() : DateUtil.getStartOfDay().getTime();
    long serviceEndMillis =
        serviceEndTime != null ? serviceEndTime.getTime() : DateUtil.getEndOfDay().getTime();
  ...
  private boolean checkTimeAvailability(
      Appointment appointment, long serviceStartTime, long serviceEndTime) {
    ...
    boolean isConflict =
        (appointmentStartTimeMilliSeconds >= appointmentEndTimeMilliSeconds)
