  ...
  public void execute() {
    ...
    boolean scheduleSMS =
        Boolean.parseBoolean(
            administrationService.getGlobalProperty(
                "sms.enableAppointmentReminderSMSAlert", "false"));

    if (!scheduleSMS) {
      return;
    }
    AppointmentsService appointmentsService = Context.getService(AppointmentsService.class);
    String schedulerReminderTime =
        administrationService
            .getGlobalPropertyObject("SchedulerReminderBeforeHours")
            .getPropertyValue();
    List<Appointment> appointments =
        appointmentsService.getAllAppointmentsReminder(schedulerReminderTime);
    for (Appointment appointment : appointments) {
      ...
      AppointmentArgumentsMapper appointmentArgumentsMapper =
          Context.getService(AppointmentArgumentsMapper.class);
      String message =
          smsBuilderService.getAppointmentReminderMessage(
