...
public class AppointmentsController extends BaseRestController {
  ...
  @Autowired private AppointmentSearchValidator appointmentSearchValidator;

  private Log log = LogFactory.getLog(this.getClass());

  @RequestMapping(method = RequestMethod.GET, value = "/{uuid}")
  public ResponseEntity<AppointmentDefaultResponse> getAppointmentByUuid(
  ...
  public ResponseEntity<Object> saveAppointment(
      @Valid @RequestBody AppointmentRequest appointmentRequest) throws IOException {
    try {
  ...
  @ResponseBody
  public List<AppointmentDefaultResponse> search(
      @Valid @RequestBody AppointmentSearchRequest appointmentSearchRequest) {
    Errors appointmentSearchErrors =
        new BeanPropertyBindingResult(appointmentSearchRequest, "appointmentSearchRequest");
  ...
  public ResponseEntity<Object> transitionAppointment(
    ...
    if (appointment != null) {
      ...
      return new ResponseEntity<>(appointmentMapper.constructResponse(appointment), HttpStatus.OK);
    } else throw new RuntimeException("Appointment does not exist");
  }
  ...
  public ResponseEntity<Object> getConflicts(@RequestBody AppointmentRequest appointmentRequest) {
    try {
      ...
      return new ResponseEntity<>(
          appointmentMapper.constructConflictResponse(appointmentConflicts), HttpStatus.OK);
    } catch (APIAuthenticationException | ContextAuthenticationException e) {
      return new ResponseEntity<>(
          RestUtil.wrapErrorResponse(e, e.getMessage()), HttpStatus.FORBIDDEN);
    } catch (Exception e) {
  ...
  public ResponseEntity<Object> updateAppointmentStatus(
      @Valid @RequestBody AppointmentStatusChangeRequest request) throws ParseException {
    ...
    try {
      ...
      log.info(
          "Updating "
              + appointmentUuids.size()
              + " appointment(s) to status: "
              + request.getToStatus());

      List<Appointment> updatedAppointments =
          appointmentsService.changeStatusForAppointments(appointmentUuids, request.getToStatus());

      return new ResponseEntity<>(
          appointmentMapper.constructResponse(updatedAppointments), HttpStatus.OK);
    } catch (APIAuthenticationException | ContextAuthenticationException e) {
      return new ResponseEntity<>(
          RestUtil.wrapErrorResponse(e, e.getMessage()), HttpStatus.FORBIDDEN);
    } catch (RuntimeException e) {
