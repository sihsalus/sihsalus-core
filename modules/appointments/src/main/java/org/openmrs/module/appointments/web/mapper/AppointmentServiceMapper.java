...
public class AppointmentServiceMapper {
  ...
  @Autowired SpecialityService specialityService;

  @Autowired AppointmentServiceDefinitionService appointmentServiceDefinitionService;

  @Autowired AppointmentServiceAttributeTypeService appointmentServiceAttributeTypeService;

  public AppointmentServiceDefinition fromDescription(
    ...
    if (!StringUtils.isBlank(appointmentServiceDescription.getUuid())) {
      appointmentServiceDefinition =
          appointmentServiceDefinitionService.getAppointmentServiceByUuid(
              appointmentServiceDescription.getUuid());
    } else {
    ...
    appointmentServiceDefinition.setMaxAppointmentsLimit(
        appointmentServiceDescription.getMaxAppointmentsLimit());
    ...
    if (StringUtils.isNotBlank(initialAppointmentStatus)) {
      appointmentServiceDefinition.setInitialAppointmentStatus(
          AppointmentStatus.valueOf(initialAppointmentStatus));
    } else {
    ...
    List<ServiceWeeklyAvailabilityDescription> weeklyAvailabilities =
        appointmentServiceDescription.getWeeklyAvailability();

    if (weeklyAvailabilities != null) {
      Set<ServiceWeeklyAvailability> availabilityList =
          weeklyAvailabilities.stream()
              .map(avb -> constructServiceWeeklyAvailability(avb, appointmentServiceDefinition))
    ...
    if (appointmentServiceDescription.getServiceTypes() != null) {
      appointmentServiceDescription
          ...
          .forEach(
              serviceType ->
                  constructAppointmentServiceTypes(serviceType, appointmentServiceDefinition));
    }
    ...
    if (appointmentServiceDescription.getAttributes() != null) {
      appointmentServiceDescription
          ...
          .forEach(
              attribute ->
                  constructAppointmentServiceAttribute(attribute, appointmentServiceDefinition));
    }
  ...
  private void constructAppointmentServiceTypes(
      AppointmentServiceTypeDescription ast,
      AppointmentServiceDefinition appointmentServiceDefinition) {
    AppointmentServiceType serviceType;
    Set<AppointmentServiceType> existingServiceTypes =
        appointmentServiceDefinition.getServiceTypes(true);
    if (ast.getUuid() != null)
      serviceType = getServiceTypeByUuid(existingServiceTypes, ast.getUuid());
    else serviceType = new AppointmentServiceType();
  ...
  private void constructAppointmentServiceAttribute(
      AppointmentServiceAttributeDescription attrDesc,
      AppointmentServiceDefinition appointmentServiceDefinition) {
    AppointmentServiceAttribute attribute;
    Set<AppointmentServiceAttribute> existingAttributes =
        appointmentServiceDefinition.getAttributes(true);

    if (attrDesc.getUuid() != null)
      attribute = getAttributeByUuid(existingAttributes, attrDesc.getUuid());
    else attribute = new AppointmentServiceAttribute();

    AppointmentServiceAttributeType attributeType =
        appointmentServiceAttributeTypeService.getAttributeTypeByUuid(
            attrDesc.getAttributeTypeUuid());
  ...
  private void validateAttributeCardinality(
      AppointmentServiceDefinition appointmentServiceDefinition) {
    List<AppointmentServiceAttributeType> allAttributeTypes =
        appointmentServiceAttributeTypeService.getAllAttributeTypes(false);
  ...
  private AppointmentServiceAttribute getAttributeByUuid(
      Set<AppointmentServiceAttribute> attributes, String attributeUuid) {
    return attributes.stream().filter(attr -> attr.getUuid().equals(attributeUuid)).findAny().get();
  }

  private ServiceWeeklyAvailability constructServiceWeeklyAvailability(
      ServiceWeeklyAvailabilityDescription avb,
      AppointmentServiceDefinition appointmentServiceDefinition) {
    ServiceWeeklyAvailability availability;
    if (avb.getUuid() != null)
      availability =
          getAvailabilityByUuid(
              appointmentServiceDefinition.getWeeklyAvailability(), avb.getUuid());
    else availability = new ServiceWeeklyAvailability();
  ...
  private ServiceWeeklyAvailability getAvailabilityByUuid(
      Set<ServiceWeeklyAvailability> weeklyAvailabilities, String availabilityUuid) {
    return weeklyAvailabilities.stream()
        .filter(avb -> avb.getUuid().equals(availabilityUuid))
        .findAny()
        .get();
  }

  private AppointmentServiceType getServiceTypeByUuid(
      Set<AppointmentServiceType> serviceTypes, String serviceTypeUuid) {
  ...
  public List<AppointmentServiceDefaultResponse> constructDefaultResponseForServiceList(
      List<AppointmentServiceDefinition> appointmentServiceDefinitions) {
    return appointmentServiceDefinitions.stream()
        .map(as -> this.mapToDefaultResponse(as, new AppointmentServiceDefaultResponse()))
  ...
  public List<AppointmentServiceFullResponse> constructFullResponseForServiceList(
      List<AppointmentServiceDefinition> appointmentServiceDefinitions) {
    return appointmentServiceDefinitions.stream()
        .map(as -> this.constructResponse(as))
        .collect(Collectors.toList());
  }
  ...
  public AppointmentServiceFullResponse constructResponse(AppointmentServiceDefinition service) {
    ...
    if (serviceWeeklyAvailability != null) {
      response.setWeeklyAvailability(
          serviceWeeklyAvailability.stream()
              .map(availability -> this.constructAvailabilityResponse(availability))
              .collect(Collectors.toList()));
    }
    ...
    if (serviceTypes != null) {
      response.setServiceTypes(
          serviceTypes.stream()
              .map(serviceType -> this.constructServiceTypeResponse(serviceType))
              .collect(Collectors.toList()));
    }
    return response;
  }

  public AppointmentServiceDefaultResponse constructDefaultResponse(
      AppointmentServiceDefinition appointmentServiceDefinition) {
    return mapToDefaultResponse(
        appointmentServiceDefinition, new AppointmentServiceDefaultResponse());
  }

  private AppointmentServiceDefaultResponse mapToDefaultResponse(
      AppointmentServiceDefinition as, AppointmentServiceDefaultResponse asResponse) {
    ...
    if (attributes != null && !attributes.isEmpty()) {
      List<AppointmentServiceAttributeResponse> attributeResponses =
          attributes.stream()
              .filter(attr -> !attr.getVoided())
  ...
  private AppointmentServiceAttributeResponse constructAttributeResponse(
      AppointmentServiceAttribute attribute) {
    ...
    try {
      Object value =
          attribute.getAttributeType().getDatatypeClassname() != null
              ? attribute.getValue()
  ...
  public AppointmentServiceAttributeTypeResponse constructAttributeTypeResponse(
      AppointmentServiceAttributeType attributeType) {
    AppointmentServiceAttributeTypeResponse response =
        new AppointmentServiceAttributeTypeResponse();
  ...
  public List<AppointmentServiceAttributeTypeResponse> constructAttributeTypeListResponse(
