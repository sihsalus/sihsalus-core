...
public class CohortMemberServiceImplTest {
  ...
  @Mock private GenericDao<CohortMemberAttribute> cohortMemberAttributeDao;

  @Mock private GenericDao<CohortMemberAttributeType> cohortMemberAttributeTypeDao;

  @Mock private GenericDao<CohortMember> cohortMemberDao;

  @Mock private SearchQueryHandler searchHandler;
  ...
  public void getCohortMemberAttributeType_shouldReturnMatchingCohortMemberAttributeType() {
    ...
    when(cohortMemberAttributeTypeDao.get(COHORT_MEMBER_ATTRIBUTE_TYPE_UUID))
        .thenReturn(cohortMemberAttributeType);
    when(cohortMemberAttributeType.getUuid()).thenReturn(COHORT_MEMBER_ATTRIBUTE_TYPE_UUID);

    CohortMemberAttributeType result =
        cohortMemberService.getCohortMemberAttributeTypeByUuid(COHORT_MEMBER_ATTRIBUTE_TYPE_UUID);
  ...
  public void saveCohortMemberAttributeType() {
    CohortMemberAttributeType cohortMemberAttributeType = mock(CohortMemberAttributeType.class);
    when(cohortMemberAttributeTypeDao.createOrUpdate(cohortMemberAttributeType))
        .thenReturn(cohortMemberAttributeType);
    when(cohortMemberAttributeType.getId()).thenReturn(1);

    CohortMemberAttributeType result =
        cohortMemberService.saveCohortMemberAttributeType(cohortMemberAttributeType);
  ...
  public void getCohortMemberAttributeByUuid() {
    ...
    when(cohortMemberAttributeDao.get(COHORT_MEMBER_ATTRIBUTE_UUID))
        .thenReturn(cohortMemberAttribute);
    when(cohortMemberAttribute.getUuid()).thenReturn(COHORT_MEMBER_ATTRIBUTE_UUID);

    CohortMemberAttribute result =
        cohortMemberService.getCohortMemberAttributeByUuid(COHORT_MEMBER_ATTRIBUTE_UUID);
    assertThat(result, notNullValue());
  ...
  public void saveCohortMemberAttribute() {
    CohortMemberAttribute cohortMemberAttribute = mock(CohortMemberAttribute.class);
    when(cohortMemberAttributeDao.createOrUpdate(cohortMemberAttribute))
        .thenReturn(cohortMemberAttribute);
