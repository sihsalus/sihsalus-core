...
public abstract class AbstractGenericDao<W extends OpenmrsObject & Auditable>
    implements GenericDao<W> {
  ...
  public W get(String uuid, boolean includeVoided) {
    ...
    return getCurrentSession()
        .createQuery(hql.toString(), clazz)
  ...
  public Collection<W> findBy(PropValue propValue, boolean includeRetired) {
    ...
    return queryParts
        .applyParameters(getCurrentSession().createQuery(queryParts.hql(), clazz))
        .list();
  }
  ...
  private QueryParts queryPartsFor(PropValue propValue, boolean includeDeleted) {
    ...
    String path =
        propValue.getAssociationPath().map(association -> "e." + association + ".").orElse("e.")
            + propValue.getProperty();
    StringBuilder hql =
        new StringBuilder(entityAlias()).append(" where ").append(path).append(" = :value");
