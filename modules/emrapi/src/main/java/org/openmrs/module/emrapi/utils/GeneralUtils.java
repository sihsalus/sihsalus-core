...
public class GeneralUtils {
  ...
  /**
   * Given a user, returns the default locale (if any) for that user Returns null if no default
   * locate
   */
  public static Locale getDefaultLocale(User user) {

    if (user != null
        && user.getUserProperties() != null
   ...
   *
   * <p>This method checks if input locale string contains control characters and tries to clean up
  ...
  public static Locale normalizeLocale(String localeString) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (c <= 0x20 || c >= 0x7f || !Character.isLetter(c) && c != 0x5f) {
        if (c == 0x09) continue; // allow horizontal tabs
    ...
    if (LocaleUtility.isValid(locale)) return locale;
    else return null;
  }
  ...
  public static PatientIdentifierType getPatientIdentifierType(
      String id, PatientService patientService) {
   ...
   * this format (We also are being slightly lenient and accepting uuids that are 37 or 38
  ...
  /**
   * Gets the specified property (referenced by string) off of a person address Returns null if the
   * underlying property is null
   */
  public static String getPersonAddressProperty(PersonAddress address, String property) {
    try {
      ...
      getPersonAddressProperty =
          personAddressClass.getMethod(
              "get" + property.substring(0, 1).toUpperCase() + property.substring(1));
      return (String) getPersonAddressProperty.invoke(address);
    } catch (Exception e) {
      throw new APIException(
          "Invalid property name " + property + " passed to getPersonAddressProperty");
    }
  }

  /**
   * Utility method to fetch the patient identifier for a patient of a certain type at a certain
   * location
   *
   * <p>Returns null if no identifiers found for that patient of that type at that location If the
   * patient has multiple identifiers for that type/location, it returns the first preferred one If
   * no preferred identifiers, returns first non-preferred one
   *
   * @param patient
  ...
  public static PatientIdentifier getPatientIdentifier(
      Patient patient, PatientIdentifierType patientIdentifierType, Location location) {

    // TODO: add some sort of data quality flag if there are two or more identifiers of the same
    // type and location?

    List<PatientIdentifier> patientIdentifiers =
        patient.getPatientIdentifiers(patientIdentifierType);
  ...
  /**
   * Gets the value of the user property EmrApiConstants.USER_PROPERTY_NAME_LAST_VIEWED_PATIENT_IDS
   * for the user as a list of patients in reverse order impying the patient that was first added
   * comes last while the last added one comes first
   *
   * @param user
  ...
  public static List<Patient> getLastViewedPatients(User user) {
    ...
    if (user != null) {
      ...
      String lastViewedPatientIdsString =
          user.getUserProperty(EmrApiConstants.USER_PROPERTY_NAME_LAST_VIEWED_PATIENT_IDS);
          ...
          limit =
              Integer.parseInt(
                  Context.getAdministrationService()
                      .getGlobalProperty(EmrApiConstants.GP_LAST_VIEWED_PATIENT_SIZE_LIMIT));
        } catch (Exception e) {
          limit = EmrApiConstants.DEFAULT_LAST_VIEWED_PATIENT_SIZE_LIMIT;
          log.warn(
              "Global property "
                  + EmrApiConstants.GP_LAST_VIEWED_PATIENT_SIZE_LIMIT
  ...
  /**
   * Finds the medical record location associated with the given location (This searches up the
   * hierarchy and returns the first location tagged as aMedical Record Location)
   *
   * @param location
