...
public class TransferToSpecificLocationDispositionAction implements DispositionAction {
  ...
  public void action(
    ...
    Location currentInpatientLocation =
        visitDomainWrapper.getInpatientLocation(encounterDomainWrapper.getEncounterDatetime());

    if (transferLocation == null) {
      log.warn(
          "Unable to create transfer action, no transfer location specified in obsgroup "
              + dispositionObsGroupBeingCreated);
      return;
    }
    // transfer the patient if a) they are not admitted (and therefore have no inpatient location)
    // or b) the inpatient location is other than the transfer location
