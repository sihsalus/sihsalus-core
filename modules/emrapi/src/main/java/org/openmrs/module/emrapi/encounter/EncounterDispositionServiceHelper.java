...
public class EncounterDispositionServiceHelper {
  ...
  private Obs voidDisposition(
      Obs existingDispositionObsGroup, EncounterTransaction.Disposition disposition) {
  ...
  private Obs createObsGroupForDisposition(EncounterTransaction.Disposition disposition)
  ...
  private Obs editExistingDisposition(
      Obs existingDispositionObsGroup, EncounterTransaction.Disposition disposition)
      throws ParseException {
    Obs existingDisposition =
        getMatchingObservation(
            existingDispositionObsGroup.getGroupMembers(), dispositionConcept.getUuid());
    ...
    if (disposition.getAdditionalObs() != null) {
      for (EncounterTransaction.Observation additionalObs : disposition.getAdditionalObs()) {
        Obs matchingObs =
            getMatchingObservation(
                existingDispositionObsGroup.getGroupMembers(), additionalObs.getConceptUuid());
        if (matchingObs == null) {
  ...
  private Obs createObsFromETObservation(EncounterTransaction.Observation observation)
      throws ParseException {
    Obs obs = new Obs();
  ...
  private Obs updateObsFromObservation(EncounterTransaction.Observation observation, Obs obs)
      throws ParseException {
  ...
  private Obs mapDispositionProperties(Obs dispositionObs, String dispositionCode) {
    if (dispositionObs != null) {
      ...
      dispositionObs.setValueCoded(
          getMatchingAnswer(dispositionConcept.getAnswers(), dispositionCode));
    }
  ...
  private Concept getMatchingAnswer(Collection<ConceptAnswer> answers, String dispositionCode) {
    Concept answerConcept =
        conceptService.getConceptByMapping(
            dispositionCode, getConceptMappings().getConceptSourceName());
