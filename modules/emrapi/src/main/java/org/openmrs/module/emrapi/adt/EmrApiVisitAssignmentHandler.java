 ...
 *
 * <p>For now, we require that a compatible visit exist before you're allowed to create an
 * encounter. However if the
 * EmrApiConstants#GP_VISIT_ASSIGNMENT_HANDLER_ENCOUNTER_TYPE_TO_VISIT_TYPE_MAP property that
 * provides a mapping between encounter types and visit types is set then a new visit is created
 * using the visit type from the saved mapping
 */
public class EmrApiVisitAssignmentHandler extends BaseEncounterVisitHandler
  ...
  public EmrApiVisitAssignmentHandler() {
    try {
      ...
      encounterTypetoVisitTypeMapper =
          Context.getRegisteredComponents(EncounterTypetoVisitTypeMapper.class).get(0);
    } catch (Exception ex) {
  ...
  public void beforeCreateEncounter(Encounter encounter) {
    ...
    if ("true"
        .equalsIgnoreCase(
            administrationService.getGlobalProperty(
                EmrApiConstants.GP_DISABLE_VISIT_ASSIGMENT_HANDLER))) {
      return;
    }
    ...
    // Eventually we should explicitly allow some encounters to be visit-free, probably via a GP
    // defining a list of EncounterTypes.
    ...
    List<Visit> candidates =
        visitService.getVisits(
            ...
            null,
            null,
            new DateTime(when).withTime(23, 59, 59, 999).toDate(),
            null,
            null,
            null,
            true,
        ...
        if (emrApiProperties.getVisitAssignmentHandlerAdjustEncounterTimeOfDayIfNecessary()) {
          if (adtService.isSuitableVisitIgnoringTime(candidate, encounter.getLocation(), when)) {
            if (when.before(candidate.getStartDatetime())) {
              updateDateActivatedOfOrdersIfNecessary(
                  encounter.getOrders(), when, candidate.getStartDatetime());
              encounter.setEncounterDatetime(candidate.getStartDatetime());
            } else if (candidate.getStopDatetime() != null
                && when.after(candidate.getStopDatetime())) {
    ...
    // there is no suitable visit so create one if there is a mapping encounter type to the visit
    // type via the Global property
  ...
  /**
   * If an encounter contains orders, and those orders have `dateActivated` that equals the
   * encounter's datetime, then if the encounter's datetime is changed, the order dateActivated
   * values should be changed accordingly
   *
   ...
   */
  protected void updateDateActivatedOfOrdersIfNecessary(
      Set<Order> orders, Date existingDatetime, Date newDatetime) {
    if (orders != null) {
