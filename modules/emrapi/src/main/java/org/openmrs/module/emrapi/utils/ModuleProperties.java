...
/**
 * Helper class that lets modules centralize their configuration details. Deprecated, replaced with
 * {@link org.openmrs.module.metadatamapping.util.ModuleProperties which supports mappings}
 */
@Deprecated
...
public abstract class ModuleProperties {
  ...
  protected PatientIdentifierType getPatientIdentifierTypeByGlobalProperty(
      String globalPropertyName, boolean required) {
    String globalProperty = getGlobalProperty(globalPropertyName, required);
    PatientIdentifierType patientIdentifierType =
        GeneralUtils.getPatientIdentifierType(globalProperty, patientService);
  ...
  protected List<PatientIdentifierType> getPatientIdentifierTypesByGlobalProperty(
      String globalPropertyName, boolean required) {
    ...
    if (StringUtils.isNotEmpty(globalProperty)) {
      for (String type : globalProperty.split(",")) {
        PatientIdentifierType patientIdentifierType =
            patientService.getPatientIdentifierTypeByUuid(type);
        if (patientIdentifierType != null) {
          ...
          log.warn(
              "Global property "
                  + globalPropertyName
                  + " specifies an unknown patient identifier type: "
                  + type);
        }
      }
    }
