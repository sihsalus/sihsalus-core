...
public class ObsMapper {
  ...
  public Obs transformEtObs(
      Encounter encounter, Obs observation, EncounterTransaction.Observation observationData) {
    if (observation == null) {
  ...
  protected Obs newObservation(
      Encounter encounter, EncounterTransaction.Observation observationData) {
    ...
    if (concept == null) {
      throw new ConceptNotFoundException(
          "Observation concept does not exist" + observationData.getConceptUuid());
    }
    ...
    observation.setFormField(
        observationData.getFormNamespace(), observationData.getFormFieldPath());
  ...
  protected void mapObservationProperties(
      EncounterTransaction.Observation observationData, Obs observation) {
    if (setVoidedObs(observationData, observation)) return;
    ...
    if (observationData.getValue() != null) {
      if (observation.getConcept().getDatatype().isCoded()) {
        ...
        obsService.getHandler(((ConceptComplex) conceptComplex).getHandler()).saveObs(observation);
          ...
          .getDatatype()
          .getUuid()
          .equals(ConceptDatatype.N_A_UUID)) {
        try {
          observation.setValueAsString(observationData.getValue().toString());
        } catch (ParseException pe) {
          throw new IllegalArgumentException(
              "Obs value for the concept uuid ["
                  + observationData.getConceptUuid()
                  + "] cannot be parsed");
        }
      }
    }
  ...
  private void setInterpretationAndStatus(
      Obs obs, EncounterTransaction.Observation observationData) {
    String interpretation = observationData.getInterpretation();
    obs.setInterpretation(
        interpretation == null ? null : Obs.Interpretation.valueOf(interpretation));
