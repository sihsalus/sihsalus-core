...
/**
 * Reflects a possible patient disposition such as "admit" or "discharge" that is generally
 * collected on a visit note These dispositions are configurable via json; each disposition needs to
 * be configured with the underlying concept that represents that disposition. Dispositions can also
 * have additional observations that are associated with them (for instance, an "Admit" disposition
 * may be configured to have an "Admit Location" obs associated with it). Certain actions can also
 * be configured to happen automatically when a disposition is saved **but this currently only works
 * in conjunction with the PIH EMR module, and may be going away in the future**
 */
public class Disposition {
  ...
  @JsonProperty private String conceptCode;

  @JsonProperty private DispositionType type;

  @JsonProperty private List<CareSetting.CareSettingType> careSettingTypes;

  @JsonProperty private List<String> encounterTypes;

  @JsonProperty private List<String> excludedEncounterTypes;

  @JsonProperty
  private Boolean
      keepsVisitOpen; // if this is the most recent disposition in a visit, visit will not be

  // automatically closed (see closeInactiveVisits and shouldBeClosed methods in
  // AdtService)

  /** These should be the names of existing Spring beans */
  @JsonProperty private List<String> actions;

  @JsonProperty private List<DispositionObs> additionalObs;
  ...
  public Disposition(
      ...
      String conceptCode,
      List<String> actions,
      List<DispositionObs> additionalObs) {
    this.uuid = uuid;
    this.name = name;
  ...
  public boolean equals(Object o) {
    ...
    if (additionalObs != null
        ? !additionalObs.equals(that.additionalObs)
        : that.additionalObs != null) return false;
