...
public class DispositionServiceImpl extends BaseOpenmrsService implements DispositionService {
  ...
  public DispositionServiceImpl(
      ConceptService conceptService, EmrConceptService emrConceptService) {
  ...
  public DispositionDescriptor getDispositionDescriptor() {
    // TODO handle this better--this property is only used to allow use to inject a mock disposition
    // descriptor
  ...
  @Override
  public List<Disposition> getValidDispositions(
    ...
    if (encounterType != null) {
      String encounterTypeId =
          encounterType.getEncounterTypeId() == null
              ? null
      ...
      for (Iterator<Disposition> it = dispositions.iterator(); it.hasNext(); ) {
        ...
        if (encounterTypes != null
            && !((encounterTypeId != null && encounterTypes.contains(encounterTypeId))
          ...
          it.remove();
        } else if (excludedEncounterTypes != null
            && ((encounterTypeId != null && excludedEncounterTypes.contains(encounterTypeId))
                || (encounterTypeUuid != null && excludedEncounterTypes.contains(encounterTypeUuid))
                || (encounterTypeName != null
                    && excludedEncounterTypes.contains(encounterTypeName)))) {
          it.remove();
        }
      }
    }
    ...
    try {
      ...
      for (Resource dispositionDefinition : dispositionDefinitions) {
        return objectMapper.readValue(
            dispositionDefinition.getInputStream(), new TypeReference<List<Disposition>>() {});
      }
      return null;
    } catch (IOException e) {
