...
public class MigrateDiagnosis {
  ...
  /**
   * Creates new Diagnosis using the Diagnosis model from the openmrs-core and saves it using the
  ...
  public Boolean migrate(DiagnosisMetadata diagnosisMetadata) {
    ...
    org.openmrs.api.DiagnosisService newDiagnosisService =
        Context.getService(org.openmrs.api.DiagnosisService.class);
    ...
    for (int i = 0; i < patientsIds.size(); i += BATCH_SIZE) {
      ...
      for (int id : patientBatch) {
        ...
        List<org.openmrs.Diagnosis> diagnoses =
            convert(oldDiagnosisService.getDiagnoses(patient, null));
  ...
  private List<org.openmrs.Diagnosis> convert(List<Diagnosis> emrapiDiagnoses) {
    ...
    for (Diagnosis emrapiDiagnosis : emrapiDiagnoses) {
      ...
      coreDiagnosis.setDiagnosis(
          new CodedOrFreeText(
              emrapiDiagnosis.getDiagnosis().getCodedAnswer(),
      ...
      coreDiagnosis.setCertainty(
          emrapiDiagnosis.getCertainty() == Diagnosis.Certainty.CONFIRMED
      ...
      Context.getObsService()
          .saveObs(obs, "Voided this Obs due to its migration to new encounter_diagnosis table");
  ...
  public static ObsGroupDiagnosisService getDeprecatedDiagnosisService() {
    return Context.getRegisteredComponent(
        "obsGroupDiagnosisService", ObsGroupDiagnosisService.class);
  }
}
