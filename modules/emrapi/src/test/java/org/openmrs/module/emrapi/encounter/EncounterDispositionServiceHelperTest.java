...
public class EncounterDispositionServiceHelperTest {
  ...
  public void setUp() throws Exception {
    ...
    dispositionGroupConcept =
        setUpConceptByMapping(EmrApiConstants.CONCEPT_CODE_DISPOSITION_CONCEPT_SET);
    dispositionConcept =
        setUpConceptByMapping(EmrApiConstants.CONCEPT_CODE_DISPOSITION, dispositionAnswers());
  ...
  public void shouldSaveDispositionAsObsGroup() {
    ...
    EncounterTransaction.Observation additionalObs =
        new EncounterTransaction.Observation()
            .setConcept(getConcept(noteConceptUuid))
            .setValue(dispositionNoteValue);
  ...
  public void shouldVoidObsWhenDispositionIsVoided() {
    ...
    Obs updatedDispositionGroupObservation =
        getObsByUuid(dispositionGroupObservation.getUuid(), encounter.getAllObs(true));
  ...
  //                new
  // EncounterTransaction.Observation().setConceptUuid(TEXT_CONCEPT_UUID).setValue("text
  // value").setComment("overweight")
  //        );
  //
  //        EncounterTransaction.Disposition disposition = new
  // EncounterTransaction.Disposition().setCode(dipositionAction);
  ...
  //
  //        Date observationDateTime = new
  // SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").parse("2005-01-01T00:00:00.000+0000");
  //
  //
  ...
  private void assertDispositionValues(
      String noteConceptUuid, String dispositionNoteValue, String code, Encounter encounter) {
    ...
    for (Obs obsGroupMember : obsGroupMembers) {
      if (obsGroupMember
          .getConcept()
          .getUuid()
          .equals(EmrApiConstants.CONCEPT_CODE_DISPOSITION + UUID_SUFFIX)) {
        ...
        assertEquals(
            "Error in disposition note value", dispositionNoteValue, obsGroupMember.getValueText());
      }
      assertNotNull(obs.getObsDatetime());
  ...
  private Obs buildDispositionGroupObservation() {
    Obs dispositionGroupObservation =
        new ObsBuilder()
            .setConcept(dispositionGroupConcept)
            .setUuid(UUID.randomUUID().toString())
            .get();
    Obs dispositionObservation =
        new ObsBuilder().setConcept(dispositionConcept).setUuid(UUID.randomUUID().toString()).get();
  ...
  private Concept setUpConceptByMapping(String mapping, List<ConceptAnswer> answers) {
    ...
    when(conceptService.getConceptByMapping(mapping, EmrApiConstants.EMR_CONCEPT_SOURCE_NAME))
        .thenReturn(concept);
    if (answers != null) {
