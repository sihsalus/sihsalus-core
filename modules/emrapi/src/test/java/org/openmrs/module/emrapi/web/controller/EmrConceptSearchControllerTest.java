...
public class EmrConceptSearchControllerTest extends BaseModuleWebContextSensitiveTest {
  ...
  @Autowired EmrConceptSearchController conceptSearchController;
  ...
  public void shouldSearchByName() throws Exception {
    List<SimpleObject> response =
        (List<SimpleObject>) conceptSearchController.search("Diabetes", 100, null);
  ...
  public void shouldSearchByCodeExact() throws Exception {
    List<SimpleObject> response =
        (List<SimpleObject>) conceptSearchController.search("ABC123", 100, null);
  ...
  public void shouldNotDoLikeSearchByCode() throws Exception {
    List<SimpleObject> response =
        (List<SimpleObject>) conceptSearchController.search("ABC12", 100, null);
  ...
  public void shouldHandleEmptyListOfDiagnosesConceptSource() throws Exception {
    ...
    source.setName(
        "foobar"); // so that "ICD-10-WHO" can't be found by name - produces empty list of
    // ConceptSources
    ...
    source.setName(
        "ICD-10-WHO"); // so that "ICD-10-WHO" conceptSource is not null - produces non-empty list
    // of ConceptSources
    cs.saveConceptSource(source);
