...
public class EmrOrderServiceImplTest {
  ...
  @Mock private OpenMRSDrugOrderMapper openMRSDrugOrderMapper;

  @Mock private OpenMRSOrderMapper openMRSOrderMapper;

  @Mock private OrderSetService orderSetService;

  @Mock private OpenMRSOrderGroupMapper openMRSOrderGroupMapper;
  ...
  public void shouldSaveNewDrugOrdersInTheSequenceOfOrdering() throws ParseException {
    EmrOrderServiceImpl emrOrderService =
        new EmrOrderServiceImpl(
            openMRSDrugOrderMapper,
            encounterService,
            openMRSOrderMapper,
            orderSetService,
            openMRSOrderGroupMapper);
    EncounterTransaction.DrugOrder drugOrder1 =
        new DrugOrderBuilder().withDrugUuid("drug-uuid1").build();
    EncounterTransaction.DrugOrder drugOrder2 =
        new DrugOrderBuilder().withDrugUuid("drug-uuid2").build();
  ...
  public void shouldSaveOrdersWithOrderGroups() throws ParseException {
    ...
    EncounterTransaction.Order order1 =
        new OrderBuilder()
            .withConceptUuid("concept-uuid1")
            .withComment("Comment")
            .withOrderGroup("orderSet-uuid1")
            .build();
    EncounterTransaction.Order order2 =
        new OrderBuilder()
            .withConceptUuid("concept-uuid2")
            .withComment("Comment")
            .withOrderGroup("orderSet-uuid1")
            .build();
    EncounterTransaction.Order order3 =
        new OrderBuilder()
            .withConceptUuid("concept-uuid3")
            .withComment("Comment")
            .withOrderGroup("orderSet-uuid2")
            .build();
    ...
    when(openMRSOrderGroupMapper.map(order1.getOrderGroup(), encounter))
        .thenReturn(mappedOrderGroup1);
    when(openMRSOrderGroupMapper.map(order2.getOrderGroup(), encounter))
        .thenReturn(mappedOrderGroup1);
    when(openMRSOrderGroupMapper.map(order3.getOrderGroup(), encounter))
        .thenReturn(mappedOrderGroup2);
    ...
    for (Order savedOrder : savedOrders) {
      if (savedOrder.getOrderGroup() != null) {
        orderGroups.put(
            savedOrder.getOrderGroup().getOrderSet().getUuid(), savedOrder.getOrderGroup());
      }
    }
  ...
  private boolean existsInOrdersList(Order order, ArrayList<Order> orderArrayList) {
    for (Order orderItem : orderArrayList) {
      if (orderItem.getConcept() != null
          && orderItem.getConcept().getUuid().equals(order.getConcept().getUuid())
          && orderItem.getCommentToFulfiller().equals(order.getCommentToFulfiller())) return true;
    }
    return false;
  }
}
