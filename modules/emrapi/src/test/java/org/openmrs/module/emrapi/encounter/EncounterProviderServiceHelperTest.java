...
public class EncounterProviderServiceHelperTest {
  ...
  public void setUp() {
    ...
    encounterProviderServiceHelper =
        new EncounterProviderServiceHelper(providerService, encounterService);
  }
  ...
  public void shouldSetEncounterRoleToUknownIfNotSpecified() {
    ...
    assertThat(encounterProvider.getProvider().getUuid(), is(equalTo("provider-uuid")));
    assertThat(
        encounterProvider.getEncounterRole().getUuid(),
        is(equalTo(EncounterRole.UNKNOWN_ENCOUNTER_ROLE_UUID)));
  }
  ...
  public void shoulAddSecondProvider() {
    ...
    EncounterTransaction.Provider encounterTransactionProvider =
        new EncounterTransaction.Provider();
    ...
    encounterProviderServiceHelper.update(
        encounter, Collections.singleton(encounterTransactionProvider));
    ...
    assertTrue(
        encounterProvider1.getProvider().getUuid().equals("provider-uuid")
  ...
  public void shouldAddProviderIfAlreadyAssociatedWithEncounterButWithDifferentEncounterRole() {
    ...
    assertTrue(
        role1.getUuid().equals("role-uuid") && role2.getUuid().equals("another-role-uuid")
