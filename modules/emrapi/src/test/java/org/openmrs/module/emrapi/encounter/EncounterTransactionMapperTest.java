...
public class EncounterTransactionMapperTest {
  ...
  @Mock private EncounterObservationsMapper encounterObservationsMapper;

  @Mock private EncounterProviderMapper encounterProviderMapper;

  @Mock private EmrOrderService emrOrderService;
  ...
  public void setUp() {
    ...
    encounterTransactionMapper =
        new EncounterTransactionMapper(
  ...
  public void shouldMap() throws Exception {
    ...
    Assert.assertEquals(
        encounter.getEncounterType().getUuid(), encounterTransaction.getEncounterTypeUuid());
    ...
    Assert.assertEquals(
        encounter.getVisit().getLocation().getUuid(), encounterTransaction.getVisitLocationUuid());
    Assert.assertEquals(
        encounter.getVisit().getVisitType().getUuid(), encounterTransaction.getVisitTypeUuid());
  }
  ...
  public void shouldMapEncounterTransactionsWithExtensions() {
    ...
    EncounterTransactionHandler encounterTransactionHandler =
        mock(EncounterTransactionHandler.class);
    mockedContext
        .when(() -> Context.getRegisteredComponents(EncounterTransactionHandler.class))
