...
public class EmrEncounterServiceTest {
	...
	public void setUp() throws Exception {
		...
		emrEncounterService = new EmrEncounterServiceImpl(patientService, visitService, encounterService, locationService,
		        providerService, administrationService, encounterObservationServiceHelper, encounterDispositionServiceHelper,
		        encounterTransactionMapper, encounterProviderServiceHelper, orderService);
	...
	private EncounterTransaction constructEncounterTransaction() {
		...
		List<EncounterTransaction.Observation> observations = asList(
		    new EncounterTransaction.Observation().setConcept(getConcept("radio-result-uuid")).setValue("text value")
		            .setComment("overweight").setOrderUuid("order-uuid"));
	...
	public void shouldDelegateDetailsFromEncounterSearchParametersToEncounterService() {
		...
		ArgumentCaptor<Collection<EncounterType>> encounterTypesArgument = (ArgumentCaptor) ArgumentCaptor
		        .forClass(Collection.class);
		ArgumentCaptor<Collection<Provider>> providersArgument = (ArgumentCaptor) ArgumentCaptor.forClass(Collection.class);
		ArgumentCaptor<Collection<VisitType>> visitTypesArgument = (ArgumentCaptor) ArgumentCaptor
		        .forClass(Collection.class);
		...
		verify(encounterService).getEncounters(patientArgument.capture(), locationArgument.capture(),
		    startDateArgument.capture(), endDateArgument.capture(), any(), encounterTypesArgument.capture(),
		    providersArgument.capture(), visitTypesArgument.capture(), visitsArgument.capture(),
	...
	public void shouldPassInEncounterParametersToEncounterMatcheronSave() {
		...
		mockedContext.when(() -> Context.getRegisteredComponents(BaseEncounterMatcher.class))
		...
		ArgumentCaptor<EncounterParameters> encounterParametersArguments = ArgumentCaptor
		        .forClass(EncounterParameters.class);
		verify(mockEncounterMatcher).findEncounter(any(Visit.class), encounterParametersArguments.capture());
		assertThat(encounterParametersArguments.getValue().getContext().get("patientProgramUuid"),
	...
	public void shouldCallEncounterTransactionHandlerOnRead() {
		...
		Encounter encounter = new EncounterBuilder().build();
		when(encounterService.getEncounters(any(Patient.class), any(Location.class), any(Date.class), any(Date.class), any(),
		    any(), any(), any(), any(), any(Boolean.class))).thenReturn(Arrays.asList(encounter));
	...
	@Test
	public void shouldUseActiveVisitWithoutLocationToSaveIfEncounterParametersIsNotPassedWithVisitLocation()
