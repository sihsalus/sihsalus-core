...
public class VisitDomainWrapperTest {
  ...
  public void shouldReturnCurrentLocationForAdmittedPatient() {
    ...
    assertThat(
        visitDomainWrapper.getInpatientLocation(DateUtils.addHours(new Date(), -2)), is(icu));
  ...
  @Test
  public void shouldNotHaveEncounterWithoutSubsequentEncounterIfNoRelevantEncounters()
      throws Exception {
    when(visit.getEncounters()).thenReturn(new LinkedHashSet<Encounter>());
    assertFalse(
        visitDomainWrapper.hasEncounterWithoutSubsequentEncounter(
            new EncounterType(), new EncounterType()));
  }

  @Test
  public void shouldHaveEncounterWithoutSubsequentEncounterIfOneEncounterOfCorrectType()
      throws Exception {
  ...
  @Test
  public void shouldNotHaveEncounterWithoutSubsequentEncounterIfOneEncounterOfWrongType()
      throws Exception {
    Encounter encounter = new Encounter();
    ...
    when(visit.getEncounters()).thenReturn(encounters);
    assertFalse(
        visitDomainWrapper.hasEncounterWithoutSubsequentEncounter(new EncounterType(), null));
  }

  @Test
  public void
      shouldHaveEncounterWithoutSubsequentEncounterIfMostRecentRelevantEncounterIsOfCorrectType()
          throws Exception {
  ...
  public void shouldReturnUniqueDiagnoses() {
    ...
    addDiagnosis(
        encounter,
        new CodedOrFreeTextAnswer(diagnosis1),
    ...
    addDiagnosis(
        encounter,
        new CodedOrFreeTextAnswer(diagnosis2),
        Diagnosis.Order.PRIMARY,
        Diagnosis.Certainty.CONFIRMED);
    ...
    addDiagnosis(
        encounter,
        new CodedOrFreeTextAnswer(diagnosis3),
        Diagnosis.Order.SECONDARY,
        Diagnosis.Certainty.PRESUMED);
  ...
  public void shouldReturnAllDiagnosesFromMostRecentEncounterWithAdmitDisposition() {
    ...
    encounterWithTransferDisposition.setEncounterDatetime(
        new DateTime(2012, 12, 12, 12, 12).toDate());

    Encounter mostRecentEncounterWithAdmitDisposition = new Encounter();
    mostRecentEncounterWithAdmitDisposition.setEncounterDatetime(
        new DateTime(2012, 11, 11, 11, 11).toDate());

    Encounter olderEncounterWithAdmitDisposition = new Encounter();
    olderEncounterWithAdmitDisposition.setEncounterDatetime(
        new DateTime(2012, 10, 10, 10, 10).toDate());
    ...
    when(dispositionService.getDispositionFromObsGroup(transferDispositionObsGroup))
        .thenReturn(transfer);
    ...
    when(dispositionService.getDispositionFromObsGroup(mostRecentAdmitDispositionObsGroup))
        .thenReturn(admit);
    ...
    when(dispositionService.getDispositionFromObsGroup(olderAdmitDispositionObsGroup))
        .thenReturn(admit);
    ...
    encounterWithTransferDisposition.addObs(
        diagnosisMetadata.buildDiagnosisObsGroup(primaryDiagnosis1));

    mostRecentEncounterWithAdmitDisposition.addObs(mostRecentAdmitDispositionObsGroup);
    mostRecentEncounterWithAdmitDisposition.addObs(
        diagnosisMetadata.buildDiagnosisObsGroup(primaryDiagnosis2));
    mostRecentEncounterWithAdmitDisposition.addObs(
        diagnosisMetadata.buildDiagnosisObsGroup(secondaryDiagnosis));

    olderEncounterWithAdmitDisposition.addObs(olderAdmitDispositionObsGroup);
    olderEncounterWithAdmitDisposition.addObs(
        diagnosisMetadata.buildDiagnosisObsGroup(primaryDiagnosis2));
    ...
    List<Diagnosis> diagnoses =
        visitDomainWrapper.getDiagnosesFromMostRecentDispositionByType(DispositionType.ADMIT);
  ...
  public void getDiagnosesFromMostRecentDispositionShouldNotFailIfDispositionHasNoType() {
    ...
    when(dispositionService.getDispositionFromObsGroup(dispositionObsGroup))
        .thenReturn(dispositionWithoutType);
  ...
  @Test
  public void
      timeAtCurrentInpatientLocationInMinutes_shouldReturnTimeAtCurrentInpatientLocationInMinutesEvenWhenMostRecentAdtEncounterDoesNotResultInLocationChange() {
  ...
  private class ExpectedDiagnosis implements ArgumentMatcher<Diagnosis> {
    ...
    public boolean matches(Diagnosis o) {
      ...
      match =
          match
              && actualDiagnosis.getDiagnosis().getCodedAnswer()
                  == expectedDiagnosis.getDiagnosis().getCodedAnswer();
