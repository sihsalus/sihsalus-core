...
public class VisitDomainWrapperTest {
	...
	@Test
	public void shouldHaveEncounterWithoutSubsequentEncounterIfMostRecentRelevantEncounterIsOfCorrectType()
	        throws Exception {
	...
	public void shouldReturnUniqueDiagnoses() {
		...
		addDiagnosis(encounter, new CodedOrFreeTextAnswer(diagnosis1), Diagnosis.Order.PRIMARY,
		    Diagnosis.Certainty.CONFIRMED);
		addDiagnosis(encounter, new CodedOrFreeTextAnswer(diagnosis1), Diagnosis.Order.SECONDARY,
		...
		addDiagnosis(encounter, new CodedOrFreeTextAnswer(diagnosis2), Diagnosis.Order.PRIMARY,
		    Diagnosis.Certainty.CONFIRMED);
		assertThat(visitDomainWrapper.getUniqueDiagnosesLegacy(false, false).size(), is(2));

		// Only return secondary or presumed if asked
		addDiagnosis(encounter, new CodedOrFreeTextAnswer(diagnosis3), Diagnosis.Order.SECONDARY,
		    Diagnosis.Certainty.PRESUMED);
	...
	private class ExpectedDiagnosis implements ArgumentMatcher<Diagnosis> {
		...
		public boolean matches(Diagnosis o) {
			...
			match = match
			        && actualDiagnosis.getDiagnosis().getCodedAnswer() == expectedDiagnosis.getDiagnosis().getCodedAnswer();
	...
	private void addDiagnosis(Encounter encounter, CodedOrFreeTextAnswer diagnosis, Diagnosis.Order order,
