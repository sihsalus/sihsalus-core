...
public class VisitControllerTest extends BaseModuleWebContextSensitiveTest {
	...
	public void shouldGetVisitsWithNotesAndDiagnosesByPatient() throws Exception {
		...
		MvcResult response = mockMvc
		        .perform(get("/rest/v1/emrapi/patient/" + patientUuid + "/visit").contentType(MediaType.APPLICATION_JSON))
	...
	public void shouldGetCustomRepresentation() throws Exception {
		...
		MvcResult response = mockMvc.perform(
		    get("/rest/v1/emrapi/patient/" + patientUuid + "/visit").param("v", rep).contentType(MediaType.APPLICATION_JSON))
		        .andExpect(status().isOk()).andReturn();
	...
	public void shouldGetVisitsWithNotesAndDiagnosesByPatientWithPagination() throws Exception {
		...
		MvcResult response = mockMvc.perform(get("/rest/v1/emrapi/patient/" + patientUuid + "/visit")
		        .param("startIndex", "0").param("limit", "1").contentType(MediaType.APPLICATION_JSON))
		        .andExpect(status().isOk()).andReturn();
	...
	public void shouldThrowErrorWhenPatientNotFound() throws Exception {
		...
		mockMvc.perform(get("/rest/v1/emrapi/patient/" + patientUuid + "/visit").contentType(MediaType.APPLICATION_JSON))
	...
	public void shouldGetVisitsWithObsGroupDiagnosesByPatient() throws Exception {
		...
		encounter.addObs(new ObsBuilder().setPerson(encounter.getPatient()).setEncounter(encounter)
		        .setConcept(dmd.getDiagnosisSetConcept()).addMember(dmd.getNonCodedDiagnosisConcept(), "Headache")
		...
		encounter.addObs(new ObsBuilder().setPerson(encounter.getPatient()).setEncounter(encounter)
		        .setConcept(dmd.getDiagnosisSetConcept()).addMember(dmd.getCodedDiagnosisConcept(), codedDiagnosisConcept)
		...
		MvcResult response = mockMvc
		        .perform(get("/rest/v1/emrapi/patient/" + patientUuid + "/visit")
		                .param("v", "custom:(diagnoses:(diagnosis,certainty,rank))").contentType(MediaType.APPLICATION_JSON))
		        .andExpect(status().isOk()).andReturn();
