...
public class OpenMRSDrugOrderMapperTest {
  ...
  @Mock private DosingInstructionsMapper dosingInstructionsMapper;

  @Mock private OrderMetadataService orderMetadataService;
  ...
  public void setup() {
    ...
    when(orderMetadataService.getDurationUnitsConceptByName(DAY_DURATION_UNIT))
        .thenReturn(DAY_DURATION_CONCEPT);

    CareSetting outPatientCareSetting =
        new CareSetting(
            OUT_PATIENT_CARE_SETTING,
            OUT_PATIENT_CARE_SETTING,
            CareSetting.CareSettingType.OUTPATIENT);
    when(orderService.getCareSettingByName(OUT_PATIENT_CARE_SETTING))
        .thenReturn(outPatientCareSetting);

    when(dosingInstructionsMapper.map(
            any(EncounterTransaction.DosingInstructions.class), any(DrugOrder.class)))
  ...
  public void shouldMapNewDrugOrders() throws ParseException {
    ...
    verify(dosingInstructionsMapper)
        .map(any(EncounterTransaction.DosingInstructions.class), any(DrugOrder.class));
  }
  ...
  public void shouldMapNewNonCodedDrugOrders() throws ParseException {
    ...
    EncounterTransaction.DrugOrder nonCodedDrugOrder =
        new DrugOrderBuilder()
  ...
  public void shouldMapRevisedDrugOrders() throws ParseException {
    EncounterTransaction.DrugOrder drugOrder =
        new DrugOrderBuilder().withDrugUuid(DRUG_UUID).withDurationUnits(DAY_DURATION_UNIT).build();
    DrugOrder openMrsDrugOrder = openMRSDrugOrderMapper.map(drugOrder, encounter);
    ...
    assertThat(
        revisedOpenMrsDrugOrder.getPreviousOrder().getUuid(),
        is(equalTo(openMrsDrugOrder.getUuid())));
    assertThat(
        revisedOpenMrsDrugOrder.getCareSetting().getName(), is(equalTo(OUT_PATIENT_CARE_SETTING)));
    assertThat(revisedOpenMrsDrugOrder.getDrug().getUuid(), is(equalTo(DRUG_UUID)));
    assertTrue(
        revisedOpenMrsDrugOrder.getDosingType().isAssignableFrom(SimpleDosingInstructions.class));
    assertTrue(openMrsDrugOrder instanceof DrugOrder);
    ...
    verify(dosingInstructionsMapper, times(2))
        .map(any(EncounterTransaction.DosingInstructions.class), any(DrugOrder.class));
  }
  ...
  public void shouldClearScheduledDateAndUrgencyWhenScheduledOrderRevisedToStartFromToday() {
    ...
    when(orderService.getOrderByUuid(previousOpenMrsDrugOrder.getUuid()))
        .thenReturn(previousOpenMrsDrugOrder);
    EncounterTransaction.DrugOrder revisedDrugOrder =
        new DrugOrderBuilder()
            .withScheduledDate(null)
            .withAction(Order.Action.REVISE.name())
            .withPreviousOrderUuid(previousOpenMrsDrugOrder.getUuid())
            .withDrugUuid(DRUG_UUID)
            .withDurationUnits(DAY_DURATION_UNIT)
            .build();
    ...
    assertThat(
        revisedOpenMrsDrugOrder.getUrgency(), is(equalTo(OpenMRSDrugOrderMapper.DEFAULT_URGENCY)));
