...
public class EmrApiVisitAssignmentHandlerTest extends BaseModuleContextSensitiveTest {
  ...
  public void before() {
    ...
    when(emrApiProperties.getVisitAssignmentHandlerAdjustEncounterTimeOfDayIfNecessary())
        .thenReturn(true);
  ...
  public void testThrowsExceptionIfNoSuitableVisitExists() throws Exception {
    ...
    when(visitService.getVisits(
            ...
            anyBoolean(),
            anyBoolean()))
        .thenReturn(Collections.singletonList(notSuitable));
  ...
  public void testAssigningASuitableVisitWhenOneExists() throws Exception {
    ...
    when(visitService.getVisits(
            ...
            eq(true),
            eq(false)))
        .thenReturn(Arrays.asList(suitable, notSuitable));
  ...
  @Test
  public void
      testAssigningANewVisitWhenOneDoesNotExistWithSpecifiedGlobalPropertyForCurrentVisitStartingToday() {
    Patient patient = new Patient();
    Location location = new Location();
    Encounter encounter = new Encounter();
    encounter.setPatient(patient);
    encounter.setLocation(location);
    location.addTag(
        new LocationTag(EmrApiConstants.LOCATION_TAG_SUPPORTS_VISITS, "Tag that supports visits"));
    ...
    Assert.assertTrue(
        DateUtils.isSameDay(
            encounter.getVisit().getStartDatetime(),
            encounter.getEncounterDatetime())); // no check for end date for a visit that is started
    // today since it is still open
  }
  ...
  public void testAssigningANewVisitWhenOneDoesNotExistWithSpecifiedGlobalPropertyForPastVisit() {
    ...
    Assert.assertTrue(
        DateUtils.isSameDay(
            encounter.getVisit().getStartDatetime(), encounter.getEncounterDatetime()));
    Assert.assertTrue(
        DateUtils.isSameDay(
            encounter.getVisit().getStopDatetime(),
            encounter.getEncounterDatetime())); // has stop time since it is in the past
  }
  ...
  public void testAdjustingEncounterTimeToStartOfVisitWhenAssignedToVisit() throws Exception {
    ...
    Visit suitable = new Visit();
    suitable.setPatient(patient);
    suitable.setStartDatetime(
        new DateTime()
            .withTimeAtStartOfDay()
            .minusDays(1)
            .plusHours(12)
            .toDate()); // start time = 12:00 yesterday
    suitable.setStopDatetime(
        new DateTime()
            .withTimeAtStartOfDay()
            .minusDays(1)
            .plusHours(16)
            .toDate()); // stop time = 16:00 yesterday
    suitable.setLocation(location);
    ...
    when(visitService.getVisits(
            ...
            eq(true),
            eq(false)))
        .thenReturn(Collections.singletonList(suitable));
