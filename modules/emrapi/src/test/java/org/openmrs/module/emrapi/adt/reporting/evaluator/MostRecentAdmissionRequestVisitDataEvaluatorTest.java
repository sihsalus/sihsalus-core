...
public class MostRecentAdmissionRequestVisitDataEvaluatorTest extends BaseReportingTest {
	...
	public void shouldReturnMostRecentAdmissionRequestForVisit() throws Exception {
		...
		Encounter encounter1 = testDataManager.encounter().patient(patient).encounterDatetime(mostRecentDate)
		        .encounterType(emrApiProperties.getVisitNoteEncounterType()).provider(encounterRole, mostRecentProvider)
		        .location(mostRecentLocation).visit(visit)
		        .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisSetConcept())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getCodedDiagnosisConcept())
		                        .value(firstCodedDiagnosisConcept).get())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisOrderConcept())
		                        .value("Primary", "org.openmrs.module.emrapi").get())
		                .get())
		        .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisSetConcept())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getCodedDiagnosisConcept())
		                        .value(secondCodedDiagnosisConcept).get())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisOrderConcept())
		                        .value("Secondary", "org.openmrs.module.emrapi").get())
		                .get())
		        .obs(testDataManager.obs().concept(dispositionDescriptor.getDispositionSetConcept())
		                .obs(testDataManager.obs().concept(dispositionDescriptor.getDispositionConcept())
		                        .value("Admit to hospital", "org.openmrs.module.emrapi").get())
		                .obs(testDataManager.obs().concept(dispositionDescriptor.getAdmissionLocationConcept()).value("3")
		                        .get())
		                .get())
		        .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisSetConcept())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getNonCodedDiagnosisConcept())
		                        .value("some diagnosis").get())
		                .obs(testDataManager.obs().concept(diagnosisMetadata.getDiagnosisOrderConcept())
		                        .value("Primary", "org.openmrs.module.emrapi").get())
		                .get())
		        .save();
		...
		Encounter encounter2 = testDataManager.encounter().patient(patient).encounterDatetime(otherDate)
		        ...
		        .obs(testDataManager.obs().concept(dispositionDescriptor.getDispositionSetConcept())
		                .obs(testDataManager.obs().concept(dispositionDescriptor.getDispositionConcept())
		                        .value("Admit to hospital", "org.openmrs.module.emrapi").get())
		                .obs(testDataManager.obs().concept(dispositionDescriptor.getAdmissionLocationConcept()).value("1")
		                        .get())
		                .get())
		        .save();
		...
		assertThat((Provider) ((Map<String, Object>) data.getData().get(visit.getId())).get("provider"),
		    is(mostRecentProvider));
		assertThat((Location) ((Map<String, Object>) data.getData().get(visit.getId())).get("fromLocation"),
