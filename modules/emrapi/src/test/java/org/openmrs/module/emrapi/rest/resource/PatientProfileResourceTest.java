...
public class PatientProfileResourceTest extends BaseModuleWebContextSensitiveTest {
  ...
  public void beforeEachTests() throws Exception {
    resource =
        (PatientProfileResource)
            Context.getService(RestService.class).getResourceBySupportedClass(PatientProfile.class);
    File personImageDirectory =
        new File(OpenmrsUtil.getApplicationDataDirectory() + "/person_images");
  ...
  public void shouldCreatePatient() throws Exception {
    ...
    patientProfileCreateObject.putAll(
        new ObjectMapper()
            .readValue(
                getClass().getClassLoader().getResourceAsStream("create_patient_profile.json"),
                HashMap.class));
    SimpleObject created =
        (SimpleObject) resource.create(patientProfileCreateObject, new RequestContext());
    ...
    Assert.assertEquals(
        "ba1b19c2-3ed6-4f63-b8c0-f762dc8d7562",
        ((Map) (((Map) relationships.get(0)).get("personB"))).get("uuid"));
    Assert.assertEquals(
        "6d9002ea-a96b-4889-af78-82d48c57a110",
  ...
  public void shouldUpdatePatient() throws Exception {
    ...
    patientProfileUpdateObject.putAll(
        new ObjectMapper()
            .readValue(
                getClass().getClassLoader().getResourceAsStream("update_patient_profile.json"),
                HashMap.class));
    SimpleObject created =
        (SimpleObject)
            resource.update(
                "da7f524f-27ce-4bb2-86d6-6d1d05312bd5",
                patientProfileUpdateObject,
                new RequestContext());
