...
public class DefaultOrderMapperTest {
  ...
  private static final CareSetting.CareSettingType OUT_PATIENT_CARE_SETTING =
  ...
  @Test
  public void shouldMapNewOrder()
      throws ParseException, NoSuchFieldException, IllegalAccessException {

    Order openMrsOrder =
        order("boil in water", "comments", "ORD-99", "ORD-100", ORDER_TYPE, "STAT");
  ...
  public void shouldSetPreviousOrder() throws NoSuchFieldException, IllegalAccessException {
    Order openMrsOrder =
        order("boil in water", "comments", "Previous Order Uuid", "ORD-100", ORDER_TYPE, "ROUTINE");
    EncounterTransaction.Order order = orderMapper.mapOrder(openMrsOrder);
  ...
  @Test
  public void shouldMapNewDrugOrder()
      throws ParseException, NoSuchFieldException, IllegalAccessException {

    DrugOrder openMrsDrugOrder =
        drugOrder(
            CareSetting.CareSettingType.OUTPATIENT,
            3,
            "3-0-2",
    ...
    assertThat(
        drugOrder.getDosingInstructionType(),
        is(equalTo(SimpleDosingInstructions.class.getName())));
    ...
    assertThat(
        drugOrder.getEffectiveStartDate(), is(equalTo(new LocalDate().plusDays(3).toDate())));
    ...
    assertThat(
        drugOrder.getDosingInstructions().getFrequency(), is(equalTo(TWICE_A_DAY_FREQUENCY)));

    assertThat(drugOrder.getDosingInstructions().getQuantity(), is(equalTo(1.0)));
    assertThat(
        drugOrder.getDosingInstructions().getQuantityUnits(), is(equalTo(TABLET_QUANTITY_UNIT)));
    assertThat(
        drugOrder.getDosingInstructions().getAdministrationInstructions(), is(equalTo("3-0-2")));
  ...
  @Test
  public void shouldMapNewNonCodedDrugOrder()
      throws ParseException, NoSuchFieldException, IllegalAccessException {

    DrugOrder nonCodedDrugOrder =
        drugOrder(
            CareSetting.CareSettingType.OUTPATIENT,
            3,
            "3-0-2",
  ...
  @Test
  public void shouldReturnDrugOrdersSortedByOrderNumber()
      throws NoSuchFieldException, IllegalAccessException {
    DrugOrder drugOrder100 =
        drugOrder(
            CareSetting.CareSettingType.OUTPATIENT,
            3,
            "3-0-2",
    ...
    Encounter encounter = new Encounter();
    encounter.setOrders(
        new HashSet<Order>(Arrays.asList(drugOrder350, drugOrder100, drugOrder201)));
  ...
  private Order order(
    ...
    durationConcept.addConceptMapping(
        new ConceptMap(
            new ConceptReferenceTerm(durationConceptSource, "D", "Day"), new ConceptMapType()));
  ...
  private DrugOrder drugOrder(
    ...
    order.setPatient(new Patient());
    order.setCareSetting(
        new CareSetting(careSettingType.name(), null, CareSetting.CareSettingType.OUTPATIENT));
    order.setAction(Order.Action.NEW);
