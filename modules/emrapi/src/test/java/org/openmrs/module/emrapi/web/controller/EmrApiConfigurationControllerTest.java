...
public class EmrApiConfigurationControllerTest extends BaseModuleWebContextSensitiveTest {
  ...
  @Autowired EmrApiProperties emrApiProperties;

  @Autowired EmrApiConfigurationController emrApiConfigurationController;

  @Autowired MetadataMappingService metadataMappingService;
  ...
  public void shouldGetFullRepresentation() {
    ...
    SimpleObject config = emrApiConfigurationController.getEmrApiConfiguration(request, response);
    Map<String, Object> unknownLocation = mapNode(config, "unknownLocation");
    DelegatingResourceDescription drd =
        new LocationResource2_0().getRepresentationDescription(Representation.FULL);
  ...
  public void shouldGetCustomRepresentation() {
    ...
    assertEquals(
        "06087111-222-11e3-9c1a-0800200c9a66",
        mapNode(config, "admissionEncounterType").get("uuid"));
  ...
  public void shouldGetDispositions() {
    ...
    for (Map<String, Object> d : dispositions) {
      if (d.get("uuid").equals("d2d89630-b698-11e2-9e96-0800200c9a66")) {
        ...
        assertThat(
            listNode(d, "additionalObs").get(0).get("conceptCode"),
        ...
        assertThat(
            d.get("conceptCode"), equalTo("org.openmrs.module.emrapi:Transfer out of hospital"));
        assertThat(listNode(d, "additionalObs").size(), equalTo(0));
  ...
  public void shouldGetDispositionDescriptor() {
    ...
    assertThat(
        mapNode(descriptor, "dispositionSetConcept", "name").get("name"),
        equalTo("Disposition Construct"));
    assertThat(
        mapNode(descriptor, "dispositionConcept", "name").get("name"), equalTo("Disposition"));
    assertThat(
        mapNode(descriptor, "admissionLocationConcept", "name").get("name"),
        equalTo("Admission Location"));
    assertThat(
        mapNode(descriptor, "internalTransferLocationConcept", "name").get("name"),
        equalTo("Internal Transfer Location"));
    assertThat(
        mapNode(descriptor, "dateOfDeathConcept", "name").get("name"), equalTo("Date of Death"));
  }
  ...
  public void shouldGetDiagnosisMetadata() {
    ...
    assertThat(
        mapNode(dm, "codedDiagnosisConcept", "name").get("name"), equalTo("Coded diagnosis"));
    assertThat(
        mapNode(dm, "nonCodedDiagnosisConcept", "name").get("name"),
        equalTo("Non-coded diagnosis"));
    assertThat(
        mapNode(dm, "diagnosisOrderConcept", "name").get("name"), equalTo("Diagnosis order"));
    assertThat(
        mapNode(dm, "diagnosisCertaintyConcept", "name").get("name"),
        equalTo("Diagnosis certainty"));
  }
