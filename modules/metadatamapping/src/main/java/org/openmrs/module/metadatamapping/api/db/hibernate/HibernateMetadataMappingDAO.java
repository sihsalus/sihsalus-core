...
public class HibernateMetadataMappingDAO implements MetadataMappingDAO {
  ...
  public MetadataTermMapping getMetadataTermMapping(Integer metadataTermMappingId) {
    return (MetadataTermMapping)
        getCurrentSession().get(MetadataTermMapping.class, metadataTermMappingId);
  }
  ...
  public List<MetadataTermMapping> getMetadataTermMappings(
    ...
    if (searchCriteria.getReferredObject() != null) {
      ...
      query.setParameter(
          "referredClass", searchCriteria.getReferredObject().getClass().getCanonicalName());
    }
    if (searchCriteria.getMetadataUuid() != null) {
    ...
    if (metadataTermMapping != null) {
      if (!type.getCanonicalName().equals(metadataTermMapping.getMetadataClass())) {
        throw new InvalidMetadataTypeException(
            "requested type "
                + type
                + " of metadata term mapping "
                + metadataTermMapping.getUuid()
                + " refers to type "
                + metadataTermMapping.getMetadataClass());
      }
  ...
  public MetadataSetMember getMetadataSetMember(Integer metadataSetMemberId) {
    return (MetadataSetMember)
        sessionFactory.getCurrentSession().get(MetadataSetMember.class, metadataSetMemberId);
  }
  ...
  public List<MetadataSetMember> getMetadataSetMembers(
    ...
    StringBuilder hql =
        new StringBuilder("from MetadataSetMember member where member.metadataSet = :metadataSet");
  ...
  private MetadataTermMapping internalSaveMetadataTermMapping(
      MetadataTermMapping metadataTermMapping) {
  ...
  private MetadataTermMapping getSourceMetadataTerm(
    ...
    return uniqueResult(
        getSourceMetadataTermsQuery(
            metadataSourceName, metadataClass, metadataTermCode, null, null));
  }
