...
/**
 * Base class for non-context-sensitive Authentication tests Sets up loggers to enable testing of
 * logging events
 */
public abstract class BaseAuthenticationTest {
  ...
  protected void setRuntimeProperties(Properties p) {
    ...
    p.setProperty(
        OpenmrsConstants.APPLICATION_DATA_DIRECTORY_RUNTIME_PROPERTY, appDataDir.getAbsolutePath());
  ...
  /**
   * @param test if the last line logged by the AuthenticationLogger contains the given test, return
   *     true
   */
