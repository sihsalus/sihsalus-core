...
public class AuthenticationUserSessionListenerTest extends BaseAuthenticationTest {
  ...
  public void shouldLogAtLoggedInOrOut() throws Exception {
    ...
    try {
      ...
      listener.loggedInOrOut(
          user, UserSessionListener.Event.LOGIN, UserSessionListener.Status.SUCCESS);
      assertLoggedEvent(login, LOGIN_SUCCEEDED);
      listener.loggedInOrOut(
          user, UserSessionListener.Event.LOGIN, UserSessionListener.Status.FAIL);
      assertLoggedEvent(login, LOGIN_FAILED);
      listener.loggedInOrOut(
          user, UserSessionListener.Event.LOGOUT, UserSessionListener.Status.SUCCESS);
      assertLoggedEvent(login, LOGOUT_SUCCEEDED);
      listener.loggedInOrOut(
          user, UserSessionListener.Event.LOGOUT, UserSessionListener.Status.FAIL);
