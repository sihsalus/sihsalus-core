...
/**
 * This class provides access to all authentication configuration settings By default, this will
 * load settings from the OpenMRS runtime properties, but can also be set programmatically
 */
public class AuthenticationConfig implements Serializable {
  ...
  /**
   * All configured authentication schemes are identified by a unique {schemeId} in the
   * configuration
   */
  public static final String SCHEME_ID = "{schemeId}";

  /**
   * If set to true, this indicates that users who have been set to require a password change are
   * directed to the corresponding `authentication.passwordChangeUrl` page before they can proceed
   * into the system
   */
  public static final String SUPPORT_FORCED_PASSWORD_CHANGE =
      "authentication.supportForcedPasswordChange";

  /**
   * If `authentication.supportForcedPasswordChange` is to true, this indicates the url that users
   * should be redirected to if they are set to require their password changed at next login.
   */
  public static final String PASSWORD_CHANGE_URL = "authentication.passwordChangeUrl";

  /**
   * This property determines which authentication scheme is used This enables implementations to
   * configure the preferred authentication scheme at runtime If not specified, then the module will
   * default to the core UsernamePasswordAuthenticationScheme This property should reference one of
   * the {schemeId} values that identify a particular configured scheme See SCHEME_TYPE and
   * SCHEME_CONFIG below
   */
  public static final String SCHEME = "authentication.scheme";

  /**
   * By default, all configuration settings are loaded at startup and cached. Subsequent changes to
   * configuration in runtime properties will not take effect until an application restart This
   * property enables changing this behavior. By setting this to `false`, settings will be reloaded
   * from the OpenMRS runtime properties file for every new HTTP request. This is expected to be
   * used only in development or testing, not in production, due to performance impacts.
   */
  public static final String SETTINGS_CACHED = "authentication.settings.cached";

  /**
   * If the configured `authentication.scheme` is a `WebAuthenticationScheme`, then by default all
   * HTTP requests will be blocked and redirected to the appropriate `challengeUrl` specified by the
   * `WebAuthenticationScheme`. In order to enable these `challengeUrl` pages to render without
   * themselves redirecting for authentication in an endless loop, this configuration setting allows
   * configuration of a comma-delimited list of url patterns that should not result in redirect for
   * authentication. This property can also be used for any additional URLs that might need to be
   * made accessible without authentication.
   */
  public static final String WHITE_LIST = "authentication.whiteList";

  /**
   * Some URL patterns should not result in a redirect to a challengeUrl, but rather should result
   * in a 401 unauthorized response with the challengeUrl set as the Location header. This is
   * generally true for all requests of RESTful endpoints that are interacting with the API in a
   * single-page app rather than redirecting to new urls
   */
  public static final String NON_REDIRECT_URLS = "authentication.nonRedirectUrls";

  /**
   * URLs that might need to be made accessible without go through the password change
   * authentication.
   */
  public static final String PASSWORD_CHANGE_WHITE_LIST = "authentication.passwordChangeWhiteList";

  /**
   * All AuthenticationScheme instances must be configured with, at minimum, a property that maps a
   * particular {schemeId} to a particular AuthenticationScheme class fully-specified name Eg
   * `authentication.scheme.basic.type =
   * org.openmrs.module.authentication.web.BasicWebAuthenticationScheme`
   */
  public static final String SCHEME_TYPE_TEMPLATE = "authentication.scheme.{schemeId}.type";

  /**
   * If a particular AuthenticationScheme instance requires configuration, each configuration
   * property can be specified using a property with this prefix, followed by the name of the
   * configuration property Eg `authentication.scheme.basic.config.loginPage =
   * /module/authentication/basic.htm`
   */
  public static final String SCHEME_CONFIG_PREFIX_TEMPLATE =
      "authentication.scheme.{schemeId}.config.";

  private static Properties config;

  private static final List<ClassLoader> classLoaders = new ArrayList<>();

  private static final Map<String, AuthenticationScheme> authenticationSchemes =
  ...
  public static Properties getConfig() {
    if (config == null) {
      config =
          AuthenticationUtil.getPropertiesWithPrefix(Context.getRuntimeProperties(), PREFIX, false);
    }
    return config;
  }

  /**
   * @param classLoader a classLoader to add to the list of classLoaders that can resolve classes
   *     for instantiation
   */
  ...
  /**
   * Registers a statically-wired authentication scheme that can be selected by {@link #SCHEME}
   * without relying on module classloading or dynamic OMOD lifecycle.
   */
  public static void registerAuthenticationScheme(
      String schemeId, AuthenticationScheme authenticationScheme) {
  ...
  /**
   * @param key the configuration property to retrieve
   * @param defaultValue the value to return if the value for the given configuration property is
   *     null
  ...
  /**
   * @param key the configuration property to retrieve
   * @return the value of the property, parsed into a List, split by comma, or an empty list if not
   *     found
   */
   ...
   * @return a new instance of the given type of class, with a type identified by the value of the
   *     given property
   */
    ...
    if (clazz != null) {
      try {
        ...
        throw new IllegalStateException(
            "Unable to instantiate class configured by " + key + ": " + clazz.getName(), e);
      }
    }
    return null;
  }
      ...
      for (ClassLoader loader : loaders) {
        try {
          ...
          if (!type.isAssignableFrom(loadedClass)) {
            throw new IllegalStateException(
                "Configured class " + trimmedClassName + " is not assignable to " + type.getName());
          }
   ...
   * @return the configuration properties that start with the given prefix, without the prefix if
   *     stripPrefix is true
   */
  ...
  public static List<String> getWhiteList() {
    ...
    whitelist.add("/authenticationui/login/loginEmail.page");
    whitelist.add("/authenticationui/login/loginTotp.page");
    whitelist.add("/authenticationui/**/*.action");
    ...
    whitelist.add("/ws/fhir2/R4/Location");
    whitelist.add("/ws/fhir2/R4/metadata");
    whitelist.add("*.js");
    whitelist.add("*.css");
    whitelist.add("*.gif");
    whitelist.add("*.jpg");
  ...
  /**
   * @return the List of url patterns that should not result in a redirect upon authentication
   *     failure
   */
  ...
  /**
   * @return the configured authentication scheme, defaulting to a
   *     UsernamePasswordAuthenticationScheme if not found
   */
  ...
  /**
   * @param schemeId the {schemeId} that identifies the authentication scheme configuration in the
   *     properties
  ...
  private static void configureAuthenticationScheme(String schemeId, AuthenticationScheme scheme) {
    if (scheme instanceof ConfigurableAuthenticationScheme) {
      ...
      String schemeConfigPropertyPrefix =
          SCHEME_CONFIG_PREFIX_TEMPLATE.replace(SCHEME_ID, schemeId);
   ...
   *
   * @param applicationName the application name from OpenMRS that identifies the name of the
