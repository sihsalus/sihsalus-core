...
public class QueueResource extends DelegatingCrudResource<Queue> {
  ...
  public QueueResource(
      QueueServicesWrapper services, QueueSearchCriteriaParser searchCriteriaParser) {
  ...
  public NeedsPaging<Queue> doGetAll(RequestContext requestContext) throws ResponseException {
    return new NeedsPaging<>(
        new ArrayList<>(getServices().getQueueService().getAllQueues()), requestContext);
  }
  ...
  @Override
  protected void delete(Queue queue, String retireReason, RequestContext requestContext)
  ...
  public QueueSearchCriteriaParser getSearchCriteriaParser() {
    if (searchCriteriaParser == null) {
      searchCriteriaParser =
          Context.getRegisteredComponents(QueueSearchCriteriaParser.class).get(0);
    }
