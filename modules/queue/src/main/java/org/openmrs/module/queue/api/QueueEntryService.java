...
public interface QueueEntryService {
  ...
  /**
   * Fetches the set of queue entries for this patient and queue that overlap with the give
   * startedAt and endedAt dates
   *
   ...
   */
  @Authorized(PrivilegeConstants.GET_QUEUE_ENTRIES)
  List<QueueEntry> getOverlappingQueueEntries(
      Patient patient, Queue queue, Date startedAt, Date endedAt);

  /**
   * Given a specified queue entry Q, return its previous queue entry P, where P has same patient
   * and visit as Q, and P.endedAt time is same as Q.startedAt time, and P.queue is same as
   * Q.queueComingFrom
   *
   * @param queueEntry
  ...
  /**
   * Transitions a queue entry by ending one queue entry and creating a new queue entry that starts
   * at that time
   *
  ...
  /**
   * @return {@link Long} count of queue entries that match the given %{@link
  ...
  String generateVisitQueueNumber(
   ...
   * sortWeightGenerator via global property but this method exists to enable setting
