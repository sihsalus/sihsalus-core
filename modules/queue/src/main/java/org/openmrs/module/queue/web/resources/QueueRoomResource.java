...
@Resource(
    name = RestConstants.VERSION_1 + "/queue-room",
...
public class QueueRoomResource extends DelegatingCrudResource<QueueRoom> {
  ...
  public QueueRoomResource(
      QueueServicesWrapper services, QueueRoomSearchCriteriaParser searchCriteriaParser) {
  ...
  public NeedsPaging<QueueRoom> doGetAll(RequestContext requestContext) throws ResponseException {
    return new NeedsPaging<>(
        new ArrayList<>(getServices().getQueueRoomService().getAllQueueRooms()), requestContext);
  }

  @Override
  public QueueRoom getByUniqueId(String uuid) {
    Optional<QueueRoom> optionalQueueRoom =
        getServices().getQueueRoomService().getQueueRoomByUuid(uuid);
  ...
  @Override
  protected void delete(QueueRoom queueRoom, String retireReason, RequestContext requestContext)
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof FullRepresentation) {
      model
          .property("queue", new RefProperty("#/definitions/QueueGetFull"))
          .property("auditInfo", new StringProperty())
          .property("queue", new StringProperty());
    }
  ...
  public QueueRoomSearchCriteriaParser getSearchCriteriaParser() {
    if (searchCriteriaParser == null) {
      searchCriteriaParser =
          Context.getRegisteredComponents(QueueRoomSearchCriteriaParser.class).get(0);
    }
