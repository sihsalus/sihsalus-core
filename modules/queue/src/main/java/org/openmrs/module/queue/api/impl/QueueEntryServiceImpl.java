...
public class QueueEntryServiceImpl extends BaseOpenmrsService implements QueueEntryService {
  ...
  @Setter private QueueEntryDao dao;

  @Setter private VisitService visitService;

  @Setter private AdministrationService administrationService;

  @Setter private SortWeightGenerator sortWeightGenerator = null;
  ...
  @Override
  @Transactional(readOnly = true)
  public List<QueueEntry> getOverlappingQueueEntries(
      Patient patient, Queue queue, Date startedAt, Date endedAt) {
    QueueEntrySearchCriteria criteria =
        QueueEntrySearchCriteria.builder()
  ...
  public QueueEntry transitionQueueEntry(QueueEntryTransition queueEntryTransition) {
    ...
    QueueEntry currentState =
        dao.get(queueEntryToStop.getId())
  ...
  public QueueEntry undoTransition(@NotNull QueueEntry queueEntry) {
    ...
    if (queueEntry.getId() == null) {
      throw new IllegalArgumentException(
          "Cannot undo transition on a queue entry that has not been saved");
    }

    // Reload from database to check current state and guard against concurrent modifications
    QueueEntry currentState =
        dao.get(queueEntry.getId())
            .orElseThrow(() -> new IllegalArgumentException("Queue entry not found"));
    if (currentState.getVoided()) {
    ...
    if (prevQueueEntry == null) {
      throw new IllegalArgumentException(
          "Specified queue entry does not have a previous queue entry");
    }
  ...
  @Override
  public String generateVisitQueueNumber(
      Location location, Queue queue, Visit visit, VisitAttributeType visitAttributeType) {
    if (location == null || queue == null || visit == null || visitAttributeType == null) {
      throw new APIException(
          "Sufficient parameters not supplied for generation of VisitQueueNumber");
    }
    ...
    Date onOrAfter =
        Date.from(
            LocalDateTime.now().with(LocalTime.MIN).atZone(ZoneId.systemDefault()).toInstant());
    criteria.setStartedOnOrAfter(onOrAfter);
    Date onOrBefore =
        Date.from(
            LocalDateTime.now().with(LocalTime.MAX).atZone(ZoneId.systemDefault()).toInstant());
