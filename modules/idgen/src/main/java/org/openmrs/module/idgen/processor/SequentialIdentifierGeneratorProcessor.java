...
public class SequentialIdentifierGeneratorProcessor implements IdentifierSourceProcessor {
  ...
  @Autowired @Lazy private IdentifierSourceService identifierSourceService;
  ...
  /**
   * This sets the next sequence value to 1 or whatever number represents the first identifier base
   * configured and saves this to the database
   *
    ...
    if (seq.getFirstIdentifierBase() != null) {
      sequenceValue =
          IdgenUtil.convertFromBase(
              seq.getFirstIdentifierBase(), seq.getBaseCharacterSet().toCharArray());
    }
  ...
  protected List<String> generateBatch(
      SequentialIdentifierGenerator seq, Long sequenceValue, int batchSize) {
