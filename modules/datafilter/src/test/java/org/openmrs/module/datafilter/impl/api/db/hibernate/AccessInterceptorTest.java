...
public class AccessInterceptorTest {
  ...
  public void beforeEachMethod() {
    ...
    when(Context.getRegisteredComponents(eq(SessionFactory.class)))
        .thenReturn(Collections.singletonList(sf));
    when(Util.skipFilter(anyString())).thenCallRealMethod();
    when(Util.isFilterDisabled(anyString())).thenReturn(false);
    when(adminService.getGlobalProperty(eq(ImplConstants.GP_RUN_IN_STRICT_MODE)))
        .thenReturn("true");
  }
  ...
  @Test
  public void
      onLoad_shouldFailWithAnExceptionIfTheAuthenticatedUserIsNotAllowedToViewThePatientGettingLoaded() {
  ...
  public void onLoad_shouldPassIfTheAuthenticatedUserIsAllowedToViewThePatientGettingLoaded() {
    final Integer userId = 1;
    final Integer patientId = 101;
    Collection<String> accessiblePatientIds =
        Stream.of(patientId.toString()).collect(Collectors.toSet());
    when(Context.getAuthenticatedUser()).thenReturn(new User(userId));
    when(AccessUtil.getAccessiblePersonIds(eq(Location.class))).thenReturn(accessiblePatientIds);
    interceptor.onLoad(new Patient(), patientId, null, null, null);
  }
  ...
  @Test
  public void
      onLoad_shouldFailWithAnExceptionIfTheAuthenticatedUserIsNotAllowedToViewThePrivilegedEncounterGettingLoaded() {
    final Integer userId = 1;
    ...
    interceptor.onLoad(
        ...
        encounterId,
        new Object[] {encType},
        new String[] {"encounterType"},
        new Type[] {new ManyToOneType((TypeFactory.TypeScope) null, null)});
  }

  @Test
  public void onLoad_shouldPassIfAllEncounterTypeViewPrivilegeBasedFiltersAreDisabled()
      throws Exception {
    User user = mock(User.class);
    when(Util.isFilterDisabled(startsWith(LOCATION_BASED_FILTER_NAME_PREFIX))).thenReturn(true);
    when(Util.isFilterDisabled(startsWith(ENC_TYPE_PRIV_BASED_FILTER_NAME_PREFIX)))
        .thenReturn(true);
    when(Context.getAuthenticatedUser()).thenReturn(user);
  ...
  public void onLoad_shouldPassIfTheEncounterTypeForTheEncounterHasNoViewPrivilege() {
    ...
    interceptor.onLoad(
        new Encounter(),
        null,
        new Object[] {new EncounterType(encounterTypeId)},
        new String[] {"encounterType"},
        null);
  }
  ...
  @Test
  public void
      onLoad_shouldFailIfTheAuthUserIsNotAllowedToViewThePrivilegedObsGettingLoadedAndTheEncTypeIsNotYetLoaded() {
    final Integer userId = 1;
    ...
    interceptor.onLoad(
        new Obs(),
        obsId,
        new Object[] {new Encounter(encounterId)},
  ...
  @Test
  public void
      onLoad_shouldFailIfTheAuthUserIsNotAllowedToViewThePrivilegedObsGettingLoadedAndTheEncTypeIsLoaded() {
    final Integer userId = 1;
    final Integer obsId = 101;
    User user = mock(User.class);
    user.setId(userId);
  ...
  public void onLoad_shouldPassIfTheAuthenticatedUserIsAllowedToViewThePatientVisitGettingLoaded() {
    ...
    when(AccessUtil.getAccessiblePersonIds(eq(Location.class))).thenReturn(accessiblePatientIds);
    interceptor.onLoad(
        new Visit(), null, new Object[] {new Patient(patientId)}, new String[] {"patient"}, null);
  }

  @Test
  public void
      onLoad_shouldPassIfTheAuthenticatedUserIsAllowedToViewThePatientEncounterGettingLoaded() {
    final Integer patientId = 101;
    Collection<String> accessiblePatientIds =
        Stream.of(patientId.toString()).collect(Collectors.toSet());
    when(Util.isFilterDisabled(startsWith(ENC_TYPE_PRIV_BASED_FILTER_NAME_PREFIX)))
    ...
    when(AccessUtil.getAccessiblePersonIds(eq(Location.class))).thenReturn(accessiblePatientIds);
    interceptor.onLoad(
        new Encounter(), null, new Object[] {new Patient(101)}, new String[] {"patient"}, null);
  }
  ...
  public void onLoad_shouldPassIfTheAuthenticatedUserIsAllowedToViewTheObsGettingLoaded() {
    ...
    when(AccessUtil.getAccessiblePersonIds(eq(Location.class))).thenReturn(accessiblePatientIds);
    interceptor.onLoad(
        new Obs(), null, new Object[] {new Patient(patientId)}, new String[] {"person"}, null);
  }

  @Test
  public void onLoad_shouldPassForAnyUserWithTheIndividualFilterByPassPrivileges() {
    when(Context.isAuthenticated()).thenReturn(true);
    when(Context.hasPrivilege(LOCATION_BASED_FILTER_NAME_ENCOUNTER + BYPASS_PRIV_SUFFIX))
        .thenReturn(true);
    when(Context.hasPrivilege(ENC_TYPE_PRIV_BASED_FILTER_NAME_ENCOUNTER + BYPASS_PRIV_SUFFIX))
        .thenReturn(true);
