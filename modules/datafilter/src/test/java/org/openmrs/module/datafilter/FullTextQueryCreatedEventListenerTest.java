...
public class FullTextQueryCreatedEventListenerTest {
  ...
  @Mock private FullTextQuery fullTextQuery;
  ...
  @Test
  public void
      onApplicationEvent_shouldNotEnableFiltersForWhichTheAuthenticatedUserHasASpecificByPassPrivilege() {
    ...
    List<FullTextFilterRegistration> filters =
        Stream.of(filterReg1, filterReg2).collect(Collectors.toList());
    when(Util.getFullTextFilterRegistrations()).thenReturn(filters);
    when(Util.skipFilter(anyString())).thenCallRealMethod();
    when(Context.isAuthenticated()).thenReturn(true);
    when(Context.hasPrivilege(eq(filter1 + DataFilterConstants.BYPASS_PRIV_SUFFIX)))
        .thenReturn(true);

    new FullTextQueryCreatedEventListener()
        .onApplicationEvent(
            new FullTextQueryCreatedEvent(new FullTextQueryAndEntityClass(fullTextQuery, clazz)));
