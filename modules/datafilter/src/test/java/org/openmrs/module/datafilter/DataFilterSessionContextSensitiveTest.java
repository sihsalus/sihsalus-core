...
public class DataFilterSessionContextSensitiveTest extends BaseFilterTest {
  ...
  @Autowired private SessionFactoryImplementor sessionFactory;
  ...
  public void setup() {
    if (currentSessionContext == null) {
      currentSessionContext =
          Whitebox.getInternalState(sessionFactory, CurrentSessionContext.class);
    }
  }

  @Test
  public void
      currentSession_shouldNotEnableFiltersForWhichTheAuthenticatedUserHasASpecificByPassPrivilege() {
    reloginAs("dyorke", "test");
    List<String> disabledFilters =
        Stream.of("datafilter_locationBasedVisitFilter", "datafilter_locationBasedEncounterFilter")
            .collect(Collectors.toList());
    disabledFilters.forEach(
        filterName ->
            Context.addProxyPrivilege(filterName + DataFilterConstants.BYPASS_PRIV_SUFFIX));
    ...
    List<String> enabledFilterNames =
        filters.stream()
            .filter(f -> !disabledFilters.contains(f.getName()))
            .map(f -> f.getName())
            .collect(Collectors.toList());
