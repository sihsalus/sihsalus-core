...
public class SubmissionFilterTest {
  ...
  @Mock private UserFormSubmissionHandler handler;
  ...
  public void doFilterToThrowExceptionWhenErrorOnHandleRequest() throws Exception {
    when(Context.getRegisteredComponent(
            "userFormSubmissionHandler", UserFormSubmissionHandler.class))
        .thenReturn(handler);

    doThrow(new IOException())
        .when(handler)
        .handle(httpServletRequest, httpServletResponse, filterChain);
