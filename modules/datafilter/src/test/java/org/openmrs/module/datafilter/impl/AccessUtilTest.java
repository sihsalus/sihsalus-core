...
public class AccessUtilTest extends BaseFilterTest {
  ...
  @Autowired private DataFilterService service;
  ...
  public void getAssignedBasisIds_shouldReturnAListOfAccessibleBasisIdsForTheAuthenticatedUser() {
    ...
    final int englandLocationId = 4000;
    final int unknownLocationId = 1;
    Collection<String> basisIds = AccessUtil.getAssignedBasisIds(Location.class);
    assertEquals(2, basisIds.size());
    assertTrue(basisIds.contains(String.valueOf(englandLocationId)));
    assertTrue(basisIds.contains(String.valueOf(unknownLocationId)));

    // Should include child locations
    final int ugandaLocationId = 4001;
    ...
    assertTrue(basisIds.contains(String.valueOf(englandLocationId)));
    assertTrue(basisIds.contains(String.valueOf(unknownLocationId)));
    assertTrue(basisIds.contains(String.valueOf(ugandaLocationId)));
  ...
  @Test
  public void
      getAccessiblePersonIds_shouldReturnAListOfAccessiblePersonIdsForTheAuthenticatedUser() {
    reloginAs("dyorke", "test");
