...
public class DataFilterSessionContextTest {
  ...
  @Mock private SessionFactoryImplementor sfImpl;
  ...
  public void currentSession_shouldSkipIfTheMethodHasAlreadyBeenCalledOnTheCurrentThread() {
    ...
    DataFilterSessionContext sessionContext =
        new DataFilterSessionContext(sfImpl) {
    ...
    try (Session ignored = sessionContext.currentSession()) {
      verify(mockLogger, times(1))
          .trace(
              startsWith(
                  "Skipping filter logic because filters are already set on the current session"));
    }
  }
  ...
  public void currentSession_shouldNotSkipIfTheMethodHasNotYetBeenCalledOnTheCurrentThread() {
    ...
    try (Session ignored = sessionContext.currentSession()) {
      verify(mockSession, times(1))
          .setProperty(eq(DataFilterConstants.MODULE_ID + "filters.applied"), eq("true"));
    }
  }
  ...
  public void currentSession_shouldSkipIfTheMethodIsCalledOnADaemonThread() {
    ...
    try (Session ignored = sessionContext.currentSession()) {
      verify(mockLogger, times(1))
          .trace(startsWith("Skipping enabling of filters on daemon thread"));
    }
  }
}
