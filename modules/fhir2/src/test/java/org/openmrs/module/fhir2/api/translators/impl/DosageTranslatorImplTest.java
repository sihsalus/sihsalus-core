...
public class DosageTranslatorImplTest {
  ...
  public void setup() {
    ...
    timingTranslator.setTimingRepeatComponentTranslator(
        new MedicationRequestTimingRepeatComponentTranslatorImpl());
  ...
  public void toFhirResource_shouldTranslateDrugOrderDoseToDoseQuantity() {
    ...
    assertThat(result, notNullValue());
    assertThat(
        result.getDoseAndRate().get(0).getDoseQuantity().getValue().doubleValue(), equalTo(20.0));
  ...
  @Test
  public void toFhirResource_shouldReturnDosageWhenSubmittingDrugOrderWithDoseUnitButNoDose()
  ...
  public void toFhirResource_shouldTranslateDrugOrderDoseToDoseQuantityPreferringRxNormIfPresent() {
    ...
    when(conceptSourceService.getUrlForConceptSource(rxNorm))
        .thenReturn(FhirConstants.RX_NORM_SYSTEM_URI);

    Concept mg = new Concept();
    mg.addName(new ConceptName("mg", Locale.ENGLISH));
    mg.addConceptMapping(
        new ConceptMap(new ConceptReferenceTerm(snomed, "snomed-ct-mg-code", "snomed"), sameAs));
    mg.addConceptMapping(
        new ConceptMap(new ConceptReferenceTerm(rxNorm, "rx-norm-mg-code", "rxnorm"), sameAs));
    ...
    assertThat(result.getDoseAndRate().get(0).getDoseQuantity().getUnit(), is("mg"));
    assertThat(
        result.getDoseAndRate().get(0).getDoseQuantity().getSystem(),
        is(FhirConstants.RX_NORM_SYSTEM_URI));
    assertThat(result.getDoseAndRate().get(0).getDoseQuantity().getCode(), is("rx-norm-mg-code"));
  }

  @Test
  public void
      toFhirResource_shouldTranslateDrugOrderDoseToDoseQuantityPreferringSnomedCtIfPresent() {
    ConceptMapType sameAs = new ConceptMapType();
    sameAs.setUuid(ConceptMapType.SAME_AS_MAP_TYPE_UUID);

    ConceptSource snomed = new ConceptSource();
    snomed.setHl7Code(Duration.SNOMED_CT_CONCEPT_SOURCE_HL7_CODE);
    when(conceptSourceService.getUrlForConceptSource(snomed))
        .thenReturn(FhirConstants.SNOMED_SYSTEM_URI);
    ...
    assertThat(
        result.getDoseAndRate().get(0).getDoseQuantity().getSystem(),
        is(FhirConstants.SNOMED_SYSTEM_URI));
