...
public class FhirConditionServiceImplTest {
  ...
  @Mock private FhirConditionDao dao;

  @Mock private ConditionTranslator<Condition> conditionTranslator;

  @Mock private FhirGlobalPropertyService globalPropertyService;

  @Mock private SearchQueryInclude<org.hl7.fhir.r4.model.Condition> searchQueryInclude;
  ...
  private SearchQuery<
          ...
          FhirConditionDao,
          ConditionTranslator<Condition>,
          SearchQueryInclude<org.hl7.fhir.r4.model.Condition>>
      searchQuery;

  private FhirConditionServiceImpl conditionService;

  @Mock private FhirDiagnosisService diagnosisService;
  ...
  public void update_shouldUpdateExistingCondition() {
    ...
    when(conditionTranslator.toFhirResource(openmrsCondition)).thenReturn(condition);
    when(dao.createOrUpdate(openmrsCondition)).thenReturn(openmrsCondition);
    when(conditionTranslator.toOpenmrsType(
            any(Condition.class), any(org.hl7.fhir.r4.model.Condition.class)))
  ...
  public void update_shouldThrowExceptionWhenConditionIsNull() {
    assertThrows(
        InvalidRequestException.class, () -> conditionService.update(CONDITION_UUID, null));
  }
  ...
  public void update_shouldThrowExceptionWhenConditionIdIsNull() {
    ...
    assertThrows(
        InvalidRequestException.class, () -> conditionService.update(CONDITION_UUID, condition));
  }
  ...
  public void searchConditions_shouldReturnTranslatedConditionReturnedByDao() {
    ...
    patientReference.addValue(
        new ReferenceOrListParam().add(new ReferenceParam(Patient.SP_GIVEN, "patient name")));
        ...
        new TokenAndListParam()
            .addAnd(
                new TokenOrListParam()
                    .add(
                        FhirConstants.CONDITION_CATEGORY_SYSTEM_URI,
                        FhirConstants.CONDITION_CATEGORY_CODE_CONDITION));

    DateRangeParam lastUpdated =
        new DateRangeParam().setLowerBound(LAST_UPDATED_DATE).setUpperBound(LAST_UPDATED_DATE);
    ...
    when(searchQuery.getQueryResults(any(), any(), any(), any()))
        .thenReturn(
            new SearchQueryBundleProvider<>(
                theParams, dao, conditionTranslator, globalPropertyService, searchQueryInclude));
    when(searchQueryInclude.getIncludedResources(any(), any())).thenReturn(Collections.emptySet());
    when(conditionTranslator.toFhirResources(daoResults))
        .thenReturn(Collections.singletonList(fhirCondition));

    IBundleProvider result =
        conditionService.searchConditions(
            new ConditionSearchParams(
                patientReference,
                codeList,
                clinicalList,
                onsetDate,
  ...
  public void shouldSearchExplicitlyFor_shouldReturnTrueForNullParam() {
    boolean result =
        conditionService.shouldSearchExplicitlyFor(
            null, FhirConstants.CONDITION_CATEGORY_CODE_CONDITION);
  ...
  public void shouldSearchExplicitlyFor_shouldReturnTrueWhenTagMatches() {
    ...
    boolean result =
        conditionService.shouldSearchExplicitlyFor(
            category, FhirConstants.CONDITION_CATEGORY_CODE_CONDITION);
  ...
  public void searchConditions_shouldReturnDiagnosisBundleWhenOnlyDiagnosisMatches() {
    ...
    IBundleProvider result =
        conditionService.searchConditions(
            new ConditionSearchParams(
                null, null, null, null, null, null, category, null, null, null, new HashSet<>()));
