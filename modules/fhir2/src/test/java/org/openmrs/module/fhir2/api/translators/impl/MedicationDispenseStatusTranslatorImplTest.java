...
public class MedicationDispenseStatusTranslatorImplTest {
  ...
  @Mock FhirConceptSourceService conceptSourceService;

  @Mock FhirConceptService conceptService;

  private MedicationDispenseStatusTranslatorImpl dispenseStatusTranslator;

  @Mock ConceptSource dispenseStatusSource;
  ...
  public void setup() {
    ...
    when(conceptSourceService.getConceptSourceByUrl(CONCEPT_SOURCE_URI))
        .thenReturn(Optional.of(dispenseStatusSource));
  }

  @Test
  public void toFhirResource_shouldTranslateConceptToDispenseStatus() {
    for (MedicationDispense.MedicationDispenseStatus expected :
        MedicationDispense.MedicationDispenseStatus.values()) {
      if (expected != MedicationDispense.MedicationDispenseStatus.NULL) {
        ...
        MedicationDispense.MedicationDispenseStatus actual =
            dispenseStatusTranslator.toFhirResource(concept);
  ...
  public void toOpenmrsType_shouldNotTranslateToConceptWithCorrectMapping() {
    for (MedicationDispense.MedicationDispenseStatus status :
        MedicationDispense.MedicationDispenseStatus.values()) {
      if (status != MedicationDispense.MedicationDispenseStatus.NULL) {
        Concept expected = new Concept();
        when(conceptService.getConceptWithSameAsMappingInSource(
                dispenseStatusSource, status.toCode()))
  ...
  public void toOpenmrsType_shouldReturnNullIfNoConceptWithCorrectMappingIsFound() {
    Concept concept =
        dispenseStatusTranslator.toOpenmrsType(
