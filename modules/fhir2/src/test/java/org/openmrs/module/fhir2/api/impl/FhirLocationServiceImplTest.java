...
public class FhirLocationServiceImplTest {
  ...
  @Mock private FhirLocationDao locationDao;

  @Mock private LocationTranslator locationTranslator;

  @Mock private FhirGlobalPropertyService globalPropertyService;

  @Mock SearchQueryInclude<Location> searchQueryInclude;
  ...
  public void searchForLocations_shouldReturnLocationsByParameters() {
    ...
    when(locationTranslator.toFhirResources(singletonList(location)))
        .thenReturn(singletonList(fhirLocation));
    when(locationDao.getSearchResults(any())).thenReturn(locations);

    IBundleProvider results =
        fhirLocationService.searchForLocations(
  ...
  private List<Location> get(IBundleProvider results) {
    return results.getResources(START_INDEX, END_INDEX).stream()
