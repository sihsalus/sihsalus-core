...
public class EncounterTranslatorImplTest {
  ...
  private static final String PRACTITIONER_URI =
  ...
  @Mock private EncounterParticipantTranslator participantTranslator;

  @Mock private EncounterLocationTranslator encounterLocationTranslator;

  @Mock private PatientReferenceTranslator patientReferenceTranslator;

  @Mock private EncounterReferenceTranslator<Visit> visitReferenceTranslator;

  @Mock private EncounterTypeTranslator<EncounterType> encounterTypeTranslator;

  @Mock private EncounterPeriodTranslator<org.openmrs.Encounter> encounterPeriodTranslator;

  @Mock private FhirEncounterClassMapDaoImpl encounterClassMap;
  ...
  public void toFhirResource_shouldAlwaysToUnknownEncounterStatus() {
    assertThat(
        encounterTranslator.toFhirResource(omrsEncounter).getStatus(),
  ...
  public void toOpenMrsType_shouldTranslateParticipantToEncounterProvider() {
    ...
    Encounter.EncounterParticipantComponent participantComponent =
  ...
  public void toFhirResource_shouldTranslateToFhirParticipant() {
    ...
    assertThat(
        result.getParticipant().get(0).getIndividual().getReference(), equalTo(PRACTITIONER_URI));
  }
  ...
  public void toOpenMrsType_shouldTranslateTypeToEncounterType() {
    ...
    when(encounterTypeTranslator.toOpenmrsType(ArgumentMatchers.any()))
        .thenReturn(omrsEncounterType);
    when(patientReferenceTranslator.toOpenmrsType(patientRef)).thenReturn(patient);
  ...
  public void toOpenMrsType_shouldTranslatePeriodToEncounterDatetime() {
    ...
    when(encounterPeriodTranslator.toOpenmrsType(ArgumentMatchers.any(), ArgumentMatchers.any()))
        .then(
            invocation -> {
  ...
  public void toFhirResource_shouldHaveEncounterTag() {
    ...
    assertThat(
        result.getMeta().getTag().get(0).getSystem(),
        equalTo(FhirConstants.OPENMRS_FHIR_EXT_ENCOUNTER_TAG));
