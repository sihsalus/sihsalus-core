...
public class PersonTranslatorImplTest {
  ...
  @Mock private FhirPatientDao patientDao;
  ...
  public void shouldTranslateOpenmrsGenderToFhirGenderType() {
    ...
    when(genderTranslator.toFhirResource(argThat(equalTo("F"))))
  ...
  public void shouldTranslateOpenmrsPersonNameToFhirPersonName() {
    ...
    when(nameTranslator.toFhirResource(
            argThat(
                allOf(
                    hasProperty("givenName", equalTo(PERSON_GIVEN_NAME)),
                    hasProperty("familyName", equalTo(PERSON_FAMILY_NAME))))))
  ...
  public void shouldTranslateOpenmrsAddressToFhirAddress() {
    ...
    when(addressTranslator.toFhirResource(
            argThat(
                allOf(
                    hasProperty("uuid", equalTo(ADDRESS_UUID)),
                    hasProperty("cityVillage", equalTo(ADDRESS_CITY))))))
        .thenReturn(address);
  ...
  public void shouldAddPersonLinkIfIsAPatient() {
    ...
    assertThat(
        result.getLinkFirstRep().getTarget().getReference(),
        equalTo(patientReference.getReference()));
  }
