...
public class ObservationQuantityCodingTranslatorImplTest {
  ...
  public void setup() throws UcumException, IOException {
    ...
    try (InputStream baseUcum =
            UcumService.class.getClassLoader().getResourceAsStream(baseUcumServiceXml);
        InputStream fhirUcum =
            this.getClass().getClassLoader().getResourceAsStream(fhirUcumServiceXml)) {
  ...
  /**
   * Tests all common fhir ucum codes specified in
   * https://www.hl7.org/fhir/valueset-ucum-common.html
   */
  @Test
  ...
  public void toFhirResource_shouldTranslateConceptWithCommonUCUMUnitCodes(String code) {
    ...
    assertThat(
        String.format("%s should be a valid UCUM code", code),
        coding.getSystem(),
        is(UCUM_SYSTEM_URI));
  ...
  @Test
  public void
      toOpenmrsType_shouldBeUnableToTranslateUCUMSystemQuantityIfOpenMRSUcumSystemIsUndefined() {
  ...
  public void toOpenmrsType_shouldTranslateUCUMSystemQuantityIfOpenMRSUcumSystemDefined() {
    ...
    when(conceptSourceService.getConceptSourceByUrl(UCUM_SYSTEM_URI))
        .thenReturn(Optional.of(mgSource));
    when(conceptService.getConceptsWithAnyMappingInSource(mgSource, "mg"))
