...
public class FhirMedicationDispenseServiceImplTest {
  ...
  @Mock private FhirMedicationDispenseDao<MedicationDispense> dao;

  @Mock private MedicationDispenseTranslator<MedicationDispense> translator;

  @Mock private FhirGlobalPropertyService globalPropertyService;

  @Mock private SearchQueryInclude<org.hl7.fhir.r4.model.MedicationDispense> searchQueryInclude;

  @Mock
  private SearchQuery<
          MedicationDispense,
          org.hl7.fhir.r4.model.MedicationDispense,
          FhirMedicationDispenseDao<MedicationDispense>,
          MedicationDispenseTranslator<MedicationDispense>,
          SearchQueryInclude<org.hl7.fhir.r4.model.MedicationDispense>>
      searchQuery;
  ...
  public void setup() {
    dispenseService =
        new FhirMedicationDispenseServiceImpl() {
  ...
  public void shouldGetMedicationDispenseByUuid() {
    ...
    org.hl7.fhir.r4.model.MedicationDispense dispense =
        dispenseService.get(MEDICATION_DISPENSE_UUID);
  ...
  public void create_shouldCreateNewMedicationDispense() {
    ...
    org.hl7.fhir.r4.model.MedicationDispense fhirDispense =
  ...
  public void update_shouldUpdateExistingMedicationDispense() {
    ...
    when(translator.toFhirResource(openmrsDispense)).thenReturn(fhirDispense);
    when(dao.createOrUpdate(openmrsDispense)).thenReturn(openmrsDispense);
    when(translator.toOpenmrsType(
            any(MedicationDispense.class), any(org.hl7.fhir.r4.model.MedicationDispense.class)))
        .thenReturn(openmrsDispense);

    org.hl7.fhir.r4.model.MedicationDispense result =
        dispenseService.update(MEDICATION_DISPENSE_UUID, fhirDispense);
  ...
  public void update_shouldThrowExceptionWhenMedicationDispenseIsNull() {
    assertThrows(
        ...
        () -> dispenseService.update(MEDICATION_DISPENSE_UUID, null));
  }
  ...
  public void searchMedicationDispenses_shouldGetSearchResults() {
    ...
    encounterParam.addValue(
        new ReferenceOrListParam().addOr(new ReferenceParam(encounterReference)));

    ReferenceAndListParam medicationRequestParam = new ReferenceAndListParam();
    encounterParam.addValue(
        new ReferenceOrListParam().addOr(new ReferenceParam(medicationRequestRef)));

    TokenAndListParam idParam =
        new TokenAndListParam().addAnd(new TokenParam(MEDICATION_DISPENSE_UUID));

    DateRangeParam lastUpdatedParam =
        new DateRangeParam().setLowerBound(lastUpdatedDate).setUpperBound(lastUpdatedDate);
    ...
    SearchParameterMap theParams =
        new SearchParameterMap()
            ...
            .addParameter(
                FhirConstants.MEDICATION_REQUEST_REFERENCE_SEARCH_HANDLER, medicationRequestParam)
