...
public class LocationTranslatorImplTest {
  ...
  @Mock private LocationAddressTranslator locationAddressTranslator;

  private final LocationReferenceTranslator locationReferenceTranslator =
      new LocationReferenceTranslatorImpl();

  @Mock private LocationTagTranslator locationTagTranslator;

  @Mock private TelecomTranslator<BaseOpenmrsData> telecomTranslator;

  @Mock private LocationTypeTranslator locationTypeTranslator;

  @Mock private FhirLocationDao fhirLocationDao;

  @Mock private FhirGlobalPropertyService propertyService;
  ...
  public void shouldSetFhirLocationToActiveIfLocationIsNotRetired() {
    ...
    assertThat(fhirLocation, notNullValue());
    assertThat(
        fhirLocation.getStatus(), equalTo(org.hl7.fhir.r4.model.Location.LocationStatus.ACTIVE));
  }
  ...
  public void shouldSetFhirLocationToInActiveIfLocationIsRetired() {
    ...
    assertThat(fhirLocation, notNullValue());
    assertThat(
        fhirLocation.getStatus(), equalTo(org.hl7.fhir.r4.model.Location.LocationStatus.INACTIVE));
  }
  ...
  public void shouldTranslateLocationAttributeToFhirContactPoint() {
    ...
    when(fhirLocationDao.getActiveAttributesByLocationsAndAttributeTypeUuid(
            any(Collection.class), eq(LOCATION_ATTRIBUTE_TYPE_UUID)))
        .thenReturn(activeAttributeMap);
  ...
  public void shouldTranslateFhirContactPointToLocationAttribute() {
    ...
    when(telecomTranslator.toOpenmrsType(any(LocationAttribute.class), eq(contactPoint)))
        .thenReturn(locationAttribute);
  ...
  public void getOpenmrsParentLocation_shouldReturnCorrectParentLocation() {
    Reference locationReference =
        new Reference()
            .setReference(FhirConstants.LOCATION + "/" + PARENT_LOCATION_UUID)
            .setType(FhirConstants.LOCATION)
            .setIdentifier(new Identifier().setValue(PARENT_LOCATION_UUID));
  ...
  public void getOpenmrsParentLocation_shouldReturnNullIfLocationHasNoIdentifier() {
    Reference locationReference =
        new Reference().setReference(FhirConstants.LOCATION + "/").setType(FhirConstants.LOCATION);
  ...
  public void getOpenmrsParentLocation_shouldThrowExceptionIfReferenceIsntForLocation() {
    Reference reference =
        new Reference().setReference("Unknown" + "/" + PARENT_LOCATION_NAME).setType("Unknown");
  ...
  public void toOpenmrsType_shouldTranslateFhirPositionToLocationLatitude() {
    org.hl7.fhir.r4.model.Location.LocationPositionComponent position =
        new org.hl7.fhir.r4.model.Location.LocationPositionComponent();
