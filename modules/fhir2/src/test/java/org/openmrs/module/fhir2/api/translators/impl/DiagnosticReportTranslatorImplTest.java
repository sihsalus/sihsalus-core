...
public class DiagnosticReportTranslatorImplTest {
  ...
  @Mock OrderReferenceTranslator orderReferenceTranslator;
  ...
  public void setup() {
    ...
    Reference obsReference =
        new Reference().setType("Observation").setReference("Observation/" + CHILD_UUID);
  ...
  public void toFhirResource_shouldSetLabCategory() {
    ...
    assertThat(
        coding,
        contains(
            hasProperty("system", equalTo(FhirConstants.DIAGNOSTIC_REPORT_SERVICE_SYSTEM_URI))));
  ...
  public void toFhirResource_shouldConvertResult() {
    ...
    assertThat(
        result.getResult(),
        contains(hasProperty("reference", equalTo("Observation/" + CHILD_UUID))));
  }
  ...
  public void toFhirResource_shouldReturnOrderRef() {
    Order radiologyOrder = new Order();
    radiologyOrder.setUuid(
        RADIOLOGY_ORDER_REFERENCE.substring(RADIOLOGY_ORDER_REFERENCE.indexOf("/")));
    fhirDiagnosticReport.setOrders(Collections.singleton(radiologyOrder));
    when(orderReferenceTranslator.toFhirResource(radiologyOrder))
