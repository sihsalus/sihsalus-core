...
public class ConceptTranslatorImplTest {
  ...
  @Mock private FhirConceptService conceptService;

  @Mock private FhirConceptSourceService conceptSourceService;
  ...
  public void setup() {
    ...
    when(conceptSourceService.getConceptSourceByUrl(FhirTestConstants.LOINC_SYSTEM_URL))
        .thenReturn(Optional.of(loinc));

    fhirCiel = new FhirConceptSource();
    fhirCiel.setConceptSource(ciel);
    fhirCiel.setUrl(FhirTestConstants.CIEL_SYSTEM_URN);
    when(conceptSourceService.getConceptSourceByUrl(FhirTestConstants.CIEL_SYSTEM_URN))
        .thenReturn(Optional.of(ciel));
    when(conceptSourceService.getFhirConceptSources())
        .thenReturn(Arrays.asList(fhirLoinc, fhirCiel));
  }
    ...
    assertThat(
        ...
        hasItem(
            allOf(
                ...
                hasProperty("code", equalTo("1000-1")),
                hasProperty("display", nullValue()))));
  }
  ...
  public void shouldTranslateLOINCCodeableConceptToConceptWithSAMEASMapType() {
    ...
    when(conceptService.getConceptsWithAnyMappingInSource(loinc, "1000-1"))
        .thenReturn(matchingConcepts);
    ...
    assertThat(
        result.getConceptMappings(),
        hasItem(hasProperty("conceptReferenceTerm", hasProperty("code", equalTo("1000-1")))));
  }

  @Test
  public void
      shouldNotTranslateCodeableConceptToAnyConceptIfMultipleConceptsExistWithNoSAMEASMapType() {
  ...
  public void shouldTranslateCIELCodeableConceptToConcept() {
    ...
    List<Concept> matchingConcepts = new ArrayList<>();
    matchingConcepts.add(concept);
    when(conceptService.getConceptsWithAnyMappingInSource(ciel, "1650"))
        .thenReturn(matchingConcepts);
  ...
  public void shouldAddCodesThatAreMappedSAME_ASIfTheyExist() {
    ...
    assertThat(
        result.getCoding(),
        hasItem(hasProperty("system", equalTo(FhirTestConstants.CIEL_SYSTEM_URN))));
  ...
  public void shouldAddCodesThatAreNotMappedSAME_ASIfOneMappingPerSourceExistsForMultipleSources() {
    ...
    assertThat(
        result.getCoding(),
        hasItem(hasProperty("system", equalTo(FhirTestConstants.LOINC_SYSTEM_URL))));
  ...
  @Test
  public void
      shouldNotAddCodesThatAreNotMappedSAME_ASIfManyMappingExistsPerSourceForMultipleSources() {
    addMapping(narrowerThan, ciel, "1650");
  ...
  public void shouldReturnDifferentInstancesForSameConceptWhenCached() {
    // given
    conceptTranslator.setCacheManager(
        new ConcurrentMapCacheManager("fhir2ConceptToCodeableConcept"));
