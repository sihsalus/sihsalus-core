...
public class PatientTranslatorImplTest {
  ...
  @Mock private PatientIdentifierTranslator identifierTranslator;

  @Mock private PersonNameTranslator nameTranslator;

  @Mock private GenderTranslator genderTranslator;

  @Mock private PersonAddressTranslator addressTranslator;

  @Mock private FhirPersonDao fhirPersonDao;

  @Mock private TelecomTranslator<BaseOpenmrsData> telecomTranslator;

  @Mock private FhirGlobalPropertyService globalPropertyService;

  @Mock private PersonAttributeTranslator personAttributeTranslator;
  ...
  public void shouldTranslatePatientIdentifierToFhirIdentifier() {
    ...
    when(identifierTranslator.toFhirResource(
            argThat(hasProperty("uuid", equalTo(PATIENT_IDENTIFIER_UUID)))))
        .thenReturn(id);
    ...
    assertThat(
        result.getIdentifier(), hasItem(hasProperty("value", equalTo(PATIENT_IDENTIFIER_UUID))));
  }
  ...
  public void shouldTranslateOpenmrsPatientNameToFhirPatientName() {
    ...
    when(nameTranslator.toFhirResource(
            argThat(
                allOf(
                    hasProperty("givenName", equalTo(PATIENT_GIVEN_NAME)),
                    hasProperty("familyName", equalTo(PATIENT_FAMILY_NAME))))))
        .thenReturn(humanName);
  ...
  public void shouldTranslateOpenMRSPatientGenderToFhirGender() {
    when(genderTranslator.toFhirResource(argThat(equalTo("M"))))
  ...
  public void shouldTranslateOpenMRSAddressToFhirAddress() {
    ...
    when(addressTranslator.toFhirResource(argThat(hasProperty("uuid", equalTo(ADDRESS_UUID)))))
        .thenReturn(address);
  ...
  public void shouldTranslateFhirIdentifierToPatientIdentifier() {
    PatientIdentifier patientIdentifier = new PatientIdentifier();
    patientIdentifier.setUuid(PATIENT_IDENTIFIER_UUID);
    when(identifierTranslator.toOpenmrsType(
            argThat(hasProperty("value", equalTo(PATIENT_IDENTIFIER_UUID)))))
    ...
    assertThat(
        result.getPatientIdentifier(), hasProperty("uuid", equalTo(PATIENT_IDENTIFIER_UUID)));
  }
  ...
  public void shouldTranslateFhirAddressToOpenMRSAddress() {
    ...
    personAddress.setUuid(ADDRESS_UUID);
    personAddress.setCityVillage(ADDRESS_CITY);
    when(addressTranslator.toOpenmrsType(argThat(hasProperty("id", equalTo(ADDRESS_UUID)))))
        .thenReturn(personAddress);
