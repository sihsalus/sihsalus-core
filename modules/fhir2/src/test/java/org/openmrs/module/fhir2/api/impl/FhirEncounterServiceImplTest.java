...
public class FhirEncounterServiceImplTest {
  ...
  @Mock private FhirEncounterDao dao;

  @Mock private EncounterTranslator<Encounter> encounterTranslator;

  @Mock private FhirVisitService visitService;

  @Mock private FhirGlobalPropertyService globalPropertyService;

  @Mock private SearchQueryInclude<org.hl7.fhir.r4.model.Encounter> searchQueryInclude;
  ...
  private SearchQuery<
          ...
          SearchQueryInclude<org.hl7.fhir.r4.model.Encounter>>
      searchQuery;
  ...
  public void update_shouldUpdateEncounter() {
    ...
    when(encounterTranslator.toOpenmrsType(openMrsEncounter, fhirEncounter))
        .thenReturn(openMrsEncounter);
    when(dao.createOrUpdate(openMrsEncounter)).thenReturn(openMrsEncounter);
    when(dao.get(ENCOUNTER_UUID)).thenReturn(openMrsEncounter);
    when(encounterTranslator.toFhirResource(openMrsEncounter)).thenReturn(fhirEncounter);
  ...
  public void searchForEncounter_shouldReturnCollectionOfEncounterByDate() {
    ...
    when(searchQuery.getQueryResults(any(), any(), any(), any()))
        .thenReturn(
            new SearchQueryBundleProvider<>(
                theParams, dao, encounterTranslator, globalPropertyService, searchQueryInclude));
    ...
    assertThat(
        ((org.hl7.fhir.r4.model.Encounter) resultList.iterator().next()).getId(),
  ...
  public void searchForEncounter_shouldReturnCollectionOfEncounterByLocation() {
    ...
    location.addValue(
        new ReferenceOrListParam()
            .add(
                new ReferenceParam()
                    .setValue(ENCOUNTER_ADDRESS_STATE)
                    .setChain(Location.SP_ADDRESS_STATE)));
  ...
  public void searchForEncounter_shouldReturnCollectionOfEncounterByParticipant() {
    ...
    participant.addValue(
        new ReferenceOrListParam()
            .add(
                new ReferenceParam()
                    .setValue(PARTICIPANT_IDENTIFIER)
                    .setChain(Practitioner.SP_IDENTIFIER)));
  ...
  public void searchForEncounter_shouldReturnCollectionOfEncounterByTypeUUID() {
    TokenAndListParam typeUuid =
        new TokenAndListParam().addAnd(new TokenParam(ENCOUNTER_TYPE_UUID));
  ...
  public void searchForEncounter_shouldOnlyReturnEncountersThatHaveAssociatedMedicationRequests() {
    ...
    SearchParameterMap theParams =
        new SearchParameterMap().addParameter(FhirConstants.HAS_SEARCH_HANDLER, hasAndListParam);
  ...
  public void searchEncounters_shouldIncludeAddResourcesThatMatchTagParam() {
    ...
    when(visitService.searchForVisits(any()))
        .thenReturn(new MockIBundleProvider<>(fhirVisits, 10, 1));
...

    {
      ...
      tokenAndListParam.addAnd(
          new TokenParam(FhirConstants.OPENMRS_FHIR_EXT_ENCOUNTER_TAG, "encounter"));
...

    {
      TokenAndListParam tokenAndListParam = new TokenAndListParam();
      tokenAndListParam.addAnd(
          new TokenParam(FhirConstants.OPENMRS_FHIR_EXT_ENCOUNTER_TAG, "visit"));
      SearchParameterMap theParams = new SearchParameterMap();
      theParams.addParameter(FhirConstants.TAG_SEARCH_HANDLER, tokenAndListParam);
      EncounterSearchParams encounterSearchParams = new EncounterSearchParams();
      encounterSearchParams.setTag(tokenAndListParam);
  ...
  public void getEncounterEverything_shouldReturnAllInformationAboutSpecifiedEncounter() {
    ...
    SearchParameterMap theParams =
        new SearchParameterMap()
            ...
            .addParameter(
                FhirConstants.COMMON_SEARCH_HANDLER, FhirConstants.ID_PROPERTY, encounterId);
  ...
  private void populateReverseIncludeForEverythingOperationParams(SearchParameterMap theParams) {
    ...
    revIncludes.add(
        new Include(FhirConstants.OBSERVATION + ":" + FhirConstants.INCLUDE_ENCOUNTER_PARAM));
    revIncludes.add(
        new Include(FhirConstants.DIAGNOSTIC_REPORT + ":" + FhirConstants.INCLUDE_ENCOUNTER_PARAM));
    revIncludes.add(
        new Include(
            FhirConstants.MEDICATION_REQUEST + ":" + FhirConstants.INCLUDE_ENCOUNTER_PARAM));
    revIncludes.add(
        new Include(FhirConstants.SERVICE_REQUEST + ":" + FhirConstants.INCLUDE_ENCOUNTER_PARAM));
  ...
  private void populateIncludeForEverythingOperationParams(SearchParameterMap theParams) {
    ...
    includes.add(
        new Include(FhirConstants.ENCOUNTER + ":" + FhirConstants.INCLUDE_PARTICIPANT_PARAM));
