...
public class PersonFhirResourceProviderWebTest extends BaseFhirR3ResourceProviderWebTest<PersonFhirResourceProvider, Person> {
	...
	public void shouldAddPatientsToResultListWhenIncluded() throws Exception {
		...
		verify(personService).searchForPeople(personSearchParamsCaptor.capture());
		Set<Include> includes = personSearchParamsCaptor.getValue().getIncludes();
		...
		assertThat(includes.iterator().next().getParamType(), equalTo(FhirConstants.PERSON));
	}
	...
	public void shouldAddLinksToResultListWhenIncluded() throws Exception {
		...
		assertThat(includes, notNullValue());
		assertThat(includes.size(), equalTo(1));
		assertThat(includes.iterator().next().getParamName(), equalTo(FhirConstants.INCLUDE_LINK_PARAM));
		assertThat(includes.iterator().next().getParamType(), equalTo(FhirConstants.PERSON));
		assertThat(includes.iterator().next().getParamTargetType(), equalTo(FhirConstants.PATIENT));
	}
