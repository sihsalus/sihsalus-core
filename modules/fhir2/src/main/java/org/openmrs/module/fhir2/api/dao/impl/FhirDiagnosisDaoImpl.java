...
public class FhirDiagnosisDaoImpl extends BaseFhirDao<Diagnosis> implements FhirDiagnosisDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Diagnosis, U> criteriaContext,
  ...
  protected <U> void handleDiagnosisCode(
      @Nonnull OpenmrsFhirCriteriaContext<Diagnosis, U> context, TokenAndListParam code) {
    if (code != null && code.size() > 0) {
      Join<?, ?> codedOrFreeTextJoin = context.addJoin("diagnosis", "diag");
      Join<?, ?> codedJoin = context.addJoin(codedOrFreeTextJoin, "coded", "dc");
      getSearchQueryHelper()
          .handleCodeableConcept(context, code, codedJoin, "dmap", "dterm")
