...
public class MedicationRequestTranslatorImpl implements MedicationRequestTranslator {
  ...
  @Getter(PROTECTED)
  @Setter(value = PROTECTED, onMethod_ = @Autowired)
  private MedicationRequestDispenseRequestComponentTranslator
      medicationRequestDispenseRequestComponentTranslator;
  ...
  public MedicationRequest toFhirResource(@Nonnull DrugOrder drugOrder) {
    ...
    if (drugOrder.getDrug() != null) {
      medicationRequest.setMedication(
          medicationReferenceTranslator.toFhirResource(drugOrder.getDrug()));
    } else {
    ...
    if (drugOrder.getUrgency() != null) {
      medicationRequest.setPriority(
          medicationRequestPriorityTranslator.toFhirResource(drugOrder.getUrgency()));
    }
    medicationRequest.setRequester(
        practitionerReferenceTranslator.toFhirResource(drugOrder.getOrderer()));
    medicationRequest.setEncounter(
        encounterReferenceTranslator.toFhirResource(drugOrder.getEncounter()));
    ...
    medicationRequest.setDispenseRequest(
        medicationRequestDispenseRequestComponentTranslator.toFhirResource(drugOrder));

    if (drugOrder.getPreviousOrder() != null
        && (drugOrder.getAction() == Order.Action.DISCONTINUE
            || drugOrder.getAction() == Order.Action.REVISE)) {
      medicationRequest.setPriorPrescription(
          medicationRequestReferenceTranslator
              .toFhirResource((DrugOrder) drugOrder.getPreviousOrder())
              .setIdentifier(
                  orderIdentifierTranslator.toFhirResource(drugOrder.getPreviousOrder())));
    } else if (drugOrder.getPreviousOrder() != null
        && drugOrder.getAction() == Order.Action.RENEW) {
      medicationRequest.setBasedOn(
  ...
  public DrugOrder toOpenmrsType(
    ...
    if (medicationRequest.hasMedicationReference()) {
      Drug drug =
          medicationReferenceTranslator.toOpenmrsType(medicationRequest.getMedicationReference());
    ...
    if (medicationRequest.getPriority() != null) {
      existingDrugOrder.setUrgency(
          medicationRequestPriorityTranslator.toOpenmrsType(medicationRequest.getPriority()));
    }
    existingDrugOrder.setOrderer(
        practitionerReferenceTranslator.toOpenmrsType(medicationRequest.getRequester()));
    existingDrugOrder.setEncounter(
        encounterReferenceTranslator.toOpenmrsType(medicationRequest.getEncounter()));
    existingDrugOrder.setPatient(
        patientReferenceTranslator.toOpenmrsType(medicationRequest.getSubject()));

    existingDrugOrder.setCommentToFulfiller(medicationRequest.getNoteFirstRep().getText());
    existingDrugOrder.setOrderReason(
        conceptTranslator.toOpenmrsType(medicationRequest.getReasonCodeFirstRep()));
    dosageTranslator.toOpenmrsType(
        existingDrugOrder, medicationRequest.getDosageInstructionFirstRep());

    medicationRequestDispenseRequestComponentTranslator.toOpenmrsType(
        existingDrugOrder, medicationRequest.getDispenseRequest());

    if (medicationRequest.getExtensionByUrl(OPENMRS_FHIR_EXT_MEDICATION_REQUEST_FULFILLER_STATUS)
        != null) {
      if (!medicationRequest
          .getExtensionByUrl(OPENMRS_FHIR_EXT_MEDICATION_REQUEST_FULFILLER_STATUS)
          .getValue()
          .isEmpty()) {
        existingDrugOrder.setFulfillerStatus(
