...
public class ImmunizationObsGroupHelper {
  ...
  public static UnprocessableEntityException createImmunizationRequestValidationError(
      @Nonnull String errMsg) {
    return new UnprocessableEntityException(errMsg, createExceptionErrorOperationOutcome(errMsg));
  }

  public static NotImplementedOperationException createImmunizationRequestSetupError(
      @Nonnull String errMsg) {
    return new NotImplementedOperationException(
        errMsg, createExceptionErrorOperationOutcome(errMsg));
  }
  ...
  public EncounterType getImmunizationsEncounterType() {
    ...
    return Optional.ofNullable(encounterService.getEncounterTypeByUuid(uuid))
        .orElseThrow(
            () ->
  ...
  public EncounterRole getAdministeringEncounterRole() {
    ...
    return Optional.ofNullable(encounterService.getEncounterRoleByUuid(uuid))
        .orElseThrow(
            () ->
                createImmunizationRequestSetupError(
  ...
  public Concept concept(String refTerm) {
    Set<String> directRefTerms =
        new HashSet<>(
            Arrays.asList(
                IMMUNIZATION_FREE_TEXT_COMMENT_CONCEPT, IMMUNIZATION_NEXT_DOSE_DATE_CONCEPT_CODE));
    if (directRefTerms.contains(refTerm)) {
      return conceptOrNull(refTerm);
    }

    return getConceptFromMapping(refTerm)
        .orElseThrow(
            () ->
                createImmunizationRequestSetupError(
  ...
  public Obs addNewObs(Obs immunizationObsGroup, String obsCode) {
    Obs obs =
        new Obs(
            immunizationObsGroup.getPerson(),
            concept(obsCode),
            immunizationObsGroup.getObsDatetime(),
  ...
  public Provider getAdministeringProvider(Obs obs) {
    ...
    return obs.getEncounter().getProvidersByRole(role).stream()
        .findFirst()
        .orElseThrow(
            () ->
  ...
  public void validateImmunizationObsGroup(Obs obs) {
    if (!concept(IMMUNIZATION_GROUPING_CONCEPT).equals(obs.getConcept())) {
      throw createImmunizationRequestSetupError(
          "The Immunization resource requires the underlying OpenMRS immunization obs group to be defined by a concept mapped as same as "
              ...
              + obs.getUuid()
              + "' that is defined by the concept named '"
              + obs.getConcept().getName().toString()
              + "'.");
    }

    final Set<String> refConcepts =
        IMMUNIZATION_CONCEPTS.stream()
            .map(this::getConceptFromMapping)
    ...
    List<String> obsConcepts =
        obs.getGroupMembers().stream()
            .map(o -> o.getConcept().getUuid())
            .filter(refConcepts::contains)
        ...
        uuid -> {
          if (refConcepts.contains(uuid)) {
            ...
            throw createImmunizationRequestValidationError(
                "The immunization obs member defined by concept with UUID '"
                    + uuid
  ...
  public Map<String, Obs> getObsMembersMap(Obs obs) {
    ...
    Map<Concept, String> concepts =
        IMMUNIZATION_CONCEPTS.stream().collect(Collectors.toMap(this::concept, refTerm -> refTerm));

    return obs.getGroupMembers().stream()
        .filter(o -> concepts.containsKey(o.getConcept()))
