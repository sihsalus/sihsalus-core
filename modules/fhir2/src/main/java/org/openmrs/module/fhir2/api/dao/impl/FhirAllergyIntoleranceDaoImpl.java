...
@Component
public class FhirAllergyIntoleranceDaoImpl extends BaseFhirDao<Allergy>
    implements FhirAllergyIntoleranceDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Allergy, U> criteriaContext,
  ...
  private <U> void handleManifestation(
      OpenmrsFhirCriteriaContext<Allergy, U> criteriaContext, TokenAndListParam code) {
    if (code != null) {
  ...
  private <U> void handleSeverity(
      OpenmrsFhirCriteriaContext<Allergy, U> criteriaContext, TokenAndListParam severityParam) {
    if (severityParam == null) {
      return;
    }
    Map<String, String> severityConceptUuids =
        globalPropertyService.getGlobalProperties(
            ...
            token -> {
              try {
                AllergyIntolerance.AllergyIntoleranceSeverity severity =
                    AllergyIntolerance.AllergyIntoleranceSeverity.fromCode(token.getValue());
                switch (severity) {
                  case MILD:
                    return Optional.of(
                        criteriaContext
  ...
  private <T, U> Optional<Predicate> handleAllergenCategory(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext,
      From<?, ?> allergyJoin,
      String propertyName,
      TokenAndListParam categoryParam) {
        ...
        token -> {
          try {
            AllergyIntolerance.AllergyIntoleranceCategory category =
                AllergyIntolerance.AllergyIntoleranceCategory.fromCode(token.getValue());
            switch (category) {
              case FOOD:
                return Optional.of(
                    criteriaContext
                        .getCriteriaBuilder()
                        .equal(allergyJoin.get(propertyName), AllergenType.FOOD));
              case MEDICATION:
                return Optional.of(
                    criteriaContext
                        .getCriteriaBuilder()
                        .equal(allergyJoin.get(propertyName), AllergenType.DRUG));
              case ENVIRONMENT:
                return Optional.of(
                    criteriaContext
                        .getCriteriaBuilder()
                        .equal(allergyJoin.get(propertyName), AllergenType.ENVIRONMENT));
              case NULL:
                return Optional.of(
                    criteriaContext
                        .getCriteriaBuilder()
                        .equal(allergyJoin.get(propertyName), AllergenType.OTHER));
            }
          } catch (FHIRException ignored) {
          }
          return Optional.empty();
