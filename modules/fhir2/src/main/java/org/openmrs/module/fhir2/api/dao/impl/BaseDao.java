 ...
 * queries simpler.
 *
 * <p>For example, to create a query for people with the first name "Liam" and last name either
 * "Hemsworth" or "Neeson", the following code can be used:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>This will generate a query that looks something like:
 *
 * <pre>{@code
 * SELECT *
 * FROM patient this_
 ...
 * }</pre>
 *
 * <p>These methods can also be used to simplify the code to generate very complex queries. For
 * example, the following code allows grouping {@link TokenParam} representing {@link
 * org.hl7.fhir.r4.model.CodeableConcept}s into groups based on systems with correct AND / OR logic:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>This can generate queries that look something like:
 *
 * <pre>{@code
 * SELECT *
 * FROM obs this_
...
public abstract class BaseDao {
  ...
  @Setter(
      ...
      onMethod_ = {@Autowired, @Qualifier("sessionFactory")})
  private SessionFactory sessionFactory;

  /**
   * If the supplied object is a {@link HibernateProxy} instead of the "normal" class, returns the
   * "normal" class version of the object. If the supplied object is not a {@link HibernateProxy},
   ...
   * @param <V> The expected type of the object. Note that a {@link ClassCastException} may be
   *     thrown if the underlying implementation of the proxied object does not match this type.
   */
  ...
  protected static <T> Stream<T> parallelStream(Iterator<T> iterator) {
    return StreamSupport.stream(
        Spliterators.spliteratorUnknownSize(iterator, Spliterator.ORDERED), true);
  }
   ...
   * @param <T> The root type and return type of queries run from the returned {@link
   ...
   */
  protected <T, U> OpenmrsFhirCriteriaContext<T, U> createCriteriaContext(
      @Nonnull Class<T> rootType, @Nonnull Class<U> resultType) {
    EntityManager em = sessionFactory.getCurrentSession();
    CriteriaBuilder cb = em.getCriteriaBuilder();
   ...
   * which is true for standard OpenMRS domain objects, but should be verified for domain objects
   * not defined in core.
   *
   ...
   */
  protected <V> String getIdPropertyName(
      @Nonnull EntityManager entityManager, @Nonnull Class<V> clazz) {
    EntityType<V> entityType = entityManager.getMetamodel().entity(clazz);
    if (!entityType.hasSingleIdAttribute()) {
      throw new IllegalArgumentException(
          "Entity must have a single id attribute: " + clazz.getName());
    }

    @SuppressWarnings({"rawtypes", "unchecked"})
    String idPropertyName =
        entityType.getId((Class) entityType.getIdType().getJavaType()).getName();
   ...
   * alias.
   *
   * @param criteriaContext The {@link BaseFhirCriteriaHolder} to extract the {@link From} object
   *     from
   ...
   */
  protected <T> From<?, ?> getRootOrJoin(
      @Nonnull BaseFhirCriteriaHolder<T> criteriaContext, @Nullable String alias) {
          ...
          .orElseThrow(
              () ->
                  new IllegalStateException(
                      "Tried to reference alias "
                          + alias
                          + " before creating a join with that name"));
    }
  }

  /**
   * Attempts to provide a consistent interface to the {@link From} object representing either the
   * {@link Root} object for the current criteria context or the {@link Join} object that matches
   * the provided {@link From} object.
   *
   ...
   */
  protected <T, U> From<?, ?> getRootOrJoin(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, From<?, ?> alias) {
    if (alias == null) {
      return criteriaContext.getRoot();
    } else {
      return criteriaContext
  ...
  /**
   * A generic handler for any subtype of {@link IQueryParameterAnd<T>} which converts the {@link
   * IQueryParameterAnd<T>} into a stream of {@link IQueryParameterOr<T>} objects.
   *
   ...
   * @return A stream of {@link IQueryParameterOr<T>}s contained in the {@link
   *     IQueryParameterAnd<T>}
   */
  protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType>
      Stream<T> handleAndListParam(IQueryParameterAnd<T> andListParameter) {
    return andListParameter.getValuesAsQueryTokens().stream();
  }

  /**
   * A generic handler for any subtype of {@link IQueryParameterAnd<T>} which creates a criterion
   * that represents the intersection of all the parameters contained<br>
   * <br>
   * This differs from {@link #handleAndListParamBy(CriteriaBuilder, IQueryParameterAnd, Function)}
   * in that the handler is called for each parameter in the {@link IQueryParameterOr<U>}.
   *
   ...
   */
  protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType>
      Optional<Predicate> handleAndListParam(
          CriteriaBuilder criteriaBuilder,
          IQueryParameterAnd<T> andListParam,
          Function<U, Optional<Predicate>> handler) {
        ...
        criteriaBuilder.and(
            toCriteriaArray(
                handleAndListParam(andListParam)
                    .map(
                        orListParam -> handleOrListParam(criteriaBuilder, orListParam, handler)))));
  }
   ...
   */
  protected <T extends IQueryParameterOr<U>, U extends IQueryParameterType>
      Optional<Predicate> handleAndListParamAsStream(
          CriteriaBuilder criteriaBuilder,
          IQueryParameterAnd<T> andListParam,
    ...
    Predicate[] predicates =
        toCriteriaArray(
            handleAndListParam(andListParam)
                .map(
                    orListParam ->
                        handleOrListParamAsStream(criteriaBuilder, orListParam, handler)));
   ...
   * @param systemTokenHandler a {@link BiFunction} taking the system and associated list of {@link
   *     TokenParam}s and returning a {@link Criterion}
   * @return a {@link Predicate} representing the intersection of all produced {@link Predicate}s
   */
  protected <T extends IQueryParameterOr<TokenParam>>
      Optional<Predicate> handleAndListParamBySystem(
          CriteriaBuilder criteriaBuilder,
          IQueryParameterAnd<T> andListParam,
        ...
        criteriaBuilder.and(
            toCriteriaArray(
                handleAndListParam(andListParam)
                    .map(
                        param ->
                            handleOrListParamBySystem(
                                criteriaBuilder, param, systemTokenHandler)))));
  }

  /**
   * A generic handler for any subtype of {@link IQueryParameterAnd<T>} which converts the {@link
   * IQueryParameterOr<T>} into a stream of {@link IQueryParameterType<T>} objects.
   *
   * @param <T> the subtype of {@link IQueryParameterType} that each {@link IQueryParameterOr} uses
   * @return A stream of {@link IQueryParameterType<T>}s contained in the {@link
   *     IQueryParameterOr<T>}
   */
  protected <T extends IQueryParameterType> Stream<T> handleOrListParam(
      IQueryParameterOr<T> orListParameter) {
   ...
   */
  protected <T extends IQueryParameterType> Optional<Predicate> handleOrListParam(
      CriteriaBuilder criteriaBuilder,
      IQueryParameterOr<T> orListParam,
   ...
   * <br>
   * Unlike {@link #handleAndListParam(CriteriaBuilder, IQueryParameterAnd, Function)}, this
   * function is called as part of the list streaming and may be slightly more efficient.
   *
   * @param criteriaBuilder the active {@link CriteriaBuilder} for the current query
   * @param orListParam the {@link IQueryParameterOr} to handle
   * @param handler a {@link Function} which maps a parameter to a {@link Criterion}
   ...
   */
  protected <T extends IQueryParameterType> Optional<Predicate> handleOrListParamAsStream(
      CriteriaBuilder criteriaBuilder,
      IQueryParameterOr<T> orListParam,
  ...
  /**
   * Handler for a {@link IQueryParameterOr} of {@link TokenParam}s where tokens should be grouped
   * and handled according to the system they belong to This is useful for queries drawing their
   * values from CodeableConcepts
   *
   * @param criteriaBuilder the active {@link CriteriaBuilder} for the current query
   ...
   */
  protected Optional<Predicate> handleOrListParamBySystem(
      CriteriaBuilder criteriaBuilder,
    ...
    return Optional.of(
        criteriaBuilder.or(
            toCriteriaArray(
                handleOrListParam(orListParam)
                    .collect(Collectors.groupingBy((t) -> StringUtils.trimToEmpty(t.getSystem())))
                    .entrySet()
                    .stream()
                    .map(e -> systemTokenHandler.apply(e.getKey(), e.getValue())))));
  }
   ...
   */
  @SafeVarargs
  protected final @Nonnull Predicate[] toCriteriaArray(
      @Nonnull Optional<? extends Predicate>... predicates) {
   ...
   *     arguments
   */
  protected @Nonnull Predicate[] toCriteriaArray(
      @Nonnull Collection<Optional<? extends Predicate>> predicates) {
    return toCriteriaArray(predicates.stream());
  }

  /**
   * Converts a given {@link Stream} of {@link Optional<Predicate>}s into a {@link Predicate[]}.
   * <br>
   * Any {@link Optional<Predicate>}s that are empty are skipped.
   *
   ...
   */
  protected @Nonnull Predicate[] toCriteriaArray(
      @Nonnull Stream<Optional<? extends Predicate>> predicates) {
    return predicates.filter(Optional::isPresent).map(Optional::get).toArray(Predicate[]::new);
  }

  /**
   * Concerts a {@link List} of {@link TokenParam}s into a {@link List} of {@link String}s where
