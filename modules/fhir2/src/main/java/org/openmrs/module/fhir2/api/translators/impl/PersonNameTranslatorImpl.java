            ...
            ext ->
                ext.getExtension()
                    .forEach(
                        e ->
                            addNameComponent(
                                personName, e.getUrl(), ((StringType) e.getValue()).getValue())));

    return personName;
  }

  private void addNameExtension(
      @Nonnull HumanName name, @Nonnull String extensionProperty, @Nonnull String value) {
    if (value == null) {
      return;
    }

    getOpenmrsNameExtension(name)
        .orElseGet(() -> name.addExtension().setUrl(FhirConstants.OPENMRS_FHIR_EXT_NAME))
        .addExtension(
            FhirConstants.OPENMRS_FHIR_EXT_NAME + "#" + extensionProperty, new StringType(value));
  }

  private void addNameComponent(
      @Nonnull PersonName name, @Nonnull String url, @Nonnull String value) {
    if (value == null
        || url == null
        || !url.startsWith(FhirConstants.OPENMRS_FHIR_EXT_NAME + "#")) {
      return;
    }
