...
@Component
public class FhirConditionServiceImpl extends BaseFhirService<Condition, org.openmrs.Condition>
    implements FhirConditionService {
  ...
  private SearchQuery<
          ...
          Condition,
          FhirConditionDao,
          ConditionTranslator<org.openmrs.Condition>,
  ...
  public Condition create(@Nonnull Condition condition) {
    ...
    FhirUtils.OpenmrsConditionType result =
        FhirUtils.getOpenmrsConditionType(condition).orElse(null);
  ...
  public IBundleProvider searchConditions(ConditionSearchParams conditionSearchParams) {
    ...
    DiagnosisSearchParams diagnosisSearchParams =
        DiagnosisSearchParams.builder()
            .patientParam(conditionSearchParams.getPatientParam())
            .code(conditionSearchParams.getCode())
            .clinicalStatus(conditionSearchParams.getClinicalStatus())
            .onsetDate(conditionSearchParams.getOnsetDate())
            .onsetAge(conditionSearchParams.getOnsetAge())
            .recordedDate(conditionSearchParams.getRecordedDate())
            .category(conditionSearchParams.getCategory())
            .id(conditionSearchParams.getId())
            .lastUpdated(conditionSearchParams.getLastUpdated())
            .sort(conditionSearchParams.getSort())
            .includes(
                conditionSearchParams.getIncludes() == null
                    ? null
                    : new HashSet<>(conditionSearchParams.getIncludes()))
            .revIncludes(
                conditionSearchParams.getRevIncludes() == null
                    ? null
    ...
    if (shouldSearchExplicitlyFor(
        conditionSearchParams.getCategory(), FhirConstants.CONDITION_CATEGORY_CODE_DIAGNOSIS)) {
      diagnosisBundle = diagnosisService.searchDiagnoses(diagnosisSearchParams);
    }

    if (shouldSearchExplicitlyFor(
        conditionSearchParams.getCategory(), FhirConstants.CONDITION_CATEGORY_CODE_CONDITION)) {
    ...
    if (conditionBundle != null && diagnosisBundle != null) {
      return new TwoSearchQueryBundleProvider(
          diagnosisBundle, conditionBundle, globalPropertyService);
   ...
   *     nothing is provided.
   */
  protected boolean shouldSearchExplicitlyFor(
      TokenAndListParam tokenAndListParam, @Nonnull String valueToCheck) {
    ...
    for (TokenOrListParam orList : tokenAndListParam.getValuesAsQueryTokens()) {
      for (TokenParam tp : orList.getValuesAsQueryTokens()) {
        ...
        if (sys != null
            && !sys.isEmpty()
            && FhirConstants.CONDITION_CATEGORY_SYSTEM_URI.equals(sys)
