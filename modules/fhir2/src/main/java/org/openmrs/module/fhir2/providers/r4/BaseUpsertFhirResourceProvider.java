...
public abstract class BaseUpsertFhirResourceProvider<T extends IAnyResource>
  ...
  @Update
  public MethodOutcome upsert(
      @IdParam IdType id, @ResourceParam T resource, RequestDetails requestDetails) {
    ...
    if (createIfNotExists) {
      Object created =
          requestDetails.getUserData().get(FhirConstants.USER_DATA_KEY_OUTCOME_CREATED);
      if (created != null) {
   ...
   */
  protected abstract MethodOutcome doUpsert(
      IdType id, T resource, RequestDetails requestDetails, boolean createIfNotExists);
