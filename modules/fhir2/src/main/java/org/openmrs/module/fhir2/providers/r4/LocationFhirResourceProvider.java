...
public class LocationFhirResourceProvider extends BaseUpsertFhirResourceProvider<Location> {
  ...
  @Override
  public MethodOutcome doUpsert(
      IdType id, Location location, RequestDetails requestDetails, boolean createIfNotExists) {
    if (id == null || id.getIdPart() == null) {
      throw new InvalidRequestException("id must be specified to update");
    }

    return FhirProviderUtils.buildUpdate(
        fhirLocationService.update(id.getIdPart(), location, requestDetails, createIfNotExists));
  }

  @Patch
  public MethodOutcome patchLocation(
      @IdParam IdType id,
  ...
  public IBundleProvider searchLocations(
    ...
    return (fhirLocationService.searchForLocations(
