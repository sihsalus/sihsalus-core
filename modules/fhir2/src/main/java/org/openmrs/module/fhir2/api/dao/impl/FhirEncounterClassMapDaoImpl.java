...
public class FhirEncounterClassMapDaoImpl extends BaseDao {
  ...
  public String getFhirClass(@Nonnull String locationUuid) {
    OpenmrsFhirCriteriaContext<FhirEncounterClassMap, String> criteriaContext =
        createCriteriaContext(FhirEncounterClassMap.class, String.class);

    Join<?, ?> locationJoin = criteriaContext.addJoin("location", "l");
    criteriaContext.addPredicate(
        criteriaContext.getCriteriaBuilder().equal(locationJoin.get("uuid"), locationUuid));
    ...
    try {
      List<String> resultsList =
          criteriaContext
              ...
              .createQuery(
                  criteriaContext
                      .finalizeQuery()
                      .select(criteriaContext.getRoot().get("encounterClass")))
              .getResultList();
      ...
      log.error(
          "Exception caught while trying to load encounter type for location '{}'", locationUuid);
    }
