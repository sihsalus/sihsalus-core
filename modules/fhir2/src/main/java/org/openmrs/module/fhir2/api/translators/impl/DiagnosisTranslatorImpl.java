...
public class DiagnosisTranslatorImpl implements DiagnosisTranslator {
  ...
  public Condition toFhirResource(@Nonnull Diagnosis diagnosis) {
    ...
    if (diagnosis.getEncounter() != null) {
      fhirCondition.setEncounter(
          encounterReferenceTranslator.toFhirResource(diagnosis.getEncounter()));
    }
    ...
    if (diagnosis.getDiagnosis() != null) {
      if (diagnosis.getDiagnosis().getCoded() != null) {
        fhirCondition.setCode(
            conceptTranslator.toFhirResource(diagnosis.getDiagnosis().getCoded()));
      }
      ...
      switch (diagnosis.getCertainty()) {
        case CONFIRMED:
          verificationStatus.addCoding(
              new Coding()
                  .setSystem(FhirConstants.CONDITION_VER_STATUS_SYSTEM_URI)
                  .setCode("confirmed")
    ...
    if (diagnosis.getEncounter() != null
        && diagnosis.getEncounter().getEncounterDatetime() != null) {
    ...
    fhirCondition.setRecorder(
        practitionerReferenceTranslator.toFhirResource(diagnosis.getCreator()));
  ...
  @Override
  public Diagnosis toOpenmrsType(
      @Nonnull Diagnosis existingDiagnosis, @Nonnull Condition condition) {
    ...
    if (condition.hasCode()) {
      ...
      Optional<Extension> nonCodedExtension =
          Optional.ofNullable(
              condition.getExtensionByUrl(FhirConstants.OPENMRS_FHIR_EXT_NON_CODED_CONDITION));
      nonCodedExtension.ifPresent(
          ext -> diagnosisCodedOrText.setNonCoded(String.valueOf(ext.getValue())));
    ...
    Optional<Extension> rankExtension =
        Optional.ofNullable(
            condition.getExtensionByUrl(FhirConstants.DIAGNOSIS_RANK_EXTENSION_URI));
    rankExtension.ifPresent(
        ext -> {
