...
public abstract class BaseEncounterDao<T extends OpenmrsObject & Auditable> extends BaseFhirDao<T> {
              ...
              switch (entry.getKey()) {
                case FhirConstants.DATE_RANGE_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleDate(criteriaContext, (DateRangeParam) param.getParam())
  ...
  protected <U> void handleHasAndListParam(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, HasAndListParam hasAndListParam) {
          ...
          .forEach(
              hasOrListParam -> {
                if (!hasOrListParam.getValuesAsQueryTokens().isEmpty()) {
                  ...
                  // Making the assumption that any "orListParams" match everything except for the
                  // value
                  HasParam hasParam = hasOrListParam.getValuesAsQueryTokens().get(0);
                  Set<String> values = new LinkedHashSet<>();
                  hasOrListParam
                      .getValuesAsQueryTokens()
                      .forEach(orParam -> values.add(orParam.getParameterValue()));

                  log.debug(
                      "Handling hasParam = {} for values in {}",
                      hasParam.getQueryParameterQualifier(),
                      values);

                  boolean handled = false;

                  // Support constraining encounter resources to those that contain only certain
                  // Medication Requests
                  if (FhirConstants.MEDICATION_REQUEST.equals(hasParam.getTargetResourceType())) {
                    if (MedicationRequest.SP_ENCOUNTER.equals(hasParam.getReferenceFieldName())) {
                      ...
                      criteriaContext.addPredicate(
                          criteriaContext
                              .getCriteriaBuilder()
                              .equal(ordersJoin.get().type(), DrugOrder.class));
                      criteriaContext.addPredicate(
                          criteriaContext
                              .getCriteriaBuilder()
                              .equal(ordersJoin.get().get("voided"), false));
                      criteriaContext.addPredicate(
                          criteriaContext
                              .getCriteriaBuilder()
                      ...
                      if (MedicationRequest.SP_INTENT.equals(paramName)) {
                        if (values.contains(
                            MedicationRequest.MedicationRequestIntent.ORDER.toCode())) {
                          // No additional constraints needed, all Orders are given/assumed
                          // intent=order
                          handled = true;
                        }
                        ...
                        if (paramValue != null) {
                          if (MedicationRequest.MedicationRequestStatus.ACTIVE
                              .toString()
                            ...
                            criteriaContext.addPredicate(
                                criteriaContext
                                    ...
                                    .and(
                                        getSearchQueryHelper()
                                            .handleQueryForActiveOrders(
                                                criteriaContext, "orders")));
                          }
                        }
                        handled = true;
                      } else if ((MedicationRequest.SP_STATUS + ":not")
                          .equalsIgnoreCase(paramName)) {
                        if (paramValue != null) {
                          if (MedicationRequest.MedicationRequestStatus.CANCELLED
                              .toString()
                              .equalsIgnoreCase(paramValue)) {
                            criteriaContext.addPredicate(
                                criteriaContext
                          ...
                          if (MedicationRequest.MedicationRequestStatus.COMPLETED
                              .toString()
                              .equalsIgnoreCase(paramValue)) {
                            Predicate notCompletedCriterion =
                                generateNotCompletedOrderQuery(criteriaContext, ordersJoin.get());
                        ...
                        if (paramValue != null) {
                          criteriaContext.addPredicate(
                              criteriaContext
                                  .getCriteriaBuilder()
                                  .and(
                                      generateFulfillerStatusRestriction(
                                          criteriaContext, ordersJoin.get(), paramValue)));
                        }
                        handled = true;
                      } else if ((FhirConstants.SP_FULFILLER_STATUS + ":not")
                          .equalsIgnoreCase(paramName)) {
                        if (paramValue != null) {
                          criteriaContext.addPredicate(
                              criteriaContext
                                  .getCriteriaBuilder()
                                  .and(
                                      generateNotFulfillerStatusRestriction(
                                          criteriaContext, ordersJoin.get(), paramValue)));
                        }
                        handled = true;
                      }
                  ...
                  if (!handled) {
                    log.warn(
                        "_has parameter not supported: {}", hasParam.getQueryParameterQualifier());
                  }
                }
              });
    }
  ...
  protected abstract <U> Optional<Predicate> handleDate(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, DateRangeParam dateRangeParam);

  protected abstract <U> Optional<Predicate> handleEncounterType(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, TokenAndListParam tokenAndListParam);

  protected abstract <U> Optional<Predicate> handleParticipant(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext,
      ReferenceAndListParam referenceAndListParam);

  protected <V, U> Predicate generateNotCompletedOrderQuery(
      OpenmrsFhirCriteriaContext<V, U> criteriaContext, Join<?, ?> ordersJoin) {
    // not implemented in Core until 2.2; see override in FhirEncounterDaoImpl_2_2
    return null;
  }

  protected <V, U> Predicate generateFulfillerStatusRestriction(
  ...
  protected <V, U> Predicate generateNotFulfillerStatusRestriction(
      OpenmrsFhirCriteriaContext<V, U> criteriaContext,
      Join<?, ?> ordersJoin,
      String fulfillerStatus) {
    // not implemented in Core until 2.2; see override in FhirEncounterDaoImpl_2_2
