...
public class ObservationFhirResourceProvider implements IResourceProvider {
        ...
        VersionConvertorFactory_30_40.convertResource(
            observationService.create(
                (org.hl7.fhir.r4.model.Observation)
                    VersionConvertorFactory_30_40.convertResource(observation))));
  }
  ...
  public IBundleProvider searchObservations(
      @OptionalParam(
              name = Observation.SP_ENCOUNTER,
              chainWhitelist = {"", Encounter.SP_TYPE},
      ...
      @OptionalParam(
              name = Observation.SP_RELATED_TYPE,
              chainWhitelist = {"", Observation.SP_CODE},
              targetTypes = Observation.class)
      ...
      @IncludeParam(
              allow = {
                ...
                "Observation:" + Observation.SP_PATIENT,
                "Observation:" + Observation.SP_RELATED_TYPE
              })
    ...
    return new SearchQueryBundleProviderR3Wrapper(
        observationService.searchForObservations(
            new ObservationSearchParams(
                encounterReference,
                patientReference,
                hasMemberReference,
                valueConcept,
  ...
  public IBundleProvider getLastnObservations(
    ...
    return new SearchQueryBundleProviderR3Wrapper(
        observationService.getLastnObservations(max, searchParams));
  }

  /**
   * The $lastn-encounters operation fetches the observations matching the most recent `N`
   * encounters corresponding to the specified patients.
   *
   ...
   * @return a bundle of observations whose corresponding encounter is among the recent `N`
   *     encounters for the specified patient
   */
  @Operation(
      name = "lastn-encounters",
      idempotent = true,
      type = Observation.class,
      bundleType = BundleTypeEnum.SEARCHSET)
  public IBundleProvider getLastnEncountersObservations(
      @OperationParam(name = "max") NumberParam max,
      @OperationParam(name = Observation.SP_SUBJECT) ReferenceAndListParam subjectParam,
      @OperationParam(name = Observation.SP_PATIENT) ReferenceAndListParam patientParam,
      @OperationParam(name = Observation.SP_CATEGORY) TokenAndListParam category,
    ...
    return new SearchQueryBundleProviderR3Wrapper(
        observationService.getLastnEncountersObservations(max, searchParams));
  }
}
