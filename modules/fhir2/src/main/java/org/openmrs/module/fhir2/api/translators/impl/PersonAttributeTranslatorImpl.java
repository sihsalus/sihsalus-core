...
public class PersonAttributeTranslatorImpl implements PersonAttributeTranslator {
  ...
  public Extension toFhirResource(@Nonnull PersonAttribute personAttribute) {
    if (personAttribute == null
        || personAttribute.getVoided()
        || personAttribute.getAttributeType() == null) {
      return null;
    }
  ...
  @Override
  public PersonAttribute toOpenmrsType(
      @Nonnull PersonAttribute personAttribute, @Nonnull Extension personAttributeExtension) {
    ...
    try {
      PersonAttributeType personAttributeType =
          personService.getPersonAttributeTypeByName(attributeTypeName);
    ...
    Extension valueExtension =
        personAttributeExtension.getExtensionByUrl(
            FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE_VALUE);
  ...
  private Extension createPersonAttributeExtension(PersonAttribute personAttribute) {
    ...
    Extension personAttributeTypeExtension =
        new Extension(FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE_TYPE);
    ...
    Extension personAttributeExtension =
        new Extension(FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE);
    personAttributeExtension.setExtension(
        Arrays.asList(personAttributeTypeExtension, valueExtension));
    return personAttributeExtension;
  }

  protected Extension createValueExtension(PersonAttribute personAttribute) {
    Extension valueExtension =
        new Extension(new UriType(FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE_VALUE));
    ...
    if (!valueExtension.hasValue()) {
      if (value != null) {
        log.warn(
            "Could not create a FHIR value for attribute '{}' with format '{}'",
            personAttribute.getUuid(),
            format);
      }
      return null;
    }
  ...
  protected boolean isValidPatientAttributeExtension(Extension extension) {
    return extension != null
        ...
        && extension.getExtensionByUrl(FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE_VALUE)
            != null
        && extension
            .getExtensionByUrl(FhirConstants.OPENMRS_FHIR_EXT_PERSON_ATTRIBUTE_TYPE)
            .hasValue()
        && extension
  ...
  protected void setPersonAttributeValue(PersonAttribute personAttribute, Type extensionValue) {
    if (extensionValue instanceof BooleanType) {
      ...
      if (reference != null) {
        referenceToId(reference)
            .ifPresent(
                (uuid) -> {
                  referenceToType(reference)
                      .ifPresent(
                          (type) -> {
                            switch (type) {
      ...
      log.warn(
          "Extension value type: {} is not supported for PersonAttribute value",
