...
public class FhirPersonDaoImpl extends BasePersonDao<Person> implements FhirPersonDao {
  ...
  @Override
  public List<PersonAttribute> getActiveAttributesByPersonAndAttributeTypeUuid(
      @Nonnull Person person, @Nonnull String personAttributeTypeUuid) {
    OpenmrsFhirCriteriaContext<PersonAttribute, PersonAttribute> criteriaContext =
        createCriteriaContext(PersonAttribute.class);
    ...
    criteriaContext.addJoin(
        ...
        (from) ->
            cb.and(
                cb.equal(from.get("personId"), person.getId()),
                cb.equal(from.get("personVoided"), false)));
    criteriaContext.addJoin(
        "attributeType",
        "pat",
        (from) ->
            cb.and(
                cb.equal(from.get("uuid"), personAttributeTypeUuid),
                cb.equal(from.get("retired"), false)));
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Person, U> criteriaContext,
                                        ...
                                        Optional.of(
                                            criteriaContext
                                                ...
                                                .isNull(
                                                    criteriaContext
                                                        .getRoot()
                                                        .get("personDateChanged"))),
                                        getSearchQueryHelper()
                                            .handleDateRange(
                                                criteriaContext, "personDateCreated", param))))))));
  }

  @Override
  protected <U> void handleVoidable(
      @Nonnull OpenmrsFhirCriteriaContext<Person, U> criteriaContext) {
