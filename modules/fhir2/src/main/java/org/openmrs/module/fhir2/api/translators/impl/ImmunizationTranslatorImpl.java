...
public class ImmunizationTranslatorImpl implements ImmunizationTranslator {
  ...
  public static final Set<String> IMMUNIZATION_CONCEPTS =
      ImmutableSet.of(
          "CIEL:984",
          "CIEL:1410",
          "CIEL:1418",
  ...
  public Obs toOpenmrsType(
      @Nonnull Obs openmrsImmunization, @Nonnull Immunization fhirImmunization) {
    ...
    if (fhirImmunization.hasLocation()) {
      Location recordedLocation =
          locationReferenceTranslator.toOpenmrsType(fhirImmunization.getLocation());
    ...
    Coding coding =
        fhirImmunization.getVaccineCode().getCoding().stream()
            .filter(code -> StringUtils.isEmpty(code.getSystem()))
            .reduce(
                (code1, code2) -> {
                  throw createImmunizationRequestValidationError(
                      "Multiple system-less coding found for the immunization's vaccine: "
                          + code1.getCode()
                          + " and "
                          + code2.getCode()
    ...
    if (!fhirImmunization.hasOccurrenceDateTimeType()
        || !fhirImmunization.getOccurrenceDateTimeType().hasValue()) {
      throw createImmunizationRequestValidationError(
          "An Immunization must have a valid occurrenceDateTime value");
    }
    ...
    if (fhirImmunization.hasProtocolApplied()) {
      ...
      ImmunizationProtocolAppliedComponent protocolApplied =
          fhirImmunization.getProtocolApplied().get(0);
      if (protocolApplied.hasDoseNumber()) {
        {
          ...
          if (obs == null) {
            ...
            obs.setValueNumeric(
                protocolApplied.getDoseNumberPositiveIntType().getValue().doubleValue());
          } else if (obs.getId() == null) {
            obs.setValueNumeric(
                protocolApplied.getDoseNumberPositiveIntType().getValue().doubleValue());
          } else {
    ...
    if (noteConcept != null
        && fhirImmunization.hasNote()
        && fhirImmunization.getNoteFirstRep().hasText()) {
    ...
    if (nextDoseDateConcept != null
        && fhirImmunization.hasExtension(
            FhirConstants.OPENMRS_FHIR_EXT_IMMUNIZATION_NEXT_DOSE_DATE)) {
      Extension extension =
          fhirImmunization.getExtensionByUrl(
              FhirConstants.OPENMRS_FHIR_EXT_IMMUNIZATION_NEXT_DOSE_DATE);
      if (extension != null
          && extension.hasValue()
  ...
  public Immunization toFhirResource(@Nonnull Obs openmrsImmunization) {
    ...
    immunization.setPatient(
        patientReferenceTranslator.toFhirResource(new Patient(openmrsImmunization.getPerson())));
    immunization.setEncounter(
        visitReferenceTranslator.toFhirResource(openmrsImmunization.getEncounter().getVisit()));
    immunization.setPerformer(
        Collections.singletonList(
            new ImmunizationPerformerComponent(
                practitionerReferenceTranslator.toFhirResource(
                    helper.getAdministeringProvider(openmrsImmunization)))));
...

    {
      ...
      if (obs != null && obs.getValueNumeric() != null) {
        immunization.addProtocolApplied(
            new ImmunizationProtocolAppliedComponent(
...

    {
      ...
      if (obs != null && obs.getValueDatetime() != null) {
        immunization.addExtension(
            new Extension()
                .setUrl(FhirConstants.OPENMRS_FHIR_EXT_IMMUNIZATION_NEXT_DOSE_DATE)
