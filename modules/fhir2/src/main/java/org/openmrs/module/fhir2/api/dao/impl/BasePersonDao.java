...
public abstract class BasePersonDao<T extends OpenmrsObject & Auditable> extends BaseFhirDao<T> {
  ...
  @Override
  protected <V, U> Collection<Order> paramToProps(
    ...
    if (param.equals(SP_NAME) || param.equals(SP_GIVEN) || param.equals(SP_FAMILY)) {
      ...
      // For person names, we have several subqueries to ensure that we are only working against a
      // single person name
  ...
  protected <U> void handleAddresses(
  ...
  protected <U> void handleNames(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, List<PropParam<?>> params) {
