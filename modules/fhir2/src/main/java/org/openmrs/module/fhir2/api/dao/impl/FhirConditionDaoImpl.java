...
public class FhirConditionDaoImpl extends BaseFhirDao<Condition> implements FhirConditionDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Condition, U> criteriaContext,
              ...
              switch (entry.getKey()) {
                ...
                case FhirConstants.CODED_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleCode(criteriaContext, (TokenAndListParam) param.getParam()));
                  break;
                ...
                case FhirConstants.QUANTITY_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleOnsetAge(
                                  criteriaContext, (QuantityAndListParam) param.getParam()));
                  break;
  ...
  private <U> void handleCode(
      OpenmrsFhirCriteriaContext<Condition, U> criteriaContext, TokenAndListParam code) {
    if (code != null) {
      ...
      getSearchQueryHelper()
          .handleCodeableConcept(criteriaContext, code, codedJoin, "map", "term")
          .ifPresent(criteriaContext::addPredicate);
    }
  }

  private <U> void handleClinicalStatus(
      OpenmrsFhirCriteriaContext<Condition, U> criteriaContext, TokenAndListParam status) {
    handleAndListParam(
            ...
            tokenParam ->
                Optional.of(
                    criteriaContext
                        .getCriteriaBuilder()
                        .equal(
                            criteriaContext.getRoot().get("clinicalStatus"),
                            convertStatus(tokenParam.getValue()))))
        .ifPresent(criteriaContext::addPredicate);
  }
  ...
  private <U> void handleOnsetAge(
      OpenmrsFhirCriteriaContext<Condition, U> criteriaContext, QuantityAndListParam onsetAge) {
    handleAndListParam(
            criteriaContext.getCriteriaBuilder(),
            onsetAge,
            onsetAgeParam ->
                getSearchQueryHelper()
                    .handleAgeByDateProperty(criteriaContext, "onsetDate", onsetAgeParam))
        .ifPresent(criteriaContext::addPredicate);
  }
