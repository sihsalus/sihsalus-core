...
public class LocationTranslatorImpl implements LocationTranslator {
  ...
  /**
   * @see
   *     org.openmrs.module.fhir2.api.translators.LocationTranslator#toFhirResource(org.openmrs.Location)
   */
  @Override
  ...
  public List<Location> toFhirResources(Collection<org.openmrs.Location> openmrsLocations) {
    final LocationTranslatorContext context =
        new LocationTranslatorContext(getLocationContactDetails(openmrsLocations));

    return openmrsLocations.stream()
        .map((location) -> toFhirResource(location, context))
        .collect(Collectors.toList());
  }

  /**
   * @see
   *     org.openmrs.module.fhir2.api.translators.LocationTranslator#toOpenmrsType(org.hl7.fhir.r4.model.Location)
   */
  @Override
  ...
  /**
   * @see
   *     org.openmrs.module.fhir2.api.translators.LocationTranslator#toOpenmrsType(org.openmrs.Location,
   *     org.hl7.fhir.r4.model.Location)
   */
  @Override
  public org.openmrs.Location toOpenmrsType(
      @Nonnull org.openmrs.Location openmrsLocation, @Nonnull Location fhirLocation) {
    ...
    fhirLocation.getTelecom().stream()
        .map(
            contactPoint ->
                (LocationAttribute)
                    telecomTranslator.toOpenmrsType(new LocationAttribute(), contactPoint))
        .distinct()
        .filter(Objects::nonNull)
        .forEach(openmrsLocation::addAttribute);

    if (fhirLocation.hasType()) {
      openmrsLocation =
          locationTypeTranslator.toOpenmrsType(openmrsLocation, fhirLocation.getType());
    }
  ...
  public org.openmrs.Location getOpenmrsParentLocation(Reference location) {
    ...
    if (location.hasType() && !location.getType().equals("Location")) {
      throw new IllegalArgumentException(
          "Reference must be to a Location not a " + location.getType());
    }
  ...
  private Map<org.openmrs.Location, List<ContactPoint>> getLocationContactDetails(
    ...
    final String locationContactPointAttributeType =
    ...
    final Map<org.openmrs.Location, List<LocationAttribute>> contactPointAttributes =
        fhirLocationDao.getActiveAttributesByLocationsAndAttributeTypeUuid(
            locations, locationContactPointAttributeType);

    return contactPointAttributes.entrySet().stream()
        .collect(
            Collectors.toMap(
                Map.Entry::getKey,
                entry ->
  ...
  private Location toFhirResource(
      org.openmrs.Location openmrsLocation, LocationTranslatorContext context) {
    ...
    if (openmrsLocation.getTags() != null) {
      for (LocationTag tag : openmrsLocation.getTags()) {
        fhirLocation
            .getMeta()
            .addTag(
                FhirConstants.OPENMRS_FHIR_EXT_LOCATION_TAG, tag.getName(), tag.getDescription());
      }
    }

    if (openmrsLocation.getParentLocation() != null) {
      fhirLocation.setPartOf(
          locationReferenceTranslator.toFhirResource(openmrsLocation.getParentLocation()));
    }
