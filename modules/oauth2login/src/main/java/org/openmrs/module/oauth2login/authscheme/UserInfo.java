...
public class UserInfo {
  ...
  public static final String MAPPINGS_PFX = "openmrs.mapping.";

  public static final String PROP_USERNAME = MAPPINGS_PFX + "user.username";

  public static final String PROP_USERNAME_SERVICE_ACCOUNT =
      MAPPINGS_PFX + "user.username.serviceAccount";

  public static final String PROP_EMAIL = MAPPINGS_PFX + "user.email";

  public static final String PROP_GIVEN_NAME = MAPPINGS_PFX + "person.givenName";

  public static final String PROP_MIDDLE_NAME = MAPPINGS_PFX + "person.middleName";

  public static final String PROP_FAMILY_NAME = MAPPINGS_PFX + "person.familyName";

  public static final String PROP_GENDER = MAPPINGS_PFX + "person.gender";

  public static final String PROP_SYSTEMID = MAPPINGS_PFX + "user.systemId";

  public static final String PROP_ROLES = MAPPINGS_PFX + "user.roles";

  public static final String PROP_PROVIDER = MAPPINGS_PFX + "user.provider";
   ...
   * @throws PathNotFoundException when the mapped JSON key cannot be located in the user info JSON.
   * @param propertyKey The OpenMRS property whose value is to be fetched out of the user info JSON,
   *     eg. "openmrs.mapping.user.username".
    ...
    if (props.containsKey(propertyKey)) {
      ...
      try {
        ...
        throw new PathNotFoundException(
            "There was an error when reading the JSON path $."
                + jsonKey
                + " mapped from '"
                + propertyKey
                + "' in the user info JSON.",
            e);
      }
    }
    return null;
  ...
  /**
   * Fetches the value in the user info JSON as a String based on the property key as defined in the
   * OAuth2 properties file. If the key is not defined in the OAuth2 properties file or if the value
   * for that key cannot be parsed from the user info JSON file, then the fallback default value is
   * returned.
   *
   * @param propertyKey The OpenMRS property whose value is to be fetched out of the user info JSON,
   *     eg. "openmrs.mapping.user.username".
  ...
  public String getString(String propertyKey, String defaultValue) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "The user info JSON value corresponding to the key '"
              + propertyKey
              + "' cannot be resolved to a simple string value, perhaps is it a structured object?",
          e);
    }
  ...
  /**
   * Convenience method that retrieves the OpenMRS user name from the OAuth2 user info JSON based on
   * the mappings defined in the OAuth2 properties file.
   *
   ...
   * mappings defined in the OAuth2 properties file. This method will not attempt to set any default
   * values when mappings are not defined in the OAuth2 properties file.
   *
