...
public abstract class BasePageableResult<T> implements PageableResult {
  ...
  /**
   * Return the total number of records available for the requested resource and the applied request
   * parameters.
   */
  ...
  /**
   * @see PageableResult#toSimpleObject(Converter) <strong>Should</strong> add property totalCount
   *     if context contains parameter totalCount which is true <strong>Should</strong> not add
   *     property totalCount if context contains parameter totalCount which is false
   *     <strong>Should</strong> not add property totalCount if context does not contains parameter
   *     totalCount
   */
  @Override
  public SimpleObject toSimpleObject(Converter preferredConverter) throws ResponseException {
    ...
    for (T match : getPageOfResults()) {
      results.add(
          ConversionUtil.convertToRepresentation(
              match, context.getRepresentation(), preferredConverter));
    }
    ...
    if (context.getStartIndex() > 0 || hasMore) {
      ...
      if (hasMore) links.add(context.getNextLink());
      if (context.getStartIndex() > 0) links.add(context.getPreviousLink());
      ret.add("links", links);
    }
