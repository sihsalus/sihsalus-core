...
public class ConversionUtil {
  ...
  static {
    ...
    nullConverter =
        new Converter<Object>() {
          ...
          @Override
          public SimpleObject asRepresentation(Object instance, Representation rep)
  ...
  public static <T> Converter<T> getConverter(Class<T> clazz) {
    ...
    try {
      try {
        Resource resource =
            Context.getService(RestService.class).getResourceBySupportedClass(clazz);
    ...
    // At this point, we don't really care if a result was found or not, we cache it regardless so
    // that repeated
   ...
   * @return The specified object converted to the specified type <strong>Should</strong> resolve
   *     TypeVariables to actual type
   */
  public static Object convert(Object object, Type toType, Object instance)
      throws ConversionException {
   ...
   * @return
   * @throws ConversionException <strong>Should</strong> convert strings to locales
  ...
  public static Object convert(Object object, Type toType) throws ConversionException {
    ...
    if (Collection.class.isAssignableFrom(toClass) || toClass.isArray()) {
      if (!(object instanceof Collection))
        throw new ConversionException(
            "Can only convert a Collection to a Collection/Array. Not "
                + object.getClass()
                + " to "
                + toType,
            null);
      ...
      if (toType instanceof ParameterizedType) {
        // if we have generic type information for the target collection, we can use it to do
        // conversion
    ...
    if (object instanceof String) {
      ...
      if (converter != null) return converter.getByUniqueId(string);
      ...
      if (toClass.isAssignableFrom(Date.class)) {
        ...
        throw new ConversionException(
            "Error converting date - correct format (ISO8601 Long): yyyy-MM-dd'T'HH:mm:ss.SSSZ",
            pex);
        ...
        try {
          ...
          throw new ConversionException(
              "Could not convert from " + object.getClass() + " to " + toType, e);
        }
      }
      ...
      try {
        ...
        if (Modifier.isStatic(method.getModifiers())
            && toClass.isAssignableFrom(method.getReturnType())) {
    ...
    throw new ConversionException(
        "Don't know how to convert from " + object.getClass() + " to " + toType, null);
  }
   ...
   * @return the result of using a converter to instantiate a new class and set map's properties on
   *     it
   * @throws ConversionException
   */
  public static Object convertMap(Map<?, ?> map, Class<?> toClass) throws ConversionException {
    ...
    // If the converter is a resource handler use the order of properties of its default
    // representation
    if (converter instanceof DelegatingResourceHandler) {
      ...
      if (resDesc != null) {
        for (Map.Entry<String, Property> prop : resDesc.getProperties().entrySet()) {
          if (map.containsKey(prop.getKey())
   ...
   * @throws ConversionException
   */
  public static Object getPropertyWithRepresentation(
      Object bean, String propertyName, Representation rep) throws ConversionException {
    Object o;
    try {
    ...
    if (o instanceof Collection) {
      ...
      for (Object element : (Collection<?>) o) ret.add(convertToRepresentation(element, rep));
      return ret;
    } else {
  ...
  public static <S> Object convertToRepresentation(
      S o, Representation rep, Converter specificConverter) throws ConversionException {
    ...
    if (o instanceof Collection) {
      ...
      Converter<S> converter =
          specificConverter != null ? specificConverter : (Converter) getConverter(o.getClass());
      if (converter == null) {
      ...
      if (converter instanceof BaseDelegatingResource) {
        String requiredPrivilege =
            ((BaseDelegatingResource<?>) converter).getRequiredGetPrivilege();
  ...
  /**
   * Converts an object to its custom representation This could be used to convert any domain
   * objects that does not have any specific converter associated with them such as SimpleObject's,
   * Map's, etc
   */
  private static SimpleObject convertToCustomRepresentation(Object o, CustomRepresentation rep) {
    ...
    for (String propertyName : drd.getProperties().keySet()) {
      ...
      Object propertyValue =
          ConversionUtil.getPropertyWithRepresentation(o, propertyName, property.getRep());
  ...
  public static SimpleObject getAuditInfo(Object delegate) {
    ...
    if (delegate instanceof Auditable) {
      ...
      ret.put(
          "dateCreated",
          convertToRepresentation(auditable.getDateCreated(), Representation.DEFAULT));
      ret.put(
          "changedBy", getPropertyWithRepresentation(auditable, "changedBy", Representation.REF));
      ret.put(
          "dateChanged",
          convertToRepresentation(auditable.getDateChanged(), Representation.DEFAULT));
    }
    if (delegate instanceof Retireable) {
      ...
      if (retireable.isRetired()) {
        ret.put(
            "retiredBy",
            getPropertyWithRepresentation(retireable, "retiredBy", Representation.REF));
        ret.put(
            "dateRetired",
    ...
    if (delegate instanceof Voidable) {
      ...
      if (voidable.isVoided()) {
        ...
        ret.put(
            "dateVoided",
            convertToRepresentation(voidable.getDateVoided(), Representation.DEFAULT));
        ret.put(
            "voidReason",
            convertToRepresentation(voidable.getVoidReason(), Representation.DEFAULT));
      }
    }
  ...
  public static DelegatingResourceDescription getCustomRepresentationDescription(
