...
public class NamedRepresentation implements Representation {
  ...
  public NamedRepresentation(String representation) {
    if (representation == null) throw new IllegalArgumentException("representation is required");
  ...
  public boolean matchesAnnotation(RepHandler ann) {
    return NamedRepresentation.class.isAssignableFrom(ann.value())
        && ann.name().equals(representation);
  }
}
