...
public class RequestContext {
  ...
  /**
   * @param limit the limit to set <strong>Should</strong> not accept a value less than one
   *     <strong>Should</strong> not accept a null value
   */
  public void setLimit(Integer limit) {
    if (limit == null || limit <= 0)
      throw new APIException(
          "If you specify a number of results to return, it must be >0 and not null");
    if (limit > RestUtil.getAbsoluteLimit())
      throw new APIException(
          "Administrator has set absolute limit at " + RestUtil.getAbsoluteLimit());
    else this.limit = limit;
  }

  /**
   * Should be used by search methods to jump results to start with this number in the list. Set by
   * users in a request parameter
   *
  ...
  public Hyperlink getNextLink() {
    ...
    return new Hyperlink(
        "next",
        RestUtil.determineSchemeHostAndPortFromRequest(request)
  ...
  public Hyperlink getPreviousLink() {
    ...
    if (prevStart > 0) query += RestConstants.REQUEST_PROPERTY_FOR_START_INDEX + "=" + prevStart;
    return new Hyperlink(
        "prev",
        RestUtil.determineSchemeHostAndPortFromRequest(request)
            .append(request.getRequestURI())
   ...
   * @return the value <strong>Should</strong> return the request parameter of given name if present
   *     in the request <strong>Should</strong> return null if the wanted request parameter is not
   *     present in the request <strong>Should</strong> return null if request is null
   */
