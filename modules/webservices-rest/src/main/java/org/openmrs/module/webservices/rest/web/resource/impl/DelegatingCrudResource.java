 ...
 */
public abstract class DelegatingCrudResource<T> extends BaseDelegatingResource<T>
    implements CrudResource, Searchable, Listable {

  protected final Log log = LogFactory.getLog(getClass());

  /**
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.api.Retrievable#retrieve(java.lang.String,
   *     org.openmrs.module.webservices.rest.web.representation.Representation)
   */
  @Override
  public Object retrieve(String uuid, RequestContext context) throws ResponseException {
    T delegate = getByUniqueId(uuid);
    if (delegate == null) throw new ObjectNotFoundException();

    SimpleObject ret = asRepresentation(delegate, context.getRepresentation());
    if (hasTypesDefined()) ret.add(RestConstants.PROPERTY_FOR_TYPE, getTypeName(delegate));
    return ret;
  }
   ...
   *
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.api.Retrievable#getAvailableRepresentations()
   */
  @Override
  ...
  /**
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.api.Creatable#create(org.springframework.web.context.request.WebRequest)
   */
  @Override
  public Object create(SimpleObject propertiesToCreate, RequestContext context)
    ...
    SimpleObject ret =
        (SimpleObject)
            ConversionUtil.convertToRepresentation(delegate, context.getRepresentation());
  ...
  public T convert(SimpleObject propertiesToCreate) {
    ...
    DelegatingResourceDescription description =
        buildCreatableProperties(delegate, handler, propertiesToCreate);
  ...
  public Object update(String uuid, SimpleObject propertiesToUpdate, RequestContext context)
    ...
    if (hasTypesDefined() && propertiesToUpdate.containsKey(RestConstants.PROPERTY_FOR_TYPE)) {
      ...
      if (!delegate.getClass().equals(getActualSubclass(type))) {
        ...
        if (nameToShow == null) nameToShow = delegate.getClass().getName();
        throw new IllegalArgumentException(
            "You passed "
                + RestConstants.PROPERTY_FOR_TYPE
                + "="
                + type
                + " but this instance is a "
                + nameToShow);
      }
    }
  ...
  public void purge(String uuid, RequestContext context) throws ResponseException {
    T delegate = getByUniqueId(uuid);
    if (delegate == null) {
      // HTTP DELETE is idempotent, so if we can't find the object, we assume it's already deleted
      // and return success
      return;
    }
  ...
  /**
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.api.Searchable#search(org.openmrs.module.webservices.rest.web.RequestContext)
   */
  @Override
  ...
  /**
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.api.Listable#getAll(org.openmrs.module.webservices.rest.web.RequestContext)
   */
  @Override
   ...
   * the specified rest resource in the database. (If the resource supports subclasses, this method
   * should return all of its documents regardless of their type/subclass.)
   *
   * @throws ResponseException
   */
   ...
   * @throws ResponseException
   */
  public Object listSubResource(String delegateUuid, String subResourceName, Representation rep)
      throws ResponseException {
    // TODO SUBCLASSHANDLER
    List<String> legal = getPropertiesToExposeAsSubResources();
    if (legal == null || !legal.contains(subResourceName)) throw new IllegalPropertyException();
   ...
   *
   * @deprecated Since 2.x the namespace must be declared in {@link Resource}'s name, {@link
   *     MainResourceController} and {@link MainSubResourceController}.
   */
  @Deprecated
