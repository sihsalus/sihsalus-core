...
public class ReflectionUtil {
   ...
   *
   * <p><strong>Should</strong> find genericInterface on a superclass if clazz does not directly
   * implement it <strong>Should</strong> ignore type variables on the declaring interface
   * <strong>Should</strong> not inspect superclasses of the specified genericInterface
   */
  @SuppressWarnings("rawtypes")
  public static Class getParameterizedTypeFromInterface(
      Class<?> clazz, Class<?> genericInterface, int index) {
   ...
   * @return
   */
  public static <T> Method findPropertyGetterMethod(
      DelegatingPropertyAccessor<? extends T> handler, String propName) {
   ...
   * @return
   */
  public static <T> Method findPropertySetterMethod(
      DelegatingPropertyAccessor<? extends T> handler, String propName) {
    String key = handler.getClass().getName().concat(propName);
  ...
  public static Method findMethod(Class<?> clazz, String name) {
    ...
    if (ret == null) throw new RuntimeException("No suitable method \"" + name + "\" in " + clazz);
    return ret;
  }
}
