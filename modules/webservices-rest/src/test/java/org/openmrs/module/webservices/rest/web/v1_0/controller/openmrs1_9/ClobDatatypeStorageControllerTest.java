...
public class ClobDatatypeStorageControllerTest extends MainResourceControllerTest {
  ...
  public void shouldAcceptAndStoreClobDataViaPost() throws Exception {
    ...
    MockMultipartFile toUpload =
        new MockMultipartFile("file", "formresource.txt", "text/plain", fileData);
  ...
  public void shouldReturnClobDataAsFileByUuid() throws Exception {
    ClobDatatypeStorage clob =
        datatypeService.getClobDatatypeStorageByUuid(
  ...
  public void shouldDeleteAnExistingClobData() throws Exception {
    ...
    MockHttpServletResponse response =
        handle(
            newDeleteRequest(
                getURI() + "/" + RestTestConstants1_9.CLOBDATATYPESTORAGE_RESOURCE_UUID));
  ...
  public long getAllCount() {
    try {
      ...
      ResultSet resultSet =
          connection
              .prepareStatement("select count('id') from clob_datatype_storage")
              .executeQuery();
      if (resultSet.next()) {
