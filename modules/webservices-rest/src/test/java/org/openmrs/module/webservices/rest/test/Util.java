...
public class Util {
  ...
  public static void log(String label, Object object) {
    ...
    if (label != null) toPrint = label + ": " + toPrint;
    ...
    try {
      for (int i = 0; i < pathElements.length; ++i) {
        ...
        if (pathElements[i].indexOf("[") > 0) {
          ...
          index =
              Integer.valueOf(
                  pathElements[i].substring(
                      pathElements[i].indexOf("[") + 1, pathElements[i].indexOf("]")));
        } else {
          property = pathElements[i];
          index = null;
        }
        if (property != null) object = PropertyUtils.getProperty(object, property);
        if (index != null) {
      ...
      throw new RuntimeException(
          "Failed to get path " + OpenmrsUtil.join(Arrays.asList(pathElements), " . "), ex);
    }
  }
