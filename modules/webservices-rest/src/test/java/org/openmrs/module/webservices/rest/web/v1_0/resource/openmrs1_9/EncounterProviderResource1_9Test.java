...
public class EncounterProviderResource1_9Test
    extends BaseDelegatingResourceTest<EncounterProviderResource1_9, EncounterProvider> {

  public static final String EXISTING_ENCOUNTER_PROVIDER_UUID =
      "19e0aae8-20ee-46b7-ade6-9e68f897b7a9";

  public static final String EXISTING_ENCOUNTER_PROVIDER_PROVIDER_UUID =
      "c2299800-cca9-11e0-9572-0800200c9a66";

  public static final String EXISTING_ENCOUNTER_PROVIDER_ENCOUNTER_ROLE_UUID =
  ...
  public void testGetAll() throws Exception {
    ...
    assertThat(
        (String) getByPath(results.get(0), "provider.uuid"),
        is("c2299800-cca9-11e0-9572-0800200c9a66"));
    assertThat(
        (String) getByPath(results.get(0), "encounterRole.uuid"),
        is("a0b03050-c99b-11e0-9572-0800200c9a66"));
  }
  ...
  public void testGetOne() throws Exception {
    ...
    Object one =
        resource.retrieve(
            encounter.getUuid(), EXISTING_ENCOUNTER_PROVIDER_UUID, new RequestContext());

    assertThat((String) getByPath(one, "uuid"), is(EXISTING_ENCOUNTER_PROVIDER_UUID));
    assertThat(
        (String) getByPath(one, "provider.uuid"), is("c2299800-cca9-11e0-9572-0800200c9a66"));
    assertThat(
        (String) getByPath(one, "encounterRole.uuid"), is("a0b03050-c99b-11e0-9572-0800200c9a66"));
  }

  @Test
  @Disabled(
      "Fails when running with all tests, but passes if run alone... needs to be investigated")
  public void testAdd() throws Exception {
    // If we don't explicitly set a locale, then a Context.getLocale() call nested somewhere below
    // will trigger a
    ...
    SimpleObject post =
        new SimpleObject()
            .add("provider", "c2299800-cca9-11e0-9572-0800200c9a66")
            .add("encounterRole", newRole.getUuid());

    Object created = resource.create(encounter.getUuid(), post, new RequestContext());
    assertThat(
        (String) getByPath(created, "provider.uuid"), is("c2299800-cca9-11e0-9572-0800200c9a66"));
    ...
    assertThat(
        getEncounterProviderCountWithoutFlushingByUuid(getByPath(created, "uuid").toString()),
        is(1));
  }
  ...
  public void testAddingDuplicateDoesNotCreateNewRecord() throws Exception {
    ...
    SimpleObject post =
        new SimpleObject()
            .add("provider", EXISTING_ENCOUNTER_PROVIDER_PROVIDER_UUID)
            .add("encounterRole", EXISTING_ENCOUNTER_PROVIDER_ENCOUNTER_ROLE_UUID);
    Object created = resource.create(encounter.getUuid(), post, new RequestContext());
  ...
  public void testDelete() throws Exception {
    ...
    EncounterProviderResource1_9 resource = getResource();
    resource.delete(
        encounter.getUuid(), EXISTING_ENCOUNTER_PROVIDER_UUID, "reason", new RequestContext());
  ...
  private int getEncounterProviderCountWithoutFlushing() {
    List<List<Object>> temp =
        Context.getAdministrationService()
            .executeSQL("select count(*) from encounter_provider where encounter_id = 3", true);
  ...
  private int getEncounterProviderCountWithoutFlushingByUuid(String uuid) {
    List<List<Object>> temp =
        Context.getAdministrationService()
            .executeSQL("select count(*) from encounter_provider where uuid ='" + uuid + "'", true);
    return ((Number) temp.get(0).get(0)).intValue();
  }
  ...
  private int getNonVoidedEncounterProviderCount() {
    List<List<Object>> temp =
        Context.getAdministrationService()
            .executeSQL(
                "select count(*) from encounter_provider where encounter_id = 3 and voided = 0",
                true);
    return ((Number) temp.get(0).get(0)).intValue();
  }
}
