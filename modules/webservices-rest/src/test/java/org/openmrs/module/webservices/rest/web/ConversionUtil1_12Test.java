...
public class ConversionUtil1_12Test extends BaseModuleWebContextSensitiveTest {
  ...
  public void convertToRepresentation_shouldConvertValuesOfMaps() throws Exception {
    ...
    SimpleObject converted =
        (SimpleObject) ConversionUtil.convertToRepresentation(toConvert, Representation.REF);

    assertThat(
        converted,
        (Matcher)
            hasEntry(
                is("date"), is(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").format(date))));
    assertThat(converted, (Matcher) hasEntry(is("string"), is("a string")));
    assertThat(
        converted, (Matcher) hasEntry(is("1"), is(hasEntry(is("uuid"), is(user.getUuid())))));
    assertThat(
        converted,
        (Matcher)
            hasEntry(
                is("map"),
                is(hasEntry(is("concept"), is(hasEntry(is("uuid"), is(concept.getUuid())))))));
  }
}
