...
public class ValidateUtilTest extends BaseModuleWebContextSensitiveTest {
  ...
  // we are not supporting this validation against 1.8, so when running against 1.8 no exception
  // should be thrown
  ...
  public void validate_shouldNotThrowValidationExceptionButShouldNotFail() throws Exception {
    ...
    // we are not supporting this validation against 1.8, so when running against 1.8 when passing
    // in an
