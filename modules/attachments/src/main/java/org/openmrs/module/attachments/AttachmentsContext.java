...
public class AttachmentsContext {
  ...
  protected EncounterType getEncounterTypeByGlobalProperty(
      String globalPropertyName, boolean required) {
    String globalProperty = administrationService.getGlobalProperty(globalPropertyName);
  ...
  public ConceptComplex getDefaultConceptComplex() {
    String globalPropertyName = AttachmentsConstants.GP_DEFAULT_CONCEPT_COMPLEX_UUID;
  ...
  public ConceptComplex getConceptComplex(ContentFamily contentFamily) {
    Map<String, String> map =
        getMapByGlobalProperty(AttachmentsConstants.GP_CONCEPT_COMPLEX_UUID_MAP);
  ...
  public List<String> getConceptComplexList() {
    ...
    try {
      ...
      log.error(
          "Could not parse global property '" + globalPropertyName + "' into a List<String>.", e);
    }
    return list;
  }
  ...
  public EncounterType getEncounterType() {
    EncounterType encounterType =
        getEncounterTypeByGlobalProperty(AttachmentsConstants.GP_ENCOUNTER_TYPE_UUID, false);
    return encounterType;
  }

  public Boolean associateWithVisit() {
    String associateWithVisitGP =
        administrationService.getGlobalProperty(AttachmentsConstants.GP_ASSOCIATE_WITH_VISIT);
  ...
  public double getMaxCompressionRatio() {
    ...
    if (maxStorageSize > 0) return Math.max(maxStorageSize, maxUploadSize) / maxStorageSize;
    else return 1;
  }
  ...
  public Integer getMaxRestResultsCount() {
    return getIntegerByGlobalProperty(
        AttachmentsConstants.GP_RESTWS_MAX_RESULTS_DEFAULT_GLOBAL_PROPERTY_NAME);
  }

  // TODO: Figure out if this is good enough
  public EncounterRole getEncounterRole() {
    EncounterRole unknownRole =
        getEncounterService().getEncounterRoleByUuid(EncounterRole.UNKNOWN_ENCOUNTER_ROLE_UUID);
    if (unknownRole == null) {
      throw new APIException(
          "No 'Unknown' encounter role with uuid "
