 ...
 *
 * <p>Se utiliza un "InMemoryFuaDao" que extiende FuaDao y sobreescribe los métodos necesarios para
 * verificar el comportamiento del servicio sin tocar base de datos ni Hibernate.
 */
public class FuaServiceTest {
  ...
  public void updateEstadoFua_shouldUpdateEstadoAndSaveFua() {
    ...
    assertSame(
        "El FUA actualizado debe haberse guardado en el DAO",
        existingFua,
        inMemoryDao.lastSavedFua);
    assertSame("El servicio debe devolver el FUA guardado", existingFua, result);
    assertEquals(
        "El servicio debe pedir el FUA al DAO con el ID correcto", fuaId, inMemoryDao.lastFuaId);
  }
