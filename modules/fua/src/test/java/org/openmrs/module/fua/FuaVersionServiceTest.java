...
public class FuaVersionServiceTest {
  ...
  @Test
  public void saveFuaVersion_shouldCreateVersionIncrementFuaVersionAndDelegateToDao()
      throws APIException {
    Fua fua = new Fua();
    ...
    assertEquals(
        "La versión del FUA debe haberse incrementado", Integer.valueOf(2), fua.getVersion());

    // 3) Comprobamos la descripción
    try {
      assertEquals(
          "Debe copiar la descripción", descripcion, inMemoryDao.lastSavedVersion.getDescripcion());
