...
public interface PatientCalculationService extends OpenmrsService {
   ...
   * <b>NOTE:</b> For memory and performance reasons, this method may call the calculation's
   * evaluate method multiple times with smaller batches than the whole input cohort
   *
   ...
   *     by the calculation
   * @param context the {@link PatientCalculationContext} to be used by this evaluation
   * @return A {@link CalculationResultMap}, with results for all all patients in cohort. Note that
   *     if cohort contains duplicate patient ids, this result's size will differ from cohort.size()
   *     <strong>Should</strong> fail if any required parameter is not set <strong>Should</strong>
   *     fail for a blank value for a required parameter if datatype is a primitive wrapper
   *     <strong>Should</strong> fail for a blank value for a required parameter if datatype is a
   *     String <strong>Should</strong> fail if the a parameter value doesnt match the allowed
   *     datatype <strong>Should</strong> pass if the passed in datatype cannot be loaded
   *     <strong>Should</strong> return the expected result size for cohort with a large number of
   *     patient
   */
  public CalculationResultMap evaluate(
