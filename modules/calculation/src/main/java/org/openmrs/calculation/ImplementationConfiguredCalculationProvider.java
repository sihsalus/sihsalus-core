...
/**
 * Automatically loads calculation classes from APPDATA/configuration/calculation. A single instance
 * of the class will be instantiated and cached, and any @Autowired properties will be injected.
 * Supports:
 *
 * <ul>
 *   <li>.groovy ... parsed by the groovy classloader (file should define a single class, that
...
public class ImplementationConfiguredCalculationProvider implements CalculationProvider {
  ...
  private File directory =
      new File(OpenmrsUtil.getApplicationDataDirectory(), "configuration/calculation");

  private Map<String, Calculation> calculations = new HashMap<String, Calculation>();

  private GroovyClassLoader groovyClassLoader =
      new GroovyClassLoader(OpenmrsClassLoader.getInstance());
  ...
  private Calculation loadCalculation(File file) {
    if (file.exists()) {
      try {
        ...
        log.error(
            "Error loading implementation-configured groovy calculation from " + file.getName(), e);
      }
    } else {
  ...
  /**
   * @return the cache of all calculations that have been loaded. May contain null entries to
   *     indicate that
   */
