...
public interface CalculationRegistrationService extends OpenmrsService {
   ...
   * @return <strong>Should</strong> return a token with a matching id
   */
   ...
   *
   * @param uuid
   * @return <strong>Should</strong> fetch a token with a matching uuid
   */
  @Authorized(CalculationConstants.PRIV_VIEW_TOKEN_REGISTRATIONS)
   ...
   *
   * @param token
   * @return <strong>Should</strong> fetch a token with a matching name
   */
  @Authorized(CalculationConstants.PRIV_VIEW_TOKEN_REGISTRATIONS)
   ...
   * @return <strong>Should</strong> save the specified calculationRegistration to the database
   *     <strong>Should</strong> update an existing token <strong>Should</strong> update the cached
   *     token registration
   */
  ...
  public CalculationRegistration saveCalculationRegistration(
      CalculationRegistration calculationRegistration) throws InvalidCalculationException;

  /**
   * Gets all tokens in the database
   *
   * @return a list of {@link CalculationRegistration}s <strong>Should</strong> get all tokens in
   *     the database
   */
  @Authorized(CalculationConstants.PRIV_VIEW_TOKEN_REGISTRATIONS)
   ...
   * @return a list of {@link CalculationRegistration}s <strong>Should</strong> get all
   *     calculationRegistrations with a matching name
   */
  @Authorized(CalculationConstants.PRIV_VIEW_TOKEN_REGISTRATIONS)
   ...
   *
   * @param calculationRegistration the calculationRegistration to purge <strong>Should</strong>
   *     purge the specified calculationRegistration from the database
   */
  @Authorized(CalculationConstants.PRIV_MANAGE_TOKEN_REGISTRATIONS)
  public void purgeCalculationRegistration(CalculationRegistration calculationRegistration);

  /**
   * Gets a {@link PatientCalculation} with given name associated to a {@link
   * CalculationRegistration} with a name that matches the specified tokenName
   *
   * @param tokenName
   * @return {@link PatientCalculation} <strong>Should</strong> return the calculation associated to
   *     the calculationRegistration with the given name
   */
