...
@Handler(
    supports = {CalculationRegistration.class},
    order = 50)
public class CalculationRegistrationValidator implements Validator {
   ...
   *     org.springframework.validation.Errors) <strong>Should</strong> fail if token registration
   *     name is not unique amongst all tokens <strong>Should</strong> fail if token registration
   *     name is empty or whitespace <strong>Should</strong> fail if token registration provider
   *     class name is empty or whitespace <strong>Should</strong> fail if token registration
   *     calculation name is empty or whitespace <strong>Should</strong> fail if token registration
   *     does not represent a valid calculation <strong>Should</strong> pass valid token
   *     registration <strong>Should</strong> pass existing token registration
   */
  public void validate(Object obj, Errors errors) {
    ...
    if (StringUtils.isNotBlank(target.getToken())) {
      CalculationRegistrationService tokenService =
          Context.getService(CalculationRegistrationService.class);
      CalculationRegistration possibleDuplicate =
          tokenService.getCalculationRegistrationByToken(target.getToken());
