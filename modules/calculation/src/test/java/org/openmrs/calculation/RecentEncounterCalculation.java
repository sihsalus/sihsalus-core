...
public class RecentEncounterCalculation extends BaseCalculation
    ...
    if (cohort != null) {
      ...
      for (Integer patientId : cohort) {
        ...
        if (patient != null) {
          ...
          TreeSet<Encounter> sortedEncounters =
              new TreeSet<Encounter>(
                  new Comparator<Encounter>() {

                    @Override
                    public int compare(Encounter o1, Encounter o2) {
                      return OpenmrsUtil.compareWithNullAsEarliest(
                          o1.getEncounterDatetime(), o2.getEncounterDatetime());
                    }
                  });

          List<Encounter> encounters = es.getEncountersByPatient(patient);
          for (Encounter encounter : encounters) {
            if (OpenmrsUtil.compareWithNullAsEarliest(encounter.getEncounterDatetime(), since)
                > 0) {
