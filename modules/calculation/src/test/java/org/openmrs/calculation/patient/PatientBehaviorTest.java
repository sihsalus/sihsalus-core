...
public class PatientBehaviorTest extends BaseModuleContextSensitiveTest {
  ...
  public void shouldCalculateThePatientAgeBasedOnContextualInfo() throws Exception {
    ...
    Assert.assertEquals(
        expected,
        getService().evaluate(patientId, ageCalculation, ctxt).asType(Integer.class).intValue());
  }
   ...
   */
  @Test
  public void shouldCalculateThePatientAgeBasedOnContextualInfoAndParameterValues()
      throws Exception {
    ...
    Assert.assertEquals(
        296,
        getService()
            .evaluate(patientId, ageCalculation, values, ctxt)
  ...
  @Test
  public void shouldCalculateTheAgesOfPatientsInACohortBasedOnContextualInfoAndParameterValues()
      throws Exception {
    PatientCalculation ageCalculation = getAgeCalculation();
    int patientId1 = 2;
    int patientId2 = 7;
  ...
  @Test
  public void ShouldGetADateBasedResultBasingOnACachedResultInTheCalculationContext()
      throws Exception {
    executeDataSet(TEST_DATA_XML);
    ...
    // sanity check, since the cache is empty, it should return the most recent obs amongst all obs
    // for the patient
    PatientCalculation mostRecentObsCalculation = getMostRecentWeightCalculation();
    ObsResult firstTestResult =
        (ObsResult) getService().evaluate(patientId, mostRecentObsCalculation, context);
    ...
    ObsResult secondTestResult =
        (ObsResult) getService().evaluate(patientId, anotherMostRecentObsCalculation, context);
  ...
  public void shouldGetListResultOfRecentEncounters() throws Exception {
    ...
    PatientCalculation calc =
        (PatientCalculation)
            new ClasspathCalculationProvider()
                .getCalculation(RecentEncounterCalculation.class.getName(), "2008-07-01");
  ...
  @Test
  public void shouldNotFailWhenInnerCalculationMakesDBChangesForOuterCalculation()
      throws Exception {
    PatientCalculation calc =
        (PatientCalculation)
            new ClasspathCalculationProvider()
                .getCalculation(OuterCalculation.class.getName(), null);
  ...
  private PatientCalculation getMostRecentEncounterCalculation()
    ...
    ClasspathCalculationProvider p = new ClasspathCalculationProvider();
    return (PatientCalculation)
        p.getCalculation(MostRecentEncounterCalculation.class.getName(), null);
  }
