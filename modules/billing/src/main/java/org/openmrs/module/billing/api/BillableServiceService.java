 ...
 *
 * <p>Billable services represent items or services that can be added to bills, such as procedures,
...
public interface BillableServiceService extends OpenmrsService {
   ...
   *
   * <p>By default, retired billable services are excluded from search results unless explicitly
   * included via {@link BillableServiceSearch#setIncludeRetired(Boolean)}. The search supports
   * filtering by service status, category, type, concept, and name.
   *
   * @param billableServiceSearch the search criteria
   * @param pagingInfo optional paging information (can be null for no paging)
   * @return a list of billable services matching the search criteria, or an empty list if none
   *     found
   * @see BillableServiceSearch
   */
  @Authorized(PrivilegeConstants.VIEW_METADATA)
  List<BillableService> getBillableServices(
      BillableServiceSearch billableServiceSearch, PagingInfo pagingInfo);

  /**
   * Saves a billable service to the database.
   *
   * <p>If the billable service is new (no ID), it will be created. If it already exists, it will be
   * updated.
   *
   ...
   *
   * <p><strong>Warning:</strong> This operation cannot be undone. All associated data (service
   * prices, attributes, etc.) will also be removed due to cascade delete rules. Consider retiring
   * the service instead for soft deletion.
   *
