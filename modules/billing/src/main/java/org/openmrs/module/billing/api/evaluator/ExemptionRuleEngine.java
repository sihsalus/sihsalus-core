...
public class ExemptionRuleEngine {
  ...
  private final Map<ScriptType, ExemptionEvaluator> evaluatorsByType =
      new EnumMap<>(ScriptType.class);
  ...
  public boolean isExemptionApplicable(BillExemption exemption, Map<String, Object> variables) {
    ...
    return exemption.getRules().stream()
        .filter(r -> !r.getVoided())
        .anyMatch(r -> evaluateRule(r, variables));
  }
}
