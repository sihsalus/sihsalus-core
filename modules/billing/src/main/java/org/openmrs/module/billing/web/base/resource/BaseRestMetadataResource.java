...
public abstract class BaseRestMetadataResource<E extends OpenmrsMetadata>
    extends MetadataDelegatingCrudResource<E> implements IMetadataDataServiceResource<E> {
  ...
  public E save(E entity) {
    try {
      ...
      log.error(
          "Exception occured when trying to save entity <"
              + entity.getName()
              + "> as privilege is missing",
          p);
      throw new PrivilegeException(
          "Can't save entity with name <" + entity.getName() + "> as privilege is missing");
    }
  }
  ...
  public E getByUniqueId(String uniqueId) {
    ...
    if (service != null) {
      try {
        ...
        log.error(
            "Exception occured when trying to get entity with ID <"
                + uniqueId
                + "> as privilege is missing",
            p);
        throw new PrivilegeException(
  ...
  public void purge(E entity, RequestContext context) {
    try {
      ...
      log.error(
          "Exception occured when trying to purge entity <"
              + entity.getName()
              + "> as privilege is missing",
          p);
      throw new PrivilegeException(
  ...
  protected PageableResult doGetAll(RequestContext context) {
    ...
    return new AlreadyPagedWithLength<E>(
        context,
        getService().getAll(context.getIncludeAll(), pagingInfo),
