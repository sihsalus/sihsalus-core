 ...
 */
@Slf4j
public abstract class BaseRestDataResource<E extends OpenmrsData>
    extends DataDelegatingCrudResource<E> implements IEntityDataServiceResource<E> {
   ...
   */
  public static <E extends OpenmrsObject> void syncCollection(
      Collection<E> base, Collection<E> sync) {
    syncCollection(
        base,
        sync,
        (collection, entity) -> collection.add(entity),
  ...
  public E getByUniqueId(String uniqueId) {
    ...
    // Ensure that a service is found for this resource. This is a fix for changes to the RESTWS
    // module as of v2.12.
    ...
    if (service != null) {
      try {
        ...
        log.error(
            "Exception occured when trying to get entity with ID <{}> as privilege is missing",
            uniqueId,
            p);
        throw new PrivilegeException(
            "Can't get entity with ID <" + uniqueId + "> as privilege is missing");
      }
    }
  ...
  protected AlreadyPaged<E> doSearch(RequestContext context) {
    ...
    return new ServiceSearcher<E>(this.getServiceClass(), "getResources", "getCountOfResources")
  ...
  protected IEntityDataService<E> getService() {
    // Ensure that the service class is not null. This is a fix for changes to the RESTWS module as
    // of v2.12.
