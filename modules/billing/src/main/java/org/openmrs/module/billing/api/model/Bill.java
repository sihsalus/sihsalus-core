...
public class Bill extends BaseOpenmrsData {
  ...
  /**
   * Returns every non-voided discount on this bill (bill-level and line-item scoped). Voided rows
   * are excluded — for the full audit history, query {@code
   * BillDiscountService.getDiscountsByBillId} (or the equivalent REST search at {@code
   * /billDiscount?bill=<uuid>}).
   */
  ...
  /**
   * Bill total net of every approved, non-voided discount. Pending and rejected discounts are
   * visible on the bill but do not affect the total or the status flip.
   */
   ...
   *     Callers (notably {@link #synchronizeBillStatus()}) should refuse to auto-flip PAID in this
   *     state so the drift surfaces for manual reconciliation.
   */
  ...
  public BillLineItem addLineItem(
      StockItem item, BigDecimal price, String priceName, int quantity) {
    if (item == null) {
  ...
  public void synchronizeBillStatus() {
    ...
    if (current == BillStatus.REFUND_REQUESTED
        || current == BillStatus.REFUNDED
