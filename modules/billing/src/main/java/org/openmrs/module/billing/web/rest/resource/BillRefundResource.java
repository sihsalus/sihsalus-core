...
@Resource(
    name = RestConstants.VERSION_1 + CashierResourceController.BILLING_NAMESPACE + "/billRefund",
    supportedClass = BillRefund.class,
...
public class BillRefundResource extends DataDelegatingCrudResource<BillRefund> {
  ...
  @Override
  protected void delete(BillRefund delegate, String reason, RequestContext context)
  ...
  protected AlreadyPaged<BillRefund> doSearch(RequestContext context) {
    ...
    List<BillRefund> results =
        Context.getService(BillRefundService.class).getRefundsByBillId(bill.getId());
