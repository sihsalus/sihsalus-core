...
public interface BillDAO {
   ...
   *
   * <p>Note: This method may return voided bills. Consider filtering voided records at the service
   * layer if needed.
   *
   ...
   *
   * <p>Results are ordered by date created descending (most recent first). Note: This method may
   * return voided bills. Consider filtering voided records at the service layer if needed.
   *
   ...
   *
   * <p>Results are ordered by date created descending (most recent first). By default, voided bills
   * are excluded from results unless {@link BillSearch#setIncludeVoided(Boolean)} is set to true.
   * The search criteria support filtering by patient, cashier, cash point, and status.
   *
   ...
   *
   * <p>If the bill has no ID, it will be created as a new record. If it has an ID, the existing
   ...
   *
   * <p><strong>Warning:</strong> This operation cannot be undone. All associated data (line items,
