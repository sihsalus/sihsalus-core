...
public class HibernateBillableServiceDAOImpl implements BillableServiceDAO {
  ...
  public BillableService getBillableServiceByUuid(@Nonnull String uuid) {
    TypedQuery<BillableService> query =
        sessionFactory
            .getCurrentSession()
            .createQuery(
                "select b from BillableService b where b.uuid = :uuid", BillableService.class);
  ...
  private List<Predicate> buildBillServicesSearchPredicate(
      CriteriaBuilder cb, Root<BillableService> root, BillableServiceSearch billableServiceSearch) {
    ...
    if (StringUtils.isNotEmpty(billableServiceSearch.getServiceCategoryUuid())) {
      predicates.add(
          cb.equal(
              root.get("serviceCategory").get("uuid"),
              billableServiceSearch.getServiceCategoryUuid()));
    }

    if (StringUtils.isNotEmpty(billableServiceSearch.getServiceTypeUuid())) {
      predicates.add(
          cb.equal(
              root.get("serviceType").get("uuid"), billableServiceSearch.getServiceTypeUuid()));
    }

    if (StringUtils.isNotEmpty(billableServiceSearch.getConceptUuid())) {
      predicates.add(
          cb.equal(root.get("concept").get("uuid"), billableServiceSearch.getConceptUuid()));
    }
    ...
    if (StringUtils.isNotEmpty(billableServiceSearch.getName())) {
      predicates.add(
          cb.like(
              ...
              "%" + billableServiceSearch.getName().toLowerCase() + "%"));
    }
