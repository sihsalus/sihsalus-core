...
public class TestOrderBillingStrategy extends AbstractDefaultOrderBillingStrategy {
  ...
  protected Optional<BillLineItem> createBillLineItem(Order order) {
    ...
    List<BillableService> searchResult =
        billableServiceService.getBillableServices(searchTemplate, null);
    ...
    BillStatus lineItemStatus =
        checkIfOrderIsExempted(order, ExemptionType.SERVICE)
            ? BillStatus.EXEMPTED
            : BillStatus.PENDING;
