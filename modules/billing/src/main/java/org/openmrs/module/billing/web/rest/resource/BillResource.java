...
@Resource(
    name = RestConstants.VERSION_1 + CashierResourceController.BILLING_NAMESPACE + "/bill",
...
public class BillResource extends DataDelegatingCrudResource<Bill> {
  ...
  public Bill save(Bill bill) {
    ...
    if (bill.getId() == null) {
      ...
      // Now that all attributes have been set (i.e., payments and bill status) we can check to see
      // if the bill
      // is fully paid.
  ...
  private void loadBillCashPoint(Bill bill) {
    ...
    if (timesheet == null) {
      ...
      if (timesheetRequired) {
        throw new RestClientException(
            "A current timesheet does not exist for cashier " + bill.getCashier());
