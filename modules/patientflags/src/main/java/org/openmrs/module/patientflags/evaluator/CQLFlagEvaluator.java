  ...
  public Boolean eval(Flag flag, Patient patient, Map<Object, Object> context) {
    if (patient.getVoided())
      throw new APIException(
          "Unable to evaluate CQL flag " + flag.getName() + " against voided patient");
  ...
  public FlagValidationResult validate(Flag flag) {
    if (StringUtils.isBlank(flag.getCriteria())) {
      return new FlagValidationResult(
          false, "Criteria should be the ID of an existing plan definition. e.g ANCDT17");
    }

    // return new FlagValidationResult(false, flag.getCriteria() + " should be the ID of an existing
    // plan definition. e.g ANCDT17");
