...
public class GroovyFlagEvaluatorThread implements Runnable {
  ...
  private static Binding getBindings() {
    try {
      ...
      if (ModuleFactory.getStartedModulesMap().containsKey("mdrtb")) {
        try {
          Class<?> mdrtbServiceClass =
              Context.loadClass("org.openmrs.module.mdrtb.service.MdrtbService");
  ...
  private void setPrivileges() {
    ...
    if (privileges != null) {
      ...
      // if a user is specified, further restrict the privileges allowed to the intersection of the
      // cache privileges
