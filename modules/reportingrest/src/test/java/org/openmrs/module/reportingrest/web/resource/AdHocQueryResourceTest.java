...
public class AdHocQueryResourceTest extends BaseModuleWebContextSensitiveTest {
  ...
  @Autowired TestDataManager testData;
  ...
  public void testRowFilterQuery() throws Exception {
    ...
    AdHocRowFilterResults result =
        (AdHocRowFilterResults)
            resource.create(jackson.readValue(json, SimpleObject.class), requestContext);
    ...
    assertThat(
        result.getResult(),
        hasExactlyIds(2, 6, 7)); // male OR encounter during period OR adult on date
  ...
  public void testRowQueryWithSomeParametersSetAndSomeGlobal() throws Exception {
    ...
    // someone other than patient 7 else needs to have an encounter of this type, outside the time
    // window, so we
    // know parameters are really being applied
    testData
        .encounter()
        .encounterDatetime("2001-01-01")
  ...
  private String adHocDataExportAsJson(ObjectMapper jackson) throws IOException {
    ...
    addRowFilter(
        rowFilters,
        CohortDefinition.class.getName(),
        BuiltInCohortDefinitionLibrary.PREFIX + "anyEncounterDuringPeriod");
    ObjectNode q =
        addRowFilter(
            rowFilters,
            CohortDefinition.class.getName(),
  ...
  // copied from ReportingMatchers in the reporting module to avoid needing to add the test-jar
  // dependency
  private static Matcher<IdSet<?>> hasExactlyIds(final Integer... expectedMemberIds) {
    return new BaseMatcher<IdSet<?>>() {
      ...
      public boolean matches(Object o) {
        ...
        return (actual.size() == expectedMemberIds.length)
            && containsInAnyOrder(expectedMemberIds).matches(actual);
      }
      ...
      public void describeTo(Description description) {
        description.appendValue(
            "IdSet with "
                ...
                + " members: "
                + OpenmrsUtil.join(Arrays.asList(expectedMemberIds), ", "));
      }
    };
  }
