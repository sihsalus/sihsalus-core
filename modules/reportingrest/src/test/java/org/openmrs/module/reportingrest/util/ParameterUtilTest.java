...
public class ParameterUtilTest extends BaseModuleWebContextSensitiveTest {
  ...
  @Test
  public void convertParameterValues_shouldLookUpOpenmrsObjectsByUuidWhenGivenString()
      throws Exception {
    ...
    Map<String, Object> converted = ParameterUtil.convertParameterValues(params, submitted);
    assertThat(
        (EncounterType) converted.get("encounterType"),
        is(encounterService.getEncounterTypeByUuid(UUID)));
    assertThat(
        ((EncounterType) converted.get("encounterType")).getId(),
        is(encounterService.getEncounterTypeByUuid(UUID).getId()));
  }
  ...
  public void convertParameterValues_shouldLookUpOpenmrsObjectsByUuidFromObject() throws Exception {
    ...
    Map fromJson =
        new ObjectMapper()
            .readValue("{\"display\": \"Emergency\", \"uuid\":\"" + UUID + "\" }", Map.class);
    Map<String, Object> submitted = new HashMap<String, Object>();
  ...
  public void convertParameterValues_shouldLookUpOpenmrsObjectsInArrayByUuid() throws Exception {
    ...
    List<Parameter> params = new ArrayList<Parameter>();
    params.add(
        new Parameter("encounterTypes", "Encounter type", EncounterType.class, List.class, null));

    List fromJson =
        new ObjectMapper()
            .readValue("[ {\"display\": \"Emergency\", \"uuid\":\"" + UUID + "\" } ]", List.class);
    Map<String, Object> submitted = new HashMap<String, Object>();
    ...
    assertThat(
        encounterTypes.get(0).getId(), is(encounterService.getEncounterTypeByUuid(UUID).getId()));
  }
}
