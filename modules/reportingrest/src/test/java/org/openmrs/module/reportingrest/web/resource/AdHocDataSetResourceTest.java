...
public class AdHocDataSetResourceTest extends BaseModuleWebContextSensitiveTest {
  ...
  public void testCreate() throws Exception {
    ...
    AdHocDataSet created =
        (AdHocDataSet) resource.create(jackson.readValue(json, SimpleObject.class), requestContext);
    ...
    assertThat(
        created.getRowFilters().get(0).getKey(),
        is(BuiltInCohortDefinitionLibrary.PREFIX + "males"));
    assertThat(created.getRowFilters().get(0).getParameterValues().size(), is(0));
    assertThat(
        created.getRowFilters().get(1).getKey(),
        is(BuiltInCohortDefinitionLibrary.PREFIX + "anyEncounterOfTypesDuringPeriod"));
    assertThat(
        (List<String>) created.getRowFilters().get(1).getParameterValues().get("encounterTypes"),
        containsInAnyOrder(repWithUuid("61ae96f4-6afe-4351-b6f8-cd4fc383cce1")));
    assertThat(
        created.getRowFilters().get(2).getKey(),
        is(BuiltInCohortDefinitionLibrary.PREFIX + "atLeastAgeOnDate"));
    assertThat((Integer) created.getRowFilters().get(2).getParameterValues().get("minAge"), is(15));
    assertThat(
        (String) created.getRowFilters().get(2).getParameterValues().get("effectiveDate"),
        startsWith("2014-01-01T00:00:00.000"));

    assertThat(created.getColumns().size(), is(2));
    assertThat(
        created.getColumns().get(0).getKey(),
        is(BuiltInPatientDataLibrary.PREFIX + "preferredName.givenName"));
    ...
    assertThat(
        created.getColumns().get(1).getKey(),
        is(BuiltInPatientDataLibrary.PREFIX + "preferredName.familyName"));
  ...
  private String adHocDataExportAsJson(ObjectMapper jackson) throws IOException {
    ...
    addRowFilter(
        rowFilters,
        CohortDefinition.class.getName(),
        BuiltInCohortDefinitionLibrary.PREFIX + "males");
    ObjectNode q =
        addRowFilter(
            rowFilters,
            CohortDefinition.class.getName(),
            BuiltInCohortDefinitionLibrary.PREFIX + "anyEncounterOfTypesDuringPeriod");
    parameterValues(q).putArray("encounterTypes").add("61ae96f4-6afe-4351-b6f8-cd4fc383cce1");
    //        parameterValues(q).putArray("encounterTypes").addObject().put("uuid",
    // "61ae96f4-6afe-4351-b6f8-cd4fc383cce1"); // does not work yet

    q =
        addRowFilter(
            rowFilters,
            CohortDefinition.class.getName(),
            BuiltInCohortDefinitionLibrary.PREFIX + "atLeastAgeOnDate");
    ...
    addColumn(
        columns,
        PatientDataDefinition.class.getName(),
        "Given Name",
        BuiltInPatientDataLibrary.PREFIX + "preferredName.givenName");
    addColumn(
        columns,
        PatientDataDefinition.class.getName(),
        "Family Name",
        BuiltInPatientDataLibrary.PREFIX + "preferredName.familyName");
  ...
  public void testPurge() throws Exception {
    ...
    AdHocDataSetResource resource = new AdHocDataSetResource();
    AdHocDataSet created =
        (AdHocDataSet) resource.create(jackson.readValue(json, SimpleObject.class), createContext);
