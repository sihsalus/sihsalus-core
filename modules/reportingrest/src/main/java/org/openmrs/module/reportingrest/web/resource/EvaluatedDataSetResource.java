...
@Resource(
    name = RestConstants.VERSION_1 + "/reportingrest/dataSet",
    supportedClass = DataSet.class,
...
public class EvaluatedDataSetResource extends EvaluatedResource<DataSet> {
  ...
  public Object retrieve(String uuid, RequestContext requestContext) throws ResponseException {
    ...
    DataSetDefinitionService dataSetDefinitionService =
        DefinitionContext.getDataSetDefinitionService();
    DataSetDefinition definition =
        getDefinitionByUniqueId(dataSetDefinitionService, DataSetDefinition.class, uuid);
    if (definition == null) {
      throw new ObjectNotFoundException();
    }

    EvaluationContext evalContext =
        getEvaluationContextWithParameters(definition, requestContext, null, null);
  ...
  /**
   * Allow POSTing to an existing dsd, in case the user needs to specify more complex parameter
   * values than can be done in a GET.
   *
   * @param uniqueId
   * @param postBody
   ...
   */
  @Override
  public Object update(String uniqueId, SimpleObject postBody, RequestContext requestContext)
      throws ResponseException {
    ReportingRestPrivileges.requireViewReports();
    DataSetDefinitionService dataSetDefinitionService =
        DefinitionContext.getDataSetDefinitionService();
    DataSetDefinition definition =
        getDefinitionByUniqueId(dataSetDefinitionService, DataSetDefinition.class, uniqueId);
    if (definition == null) {
      throw new ObjectNotFoundException();
    }

    EvaluationContext evalContext =
        getEvaluationContextWithParameters(definition, requestContext, null, postBody);
    evaluateAndSetBaseCohort(requestContext, evalContext);
  ...
  /**
   * If there is a "cohort" parameter in the request, use that to look for a CohortDefinition to run
   * against, otherwise we'll use all patients
   *
   * @param requestContext
   * @param evalContext
   */
  private void evaluateAndSetBaseCohort(
      RequestContext requestContext, EvaluationContext evalContext) {
    ...
    if (StringUtils.hasLength(cohortUniqueId)) {
      try {
        EvaluatedCohort cohort =
            new EvaluatedCohortResource()
                .getEvaluatedCohort(cohortUniqueId, requestContext, "cohort.");
  ...
  /**
   * We let the user POST the serialized XML version of a Definition to this resource in order to
   * evaluate a non-saved definition on the fly.
   *
   * @param postBody
   * @param context
   * @return
    ...
    try {
      ...
      definition =
          Context.getSerializationService()
              .getSerializer(ReportingSerializer.class)
              .deserialize(xml, DataSetDefinition.class);
    } catch (Exception ex) {
      throw new IllegalArgumentException("Invalid submitted data set definition", ex);
    }
    EvaluationContext evalContext =
        getEvaluationContextWithParameters(definition, context, null, postBody);
    try {
      Evaluated<DataSetDefinition> evaluated =
          evaluate(definition, DefinitionContext.getDataSetDefinitionService(), evalContext);
  ...
  public List<Map<String, Object>> getRowsOfDataSetDefinition(DataSet dataSet) {
    ...
    while (iterator.hasNext()) {
      ...
      for (Map.Entry<DataSetColumn, Object> rowEntry : row.getColumnValues().entrySet()) {
        ...
        // If the value we return has any pointers to hibernate proxies, conversion to JSON will
        // fail when we
        // try to return it to the client. If we pass through an indicator result with an
        // EvaluationContext,
        // its cache will likely contain hibernate proxies an break things. So we just return the
        // numeric value,
        // and not the pointers to how we evaluated things. (Plus I don't think we really should be
        // sending mor
        ...
        if (value instanceof IndicatorResult) {
          ...
          value =
              new SimpleObject()
                  ...
                  .add("memberIds", idSet.getMemberIds());
