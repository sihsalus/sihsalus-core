...
public class AdHocDataSet {
  ...
  @JsonProperty private String customRowFilterCombination;

  @JsonProperty private List<AdHocParameter> parameters = new ArrayList<AdHocParameter>();

  @JsonProperty private List<AdHocRowFilter> rowFilters = new ArrayList<AdHocRowFilter>();

  @JsonProperty private List<AdHocColumn> columns = new ArrayList<AdHocColumn>();
  ...
  public AdHocDataSet(RowPerObjectDataSetDefinition definition) {
    ...
    if (definition instanceof PatientDataSetDefinition) {
      ...
      if (filters.size() == 1
          && filters.get(0).getParameterizable() instanceof CompositionCohortDefinition) {
        // this composition will have the individual searches we want represented as "1", "2", etc
        CompositionCohortDefinition ccd =
            (CompositionCohortDefinition) filters.get(0).getParameterizable();

        // ccd.searches is a HashMap with Strings like "1", "2", etc as keys
        // we transform this to be sorted by the parsed integer values
        SortedMap<Integer, Mapped<CohortDefinition>> sortedSearches =
            new TreeMap<Integer, Mapped<CohortDefinition>>();
  ...
  public RowPerObjectDataSetDefinition toDataSetDefinition(
      AdHocExportManager adHocExportManager, AllDefinitionLibraries definitionLibraries)
      throws Exception {
      ...
      if (dsd instanceof PatientDataSetDefinition) {
        if (StringUtils.isNotBlank(customRowFilterCombination)) {
          ...
          for (AdHocRowFilter filter : rowFilters) {
            ...
            // Mapped.straightThroughMappings(cohortDefinition);
  ...
  public String getCustomRowFilterCombination() {
    return customRowFilterCombination;
  }

  public void setCustomRowFilterCombination(String customRowFilterCombination) {
    this.customRowFilterCombination = customRowFilterCombination;
  }
  ...
  /**
   * Top-level parameter values are submitted from the client as a property on the parameter itself,
   * but reporting tends to expect a free-standing "Map<String, Object> parameterValues". This
   * convenience method will get that for you
   *
