...
public abstract class EvaluatedResource<T extends Evaluated> extends DelegatingCrudResource<T>
   ...
   *
   * @see
   *     org.openmrs.module.webservices.rest.web.resource.impl.BaseDelegatingResource#getUniqueId(java.lang.Object)
   */
  @Override
  ...
  @Override
  protected void delete(T delegate, String reason, RequestContext context)
  ...
  /**
   * Fetches all parameters needed for definition from the request. First looks at request
   * parameters, and looks at postBody next
   *
   * @param definition
   ...
   */
  protected EvaluationContext getEvaluationContextWithParameters(
    ...
    for (Parameter param : definition.getParameters()) {
      String paramName =
          StringUtils.hasText(parameterPrefix)
              ? parameterPrefix + param.getName()
              : param.getName();
            ...
            if (posted != null) {
              if (posted instanceof Collection<?> postedCollection) {
                ...
                throw new IllegalArgumentException(
                    "Parameter " + paramName + " in POST body should be an array");
              }
            }
          }
        }
  ...
  protected <Def extends Definition> Def getDefinitionByUniqueId(
      DefinitionService<Def> svc, Class<Def> clazz, String uniqueId) {
    AllDefinitionLibraries definitionLibraries =
        Context.getRegisteredComponents(AllDefinitionLibraries.class).get(0);
  ...
  protected <Def extends Definition> Evaluated<Def> evaluate(
      Def definition, DefinitionService<Def> svc, EvaluationContext ctx)
