...
@Resource(
    name = RestConstants.VERSION_1 + ReportingRestController.REPORTING_REST_NAMESPACE + "/cohort",
    supportedClass = EvaluatedCohort.class,
...
public class EvaluatedCohortResource extends EvaluatedResource<EvaluatedCohort> {
  ...
  /**
   * We let the user POST to an existing cohort definition in order to specify more complex
   * parameters than they could do in a GET
   *
   * @param uniqueId
   * @param postBody
   * @param context
   ...
   */
  @Override
  public Object update(String uniqueId, SimpleObject postBody, RequestContext context)
      throws ResponseException {
    ReportingRestPrivileges.requireViewReports();
    CohortDefinitionService definitionService = DefinitionContext.getCohortDefinitionService();
    CohortDefinition definition =
        getDefinitionByUniqueId(definitionService, CohortDefinition.class, uniqueId);
    if (definition == null) {
    ...
    try {
      Evaluated<CohortDefinition> evaluatedCohort =
          evaluate(definition, DefinitionContext.getCohortDefinitionService(), evalContext);
  ...
  /**
   * We let the user POST the serialized XML version of a CohortDefinition to this resource in order
   * to evaluate a non-saved cohort definition on the fly.
   *
   * @param postBody
   * @param context
   * @return
    ...
    try {
      ...
      definition =
          Context.getSerializationService()
              ...
              .deserialize(xml, CohortDefinition.class);
    } catch (Exception ex) {
  ...
  /**
   * Helper method used by this and other resources that can have base cohorts (like
   * EvaluatedDataSetResource)
   *
   ...
   * @return
   */
  protected EvaluatedCohort getEvaluatedCohort(
      String uuid, RequestContext requestContext, String parameterPrefix)
      throws EvaluationException {
    CohortDefinitionService definitionService = DefinitionContext.getCohortDefinitionService();
    CohortDefinition definition =
        getDefinitionByUniqueId(definitionService, CohortDefinition.class, uuid);
    if (definition == null) {
      throw new ObjectNotFoundException();
    }
    ...
    return (EvaluatedCohort)
        evaluate(
            definition,
            definitionService,
            getEvaluationContextWithParameters(definition, requestContext, parameterPrefix, null));
  }
   ...
   * REST). To avoid a performance penalty for large cohorts, we only query the id and uuid fields,
   * which are enough for an anemic REF representation of the patient, with a blank 'display'
   * property.
   */
  @PropertyGetter("members")
  public List<Patient> getMembers(EvaluatedCohort evaluatedCohort) {
    // it is impractical to write a test case that verifies this produces an efficient hibernate
    // query, but I manually
