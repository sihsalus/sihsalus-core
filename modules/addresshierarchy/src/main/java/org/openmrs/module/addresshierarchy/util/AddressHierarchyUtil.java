...
public class AddressHierarchyUtil {
  ...
  public static final Boolean getGlobalPropertyAsBoolean(String globalPropertyName) {
    ...
    String globalPropertyValue =
        Context.getAdministrationService().getGlobalProperty(globalPropertyName, "true");
    ...
    throw new AddressHierarchyModuleException(
        "Global property " + globalPropertyName + " must be set to either 'true' or 'false'.");
  }

  /**
   * Given a person address object, this method uses reflection fetch the value of the specified
   * field
   */
  public static final String getAddressFieldValue(PersonAddress address, AddressField field) {
    try {
      Method getter =
          PersonAddress.class.getMethod(
              "get" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1));
      return (String) getter.invoke(address);

    } catch (Exception e) {
      throw new AddressHierarchyModuleException(
          "Unable to get address field " + field.getName() + " off of PersonAddress", e);
    }
  }

  /**
   * Given a person address object, this method uses reflection to set the value of the specified
   * field
   */
  public static final void setAddressFieldValue(
      PersonAddress address, AddressField field, String value) {
    try {
      Method setter =
          PersonAddress.class.getMethod(
      ...
      throw new AddressHierarchyModuleException(
          "Unable to set address field " + field.getName() + " on PersonAddress", e);
    }
  }

  /**
   * Converts a map of address field name to address value pairs to an actualy person address Auto
   * generated method comment
   *
   * @param addressMap
   * @return
   */
  public static final PersonAddress convertAddressMapToPersonAddress(
      Map<String, String> addressMap) {
    PersonAddress address = new PersonAddress();

    for (String addressFieldName : addressMap.keySet()) {
      setAddressFieldValue(
          address, AddressField.getByName(addressFieldName), addressMap.get(addressFieldName));
    }
  ...
  /**
   * Tests whether the first addresses hierarchy entry is the descendant of the second--i.e., can
   * you reach the second entry by travelling up the tree from the first hierarchy entry
   */
  public static final boolean isDescendantOf(
      AddressHierarchyEntry descendant, AddressHierarchyEntry ancestor) {
    // handle null case
  ...
  /**
   * Given a Java Date, this method finds all the non-voided Patients that have a date changed after
   * the specified date, and the updates the AddressToEntryMaps for all PersonAddresses associated
   * with this patient
   *
   * <p>(Pulled this out the AddressHierarchyService so that the entire operation doesn't happen as
  ...
  /**
   * (This method is copied from org.apache.commons.lang3.StringUtils -- unfortunately, the lang3
   * library is only included in OpenMRS 1.9; I didn't want to import the class into address
   * hierarchy module directly for fear of having classloading issues when running on OpenMRS 1.9;
   * once Address Hierarchy switches to only support OpenMRS 1.9+, we can import lang3 and delete
   * this method)
   *
   * <p>Removes diacritics (~= accents) from a string. The case will not be altered.
   *
