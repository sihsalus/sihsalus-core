...
public class AddressHierarchyImportUtil {
  ...
  // we want to save in batches to improve performance, but if try to save ALL at once we can run
  // into memory issues
   ...
   */
  public static final void importAddressHierarchyFile(
      InputStream stream,
      String delimiter,
      String userGeneratedIdDelimiter,
      AddressHierarchyLevel startingLevel) {
    ...
    Map<AddressHierarchyEntry, Map<String, AddressHierarchyEntry>> entryCache =
        new HashMap<AddressHierarchyEntry, Map<String, AddressHierarchyEntry>>();
    ...
    // if we aren't starting at the top level of the hierarchy, remove all the levels before the one
    // we wish to start at
    ...
    try (BufferedReader reader =
        ...
        if (StringUtils.isNotBlank(line)) {
          ...
          if (locations != null) {
            ...
            for (int i = 0; i < locations.length; i++) {
              ...
              String[] entryNameAndIdPair =
                  splitIntoNameAndUserGeneratedId(
                      StringUtils.trim(locations[i]), userGeneratedIdDelimiter);
              ...
              // if it is not in the cache, see if it is in the database if there are existing
              // entries
              else if (hasExistingEntries) {
                entry =
                    ahService.getChildAddressHierarchyEntryByName(parent, entryNameAndIdPair[0]);
      ...
      throw new AddressHierarchyModuleException(
          "Error accessing address hierarchy import stream ", e);
    }
  ...
  public static final void importAddressHierarchyFile(
      InputStream stream, String delimiter, String userGeneratedIdDelimiter) {
  ...
  /** Utility methods */
  private static final String[] splitIntoNameAndUserGeneratedId(
      String location, String userGeneratedIdDelimiter) {
  ...
  private static final void addToCache(
      Map<AddressHierarchyEntry, Map<String, AddressHierarchyEntry>> entryCache,
