...
public class AddressHierarchyServiceImpl implements AddressHierarchyService {
  ...
  @Autowired private I18nCache i18nCache;

  public void setI18nCache(I18nCache i18nCache) {
  ...
  public List<String> getPossibleAddressValues(PersonAddress address, String fieldName) {
    ...
    if (field == null) {
      throw new AddressHierarchyModuleException(
          fieldName + " is not the name of a valid address field");
    }
  ...
  public List<String> getPossibleAddressValues(Map<String, String> addressMap, String fieldName) {
    return getPossibleAddressValues(
        AddressHierarchyUtil.convertAddressMapToPersonAddress(addressMap), fieldName);
  }
  ...
  public List<String> getPossibleAddressValues(PersonAddress address, AddressField field) {
    ...
    // iterate through the ordered levels until we reach the level associated with the specified
    // fieldName
    ...
    // calls getPossibleAddressHierarchyEntries(PersonAddress, AddressHierarchLevel) to perform the
    // actual search
    ...
    // take the entries returns and convert them into a list of *unique* names (using
    ...
    for (AddressHierarchyEntry entry : entries) {
      ...
      if (!possibleAddressValues.containsKey(entry.getLocalizedName().toLowerCase())) {
        // if not, add the key/value pair for this entry name, where the value equals the entry
        // name,
  ...
  public List<AddressHierarchyEntry> getPossibleAddressHierarchyEntries(
      PersonAddress address, AddressHierarchyLevel targetLevel) {
    ...
    if (!reachedFieldLevel) {
      // if we haven't found an address hierarchy level associated with this field, then we
      // certainly aren't going to be
      // able to find a list of possible values
      // return an empty set here, not null, because null is the default method in core if not
      // overridden
    ...
    if (StringUtils.isNotBlank(topLevelValue)) {
      AddressHierarchyEntry entry =
          getChildAddressHierarchyEntryByName(null, getL10nMessage(topLevelValue));
      if (entry != null) {
    ...
    for (AddressHierarchyLevel level : higherLevels) {
      List<AddressHierarchyEntry> possibleEntriesAtNextLevel =
          new ArrayList<AddressHierarchyEntry>();

      // find the value of the address field at the level we are dealing with
      String levelValue =
          level.getAddressField() != null
      ...
      for (AddressHierarchyEntry entry : possibleEntries) {
        ...
        if (StringUtils.isNotBlank(levelValue)) {
          AddressHierarchyEntry childEntry =
              getChildAddressHierarchyEntryByName(entry, getL10nMessage(levelValue));
          if (childEntry != null) {
    ...
    // now we need to handle the any person address field values that may have been specified
    // *below* the field
    ...
    // if we haven't found any possible lower level entries, then we can just return the
    // possibleEntries we calculated by higher level
  ...
  private void generatePossibleFullAddressesHelper(
      AddressHierarchyEntry entry, List<String> addresses) {

    List<AddressHierarchyEntry> entries = getChildAddressHierarchyEntries(entry);

    // if this is leaf node, then create the full address and add it to the list of addresses to
    // return
    if (entries == null || entries.isEmpty()) {
      ...
      // follow back up the tree to the top level and concatenate the names to create the full
      // address string
  ...
  public Set<String> searchAddresses(String searchString, AddressHierarchyLevel level) {
    ...
    // (more specifically, this pattern matches sets of 1 or more characters that are both non-word
    // (\W) and non-whitespace (\S))
    searchString =
        AddressHierarchyConstants.PATTERN_NON_WORD_AND_NON_WHITESPACE
    ...
    // now go through and remove from the results list any addresses that don't contain the other
    ...
    // note that we create the results using a List so that we can order them, but then copy back
    // into a LinkedHashSet so we preserve order but remove dups
    return results;
  }

  // utility method used by searchAddress(String, AddressHierarchyLevel)
  private Pattern generateSearchPattern(
      Method encodeStringMethod, String searchString, String phoneticProcessor) {
    ...
    // otherwise encode the string to create the pattern
      ...
      // note that we want to do only whole entry phonetic matching, so we prepend and append the
      // string with a pattern that matches either
  ...
  @Transactional(readOnly = true)
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevel(
   ...
   */
  protected List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndCodes(
      AddressHierarchyLevel level, List<String> codes) {
    if (level == null) {
      return null;
    }
    ...
    for (String code : codes) {
      List<AddressHierarchyEntry> entries =
          dao.getAddressHierarchyEntriesByLevelAndName(level, code);
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndNameAndParent(
    ...
    return dao.getAddressHierarchyEntriesByLevelAndNameAndParent(
        level, i18nCache.getMessageKey(name), parent);
  }
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndLikeNameAndParent(
    ...
    if (!i18nCache.isEnabled()) {
      matchedEntries.addAll(
          dao.getAddressHierarchyEntriesByLevelAndLikeNameAndParent(level, searchString, parent));
    } else {
      for (String code : i18nCache.getMessageKeysByLikeName(searchString)) {
        matchedEntries.addAll(
            dao.getAddressHierarchyEntriesByLevelAndNameAndParent(level, code, parent));
      }
    }
  ...
  public AddressHierarchyEntry getChildAddressHierarchyEntryByName(
    ...
    if (entry != null) {
      ...
      List<AddressHierarchyEntry> entries =
          dao.getAddressHierarchyEntriesByLevelAndName(
              getTopAddressHierarchyLevel(), i18nCache.getMessageKey(childName));
      if (entries.size() == 0) {
        return null;
        ...
        throw new AddressHierarchyModuleException(
            "Two or more top-level entries have the same name");
      }
    }
  }
  ...
  @Transactional(readOnly = true)
  public synchronized void initializeFullAddressCache() {

    // generally, this global property should be set to true; it just allows cache load to be
    // disabled to speed startup
    if (Context.getAdministrationService()
        .getGlobalProperty(
            AddressHierarchyConstants.GLOBAL_PROP_INITIALIZE_ADDRESS_HIERARCHY_CACHE_ON_STARTUP)
  ...
  private synchronized Map<String, List<String>> getAddressesForLocale(Locale locale) {
    ...
    if (ret == null) {
      ...
      for (AddressHierarchyEntry entry :
          getAddressHierarchyEntriesByLevel(getTopAddressHierarchyLevel())) {
  ...
  private void initializeFullAddressCacheHelper(
      Locale locale,
      AddressHierarchyEntry entry,
      String phoneticProcessor,
    ...
    if (entries == null || entries.isEmpty()) {
      ...
      while (tempEntry.getParent() != null) {
        ...
        key.insert(
            0,
            encodeString(encodeStringMethod, tempEntry.getLocalizedName(), phoneticProcessor)
                + "|");
    ...
    // if not a leaf node, process it's children recursively
      ...
      for (AddressHierarchyEntry currentEntry : entries) {
        initializeFullAddressCacheHelper(
            locale, currentEntry, phoneticProcessor, encodeStringMethod);
      }
    }
  }
  ...
  private String fetchPhoneticProcessor() {
    ...
    // only relevant if the name phonetics code and service were imported into the static monolith
    Class<?> namePhoneticsServiceClass =
        loadOptionalStaticClass("org.openmrs.module.namephonetics.NamePhoneticsService");
    if (namePhoneticsServiceClass != null) {
      phoneticProcessor =
      ...
      if (StringUtils.isNotBlank(phoneticProcessor)) {
        ...
        // call the name phonetics service by reflection and see if the processor has been
        // registered
        try {
          Object namePhonetics = Context.getService(namePhoneticsServiceClass);
          Method getProcessorClassName =
              namePhoneticsServiceClass.getMethod("getProcessorClassName", String.class);
          ...
          if (getProcessorClassName.invoke(namePhonetics, phoneticProcessor) == null) {
            log.error(
                "No soundex processor found with name "
                    + phoneticProcessor
  ...
  private Method fetchEncodeStringMethod() {
    ...
    Class<?> namePhoneticsUtilClass =
        loadOptionalStaticClass("org.openmrs.module.namephonetics.NamePhoneticsUtil");
    if (namePhoneticsUtilClass != null) {
      try {
        encodeStringMethod =
  ...
  private Class<?> loadOptionalStaticClass(String className) {
    try {
  ...
  private String encodeString(
      Method encodeStringMethod, String stringToEncode, String phoneticProcessor) {

    // first strip off any accent marks
    stringToEncode = AddressHierarchyUtil.stripAccents(stringToEncode);

    // if the phonetics processor hasn't been set, or the string to encode is blank, just return
    // original string
    if (StringUtils.isBlank(stringToEncode)
        || StringUtils.isBlank(phoneticProcessor)
        || encodeStringMethod == null) {
    ...
    for (String word : words) {
      ...
      if (AddressHierarchyConstants.PATTERN_ANY_DIGIT.matcher(word).find()) {
        ...
        try {
          codedString.append(
              (String) encodeStringMethod.invoke(null, word, phoneticProcessor) + " ");
        } catch (Exception e) {
  ...
  private List<AddressToEntryMap> createAddressToEntryMapsForPersonAddress(PersonAddress address) {
    ...
    // iterate through all the address hierarchy levels, and see if we can find a match at each
    // level
    ...
    for (AddressHierarchyLevel level : getOrderedAddressHierarchyLevels(false, false)) {
      ...
      entries =
          getAddressHierarchyEntriesByLevelAndName(
              level, AddressHierarchyUtil.getAddressFieldValue(address, level.getAddressField()));
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndLikeName(
    ...
    if (!i18nCache.isEnabled()) {
      matchedEntries.addAll(
          dao.getAddressHierarchyEntriesByLevelAndLikeName(level, searchString, limit));
    } else {
      for (String code : i18nCache.getMessageKeysByLikeName(searchString)) {
  ...
  protected void initI18nCache(
      List<AddressHierarchyLevel> orderedLevels, I18nCache i18nCache, Locale locale) {
    ListIterator<AddressHierarchyLevel> iterator = orderedLevels.listIterator(orderedLevels.size());
    // Iterate in reverse.
    while (iterator.hasPrevious()) {
      for (AddressHierarchyEntry entry : getAddressHierarchyEntriesByLevel(iterator.previous())) {
        i18nCache.getMessage(
            entry.getName(),
            locale); // this fills the cache down from the entry level up to the top
      }
    }
  }
  ...
  public void initI18nCache() {
    if (i18nCache.isEnabled()) {
      ...
      i18nCache.setOrderedAddressFields(orderedAddressFields);
      for (Locale locale : Context.getService(AdministrationService.class).getAllowedLocales()) {
        initI18nCache(orderedLevels, i18nCache, locale);
      }
    }
  }
