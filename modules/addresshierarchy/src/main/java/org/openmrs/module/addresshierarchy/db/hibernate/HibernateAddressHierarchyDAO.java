...
public class HibernateAddressHierarchyDAO implements AddressHierarchyDAO {
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevel(
      AddressHierarchyLevel addressHierarchyLevel) {
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndName(
    ...
    Query<AddressHierarchyEntry> query =
        getCurrentSession()
            .createQuery(
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndNameAndParent(
    ...
    Query<AddressHierarchyEntry> query =
        getCurrentSession()
            .createQuery(
                "from AddressHierarchyEntry entry where entry.level.levelId = :levelId and "
                    + parentPredicate("entry.parent", parent)
                    + " and "
                    + namePredicate("entry.name"),
                AddressHierarchyEntry.class)
            .setParameter("levelId", addressHierarchyLevel.getId());
    ...
    Query<AddressHierarchyEntry> query =
        getCurrentSession()
            .createQuery(
                "from AddressHierarchyEntry entry where entry.level.levelId = :levelId and "
                    + parentPredicate("entry.parent", parent)
                    + " and lower(entry.name) like :name",
                AddressHierarchyEntry.class)
            .setParameter("levelId", addressHierarchyLevel.getId())
  ...
  public AddressHierarchyEntry getChildAddressHierarchyEntryByName(
    ...
    Query<AddressHierarchyEntry> query =
        getCurrentSession()
            .createQuery(
                "from AddressHierarchyEntry entry where "
                    + parentPredicate("entry.parent", entry)
                    + " and "
                    + namePredicate("entry.name"),
                AddressHierarchyEntry.class);
    setParentParameter(query, entry);
  ...
  public AddressHierarchyLevel getTopAddressHierarchyLevel() {
    try {
      ...
      throw new AddressHierarchyModuleException(
          "Unable to fetch top level address hierarchy type", e);
    }
  }
  ...
  public AddressHierarchyLevel getAddressHierarchyLevelByParent(AddressHierarchyLevel parent) {
    try {
      Query<AddressHierarchyLevel> query =
          getCurrentSession()
              .createQuery(
  ...
  public AddressHierarchyEntry getAddressHierarchyEntryByUuid(String uuid) {
    return firstOrNull(
        getCurrentSession()
            .createQuery(
                "from AddressHierarchyEntry entry where entry.uuid = :uuid",
                AddressHierarchyEntry.class)
  ...
  private String namePredicate(String property) {
    return "((:nameIsNull = true and "
        + property
        + " is null) or (:nameIsNull = false and lower("
        + property
        + ") = :name))";
  }
