...
public interface AddressHierarchyService {
   ...
   * specified addressField. (Excluding duplicate names and ignoring any current value of the
   * specified addressField)
   *
   * <p>This method can handle restrictions based on address field values not only above but also
   * *below* the specified level. (For instance, if the city is set to "Boston", and we ask for
   * possible values for the "state" level, only Massachusetts should be returned)
   *
   * @param address the address we are testing against
   * @param fieldName name of the address field to look up possible values for
   * @return a list of the names of the possible valid address values for the specified field;
   *     returns an empty list if no matches, should return null only if error
   */
  public List<String> getPossibleAddressValues(PersonAddress address, String fieldName);

  /**
   * Given a map of address fields to address field values, returns the names of all entries that
   * are hierarchically valid for the specified addressField. (Excluding duplicate names and
   * ignoring any current value of the specified addressField)
   *
   * <p>This method can handle restrictions based on address field values not only above but also
   * *below* the specified level. (For instance, if the city is set to "Boston", and we ask for
   * possible values for the "state" level, only Massachusetts should be returned)
   ...
   * @param address
   * @param field
   * @return a list of the names of the possible valid address hierarchy entries; returns an empty
   *     list if no matches, should return null only if error
   */
  public List<String> getPossibleAddressValues(PersonAddress address, AddressField field);

  /**
   * Given a person address, returns all the address hierarchy entries that are hierarchically valid
   * for the specified level. (Ignoring any current value of the addressField associated with the
   * specified level).
   *
   * <p>This method can handle restrictions based on address field values not only above but also
   * *below* the specified level. (For instance, if the city is set to "Boston", and we ask for
   ...
   * @param address
   * @param level
   * @return a list of possible valid address hierarchy entries; returns an empty list if no
   *     matches, should return null only if error
   */
  public List<AddressHierarchyEntry> getPossibleAddressHierarchyEntries(
      PersonAddress address, AddressHierarchyLevel level);

  /**
   * Given an AddressHierarchyEntry, returns all the "full addresses" that contain that entry,
   * represented as a pipe-delimited string of address hierarchy entry names, ordered from the entry
   * at the highest level to the entry at the lowest level in the tree. For example, the full
   * address for the Beacon Hill neighborhood in the city of Boston might be: "United
   * States|Massachusetts|Suffolk County|Boston|Beacon Hill"
   *
   * @param entry
  ...
  /**
   * Given a search string, returns all the "full addresses" that match that search string Returns a
   * list of full addresses, represented as a pipe-delimited string of address hierarchy entry
   * names, ordered from the entry at the highest level to the entry at the lowest level in the
   * tree. For example, the full address for the Beacon Hill neighborhood in the city of Boston
   * might be: "United States|Massachusetts|Suffolk County|Boston|Beacon Hill"
   *
   * <p>Note that if the Name Phonetics module has been installed, and the global property
   * addresshierarchy.soundexProcessor has been set to the name of a recognized soundex processor,
   * this method will perform a soundex search as opposed to a straight word search
   *
  ...
  /**
   * Given a search string, returns the name of all the entries for the specified level that match
   * that search string
   *
   * <p>Note that if the Name Phonetics module has been installed, and the global property
   * addresshierarchy.soundexProcessor has been set to the name of a recognized soundex processor,
   * this method will perform a soundex search as opposed to a straight word search
   *
   * <p>If no level is specified, this method operates as if a searchAddress(String) call
   *
   * @param searchString the search string to search for
  ...
  /**
   * Returns a count of the total number of address hierarchy entries associated with the given
   * level
   *
   * @param level
  ...
  /**
   * Returns all address hierarchy entries at the given level that have the specified name (name
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndName(
      AddressHierarchyLevel level, String name);

  /**
   * Returns all address hierarchy entries at the given level that have both the specified name and
   * the specified parent (name match is case-insensitive)
   *
   * @param level
   * @param name
   * @param parent
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndNameAndParent(
      AddressHierarchyLevel level, String name, AddressHierarchyEntry parent);

  /**
   * Returns all address hierarchy entries at the given level who have the specified parent and the
   * specified search string in their name (name match is case-insensitive)
   *
   * @param level
   * @param searchString
   * @param parent
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndLikeNameAndParent(
      AddressHierarchyLevel level, String searchString, AddressHierarchyEntry parent);
  ...
  /**
   * Returns all address hierarchy entries that are children of the specified entry (If no entry
   * specified, returns all the entries at the top level)
   *
   * @param entry
  ...
  /**
   * Returns the address hierarchy entry that is the child entry of the specified entry and have the
   * specified name (case-insensitive) (If no entry specified, tests against all entries at the top
   * level) (Throws an exception if there is only one match, because there should be no two entries
   ...
   */
  public AddressHierarchyEntry getChildAddressHierarchyEntryByName(
      AddressHierarchyEntry entry, String childName);
  ...
  /**
   * Saves a block of address hierarchy entries within a single transaction (This should be used
   * with care since the save interceptors may not be used properly. This method is mainly used to
   * speed up the performance of importing a hierarchy from a file)
   *
   * @param entries
   */
  @Authorized({AddressHierarchyConstants.PRIV_MANAGE_ADDRESS_HIERARCHY})
   ...
   *
   * @param includeUnmapped specifies whether or not to include hierarchy levels that aren't mapped
   *     to an underlying address field
   * @return the ordered list of address hierarchy levels
   */
   ...
   * @param includeUnmapped specifies whether or not to include hierarchy levels that aren't mapped
   *     to an underlying address field
   * @param includeEmptyLevels specified whether or not include hierarchy levels that don't have any
   *     address hierarchy entries entries
   * @return the ordered list of address hierarchy levels
   */
  public List<AddressHierarchyLevel> getOrderedAddressHierarchyLevels(
  ...
  /**
   * Attempt to determine the hierarchy of address hierarchy levels based on the hierarchy of
   * entries and assign the parent levels accordingly (used as part of 1.2 to 1.6 migration)
   */
  public void setAddressHierarchyLevelParents();

  /**
   * Builds a key/value map of pipe-delimited strings that represents all the possible full
   * addresses, and stores this in a local cache for use by the searchAddresses(String) method
   *
   * <p>The map values are full addresses represented as a pipe-delimited string of address
   * hierarchy entry names, ordered from the entry at the highest level to the entry at the lowest
   * level in the tree. For example, the full address for the Beacon Hill neighborhood in the city
   * of Boston might be: "United States|Massachusetts|Suffolk County|Boston|Beacon Hill"
   *
   * <p>In the standard implementation, the keys are the same as the values. However, if the Name
   * Phonetics module has been installed, and the addresshierarchy.soundexProcessor global property
   * has been configured, the keys will be the same pipe-delimited string, but with each entry name
   * transformed via the specified soundex processor
   *
   * <p>The searchAddresses method compares the input string against the keys, and returns the
   * values of any matches
   *
   * <p>Need to make sure we synchronize to avoid having multiple threads trying to initialize it at
   * the same time, or one using it before it is initialized (Note that the one thing this won't
   * prevent against is it being re-initialized while another thread is accessing it)
   *
   * <p>NOTE: this method ONLY initializes the full address cache if it has not yet been
   * initialized-- if you want to RE-initialized the full address cache, first call
   * resetFullAddressCache and then this method
   */
  ...
  /**
   * Given a PersonAddress, updates all the AddressToEntryMaps for that PersonAddress
   *
   * @param address
   */
  ...
  /**
   * Given a person, updates all the AddressToEntryMaps for all non-voided PersonAddresses
  ...
  public List<AddressHierarchyEntry> getAddressHierarchyEntriesByLevelAndLikeName(
      AddressHierarchyLevel level, String searchString, int limit);
