...
public class AutocompleteWidget extends CodedWidget {
  ...
  public void render(WidgetConfig config, Writer w) throws IOException {
    ...
    HtmlUtil.renderResource(
        w, config.getRequest(), "/moduleResources/htmlwidgets/jquery.autocomplete.css");
    HtmlUtil.renderResource(
        w, config.getRequest(), "/moduleResources/htmlwidgets/jquery.autocomplete.min.js");
    String id = config.getId();

    w.write(
        "<script type=\"text/javascript\" charset=\"utf-8\">"
            + "		function setupAutocomplete_"
            + id
            + "(element) {"
    ...
    HtmlUtil.renderSimpleTag(
        ...
        "id="
            ...
            + "TextField|size="
            + size
            + "|type=text|class=autoCompleteText|onfocus=setupAutocomplete_"
            + id
            + "(this);"
            + labelAtt);
