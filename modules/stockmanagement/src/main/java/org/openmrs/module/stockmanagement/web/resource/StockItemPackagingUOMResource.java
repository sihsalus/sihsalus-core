...
@Resource(
    name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/stockitempackaginguom",
    supportedClass = StockItemPackagingUOMDTO.class,
...
public class StockItemPackagingUOMResource extends ResourceBase<StockItemPackagingUOMDTO> {
  ...
  public StockItemPackagingUOMDTO getByUniqueId(String uniqueId) {
    ...
    Result<StockItemPackagingUOMDTO> result =
        getStockManagementService().findStockItemPackagingUOMs(filter);
    return result.getData().isEmpty() ? null : result.getData().get(0);
  }

  @Override
  protected void delete(StockItemPackagingUOMDTO delegate, String reason, RequestContext context)
      throws ResponseException {
    getStockManagementService()
        .voidStockItemPackagingUOM(
  ...
  public StockItemPackagingUOMDTO save(StockItemPackagingUOMDTO delegate) {
    StockItemPackagingUOM stockItemPackagingUOM =
        getStockManagementService().saveStockItemPackagingUOM(delegate);
    return getByUniqueId(stockItemPackagingUOM.getUuid());
  }

  @Override
  public void purge(StockItemPackagingUOMDTO delegate, RequestContext context)
      throws ResponseException {
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation || rep instanceof FullRepresentation) {
      modelImpl
          ...
          .property("factor", new DecimalProperty())
