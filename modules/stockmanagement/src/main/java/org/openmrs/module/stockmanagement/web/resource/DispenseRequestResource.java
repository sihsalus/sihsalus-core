...
@Resource(
    name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/dispenserequest",
    supportedClass = DispenseRequest.class,
...
public class DispenseRequestResource extends ResourceBase<DispenseRequest> {
  ...
  @Override
  protected void delete(DispenseRequest delegate, String reason, RequestContext context)
  ...
  public Object create(SimpleObject propertiesToCreate, RequestContext context)
    ...
    String DISPENSE_CUSTOM_REP =
        "(dispenseLocation,patient,order,encounter,stockItem,quantity,stockItemPackagingUOM)";

    if (propertiesToCreate.containsKey("dispenseItems")
        && !propertiesToCreate.get("dispenseItems").equals(null)
        && !propertiesToCreate.get("dispenseItems").equals("")) {
  ...
  public DispenseRequest covertToDispenseRequest(Map dispenseItem) {
    ...
    if (dispenseItem.get("patient") != null) {
      patient =
          Context.getPatientService().getPatientByUuid(dispenseItem.get("patient").toString());
    }

    if (dispenseItem.get("encounter") != null) {
      encounter =
          Context.getEncounterService()
              .getEncounterByUuid(dispenseItem.get("encounter").toString());
    }
    ...
    if ((dispenseItem.get("stockItem") != null
            && dispenseItem.get("stockBatch") != null
            && dispenseItem.get("stockItemPackagingUOM") != null)
        && dispenseItem.get("dispenseLocation") != null) {
      ...
      dispenseRequest.setStockItemPackagingUOMUuid(
          dispenseItem.get("stockItemPackagingUOM").toString());
    } else {
      throw new IllegalArgumentException(
          "stockItem,stockBatch,and stockItemPackagingUOM,dispensingLocation can not be null");
    }
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation || rep instanceof FullRepresentation) {
      modelImpl
          .property("dispenseLocation", new StringProperty())
          .property("patient", new StringProperty())
          .property("order", new StringProperty())
          .property("encounter", new StringProperty())
          .property("stockItem", new StringProperty())
          .property("stockBatch", new StringProperty())
