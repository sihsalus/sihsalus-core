...
@Resource(
    name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/stockiteminventory",
    supportedClass = StockItemInventory.class,
...
public class StockItemInventoryResource extends ResourceBase<StockItemInventory> {
  ...
  @Override
  protected void delete(StockItemInventory delegate, String reason, RequestContext context)
  ...
  protected PageableResult doSearch(RequestContext context) {
    ...
    if (!StringUtils.isBlank(param)) {
      ...
      if (drugId != null || conceptId != null) {
        ...
        Result<StockItemDTO> stockItems =
            getStockManagementService().findStockItems(stockItemSearchFilter);
        if (stockItems.getData().isEmpty()) {
          return emptyResult(context);
        }
        stockItemIds =
            stockItems.getData().stream().map(p -> p.getId()).collect(Collectors.toList());
      } else {
        ...
        if (!StringUtils.isBlank(param)) {
          ...
          stockItemIds =
              items.getData().stream().map(p -> p.getStockItemId()).collect(Collectors.toList());
        ...
        if (!StringUtils.isBlank(param)) {
          ...
          List<PartyDTO> partyList =
              getStockManagementService()
                  .getCompleteStockDispensingLocationPartyList(location.getLocationId());
          ...
          if (StringUtils.isNotBlank(dispenseAtLocation)
              && (dispenseAtLocation.equalsIgnoreCase("true")
                  || dispenseAtLocation.equalsIgnoreCase("1"))) {
          ...
          if (includeDefaultMainPharmacy) {
            ...
            if (!mainPharmacies.isEmpty()) {
              partyIds.addAll(
                  mainPharmacies.stream().map(p -> p.getId()).collect(Collectors.toList()));
            }
          }
          ...
          isDispensing =
              Context.getUserContext()
                  .hasPrivilege(Privileges.TASK_STOCKMANAGEMENT_STOCKITEMS_DISPENSE_QTY);
          if (isDispensing) {
    ...
    if (!StringUtils.isBlank(param)) {
      ...
      Result<StockBatchDTO> stockBatches =
          getStockManagementService().findStockBatches(stockBatchSearchFilter);
    ...
    if (!StringUtils.isBlank(param)) {
      StockItemInventorySearchFilter.InventoryGroupBy groupBy =
          (StockItemInventorySearchFilter.InventoryGroupBy)
              Enum.valueOf(StockItemInventorySearchFilter.InventoryGroupBy.class, param);
    ...
    if (stockItemId != null) {
      if (partyIds == null) {
        StockItemInventorySearchFilter.ItemGroupFilter itemGroupFilter =
            new StockItemInventorySearchFilter.ItemGroupFilter(
                stockItemId, partyUuid != null ? Arrays.asList(partyUuid) : null, stockBatchId);
        itemsToSearch.add(itemGroupFilter);
      } else {
        StockItemInventorySearchFilter.ItemGroupFilter itemGroupFilter =
            new StockItemInventorySearchFilter.ItemGroupFilter(partyIds, stockItemId, stockBatchId);
        itemsToSearch.add(itemGroupFilter);
      }
      ...
      for (Integer stockItemId0 : stockItemIds) {
        if (partyIds == null) {
          StockItemInventorySearchFilter.ItemGroupFilter itemGroupFilter =
              new StockItemInventorySearchFilter.ItemGroupFilter(
                  stockItemId0, partyUuid != null ? Arrays.asList(partyUuid) : null, stockBatchId);
          itemsToSearch.add(itemGroupFilter);
        } else {
          StockItemInventorySearchFilter.ItemGroupFilter itemGroupFilter =
              new StockItemInventorySearchFilter.ItemGroupFilter(
                  partyIds, stockItemId0, stockBatchId);
          itemsToSearch.add(itemGroupFilter);
        }
      }
    }
    ...
    StockInventoryResult stockItemInventoryResult =
        getStockManagementService().getStockInventory(filter);
    return new StockInventoryPageableResult(
        context,
        stockItemInventoryResult,
        stockItemInventoryResult.hasMoreResults(),
        stockItemInventoryResult.getTotalRecordCount());
  }
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation || rep instanceof FullRepresentation) {
      modelImpl
          ...
          .property("drugId", new StringProperty())
          .property("drugUuid", new StringProperty())
          .property("drugStrength", new StringProperty())
          .property("conceptId", new StringProperty())
          ...
          .property("quantityUoM", new StringProperty())
          .property("quantityFactor", new DecimalProperty())
          .property("quantityUoMUuid", new StringProperty())
