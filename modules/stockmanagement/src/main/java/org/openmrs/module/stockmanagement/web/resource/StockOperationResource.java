...
@Resource(
    name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/stockoperation",
    supportedClass = StockOperationDTO.class,
...
public class StockOperationResource extends ResourceBase<StockOperationDTO> {
  ...
  public StockOperationDTO getByUniqueId(String uniqueId) {
    ...
    if (stockOperationDTO != null
        && StockOperationType.STOCK_ISSUE.equals(stockOperationDTO.getOperationType())) {
      Result<StockOperationLinkDTO> parents =
          getStockManagementService().getParentStockOperationLinks(stockOperationDTO.getUuid());
      if (parents.getData() != null && parents.getData().size() > 0) {
        stockOperationDTO.setRequisitionStockOperationUuid(
            parents.getData().get(0).getParentUuid());
      }
    }
  ...
  @Override
  protected void delete(StockOperationDTO delegate, String reason, RequestContext context)
  ...
  protected PageableResult doSearch(RequestContext context) {
    ...
    if (StringUtils.isNotBlank(param)) filter.setSearchText(param);
    ...
    if (StringUtils.isNotBlank(param)) {
      ...
      for (String typeUuid : params) {
        StockOperationType operationType =
            stockManagementService.getStockOperationTypeByUuid(typeUuid);
    ...
    if (StringUtils.isNotBlank(param)) {
      ...
      for (String status : params) {
        StockOperationStatus opStatus =
            (StockOperationStatus) Enum.valueOf(StockOperationStatus.class, status);
    ...
    if (!result.getData().isEmpty() && context.getRepresentation() == Representation.FULL) {
      ...
      itemSearchFilter.setStockOperationUuids(
          result.getData().stream().map(p -> p.getUuid()).collect(Collectors.toList()));
      Result<StockOperationItemDTO> items =
          getStockManagementService().findStockOperationItems(itemSearchFilter);
  ...
  public void setStockOperationItems(StockOperationDTO instance, ArrayList<Map<String, ?>> items) {
    ...
    if (StringUtils.isBlank(instance.getUuid())) {
      ...
      existingStockOperationItems =
          getStockManagementService().findStockOperationItems(itemSearchFilter).getData();
    }
    ...
    for (Map<String, ?> item : items) {
      ...
      if (item.containsKey("uuid")
        ...
        Optional<StockOperationItemDTO> existingItem =
            existingStockOperationItems.stream().filter(p -> uuid.equals(p.getUuid())).findFirst();
        if (existingItem.isPresent()) {
          isNew = false;
          itemDTO = existingItem.get();
        } else throw new RestClientException(String.format("Item %s not found", uuid));

      } else {
        itemDTO = new StockOperationItemDTO();
      }

      DelegatingResourceDescription propertiesToApply =
          isNew ? creatableProperties : modifiableProperties;
      for (Map.Entry<String, DelegatingResourceDescription.Property> prop :
          propertiesToApply.getProperties().entrySet()) {
        if (item.containsKey(prop.getKey())
            && !RestConstants.PROPERTY_FOR_TYPE.equals(prop.getKey())) {
  ...
  public SimpleObject getPermission(StockOperationDTO stockOperationDTO) {
    ...
    boolean canUpdateBatchInformation =
        stockOperationDTO.canUpdateBatchInformation(stockOperationType);
    Boolean userHasEditPermissions = null;

    if (canEdit || canApprove) {
      HashSet<PrivilegeScope> privilegeScopes =
          getStockManagementService()
              .getPrivilegeScopes(
                  Context.getAuthenticatedUser(),
                  Context.getLocationService()
                      .getLocationByUuid(stockOperationDTO.getAtLocationUuid()),
                  getStockManagementService()
                      .getStockOperationTypeByUuid(stockOperationDTO.getOperationTypeUuid()),
                  Arrays.asList(
            ...
            canEdit
                && privilegeScopes.stream()
                    .anyMatch(
                        p ->
                            p.getPrivilege()
                                .equals(Privileges.TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE));
      } else {
          ...
          canApprove
              && privilegeScopes.stream()
                  .anyMatch(
                      p ->
                          p.getPrivilege()
                              .equals(Privileges.TASK_STOCKMANAGEMENT_STOCKOPERATIONS_APPROVE));
    }
    ...
    if (canReceiveItems || isRequisitionAndCanIssueStock) {
      if (StringUtils.isBlank(stockOperationDTO.getDestinationUuid())) {
        ...
        Party party =
            getStockManagementService().getPartyByUuid(stockOperationDTO.getDestinationUuid());
                      ...
                      .anyMatch(
                          p ->
                              p.getPrivilege()
                                  .equals(
                                      Privileges
                                          .TASK_STOCKMANAGEMENT_STOCKOPERATIONS_RECEIVEITEMS));
        }
  ...
  public Collection<StockOperationItemDTO> getStockOperationItems(
      StockOperationDTO stockOperationDTO) {
    ...
    filter.setStockItemUuids(
        items.getData().stream()
            .map(p -> p.getStockItemUuid())
            .distinct()
            .collect(Collectors.toList()));
    List<StockItemPackagingUOMDTO> packagingUnits =
        getStockManagementService().findStockItemPackagingUOMs(filter).getData();
    ...
    if (permissions != null) {
      if (permissions.containsKey("canUpdateBatchInformation")) {
        ...
        if (canUpdateBatchInformation) {
          stockBatchHasTransactions =
              getStockManagementService()
                  .checkStockBatchHasTransactionsAfterOperation(
                      stockOperationDTO.getId(),
                      items.getData().stream()
                          .filter(p -> p.getStockBatchId() != null)
                          .map(p -> p.getStockBatchId())
                          .distinct()
                          .collect(Collectors.toList()));
        }
      }
    ...
    for (StockOperationItemDTO itemDTO : items.getData()) {
      List<StockItemPackagingUOMDTO> units =
          packagingUnits.stream()
              .filter(p -> p.getStockItemUuid().equals(itemDTO.getStockItemUuid()))
              .collect(Collectors.toList());
      if (!units.isEmpty()) {
        itemDTO.setPackagingUnits(units);
      }
      if (canUpdateBatchInformation) {
        itemDTO.setCanUpdateBatchInformation(
            stockBatchHasTransactions != null
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation || rep instanceof FullRepresentation) {
      modelImpl
          ...
          .property("cancelReason", new StringProperty())
          ...
          .property("cancelledDate", new DateTimeProperty())
          .property("completedBy", new IntegerProperty())
          .property("completedByGivenName", new StringProperty())
          .property("completedByFamilyName", new StringProperty())
          .property("completedDate", new DateTimeProperty())
          .property("destinationUuid", new StringProperty())
          .property("destinationName", new StringProperty())
          .property("externalReference", new StringProperty())
          .property("atLocationUuid", new StringProperty())
          .property("atLocationName", new StringProperty())
          .property("operationDate", new DateTimeProperty())
          .property("locked", new BooleanProperty())
          .property("operationNumber", new StringProperty())
          .property("operationOrder", new IntegerProperty())
          .property("reasonUuid", new StringProperty())
          .property("reasonName", new StringProperty())
          .property("remarks", new StringProperty())
          .property("sourceUuid", new StringProperty())
          .property("sourceName", new StringProperty())
          .property("status", new StringProperty())
          .property("returnReason", new StringProperty())
          .property("rejectionReason", new StringProperty())
          ...
          .property("approvalRequired", new BooleanProperty())
          ...
          .property("returnedByFamilyName", new StringProperty())
          .property("returnedDate", new DateProperty())
          .property("rejectedByGivenName", new StringProperty())
          .property("rejectedByFamilyName", new StringProperty())
          .property("rejectedDate", new DateProperty());
    }
    if (rep instanceof DefaultRepresentation) {}
