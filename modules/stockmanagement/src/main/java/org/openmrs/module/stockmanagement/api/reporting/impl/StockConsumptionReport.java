...
public class StockConsumptionReport
    extends StockItemInventoryReport<StockItemInventoryConsumption> {
        ...
        stockInventoryResult
            ...
            .forEach(
                p -> {
                  ...
                  if (consumption.getQuantityConsumed() != null
                      && inventorySearchFilter.getStartDate() != null
                      && inventorySearchFilter.getEndDate() != null) {
                    long totalMiliseconds =
                        Math.abs(
                            inventorySearchFilter.getEndDate().getTime()
                                - inventorySearchFilter.getStartDate().getTime());
                    long dayCount =
                        Math.max(TimeUnit.DAYS.convert(totalMiliseconds, TimeUnit.MILLISECONDS), 1);
                    consumption.setConsumptionRate(
                        consumption
                            .getQuantityConsumed()
                            .divide(
