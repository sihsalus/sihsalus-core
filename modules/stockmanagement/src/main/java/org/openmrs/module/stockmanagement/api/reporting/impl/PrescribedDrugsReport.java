...
public class PrescribedDrugsReport extends ReportGenerator {
  ...
  public void generateReport(BatchJob batchJob, Function<BatchJob, Boolean> shouldStopExecution) {
    ...
    try {
      ...
      if (!StringUtils.isBlank(locationUuid)) {
        ...
        if (location == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Report location parameter not found");
          return;
        }
      ...
      if (!StringUtils.isBlank(stockItemUuid)) {
        ...
        if (stockItem == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Report stock item parameter not found");
          return;
        }
      ...
      if (!StringUtils.isBlank(stockItemCategoryUuid)) {
        ...
        if (concept == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Report stock item category parameter not found");
          return;
        }
      ...
      if (!StringUtils.isBlank(configuredConcept)) {
        ...
        if (concept == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Concept configured for Observation: drug concept not found");
          return;
        }
      ...
      while (hasMoreRecords) {
        ...
        Result<PrescriptionLineItem> data =
            stockManagementService.findPrescriptionLineItems(filter);
        if (shouldStopExecution.apply(batchJob)) {
          return;
        }
        ...
        java.util.List<PrescriptionLineItem> rows =
      ...
      stockManagementService.failBatchJob(
          batchJob.getUuid(), "Input/Output error: " + e.getMessage());
      log.error(e.getMessage(), e);
    } finally {
      if (csvWriter != null) {
        try {
  ...
  protected void writeRow(
      CSVWriter csvWriter, PrescriptionLineItem row, MessageSourceService messageSourceService) {
    ...
    columns[columnIndex++] =
        row.getDateActivated() != null ? TIMESTAMP_FORMATTER.format(row.getDateActivated()) : null;
    columns[columnIndex++] =
        row.getDateStopped() != null ? TIMESTAMP_FORMATTER.format(row.getDateStopped()) : null;
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          row.getDateDispensed() != null
              ? TIMESTAMP_FORMATTER.format(row.getDateDispensed())
              : null;
    }
    columns[columnIndex++] =
        String.format("%1$s %2$s", row.getOrdererFamilyName(), row.getOrdererGivenName());
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          row.getDispenserFamilyName() != null
              ? String.format(
                  "%1$s %2$s", row.getDispenserFamilyName(), row.getDispenserGivenName())
              : null;
    }
    columns[columnIndex++] = row.getPatientIdentifier();
    columns[columnIndex++] =
        row.getStockItemDrugName() == null
            ? row.getStockItemConceptName()
            : row.getStockItemDrugName();
    columns[columnIndex++] =
        row.getStockItemDrugName() == null ? "" : row.getStockItemConceptName();
    ...
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          row.getQuantityDispensed() == null
              ? null
              : row.getQuantityDispensed().multiply(negativeOne).toPlainString();
      columns[columnIndex++] = row.getQuantityDispensedStockItemPackagingUOMName();
      columns[columnIndex++] =
          row.getQuantityDispensedStockItemPackagingUOMFactor() == null
              ? null
              : row.getQuantityDispensedStockItemPackagingUOMFactor().toPlainString();
      columns[columnIndex++] = row.getBatchNo();
      columns[columnIndex++] =
          row.getBatchExpiryDate() != null ? DATE_FORMATTER.format(row.getBatchExpiryDate()) : null;
    }
    columns[columnIndex++] = row.getDose() != null ? row.getDose().toPlainString() : null;
    columns[columnIndex++] = row.getDoseUnitsConceptName();
    columns[columnIndex++] =
        row.getFrequencyPerDay() != null ? row.getFrequencyPerDay().toPlainString() : null;
    ...
    columns[columnIndex++] =
        row.getDispenseAsWritten() == null
            ? null
            : (row.getDispenseAsWritten()
                ? messageSourceService.getMessage("stockmanagement.report.yes")
                : messageSourceService.getMessage("stockmanagement.report.no"));
    columns[columnIndex++] =
        row.getAsNeeded() == null
            ? null
            : (row.getAsNeeded()
                ? messageSourceService.getMessage("stockmanagement.report.yes")
                : messageSourceService.getMessage("stockmanagement.report.no"));
    ...
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          row.getStockItemTransactionId() == null
              ? null
  ...
  protected void writeHeaders(CSVWriter csvWriter) {
    ...
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.preferreddispensery");
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.dispensedfrom");
    }
    columns[columnIndex++] = messageSourceService.getMessage("stockmanagement.report.datecreated");
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.dateActivated");
    columns[columnIndex++] = messageSourceService.getMessage("stockmanagement.report.dateStopped");
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.datedispensed");
    }
    ...
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.quantityprescribed");
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.qtyunitprescribed");
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.packsizeprescribed");
    } else {
    ...
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.quantitydispensed");
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.qtyunitdispensed");
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.packsizedispensed");
    ...
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.frequencyperday");
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.frequencyunits");
    ...
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.durationunits");
    columns[columnIndex++] = messageSourceService.getMessage("stockmanagement.report.route");
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.dispenseaswritten");
    columns[columnIndex++] = messageSourceService.getMessage("stockmanagement.report.asNeeded");
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.asNeededCondition");
    columns[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.dosingInstructions");
    columns[columnIndex++] = messageSourceService.getMessage("stockmanagement.report.ordernumber");
    if (isFullfullmentReport()) {
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.prescriptiontxnid");
      columns[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.dispensetxnid");
    } else {
