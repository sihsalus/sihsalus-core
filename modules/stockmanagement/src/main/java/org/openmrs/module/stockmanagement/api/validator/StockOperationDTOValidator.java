...
@Handler(
    supports = {StockOperationDTO.class},
    order = 50)
  ...
  public void validate(Object target, Errors errors) {
    ...
    if (stockOperationType.getHasSource() != null && stockOperationType.getHasSource()) {
      ...
      if (stockOperationType.getSourceType() == LocationType.Location) {
        ...
        Set<StockOperationTypeLocationScope> locationScope =
            stockOperationType.getStockOperationTypeLocationScopes();
        if (locationScope != null && !locationScope.isEmpty()) {
          List<StockOperationTypeLocationScope> sourceScope =
              locationScope.stream().filter(p -> p.getIsSource()).collect(Collectors.toList());
          if (!sourceScope.isEmpty()) {
            Set<LocationTag> locationTags = party.getLocation().getTags();
            if (locationTags == null
                || locationTags.isEmpty()
                || !sourceScope.stream()
                    .anyMatch(
                        p ->
                            locationTags.stream()
              ...
              errors.rejectValue(
                  "sourceUuid",
                  messageSourceService.getMessage(
                      "stockmanagement.stockoperation.sourcelocationtagnotmatched"));
              return;
            }
          }
        }
        ...
        if (party.getStockSource() == null) {
          errors.rejectValue(
              "sourceUuid",
              messageSourceService.getMessage(
                  "stockmanagement.stockoperation.sourcenotstocksource"));
          return;
        }
      }
    }
    ...
    if (stockOperationType.getHasDestination() != null && stockOperationType.getHasDestination()) {
      ...
      if (party == null || party.getVoided()) {
        errors.rejectValue(
            "destinationUuid",
            messageSourceService.getMessage(
                "stockmanagement.stockoperation.destinationuuidinvalid"));
        return;
      }
      if (stockOperationType.getDestinationType() == LocationType.Location) {
        if (party.getLocation() == null) {
          errors.rejectValue(
              "destinationUuid",
              messageSourceService.getMessage(
                  "stockmanagement.stockoperation.destinationnotlocation"));
          return;
        }
        ...
        Set<StockOperationTypeLocationScope> locationScope =
            stockOperationType.getStockOperationTypeLocationScopes();
        if (locationScope != null && !locationScope.isEmpty()) {
          List<StockOperationTypeLocationScope> destinationScope =
              locationScope.stream().filter(p -> p.getIsDestination()).collect(Collectors.toList());
          if (!destinationScope.isEmpty()) {
            Set<LocationTag> locationTags = party.getLocation().getTags();
            if (locationTags == null
                || locationTags.isEmpty()
                || !destinationScope.stream()
                    .anyMatch(
                        p ->
                            locationTags.stream()
                                .anyMatch(x -> x.getName().equals(p.getLocationTag())))) {
              errors.rejectValue(
                  "destinationUuid",
                  messageSourceService.getMessage(
                      "stockmanagement.stockoperation.destinationlocationtagnotmatched"));
              return;
            }
          }
        }

        if (permissionLocation == null) permissionLocation = party.getLocation();
        ...
        if (party.getStockSource() == null) {
          errors.rejectValue(
              "destinationUuid",
              messageSourceService.getMessage(
                  "stockmanagement.stockoperation.destinationnotstockdestination"));
          return;
        }
      }
                      ...
                      .anyMatch(
                          x ->
                              p.getUuid().equals(x.getUuid())
                                  && p.getStockItemUuid().equals(x.getStockItemUuid())))) {
        errors.rejectValue(
            "stockOperationItems",
    ...
    if (!StringUtils.isBlank(object.getRemarks()) && object.getRemarks().length() > 255) {
      errors.rejectValue(
          "remarks", messageSourceService.getMessage("stockmanagement.stockoperation.remarks255"));
      return;
    }
    ...
    if (!StringUtils.isBlank(object.getResponsiblePersonOther())
      ...
      errors.rejectValue(
          ...
          messageSourceService.getMessage(
              "stockmanagement.stockoperation.responsiblePersonOther150"));
      return;
    }
    ...
    for (StockOperationItemDTO stockOperationItemDTO : requestedStockOperationItems) {
      ...
      if (stockOperationType.requiresBatchUuid()
        ...
        errors.rejectValue(
            "stockOperationItems",
            String.format(
                messageSourceService.getMessage("stockmanagement.stockoperation.batchuuidrequired"),
                index));
        return;
      }
      ...
      if (stockOperationType.requiresActualBatchInformation()) {
        if (StringUtils.isBlank(stockOperationItemDTO.getBatchNo())) {
          errors.rejectValue(
              "stockOperationItems",
              String.format(
                  messageSourceService.getMessage("stockmanagement.stockoperation.batchnorequired"),
                  index));
          return;
        }
        ...
        if (stockOperationItemDTO.getExpiration() != null
          ...
          errors.rejectValue(
              ...
              String.format(
                  messageSourceService.getMessage(
                      "stockmanagement.stockoperation.expirydateinpast"),
                  index));
          return;
        }
      ...
      if (!stockOperationType.isQuantityOptional() && stockOperationItemDTO.getQuantity() == null) {
        errors.rejectValue(
            "stockOperationItems",
            String.format(
                messageSourceService.getMessage("stockmanagement.stockoperation.qtyrequired"),
                index));
        return;
      }
        ...
        if (stockOperationItemDTO.getQuantityRequested() != null
          ...
          if (stockOperationItemDTO.getQuantityRequested() != null
            ...
            errors.rejectValue(
                ...
                String.format(
                    messageSourceService.getMessage(
                        "stockmanagement.stockoperation.qtyrequestedrequired"),
                    index));
            return;
          }
          ...
          if (stockOperationItemDTO.getQuantityRequestedPackagingUOMUuid() == null) {
            errors.rejectValue(
                ...
                String.format(
                    messageSourceService.getMessage(
                        "stockmanagement.stockoperation.qtyrequesteduomrequired"),
                    index));
            return;
          }
        }
