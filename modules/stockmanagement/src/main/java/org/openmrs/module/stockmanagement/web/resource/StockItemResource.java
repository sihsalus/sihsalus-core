...
@Resource(
    name = RestConstants.VERSION_1 + "/" + ModuleConstants.MODULE_ID + "/stockitem",
    supportedClass = StockItemDTO.class,
...
public class StockItemResource extends ResourceBase<StockItemDTO> {
  ...
  @Override
  protected void delete(StockItemDTO delegate, String reason, RequestContext context)
  ...
  private Boolean isDrugSearch(RequestContext context) {
    ...
    if (StringUtils.isBlank(isDrug)) return null;
  ...
  protected PageableResult searchDrugsAndConcepts(RequestContext context, String searchToken) {
    ...
    Integer maxIntermediateResult =
        GlobalProperties.getStockItemSearchMaxDrugConceptIntermediateResult();
    List<Integer> itemsFound =
        getStockManagementService()
            .searchStockItemCommonName(
                searchToken, filter.getIsDrug(), context.getIncludeAll(), maxIntermediateResult);
    ...
    if (searchConcepts) {
      ...
      if (searchResults.isEmpty()) searchConcepts = false;
      else filter.setConcepts(searchResults);
    }
    if (searchDrugs) {
      List<Drug> drugs =
          service.getDrugs(
              ...
              maxIntermediateResult
                  + (searchConcepts
                      ? Math.max(
                          0,
                          maxIntermediateResult
                              - (filter.getConcepts() != null ? filter.getConcepts().size() : 0))
                      : maxIntermediateResult));
      if (drugs.isEmpty()) searchDrugs = false;
      else filter.setDrugs(drugs);
    }
  ...
  public Collection<StockItemReferenceDTO> getStockItemReferences(StockItemDTO stockItemDTO) {
    ...
    for (StockItemReference stockItemReference :
        getStockManagementService().getStockItemReferenceByStockItem(stockItemDTO.getUuid())) {
  ...
  public SimpleObject getPermission(StockItemDTO stockItemDTO) {
    ...
    boolean canEdit =
        Context.getAuthenticatedUser()
            .hasPrivilege(Privileges.TASK_STOCKMANAGEMENT_STOCKITEMS_MUTATE);
