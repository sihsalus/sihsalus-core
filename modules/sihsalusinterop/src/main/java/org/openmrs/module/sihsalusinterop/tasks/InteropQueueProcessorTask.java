 ...
 *
 * <p>Esta tarea se ejecuta periódicamente para procesar automáticamente los mensajes con estado
 * PENDING en la cola y enviarlos al servidor FHIR (RENHICE).
 *
 * <p>Configuración en OpenMRS: - Intervalo recomendado: 300 segundos (5 minutos) - Se ejecuta
 * automáticamente cada X segundos
 *
    ...
    try {
      ...
      if (sentCount > 0) {
        log.info(
            "✅ SCHEDULER: "
                + sentCount
                + " mensaje(s) enviado(s) exitosamente de "
                + processedCount
                + " procesados");
      } else {
        log.info(
            "✓ SCHEDULER: Procesados: "
                + processedCount
                + ". No hay mensajes pendientes para enviar");
      }
