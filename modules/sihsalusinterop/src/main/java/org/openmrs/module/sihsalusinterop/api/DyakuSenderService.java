 ...
 *
 * <p>Gestiona el envío de mensajes FHIR (Dyaku profiles) al servidor HAPI FHIR simulado (RENHICE).
 * Implementa arquitectura Offline-First con cola de mensajes y reintentos automáticos.
 *
...
public interface DyakuSenderService extends OpenmrsService {
  ...
  /**
   * Procesa la cola de mensajes pendientes Envía todos los mensajes con status=PENDING al servidor
   * HAPI FHIR
   *
   ...
   *
   * <p>Este método realiza la conversión completa del paciente OpenMRS a FHIR R4 (perfil MINSA),
   * serializa a JSON y lo encola para envío asíncrono.
   *
   * @param patient Paciente de OpenMRS a enviar
   * @return El item de cola creado
   * @throws org.openmrs.module.sihsalusinterop.api.exception.InteropException Si el paciente no
   *     tiene DNI o hay error en la conversión
   */
