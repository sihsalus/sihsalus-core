 ...
 * invoke a DWR-exposed method. The annotation is the request-boundary counterpart to the {@code
 * <openmrs:require>} JSP tag and is enforced by {@link AuthorizationHandlerInterceptor} for Spring
 * controllers and by {@code DwrAuthorizationFilter} for DWR endpoints. Both enforcers reject
 * requests <em>before</em> any controller code, view, or service method runs - in particular,
 * before a Hibernate session loads any entity, which closes the dirty-entity exploitation pattern
 * that service-layer {@code @Authorized} alone cannot defend against.
 *
 * <p>Placement:
 *
 * <ul>
 *   <li><strong>{@code @Controller}/{@code @RequestMapping} controllers</strong> &mdash; annotate
 *       the handler method. Each method's annotation is independent.
...
public @interface RequirePrivilege {
  ...
  /**
   * The privileges to check. An empty array means no specific privilege is required, only that the
