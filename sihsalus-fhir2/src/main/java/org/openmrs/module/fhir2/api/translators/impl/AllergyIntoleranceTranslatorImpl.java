...
public class AllergyIntoleranceTranslatorImpl implements AllergyIntoleranceTranslator {
  ...
  public AllergyIntolerance toFhirResource(@Nonnull Allergy omrsAllergy) {
    ...
    if (omrsAllergy.getAllergen() != null) {
      allergy.addCategory(
          categoryTranslator.toFhirResource(omrsAllergy.getAllergen().getAllergenType()));
    }
    allergy.setClinicalStatus(setClinicalStatus(omrsAllergy.getVoided()));
    allergy.setVerificationStatus(
        new CodeableConcept()
            .setText("Confirmed")
            .addCoding(
                new Coding(
                    FhirConstants.ALLERGY_INTOLERANCE_VERIFICATION_STATUS_SYSTEM_URI,
                    "confirmed",
                    "Confirmed")));
    ...
    allergy.setCriticality(
        criticalityTranslator.toFhirResource(
            severityTranslator.toFhirResource(omrsAllergy.getSeverity())));
  ...
  public Allergy toOpenmrsType(@Nonnull Allergy allergy, @Nonnull AllergyIntolerance fhirAllergy) {
    ...
    if (fhirAllergy.hasCategory() && allergy.getAllergen() != null) {
      allergy
          .getAllergen()
          .setAllergenType(
              categoryTranslator.toOpenmrsType(fhirAllergy.getCategory().get(0).getValue()));
    }
    ...
    if (fhirAllergy.hasReaction()) {
      for (AllergyIntolerance.AllergyIntoleranceReactionComponent reaction :
          fhirAllergy.getReaction()) {
  ...
  private CodeableConcept setClinicalStatus(boolean voided) {
    ...
    if (voided) {
      ...
      status.addCoding(
          new Coding(
              FhirConstants.ALLERGY_INTOLERANCE_CLINICAL_STATUS_SYSTEM_URI,
              "inactive",
              "Inactive"));
    } else {
      ...
      status.addCoding(
          new Coding(
              FhirConstants.ALLERGY_INTOLERANCE_CLINICAL_STATUS_SYSTEM_URI, "active", "Active"));
    }
  ...
  private boolean isAllergyInactive(CodeableConcept status) {
    ...
    return status.getCoding().stream()
        .filter(
            c -> FhirConstants.ALLERGY_INTOLERANCE_CLINICAL_STATUS_SYSTEM_URI.equals(c.getSystem()))
