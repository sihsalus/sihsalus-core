...
public class TwoSearchQueryBundleProvider implements IBundleProvider {
  ...
  @Getter private final IPrimitiveType<Date> published;

  @Getter private final String uuid;

  public TwoSearchQueryBundleProvider(
      IBundleProvider firstProvider,
      IBundleProvider secondProvider,
  ...
  public List<IBaseResource> getResources(int fromIndex, int toIndex) {
    ...
    if (firstResult >= firstProviderSize) {
      return secondProvider.getResources(
          firstResult - firstProviderSize, lastResult - firstProviderSize);
    }
    ...
    List<IBaseResource> resourcesFromFirstProvider =
        firstProvider.getResources(firstResult, firstProviderSize);
    List<IBaseResource> requiredResourcesFromFirstProvider =
        resourcesFromFirstProvider.subList(0, numResultsFromFirstProvider);
    List<IBaseResource> includedResourcesFromFirstProvider =
        resourcesFromFirstProvider.subList(
            numResultsFromFirstProvider, resourcesFromFirstProvider.size());

    List<IBaseResource> resourcesFromSecondProvider =
        secondProvider.getResources(0, numResultsFromSecondProvider);
    List<IBaseResource> requiredResourcesFromSecondProvider =
        resourcesFromSecondProvider.subList(0, numResultsFromSecondProvider);
    List<IBaseResource> includedResourcesFromSecondProvider =
        resourcesFromSecondProvider.subList(
            numResultsFromSecondProvider, resourcesFromSecondProvider.size());
    ...
    if (pageSize == null) {
      pageSize =
          globalPropertyService.getGlobalPropertyAsInteger(
              FhirConstants.OPENMRS_FHIR_DEFAULT_PAGE_SIZE, 10);
    }
