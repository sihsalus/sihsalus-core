...
public class FhirMedicationDaoImpl extends BaseFhirDao<Drug> implements FhirMedicationDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Drug, U> criteriaContext,
  ...
  private <U> void handleIngredientCode(
      OpenmrsFhirCriteriaContext<Drug, U> criteriaContext, TokenAndListParam ingredientCode) {
    if (ingredientCode != null) {
      ...
      OpenmrsFhirCriteriaSubquery<Concept, Integer> conceptSubquery =
          criteriaContext.addSubquery(Concept.class, Integer.class);
      ...
      criteriaContext.addPredicate(
          criteriaContext
              .getCriteriaBuilder()
              .in(conceptJoin.get("conceptId"))
              .value(conceptSubquery.finalizeQuery()));
    }
  }

  private <U> void handleMedicationCode(
      OpenmrsFhirCriteriaContext<Drug, U> criteriaContext, TokenAndListParam code) {
    if (code != null) {
  ...
  private <U> void handleMedicationDosageForm(
      OpenmrsFhirCriteriaContext<Drug, U> criteriaContext, TokenAndListParam dosageForm) {
    if (dosageForm != null) {
