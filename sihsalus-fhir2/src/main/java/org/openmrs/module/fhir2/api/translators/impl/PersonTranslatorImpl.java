...
public class PersonTranslatorImpl implements PersonTranslator {
  ...
  public org.hl7.fhir.r4.model.Person toFhirResource(@Nonnull Person openmrsPerson) {
    ...
    if (openmrsPerson.getIsPatient()) {
      person.addLink(
          new org.hl7.fhir.r4.model.Person.PersonLinkComponent()
              .setTarget(
                  patientReferenceTranslator.toFhirResource(
                      patientDao.get(openmrsPerson.getUuid()))));
    }
  ...
  @Override
  public Person toOpenmrsType(
      @Nonnull Person openmrsPerson, @Nonnull org.hl7.fhir.r4.model.Person person) {
    ...
    person.getTelecom().stream()
        ...
        .forEach(openmrsPerson::addAttribute);

    List<Extension> personAttributeExtensions =
        person.getExtension().stream()
            .filter(
                extension ->
