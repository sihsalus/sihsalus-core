...
public class EncounterParticipantTranslatorImpl implements EncounterParticipantTranslator {
  ...
  private static final String DEFAULT_ENCOUNTER_ROLE_UUID_PROPERTY =
      "fhir2.encounterParticipantComponentUuid";
  ...
  @Override
  public Encounter.EncounterParticipantComponent toFhirResource(
    ...
    participantComponent.setIndividual(
        createPractitionerReference(encounterProvider.getProvider()));
    return participantComponent;
  }

  @Override
  public EncounterProvider toOpenmrsType(
      @Nonnull EncounterProvider encounterProvider,
    ...
    notNull(
        ...
        "The EncounterParticipantComponent object should not be null");

    getReferenceId(encounterParticipantComponent.getIndividual())
        .map(practitionerUuid -> practitionerDao.get(practitionerUuid))
        .ifPresent(encounterProvider::setProvider);
  ...
  protected EncounterRole getDefaultEncounterRole() {
    String defaultEncounterRoleUuid =
        globalPropertyService.getGlobalProperty(DEFAULT_ENCOUNTER_ROLE_UUID_PROPERTY);

    String encounterRoleUuid =
        (defaultEncounterRoleUuid != null && !defaultEncounterRoleUuid.isEmpty())
