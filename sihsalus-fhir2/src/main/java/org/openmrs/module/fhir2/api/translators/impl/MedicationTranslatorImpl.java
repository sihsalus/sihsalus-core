...
public class MedicationTranslatorImpl implements MedicationTranslator {
  ...
  public Drug toOpenmrsType(@Nonnull Drug existingDrug, @Nonnull Medication medication) {
    ...
    if (medication.hasIngredient()) {
      for (Medication.MedicationIngredientComponent ingredient : medication.getIngredient()) {
        ...
        omrsIngredient.setIngredient(
            conceptTranslator.toOpenmrsType(ingredient.getItemCodeableConcept()));
                    ...
                    .forEach(
                        e ->
                            addMedicineComponent(
                                existingDrug, e.getUrl(), ((StringType) e.getValue()).getValue())));
  ...
  public void addMedicineComponent(@Nonnull Drug drug, @Nonnull String url, @Nonnull String value) {
    if (value == null
        || url == null
        || !url.startsWith(FhirConstants.OPENMRS_FHIR_EXT_MEDICINE + "#")) {
      return;
    }
  ...
  private void addMedicineExtension(
      @Nonnull Medication medication,
      @Nonnull java.lang.String extensionProperty,
    ...
    getOpenmrsMedicineExtension(medication)
        ...
        .addExtension(
            FhirConstants.OPENMRS_FHIR_EXT_MEDICINE + "#" + extensionProperty,
            new StringType(value));
  }

  private Optional<Extension> getOpenmrsMedicineExtension(@Nonnull Medication medication) {
    return Optional.ofNullable(
        medication.getExtensionByUrl(FhirConstants.OPENMRS_FHIR_EXT_MEDICINE));
  }
}
