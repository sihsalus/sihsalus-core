...
@Component
public class FhirRelatedPersonDaoImpl extends BaseFhirDao<Relationship>
    implements FhirRelatedPersonDao {

  @Override
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Relationship, U> criteriaContext,
                      ...
                      .forEach(
                          param ->
                              getSearchQueryHelper()
                                  .handleNames(
                                      criteriaContext,
                                      (StringAndListParam) param.getParam(),
                                      null,
                                      null,
                                      personJoin));
                  break;
  ...
  private <U> Optional<Predicate> handleDateRange(
      OpenmrsFhirCriteriaContext<Relationship, U> criteriaContext,
      From<?, ?> personJoin,
    ...
    return Optional.ofNullable(
        criteriaContext
            ...
            .and(
                toCriteriaArray(
                    Stream.of(
                        handleDate(criteriaContext, personJoin, param.getLowerBound()),
                        handleDate(criteriaContext, personJoin, param.getUpperBound())))));
  }

  private <U> Optional<Predicate> handleDate(
      OpenmrsFhirCriteriaContext<Relationship, U> criteriaContext,
      From<?, ?> personJoin,
      DateParam dateParam) {
    if (dateParam == null) {
        ...
        return Optional.of(
            criteriaContext
                ...
                .and(
                    ...
                    criteriaContext
                        .getCriteriaBuilder()
                        .lessThan(personJoin.get("birthdate"), dateEnd)));
      case NOT_EQUAL:
        return Optional.of(
            criteriaContext
                .getCriteriaBuilder()
  ...
  @Override
  protected <T, U> Collection<Order> paramToProps(
      @Nonnull OpenmrsFhirCriteriaContext<T, U> criteriaContext, @Nonnull SortState<T> sortState) {
    ...
    if (param.equals(SP_NAME) || param.equals(SP_GIVEN) || param.equals(SP_FAMILY)) {
       ...
       *     )
       */
      OpenmrsFhirCriteriaSubquery<PersonName, Integer> personNameFirstSubquery =
          criteriaContext.addSubquery(PersonName.class);
      personNameFirstSubquery.addPredicate(
          cb.and(
              cb.equal(personNameFirstSubquery.getRoot().get("voided"), false),
              cb.equal(personNameFirstSubquery.getRoot().get("preferred"), true),
              cb.equal(
                  personNameFirstSubquery.getRoot().get("person"), criteriaContext.getRoot())));
      personNameFirstSubquery
          .getSubquery()
          .select(cb.min(personNameFirstSubquery.getRoot().get("personNameId")));
       ...
       *     )
       */
      OpenmrsFhirCriteriaSubquery<PersonName, Integer> personNameSecondSubquery =
          criteriaContext.addSubquery(PersonName.class);
      personNameSecondSubquery.addPredicate(
          cb.and(
              cb.equal(personNameSecondSubquery.getRoot().get("voided"), false),
              cb.equal(personNameSecondSubquery.getRoot().get("preferred"), true),
              cb.equal(
                  personNameSecondSubquery.getRoot().get("person"), criteriaContext.getRoot())));
      personNameSecondSubquery
          .getSubquery()
          .select(personNameSecondSubquery.getRoot().get("personNameId"));
       ...
       *     )
       */
      OpenmrsFhirCriteriaSubquery<PersonName, Integer> personNameThirdSubquery =
          criteriaContext.addSubquery(PersonName.class);
      personNameThirdSubquery.addPredicate(
          cb.and(
              cb.equal(personNameThirdSubquery.getRoot().get("voided"), false),
              cb.equal(
                  personNameThirdSubquery.getRoot().get("person"), criteriaContext.getRoot())));
      personNameThirdSubquery
          .getSubquery()
          .select(cb.min(personNameThirdSubquery.getRoot().get("personNameId")));
      ...
      Join<?, ?> personName =
          criteriaContext.addJoin(
              ...
              (personNameJoin) ->
                  cb.and(
                      cb.equal(personNameJoin.get("voided"), false),
                      cb.or(
                          cb.equal(
                              personNameJoin.get("personNameId"),
                              personNameFirstSubquery.finalizeQuery()),
                          cb.and(
                              cb.not(cb.exists(personNameSecondSubquery.finalizeQuery())),
                              cb.equal(
                                  personNameJoin.get("personNameId"),
                                  personNameThirdSubquery.finalizeQuery())),
        ...
        case ASC:
          for (String property : properties) {
            criteriaContext.addOrder(
                criteriaContext.getCriteriaBuilder().asc(personName.get(property)));
          }
          break;
        case DESC:
          for (String property : properties) {
  ...
  protected <V, U> Path<?> paramToProp(
    ...
    From<?, ?> personJoin =
        criteriaContext.getJoin("m").orElseGet(() -> criteriaContext.addJoin("person", "m"));
    if (param.startsWith("address")) {
      From<?, ?> personAddressJoin =
          criteriaContext
              .getJoin("pad")
              .orElseGet(
                  () -> criteriaContext.addJoin(personJoin, "addresses", "pad", JoinType.LEFT));
  ...
  private <U> void handleAddresses(
      OpenmrsFhirCriteriaContext<Relationship, U> criteriaContext,
