...
@Component
public class MedicationDispenseTranslatorImpl
    implements MedicationDispenseTranslator<MedicationDispense> {
  ...
  @Override
  public org.hl7.fhir.r4.model.MedicationDispense toFhirResource(
      @Nonnull MedicationDispense openmrsObject) {
    notNull(openmrsObject, "The Openmrs MedicationDispense object should not be null");

    org.hl7.fhir.r4.model.MedicationDispense fhirObject =
        new org.hl7.fhir.r4.model.MedicationDispense();
    ...
    fhirObject.setContext(
        encounterReferenceTranslator.toFhirResource(openmrsObject.getEncounter()));
    fhirObject.addAuthorizingPrescription(
        medicationRequestReferenceTranslator.toFhirResource(openmrsObject.getDrugOrder()));
    fhirObject.setStatus(
        medicationDispenseStatusTranslator.toFhirResource(openmrsObject.getStatus()));
    ...
    extension.setUrl(FhirConstants.OPENMRS_FHIR_EXT_RECORDED);
    extension.setValue(new DateTimeType(openmrsObject.getDateCreated()));
    fhirObject.addExtension(extension);

    // There is significant overlap in translating between MedicationRequests and Drug Orders.
    // Create a temp drug order and use the translated fields to set Medication, Dosing, and
    // Quantity
    ...
    if (openmrsObject.getWasSubstituted() != null
        || openmrsObject.getSubstitutionType() != null
        || openmrsObject.getSubstitutionReason() != null) {
      MedicationDispenseSubstitutionComponent substitution =
      ...
      substitution.addReason(
          conceptTranslator.toFhirResource(openmrsObject.getSubstitutionReason()));
  ...
  @Override
  public MedicationDispense toOpenmrsType(
  ...
  public MedicationDispense toOpenmrsType(
    ...
    if (fhirObject.hasContext()) {
      openmrsObject.setEncounter(
          encounterReferenceTranslator.toOpenmrsType(fhirObject.getContext()));
    } else {
    ...
    if (fhirObject.hasStatus()) {
      openmrsObject.setStatus(
          medicationDispenseStatusTranslator.toOpenmrsType(fhirObject.getStatus()));
    } // status mandatory, so we don't reset it, even if null

    if (fhirObject.hasStatusReasonCodeableConcept()) {
      openmrsObject.setStatusReason(
          conceptTranslator.toOpenmrsType(fhirObject.getStatusReasonCodeableConcept()));
    } else {
      openmrsObject.setStatusReason(null);
    }

    if (fhirObject.hasLocation()) {
      openmrsObject.setLocation(
          locationReferenceTranslator.toOpenmrsType(fhirObject.getLocation()));
    } else {
    ...
    if (fhirObject.hasPerformer()) {
      ...
      if (performerComponent != null && performerComponent.hasActor()) {
        openmrsObject.setDispenser(
            practitionerReferenceTranslator.toOpenmrsType(performerComponent.getActor()));
      } else {
    ...
    if (fhirObject.hasSubstitution()) {
      ...
      if (substitution.hasReason()) {
        openmrsObject.setSubstitutionReason(
            conceptTranslator.toOpenmrsType(substitution.getReasonFirstRep()));
      } else {
