...
public class SearchParameterMap implements Serializable {
  ...
  @Getter @Setter private int fromIndex = 0;

  @Getter @Setter private int toIndex = Integer.MAX_VALUE;
  ...
  public <T> List<PropParam<T>> getParameters(@Nonnull String key) {
    ...
    List<PropParam<T>> result =
        this.params.getOrDefault(key, new ArrayList<>()).stream()
            .filter(param -> type.getRawType().isAssignableFrom(param.getClass()))
            .map(param -> (PropParam<T>) param)
