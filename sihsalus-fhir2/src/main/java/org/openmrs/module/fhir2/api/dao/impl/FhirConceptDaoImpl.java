...
public class FhirConceptDaoImpl extends BaseFhirDao<Concept> implements FhirConceptDao {
  ...
  public Optional<Concept> getConceptWithSameAsMappingInSource(
      @Nonnull ConceptSource conceptSource, @Nonnull String mappingCode) {
    if (conceptSource == null || mappingCode == null) {
      return Optional.empty();
    }

    OpenmrsFhirCriteriaContext<ConceptMap, Concept> criteriaContext =
        ...
        criteriaContext
            ...
            .or(
                criteriaContext
                    .getCriteriaBuilder()
                    .equal(
                        conceptMapTypeJoin.get().get("uuid"), ConceptMapType.SAME_AS_MAP_TYPE_UUID),
                criteriaContext
                    .getCriteriaBuilder()
                    .equal(conceptMapTypeJoin.get().get("name"), "SAME-AS")));

    criteriaContext.addOrder(
        criteriaContext.getCriteriaBuilder().asc(conceptJoin.get().get("retired")));

    TypedQuery<Concept> query =
        criteriaContext.getEntityManager().createQuery(criteriaContext.finalizeQuery());
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Concept, U> criteriaContext,
      @Nonnull SearchParameterMap theParams) {
    criteriaContext
        .getCriteriaBuilder()
        .and(
            criteriaContext.getCriteriaBuilder().equal(criteriaContext.getRoot().get("set"), true));
    theParams
        .getParameters()
        .forEach(
            entry -> {
              switch (entry.getKey()) {
                case TITLE_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleTitle(criteriaContext, (StringAndListParam) param.getParam()));
                  break;
              }
            });
  }

  protected <U> void handleTitle(
      OpenmrsFhirCriteriaContext<Concept, U> criteriaContext, StringAndListParam titlePattern) {
    ...
    handleAndListParam(
            ...
            (title) ->
                getSearchQueryHelper()
                    .propertyLike(criteriaContext, conceptNamesJoin, "name", title))
  ...
  protected OpenmrsFhirCriteriaContext<ConceptMap, Concept> createConceptMapCriteriaBuilder(
    ...
    OpenmrsFhirCriteriaContext<ConceptMap, Concept> criteriaContext =
        createCriteriaContext(ConceptMap.class, Concept.class);
    ...
    if (Context.getAdministrationService().isDatabaseStringComparisonCaseSensitive()) {
      ...
      criteriaContext.addPredicate(
          criteriaContext
              .getCriteriaBuilder()
              .equal(conceptReferenceTermJoin.get("code"), mappingCode));
    }

    criteriaContext.addPredicate(
        criteriaContext
            .getCriteriaBuilder()
            .equal(conceptReferenceTermJoin.get("conceptSource"), conceptSource));
