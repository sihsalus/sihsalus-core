...
/**
 * This is a base class for FHIR2 Dao objects providing default implementations for the {@link
 * FhirDao} interface. It includes a {@link FhirSearchQueryHelper} so that the criteria helpers used
 * there will be available to all subclasses. <br>
...
@Transactional
@Slf4j
public abstract class BaseFhirDao<T extends OpenmrsObject & Auditable> extends BaseDao
    implements FhirDao<T> {
  ...
  protected BaseFhirDao() {
    ...
    this.isImmutable =
        Order.class.isAssignableFrom(typeToken.getRawType())
  ...
  public List<T> get(@Nonnull Collection<String> uuids) {
    ...
    OpenmrsFhirCriteriaContext<T, T> criteriaContext =
        createCriteriaContext((Class<T>) typeToken.getRawType());
  ...
  public List<T> getSearchResults(@Nonnull SearchParameterMap theParams) {
    ...
    if (hasDistinctResults()) {
      ...
      @SuppressWarnings({"UnstableApiUsage", "unchecked"})
      OpenmrsFhirCriteriaContext<T, Object> criteriaContext =
          getSearchResultCriteria(
      ...
      TypedQuery<Object> executableIdQuery =
          criteriaContext.getEntityManager().createQuery(idQuery);
      ...
      // Extract IDs from results - when ORDER BY is present, results are Object[] with ID as first
      // element
      ...
      // We still need to sort the wrapper query to maintain the order, as IN() doesn't guarantee
      // order
      @SuppressWarnings({"UnstableApiUsage", "unchecked"})
      OpenmrsFhirCriteriaContext<T, T> wrapperQuery =
          createCriteriaContext((Class<T>) typeToken.getRawType());
      ...
      results =
          wrapperQuery
              .getEntityManager()
              .createQuery(wrapperQuery.finalizeWrapperQuery(idProperty, ids))
              .getResultList();
    }
  ...
  public int getSearchResultsCount(@Nonnull SearchParameterMap theParams) {
    @SuppressWarnings({"UnstableApiUsage", "unchecked"})
    OpenmrsFhirCriteriaContext<T, Long> criteriaContext =
        getSearchResultCriteria(
    ...
    if (hasDistinctResults()) {
      criteriaContext
          .getCriteriaQuery()
          .select(criteriaContext.getCriteriaBuilder().count(criteriaContext.getRoot()));
    } else {
      criteriaContext
          .getCriteriaQuery()
          .select(
              criteriaContext
                  ...
                  .countDistinct(
                      criteriaContext
                          .getRoot()
                          .get(getIdPropertyName(criteriaContext.getEntityManager()))));
    }
  ...
  protected <V, U> void applyExactTotal(
      @Nonnull OpenmrsFhirCriteriaContext<V, U> criteriaContext, SearchParameterMap theParams) {
  ...
  /**
   * This is an overrideable implementation to convert a result that may be a proxied Hibernate
   * object into its underlying implementation. <br>
   * Overrides are most commonly needed where, e.g., "deep" properties of an object are usually
   * needed to properly convert that object to FHIR, e.g., to ensure that the {@link
   * org.openmrs.Concept} associated with an {@link org.openmrs.Obs} is also deproxied when the
   * {@link org.openmrs.Obs} is deproxied. <br>
  ...
  /**
   * Gets the name of the property annotated as the {@link jakarta.persistence.Id} for the
   * persistent class this manages.
   *
  ...
  protected OpenmrsFhirCriteriaContext<T, T> getSearchResultCriteria(SearchParameterMap theParams) {
    return getSearchResultCriteria(
        createCriteriaContext((Class<T>) typeToken.getRawType()), theParams);
  }

  protected <U> OpenmrsFhirCriteriaContext<T, U> getSearchResultCriteria(
      OpenmrsFhirCriteriaContext<T, U> criteriaContext, SearchParameterMap theParams) {
    if (isVoidable) {
      handleVoidable(criteriaContext);
    } else if (isRetireable) {
      handleRetireable(criteriaContext);
  ...
  protected <V, U> Optional<Predicate> handleCommonSearchParameters(
      OpenmrsFhirCriteriaContext<V, U> criteriaContext, List<PropParam<?>> theCommonParams) {
    ...
    for (PropParam<?> commonSearchParam : theCommonParams) {
      switch (commonSearchParam.getPropertyName()) {
        case FhirConstants.ID_PROPERTY:
          predicateList.add(
              handleAndListParam(
                  ...
                  (TokenAndListParam) commonSearchParam.getParam(),
                  param ->
                      Optional.of(
                          criteriaContext
                              .getCriteriaBuilder()
        ...
        case FhirConstants.LAST_UPDATED_PROPERTY:
          predicateList.add(
              handleLastUpdated(criteriaContext, (DateRangeParam) commonSearchParam.getParam()));
          break;
      }
    }
    return Optional.of(
   ...
   */
  protected <V, U> void handleIdPropertyOrdering(
      @Nonnull OpenmrsFhirCriteriaContext<V, U> criteriaContext, String idPropertyName) {
   ...
   */
  protected <V, U> Optional<Predicate> handleLastUpdated(
  ...
  // Implementation of handleLastUpdated for "immutable" types, that is, those that cannot be
  // changed
  ...
  protected <V, U> Optional<Predicate> handleLastUpdatedMutable(
      @Nonnull OpenmrsFhirCriteriaContext<V, U> criteriaContext, DateRangeParam param) {
    // @formatter:off
    return Optional.of(
        criteriaContext
            ...
            .or(
                toCriteriaArray(
                    getSearchQueryHelper().handleDateRange(criteriaContext, "dateChanged", param),
                    Optional.of(
                        criteriaContext
                            .getCriteriaBuilder()
                            .and(
                                    ...
                                    Stream.of(
                                        Optional.of(
                                            criteriaContext
                                                .getCriteriaBuilder()
                                                .isNull(
                                                    criteriaContext.getRoot().get("dateChanged"))),
                                        getSearchQueryHelper()
                                            .handleDateRange(
                                                criteriaContext, "dateCreated", param))))))));
    // @formatter:on
  }
  ...
  /**
   * Use this method to properly implement sorting for your query. Note that for this method to
   * work, you must override one or more of: {@link #paramToProps(OpenmrsFhirCriteriaContext,
   * SortState)}, {@link #paramToProps(OpenmrsFhirCriteriaContext, String)}, or {@link
   * #paramToProp(OpenmrsFhirCriteriaContext, String)}.
   *
   * @param criteriaContext The {@link OpenmrsFhirCriteriaContext} for the current query
   * @param sort the {@link SortSpec} which defines the sorting to be translated
   */
  protected <V, U> void handleSort(
      OpenmrsFhirCriteriaContext<V, U> criteriaContext, SortSpec sort) {
    handleSort(criteriaContext, sort, this::paramToProps)
        .ifPresent(l -> l.forEach(criteriaContext::addOrder));
  }
  ...
  protected <V, U> Optional<List<jakarta.persistence.criteria.Order>> handleSort(
      ...
      SortSpec sort,
      BiFunction<
              OpenmrsFhirCriteriaContext<V, U>,
              SortState<V>,
              Collection<jakarta.persistence.criteria.Order>>
          paramToProp) {
    ...
    while (sortSpec != null) {
      SortState<V> state =
          SortState.<V>builder()
              ...
              .sortOrder(sortSpec.getOrder())
              .parameter(sortSpec.getParamName().toLowerCase())
              .build();

      Collection<jakarta.persistence.criteria.Order> orders =
          paramToProp.apply(criteriaContext, state);
      if (orders != null) {
   ...
   * relationship between {@link org.openmrs.Patient} and {@link org.openmrs.Person} where {@link
   * org.openmrs.Patient} extends {@link org.openmrs.Person}.
   *
  ...
  /**
   * This function should be overridden by implementations. It is used to map FHIR parameter names
   * to their corresponding values in the query.
   *
  ...
  protected <V, U> Collection<jakarta.persistence.criteria.Order> paramToProps(
    ...
    if (FhirConstants.SP_LAST_UPDATED.equalsIgnoreCase(param)) {
      ...
      Expression<?> coalescedAttributes =
          cb.coalesce(
              criteriaContext.getRoot().get("dateChanged"),
    ...
    if (prop != null) {
      switch (sortState.getSortOrder()) {
        case ASC:
          return prop.stream()
              .map(p -> criteriaContext.getCriteriaBuilder().asc(p))
              .collect(Collectors.toList());
        case DESC:
          return prop.stream()
              .map(p -> criteriaContext.getCriteriaBuilder().desc(p))
              .collect(Collectors.toList());
      }
    }
  ...
  /**
   * This function should be overridden by implementations. It is used to map FHIR parameter names
   * to properties where there is only a single property.
   *
   * @param param the FHIR parameter to map
   * @return the name of the corresponding property from the current query
   */
  protected <V, U> Collection<Path<?>> paramToProps(
      @Nonnull OpenmrsFhirCriteriaContext<V, U> criteriaContext, @Nonnull String param) {
   ...
   * parameters. <br>
   * The {@link SearchParameterMap} provides an interface for tracking all the FHIR search
   * parameters the user supplied for the current search query. It is the responsibility of this
   * function to map all known parameters into JPA criteria {@link Predicate}s that are added to the
   * current {@link OpenmrsFhirCriteriaContext}, so that they act as appropriate restrictions on the
   * generated queries. <br>
   * The {@link BaseDao} and {@link FhirSearchQueryHelper} classes provide helpers for constructing
   * such queries, with the {@link BaseDao} providing generic handling for HAPI's {@link
   * ca.uhn.fhir.model.api.IQueryParameterAnd} and {@link ca.uhn.fhir.model.api.IQueryParameterOr}
   * types. Meanwhile, the {@link FhirSearchQueryHelper} provides helpers for correctly constructing
   * predicates for various HAPI FHIR types or common query elements, e.g., translating {@link
   * ca.uhn.fhir.rest.param.TokenParam}s into queries against the OpenMRS Concept Dictionary. <br>
            ...
            param -> {
              switch (param.getKey()) {
                case COMMON_SEARCH_HANDLER:
                  handleCommonSearchParameters(criteriaContext, param.getValue())
