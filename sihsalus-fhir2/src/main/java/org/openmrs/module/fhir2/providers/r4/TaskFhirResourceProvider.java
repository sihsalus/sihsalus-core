...
public class TaskFhirResourceProvider extends BaseUpsertFhirResourceProvider<Task> {
  ...
  protected MethodOutcome doUpsert(
      IdType id, Task task, RequestDetails requestDetails, boolean createIfNotExists) {
    return FhirProviderUtils.buildUpdate(
        service.update(id.getIdPart(), task, requestDetails, createIfNotExists));
  }

  @Patch
  public MethodOutcome patchTask(
      @IdParam IdType id,
