...
public class MedicationFhirResourceProvider extends BaseUpsertFhirResourceProvider<Medication> {
  ...
  @Override
  protected MethodOutcome doUpsert(
      IdType id, Medication medication, RequestDetails requestDetails, boolean createIfNotExists) {
    ...
    return FhirProviderUtils.buildUpdate(
        fhirMedicationService.update(
            id.getIdPart(), medication, requestDetails, createIfNotExists));
  }

  @Patch
  public MethodOutcome patchMedication(
      @IdParam IdType id,
    ...
    Medication medication =
        fhirMedicationService.patch(id.getIdPart(), patchType, body, requestDetails);
