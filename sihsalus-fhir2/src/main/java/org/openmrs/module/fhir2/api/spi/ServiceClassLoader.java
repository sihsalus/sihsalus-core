...
/**
 * This is an implementation of the Java Service Provider Interface that differs from {@link
 * ServiceLoader} in that it returns class objects rather than instantiated objects. Of course, this
 * means that it cannot guarantee that the provided services are loaded once and only once.
 *
 * @param <C>
 */
public class ServiceClassLoader<C> {
  ...
  private void loadResources() {
    ...
    try {
      ...
      throw new ServiceConfigurationError(
          service.getName() + ": Error locating configuration files", e);
    }
      ...
      while (line != null) {
        ...
        if (className != null) {
          ...
          try {
            ...
            throw new ServiceConfigurationError(
                service.getName() + ": Provider " + className + " not found");
          }
  ...
  private String parseLine(String line) {
    ...
    if (!Character.isJavaIdentifierStart(line.charAt(0))) {
      throw new ServiceConfigurationError(
          service.getName() + ": Illegal provider-class name: " + line);
    }
