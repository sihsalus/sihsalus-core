...
@Component
public class FhirObservationServiceImpl extends BaseFhirService<Observation, org.openmrs.Obs>
    implements FhirObservationService {
  ...
  public IBundleProvider searchForObservations(ObservationSearchParams observationSearchParams) {
    return searchQuery.getQueryResults(
        observationSearchParams.toSearchParameterMap(), dao, translator, searchQueryInclude);
  }
  ...
  public IBundleProvider getLastnObservations(
    ...
    SearchParameterMap theParams =
        observationSearchParams
            .toSearchParameterMap()
            .addParameter(FhirConstants.LASTN_OBSERVATION_SEARCH_HANDLER, new StringParam())
            .addParameter(
                FhirConstants.MAX_SEARCH_HANDLER,
                Optional.ofNullable(max).orElse(new NumberParam(1)));
  ...
  protected Observation applyUpdate(org.openmrs.Obs existingObject, Observation updatedResource) {
    ...
    if (translator instanceof UpdatableOpenmrsTranslator) {
      UpdatableOpenmrsTranslator<org.openmrs.Obs, Observation> updatableOpenmrsTranslator =
          (UpdatableOpenmrsTranslator<org.openmrs.Obs, Observation>) translator;
    ...
    return translator.toFhirResource(
        Context.getObsService().saveObs(updatedObject, "Updated via the FHIR2 API"));
  }
}
