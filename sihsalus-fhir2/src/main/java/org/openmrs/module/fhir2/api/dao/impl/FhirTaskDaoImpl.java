...
public class FhirTaskDaoImpl extends BaseFhirDao<FhirTask> implements FhirTaskDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<FhirTask, U> criteriaContext,
  ...
  public FhirTask createOrUpdate(@Nonnull FhirTask task) throws DAOException {
    ...
    if (task.getOwnerReference() != null && task.getOwnerReference().getReference() != null) {
      task.setOwnerReference(
          getSessionFactory().getCurrentSession().merge(task.getOwnerReference()));
    }
  ...
  private <U> Optional<Predicate> handleStatus(
      OpenmrsFhirCriteriaContext<FhirTask, U> criteriaContext,
            ...
            try {
              return Optional.of(
                  criteriaContext
                      .getCriteriaBuilder()
                      .equal(
                          criteriaContext.getRoot().get("status"),
  ...
  private <U> Optional<Predicate> handleReference(
      @Nonnull OpenmrsFhirCriteriaContext<FhirTask, U> criteriaContext,
      ReferenceAndListParam reference,
      String property,
      String alias) {
    return handleAndListParam(
        criteriaContext.getCriteriaBuilder(),
        ...
        param -> {
          if (validReferenceParam(param)) {
            ...
            return Optional.of(
                criteriaContext.getCriteriaBuilder().and(toCriteriaArray(predicateList)));
          }
  ...
  private <U> Optional<Predicate> handleTaskCodeConcept(
      @Nonnull OpenmrsFhirCriteriaContext<FhirTask, U> criteriaContext, TokenAndListParam code) {
    if (code == null) {
      return Optional.empty();
    }
