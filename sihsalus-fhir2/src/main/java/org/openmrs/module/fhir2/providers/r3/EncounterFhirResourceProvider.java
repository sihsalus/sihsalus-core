...
public class EncounterFhirResourceProvider implements IResourceProvider {
        ...
        VersionConvertorFactory_30_40.convertResource(
            encounterService.create(
                (org.hl7.fhir.r4.model.Encounter)
                    VersionConvertorFactory_30_40.convertResource(encounter))));
  }
  ...
  @Search
  public IBundleProvider searchEncounter(
      @OptionalParam(name = Encounter.SP_DATE) DateRangeParam date,
      @OptionalParam(
              name = Encounter.SP_LOCATION,
              chainWhitelist = {
                "",
          ...
          ReferenceAndListParam location,
      @OptionalParam(
              name = Encounter.SP_PARTICIPANT,
              chainWhitelist = {
                "",
      ...
      @IncludeParam(
              allow = {
                "Encounter:" + Encounter.SP_LOCATION,
                "Encounter:" + Encounter.SP_PATIENT,
      ...
      @IncludeParam(
              reverse = true,
              allow = {
                "Observation:" + Observation.SP_ENCOUNTER,
                "DiagnosticReport:" + DiagnosticReport.SP_ENCOUNTER,
                "MedicationRequest:" + MedicationRequest.SP_CONTEXT,
    ...
    return new SearchQueryBundleProviderR3Wrapper(
        encounterService.searchForEncounters(
            new EncounterSearchParams(
                date,
                location,
  ...
  /**
   * Custom search endpoint that fetches encounters that include medication requests. NOTE: This
   * query has been designed to provide the backend query functionality needed by the Dispensing
   * ESM, and so the exact contract on what is returned may be modified as that ESM evolves. NOTE:
   * requires Core 2.6.1 or higher (with new "declined" fulfiller status)
   *
   ...
   * @param patientSearchTerm restrict to encounters for patients whose name or identifier matches
   *     the search term
   * @param location restrict to encounters at certain location(s)
   * @param type restrict to encounters of certain (encounter) type(s)
   * @return bundle that includes the medication requests and any medication dispenses that
   *     reference those requests
   */
  ...
  public IBundleProvider getEncountersWithMedicationRequestsSearch(
    ...
    params.setTag(
        new TokenAndListParam()
            .addAnd(
                new TokenParam().setSystem(OPENMRS_FHIR_EXT_ENCOUNTER_TAG).setValue("encounter")));
    ...
    if (status != null && !status.isEmpty() && status.getValue().equalsIgnoreCase("active")) {
      // encounter must have a medication request that is neither completed nor cancelled nor
      // declined (expired is okay, since we are using another definition of expired for dispensing
      // purposes)
      HasOrListParam notCompletedHasParam =
          new HasOrListParam()
              .add(
                  new HasParam(
                      "MedicationRequest", "encounter", "fulfillerStatus:not", "completed"));
      HasOrListParam notDeclinedHasParam =
          new HasOrListParam()
              .add(
                  new HasParam(
                      "MedicationRequest", "encounter", "fulfillerStatus:not", "declined"));
      HasOrListParam notCancelledHasParam =
          new HasOrListParam()
              .add(new HasParam("MedicationRequest", "encounter", "status:not", "cancelled"));
      params.setHasAndListParam(
          new HasAndListParam()
              .addAnd(notCancelledHasParam)
              .addAnd(notDeclinedHasParam)
              .addAnd(notCompletedHasParam));
    } else {
      ...
      params.setHasAndListParam(
          new HasAndListParam()
              .addAnd(
                  new HasOrListParam()
                      .add(new HasParam("MedicationRequest", "encounter", "intent", "order"))));
    }
    ...
    // all this convoluted testing is just a null/empty check (so "location=empty_string" is
    // ignored)
    if (location != null
        && !location.getValuesAsQueryTokens().isEmpty()
    ...
    // include all medication requests associated with the encounter, and then all dispenses
