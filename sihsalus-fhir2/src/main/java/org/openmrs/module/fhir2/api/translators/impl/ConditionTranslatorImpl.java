...
public class ConditionTranslatorImpl implements ConditionTranslator<Condition> {
  ...
  public org.hl7.fhir.r4.model.Condition toFhirResource(@Nonnull Condition condition) {
    ...
    fhirCondition.setClinicalStatus(
        clinicalStatusTranslator.toFhirResource(condition.getClinicalStatus()));
    fhirCondition.setVerificationStatus(
        verificationStatusTranslator.toFhirResource(condition.getVerificationStatus()));
    ...
    fhirCondition.setRecorder(
        practitionerReferenceTranslator.toFhirResource(condition.getCreator()));
    fhirCondition.setRecordedDate(condition.getDateCreated());

    CodeableConcept category = new CodeableConcept();
    category
        .addCoding()
        .setSystem(FhirConstants.CONDITION_CATEGORY_SYSTEM_URI)
  ...
  @Override
  public Condition toOpenmrsType(
      @Nonnull Condition existingCondition, @Nonnull org.hl7.fhir.r4.model.Condition condition) {
    ...
    existingCondition.setClinicalStatus(
        clinicalStatusTranslator.toOpenmrsType(condition.getClinicalStatus()));
    existingCondition.setVerificationStatus(
        verificationStatusTranslator.toOpenmrsType(condition.getVerificationStatus()));
    ...
    extension.ifPresent(
        value -> conditionCodedOrText.setNonCoded(String.valueOf(value.getValue())));
    ...
    existingCondition.setCreator(
        practitionerReferenceTranslator.toOpenmrsType(condition.getRecorder()));
