...
public class FhirLocationDaoImpl extends BaseFhirDao<Location> implements FhirLocationDao {
  ...
  protected <U> void setupSearchParams(
      @Nonnull OpenmrsFhirCriteriaContext<Location, U> criteriaContext,
              ...
              switch (entry.getKey()) {
                case FhirConstants.NAME_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleName(criteriaContext, (StringAndListParam) param.getParam())
                ...
                case FhirConstants.CITY_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleCity(criteriaContext, (StringAndListParam) param.getParam())
                                  .ifPresent(criteriaContext::addPredicate));
                  break;
                case FhirConstants.STATE_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleState(criteriaContext, (StringAndListParam) param.getParam())
                                  .ifPresent(criteriaContext::addPredicate));
                  break;
                case FhirConstants.COUNTRY_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleCountry(criteriaContext, (StringAndListParam) param.getParam())
                                  .ifPresent(criteriaContext::addPredicate));
                  break;
                case FhirConstants.POSTALCODE_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handlePostalCode(
                                      criteriaContext, (StringAndListParam) param.getParam())
                                  .ifPresent(criteriaContext::addPredicate));
                  break;
                ...
                case FhirConstants.TAG_SEARCH_HANDLER:
                  entry
                      ...
                      .forEach(
                          param ->
                              handleTag(criteriaContext, (TokenAndListParam) param.getParam())
                                  .ifPresent(criteriaContext::addPredicate));
                  break;
  ...
  @Override
  public List<LocationAttribute> getActiveAttributesByLocationAndAttributeTypeUuid(
      @Nonnull Location location, @Nonnull String locationAttributeTypeUuid) {
    OpenmrsFhirCriteriaContext<LocationAttribute, LocationAttribute> criteriaContext =
        createCriteriaContext(LocationAttribute.class);
        ...
        criteriaContext
            ...
            .and(
                criteriaContext
                    .getCriteriaBuilder()
                    .equal(
                        criteriaContext.getRoot().join("location").get("locationId"),
                        location.getId()),
                criteriaContext
                    .getCriteriaBuilder()
                    .equal(
  ...
  public Map<Location, List<LocationAttribute>> getActiveAttributesByLocationsAndAttributeTypeUuid(
    ...
    final CriteriaQuery<LocationAttribute> criteria =
        criteriaBuilder.createQuery(LocationAttribute.class);
    final Root<LocationAttribute> locationAttributeRoot = criteria.from(LocationAttribute.class);

    final Predicate byId =
        locationAttributeRoot
            .get("location")
            .get("locationId")
            .in(location.stream().map(Location::getLocationId).collect(toList()));
    final Predicate byAttributeType =
        criteriaBuilder.equal(
            locationAttributeRoot.get("attributeType").get("uuid"), locationAttributeTypeUuid);
  ...
  private <U> Optional<Predicate> handleName(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext, StringAndListParam namePattern) {
    return handleAndListParam(
        criteriaContext.getCriteriaBuilder(),
        namePattern,
        (name) ->
            getSearchQueryHelper()
                .propertyLike(criteriaContext, criteriaContext.getRoot(), "name", name));
  }

  private <U> Optional<Predicate> handleCity(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext, StringAndListParam cityPattern) {
    return handleAndListParam(
        criteriaContext.getCriteriaBuilder(),
        cityPattern,
        (city) ->
            getSearchQueryHelper()
                .propertyLike(criteriaContext, criteriaContext.getRoot(), "cityVillage", city));
  }

  private <U> Optional<Predicate> handleCountry(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext, StringAndListParam countryPattern) {
    return handleAndListParam(
        criteriaContext.getCriteriaBuilder(),
        countryPattern,
        (country) ->
  ...
  private <U> Optional<Predicate> handlePostalCode(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext,
    ...
    return handleAndListParam(
        ...
        (postalCode) ->
            getSearchQueryHelper()
                .propertyLike(
                    criteriaContext, criteriaContext.getRoot(), "postalCode", postalCode));
  }

  private <U> Optional<Predicate> handleState(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext, StringAndListParam statePattern) {
    return handleAndListParam(
        criteriaContext.getCriteriaBuilder(),
        statePattern,
        (state) ->
  ...
  private <U> Optional<Predicate> handleTag(
      OpenmrsFhirCriteriaContext<Location, U> criteriaContext, TokenAndListParam tags) {
    ...
    return handleAndListParam(
        ...
        (tag) ->
            Optional.of(
                criteriaContext.getCriteriaBuilder().equal(tagJoin.get("name"), tag.getValue())));
  }

  protected <T, U> Optional<Predicate> handleLocationReference(
      @Nonnull OpenmrsFhirCriteriaContext<T, U> criteriaContext,
      @Nonnull From<?, ?> locationAlias,
      ReferenceAndListParam locationAndReferences) {
    ...
    // **NOTE: this is a *bug* in the current HAPI FHIR implementation, "below" should be the
    // "queryParameterQualifier",
    // not the resource type; likely need update this when/fix the HAPI FHIR implementation is
    // fixed**
    ...
    if ("below".equalsIgnoreCase(locationReference.getResourceType())) {
      if (locationOrReference.size() > 1 || locationReferences.size() > 1) {
        throw new IllegalArgumentException(
            "Only one location reference is supported for :below queries");
      }

      int searchDepth =
          globalPropertyService.getGlobalPropertyAsInteger(
              FhirConstants.SUPPORTED_LOCATION_HIERARCHY_SEARCH_DEPTH, 5);
      ...
      for (int depth = 1; depth < searchDepth; depth++) {
        ...
        predicates.add(
            criteriaContext
                .getCriteriaBuilder()
                .equal(join.get("uuid"), locationReference.getIdPart()));
      ...
      if (predicates.isEmpty()) {
        ...
        return Optional.of(
            criteriaContext.getCriteriaBuilder().or(predicates.toArray(new Predicate[0])));
      }
    } else {
      // this is to support queries of the type "Location?partof=uuid" or chained search like
      // "Location?partof:Location=Location:name=xxx"
      Join<?, ?> join = criteriaContext.addJoin("parentLocation", "loc");
      return getSearchQueryHelper()
          .handleLocationReference(criteriaContext, join, locationAndReferences);
    }
  }
