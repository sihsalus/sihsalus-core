...
public abstract class BaseAddressTranslator {
  ...
  protected void addAddressExtension(
      @Nonnull Address address, @Nonnull String extensionProperty, @Nonnull String value) {
    if (value == null) {
      return;
    }
    ...
    getOpenmrsAddressExtension(address)
        ...
        .addExtension(
            FhirConstants.OPENMRS_FHIR_EXT_ADDRESS + "#" + extensionProperty,
            new StringType(value));
  }

  protected void addAddressComponent(
      @Nonnull org.openmrs.Address addressOfResource, @Nonnull String url, @Nonnull String value) {
    if (value == null
        || url == null
        || !url.startsWith(FhirConstants.OPENMRS_FHIR_EXT_ADDRESS + "#")) {
      return;
    }
