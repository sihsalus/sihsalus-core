...
public class FhirCapabilityService {
  ...
  public FhirCapabilityService(
      ...
      @R4Provider java.util.List<IResourceProvider> providers) {
              ...
              interactions.forEach(
                  interaction ->
                      resource.addInteraction(
                          new ResourceInteractionComponent().setCode(interaction)));
            });
    ...
    providers.stream()
        ...
        .forEach(
            provider ->
                resources
                    .computeIfAbsent(
                        providerResourceType(provider), ignored -> new LinkedHashSet<>())
  ...
  private String resourceType(Class<?> resourceClass) {
    return fhirContext.getResourceType(
        resourceClass.asSubclass(org.hl7.fhir.instance.model.api.IBaseResource.class));
  }
