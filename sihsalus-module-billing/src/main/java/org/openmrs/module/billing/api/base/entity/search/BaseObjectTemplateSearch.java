...
public class BaseObjectTemplateSearch<T extends OpenmrsObject> {
  ...
  protected Predicate createPredicate(
      CriteriaBuilder cb, Root<?> root, String field, Object value, ComparisonType comparisonType) {
  ...
  protected Predicate createPredicate(
    ...
    StringComparisonType comparison =
        comparisonType == null ? StringComparisonType.EQUAL : comparisonType;
    return switch (comparison) {
      case EQUAL -> cb.equal(BillingRestrictions.path(root, field), value);
      case NOT_EQUAL -> cb.notEqual(BillingRestrictions.path(root, field), value);
      case IS_NULL -> cb.isNull(BillingRestrictions.path(root, field));
      ...
      case IS_EMPTY ->
          cb.or(
              cb.isNull(BillingRestrictions.path(root, field)),
              cb.equal(BillingRestrictions.path(root, field), ""));
      case IS_NOT_EMPTY ->
          cb.and(
              cb.isNotNull(BillingRestrictions.path(root, field)),
              cb.notEqual(BillingRestrictions.path(root, field), ""));
      case LIKE ->
          cb.like(
              cb.lower(BillingRestrictions.path(root, field).as(String.class)),
              "%" + value.toLowerCase() + "%");
    };
  }
  ...
  protected Predicate createPredicate(
    ...
    DateComparisonType comparison =
        comparisonType == null ? DateComparisonType.EQUAL : comparisonType;
    return switch (comparison) {
      case EQUAL -> cb.equal(BillingRestrictions.path(root, field), value);
      case NOT_EQUAL -> cb.notEqual(BillingRestrictions.path(root, field), value);
      case IS_NULL -> cb.isNull(BillingRestrictions.path(root, field));
      case IS_NOT_NULL -> cb.isNotNull(BillingRestrictions.path(root, field));
      case GREATER_THAN ->
          cb.greaterThan(BillingRestrictions.path(root, field).as(Date.class), value);
      case GREATER_THAN_EQUAL ->
          cb.greaterThanOrEqualTo(BillingRestrictions.path(root, field).as(Date.class), value);
      case LESS_THAN -> cb.lessThan(BillingRestrictions.path(root, field).as(Date.class), value);
      case LESS_THAN_EQUAL ->
          cb.lessThanOrEqualTo(BillingRestrictions.path(root, field).as(Date.class), value);
    };
  }
