...
public class SequentialReceiptNumberGenerator implements IReceiptNumberGenerator {
  ...
  private String createGrouping(Bill bill) {
    ...
    switch (model.getGroupingType()) {
      ...
      case CASHIER_AND_CASH_POINT:
        result =
            model.getCashierPrefix()
                + bill.getCashier().getId()
                + model.getSeparator()
                + model.getCashPointPrefix()
                + bill.getCashPoint().getId();
        break;
      default:
        break;
    }
  ...
  private String getSequence(String grouping) {
    ...
    // Any changes to the date/time generation must be also applied to the unit tests that mock the
    // date generation
