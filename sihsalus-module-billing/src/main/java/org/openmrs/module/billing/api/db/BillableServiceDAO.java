...
public interface BillableServiceDAO {
   ...
   *
   * <p>Note: This method may return retired billable services. Consider filtering retired records
   ...
   *
   * <p>By default, retired billable services are excluded from results unless {@link
   * BillableServiceSearch#setIncludeRetired(Boolean)} is set to true. The search criteria support
   * filtering by service status, category, type, concept, and name (partial match).
   *
  ...
  List<BillableService> getBillableServices(
      @Nonnull BillableServiceSearch billableServiceSearch, PagingInfo pagingInfo);

  /**
   * Persists a billable service to the database.
   *
   * <p>If the billable service has no ID, it will be created as a new record. If it has an ID, the
   ...
   * prices, attributes, etc.) will also be removed due to cascade delete rules.
   *
