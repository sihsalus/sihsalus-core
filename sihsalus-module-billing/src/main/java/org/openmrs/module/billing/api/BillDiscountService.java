 ...
 * {@link PrivilegeConstants#VIEW_BILL_DISCOUNTS}; saves are validated by {@code
 * BillDiscountValidator}, which enforces feature-flag, scope-exclusivity and amount derivation
 * rules.
 */
public interface BillDiscountService {
  ...
  /**
   * Returns the active bill-level discount for a bill (i.e. {@code lineItem == null} and {@code
   * voided == false}). Line-item scoped discounts are excluded — use {@link
   * #getActiveLineItemDiscount(Integer)} for those.
   *
   ...
   * first. Intended for audit-trail surfaces; for the currently effective discount(s) use {@link
   * #getBillDiscountByBillId(Integer)} or {@link #getActiveLineItemDiscount(Integer)}.
   *
   * @param billId the bill's primary key
  ...
  /**
   * Returns the persisted status of a discount via a scalar query — the entity is not loaded into
