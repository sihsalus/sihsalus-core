...
public class OrderBillingEventListener implements BillingEventListener {
  ...
  void processOrder(Order order) {
    ...
    List<OrderBillingStrategy> strategies =
        Context.getRegisteredComponents(OrderBillingStrategy.class);
    ...
    for (OrderBillingStrategy strategy : strategies) {
      if (strategy.supports(realOrder)) {
        ...
        switch (result.getAction()) {
          case CREATED:
            log.info(
                "Bill {} created for order {} by {}",
                result.getBill().getUuid(),
                order.getUuid(),
                strategy.getClass().getSimpleName());
            break;
          case UPDATED:
            log.info(
                "Bill {} updated for order {} by {}",
                result.getBill().getUuid(),
                order.getUuid(),
                strategy.getClass().getSimpleName());
            break;
