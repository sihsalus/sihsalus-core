...
public abstract class BaseInstanceCustomizableType<TAttributeType extends IInstanceAttributeType<?>>
  ...
  public void addAttributeType(Integer index, TAttributeType attributeType) {
    ...
    if (attributeType.getOwner() != this) {
      // Note that this may cause issues if the attribute type class does not have this class as the
      // owner class.
      //  I'm not sure how to make generics check this at compile-time, I tried with self-bounded
      // generic parameters
    ...
    if (index == null) {
      ...
      if (index > getAttributeTypes().size()) {
        throw new APIException(
            "Invalid attribute order. Should not leave space in the list (list length: "
