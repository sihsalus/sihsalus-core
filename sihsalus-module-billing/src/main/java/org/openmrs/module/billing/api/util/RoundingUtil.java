...
public class RoundingUtil {
  ...
  public static BigDecimal round(
      BigDecimal value, Integer nearest, CashierOptions.RoundingMode mode) {
  ...
  public static void handleRoundingLineItem(Bill bill) {
    ...
    if (difference.equals(BigDecimal.ZERO) && roundingLineItem != null) {
      ...
      bill.addLineItem(
          roundingItem, difference.abs(), "", difference.compareTo(BigDecimal.ZERO) > 0 ? -1 : 1);
  ...
  private static void updateRoundingItem(
      Bill bill, BigDecimal difference, BillLineItem roundingLineItem) {
  ...
  private static BigDecimal calculateRoundingValue(
      Bill bill, CashierOptions options, BillLineItem roundingLineItem) {
    ...
    return itemTotal.subtract(
        RoundingUtil.round(itemTotal, options.getRoundToNearest(), options.getRoundingMode()));
  }
