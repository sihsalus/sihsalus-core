...
@Handler(
    supports = {BillRefund.class},
    order = 50)
public class BillRefundValidator implements Validator {

  private static final Set<BillStatus> REFUND_ELIGIBLE_NEW_BILL_LEVEL =
      EnumSet.of(BillStatus.PAID, BillStatus.PARTIALLY_REFUNDED);

  private static final Set<BillStatus> REFUND_ELIGIBLE_NEW_LINE_SCOPED =
      EnumSet.of(BillStatus.PAID, BillStatus.REFUND_REQUESTED, BillStatus.PARTIALLY_REFUNDED);

  private static final Set<BillStatus> REFUND_ELIGIBLE_UPDATE =
      EnumSet.of(
          BillStatus.PAID,
          BillStatus.REFUND_REQUESTED,
          BillStatus.REFUNDED,
  ...
  public void validate(@Nonnull Object target, @Nonnull Errors errors) {
    ...
    String enabled =
        Context.getAdministrationService().getGlobalProperty(ModuleSettings.REFUND_ENABLED);
    ...
    if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
      ...
      errors.rejectValue("refundAmount", "billing.error.refund.amountScale");
        ...
        && exceedsLineItemRemaining(refundService, bill.getId(), lineItem, amount, selfId)) {
      errors.rejectValue("refundAmount", "billing.error.refund.exceedsLineItemTotal");
    } else if (lineItem == null
        && bill.getTotal() != null
        && amount.compareTo(bill.getTotal()) > 0) {
  ...
  private void validateStatusTransition(BillRefund refund, Errors errors) {
    ...
    if (previous == RefundStatus.REQUESTED
  ...
  private boolean hasActiveLineScopedRefund(
      BillRefundService refundService, Integer billId, Integer excludeId) {
    return refundService.getActiveLineScopedRefunds(billId).stream()
        .anyMatch(r -> excludeId == null || !excludeId.equals(r.getId()));
  }

  private boolean exceedsRemainingRefundable(
      BillRefundService refundService, Bill bill, BigDecimal proposed, Integer excludeId) {
    BigDecimal alreadyCommitted =
        refundService.getRefundsByBillId(bill.getId()).stream()
            ...
            .map(BillRefund::getRefundAmount)
            .filter(a -> a != null)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    return alreadyCommitted.add(proposed).compareTo(bill.getAmountAfterDiscount()) > 0;
  }

  private boolean exceedsLineItemRemaining(
      BillRefundService refundService,
      Integer billId,
    ...
    BigDecimal priorOnLine =
        refundService.getRefundsByBillId(billId).stream()
            .filter(r -> !Boolean.TRUE.equals(r.getVoided()))
            .filter(
                r ->
                    r.getStatus() == RefundStatus.REQUESTED
                        ...
                        || r.getStatus() == RefundStatus.COMPLETED)
            .filter(
                r -> r.getLineItem() != null && r.getLineItem().getId().equals(lineItem.getId()))
            .filter(r -> excludeId == null || !excludeId.equals(r.getId()))
            .map(BillRefund::getRefundAmount)
            .filter(a -> a != null)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
