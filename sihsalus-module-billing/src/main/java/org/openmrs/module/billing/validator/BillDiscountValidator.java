...
@Handler(
    supports = {BillDiscount.class},
    order = 50)
public class BillDiscountValidator implements Validator {

  private static final Set<BillStatus> DISCOUNT_ELIGIBLE_STATUSES =
      EnumSet.of(BillStatus.PENDING, BillStatus.POSTED, BillStatus.ADJUSTED);
  ...
  public void validate(@Nonnull Object target, @Nonnull Errors errors) {
    ...
    String enabled =
        Context.getAdministrationService().getGlobalProperty(ModuleSettings.DISCOUNT_ENABLED);
    ...
    if (value == null || value.compareTo(BigDecimal.ZERO) <= 0) {
      ...
      if (cap != null && value.compareTo(cap) > 0) {
        errors.rejectValue(
            "discountValue",
            lineItem != null
                ? "billing.error.discount.exceedsLineItemTotal"
    ...
    if (status == null) {
      ...
      if ((status == DiscountStatus.APPROVED || status == DiscountStatus.REJECTED)
          && discount.getApprover() == null) {
