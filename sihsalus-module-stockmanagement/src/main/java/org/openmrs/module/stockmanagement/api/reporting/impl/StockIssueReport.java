...
public class StockIssueReport extends StockOperationLineItemReport {

  @Override
  protected void setLocationFilter(
      StockOperationLineItemFilter filter, Location location, Boolean childLocations) {
  ...
  protected void setFilters(StockOperationLineItemFilter filter, Properties parameters) {
    StockOperationType stockOperationType =
        stockManagementService.getStockOperationTypeByType(StockOperationType.STOCK_ISSUE);
  ...
  protected void writeRow(CSVWriter csvWriter, StockOperationLineItem row) {
    writeLineToCsv(
        ...
        emptyIfNull(
            row.getResponsiblePerson() != null
                ? String.format(
                    "%1$s %2$s",
                    row.getResponsiblePersonFamilyName(), row.getResponsiblePersonGivenName())
                : row.getResponsiblePersonOther()),
        row.getCompletedDate() == null ? "" : TIMESTAMP_FORMATTER.format(row.getCompletedDate()),
        String.format("%1$s %2$s", row.getCompletedByFamilyName(), row.getCompletedByGivenName()),
        emptyIfNull(row.getRemarks()),
        emptyIfNull(row.getSourceName()),
        emptyIfNull(row.getDestinationName()),
        ...
        row.getQuantityRequested() == null ? null : row.getQuantityRequested().toPlainString(),
        row.getQuantityRequestedPackagingUOMName() == null
            ? null
            : row.getQuantityRequestedPackagingUOMName(),
        row.getQuantityRequestedPackagingUOMFactor() == null
            ? null
            : row.getQuantityRequestedPackagingUOMFactor().toPlainString(),
        row.getQuantity().toPlainString(),
