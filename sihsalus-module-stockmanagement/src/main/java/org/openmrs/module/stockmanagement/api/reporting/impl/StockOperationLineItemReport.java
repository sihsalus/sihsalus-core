...
public abstract class StockOperationLineItemReport extends ReportGenerator {
  ...
  public void generateReport(BatchJob batchJob, Function<BatchJob, Boolean> shouldStopExecution) {
    ...
    try {
      ...
      if (!StringUtils.isBlank(stockSourceUuid)) {
        ...
        if (stockSource == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Report source parameter not found");
          return;
        }
        Party party = stockManagementService.getPartyByStockSource(stockSource);
        if (party == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Party related to report source parameter not found");
          return;
        }
      ...
      if (!StringUtils.isBlank(stockSourceDestinationUuid)) {
        StockSource stockSourceDestination =
            stockManagementService.getStockSourceByUuid(stockSourceDestinationUuid);
        if (stockSourceDestination == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Report destination parameter not found");
          return;
        }
        Party party = stockManagementService.getPartyByStockSource(stockSourceDestination);
        if (party == null) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Party related to report destination parameter not found");
          return;
        }
      ...
      while (hasMoreRecords) {
        ...
        Result<StockOperationLineItem> data =
            stockManagementService.findStockOperationLineItems(filter);
        if (shouldStopExecution.apply(batchJob)) {
          return;
        }
        ...
        java.util.List<StockOperationLineItem> rows =
        ...
        if (!rows.isEmpty()) {
          ...
          updateExecutionState(
              ...
              pageIndex,
              recordsProcessed,
              lastRecord.getStockOperationItemId(),
              lastRecord.getStockOperationId(),
  ...
  protected abstract void setLocationFilter(
