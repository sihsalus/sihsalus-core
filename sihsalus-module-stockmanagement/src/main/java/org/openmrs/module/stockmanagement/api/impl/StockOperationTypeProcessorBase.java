...
public abstract class StockOperationTypeProcessorBase implements StockOperationTypeProcessor {
  ...
  public boolean userCanProcess(User currentUser, Location location) {
    return userCanProcess(
        currentUser, location, Privileges.TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE);
  }

  @Override
  public boolean userCanProcess(User user, Location location, String privilege) {
    return Context.getService(StockManagementService.class)
        .userHasStockManagementPrivilege(user, location, this.stockOperationType, privilege);
  }
  ...
  protected Set<StockItemTransaction> executeCopyReserved(
      StockOperation operation, Action2<ReservedTransaction, StockItemTransaction> action) {
  ...
  protected Set<StockItemTransaction> executeCopyReservedAndClear(
      StockOperation operation, Action2<ReservedTransaction, StockItemTransaction> action) {
