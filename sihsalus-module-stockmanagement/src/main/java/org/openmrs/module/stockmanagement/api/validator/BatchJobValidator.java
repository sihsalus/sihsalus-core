...
public class BatchJobValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    ...
    if (object.getBatchJobType() == null) {
      errors.rejectValue(
          "batchJobType",
          String.format(
              messageSourceService.getMessage("stockmanagement.batchjob.fieldrequired"),
              "Batch Job Type"));
      return;
    } else {
      ...
      if (!batchJobType.equals(BatchJobType.Report)) {
        errors.rejectValue(
            "batchJobType",
            String.format(
                messageSourceService.getMessage("stockmanagement.batchjob.fieldvaluenotexist"),
                "Batch Job Type"));
        return;
      }
    }
    ...
    if (StringUtils.isBlank(object.getDescription())) {
      ...
      errors.rejectValue(
          "description",
          String.format(
              messageSourceService.getMessage("stockmanagement.batchjob.exceedslimit"),
              "description",
              255));
      return;
    }
    ...
    if ((!properties.containsKey("param.report"))
        || StringUtils.isBlank(reportSystemName = properties.getProperty("param.report"))) {
      errors.rejectValue(
          "parameters",
          String.format(
              messageSourceService.getMessage("stockmanagement.batchjob.fieldrequired"),
    ...
    for (Map.Entry<Object, Object> property : properties.entrySet()) {
      ...
      if (!propertyName.startsWith("param.")) {
        errors.rejectValue(
            "parameters",
            messageSourceService.getMessage("stockmanagement.batchjob.paramformatinvalid"));
        return;
      }
      if (propertyName.equals("param.report")) {
        ...
        if (indexOfToken > 6) {
          ...
          ReportParameter reportParameter =
              ReportParameter.findInList(parameterName, report.get().getParameters());
          if (reportParameter == null) {
            errors.rejectValue(
                ...
                String.format(
                    ...
                    "parameter name"));
            return;
          } else if (!isValidReportParameter(
              reportParameter, (String) property.getValue(), object, stockManagementService)) {
            errors.rejectValue(
                "parameters",
                String.format(
                    messageSourceService.getMessage("stockmanagement.batchjob.fieldvaluenotexist"),
          ...
          errors.rejectValue(
              "parameters",
              messageSourceService.getMessage("stockmanagement.batchjob.supportedparams"));
          return;
        }
  ...
  private boolean isValidReportParameter(
      ReportParameter reportParameter,
      String value,
    ...
    if (reportParameter.isDate()) {
      ...
      StockItemInventorySearchFilter.InventoryGroupBy result =
          StockItemInventorySearchFilter.InventoryGroupBy.findByName(value);
      if (result == null) {
        return false;
      }
