  ...
  public void validate(Object target, Errors errors) {
    ...
    if (StringUtils.isBlank(object.getStockItemUuid())) {
      errors.rejectValue(
          "stockItemUuid",
          String.format(
              messageSourceService.getMessage("stockmanagement.stockrule.fieldrequired"),
              "stock item"));
      return;
    } else {
      stockItem =
          Context.getService(StockManagementService.class)
              .getStockItemByUuid(object.getStockItemUuid());
      if (stockItem == null) {
        errors.rejectValue(
            "stockItemUuid",
            String.format(
                messageSourceService.getMessage("stockmanagement.stockrule.fieldvaluenotexist"),
                "stock item"));
        return;
      }
    }
    ...
    if (StringUtils.isBlank(object.getName())) {
      errors.rejectValue(
          "name",
          String.format(
              messageSourceService.getMessage("stockmanagement.stockrule.fieldrequired"), "name"));
      return;
    } else if (object.getName().length() > 255) {
      errors.rejectValue(
          "name",
          String.format(
              messageSourceService.getMessage("stockmanagement.stockrule.exceedslimit"),
              "name",
              255));
      return;
    }
    ...
    if (object.getQuantity() == null) {
      ...
      errors.rejectValue(
          "quantity",
          String.format(
              messageSourceService.getMessage("stockmanagement.stockrule.qtygreaterthanequalzero"),
              "quantity"));
      return;
    }
      ...
      if (stockItemPackagingUOM == null) {
        ...
        errors.rejectValue(
            ...
            messageSourceService.getMessage(
                "stockmanagement.stockrule.packagingunitinvalidforstockitem"));
        return;
      }
    }
