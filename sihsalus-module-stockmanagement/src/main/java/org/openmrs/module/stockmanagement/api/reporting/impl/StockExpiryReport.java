...
public class StockExpiryReport extends ReportGenerator {
  ...
  public void generateReport(BatchJob batchJob, Function<BatchJob, Boolean> shouldStopExecution) {
    ...
    try {
      ...
      while (hasMoreRecords) {
        ...
        java.util.List<StockBatchLineItem> rows =
            (data == null || data.getData() == null)
                ? java.util.Collections.emptyList()
                : data.getData();
        if (!rows.isEmpty()) {
          Map<Integer, List<StockBatchLineItem>> stockItemGroups =
              rows.stream().collect(Collectors.groupingBy(p -> p.getStockItemId()));
          int startIndex = 0;
          StockItemInventorySearchFilter inventorySearchFilter =
              new StockItemInventorySearchFilter();
          ...
          do {
            ...
            Map<Integer, List<StockItemInventory>> inventoryResult =
                stockManagementService
                    .getStockInventory(inventorySearchFilter, null)
                    .getData()
                    .stream()
                    .collect(Collectors.groupingBy(p -> p.getStockBatchId()));
            for (Map.Entry<Integer, List<StockBatchLineItem>> p : stockItemGroupsPage) {
              for (StockBatchLineItem stockBatchLineItem : p.getValue()) {
                List<StockItemInventory> stockItemInventory =
                    inventoryResult.get(stockBatchLineItem.getStockBatchId());
  ...
  protected void writeRow(
      CSVWriter csvWriter, StockBatchLineItem row, StockItemInventory inventory) {
    writeLineToCsv(
        csvWriter,
        ...
        emptyIfNull(row.getCommonName()),
        emptyIfNull(row.getAcronym()),
        emptyIfNull(row.getStockItemCategoryName()),
        emptyIfNull(inventory.getPartyName()),
        ...
        inventory.getQuantityUoM(),
        inventory.getQuantityFactor().toPlainString(),
