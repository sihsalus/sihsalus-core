...
public class StockItemInventoryReport<T extends StockItemInventory> extends ReportGenerator {
  ...
  public void generateReport(BatchJob batchJob, Function<BatchJob, Boolean> shouldStopExecution) {
    ...
    try {
      ...
      inventorySearchFilter.setInventoryGroupBy(
      ...
      if (!StringUtils.isBlank(locationUuid)) {
        ...
        if (childLocations != null && childLocations) {
          partyIds =
              stockManagementService.getCompletePartyList(location.getLocationId()).stream()
                  .map(p -> p.getId())
          ...
          if (party == null) {
            stockManagementService.failBatchJob(
                batchJob.getUuid(), "Report location party parameter not found");
            return;
          }
      ...
      boolean step1Complete =
          requireStockInventory(inventorySearchFilter, shouldStopExecution, parameters);
      if (abortReadingInventory > 0) {
        if (abortReadingInventory == 1) {
          return;
        } else if (!step1Complete) {
          stockManagementService.failBatchJob(
              batchJob.getUuid(), "Failed to fetch the stock inventory in step 1");
          return;
        }
      }
      ...
      if (step1File == null) {
        stockManagementService.failBatchJob(
            batchJob.getUuid(), "Stock inventory staging file was not initialized");
        return;
      }
      ...
      while ((inventoryLine = bufferedStagingReader.readLine()) != null) {
        ...
        if (includeConsumptionInfo()) {
          if (lineParts[4].length() > 0) {
            ((StockItemInventoryConsumption) stockItemInventory)
                .setClosingQuantity(new BigDecimal(lineParts[4]));
          }
          if (lineParts[5].length() > 0) {
            ((StockItemInventoryConsumption) stockItemInventory)
                .setQuantityConsumed(new BigDecimal(lineParts[5]));
          }
          if (lineParts[6].length() > 0) {
            ((StockItemInventoryConsumption) stockItemInventory)
                .setQuantityReceived(new BigDecimal(lineParts[6]));
          }
        }
  ...
  protected void preWriteBuffer(
      StockItemInventorySearchFilter inventorySearchFilter,
    ...
    if (!stockInventoryResult.getData().isEmpty()) {
      ...
      stockManagementService.postProcessInventoryResult(
          inventorySearchFilter, stockInventoryResult);
    }
  }

  private StockInventoryResult writeBuffer(
      BatchJob batchJob,
      StockItemInventorySearchFilter inventorySearchFilter,
      StockInventoryResult stockInventoryResult,
      boolean includeBatchInfo,
      boolean includeLocationInfo,
      Function<BatchJob, Boolean> shouldStopExecution)
      throws IOException {
  ...
  protected void writeRows(
      List<StockItemInventory> stockItemInventories,
      boolean includeBatchInfo,
      boolean includeLocationInfo)
      throws IOException {
  ...
  protected void writeRow(
    ...
    line[columnIndex++] =
        row.getReorderLevel() == null ? "" : row.getReorderLevel().toPlainString();
    line[columnIndex++] = emptyIfNull(row.getReorderLevelUoM());
    line[columnIndex++] =
        row.getReorderLevelFactor() == null ? "" : row.getReorderLevelFactor().toPlainString();
  ...
  protected void writeHeaders(
      CSVWriter csvWriter, boolean includeBatchInfo, boolean includeLocationInfo) {
    ...
    if (includeConsumptionInfo()) {
      headers[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.openingquantity");
      headers[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.quantityreceived");
      headers[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.quantityconsumed");
      headers[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.closingquantity");
      headers[columnIndex++] =
          messageSourceService.getMessage("stockmanagement.report.consumptionrate");
    } else {
    ...
    headers[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.reorderlevelunit");
    headers[columnIndex++] =
        messageSourceService.getMessage("stockmanagement.report.reorderlevelpacksize");
  ...
  protected boolean requireStockInventory(
    ...
    try {
      ...
      stockManagementService.getStockInventory(
          filter,
          null,
          inventory -> {
            if (shouldStopExecution.apply(batchJob)) {
            ...
            try {
              if (includeConsumptionInfo()) {
                StockItemInventoryConsumption consumption =
                    (StockItemInventoryConsumption) inventory;
                finalBufferedWriter.write(
                    String.format(
                        "%1$s,%2$s,%3$s,%4$s,%5$s,%6$s,%7$s%8$s",
                        consumption.getPartyId() == null ? "" : consumption.getPartyId().toString(),
                        consumption.getStockItemId() == null
                            ? ""
                            : consumption.getStockItemId().toString(),
                        consumption.getStockBatchId() == null
                            ? ""
                            : consumption.getStockBatchId().toString(),
                        consumption.getQuantity() == null
                            ? ""
                            : consumption.getQuantity().toPlainString(),
                        consumption.getClosingQuantity() == null
                            ? ""
                            : consumption.getClosingQuantity().toPlainString(),
                        consumption.getQuantityConsumed() == null
                            ? ""
                            : consumption.getQuantityConsumed().toPlainString(),
                        consumption.getQuantityReceived() == null
                            ? ""
                            : consumption.getQuantityReceived().toPlainString(),
                        System.lineSeparator()));
              } else {
                finalBufferedWriter.write(
                    String.format(
                        "%1$s,%2$s,%3$s,%4$s%5$s",
                        inventory.getPartyId() == null ? "" : inventory.getPartyId().toString(),
                        inventory.getStockItemId() == null
                            ? ""
                            : inventory.getStockItemId().toString(),
                        inventory.getStockBatchId() == null
                            ? ""
                            : inventory.getStockBatchId().toString(),
                        inventory.getQuantity() == null
                            ? ""
                            : inventory.getQuantity().toPlainString(),
                        System.lineSeparator()));
              }
  ...
  private void requireHeaders(boolean includeBatchInfo, boolean includeLocationInfo)
      throws IOException {
