...
public class StockRuleEvaluationJob extends AbstractTask {
  ...
  protected void executeInternal() {
    ...
    do {
      List<StockRuleNotificationUser> rulesFetched =
          stockManagementService.getDueStockRules(lastStockRuleId, batchSize);
    ...
    if (usersToNotify.isEmpty()
        || !usersToNotify.entrySet().stream().anyMatch(p -> p.getValue() != null)) {
    ...
    rules.removeIf(
        p -> {
          List<Integer> alertUsers =
              usersToNotify.getOrDefault(p.getAlertRoleLocationHashCode(), null);
          List<Integer> mailUsers =
              usersToNotify.getOrDefault(p.getMailRoleLocationHashCode(), null);
        ...
        null;
    {
      ...
      List<StockRuleCurrentQuantity> itemBalances =
          rules.stream()
              .filter(p -> p.getEnableDescendants() != null && p.getEnableDescendants())
              .map(
                  p ->
                      new StockRuleCurrentQuantity(
                          p.getStockItemId(), p.getLocationId(), BigDecimal.ZERO, true))
              .distinct()
      ...
      List<StockRuleCurrentQuantity> itemBalances2 =
          rules.stream()
              ...
              .map(
                  p ->
                      new StockRuleCurrentQuantity(
                          p.getStockItemId(), p.getLocationId(), BigDecimal.ZERO, false))
              .distinct()
              .collect(Collectors.toList());
      stockManagementService.setStockItemCurrentBalanceWithoutDescendants(itemBalances2);
      itemBalances.addAll(itemBalances2);
      stockItemLocationEnableDescendantsBalanceMap =
          itemBalances.stream()
              .collect(
                  Collectors.groupingBy(
                      StockRuleCurrentQuantity::getStockItemLocationEnableDescendantsHashCode));
    }
    ...
    processRulesWithEnoughQuantities(
        rules, stockItemLocationEnableDescendantsBalanceMap, stockManagementService);
    ...
    rules.stream()
        ...
        .forEach(
            (evaluationFrequency, rulesForFrequency) -> {
              updateStockRuleJobNextEvaluationDate(
                  rulesForFrequency, evaluationFrequency.intValue(), stockManagementService);
            });

    // update the rules next date a notification is to be resent
    rules.stream()
        .collect(Collectors.groupingBy(StockRuleNotificationUser::getActionFrequency))
        .forEach(
            (actionFrequency, rulesForFrequency) -> {
              updateStockRuleJobNextActionDate(
                  rulesForFrequency, actionFrequency.intValue(), stockManagementService);
            });
  }
  ...
  private void processMailNotifications(
    ...
    if (!SmtpUtil.hasSmptHostSetup()) {
      log.error(
          "Stock Rule Job: Not processing mail notifications since smtp settings are not setup");
      return;
    }
    ...
    try {
      List<Template> templates =
          Context.getMessageService().getTemplatesByName("STOCK_MGMT_QTY_BELOW_THRESHOLD");
    ...
    String mailMoreInfoMessage =
        Context.getMessageSourceService()
            .getMessage("stockmanagement.stockrule.mailmoreinfobelowthreshold");
    for (Map.Entry<Integer, List<Pair<Integer, Integer>>> userIdLocationRoleRuleMapping :
        userIdLocationRoleRuleMappings.entrySet()) {
      ...
      List<StockRuleNotificationUser> rulesToAlertTheUserAbout =
          rules.stream()
              .filter(
                  p ->
                      userIdLocationRoleRuleMapping.getValue().stream()
                          .anyMatch(x -> x.getValue1().equals(p.getMailRoleLocationHashCode())))
              .sorted(
                  Comparator.comparing(StockRuleNotificationUser::getLocationId)
                      .thenComparing(StockRuleNotificationUser::getStockItemId))
              .collect(Collectors.toList());
      ...
      for (StockRuleNotificationUser rule : rulesToAlertTheUserAbout) {
        ...
        if (itemCount > 1) {
          ...
          stringBuilder.append(
              String.format(
                  "<td style='padding: 0.2rem 0.5rem 0.2rem 0.5rem; font-size: 95%%;'>%1s %2s</td>",
                  NumberFormatUtil.qtyDisplayFormat(currentQuantity),
                  conceptNames.getOrDefault(rule.getPackagingConceptId(), "")));
          stringBuilder.append("</tr>");
        }
      ...
      String body =
          template
              ...
              .replace(
                  "%RECPT_NAME%",
                  String.format("%1s %2s", user.getFamilyName(), user.getGivenName()));
      try {
        SmtpUtil.sendEmail(
            session,
  ...
  private void ensureStockItemNamesLoaded(
      List<StockRuleNotificationUser> rules,
      Map<Integer, String> stockItemNames,
      StockManagementService stockManagementService) {
    List<Integer> stockItemNamesToFetch =
        rules.stream()
            .map(p -> p.getStockItemId())
            .distinct()
            .filter(p -> !stockItemNames.containsKey(p))
            .collect(Collectors.toList());
  ...
  private void ensureConceptNamesLoaded(
      List<StockRuleNotificationUser> rules,
      Map<Integer, String> conceptNames,
      StockManagementService stockManagementService) {
    List<Integer> conceptNamesToFetch =
        rules.stream()
            .map(p -> p.getPackagingConceptId())
            .distinct()
            .filter(p -> !conceptNames.containsKey(p))
            .collect(Collectors.toList());
  ...
  private void ensureLocationNamesLoaded(
      List<StockRuleNotificationUser> rules,
      Map<Integer, String> locationNames,
      StockManagementService stockManagementService) {
    List<Integer> locationNamesToFetch =
        rules.stream()
            .map(p -> p.getLocationId())
            .distinct()
            .filter(p -> !locationNames.containsKey(p))
            .collect(Collectors.toList());
    ...
    for (Map.Entry<Integer, List<Pair<Integer, Integer>>> userIdLocationRoleRuleMapping :
      ...
      List<StockRuleNotificationUser> rulesToAlertTheUserAbout =
          rules.stream()
              .filter(
                  p ->
                      userIdLocationRoleRuleMapping.getValue().stream()
                          .anyMatch(x -> x.getValue1().equals(p.getAlertRoleLocationHashCode())))
              .collect(Collectors.toList());
      if (rulesToAlertTheUserAbout.isEmpty()) continue;
      int maxAlerts = 4;
      if (rulesToAlertTheUserAbout.stream().limit(maxAlerts).anyMatch(p -> p.getAlert() == null)) {
        ensureStockItemNamesLoaded(
            rulesToAlertTheUserAbout, stockItemNames, stockManagementService);
        ensureConceptNamesLoaded(rulesToAlertTheUserAbout, conceptNames, stockManagementService);
        ensureLocationNamesLoaded(rulesToAlertTheUserAbout, locationNames, stockManagementService);
      }
      ...
      for (StockRuleNotificationUser rule : rulesToAlertTheUserAbout) {
        ...
        if (rule.getAlert() == null) {
          ...
          String alertLine =
              String.format(
                  "Stock alert: %1$s at %2$s currently %3$s %4$s at or below threshold %5$s %4$s",
                  stockItemNames.getOrDefault(rule.getStockItemId(), ""),
                  locationNames.getOrDefault(rule.getLocationId(), ""),
                  NumberFormatUtil.qtyDisplayFormat(currentQuantity),
                  conceptNames.getOrDefault(rule.getPackagingConceptId(), ""),
  ...
  private void processRulesWithEnoughQuantities(
      List<StockRuleNotificationUser> rules,
      Map<Integer, List<StockRuleCurrentQuantity>> stockItemLocationEnableDescendantsBalanceMap,
      StockManagementService stockManagementService) {
    List<StockRuleNotificationUser> rulesWithEnoughQuantites =
        rules.stream()
            .filter(
                p -> {
                  List<StockRuleCurrentQuantity> stockRuleCurrentQuantity =
                      stockItemLocationEnableDescendantsBalanceMap.getOrDefault(
                          p.getStockItemLocationEnableDescendantsHashCode(), null);
                  return stockRuleCurrentQuantity != null
                      && stockRuleCurrentQuantity.get(0).getQuantity().compareTo(p.getQuantity())
                          > 0;
                })
            .collect(Collectors.toList());
    ...
    rulesWithEnoughQuantites.stream()
        .collect(Collectors.groupingBy(StockRuleNotificationUser::getEvaluationFrequency))
        .forEach(
            (evaluationFrequency, rulesForFrequency) -> {
              updateStockRuleJobNextEvaluationDate(
  ...
  private void processRulesNoRecipientsAtAll(
      List<StockRuleNotificationUser> rules, StockManagementService stockManagementService) {
    rules.stream()
        .collect(Collectors.groupingBy(StockRuleNotificationUser::getEvaluationFrequency))
        .forEach(
            (evaluationFrequency, rulesForFrequency) -> {
  ...
  private void processRulesWithNoRecipients(
      List<StockRuleNotificationUser> rules,
      Map<Integer, List<Integer>> usersToNotify,
      StockManagementService stockManagementService) {
    rules.stream()
  ...
  private Map<Integer, List<Integer>> getUsersToBeNotified(
      List<StockRuleNotificationUser> rules, StockManagementService stockManagementService) {
    ...
    for (StockRuleNotificationUser stockRuleNotificationUser : rules) {
      if (stockRuleNotificationUser.getAlertRole() != null
          && !roleLocationUsersToNotify.containsKey(
              stockRuleNotificationUser.getAlertRoleLocationHashCode())) {
        roleLocationUsersToNotify.putIfAbsent(
            stockRuleNotificationUser.getAlertRoleLocationHashCode(),
            new Pair<>(
                stockRuleNotificationUser.getLocationId(),
                new ArrayList<>(Arrays.asList(stockRuleNotificationUser.getAlertRole()))));
      }

      if (stockRuleNotificationUser.getMailRole() != null
          && !roleLocationUsersToNotify.containsKey(
              stockRuleNotificationUser.getMailRoleLocationHashCode())) {
        roleLocationUsersToNotify.putIfAbsent(
            stockRuleNotificationUser.getMailRoleLocationHashCode(),
            new Pair<>(
                stockRuleNotificationUser.getLocationId(),
                new ArrayList<>(Arrays.asList(stockRuleNotificationUser.getMailRole()))));
      }
    }
    ...
    for (Map.Entry<Integer, Pair<Integer, List<String>>> entry :
        roleLocationUsersToNotify.entrySet()) {
      ...
      if (roleChain.containsKey(roleName)) {
        ...
        if (role == null || Boolean.TRUE.equals(role.getRetired())) {
          roleChain.putIfAbsent(roleName, null);
          result.put(entry.getKey(), null);
        } else {
          List<String> chain =
              role.getAllChildRoles().stream()
                  .filter(p -> !p.getRetired())
                  .map(p -> p.getRole())
                  .collect(Collectors.toList());
    ...
    List<Map.Entry<Integer, Pair<Integer, List<String>>>> recordsToFetch =
        roleLocationUsersToNotify.entrySet().stream()
            .filter(p -> !result.containsKey(p.getKey()))
            .collect(Collectors.toList());
    ...
    for (Map.Entry<Integer, Pair<Integer, List<String>>> recordToFetch : recordsToFetch) {
      if (recordToFetch.getValue().getValue2().isEmpty()) {
        ...
        List<Integer> usersAssigned =
            stockManagementService.getActiveUsersAssignedForScope(
                recordToFetch.getValue().getValue1(), recordToFetch.getValue().getValue2());
  ...
  private void updateStockRuleJobNextEvaluationDate(
      List<StockRuleNotificationUser> ruleNotificationUsers,
    ...
    do {
      ...
      stockManagementService.updateStockRuleJobNextEvaluationDate(
          batch, DateUtils.addMinutes(new Date(), evaluationFrequency));
      try {
        Thread.sleep(1000);
  ...
  private void updateStockRuleJobNextActionDate(
      List<StockRuleNotificationUser> ruleNotificationUsers,
      int actionFrequency,
      StockManagementService stockManagementService) {
    boolean hasMoreRecords = true;
    ...
    do {
      ...
      stockManagementService.updateStockRuleJobNextActionDate(
          batch, DateUtils.addMinutes(new Date(), actionFrequency));
      try {
        Thread.sleep(1000);
      } catch (InterruptedException e) {
        throw new RuntimeException(e);
