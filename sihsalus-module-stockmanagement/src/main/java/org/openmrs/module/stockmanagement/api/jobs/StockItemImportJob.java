...
public class StockItemImportJob {
  ...
  private Object validateLine(String[] line) {
    ...
    if (line.length < 3) {
      errors.add(
          Context.getMessageSourceService()
              .getMessage("stockmanagement.importoperation.minimumfields"));
      return errors;
    }
    if (!isBlank(line[DRUG_ID])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.drugidpositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.drugidnumber"));
      }
    }
    ...
    if (objects[DRUG_ID] == null && !isBlank(line[CONCEPT_ID])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.conceptidpositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.conceptidnumber"));
      }
    } else if (objects[DRUG_ID] == null) {
      errors.add(
          Context.getMessageSourceService()
              .getMessage("stockmanagement.importoperation.drugconceptidrequired"));
    }
    ...
    if (!isBlank(line[EXPIRES])) {
      ...
      if (value != null) {
        ...
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.expiresinvalid"));
      }
    }
    ...
    if (!isBlank(line[EXPIRY_NOTICE])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.expirynoticepositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.expirynoticenumber"));
      }
    }
    ...
    if (!isBlank(line[CATEGORY])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.categorypositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.categorynumber"));
      }
    }
    ...
    if (line.length > DISPENSING_UNIT && !isBlank(line[DISPENSING_UNIT])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.dispenseunitpositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.dispenseunitnumber"));
      }
    }
    ...
    if (line.length > DISPENSING_PUOM && !isBlank(line[DISPENSING_PUOM])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.dispensepuompositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.dispensepuomnumber"));
      }
    }

    if (line.length > PACK_SIZE_FOR_DISPENSING_PUOM
        && !isBlank(line[PACK_SIZE_FOR_DISPENSING_PUOM])) {
      try {
        ...
        if (value.compareTo(BigDecimal.ZERO) > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.packsizepositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.packsizenumber"));
      }
    }
    ...
    if (objects[PACK_SIZE_FOR_DISPENSING_PUOM] != null && objects[DISPENSING_PUOM] == null) {
      errors.add(
          Context.getMessageSourceService()
              .getMessage("stockmanagement.importoperation.packsizeeanddispensinguomrequired"));
    }
    ...
    if (line.length > REORDER_LEVEL && !isBlank(line[REORDER_LEVEL])) {
      try {
        ...
        if (value.compareTo(BigDecimal.ZERO) >= 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.reorderlevelpositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.reorderlevelnumber"));
      }
    }
    ...
    if (line.length > REORDER_LVEL_PUOM && !isBlank(line[REORDER_LVEL_PUOM])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.reorderleveluompositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.reorderleveluomnumber"));
      }
    }
    ...
    if (line.length > PURCHASE_PRICE && !isBlank(line[PURCHASE_PRICE])) {
      try {
        ...
        if (value.compareTo(BigDecimal.ZERO) >= 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.purchasepricepositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.purchasepricenumber"));
      }
    }
    ...
    if (line.length > PURCHASE_PRICE_PUOM && !isBlank(line[PURCHASE_PRICE_PUOM])) {
      try {
        ...
        if (value > 0) {
          ...
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.purchasepriceuompositive"));
        }
      } catch (Exception ex) {
        errors.add(
            Context.getMessageSourceService()
                .getMessage("stockmanagement.importoperation.purchasepriceuomnumber"));
      }
    }

    if ((objects[PURCHASE_PRICE] != null || objects[PURCHASE_PRICE_PUOM] != null)
        && (objects[PURCHASE_PRICE] == null || objects[PURCHASE_PRICE_PUOM] == null)) {
      errors.add(
          Context.getMessageSourceService()
              .getMessage("stockmanagement.importoperation.purchasepriceanduomrequired"));
    }
  ...
  private void updateStockItems(Map<Integer, Object[]> stockItems) {
    ...
    List<StockItemDTO> stockItemIds =
        stockManagementService.getExistingStockItemIds(
            stockItems.values().stream()
                .map(
                    p -> {
                      StockItemSearchFilter.ItemGroupFilter itemGroupFilter =
                          new StockItemSearchFilter.ItemGroupFilter();
                      if (p[DRUG_ID] != null) {
    ...
    Map<Object, List<Object[]>> rowsToUpdate =
                ...
                p ->
                    new Object[] {
                      ...
                      stockItemIds.stream()
                          .filter(
                              x ->
                                  (p.getValue()[DRUG_ID] != null
                                          && p.getValue()[DRUG_ID].equals(x.getDrugId()))
                                      || (p.getValue()[DRUG_ID] == null
                                          && x.getDrugId() == null
                                          && p.getValue()[CONCEPT_ID].equals(x.getConceptId())))
                          .findFirst()
                    })
            .filter(p -> ((Optional<?>) p[1]).isPresent())
            .collect(Collectors.groupingBy(p -> p[0]));

    List<Map.Entry<Integer, Object[]>> newStockItems =
        stockItems.entrySet().stream()
            .filter(p -> !rowsToUpdate.containsKey(p.getKey()))
    ...
    if (!newStockItems.isEmpty()) {
      ...
      for (Map.Entry<Integer, Object[]> entryToCreate : newStockItems) {
        ...
        if (entryToCreate.getValue()[EXPIRES] == null) {
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.expiresrequired"));
        }
        if (entryToCreate.getValue()[DISPENSING_UNIT] == null) {
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.dispenseunitrequired"));
        }
        if (entryToCreate.getValue()[DISPENSING_PUOM] == null) {
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.dispensepuomrequired"));
        }
        if (entryToCreate.getValue()[PACK_SIZE_FOR_DISPENSING_PUOM] == null) {
          errors.add(
              Context.getMessageSourceService()
                  .getMessage("stockmanagement.importoperation.packsizerequired"));
        }
        if (!errors.isEmpty()) {
          result
              .getErrors()
              .add(
                  String.format("Row %1s: %2s", entryToCreate.getKey(), String.join(", ", errors)));
          continue;
        }
    ...
    Map<Integer, List<Drug>> drugs =
        stockManagementService.getDrugs(drugIds).stream()
            .collect(Collectors.groupingBy(Drug::getDrugId));
    Map<Integer, List<Concept>> concepts =
        stockManagementService.getConcepts(conceptIds).stream()
    ...
    if (!rowsToUpdate.isEmpty()) {
      stockItemsToUpdate =
          stockManagementService
              .getStockItems(stockItemIds.stream().map(p -> p.getId()).collect(Collectors.toList()))
              .stream()
              .collect(Collectors.groupingBy(StockItem::getId));

      // Prefetch the stockitempackaging uoms
      stockItemPackagingUoms =
          stockManagementService
              .getStockItemPackagingUOMs(
                  rowsToUpdate.entrySet().stream()
                      .map(
                          p -> {
                            Integer stockItemId =
                                ((Optional<StockItemDTO>) (p.getValue().get(0)[1])).get().getId();
                            Object[] updates = stockItems.get(p.getKey());
                            List<Integer> conceptIdsForUom = new ArrayList<Integer>();
                            if (DISPENSING_PUOM < updates.length
                            ...
                            if (PURCHASE_PRICE_PUOM < updates.length
                                && updates[PURCHASE_PRICE_PUOM] != null) {
                              if (conceptIdsForUom.isEmpty()
                                  || (!conceptIdsForUom.get(0).equals(updates[PURCHASE_PRICE_PUOM])
                                      && (conceptIdsForUom.size() < 2
                                          || !conceptIdsForUom
                                              .get(1)
                                              .equals(updates[PURCHASE_PRICE_PUOM])))) {
                                conceptIdsForUom.add((Integer) updates[PURCHASE_PRICE_PUOM]);
                              }
                            }
                            if (conceptIdsForUom.isEmpty()) return null;
                            return new StockItemPackagingUOMSearchFilter.ItemGroupFilter(
                                stockItemId, conceptIdsForUom);
                          })
                      .filter(p -> p != null)
                      .collect(Collectors.toList()))
              .stream()
    ...
    for (Map.Entry<Integer, Object[]> recordToProcess : stockItems.entrySet()) {
      ...
      if (rowsToUpdate.containsKey(recordToProcess.getKey())) {
        ...
        Optional<Map.Entry<Integer, Object[]>> rowFound =
            rowsToCreate.stream()
                .filter(p -> p.getKey().equals(recordToProcess.getKey()))
                .findFirst();
              ...
              result
                  ...
                  .add(
                      String.format(
                          ...
                          String.format(
                              Context.getMessageSourceService()
                                  .getMessage("stockmanagement.importoperation.drugnofound"),
      ...
      if (updates[EXPIRES] != null
          && !updates[EXPIRES].equals(Boolean.valueOf(stockItem.getHasExpiration()))) {
        expiresToSet = (Boolean) updates[EXPIRES];
      }

      if (updates[EXPIRY_NOTICE] != null
          && (stockItem.getExpiryNotice() == null
              || !updates[EXPIRY_NOTICE].equals(stockItem.getExpiryNotice()))) {
      ...
      if (DISPENSING_UNIT < updates.length && updates[DISPENSING_UNIT] != null) {
        ...
        if (stockItem.getDispensingUnit() == null
            || !stockItem
                .getDispensingUnit()
                .getUuid()
                .equals(conceptCollection.get(0).getUuid())) {
      ...
      if (DISPENSING_PUOM < updates.length && updates[DISPENSING_PUOM] != null) {
        ...
        if (PACK_SIZE_FOR_DISPENSING_PUOM < updates.length
            && updates[PACK_SIZE_FOR_DISPENSING_PUOM] != null) {
        ...
        if (dispensingStockItemPackagingUOM != null) {
          if (packSize != null
              && packSize.compareTo(dispensingStockItemPackagingUOM.getFactor()) != 0) {
            ...
            result
                ...
                .add(
                    String.format(
                        ...
                        Context.getMessageSourceService()
                            .getMessage(
                                "stockmanagement.importoperation.packsizeeanddispensinguomrequired")));
            continue;
          }
      ...
      if (PREFERRED_VENDOR < updates.length && updates[PREFERRED_VENDOR] != null) {
          ...
          result
              ...
              .add(
                  String.format(
                      ...
                      recordToProcess.getKey(),
                      Context.getMessageSourceService()
                          .getMessage("stockmanagement.importoperation.exceeds255")));
          continue;
        }

        String cacheKey =
            NON_ASCII_PATTERN.matcher(preferredVendorName).replaceAll("").toLowerCase();
        if (isBlank(cacheKey)) {
        ...
        if (stockSource == null) {
          ...
          List<StockSource> stockSourcesInDb =
              stockManagementService.findStockSources(filter).getData();
            ...
            result
                ...
                .add(
                    String.format(
                        ...
                        Context.getMessageSourceService()
                            .getMessage(
                                "stockmanagement.importoperation.unknownconceptnofoundstocksource")));
            continue;
          }
        ...
        if (stockItem.getPreferredVendor() == null
            || !stockItem.getPreferredVendor().getUuid().equals(stockSource.getUuid())) {
      ...
      if (REORDER_LVEL_PUOM < updates.length && updates[REORDER_LVEL_PUOM] != null) {
        ...
        if (stockItem.getReorderLevelUOM() == null
            || !stockItem.getReorderLevelUOM().getUuid().equals(uom.getUuid())) {
          reorderLevelUomToSet = uom;
        }

        if (REORDER_LEVEL < updates.length && updates[REORDER_LEVEL] != null) {
          if (stockItem.getReorderLevel() == null
              || stockItem.getReorderLevel().compareTo((BigDecimal) updates[REORDER_LEVEL]) != 0) {
      ...
      if (PURCHASE_PRICE_PUOM < updates.length && updates[PURCHASE_PRICE_PUOM] != null) {
        ...
        if (uom == null) {
          if (updates[DISPENSING_PUOM] != null
              && updates[DISPENSING_PUOM].equals(updates[PURCHASE_PRICE_PUOM])) {
            uom = dispensingStockItemPackagingUOM;
          } else {
            result
                .getErrors()
        ...
        if (stockItem.getPurchasePriceUoM() == null
            || !stockItem.getPurchasePriceUoM().getUuid().equals(uom.getUuid())) {
          purchasePriceUomToSet = uom;
        }

        if (PURCHASE_PRICE < updates.length && updates[PURCHASE_PRICE] != null) {
          if (stockItem.getPurchasePrice() == null
              || stockItem.getPurchasePrice().compareTo((BigDecimal) updates[PURCHASE_PRICE])
                  != 0) {
      ...
      if (packSizeToUpdate != null) {
        ...
        dispensingStockItemPackagingUOM.setChangedBy(Context.getAuthenticatedUser());
      }
  ...
  private String normalizedCacheKey(String value) {
    if (StringUtils.isBlank(value)) {
      return "";
    }
    String cacheKey = NON_ASCII_PATTERN.matcher(value).replaceAll("").toLowerCase();
    return isBlank(cacheKey) ? value.toLowerCase() : cacheKey;
  }
      ...
      try (Writer writer =
        ...
        if (hasErrors) {
          ...
          result
              ...
              .add(
                  Context.getMessageSourceService()
                      .getMessage("stockmanagement.importoperation.errorswhileimporting"));
