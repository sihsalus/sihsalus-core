...
public class StockBatchExpiryJob extends AbstractTask {
  ...
  protected void executeInternal() {
    ...
    Integer defaultExpNotificationPeriod =
        GlobalProperties.getStockBatchDefaultExpiryNotificationNoticePeriod();
    Map<Integer, StockBatchDTO> stockBatchNumbers =
        stockManagementService
            .getExpiringStockBatchesDueForNotification(defaultExpNotificationPeriod)
            .stream()
    ...
    List<Integer> mainStoreLocationIds = new ArrayList<>();
    {
      ...
      LocationTag mainStore =
          locationService.getLocationTagByName(StockLocationTags.MAIN_STORE_LOCATION_TAG);
      if (mainStore != null) {
        List<Location> mainStoreLocations =
    ...
    Map<Integer, List<Integer>> usersToNotify =
        getUsersToBeNotified(
            mainStoreLocationIds, partyDTOLocationMap, stockBatchBalance, stockManagementService);
    if (usersToNotify.isEmpty()
        || usersToNotify.entrySet().stream().allMatch(p -> p.getValue().isEmpty())) {
      log.debug("No person to notify for the stock batch number that are expiring");
      return;
    }
    partyDTOLocationMap
        .entrySet()
        .forEach(p -> locationPartyDTOMap.putIfAbsent(p.getValue().getLocationId(), p.getValue()));

    log.debug("Processing alerts");
    Map<Pair<Integer, Integer>, List<StockItemInventory>> partyStockItemMap =
        stockBatchBalance.entrySet().stream()
            .map(p -> p.getValue())
            .flatMap(Collection::stream)
            .collect(
                Collectors.groupingBy(
                    p -> new Pair<Integer, Integer>(p.getPartyId(), p.getStockItemId())));
  ...
  private Map<Integer, List<Pair<Integer, Integer>>> processAlerts(
      Map<Integer, List<StockItemInventory>> stockBatchBalance,
      Map<Integer, StockBatchDTO> stockBatchNumbers,
      ...
      StockManagementService stockManagementService) {
    Map<Integer, List<Pair<Integer, Integer>>> userLocationsToNotify =
        locationUsersToNotify.entrySet().stream()
            .map(
                p -> {
                  return p.getValue().stream().map(x -> new Pair<Integer, Integer>(x, p.getKey()));
                })
            .flatMap(Function.identity())
            .collect(Collectors.groupingBy(p -> p.getValue1()));
    ensureStockItemNamesLoaded(partyStockItemMap, stockItemNames, stockManagementService);
    Map<Integer, Alert> alertMap = new HashMap<>();
    for (Map.Entry<Integer, List<Pair<Integer, Integer>>> userLocationToNotify :
        userLocationsToNotify.entrySet()) {
      List<Integer> partyIds =
          userLocationToNotify.getValue().stream()
              .map(
                  p -> {
      ...
      List<Map.Entry<Pair<Integer, Integer>, List<StockItemInventory>>> userPartyStockItemMap =
          partyStockItemMap.entrySet().stream()
              .filter(p -> partyIds.contains(p.getKey().getValue1()))
              .collect(Collectors.toList());
      if (userPartyStockItemMap.isEmpty()) {
        // Check if user has permissions to main store. They automatically get access to every other
        // location
        if (userLocationToNotify.getValue().stream()
            .anyMatch(p -> mainStoreLocationIds.contains(p.getValue2()))) {
          userPartyStockItemMap =
              partyStockItemMap.entrySet().stream().collect(Collectors.toList());
        } else {
          continue;
        }
      }
      ...
      for (Map.Entry<Pair<Integer, Integer>, List<StockItemInventory>> partyStockItems :
          userPartyStockItemMap) {
        ...
        for (StockItemInventory stockItemInventory : partyStockItems.getValue()) {
          if (maxAlerts == 0) break;
          Alert alert =
              alertMap.getOrDefault(stockItemInventory.getPartyStockItemBatchHashCode(), null);
          if (alert == null) {
            StockBatchDTO stockBatchDTO =
                stockBatchNumbers.getOrDefault(stockItemInventory.getStockBatchId(), null);
            if (stockBatchDTO == null) continue;
            String alertLine =
                String.format(
                    "Batch %1$s for %2$s at %3$s with %4$s %5$s expires on %6$s",
  ...
  private boolean processMailNotifications(
      Map<Integer, List<StockItemInventory>> stockBatchBalance,
      Map<Integer, StockBatchDTO> stockBatchNumbers,
      Map<Integer, List<Integer>> locationUsersToNotify,
      List<Integer> mainStoreLocationIds,
    ...
    if (!SmtpUtil.hasSmptHostSetup()) {
      log.error(
          "Stock Batch Job: Not processing mail notifications since smtp settings are not setup");
      return false;
    }
    ...
    try {
      List<Template> templates =
          Context.getMessageService().getTemplatesByName("STOCK_MGMT_BATCH_EXPIRING");
    ...
    String mailMoreInfoMessage =
        Context.getMessageSourceService()
            .getMessage("stockmanagement.stockrule.mailmoreinfoexpiring");
    boolean sentAtleastOneEmail = false;

    List<Integer> usersWithMainStorePermissions =
        userLocationsToNotify.entrySet().stream()
            .filter(
                p ->
                    p.getValue().stream()
                        .anyMatch(x -> mainStoreLocationIds.contains(x.getValue2())))
            .map(p -> p.getKey())
            .distinct()
            .collect(Collectors.toList());

    boolean hasProcessedMainStore = false;
    for (Map.Entry<Integer, List<Pair<Integer, Integer>>> userIdLocationMapping :
        userLocationsToNotify.entrySet()) {
      ...
      List<Integer> partyIds =
          isProcessingAllLocations
              ? null
              : userIdLocationMapping.getValue().stream()
                  .map(
                      p -> {
                        PartyDTO partyDTO = locationPartyDTOMap.get(p.getValue2());
                        return partyDTO != null ? partyDTO.getId() : null;
                  ...
                  .filter(p -> p != null)
                  .collect(Collectors.toList());
      Collection<Map.Entry<Pair<Integer, Integer>, List<StockItemInventory>>>
          userPartyStockItemMap =
              isProcessingAllLocations
                  ? partyStockItemMap.entrySet()
                  : partyStockItemMap.entrySet().stream()
                      .filter(p -> partyIds.contains(p.getKey().getValue1()))
                      .collect(Collectors.toList());
      if (userPartyStockItemMap.isEmpty()) {
      ...
      int locationCount = 0;
      // Locations inventory
      for (Map.Entry<Integer, List<Map.Entry<Pair<Integer, Integer>, List<StockItemInventory>>>>
          partyItems :
              userPartyStockItemMap.stream()
                  .collect(Collectors.groupingBy(p -> p.getKey().getValue1()))
                  .entrySet()) {
        if (itemCount == 100) {
          hasMore = true;
          break;
        ...
        for (Map.Entry<Pair<Integer, Integer>, List<StockItemInventory>> stockItem :
          ...
          for (StockItemInventory batchNumber : stockItem.getValue()) {
            ...
            StockBatchDTO stockBatchDTO =
                stockBatchNumbers.getOrDefault(batchNumber.getStockBatchId(), null);
            if (stockBatchDTO == null) continue;
      ...
      try {
        SmtpUtil.sendEmail(
            ...
            body,
            session,
            receipients.toArray(new String[receipients.size()]));
  ...
  private void ensureStockItemNamesLoaded(
    ...
    List<Integer> stockItemNamesToFetch =
        partyStockItemMap.keySet().stream()
            .map(p -> p.getValue2())
            .distinct()
  ...
  private Map<Integer, List<Integer>> getUsersToBeNotified(
      List<Integer> mainStoreLocationIds,
      Map<Integer, PartyDTO> partyLocationDTOMap,
      Map<Integer, List<StockItemInventory>> stockBatchBalances,
      StockManagementService stockManagementService) {
    ...
    if (rolesWithPrivileges.isEmpty()) {
      rolesWithPrivileges =
          userService.getAllRoles().stream()
              .filter(p -> p.hasPrivilege(Privileges.TASK_STOCKMANAGEMENT_STOCKOPERATIONS_MUTATE))
              .collect(Collectors.toList());
    }
    if (rolesWithPrivileges.isEmpty()) {
      return new HashMap<>();
    ...
    List<Integer> partyIds =
        stockBatchBalances.entrySet().stream()
            .map(p -> p.getValue())
            .flatMap(Collection::stream)
            .map(p -> p.getPartyId())
            .distinct()
    ...
    locationsToFetch.addAll(
        partyDTOList.stream().map(p -> p.getLocationId()).collect(Collectors.toList()));
    for (Integer recordToFetch : locationsToFetch) {
      if (!result.containsKey(recordToFetch)) {
        List<Integer> usersAssigned =
            stockManagementService.getActiveUsersAssignedForScope(recordToFetch, rolesToUse);
  ...
  private void removeItemsWithoutStock(
      Map<Integer, StockBatchDTO> stockBatchDTOs,
      Map<Integer, List<StockItemInventory>> stockBatchBalance,
      StockManagementService stockManagementService) {
    ...
    do {
      ...
      List<StockItemInventory> inventory =
          stockManagementService.getStockBatchLocationInventory(
              batch.stream().map(p -> p.getId()).collect(Collectors.toList()));
      Map<Integer, List<StockItemInventory>> batchInventories =
          inventory.stream().collect(Collectors.groupingBy(StockItemInventory::getStockBatchId));
      List<StockBatchDTO> toRemove = new ArrayList<>();
      for (StockBatchDTO stockBatchDTO : batch) {
        List<StockItemInventory> batchInventory =
            batchInventories.getOrDefault(stockBatchDTO.getId(), null);
