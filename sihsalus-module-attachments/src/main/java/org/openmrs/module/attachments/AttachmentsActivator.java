...
public class AttachmentsActivator extends BaseModuleActivator {
  ...
  private static final String DEFAULT_CONCEPT_COMPLEX_NAME =
      AttachmentsConstants.MODULE_SHORT_ID + " DEFAULT ATTACHMENT";

  private static final String DEFAULT_CONCEPT_COMPLEX_DESCRIPTION =
      "Concept complex for 'default attachment' complex obs.";

  private static final String IMAGE_CONCEPT_COMPLEX_NAME =
      AttachmentsConstants.MODULE_SHORT_ID + " IMAGE ATTACHMENT";

  private static final String IMAGE_CONCEPT_COMPLEX_DESCRIPTION =
      "Concept complex for 'image attachments with thumbnails' complex obs.";
  ...
  public void started() {
    ...
    // Encounter Type
    {
      ...
      final String uuid =
          getConfiguredUuidOrDefault(
              AttachmentsConstants.GP_ENCOUNTER_TYPE_UUID,
  ...
  ConceptComplexDefinition getDefaultAttachmentConceptDefinition() {
    return new ConceptComplexDefinition(
        ...
        getGlobalPropertyOrDefault(
            AttachmentsConstants.GP_DEFAULT_CONCEPT_COMPLEX_NAME, DEFAULT_CONCEPT_COMPLEX_NAME),
        getGlobalPropertyOrDefault(
  ...
  ConceptComplexDefinition getImageAttachmentConceptDefinition() {
    ...
    return new ConceptComplexDefinition(
        validateUuid(
            AttachmentsConstants.GP_CONCEPT_COMPLEX_UUID_MAP + "[" + ContentFamily.IMAGE + "]",
            imageConceptUuid),
        getGlobalPropertyOrDefault(
            AttachmentsConstants.GP_IMAGE_CONCEPT_COMPLEX_NAME, IMAGE_CONCEPT_COMPLEX_NAME),
        getGlobalPropertyOrDefault(
            AttachmentsConstants.GP_IMAGE_CONCEPT_COMPLEX_DESCRIPTION,
            IMAGE_CONCEPT_COMPLEX_DESCRIPTION),
        ImageAttachmentHandler.class.getSimpleName());
  }

  private void ensureConceptComplex(
      ConceptService conceptService, ConceptComplexDefinition definition) {
    Concept concept = conceptService.getConceptByUuid(definition.getUuid());
    if (concept == null) {
      throw new APIException(
          "Configured attachment concept "
              + definition.getUuid()
  ...
  private Map<String, String> getConceptComplexUuidMap() {
    ...
    try {
      ...
      log.warn(
          "Could not parse global property '"
              + AttachmentsConstants.GP_CONCEPT_COMPLEX_UUID_MAP
              + "'. Falling back to the built-in attachment concept UUIDs.",
          e);
      return Collections.emptyMap();
    }
  }
  ...
  private String validateUuid(String propertyName, String uuid) {
    try {
      ...
      throw new APIException(
          "Global property " + propertyName + " must be a valid UUID: " + uuid, e);
    }
    return uuid;
  }
