...
/**
 * Helper class that can be used to convert the "valueComplex" field of an obs into its parts.
 *
 * <p>Basically, in Core, the valueComplex field has always beeb stored in the following pattern:
 *
 * <p>"filename (image|file|null) |path-to-file"
 *
 * <p>for example: "Printed Patient Summary.pdf file |Printed Patient Summary.pdf"
 *
 * <p>Traditionally (prior to making Attachments Core 2.8 compatible), Attachments morphed this into
 * pattern: "m3ks | instructions.default | application/pdf | Printed Patient Summary.pdf"
 *
 * <p>(When "m3ks" is a unique prefix that tells the handler that this is an Attachments-specific
 * valueComplex)
 *
 * <p>This worked prior to Core 2.8, because the filename always (I believe) equals the
 * path-to-file. However, in Core 2.8, the filename is no longer equal to the path-to-file, so we
 * now need to store both in the Attachments-specific valueComplex.
 *
 * <p>The new format we use is:
 *
 * <p>"m3ks | instructions.default | application/pdf | Printed Patient Summary.pdf
 * |/some/path/Printed Patient Summary.pdf"
 *
 * <p>(Note that we still remove the "file" or "image" suffix from the filename field, as we now
 * have a mime-type field)
 */
public class ValueComplex {

  public static final String INSTRUCTIONS_NONE = AttachmentsConstants.INSTRUCTIONS_PREFIX + ".none";

  public static final String INSTRUCTIONS_DEFAULT =
      AttachmentsConstants.INSTRUCTIONS_PREFIX + ".default";

  protected static final String UNIQUE_PREFIX = "m3ks";
  ...
  // filename from key
  protected static final String PIPE_WITH_LEADING_AND_TRAILING_SPACE = " | ";
  protected static final String PIPE_WITH_LEADING_SPACE = " |";
  protected static final Pattern PIPE_WITH_LEADING_SPACE_AND_NO_TRAILING_SPACE_REGEX =
      Pattern.compile(" \\|\\S");
  protected static final String SPACE = " ";

  protected static final String IMAGE_FILE_TYPE = "image";
  protected static final String BINARY_DATA_FILE_TYPE = "file";
  ...
  public ValueComplex(String valueComplex) {
    ...
    if (!StringUtils.substringBefore(valueComplex, PIPE_WITH_LEADING_AND_TRAILING_SPACE)
        .equals(UNIQUE_PREFIX)) {
      this.instructions = INSTRUCTIONS_NONE;
      return;
    }

    String metaData =
        StringUtils.substringAfter(valueComplex, PIPE_WITH_LEADING_AND_TRAILING_SPACE);
  ...
  public static String buildValueComplex(
      String instructions, String mimeType, String savedFileName, String key) {
    if (StringUtils.isBlank(key)) {
      // old format, prior to Core 2.8, no key, just filename
      return UNIQUE_PREFIX
          + PIPE_WITH_LEADING_AND_TRAILING_SPACE
          + instructions
          + PIPE_WITH_LEADING_AND_TRAILING_SPACE
          + mimeType
          + PIPE_WITH_LEADING_AND_TRAILING_SPACE
