...
public class UpdateScheduler {
  ...
  public synchronized void schedule(Subscription subscription) {
    ...
    if (subscription.getDays() != 0
        || subscription.getHours() != 0
        || subscription.getMinutes() != 0) {
      ...
      scheduledUpdate =
          scheduler.scheduleAtFixedRate(
              importer, calendar.getTime(), subscription.getDays() * DAY_PERIOD);
    }
  }
  ...
  public void scheduleNow() {
    if (importer.isRunning()) {
      throw new IllegalStateException(
          "Cannot start the update, if there is another update in progress.");
    }
