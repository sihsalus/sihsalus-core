...
public class Saver {
   ...
   */
  public Item saveConcept(
      final CacheService cacheService, final Import anImport, final OclConcept oclConcept)
      throws ImportException {
    ...
    while (true) {
      try {
        try {
          ...
          resolutionLog.add(
              "Fixing duplicate names for concept "
                  + concept.getUuid()
                  + " after failure due to "
                  + e.getMessage());
  ...
  public Concept toConcept(CacheService cacheService, OclConcept oclConcept)
      throws ImportException {
    ...
    if (datatype == null) {
      throw new ImportException(
          "Datatype '" + oclConcept.getDatatype() + "' is not supported by OpenMRS");
    }
    ...
    if (concept == null) {
      concept =
          cacheService.getConceptWithSameAsMapping(oclConcept.getSource(), oclConcept.getId());
    }
    ...
    for (OclConcept.Name oclName : oclConcept.getNames()) {
      if (oclName.getNameType() != null) {
        switch (oclName.getNameType().toUpperCase(Locale.ENGLISH).replaceAll("[\\-_]", " ")) {
          ...
          default:
            log.warn(
                "Found unknown name type {} for concept {}. Treating as synonym.",
                oclName.getNameType(),
                oclConcept.getVersionUrl());
            oclName.setNameType(null);
            break;
        }
      }
   ...
   */
  public Item saveMapping(
      final CacheService cacheService, final Import update, final OclMapping oclMapping)
      throws ImportException {
    try {
      Item oldMappingItem =
          cacheService.getLastSuccessfulItemByUrl(oclMapping.getUrl(), importService);
      ...
      if (MapType.Q_AND_A.equals(oclMapping.getMapType())
          || MapType.SET.equals(oclMapping.getMapType())) {
        if (fromConcept == null) {
          throw new SavingException(
              "Cannot create mapping for "
                  + oclMapping.getUrl()
                  + " as no from concept is"
                  + " defined");
        }
        ...
        if (toConcept == null) {
          throw new SavingException(
              "Cannot create mapping " + oclMapping.getUrl() + " as no to concept is defined");
        }
        ...
        if (toSource == null) {
          if (StringUtils.isBlank(oclMapping.getToSourceName())) {
            throw new SavingException(
                "Cannot create mapping " + oclMapping.getUrl() + " as no to source is defined");
          }
        ...
        if (fromConcept != null) {
          ...
          ConceptReferenceTerm term =
              cacheService.getConceptReferenceTermByCode(oclMapping.getToConceptCode(), toSource);
          if (term == null) {
          ...
          // Otherwise, if no existing mapping is found, create if the incoming mapping is not
          // retired
          else {
  ...
  public boolean isMappingUpToDate(Item oldItem, OclMapping newMapping) {
    boolean updateDatesMatch =
        OpenmrsUtil.nullSafeEquals(oldItem.getUpdatedOn(), newMapping.getUpdatedOn());
    boolean retiredMatch = newMapping.isRetired() == (oldItem.getState() == ItemState.RETIRED);
    return updateDatesMatch && retiredMatch;
  }

  Item updateOrAddSetMembersFromOcl(
      Import update, OclMapping oclMapping, Concept set, Concept member) {
    Item item = null;
  ...
  private Item updateOrAddAnswersFromOcl(
      Import update, OclMapping oclMapping, Concept question, Concept answer) {
    Item item = null;
  ...
  private void updateOrAddNamesFromOcl(Concept concept, OclConcept oclConcept) {
    ...
    for (OclConcept.Name oclName : sortedNames(oclConcept.getNames())) {
      ...
      try {
        oclNameType =
            StringUtils.isNotBlank(oclName.getNameType())
      ...
      if (!nameFound) {
        ...
        if (StringUtils.isNotEmpty(oclName.getExternalId())) {
          ...
          name.setUuid(
              version5Uuid(oclConcept.getUrl() + "/names/" + oclName.getUuid()).toString());
        }
        name.setConceptNameType(oclNameType);
        name.setLocalePreferred(oclName.isLocalePreferred());
  ...
  private List<OclConcept.Name> sortedNames(List<OclConcept.Name> names) {
    ...
    Collections.sort(
        sortedNames,
        new Comparator<OclConcept.Name>() {
          @Override
          public int compare(OclConcept.Name name0, OclConcept.Name name1) {
            ConceptNameType nameType0 =
                StringUtils.isNotBlank(name0.getNameType())
                    ? ConceptNameType.valueOf(name0.getNameType())
                    : null;
            ConceptNameType nameType1 =
                StringUtils.isNotBlank(name1.getNameType())
                    ? ConceptNameType.valueOf(name1.getNameType())
                    : null;
      ...
      for (Description oclDescription : oclConcept.getDescriptions()) {
        ...
        if (!descriptionFound) {
          ConceptDescription description =
              new ConceptDescription(oclDescription.getDescription(), oclDescription.getLocale());
          if (oclDescription.getExternalId() != null) {
            description.setUuid(oclDescription.getExternalId());
          } else {
            description.setUuid(
                version5Uuid(oclConcept.getUrl() + "/descriptions/" + oclDescription.getUuid())
                    .toString());
          }
