...
public class OpenConceptLabActivator extends BaseModuleActivator implements DaemonTokenAware {
  ...
  public void contextRefreshed() {
    ...
    if (StringUtils.isBlank(loadAtStartupPath)) {
      loadAtStartupPath =
          new File(
                  new File(
                      new File(OpenmrsUtil.getApplicationDataDirectory(), "ocl"), "configuration"),
                  "loadAtStartup")
              .getAbsolutePath();
      Context.getAdministrationService()
          .saveGlobalProperty(
              new GlobalProperty(
                  OpenConceptLabConstants.GP_OCL_LOAD_AT_STARTUP_PATH, loadAtStartupPath));
    }
    ...
    if (files != null && files.length > 1) {
      throw new IllegalStateException(
          "There is more than one file in ocl/loadAtStartup directory\n"
              ...
              + loadAtStartupDir.getAbsolutePath());
    } else if (files != null && files.length != 0) {
      if (files[0].getName().endsWith(".zip")) {
        try (ZipFile zipFile = new ZipFile(files[0])) {
          Importer importer =
              Context.getRegisteredComponent("openconceptlab.importer", Importer.class);
  ...
  /**
   * If OpenMRS is unexpectedly shutdown while an OCL Import is in progress, the import record in
   * the database can be left in an "in progress" state, even though the import is no longer
   * actually running. In this situation, we want to ensure these imports are marked as failed at
   * the next startup, so that subsequent import attempts can succeed without errors that imports
