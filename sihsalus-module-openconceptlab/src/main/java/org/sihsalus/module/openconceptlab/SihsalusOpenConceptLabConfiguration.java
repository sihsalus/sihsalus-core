...
public class SihsalusOpenConceptLabConfiguration {
    ...
    private static final String CONCEPT_HEADER_EXTENSION_POINT = "org.openmrs.dictionary.conceptHeader";

    private static final Map<String, String> OPEN_CONCEPT_LAB_GLOBAL_PROPERTIES = Map.of(
            OpenConceptLabConstants.GP_SUBSCRIPTION_URL, "The OCL subscription URL",
            OpenConceptLabConstants.GP_SCHEDULED_DAYS, "Interval in days when the process is repeated",
            OpenConceptLabConstants.GP_SCHEDULED_TIME, "The the time when the process should be carried on",
    ...
    private static void ensureGlobalProperties(AdministrationService administrationService) {
        ...
        try {
            OPEN_CONCEPT_LAB_GLOBAL_PROPERTIES.forEach((property, description) -> {
                if (administrationService.getGlobalPropertyObject(property) == null) {
                    administrationService.saveGlobalProperty(new GlobalProperty(property, "", description));
                }
            });
        } finally {
