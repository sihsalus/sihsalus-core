...
public interface MetadataMappingService extends OpenmrsService {
   ...
   *
   * <p>The local concept source is in a format 'implementationId-dict'. The '-dict' postfix is
   * defined in {@link MetadataMapping#LOCAL_SOURCE_NAME_POSTFIX}.
   *
   ...
   *
   * <p>The local concept source is read from the {@link MetadataMapping#GP_LOCAL_SOURCE_UUID}
   * global property.
   *
   * @return the local concept source
   ...
   *
   * <p>A mapping in a format 'localSource:conceptId' is added to a concept if there is no other
   * mapping to the local source in the concept.
   *
   * <p>The concept is saved at the end.
   *
   * <p>It delegates to {@link ConceptAdapter#addMapping(Concept, ConceptSource, String)}
   *
   ...
   *
   * <p>It iterates over all concept and calls {@link #addLocalMappingToConcept(Concept)}.
   *
   ...
   *
   * <p>A concept is local if it does not contain a source returned by {@link
   * #getSubscribedConceptSources()}.
   *
   ...
   *
   * <p>It delegates to {@link ConceptService#getConceptByMapping(String, String)} in case (1) and
   * to {@link #getConcept(Integer)} in case (2).
   *
   ...
   *
   * <p>It is a convenience method in case id is passed as an integer and not a string.
   *
   ...
   * @should create new MetadataTermMapping if none exists within the existing source with the given
   *     mappingName
   * @should update existing MetadataTermMapping if a mapping with that name already exists within
   *     the given source
   * @should fail if no source with the give sourceName
   */
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  MetadataTermMapping mapMetadataItem(
      String referredObjectUuid,
      String referredObjectClassName,
      String sourceName,
      String mappingCode);
  ...
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  MetadataTermMapping mapMetadataItem(
      OpenmrsMetadata referredObject, String sourceName, String mappingCode);
   ...
   * @should create new MetadataSet, MetadataSetMembers, and MetadataTermMapping if none exists
   *     within the existing source with the given mapping name
   * @should if existing MetadataTermMapping, should add any objects in referredObjects that aren't
   *     currently items in MetadataSet
   * @should if existing MetadateTermMapping, should remove any objects in referredObjects that
   *     aren't currently items in MetadataSet
   ...
   */
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  <T extends OpenmrsMetadata> MetadataTermMapping mapMetadataItems(
      List<T> referredObjects, String sourceName, String mappingCode);
   ...
   */
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  Collection<MetadataTermMapping> saveMetadataTermMappings(
      Collection<MetadataTermMapping> metadataTermMappings);
   ...
   */
  @Authorized()
  List<MetadataTermMapping> getMetadataTermMappings(
   ...
   */
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  MetadataTermMapping retireMetadataTermMapping(
   ...
   */
  @Authorized()
  MetadataTermMapping getMetadataTermMapping(
   ...
   * @return metadata item or null, if not found or if either the metadata term mapping or the
   *     metadata item itself are retired
   * @throws InvalidMetadataTypeException when the requested type does not match the type of the
   *     metadata item referred to by the metadata term mapping
   * @since 1.1
  ...
  <T extends OpenmrsMetadata> T getMetadataItem(
      Class<T> type, String metadataSourceName, String metadataTermCode);

  /**
   * Get metadata items of the given type that are referred to by any metadata term mappings in the
   ...
   */
  @Authorized(MetadataMapping.PRIVILEGE_MANAGE)
  Collection<MetadataSetMember> saveMetadataSetMembers(
      Collection<MetadataSetMember> metadataSetMembers);
  ...
  /**
   * Get members of a metadata set. If members have {@link MetadataSetMember#getSortWeight()} set
   * they will be ordered in ascending order according to said weight. Note that due to differences
   * in database implementations, the order will be unpredictable, if there are null sort weights in
   * the set.
   *
   * @param metadataSet metadata set
   * @param retiredHandlingMode handle retired objects using this mode
   * @return list of members in the order defined by the optional {@link
   *     MetadataSetMember#getSortWeight()} values
   * @since 1.2
   * @should get members in desired order 1
   * @should respect retire fetch mode 1
   */
  @Authorized()
  List<MetadataSetMember> getMetadataSetMembers(
      MetadataSet metadataSet, RetiredHandlingMode retiredHandlingMode);
  ...
  /**
   * Get members of a metadata set with given uuid. If members have {@link
   * MetadataSetMember#getSortWeight()} set they will be ordered in ascending order according to
   * said weight. Note that due to differences in database implementations, the order will be
   * unpredictable, if there are null sort weights in the set.
   *
  ...
  @Authorized()
  List<MetadataSetMember> getMetadataSetMembers(
      String metadataSetUuid, RetiredHandlingMode retiredHandlingMode);
  ...
  @Authorized()
  List<MetadataSetMember> getMetadataSetMembers(
      String metadataSetUuid,
      Integer firstResult,
      Integer maxResults,
      RetiredHandlingMode retiredHandlingMode);

  /**
   * Get unretired metadata items in the set of specified type. If set members have {@link
   * MetadataSetMember#getSortWeight()} set they will be ordered in ascending order according to
   * said weight. Note that due to differences in database implementations, the order will be
   * unpredictable, if there are null sort weights in the set.
   *
   ...
   * @return list of items in the order defined by the optional {@link
   *     MetadataSetMember#getSortWeight()} values
   * @since 1.2
   ...
   */
  @Authorized(MetadataMapping.PRIVILEGE_VIEW_METADATA)
  <T extends OpenmrsMetadata> List<T> getMetadataSetItems(
