...
public class ModifyColumnGenerator extends AbstractSqlGenerator<ModifyColumnStatement> {
  ...
  public Warnings warn(
      ModifyColumnStatement modifyColumnStatement,
      Database database,
  ...
  public ValidationErrors validate(
      ModifyColumnStatement statement, Database database, SqlGeneratorChain sqlGeneratorChain) {
    ...
    for (ColumnConfig column : statement.getColumns()) {
      if (column.getConstraints() != null
          && column.getConstraints().isPrimaryKey() != null
    ...
    for (ColumnConfig column : statement.getColumns()) {
      ...
      alterTable +=
          database.escapeColumnName(
              null, statement.getSchemaName(), statement.getTableName(), column.getName());
      ...
      if (supportsExtraMetaData(database)) {
        ...
        if (column.isAutoIncrement() != null && column.isAutoIncrement()) {
          alterTable +=
              " " + database.getAutoIncrementClause(BigInteger.ONE, BigInteger.ONE, null, null);
        }
