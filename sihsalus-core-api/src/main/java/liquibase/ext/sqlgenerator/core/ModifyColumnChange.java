...
public class ModifyColumnChange extends AbstractChange implements ChangeWithColumns<ColumnConfig> {
  ...
  @DatabaseChangeProperty(
      ...
      description = "Name of the table to modify the column in")
  ...
  public SqlStatement[] generateStatements(Database database) {
    ModifyColumnStatement statement =
        new ModifyColumnStatement(
            getSchemaName(), getTableName(), getColumns().toArray(new ColumnConfig[0]));
    log.debug(
        "modify column statement for table '{}' in schema '{}' with columns {}",
