 ...
 *
 * <p>To use these properties in your logger configuration, reference them like
 * <tt>${openmrs:&lt;property&gt;}</tt>, e.g. <tt>${openmrs:applicationDirectory}</tt>.
 *
 * <p>Supported properties:
 *
 * <dl>
 *   <dt>applicationDirectory
 *   <dd>The OpenMRS application directory as a string
 *   <dt>logLocation
 *   <dd>The current value for the <tt>log.location</tt> setting
 *   <dt>logLayout
 *   <dd>The current value for the <tt>log.layout</tt> setting
 * </dl>
 *
  ...
  public String lookup(LogEvent event, String key) {
    ...
    switch (key) {
      case "applicationDirectory":
        ...
        return applicationDirectory == null || applicationDirectory.isEmpty()
            ? null
            : applicationDirectory;
      case "logLocation":
        final String logLocation =
            getGlobalProperty(adminService, OpenmrsConstants.GP_LOG_LOCATION);
        return logLocation == null
            ? null
            : logLocation.endsWith("/")
                ? logLocation.substring(0, logLocation.length() - 1)
                : logLocation;
      case "logLayout":
