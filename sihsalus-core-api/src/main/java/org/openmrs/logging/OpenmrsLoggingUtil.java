...
/**
 * Utility methods related to logging. In general, module-level code should likely only call {@link
 * #getMemoryAppender()} and {@link #getOpenmrsLogLocation()}, however, the methods to update
...
public final class OpenmrsLoggingUtil {
  ...
  public static MemoryAppender getMemoryAppender() {
    MemoryAppender memoryAppender =
        ((LoggerContext) LogManager.getContext(true))
            .getConfiguration()
   ...
   *
   * <p><strong>Warning:</strong> the result of this call can return null if either the file
   * appender uses a name other than {@link OpenmrsConstants#LOG_OPENMRS_FILE_APPENDER} or if the
   * appender with that name is not one of the default log4j2 file appending types.
   *
   * @return the path to the OpenMRS log file
   */
  public static String getOpenmrsLogLocation() {
    Appender fileAppender =
        ((LoggerContext) LogManager.getRootLogger())
            .getConfiguration()
   ...
   *
   * @param logClass optional string giving the class level to change. Defaults to {@link
  ...
  /**
   * This method is the implementation of applying a level to a logger. It is intended to be called
   * in an already synchronized context. Note these changes will only be applied once a call to
