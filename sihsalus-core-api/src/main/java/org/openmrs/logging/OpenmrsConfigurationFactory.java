 ...
 *
 * <p>Functionality provided by this {@link ConfigurationFactory}:
 *
 * <ul>
 *   <li>Load log4j2 configuration files from the OpenMRS application directory
 *   <li>Ensures that the configuration includes the MEMORY_APPENDER to keep log files in memory
 *   <li>Allows the <tt>log.level</tt> setting to override logger settings
 * </ul>
 */
...
public class OpenmrsConfigurationFactory extends ConfigurationFactory {
  ...
  private static final org.slf4j.Logger log =
      LoggerFactory.getLogger(OpenmrsConfigurationFactory.class);
    ...
    if (configLocation == null) {
      ...
      if (!configurationFiles.isEmpty()) {
        if (configurationFiles.size() == 1) {
          System.out.println(
              "Adding log4j2 configuration file: " + configurationFiles.get(0).getPath());
          ...
          for (File configFile : configurationFiles) {
            Configuration configuration =
                super.getConfiguration(loggerContext, name, configFile.toURI());
            if (configuration instanceof AbstractConfiguration) {
              ...
              System.err.println(
                  "Unable to add log4j2 configuration file: " + configFile.getPath());
            }
          }
      ...
      switch (FilenameUtils.getExtension(source.getLocation()).toLowerCase(Locale.ROOT)) {
        ...
        default:
          throw new IllegalArgumentException(
              OpenmrsConfigurationFactory.class.getName()
                  + " does not know how to handle source "
                  + source.getFile());
      }
    }
    return null;
  ...
  protected static void doOpenmrsCustomisations(AbstractConfiguration configuration) {
    ...
    MemoryAppender memoryAppender =
        configuration.getAppender(OpenmrsConstants.MEMORY_APPENDER_NAME);
    ...
    try {
      ...
      // if AdministrativeService is not available, we'll assume we're starting up and everything is
      // ok
      ...
      log.debug(
          "Skipping OpenMRS log level overrides until an OpenMRS user context is available", e);
    }
  }

  private static void applyLogLevels(
      AbstractConfiguration configuration, AdministrationService adminService) {
    String logLevel =
        adminService.getGlobalProperty(OpenmrsConstants.GLOBAL_PROPERTY_LOG_LEVEL, "");
  ...
  private static void applyLogLevel(
      AbstractConfiguration configuration, @NotNull String loggerName, String loggerLevel) {
