...
public class NameTemplate extends LayoutTemplate {
  ...
  public String format(PersonName personName) {
    ...
    try {
      ...
      // bit of hack, but we ignore the "line-by-line" format and just delimit a "line" with blank
      // space
      return StringUtils.join(personNameLines, " ");
    } catch (Exception e) {
      throw new APIException(
          "Unable to format personName " + personName.getId() + " using name template", e);
    }
  }
