...
public final class Module {
  ...
  private String downloadURL =
      null; // will only be populated when the remote file is newer than the current module
  ...
  public ModuleActivator getModuleActivator() {
    try {
      ...
      throw new ModuleException(
          "Unable to load/find moduleActivator: '" + getActivatorName() + "'", name, e);
    } catch (IllegalAccessException e) {
      throw new ModuleException(
          "Unable to load/access moduleActivator: '" + getActivatorName() + "'", name, e);
    } catch (InstantiationException e) {
      throw new ModuleException(
          "Unable to load/instantiate moduleActivator: '" + getActivatorName() + "'", name, e);
    }
   ...
   *
   * <p><strong>Should</strong> return null if no required modules exist<br>
   ...
   *
   * <p><strong>Should</strong> set modules when there is a null required modules map
   *
  ...
  /**
   * @param requiredModulesMap <code>Map&lt;String,String&gt;</code> of the <code>requiredModule
   *     </code>s to set
   * @since 1.5
   */
  ...
  public List<String> getStartBeforeModules() {
    return this.startBeforeModulesMap == null
        ? null
        : new ArrayList<>(this.startBeforeModulesMap.keySet());
  }

  /**
   * Sets the modules that this module is aware of.
   *
   * @param awareOfModulesMap <code>Map&lt;String,String&gt;</code> of the <code>awareOfModulesMap
   *     </code>s to set
   * @since 1.9
   */
  ...
  /**
   * Expands (i.e. creates instances of) {@code Extension}s defined by their class name in {@link
   * #setExtensionNames(Map)}.
   *
   * <p><strong>Should</strong> not expand extensionNames if extensionNames is null<br>
  ...
  /**
   * A map of pointId to classname. The classname is expected to be a class that extends the {@link
   * Extension} object. <br>
   * This map will be expanded into full Extension objects the first time {@link #getExtensions()}
   * is called.
   *
   * <p>The map is a direct representation of {@code extension} tags in a module's config.xml. For
   * example
   *
   * <pre>{@code
   * <extension>
  ...
  private List<Extension> expandExtensionNames() {
    ...
    if (moduleClsLoader == null) {
      log.debug(
          "Module class loader is not available, maybe the module {} is stopped/stopping",
          getName());
      return extensions;
    }
    ...
    for (Map.Entry<String, String> entry : extensionNames.entrySet()) {
      ...
      log.debug(
          getModuleId() + ": Expanding extension name (point|class): {}|{}", point, className);
      try {
        ...
        log.warn(
            getModuleId()
                + ": Unable to create instance of class defined for extension point: "
                + point,
            e);
      }
    }
   ...
   *
   * <p><strong>Should</strong> throw exception when throwable is null<br>
