...
public class JobRunrSchedulerService extends BaseOpenmrsService implements SchedulerService {
        ...
        if (task.getRepeatInterval() != null && task.getRepeatInterval() > 0) {
          if (task.getStartTime() == null) {
            ...
            JobId jobId =
                jobScheduler.schedule(
                    ...
                    (JobRunrSchedulerService service) ->
                        service.scheduleRecurrently(task.getUuid()));
            updateJobWithName(jobId, task.getName());
            // Create a placeholder recurring task that will be updated by the above task to the
            // correct interval
            String recurringJobId =
                jobRequestScheduler.scheduleRecurrently(
                    task.getUuid(),
                    Duration.between(Instant.now(), nextExecution.toInstant())
          ...
          JobId jobId =
              jobRequestScheduler.schedule(
                  UUID.fromString(task.getUuid()), runAt, new JobRequestAdapter(task, scheduledBy));
          updateJobWithName(jobId, name);
        } else {
          JobId jobId =
              jobRequestScheduler.enqueue(
          ...
          public boolean tryAdvance(Consumer<? super TaskDetails> action) {
            if (currentBatch == null || !currentBatch.hasNext()) {
              ...
              List<Job> jobs =
                  storageProvider
                      .getJobs(
                          StateName.valueOf(state.name()),
                          new OffsetBasedPageRequest("updatedAt:ASC", offset, limit))
                      .getItems();
              ...
              currentBatch =
                  jobs.stream()
                      .filter(
                          job ->
                              (isSchedulerManager
                                      || isScheduledBy(job.getJobDetails(), userSystemId))
  ...
  private RecurringJob updateRecurringJobWithName(String uuid, String name) {
    RecurringJob recurringJob =
        storageProvider.getRecurringJobs().stream()
            .filter(rj -> rj.getId().equals(uuid))
            .findFirst()
  ...
  public RecurringTaskDetails scheduleRecurrently(String name, TaskData taskData, String cron) {
    ...
    String jobId =
        jobRequestScheduler.scheduleRecurrently(
            UUID.randomUUID().toString(), cron, new JobRequestAdapter(taskData, scheduledBy));
  ...
  @Override
  public RecurringTaskDetails scheduleRecurrently(
  ...
  @Override
  public RecurringTaskDetails scheduleRecurrently(
      String name, TaskData taskData, Duration interval) {
    String scheduledBy = getScheduledBySystemId();
    String jobId =
        jobRequestScheduler.scheduleRecurrently(
            UUID.randomUUID().toString(), interval, new JobRequestAdapter(taskData, scheduledBy));
    RecurringJob job = updateRecurringJobWithName(jobId, name);
    return new JobRunrRecurringTaskDetails(job);
  }
