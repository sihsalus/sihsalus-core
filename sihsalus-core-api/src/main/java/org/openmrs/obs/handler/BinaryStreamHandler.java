...
public class BinaryStreamHandler extends AbstractHandler implements ComplexObsHandler {
   ...
   * the raw byte[] of data (If the view is set to "download", all commas and whitespace are
   * stripped out of the filename to fix an issue where the browser wasn't handling a filename with
   * whitespace properly) Note that if the method cannot find the file associated with the obs, it
  ...
  public Obs saveObs(Obs obs) throws APIException {
    try {
      ...
      obs.setValueComplex(
          StringUtils.defaultIfBlank(obs.getComplexData().getTitle(), key) + "|" + key);
