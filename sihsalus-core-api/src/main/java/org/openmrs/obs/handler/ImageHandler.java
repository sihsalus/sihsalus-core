 ...
 * taken from the image name. if the .* image name suffix matches {@link
 * javax.imageio.ImageIO#getWriterFormatNames()} then that mime type will be used to save the image.
 * Images are stored in the location specified by the global property: "obs.complex_obs_dir"
 *
...
public class ImageHandler extends AbstractHandler implements ComplexObsHandler {
  ...
  public Obs getObs(Obs obs, String view) {
    ...
    if (ComplexObsHandler.RAW_VIEW.equals(view)) {
      ...
      // hack--if this is old version of the image where we aren't explicitly saving the filename,
      // we
      // need to use "key"/"path" portion as the file, see
              ...
              (out) -> {
                ...
                if (img == null) {
                  throw new APIException(
                      "Obs.error.cannot.save.complex", new Object[] {obs.getObsId()});
                }
                ImageIO.write(img, extension, out);
                out.flush();
              },
              ObjectMetadata.builder().setFilename(filename).build(),
              getObsDir());
