...
public class ConceptReferenceRangeUtility {
   ...
   */
  private static final Cache<String, Expression> EXPRESSION_CACHE =
      Caffeine.newBuilder().maximumSize(20000).expireAfterAccess(5, TimeUnit.MINUTES).build();
  ...
  /**
   * Static {@link org.springframework.expression.EvaluationContext} which is used to run
   * evaluations. This class is thread-safe, so shareable.
   */
  private static final SimpleEvaluationContext EVAL_CONTEXT =
  ...
  public boolean evaluateCriteria(String criteria, Obs obs) {
    ...
    if (obs.getPerson() == null) {
      throw new IllegalArgumentException(
          "Failed to evaluate criteria with reason: patient is null");
    }

    if (StringUtils.isBlank(criteria)) {
      throw new IllegalArgumentException(
          "Failed to evaluate criteria with reason: criteria is empty");
    }
   ...
   * constructed from an Obs, {@code $obs} is available in the expression; otherwise only {@code
   * $patient}, {@code $fn}, {@code $context}, {@code $date}, and {@code $encounter} are available.
   *
  ...
  public boolean evaluateCriteria(String criteria, ConceptReferenceRangeContext context) {
    if (context == null) {
      throw new IllegalArgumentException(
          "Failed to evaluate criteria with reason: context is null");
    }
   ...
   *
   * <p>This class is intentionally separate from the outer class so that {@code evaluateCriteria}
   * is not callable from within expressions.
   *
  ...
  static class CriteriaFunctions {
     ...
     * contains a valid value (coded, numeric, date, text etc.) and the concept in Obs is the same
    ...
    public Obs getCurrentObs(String conceptRef, Obs currentObs) {
      ...
      if (concept != null
          && concept.equals(currentObs.getConcept())
     ...
     * @param person the person
     * @param answerConceptRef can be either concept uuid or conceptMap's code and sourceName for
     *     the observation's coded answer
     * @return true if the given concept is equal to the observation's value coded answer
     * @since 2.7.0
     */
    public boolean isObsValueCodedAnswer(
        String conceptRef, Person person, String answerConceptRef) {
      Obs obs = getLatestObs(conceptRef, person);
      if (obs == null) {
        return false;
      }
    ...
    private List<PatientProgram> getPatientPrograms(Patient patient, Date onDate) {
      ...
      return Context.getProgramWorkflowService()
          .getPatientPrograms(patient, null, null, onDate, onDate, null, false);
    }
  }
}
