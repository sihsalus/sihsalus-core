...
public class HandlerUtil implements ApplicationListener<ContextRefreshedEvent> {
   ...
   *
   * <ul>
   *   <li>The handlerType is annotated as a {@link Handler} that supports the passed type
   ...
   *
   * <p><strong>Should</strong> return a list of all classes that can handle the passed type<br>
  ...
  /**
   * Retrieves the preferred Handler for a given handlerType and type. A <em>preferred</em> handler
   * is the Handler that has the lowest defined <em>order</em> attribute in it's annotation. If
   * multiple Handlers are found for the passed parameters at the lowest specified order, then an
   * APIException is thrown.
   *
   * <p><strong>Should</strong> return the preferred handler for the passed handlerType and type<br>
   * <strong>Should</strong> throw a APIException if no handler is found<br>
   * <strong>Should</strong> throw a APIException if multiple preferred handlers are found<br>
   * <strong>Should</strong> should return patient validator for patient<br>
