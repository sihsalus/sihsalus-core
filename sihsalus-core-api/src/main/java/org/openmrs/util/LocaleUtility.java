...
public class LocaleUtility implements GlobalPropertyListener {
	...
	/**
	 * Cached version of the default locale. This is cached so that we don't have to look it up in the
	 * global property table every page load
	 */
	private static volatile Locale defaultLocaleCache = null;

	/**
	 * Cached version of the localeAllowedList. This is cached so that we don't have to look it up in
	 ...
	 * <p>
	 * <strong>Should</strong> not return null if global property does not exist<br/>
	 * <strong>Should</strong> not fail with empty global property value<br/>
	 * <strong>Should</strong> not fail with bogus global property value<br/>
	 * <strong>Should</strong> return locale object for global property<br/>
	...
	/**
	 * Compatible is a looser matching than that provided by Locale.equals(). Two locales are considered
	 * equal if they are equal, or if either does not have a country specified and the languages match.
	 * <p>
	 * <strong>Should</strong> confirm different language missing country as compatible<br/>
	 * <strong>Should</strong> confirm same language missing country as compatible<br/>
	 * <strong>Should</strong> not confirm different country as compatible<br/>
	 * <strong>Should</strong> confirm matching country as compatible<br/>
	 * <strong>Should</strong> not confirm different language as compatible<br/>
	...
	public static boolean areCompatible(Locale lhs, Locale rhs) {
		return lhs.equals(rhs) || (("".equals(lhs.getCountry())) || ("".equals(rhs.getCountry())))
	 ...
	 * </ul>
	 * For example: en_US_Traditional_WIN ...represents English language in the United States with the
	 * traditional collation for windows.
	 * <p>
	 * <strong>Should</strong> get locale from two character language code<br/>
	 * <strong>Should</strong> get locale from language code and country code<br/>
	 ...
	 * current logged in user's preferred locale if any is set, the default locale, allowed locales in
	 * the order they are specified in the 'allowed.locale.list' global property and 'en' at the very
	 * end of the set if it isn't yet among them.
	 * <p>
	 * <strong>Should</strong> return a set of locales with a predictable order<br/>
	 * <strong>Should</strong> return a set of locales with no duplicates<br/>
	 * <strong>Should</strong> have default locale as the first element if user has no preferred
	 * locale<br/>
	 * <strong>Should</strong> have default locale as the second element if user has a preferred
	 * locale<br/>
	 * <strong>Should</strong> always have english included in the returned collection<br/>
	 * <strong>Should</strong> always have default locale default value included in the returned
	 * collection
	 *
