...
public class DateUtil {
   ...
   * <ul>
   *   <li>{@code Z} — UTC
   *   <li>{@code +05:00} — offset with colon (standard ISO 8601)
   *   <li>{@code +0500} — offset without colon (used by the REST-WS module)
   *   <li>{@code +05} — hour-only offset
   *   <li>No offset — interpreted as the system default timezone
   * </ul>
   *
  ...
  public static Date parseDatetimeString(String str) throws ParseException {
    if (str.contains("T")) {
      ...
      // ISO_DATE_TIME only accepts +HH:MM offsets, so we build a formatter that also accepts +HHMM
      // (no colon--which is what the REST-WS module uses)
      DateTimeFormatter isoFormatter =
          new DateTimeFormatterBuilder()
              .append(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
              .optionalStart()
              .appendPattern("XXX")
              .optionalEnd() // e.g. +05:00 or Z
              .optionalStart()
              .appendPattern("XX")
              .optionalEnd() // e.g. +0500
              .optionalStart()
              .appendPattern("X")
              .optionalEnd() // e.g. +05
              .toFormatter();
      ...
      // If the string included a timezone/offset, convert using that offset; otherwise assume
      // system timezone
      Instant instant =
