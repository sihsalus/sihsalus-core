 ...
 */
public class ThreadSafeCircularFifoQueue<E> extends AbstractQueue<E>
  ...
  public <T> T[] toArray(T[] a) {
    ...
    try {
      ...
      final T[] result =
          a.length < size ? (T[]) Array.newInstance(a.getClass().getComponentType(), size) : a;
   ...
   * invalidated if the underlying queue becomes incompatible with the iterator's view of it. This
   * is based on the implementation of ArrayBlockingQueue.Itrs and involves the same garbage
   * collection scheme described there.
   */
   ...
   * It should iterate over each member in the queue only once, assuming that the queue is not
   * modified while this iterator remains in use. If the underlying queue is modified, the iterator
   * will attempt to recover and keep going. If it becomes too far out of synch with the underlying
   * data, an iterator may fail before iterating over all elements in a queue. However if {@link
   * #hasNext()} returns true, {@link #next()} will always return a result.
   */
