...
public class OpenmrsDateFormat extends SimpleDateFormat {
  ...
  public Date parse(String text) throws ParseException {
    ...
    if (patternMatch.find()) {
      // if it does, make sure that the string to parse ends in
      // "{non-alphanumeric-character}{digit}{digit}{digit}{digit}"
    ...
    // otherwise, verify that the pattern and the string are the same length
      ...
      if (this.toPattern().length() != text.length()) {
        throw new ParseException(
            "Unparseable date \""
                + text
                + "\" - length of date string doesn't match length of date pattern",
            0);
      }
    }
