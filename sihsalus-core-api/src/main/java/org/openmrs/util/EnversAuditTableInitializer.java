...
public class EnversAuditTableInitializer {
  ...
  public static void initialize(
      Metadata metadata, Properties hibernateProperties, ServiceRegistry serviceRegistry) {
   ...
   * @param serviceRegistry Hibernate service registry
   */
  private static void updateAuditTables(
      Metadata metadata, Properties hibernateProperties, ServiceRegistry serviceRegistry) {
    String auditTablePrefix =
        hibernateProperties.getProperty("org.hibernate.envers.audit_table_prefix", "");
    String auditTableSuffix =
        hibernateProperties.getProperty("org.hibernate.envers.audit_table_suffix", "_audit");
    ...
    SchemaMigrator schemaMigrator =
        serviceRegistry.getService(SchemaManagementTool.class).getSchemaMigrator(settings);
    ...
    if (hasErrors.get()) {
      ...
      log.info(
          "Successfully created/updated Envers audit tables using Hibernate SchemaManagementTool.");
    }
  }
  ...
  private static ExecutionOptions getExecutionOptions(
      Map<String, Object> settings, AtomicBoolean hasErrors) {
