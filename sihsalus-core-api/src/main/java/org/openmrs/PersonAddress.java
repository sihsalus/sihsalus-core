...
public class PersonAddress extends BaseChangeableOpenmrsData
  ...
  /**
   * Compares this PersonAddress object to the given otherAddress. This method differs from {@link
   * #equals(Object)} in that this method compares the inner fields of each address for equality.
   * Note: Null/empty fields on <code>otherAddress</code> /will not/ cause a false value to be
   * returned
   *
  ...
  public boolean equalsContent(PersonAddress otherAddress) {
    return new EqualsBuilder()
        .append(defaultString(otherAddress.getAddress1()), defaultString(address1))
   ...
   * PersonAddress.person object in this object AND the cloned object will point at the same person
   *
  ...
  public boolean isBlank() {
    ...
    return StringUtils.isBlank(getAddress1())
        && StringUtils.isBlank(getAddress2())
        && StringUtils.isBlank(getAddress3())
        && StringUtils.isBlank(getAddress4())
        && StringUtils.isBlank(getAddress5())
        && StringUtils.isBlank(getAddress6())
        && StringUtils.isBlank(getCityVillage())
        && StringUtils.isBlank(getStateProvince())
        && StringUtils.isBlank(getCountry())
        && StringUtils.isBlank(getCountyDistrict())
        && StringUtils.isBlank(getPostalCode())
        && StringUtils.isBlank(getLatitude())
