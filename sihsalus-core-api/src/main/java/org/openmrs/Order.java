...
public class Order extends BaseCustomizableData<OrderAttribute> implements FormRecordable {
  ...
  /**
   * Allows the orders if ordered as an orderGroup, to maintain a sequence of how members are added
   * in the group ex - for two orders of isoniazid and ampicillin, the sequence of 1 and 2 needed to
   * be maintained
   */
   ...
   *
   * <p><strong>Should</strong> copy all fields
   *
   ...
   *
   * <p><strong>Should</strong> return true if an order was activated on the check date<br>
   ...
   *
   * <p><strong>Should</strong> return true if an order expired on the check date<br>
   ...
   * <strong>Should</strong> return true if an order was activated on the check date but scheduled
   * for the future<br>
   ...
   *
   * <p><strong>Should</strong> return false for a voided order<br>
   * <strong>Should</strong> return false if dateActivated is null<br>
   ...
   * check date<br>
   * <strong>Should</strong> return false if auto expire date is null and date stopped is after
   * check date<br>
   * <strong>Should</strong> return false if dateActivated is after check date<br>
   * <strong>Should</strong> return true if auto expire date is null and date stopped is before
   * check date<br>
   ...
   *
   * @param aCheckDate - the date on which to check order. if null, will use current date
   * @return boolean indicating whether the order was discontinued on the input date the check date
   *     is after date stopped
   */
   ...
   * check date<br>
   * <strong>Should</strong> return false if date stopped is null and auto expire date is after
   * check date<br>
   ...
   * date<br>
   * <strong>Should</strong> fail if date stopped is after auto expire date<br>
   * <strong>Should</strong> return true if date stopped is null and auto expire date is before
   * check date
   *
   * @param aCheckDate - the date on which to check order. if null, will use current date
  ...
  public void setOrderNumber(String orderNumber) {
    ...
    if (!ConfigUtil.getProperty(OpenmrsConstants.GP_ALLOW_SETTING_ORDER_NUMBER, false)
      ...
      throw new APIException(
          "Unable to set order number because GP_ALLOW_SETTING_ORDER_NUMBER is set to false");
    }
   ...
   *
   * <p><strong>Should</strong> set all the relevant fields
   *
   ...
   *
   * <p><strong>Should</strong> set all the relevant fields<br>
   ...
   *
   * <p><strong>Should</strong> true if it is the same or is a subtype<br>
   ...
   *
   * <p><strong>Should</strong> return false if the concept of the orders do not match<br>
   ...
   *
   * <p><strong>Should</strong> return scheduledDate if Urgency is Scheduled<br>
   * <strong>Should</strong> return dateActivated if Urgency is not Scheduled
   *
   * @since 1.10
   */
  public Date getEffectiveStartDate() {
    return this.urgency == Urgency.ON_SCHEDULED_DATE
        ? this.getScheduledDate()
        : this.getDateActivated();
  }

  /**
   * A convenience method to return end of the schedule for order.
   *
   * <p><strong>Should</strong> return dateStopped if dateStopped is not null<br>
   ...
   * internally by the API to manage the sequencing of orders when grouped. This value may be
   * changed by the API as needed for that purpose. Instead of setting this internal value directly
   * please use {@link OrderGroup#addOrder(Order, Integer)}.
   *
  ...
  public void setFormField(String namespace, String formFieldPath) {
    formNamespaceAndPath =
        BaseFormRecordableOpenmrsData.getFormNamespaceAndPath(namespace, formFieldPath);
  }
}
