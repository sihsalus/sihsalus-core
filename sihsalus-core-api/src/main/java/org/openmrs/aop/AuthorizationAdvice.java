...
public class AuthorizationAdvice implements MethodBeforeAdvice {
  ...
  private static final String USER_IS_NOT_AUTHORIZED_TO_ACCESS =
      "User {} is not authorized to access {}";

  /**
   * Allows us to check whether a user is authorized to access a particular method.
   *
   * <p><strong>Should</strong> notify listeners about checked privileges
   *
   * @param method
   * @param args
   * @param target
    ...
    throw new APIAuthenticationException(
        Context.getMessageSourceService()
            .getMessage(
                ...
                new Object[] {StringUtils.join(attrs, ",")},
                Locale.getDefault()));
  }
  ...
  private void throwUnauthorized(User user, Method method, String attr) {
    ...
    throw new APIAuthenticationException(
        Context.getMessageSourceService()
            .getMessage("error.privilegesRequired", new Object[] {attr}, Locale.getDefault()));
  }
