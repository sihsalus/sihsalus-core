...
public class Obs extends BaseFormRecordableOpenmrsData {
	...
	/**
	 * This method does NOT add this current obs to the list of obs in obsGroup.getGroupMembers(). That
	 * must be done (and should be done) manually. (I am not doing it here for fear of screwing up the
	 * normal loading and creation of this object via hibernate/spring)
	 *
	 ...
	 * non-voided) Note this method differs from hasGroupMembers(), as that method excludes voided obs;
	 * logic is that while a obs that has only voided group members should be seen as "having no group
	 * members" it still should be considered an "obs grouping"
	 * <p>
	 ...
	 * method and a "getObsGroup" method. Which one should be returned with a simplified jstl call like
	 * ${obs.obsGroup} ? With this setup, ${obs.obsGrouping} returns a boolean of whether this obs is a
	 * parent and has members. ${obs.obsGroup} returns the parent object to this obs if this obs is a
	 * group member of some other group.
	 *
	 ...
	 * By default, this ignores voided-objects. To include voided, use {@link #hasGroupMembers(boolean)}
	 * with value true.
	 * <p>
	 ...
	 * The parameter specifies if this method whether or not voided obs should be considered.
	 * <p>
	...
	/**
	 * Get the non-voided members of the obs group, if this obs is a group. By default this method only
	 * returns non-voided group members. To get all group members, use {@link #getGroupMembers(boolean)}
	 * with value true.
	 * <p>
	 ...
	 * then this returns null.
	 * <p>
	 * <strong>Should</strong> mark the obs as dirty when the set is changed from null to a non empty
	 * one<br/>
	 ...
	 * <strong>Should</strong> not mark the obs as dirty when the set is replaced with another with same
	 * members
	 *
	...
	/**
	 * Convenience method to add the given <code>obs</code> to this grouping. Will implicitly make this
	 * obs an ObsGroup.
	 * <p>
	 * <strong>Should</strong> return true when a new obs is added as a member<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> return true when an obs is removed<br/>
	 ...
	 * in which case an empty set is returned. If the Obs argument is an ObsGroup: a Set&lt;Obs&gt; will
	 * be returned containing 1. all of this Obs' group members, and 2. all ancestor Obs that are not
	 * themselves obsGroups.
	 *
	 * @return Set&lt;Obs&gt;
	 */
	 ...
	 * <code>{@link #getPerson()}.getPersonId()</code>. It is duplicated here for speed and simplicity
	 * reasons
	 *
	...
	public void setValueBoolean(Boolean valueBoolean) {
		if (getConcept() != null && getConcept().getDatatype() != null && getConcept().getDatatype().isBoolean()) {
			if (valueBoolean != null) {
				setValueCoded(valueBoolean ? Context.getConceptService().getTrueConcept()
	 ...
	 * <p>
	 * <strong>Should</strong> return true for value_numeric concepts if value is 1<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> return true if value coded answer concept is true concept<br/>
	...
	/**
	 * Get the value for the ComplexData. This method is used by the ComplexObsHandler. The valueComplex
	 * has two parts separated by a bar '|' character: part A) the title; and part B) the URI. The title
	 * is the readable description of the valueComplex that is returned by
	 * {@link Obs#getValueAsString(java.util.Locale)}. The URI is the location where the ComplexData is
	 * stored.
	 *
	...
	/**
	 * Set the ComplexData for this Obs. The ComplexData is stored in the file system or elsewhere, but
	 * is not persisted to the database. <br>
	 * <br>
	 ...
	 * returns the title of the complexData denoted by the section of getValueComplex() before the first
	 * bar '|' character; or returns the entire getValueComplex() if the bar '|' character is missing.
	 * <p>
	 * <strong>Should</strong> return first part of valueComplex for complex obs<br/>
	 ...
	 * <strong>Should</strong> return regular number for size of zero to or greater than ten digits<br/>
	 ...
	 * <p>
	 * <strong>Should</strong> set value as boolean if the datatype of the question concept is
	 * boolean<br/>
	...
	/**
	 * When ObsService updates an obs, it voids the old version, creates a new Obs with the updates, and
	 * adds a reference to the previousVersion in the new Obs. getPreviousVersion returns the last
	 * version of this Obs.
	 */
	...
	/**
	 * Returns true if any change has been made to an Obs instance. In general, the only time isDirty()
	 * is going to return false is when a new Obs has just been instantiated or loaded from the database
	 * and no method that modifies it internally has been invoked.
	 * <p>
	 * <strong>Should</strong> return false when no change has been made<br/>
	 ...
	 * <strong>Should</strong> return true when an immutable field is changed from a null to a non null
	 * value<br/>
	 * <strong>Should</strong> return true when an immutable field is changed from a non null to a null
	 * value
	 *
