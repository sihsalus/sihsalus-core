...
public class ServiceContext implements ApplicationContextAware {
	...
	/**
	 * Static variable holding whether or not to use the system classloader. By default this is false so
	 * the openmrs classloader is used instead
	 */
	...
	/**
	 * Services implementing the OpenmrsService interface for each module. The map is keyed by the full
	...
	/**
	 * Set this service context to use the system class loader if the <code>useSystemClassLoader</code>
	 * is set to true. If false, the openmrs class loader is used to load module services
	 *
	...
	/**
	 * Returns true/false whether startRefreshingContext() has been called without a subsequent call to
	 * doneRefreshingContext() yet. All methods involved in starting/stopping a module should call this
	 * if a service method is needed -- otherwise a deadlock will occur.
	 *
	 ...
	 * <p>
	 * <b>NOTE: This method introspects top-level beans only.</b> It does <i>not</i> check nested beans
	 * which might match the specified type as well.
	 * <p>
	 * <strong>Should</strong> return a list of all registered beans of the passed type<br/>
	...
	/**
	 * Private method which returns all components registered in a Spring applicationContext of a given
	 * type This method recurses through each parent ApplicationContext
	 *
