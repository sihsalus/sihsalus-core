...
public class HibernateUtil {
  ...
  /**
   * Persists a new entity or merges a detached entity, emulating the old Hibernate {@code
   * saveOrUpdate()} behavior. For entities that are already managed, this is a no-op. For new
  ...
  public static boolean isHSQLDialect(SessionFactory sessionFactory) {
    ...
    if (isHSQLDialect == null) {
      ...
      isHSQLDialect =
          HSQLDialect.class.getName().equals(getDialect(sessionFactory).getClass().getName());
    }
  ...
  public static boolean isPostgreSQLDialect(SessionFactory sessionFactory) {
    ...
    if (isPostgreSQLDialect == null) {
      // check and cache the dialect
      isPostgreSQLDialect =
          PostgreSQLDialect.class.getName().equals(getDialect(sessionFactory).getClass().getName());
    }
  ...
  public static String escapeSqlWildcards(final String oldString, SessionFactory sessionFactory) {
    return sessionFactory
        .getCurrentSession()
        .doReturningWork(connection -> escapeSqlWildcards(oldString, connection));
  }
   ...
   */
  public static <AT extends AttributeType> List<Predicate> getAttributePredicate(
      CriteriaBuilder cb, Root<Location> locationRoot, Map<AT, String> serializedAttributeValues) {
    ...
    for (Map.Entry<AT, String> entry : serializedAttributeValues.entrySet()) {
      ...
      Predicate[] attributePredicates =
          new Predicate[] {
            cb.equal(attributeJoin.get("attributeType"), entry.getKey()),
  ...
  public static <T> T getRealObjectFromProxy(T persistentObject) {
    ...
    if (persistentObject instanceof HibernateProxy) {
      ...
      persistentObject =
          (T) ((HibernateProxy) persistentObject).getHibernateLazyInitializer().getImplementation();
    }
   ...
   */
  public static <T> T getUniqueEntityByUUID(
      SessionFactory sessionFactory, Class<T> entityClass, String uuid) throws DAOException {
   ...
   */
  public static <T> ScrollableResults getScrollableResult(
      SessionFactory sessionFactory, Class<T> type, int fetchSize) {
    Session session = sessionFactory.getCurrentSession();
