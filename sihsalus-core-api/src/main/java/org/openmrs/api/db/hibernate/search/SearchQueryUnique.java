 ...
 *
 * <p>Due to max clause count limit dictated by performance reasons only first <code>
 * max = {@link BooleanQuery#getMaxClauseCount()} / 2.5</code> duplicate keys are removed from
 * results.
 *
 * <p>The same <code>max</code> limit applies when combining results of multiple queries. Up to
...
public class SearchQueryUnique<T, R> {
   ...
   */
  public static <T, R> SearchQueryUnique<T, R> newQuery(
      Class<? extends T> scope,
      Function<SearchPredicateFactory, SearchPredicate> search,
      String uniqueKey,
      Function<T, R> mapper) {
  ...
  /**
   * Runs the search calculating the total hit count only. See {@link #search(SearchSessionFactory,
   * SearchQueryUnique, Integer, Integer, Boolean)}.
   *
  ...
  public static Long searchCount(
      SearchSessionFactory searchSessionFactory, SearchQueryUnique<?, ?> uniqueQuery) {
    return search(searchSessionFactory, uniqueQuery, 0, 0, true).getTotalHitCount();
  }

  /**
   * Runs the search without calculating the total hit count. See {@link
   * #search(SearchSessionFactory, SearchQueryUnique, Integer, Integer, Boolean)}.
   *
   * @param searchSessionFactory SearchSessionFactory
   * @param uniqueQuery unique query {@link #newQuery(Class, Function, String, Function)}
   * @return the results
  ...
  public static <T> List<T> search(
      SearchSessionFactory searchSessionFactory, SearchQueryUnique<?, T> uniqueQuery) {
   ...
   * @param <T> the type of results
   */
  public static <T> SearchUniqueResults<T> search(
      SearchSessionFactory searchSessionFactory,
      SearchQueryUnique<?, T> uniqueQuery,
      final Integer offset,
      final Integer limit,
      Boolean includeTotalHitCount) {
    ...
    while (nextQuery != null) {
      ...
      if (nextQuery.getUniqueKey() != null) {
        ...
        SearchQuery<List<?>> uniqueKeyQuery =
            searchSession
                .search(scope)
                .select(f -> f.composite(f.field(uniqueKey), f.id()))
                        ...
                        f.bool()
                            .with(
                                b -> {
                                  ...
                                  // Get rid of unique keys that were added to results in a previous
                                  // query
                                  if (!previousQueryUniqueKeys.isEmpty()) {
                                    b.filter(
                                        f.not(
                                            f.terms()
                                                .field(uniqueKey)
                                                .matchingAny(previousQueryUniqueKeys)));
                                  }
                                }))
                .toQuery();
      } else {
        ...
        if (nextQuery.getMapper() != null) {
          ...
          results.addAll(
              partialResults.stream()
                  .map((Function<Object, T>) nextQuery.getMapper())
      ...
      if (includeTotalHitCount
          || (nextQuery.getJoinedQuery() != null
              && partialResults.isEmpty()
              && currentOffset != null
              && currentOffset != 0)) {
        // Fetch total hit count only if explicitly requested or if offset caused the query to
        // return 0 results,
    ...
    return new SearchUniqueResults<>(
        results, offset, limit, includeTotalHitCount ? totalHitCount : null);
  }

  /**
   * Finds unique keys for the specified search.
   *
   * <p>It returns up to <code>max = {@link BooleanQuery#getMaxClauseCount()} / 2</code> unique
   * keys.
   *
   ...
   * @return unique keys
   */
  public static List<Object> findUniqueKeys(
