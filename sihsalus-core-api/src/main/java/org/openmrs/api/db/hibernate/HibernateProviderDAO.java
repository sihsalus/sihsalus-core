...
public class HibernateProviderDAO implements ProviderDAO {
  ...
  public List<Provider> getProviders(
    ...
    if (serializedAttributeValues != null) {
      CollectionUtils.filter(
          providers,
          new AttributeMatcherPredicate<Provider, ProviderAttributeType>(
   ...
   * @param includeRetired Boolean flag indicating whether to include retired providers in the
   *     search.
   * @return List<Predicate> A list of predicates that can be added to a CriteriaQuery for filtering
   *     Provider entities.
   */
  private List<Predicate> prepareProviderCriteria(
      CriteriaBuilder cb, Root<Provider> root, String name, boolean includeRetired) {
   ...
   */
  private Predicate getNameSearchExpression(
      String name, CriteriaBuilder cb, Join<Person, PersonName> personNameJoin) {
    ...
    Predicate givenNamePredicate =
        cb.like(cb.lower(personNameJoin.get("givenName")), mode.toLowerCasePattern(name));
    Predicate middleNamePredicate =
        cb.like(cb.lower(personNameJoin.get("middleName")), mode.toLowerCasePattern(name));
    Predicate familyNamePredicate =
        cb.like(cb.lower(personNameJoin.get("familyName")), mode.toLowerCasePattern(name));
    Predicate familyName2Predicate =
        cb.like(cb.lower(personNameJoin.get("familyName2")), mode.toLowerCasePattern(name));
  ...
  public Provider getProviderByIdentifier(String identifier) {
    ...
    cq.where(
        cb.equal(cb.lower(root.get("identifier")), MatchMode.EXACT.toLowerCasePattern(identifier)));
