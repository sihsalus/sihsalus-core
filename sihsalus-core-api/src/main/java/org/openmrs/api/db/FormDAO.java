...
public interface FormDAO {
   ...
   * historical trail would be lost). This method only clears form roles and attempts to delete the
   * form record. If the form has been included in any other parts of the database (through a
   ...
   *
   * <p>This will fail if any other entities reference this field via a foreign key
   *
   ...
   *     fields must be
   * @param concepts <code>Collection</code> of <code>Concepts</code> which the fields must point to
   ...
   *     selectMultiple
   * @param containsAllAnswers <code>Collection</code> of <code>FieldAnswer</code>s, all of which a
   *     matching field must contain (not yet implemented)
   * @param containsAnyAnswer <code>Collection</code> of <code>FieldAnswer</code>s, any one of which
