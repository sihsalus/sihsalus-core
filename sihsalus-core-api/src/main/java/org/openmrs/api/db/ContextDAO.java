...
public interface ContextDAO {
   ...
   *
   * <p><strong>Should</strong> authenticate given username and password<br>
   * <strong>Should</strong> authenticate given systemId and password<br>
   * <strong>Should</strong> authenticate given systemId without hyphen and password<br>
   * <strong>Should</strong> not authenticate given username and incorrect password<br>
   * <strong>Should</strong> not authenticate given systemId and incorrect password<br>
   * <strong>Should</strong> not authenticate given incorrect username<br>
   * <strong>Should</strong> not authenticate given incorrect systemId<br>
   * <strong>Should</strong> not authenticate given null login<br>
   * <strong>Should</strong> not authenticate given empty login<br>
   * <strong>Should</strong> not authenticate given null password when password in database is null
   * <br>
   * <strong>Should</strong> not authenticate given non null password when password in database is
   * null<br>
   * <strong>Should</strong> not authenticate when password in database is empty<br>
   * <strong>Should</strong> give identical error messages between username and password mismatch
   * <br>
   * <strong>Should</strong> lockout user after eight failed attempts<br>
   * <strong>Should</strong> authenticateWithCorrectHashedPassword<br>
   * <strong>Should</strong> authenticateWithIncorrectHashedPassword<br>
   * <strong>Should</strong> set uuid on user property when authentication fails with valid user<br>
   * <strong>Should</strong> pass regression test for 1580<br>
   ...
   * {@link Context#getAuthenticatedUser()} or {@link
   * Context#authenticate(org.openmrs.api.context.Credentials)} methods.
   *
  ...
  /**
   * Creates a new user. When the users are managed by a third-party authentication provider, it
   * will happen that a successfully authenticated user still needs to be created in OpenMRS. This
   * method is made available to authentication schemes to create new users on the fly.
   *
   ...
   *
   * <p>Should be called prior to any kind of activity
   *
   ...
   *
   * <p>The update is triggered if {@link OpenmrsConstants#GP_SEARCH_INDEX_VERSION} is blank or the
