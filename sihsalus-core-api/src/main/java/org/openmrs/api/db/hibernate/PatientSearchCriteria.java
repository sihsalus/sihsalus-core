...
public class PatientSearchCriteria {
  ...
  /**
   * Prepare a {@link QueryResult} for searching patients by name and/or identifier. The visibility
   * of this method remains public in order not to break OpenMRS modules that use this method.
   * Instead of calling this method consider using {@link org.openmrs.api.PatientService} or {@link
   ...
   */
  public QueryResult prepareCriteria(
      CriteriaBuilder cb,
      Join<Encounter, Patient> patientJoin,
      String name,
      String identifier,
    ...
    PatientSearchMode patientSearchMode =
        getSearchMode(name, identifier, identifierTypes, searchOnNamesOrIdentifiers);
    ...
    switch (patientSearchMode) {
      ...
      case PATIENT_SEARCH_BY_IDENTIFIER:
        ...
        predicates.add(
            preparePredicateForIdentifier(
                cb, identifierJoin, identifier, identifierTypes, matchIdentifierExactly));
        break;
      ...
      case PATIENT_SEARCH_BY_NAME_OR_IDENTIFIER:
        ...
        //
        // As a consequence, the *single* parameter is used to search for both names and
        // identifiers.
        //
        ...
        predicates.add(
            (cb.or(
                ...
                preparePredicateForIdentifier(
                    cb, idsJoin, identifier, identifierTypes, matchIdentifierExactly))));
        break;
      ...
      case PATIENT_SEARCH_BY_NAME_AND_IDENTIFIER:
        ...
        predicates.add(
            (cb.and(
                preparePredicateForName(cb, nameJoin, name),
                preparePredicateForIdentifier(
                    cb, idJoin, identifier, identifierTypes, matchIdentifierExactly))));
        break;
   ...
   * calling this method consider using {@link org.openmrs.api.PatientService} or {@link
   * org.openmrs.api.db.PatientDAO}.
   *
  ...
  QueryResult prepareCriteria(
    ...
    Join<Attribute, AttributeType> attributeTypeJoin =
        personSearchCriteria.addAliasForAttributeType(attributeJoin);
    Join<Patient, PatientIdentifier> idsJoin = addAliasForIdentifiers(patientJoin);
    predicates.add(
        cb.or(
            preparePredicateForName(cb, nameJoin, query, includeVoided),
            prepareCriterionForAttribute(
                cb, attributeJoin, attributeTypeJoin, query, includeVoided),
            preparePredicateForIdentifier(
                cb, idsJoin, query, new ArrayList<>(), false, includeVoided)));
    if (!includeVoided) {
      predicates.add(cb.isFalse(patientJoin.get("voided")));
    }
   ...
   */
  QueryResult prepareCriteria(
      CriteriaBuilder cb, Join<Encounter, Patient> patientJoin, String query) {
  ...
  QueryResult prepareCriteria(
    ...
    Join<Patient, PersonName> nameJoin =
        addAliasForName(cb, patientJoin, orderByNames, queryResult);
    ...
    if (matchExactly == null) {
      ...
      Join<Patient, Attribute> attributeJoin =
          personSearchCriteria.addAliasForAttribute(patientJoin);
      Join<Attribute, AttributeType> attributeTypeJoin =
          personSearchCriteria.addAliasForAttributeType(attributeJoin);
      Join<Patient, PatientIdentifier> idsJoin = addAliasForIdentifiers(patientJoin);
   ...
   */
  PatientSearchMode getSearchMode(
      String name,
      String identifier,
      List<PatientIdentifierType> identifierTypes,
      boolean searchOnNamesOrIdentifiers) {
  ...
  private Join<Patient, PersonName> addAliasForName(
      CriteriaBuilder cb,
      Join<Encounter, Patient> patientJoin,
      boolean orderByNames,
  ...
  private Join<Patient, PatientIdentifier> addAliasForIdentifiers(
      Join<Encounter, Patient> patientJoin) {
   ...
   * @param includeVoided true/false whether or not to included voided patients
   */
  private Predicate preparePredicateForIdentifier(
      CriteriaBuilder cb,
      Join<Patient, PatientIdentifier> idsJoin,
      String identifier,
      List<PatientIdentifierType> identifierTypes,
      boolean matchIdentifierExactly,
      boolean includeVoided) {
    ...
    if (identifier != null) {
      ...
      if (matchIdentifierExactly) {
        ...
        else if (regex.isEmpty() || HibernateUtil.isHSQLDialect(sessionFactory)) {
        ...
        // if the regex is present, search on that
          ...
          predicates.add(
              cb.isTrue(
                  cb.function(
                      "regexp", Boolean.class, idsJoin.get("identifier"), cb.literal(regex))));
        }
      }
    }
  ...
  private Predicate preparePredicateForIdentifier(
      ...
      List<PatientIdentifierType> identifierTypes,
      boolean matchIdentifierExactly) {
    return preparePredicateForIdentifier(
        cb, idsJoin, identifier, identifierTypes, matchIdentifierExactly, false);
  }
   ...
   * @param adminService
   */
  private Predicate getPredicateForSimpleSearch(
      CriteriaBuilder cb,
      Join<Patient, PatientIdentifier> idsJoin,
      String identifier,
   ...
   * @param patternSearch
   */
  private Predicate splitAndGetSearchPattern(
      CriteriaBuilder cb,
      Join<Patient, PatientIdentifier> idsJoin,
      String identifier,
      String patternSearch) {
   ...
   * @param includeVoided true/false whether or not to included voided patients
   */
  private Predicate preparePredicateForName(
      CriteriaBuilder cb,
      Join<Patient, PersonName> nameJoin,
      String name,
      Boolean matchExactly,
    ...
    if (nameParts.length > 0) {
      ...
      for (int i = 0; i < nameParts.length; i++) {
        ...
        if (singleName != null && !singleName.isEmpty()) {
          Predicate singleNamePredicate =
              getPredicateForName(cb, nameJoin, singleName, matchExactly, includeVoided);

          if (i > 0) {
            multiName.append(" ");
            multiName.append(singleName);
            Predicate multiNamePredicate =
                getPredicateForName(
                    cb, nameJoin, multiName.toString(), matchExactly, includeVoided);
   ...
   */
  private Predicate preparePredicateForName(
      CriteriaBuilder cb, Join<Patient, PersonName> nameJoin, String name) {
    return preparePredicateForName(cb, nameJoin, name, null, false);
  }

  private Predicate preparePredicateForName(
      CriteriaBuilder cb, Join<Patient, PersonName> nameJoin, String name, boolean includeVoided) {
   ...
   * </pre>
   *
   * Except when the name provided is less than min characters (usually 3) then we will look for an
   * EXACT match by default
   *
   * <p><strong>Should</strong> process simple space as separator<br>
   ...
   */
  private Predicate getPredicateForName(
      CriteriaBuilder cb,
      Join<Patient, PersonName> nameJoin,
      String name,
      Boolean matchExactly,
  ...
  private Predicate getPredicateForShortName(
      CriteriaBuilder cb, Join<Patient, PersonName> nameJoin, String name, boolean includeVoided) {
  ...
  private Predicate getPredicateForLongName(
      CriteriaBuilder cb, Join<Patient, PersonName> nameJoin, String name, boolean includeVoided) {
  ...
  private Predicate getPredicateForNoExactName(
      CriteriaBuilder cb, Join<Patient, PersonName> nameJoin, String name, boolean includeVoided) {
    String pattern = getMatchMode().toCaseSensitivePattern(name);
    Predicate givenNamePredicate =
        cb.and(
            cb.isNotNull(nameJoin.get("givenName")), cb.like(nameJoin.get("givenName"), pattern));
    Predicate middleNamePredicate =
        cb.and(
            cb.isNotNull(nameJoin.get("middleName")), cb.like(nameJoin.get("middleName"), pattern));
    Predicate familyNamePredicate =
        cb.and(
            cb.isNotNull(nameJoin.get("familyName")), cb.like(nameJoin.get("familyName"), pattern));
    Predicate familyName2Predicate =
        cb.and(
            cb.isNotNull(nameJoin.get("familyName2")),
            cb.like(nameJoin.get("familyName2"), pattern));

    Predicate namePredicates =
        cb.or(givenNamePredicate, middleNamePredicate, familyNamePredicate, familyName2Predicate);

    Predicate notGivenName =
        cb.or(cb.isNull(nameJoin.get("givenName")), cb.notEqual(nameJoin.get("givenName"), name));
    Predicate notMiddleName =
        cb.or(cb.isNull(nameJoin.get("middleName")), cb.notEqual(nameJoin.get("middleName"), name));
    Predicate notFamilyName =
        cb.or(cb.isNull(nameJoin.get("familyName")), cb.notEqual(nameJoin.get("familyName"), name));
    Predicate notFamilyName2 =
        cb.or(
            cb.isNull(nameJoin.get("familyName2")), cb.notEqual(nameJoin.get("familyName2"), name));

    Predicate combinedPredicate =
        cb.and(namePredicates, notGivenName, notMiddleName, notFamilyName, notFamilyName2);
    ...
    if (identifierSearched.length() > 1) {
      ...
      returnString =
          returnString.replaceAll(
              "@SEARCH-1@", identifierSearched.substring(0, identifierSearched.length() - 1));
      returnString =
          returnString.replaceAll(
              "@CHECKDIGIT@", identifierSearched.substring(identifierSearched.length() - 1));
    } else {
  ...
  private Predicate prepareCriterionForAttribute(
      CriteriaBuilder cb,
      Join<Patient, Attribute> attributeJoin,
      Join<Attribute, AttributeType> attributeTypeJoin,
      String query,
    ...
    for (String queryPart : queryParts) {
      predicates.add(
          personSearchCriteria.preparePredicateForAttribute(
              cb, attributeJoin, attributeTypeJoin, queryPart, includeVoided, matchMode));
    }
