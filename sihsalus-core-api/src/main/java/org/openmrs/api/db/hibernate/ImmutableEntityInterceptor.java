...
public abstract class ImmutableEntityInterceptor implements Interceptor {
  ...
  /**
   * Subclasses can override this to return fields that are allowed to be edited, returning null or
   * an empty array implies the entity is immutable
   *
   ...
   * true<br>
   * <strong>Should</strong> fail if the edited object is voided or retired and ignore is set to
   * false
   */
  @Override
  public boolean onFlushDirty(
    ...
    if (getSupportedType().isAssignableFrom(entity.getClass())) {
      ...
      if (CollectionUtils.isNotEmpty(changedProperties)) {
        log.debug(
            "The following fields cannot be changed for {} : {}",
            getSupportedType(),
            changedProperties);

        throw new UnchangeableObjectException(
   ...
   * allowing additional properties to be considered mutable for the duration of the thread. Note
   * that the caller should ensure that {@link #removeMutablePropertiesForThread} is invoked when
  ...
  /**
   * If any additional properties were added for a given thread by invoking {@link
   * #addMutablePropertiesForThread}, this removes them. NOTE, any usage of {@link
   * #addMutablePropertiesForThread} should typically be followed by an invocation of this method in
   * a finally block
   */
