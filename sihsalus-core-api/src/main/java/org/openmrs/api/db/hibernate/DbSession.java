...
public class DbSession {
   ...
   * transaction is required, begin the transaction. Otherwise continue the new work in the context
   * of the existing underlying transaction.
   *
   ...
   * transaction and closing the session (depending on {@link #setFlushMode(FlushMode)}, {@link
   * Transaction#commit()} calls this method).
   *
   * <p><i>Flushing</i> is the process of synchronizing the underlying persistent store with
   ...
   *
   * <p>The flush mode determines the points at which the session is flushed. <i>Flushing</i> is the
   * process of synchronizing the underlying persistent store with persistable state held in memory.
   *
   * <p>For a logically "read only" session, it is reasonable to set the session's flush mode to
   ...
   * Set the cache mode.
   *
   * <p>Cache mode determines the manner in which this session can interact with the second level
   * cache.
   *
  ...
  /**
   * End the session by releasing the JDBC connection and cleaning up. It is not strictly necessary
   * to close the session. If you’re running inside a Spring‐managed transaction, Spring will
   ...
   *
   * <p>This is the sole method on session which may be safely called from another thread.
   *
   ...
   * read-only mode, or from modifiable to read-only mode. Read-only entities are not dirty-checked
   * and snapshots of persistent state are not maintained. Read-only entities can be modified, but
   * changes are not persisted. When a proxy is initialized, the loaded entity will have the same
   * read-only/modifiable setting as the uninitialized proxy has, regardless of the session's
   * current setting. To change the read-only/modifiable setting for a particular entity or proxy
   * that is already in this session:
   *
   ...
   * operation cascades to associated instances if the association is mapped with {@code
   * cascade="replicate"}
   *
   ...
   * checking).
   *
   * <p>This operation cascades to associated instances if the association is mapped with {@code
   * cascade="save-update"}
   *
  ...
  /**
   * Update the persistent instance with the identifier of the given detached instance. If there is
   * a persistent instance with the same identifier, an exception is thrown. This operation cascades
   * to associated instances if the association is mapped with {@code cascade="save-update"}
   *
   ...
   * Return the persistent instance. If the given instance is unsaved, save a copy of and return it
   * as a newly persistent instance. The given instance does not become associated with the session.
   * This operation cascades to associated instances if the association is mapped with {@code
   * cascade="merge"}
   *
   * <p>The semantics of this method are defined by JSR-220.
   *
   ...
   * with the receiving <tt>Session</tt> or a transient instance with an identifier associated with
   * existing persistent state. This operation cascades to associated instances if the association
   * is mapped with {@code cascade="delete"}
   *
   ...
   * associated with existing persistent state. This operation cascades to associated instances if
   ...
   *
   * <p>This replaces the removed {@code buildLockRequest(LockOptions)} API.
   *
   * @param object a persistent instance
   ...
   * Return the persistent instance of the given entity class with the given identifier, or null if
   * there is no such persistent instance. (If the instance is already associated with the session,
   * return that instance. This method never returns an uninitialized instance.) Obtain the
   ...
   * @param entityName The entity name of the entity type to be retrieved
   * @return load delegate for loading the specified entity type by natural id
   * @throws HibernateException If the specified entityClass cannot be resolved as a mapped entity,
   ...
   * @return {@code true} if the entity or proxy is read-only, {@code false} if the entity or proxy
   *     is modifiable.
   */
