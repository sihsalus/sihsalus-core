...
@Transactional
public class ConditionServiceImpl extends BaseOpenmrsService
    implements ConditionService, RefByUuid {
  ...
  public Condition saveCondition(Condition condition) throws APIException {
    ...
    // If there is an existing condition, create a new condition from it and reset the existing
    // instance state
    ...
    // If the intention is to void or change the original Condition, then void the existing and save
    // the new
    if (voidOriginal) {
      User currentUser = Context.getAuthenticatedUser();
      String reason =
          saveNew ? "Condition replaced by " + newCondition.getUuid() : "Condition removed";
      condition.setVoided(true);
      condition.setVoidedBy(
          newCondition.getVoidedBy() == null ? currentUser : newCondition.getVoidedBy());
      condition.setVoidReason(
          newCondition.getVoidReason() == null ? reason : newCondition.getVoidReason());
   ...
   * in interface any longer (see #voidCondition(Condition) for that one) If other things link to
