    ...
    if (CollectionUtils.isNotEmpty(encounters)) {
      for (Encounter encounter : encounters) {
        if (!encounter.getVoided()) {
          // EncounterServiceImpl.voidEncounter and the requiredDataAdvice will set dateVoided to
          // current date
          // if it is null, we need to set it now to match the patient's date voided so that the
          // unvoid
          // handler's logic doesn't fail when comparing dates while unvoiding encounters that were
          // voided
          // with the patient
