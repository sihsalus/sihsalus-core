...
/**
 * This is the super interface for all unretire* actions that take place on all services. The {@link
 * RequiredDataAdvice} class uses AOP around each method in every service to check to see if its a
 * unretire* method. If it is a unretire* method, this class is called to handle setting the {@link
 * Retireable#getRetired()}, {@link Retireable#setRetireReason(String)}, {@link
 * Retireable#setRetiredBy(User)}, and {@link Retireable#setDateRetired(Date)} all to null. <br>
 * <br>
...
public class BaseUnretireHandler implements UnretireHandler<Retireable> {
   ...
   * <br>
   *
   * <p><strong>Should</strong> unset the retired bit<br>
  ...
  @Override
  public void handle(
      Retireable retireableObject, User retiringUser, Date origParentRetiredDate, String unused) {

    // only act on retired objects
    if (retireableObject.getRetired()
        && (origParentRetiredDate == null
            || origParentRetiredDate.equals(retireableObject.getDateRetired()))) {
