...
/**
 * This class deals with any object that implements {@link OpenmrsObject}. When an {@link
 * OpenmrsObject} is saved (via a save* method in a service), this handler is automatically called
 * by the {@link RequiredDataAdvice} AOP class. <br>
 * <br>
...
public class OpenmrsObjectSaveHandler implements SaveHandler<OpenmrsObject> {
   ...
   *
   * <p><strong>Should</strong> set empty string properties to null<br>
   * <strong>Should</strong> not set empty string properties to null for AllowEmptyStrings
   * annotation<br>
   * <strong>Should</strong> not trim empty strings for AllowLeadingOrTrailingWhitespace annotation
   * <br>
   * <strong>Should</strong> trim strings without AllowLeadingOrTrailingWhitespace annotation<br>
  ...
  public void handle(OpenmrsObject openmrsObject, User creator, Date dateCreated, String reason) {
    ...
    for (PropertyDescriptor property : properties) {
      ...
      try {
        ...
        if (property.getWriteMethod().getAnnotation(AllowLeadingOrTrailingWhitespace.class)
            == null) {
        ...
        if ("".equals(value)
            && !(openmrsObject instanceof Voidable && ((Voidable) openmrsObject).getVoided())) {
        ...
        if (log.isInfoEnabled()) {
          log.info(
              "The property "
                  + property.getName()
                  + " is no longer supported and should be ignored.",
              ex);
        }
      } catch (InvocationTargetException ex) {
        if (log.isWarnEnabled()) {
          log.warn(
              "Failed to access property " + property.getName() + "; accessor threw exception.",
              ex);
        }
      } catch (Exception ex) {
        throw new APIException(
            "failed.change.property.value", new Object[] {property.getName()}, ex);
      }
    }
  }
}
