...
public interface EncounterVisitHandler {
  ...
  /**
   * @param locale optional locale to specify. If none is passed, {@link Context#getLocale()} should
   *     be used
   * @return a displayable string so that users can pick between different assignment handlers
   */
   ...
   * whether or not it should be assigned to a {@link Visit} that is already open or if it should be
   * part of a new visit. <br>
   * <br>
   * The decision of what to do is up to the handler, but it should call {@link
   * Encounter#setVisit(Visit)} with the outcome. The visit assigned to the encounter will be
   * persisted to the database after this method is returned, so the handler is not required to save
   * it.
   *
