...
public interface UserService extends OpenmrsService {
   ...
   *
   * <p><strong>Should</strong> throw APIException if old password is not correct<br>
   ...
   *
   * <p><strong>Should</strong> fetch user with given userId
   *
   ...
   *
   * <p><strong>Should</strong> fetch user with given uuid<br>
   ...
   *
   * <p><strong>Should</strong> get user by username
   *
   ...
   *
   * <p><strong>Should</strong> verify that username and system id is unique
   *
   ...
   *
   * <p><strong>Should</strong> fetch users assigned given role<br>
   ...
   *
   * <p><strong>Should</strong> retire user and set attributes
   *
   * @param user
   * @param reason
   * @throws APIException
   ...
   *
   * <p><strong>Should</strong> unretire and unmark all attributes
   *
   * @param user
   * @throws APIException
   */
   ...
   *
   * <p><strong>Should</strong> delete given user
   *
   ...
   * this method should <b>never</b> be called &mdash; <code>Users</code> should be <em>voided</em>
   * and not <em>deleted</em> altogether (since many foreign key constraints depend on users,
   * deleting a user would require deleting all traces, and any historical trail would be lost).
   * This method only clears user roles and attempts to delete the user record. If the user has been
   * included in any other parts of the database (through a foreign key), the attempt to delete the
   * user will violate foreign key constraints and fail.
   *
   * <p><strong>Should</strong> throw APIException if cascade is true<br>
   ...
   *
   * <p><strong>Should</strong> return all privileges in the system
   *
   ...
   *
   * <p><strong>Should</strong> return all roles in the system
   *
   ...
   *
   * <p><strong>Should</strong> throw error if role inherits from itself<br>
   ...
   *
   * <p><strong>Should</strong> throw error when role is a core role<br>
   ...
   *
   * <p><strong>Should</strong> save given privilege to the database
   *
   ...
   *
   * <p><strong>Should</strong> delete given privilege from the database<br>
   ...
   *
   * <p><strong>Should</strong> fetch role for given role name
   *
   ...
   *
   * <p><strong>Should</strong> fetch privilege for given name
   *
   * @return Privilege
   * @throws APIException
   */
   ...
   *
   * <p><strong>Should</strong> fetch all users in the system<br>
   ...
   *
   * <p><strong>Should</strong> match on correctly hashed sha1 stored password<br>
   ...
   *
   * <p><strong>Should</strong> update password of given user when logged in user has edit users
   * password privilege<br>
   * <strong>Should</strong> not update password of given user when logged in user does not have
   * edit users password privilege
   *
   ...
   *
   * <p><strong>Should</strong> change the hashed password for the given user
   *
   ...
   *
   * <p><strong>Should</strong> change the secret question and answer for given user
   *
   ...
   *
   * <p><strong>Should</strong> match on correctly hashed stored password<br>
   ...
   *
   * <p><strong>Should</strong> return true when given answer matches stored secret answer<br>
   ...
   * the search matches first/last/system id and the user has one at least one of the given <code>
   * roles</code> assigned to them
   *
   * <p><strong>Should</strong> match search to familyName2<br>
   ...
   */
  @Authorized({PrivilegeConstants.GET_USERS})
  List<User> getUsers(String nameSearch, List<Role> roles, boolean includeVoided)
      throws APIException;

  /**
   * Search for a list of users by exact first name and last name.
   *
   * <p><strong>Should</strong> fetch users exactly matching the given givenName and familyName<br>
   ...
   */
  @Authorized({PrivilegeConstants.GET_USERS})
  List<User> getUsersByName(String givenName, String familyName, boolean includeRetired)
      throws APIException;

  /**
   * Get all user accounts that belong to a given person.
   *
   * <p><strong>Should</strong> fetch all accounts for a person when include retired is true<br>
   ...
   *
   * <p><b>Implementations of this method should handle privileges</b>
   *
   * <p><strong>Should</strong> return null if user is null<br>
  ...
  /**
   * Return a batch of users of a specific size sorted by personName (see {@link
   * PersonByNameComparator}) if any part of the search matches first/last/system id and the user
   * has one at least one of the given <code>roles</code> assigned to them. If start and length are
   * not specified, then all matches are returned, If name is empty or null, then all all users will
   * be returned taking into consideration the values of start and length arguments.
   *
   * <p><strong>Should</strong> return users whose roles inherit requested roles
   *
   ...
   *
   * <p><strong>Should</strong> update password if secret is correct<br>
