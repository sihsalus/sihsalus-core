...
public class ProgramWorkflow extends BaseChangeableOpenmrsMetadata {
   ...
   * @return a {@link ProgramWorkflowState} whose identifier matches the passed <code>
   *     programWorkflowStateId</code>
   */
   ...
   * @return Returns a {@link ProgramWorkflowState} whose {@link Concept} matches the passed <code>
   *     concept</code>
   */
  ...
  /**
   * Returns a {@link ProgramWorkflowState} whose Concept name matches the passed name in any {@link
   * Locale}
   *
   * @param name the Concept name to match in any {@link Locale}
   * @return a {@link ProgramWorkflowState} whose {@link Concept} name matches the passed <code>name
   *     </code>
   */
   ...
   * @return a {@link ProgramWorkflowState} which has the passed <code>name</code> in any {@link
   *     Locale}
   */
   ...
   * ProgramWorkflowStates and all retired ProgramWorkflowStates in this ProgramWorkflow if <code>
   * includeRetired</code> is true
   *
   * @param includeRetired - if false, returns only non-retired {@link ProgramWorkflowState} objects
   *     in this ProgramWorkflow
   ...
   * by {@link ConceptName}
   *
   * <p><strong>Should</strong> sort names containing numbers intelligently
   *
   * @return Set&lt;ProgramWorkflowState&gt; - all ProgramWorkflowStates, sorted by {@link
   *     ConceptName}
   */
  public Set<ProgramWorkflowState> getSortedStates() {
    ...
    Comparator<ProgramWorkflowState> stateComparator =
        (o1, o2) ->
            naturalComparator.compare(
                o1.getConcept().getName().getName(), o2.getConcept().getName().getName());
   ...
   * @return boolean true if it is allowable to transition from <code>fromState</code> to <code>
   *     toState</code>
   */
