...
public class Result extends ArrayList<Result> {
   ...
   * datatype representations of a single result can be overridden. For example, a result could have
   * a <em>numeric</em> value of 0.15 and its text value could be overridden to be "15 percent" or
   * "Fifteen percent."
   */
   ...
   *
   * <p><strong>Should</strong> not fail with null result
   *
   ...
   *
   * <p><strong>Should</strong> not fail with null list<br>
   ...
   * automatically into string format), this contructor allows the various datatype representations
   * of the result to be individually controlled. Any values set to <em>null</em> will yield the
   * natural translation of the default datatype. For example,
   *
   * <pre>
   ...
   * represents an observation like DATE STARTED ON HIV TREATMENT, the <em>result date</em>
   ...
   *     default datatype. If the result is a list, then return false only if all members are false
   ...
   *     result's own datetime). For non-datetime results, this will return the overridden datetime
   *     value (if specifically defined) or datetime representation of the default datatype. If the
   *     result is a list, then the datetime representation of the first member is returned.
   *     <table summary="Return logic">
   *         <tr>
   *         <th>Datatype</th>
   *         <th>Returns</th>
   ...
   *     default datatype. If the result is a list, then the value of the first element is returned.
   *     <table summary="Return logic">
   *         <tr>
   *         <th>Datatype</th>
   *         <th>Returns</th>
  ...
  public boolean exists() {
    if (isSingleResult()) {
      return ((valueBoolean != null && valueBoolean)
          || valueCoded != null
          || valueDatetime != null
          || (valueNumeric != null && valueNumeric != 0)
  ...
  /**
   * @return true if the result is equal to the given result or is a list containing a member equal
   *     to the given result
   */
   ...
   *     return the result only if <em>index</em> is equal to zero (0); otherwise, returns an empty
   *     result
