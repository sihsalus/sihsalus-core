 ...
 *
 * <ol>
 *   <li>provide a mechanism for building criteria
 *   <li>provide a structure that can be passed to the DAO level for analysis &amp; execution
 * </ol>
 *
 ...
 *
 * <ul>
 *   <li><code>LogicService.eval(myPatient, "CD4 COUNT");</code>
 *   <li><code>LogicService.eval(myPatient, new LogicCriteria("CD4 COUNT"));</code>
 * </ul>
 *
 * However, when criteria or restrictions need to be placed on the token, then a LogicCriteria can
 * be used to define these restrictions, e.g. <code>
 * new LogicCriteriaImpl("CD4 COUNT").lt(200).within(Duration.months(6))</code>
 */
public interface LogicCriteria {

  /**
   * Create a new LogicExpression using the <code>operand</code> and <code>operator</code> and then
   * append them to the current LogicCriteria
   *
