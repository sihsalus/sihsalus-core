...
public class User extends BaseOpenmrsObject
  ...
  @Transient private List<Locale> proficientLocales = null;

  @Transient private String parsedProficientLocalesProperty = "";
   ...
   * that anonymous/authenticated/proxy privileges are all included Return true if this user has the
   * specified privilege
   *
   * @param privilege
   ...
   *
   * <p><strong>Should</strong> return true if the user has the given role<br>
  ...
  public List<Locale> getProficientLocales() {
    String proficientLocalesProperty =
        getUserProperty(OpenmrsConstants.USER_PROPERTY_PROFICIENT_LOCALES);
            ...
            if (!proficientLocales.contains(proficientLocale)) {
              ...
              if (StringUtils.isNotEmpty(proficientLocale.getCountry())) {
                ...
                Locale languageOnlyLocale =
                    LocaleUtility.fromSpecification(proficientLocale.getLanguage());
                if (!proficientLocales.contains(languageOnlyLocale)) {
                  proficientLocales.add(
                      LocaleUtility.fromSpecification(proficientLocale.getLanguage()));
                }
              }
            }
