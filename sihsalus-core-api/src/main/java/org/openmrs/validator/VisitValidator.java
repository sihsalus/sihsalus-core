...
public class VisitValidator extends BaseCustomizableValidator implements Validator {
  ...
  public void validate(Object target, Errors errors) {
    ...
    if (visit.getStartDatetime() != null
        && OpenmrsUtil.compareWithNullAsLatest(visit.getStartDatetime(), visit.getStopDatetime())
            > 0) {
    ...
    if (visit.getId() != null) {
      ...
      for (Encounter encounter : encounters) {
        if (encounter.getEncounterDatetime().before(startDateTime)) {
          ...
          errors.rejectValue(
              "stopDatetime",
              "Visit.encountersCannotBeAfterStopDate",
    ...
    if (!nonExistingPatient && disallowOverlappingVisits() && !endingVisit) {
      VisitSearchCriteria visitSearchCriteria =
          new VisitSearchCriteriaBuilder()
      ...
      boolean overlappingSameVisit =
          overLappingVisits.size() == 1 && overLappingVisits.get(0).getId().equals(visit.getId());

      if (!overlappingSameVisit && !overLappingVisits.isEmpty()) {
        errors.rejectValue(
            "startDatetime",
            "Visit.visitCannotOverlapAnotherVisitOfTheSamePatient",
  ...
  private void validateVisitStartedBeforePatientBirthdate(Visit visit, Errors errors) {
    if (visit.getPatient() == null
        ...
        || visit.getStartDatetime() == null) {
      return;
    }

    if (visit
        .getStartDatetime()
        .before(getPatientBirthdateAdjustedIfEstimated(visit.getPatient()))) {
      errors.rejectValue(
          "startDatetime",
          "Visit.startDateCannotFallBeforeTheBirthDateOfTheSamePatient",
  ...
  private int calculateGracePeriodInYears(int age) {
    return Math.min(
        ESTIMATED_BIRTHDATE_ERROR_MARGIN_MINIMUM_YEARS,
