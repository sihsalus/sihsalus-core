  ...
  public void validate(Object obj, Errors errors) {
    if (obj == null || !(obj instanceof Cohort)) {
      throw new IllegalArgumentException(
          "The parameter obj should not be null and must be of type" + Cohort.class);
    }
    ...
    if (!cohort.getVoided()) {
      ...
      if (!CollectionUtils.isEmpty(members)) {
        for (CohortMembership member : members) {
          ...
          int dateCompare =
              OpenmrsUtil.compareWithNullAsLatest(member.getStartDate(), member.getEndDate());
          if (p != null && p.getVoided() && !member.getVoided()) {
            String message =
                "Patient " + p.getPatientId() + " is voided, cannot add voided members to a cohort";
          ...
          if (dateCompare == 1) {
            ...
            errors.rejectValue(
                "memberships", "Cohort.startDateShouldNotBeNullOrBeforeEndDate", message);
          }
        }
      }
    }
