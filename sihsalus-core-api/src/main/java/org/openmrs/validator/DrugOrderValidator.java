...
public class DrugOrderValidator extends OrderValidator implements Validator {
   ...
   *
   * <p><strong>Should</strong> fail validation if asNeeded is null<br>
    ...
    if (requireDrug) {
      if (order.getConcept() != null
          && OpenmrsUtil.nullSafeEquals(
              orderService.getNonCodedDrugConcept(), order.getConcept())) {
    ...
    if (order.getAction() != Order.Action.DISCONTINUE
      ...
      if (requireQuantity) {
        ValidationUtils.rejectIfEmpty(
            errors, "quantity", "DrugOrder.error.quantityIsNullForOutPatient");
        ValidationUtils.rejectIfEmpty(
            errors, "numRefills", "DrugOrder.error.numRefillsIsNullForOutPatient");
      }
    }
  }
  ...
  private void validatePairedFields(DrugOrder order, Errors errors) {
    if (order.getDose() != null) {
      ValidationUtils.rejectIfEmpty(
          errors, "doseUnits", "DrugOrder.error.doseUnitsRequiredWithDose");
    ...
    if (order.getQuantity() != null) {
      ValidationUtils.rejectIfEmpty(
          errors, "quantityUnits", "DrugOrder.error.quantityUnitsRequiredWithQuantity");
    }
    if (order.getDuration() != null) {
      ValidationUtils.rejectIfEmpty(
          errors, "durationUnits", "DrugOrder.error.durationUnitsRequiredWithDuration");
    }
  }
  ...
  private void validateUnitsAreAmongAllowedConcepts(Errors errors, DrugOrder order) {
    ...
    if (order.getDurationUnits() != null) {
      ...
      if (Duration.getCode(order.getDurationUnits()) == null) {
        errors.rejectValue(
            "durationUnits", "DrugOrder.error.durationUnitsNotMappedToSnomedCtDurationCode");
      }
    }
