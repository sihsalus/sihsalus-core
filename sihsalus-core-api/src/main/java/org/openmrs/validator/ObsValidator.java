 ...
 *   <li>Makes sure the obs has at least one value (if not an obs grouping)
 * </ul>
 *
 * @see org.openmrs.Obs
 */
...
public class ObsValidator implements Validator {
  ...
  private void validateHelper(Obs obs, Errors errors, List<Obs> ancestors, boolean atRootNode) {
    ...
    // if this is an obs group (i.e., parent) make sure that it has no values (other than
    // valueGroupId) set
    if (isObsGroup) {
    ...
    // if this is NOT an obs group, make sure that it has at least one value set (not counting
    // obsGroupId)
    else if (obs.getValueBoolean() == null
        && obs.getValueCoded() == null
        && obs.getValueCodedName() == null
        && obs.getValueComplex() == null
        && obs.getValueDatetime() == null
        && obs.getValueDrug() == null
        && obs.getValueModifier() == null
    ...
    // if there is a concept, and this isn't a group, perform validation tests specific to the
    // concept datatype
    else if (!isObsGroup) {
      ...
      if (dt != null) {
        ...
        } else if ((dt.isDateTime() || dt.isDate() || dt.isTime())
            && obs.getValueDatetime() == null) {
          if (atRootNode) {
    ...
    if (obs.getValueCoded() != null
        && obs.getValueDrug() != null
        && obs.getValueDrug().getConcept() != null) {
      ...
      if (!obs.getValueCoded().equals(trueConcept)
   ...
   *
   * <ol>
   *   <li>Validates Obs in relation to criteria e.g. checks patient's age is within the valid range
   *   <li>Validates if Obs' numeric value is within the valid range; i.e. >= low absolute && <=
   *       high absolute.
   *   <li>Sets field errors if numeric value is outside the valid range
   *       <ol/>
   *
  ...
  public ConceptReferenceRange getReferenceRange(Obs obs) {
    ...
    return Context.getConceptService()
        .getConceptReferenceRange(new ConceptReferenceRangeContext(obs));
  }
   ...
   * @param errors Errors to record validation issues
   */
  private void validateAbsoluteRanges(
      Obs obs, ConceptReferenceRange conceptReferenceRange, Errors errors, boolean atRootNode) {
    if (conceptReferenceRange.getHiAbsolute() != null
        && conceptReferenceRange.getHiAbsolute() < obs.getValueNumeric()) {
      if (atRootNode) {
        errors.rejectValue(
            "valueNumeric",
