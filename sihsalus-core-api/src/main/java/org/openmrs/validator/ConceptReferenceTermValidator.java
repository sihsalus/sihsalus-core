 ...
 */
@Handler(
    supports = {ConceptReferenceTerm.class},
    order = 50)
public class ConceptReferenceTermValidator implements Validator {
   ...
   *
   * <p><strong>Should</strong> fail if the concept reference term object is null<br>
  ...
  public void validate(Object obj, Errors errors) throws APIException {
    ...
    if (!StringUtils.hasText(code)) {
      errors.rejectValue(
          "code",
          "ConceptReferenceTerm.error.codeRequired",
    ...
    if (conceptReferenceTerm.getConceptSource() == null) {
      errors.rejectValue(
          "conceptSource",
          "ConceptReferenceTerm.error.sourceRequired",
    ...
    ConceptReferenceTerm termWithDuplicateCode =
        Context.getConceptService()
            .getConceptReferenceTermByCode(code, conceptReferenceTerm.getConceptSource());
    if (termWithDuplicateCode != null
        && !termWithDuplicateCode.getRetired()
        && !OpenmrsUtil.nullSafeEquals(
            termWithDuplicateCode.getUuid(), conceptReferenceTerm.getUuid())) {
      errors.rejectValue(
          "code",
          "ConceptReferenceTerm.duplicate.code",
    ...
    if (CollectionUtils.isNotEmpty(conceptReferenceTerm.getConceptReferenceTermMaps())) {
      ...
      for (ConceptReferenceTermMap map : conceptReferenceTerm.getConceptReferenceTermMaps()) {
        ...
        if (map.getConceptMapType() == null) {
          errors.rejectValue(
              "conceptReferenceTermMaps[" + index + "].conceptMapType",
              "ConceptReferenceTerm.error.mapTypeRequired",
              "Concept Map Type is required");
        } else if (map.getTermB() == null) {
          errors.rejectValue(
              "conceptReferenceTermMaps[" + index + "].termB",
              "ConceptReferenceTerm.error.termBRequired",
              "Mapped Term is required");
    ...
    ValidateUtil.validateFieldLengths(
        errors, obj.getClass(), "name", "code", "version", "description", "retireReason");
  }
}
