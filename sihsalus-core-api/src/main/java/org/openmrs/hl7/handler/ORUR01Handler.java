...
public class ORUR01Handler implements Application {
   ...
   *
   * <p><strong>Should</strong> create encounter and obs from hl7 message<br>
   ...
   * <strong>Should</strong> set value_Coded matching a boolean concept for obs if the answer is 0
   * or 1 and<br>
   * <strong>Should</strong> set value as boolean for obs if the answer is 0 or 1 and Question
   * datatype is Boolean<br>
   * <strong>Should</strong> set value_Numeric for obs if Question datatype is Numeric and the
   * answer is either 0<br>
    ...
    try {
      ...
      throw new ApplicationException(
          Context.getMessageSourceService()
              .getMessage(
                  "ORUR01.error.invalidMessageType ",
   ...
   *
   * <p><strong>Should</strong> process multiple NK1 segments
   *
  ...
  private Message processORU_R01(ORU_R01 oru) throws HL7Exception {
    ...
    String obrProblemListConceptId =
        Context.getAdministrationService()
    ...
    for (int i = 0; i < numObr; i++) {
      ...
      if (!StringUtils.hasText(obr.getUniversalServiceIdentifier().getIdentifier().getValue())) {
        throw new HL7Exception(
            Context.getMessageSourceService()
                .getMessage("ORUR01.errorInvalidOBR ", new Object[] {messageControlId}, null));
      }
   ...
   *
   * <p><strong>Should</strong> create a relationship from a NK1 segment<br>
  ...
  protected void processNK1(Patient patient, NK1 nk1) throws HL7Exception {
    ...
    if (!relCodingSystem.equals(HL7Constants.HL7_LOCAL_RELATIONSHIP)) {
      throw new HL7Exception(
          Context.getMessageSourceService()
              .getMessage("ORUR01.error.relationshipCoding", new Object[] {relCodingSystem}, null));
    }
    ...
    if (!Pattern.matches("[0-9]+[AB]", relIdentifier)) {
      throw new HL7Exception(
          Context.getMessageSourceService()
              .getMessage("ORUR01.error.relationshipType", new Object[] {relIdentifier}, null));
    }
   ...
   * created after all obs have been attached to it Creates an encounter pojo to be attached later.
  ...
  private Encounter createEncounter(MSH msh, Patient patient, PV1 pv1, ORC orc)
    ...
    if (encounterId != null) {
      ...
      //			Date dateEntered = getDateEntered(orc); // ignore this since we have no place in the data
      // model to store it
   ...
   *
   * <p><strong>Should</strong> add comments to an observation from NTE segments<br>
   * <strong>Should</strong> add multiple comments for an observation as one comment<br>
    ...
    if ("NM".equals(hl7Datatype)) {
      ...
      if (value == null || value.length() == 0) {
        ...
        if (concept.getDatatype().isBoolean()) {
          ...
          // throw this exception to make sure that the handler doesn't silently ignore bad hl7
          // message
          throw new HL7Exception(
              Context.getMessageSourceService()
                  .getMessage(
                      "ORUR01.error.CannotSetBoolean",
                      new Object[] {obs.getConcept().getConceptId()},
                      null));
        }
      } else {
        try {
      ...
      if (isConceptProposal(valueIdentifier)) {
        ...
        try {
          ...
          if (HL7Constants.HL7_LOCAL_DRUG.equals(
          ...
          throw new HL7Exception(
              Context.getMessageSourceService()
                  .getMessage(
                      "ORUR01.error.InvalidConceptId",
                      new Object[] {valueIdentifier, valueName},
                      null));
        }
      }
      if (log.isDebugEnabled()) {
      ...
      if (value == null
          || value.getData() == null
          || !StringUtils.hasText(value.getData().getValue())) {
        log.warn("Not creating null valued obs for concept " + concept);
        return null;
      }
      ...
      throw new HL7Exception(
          Context.getMessageSourceService()
              .getMessage("ORUR01.error.UpsupportedObsType", new Object[] {hl7Datatype}, null));
    }
  ...
  private ConceptName getConceptName(ST altIdentifier, ID altCodingSystem) throws HL7Exception {
    if (altIdentifier != null
        && HL7Constants.HL7_LOCAL_CONCEPT_NAME.equals(altCodingSystem.getValue())) {
   ...
   * segment. This method assumes that the check for 99NAM has been done already and is being given
   * an openmrs conceptNameId
   *
   ...
   * that code.
   *
   * <p><strong>Should</strong> return null if codingSystem not found<br>
   ...
   */
  protected Concept getConcept(String hl7ConceptId, String codingSystem, String uid)
      throws HL7Exception {
    if (codingSystem == null || HL7Constants.HL7_LOCAL_CONCEPT.equals(codingSystem)) {
      ...
      try {
        ...
        throw new HL7Exception(
            Context.getMessageSourceService()
                .getMessage("ORUR01.error.hl7ConceptId", new Object[] {hl7ConceptId, uid}, null));
      }
    } else {
  ...
  private Patient getPatient(PID pid) throws HL7Exception {
    ...
    if (patientId == null) {
      throw new HL7Exception(
          Context.getMessageSourceService().getMessage("ORUR01.error.UnresolvedPatient"));
    }
  ...
  private Person getRelative(NK1 nk1) throws HL7Exception {
    ...
    return Context.getHL7Service()
        .resolvePersonFromIdentifiers(nk1.getNextOfKinAssociatedPartySIdentifiers());
  }
  ...
  private Location getLocation(PV1 pv1) throws HL7Exception {
    ...
    if (locationId == null) {
      throw new HL7Exception(
          Context.getMessageSourceService().getMessage("ORUR01.error.UnresolvedLocation"));
    }
   ...
   *
   * <p><strong>Should</strong> pass if return value is null when uuid and id is null<br>
  ...
  public Form getForm(MSH msh) throws HL7Exception {
    ...
    if (id != null) {
      ...
      try {
        ...
        throw new HL7Exception(
            Context.getMessageSourceService().getMessage("ORUR01.error.parseFormId"), e);
      }
    }
  ...
  private User getEnterer(ORC orc) throws HL7Exception {
    ...
    if (entererId == null) {
      throw new HL7Exception(
          Context.getMessageSourceService().getMessage("ORUR01.error.UnresolvedEnterer"));
    }
   ...
   * @return
   */
  private ConceptProposal createConceptProposal(
      Encounter encounter, Concept concept, String originalText) {
  ...
  private void updateHealthCenter(Patient patient, PV1 pv1) {
    ...
    if (dischargeToLocation != null && dischargeToLocation.length() > 0) {
      ...
      PersonAttributeType healthCenterAttrType =
          Context.getPersonService().getPersonAttributeTypeByName("Health Center");

      if (healthCenterAttrType == null) {
        log.error(
            "A person attribute type with name 'Health Center' is not defined but patient "
