...
public class PersonName extends BaseChangeableOpenmrsData
    implements java.io.Serializable, Cloneable, Comparable<PersonName> {
  ...
  // Fields
  @DocumentId private Integer personNameId;
  ...
  @FullTextField(
      name = "givenNameStart",
      analyzer = SearchAnalysis.START_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  @FullTextField(
      name = "givenNameAnywhere",
      analyzer = SearchAnalysis.ANYWHERE_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  ...
  @FullTextField(
      name = "middleNameStart",
      analyzer = SearchAnalysis.START_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  @FullTextField(
      name = "middleNameAnywhere",
      analyzer = SearchAnalysis.ANYWHERE_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  ...
  @FullTextField(
      name = "familyNameStart",
      analyzer = SearchAnalysis.START_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  @FullTextField(
      name = "familyNameAnywhere",
      analyzer = SearchAnalysis.ANYWHERE_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  ...
  @FullTextField(
      name = "familyName2Start",
      analyzer = SearchAnalysis.START_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  @FullTextField(
      name = "familyName2Anywhere",
      analyzer = SearchAnalysis.ANYWHERE_ANALYZER,
      searchAnalyzer = SearchAnalysis.EXACT_ANALYZER)
  ...
  /**
   * Compares this PersonName object to the given otherName. This method differs from {@link
   * #equals(Object)} in that this method compares the inner fields of each name for equality. Note:
   * Null/empty fields on <code>otherName</code> /will not/ cause a false value to be returned
   *
   * <p><strong>Should</strong> return true if given middle and family name are equal
   *
  ...
  public boolean equalsContent(PersonName otherName) {
    return new EqualsBuilder()
        .append(defaultString(otherName.getPrefix()), defaultString(prefix))
        ...
        .append(defaultString(otherName.getFamilyNameSuffix()), defaultString(familyNameSuffix))
        .isEquals();
  }
   ...
   *
   * <p><strong>Should</strong> copy every property of given personName
   *
   ...
   * spaces in between. If any part of {@link #getPrefix()}, {@link #getGivenName()}, {@link
   * #getMiddleName()}, etc are null, they are not included in the returned name
   *
   * <p><strong>Should</strong> not put spaces around an empty middle name
   *
      ...
      if (ret == 0) {
        ret =
            OpenmrsUtil.compareWithNullAsGreatest(
                pn1.getFamilyNamePrefix(), pn2.getFamilyNamePrefix());
      }
      if (ret == 0) {
        ret =
            OpenmrsUtil.compareWithNullAsGreatest(
