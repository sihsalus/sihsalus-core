...
public class EventClassScanner implements AutoCloseable {
   ...
   */
  public <T> List<Class<? extends T>> getClasses(Class<? extends T> type)
        ...
        if (resources == null) {
          ...
          if (metadataReaderFactory instanceof CachingMetadataReaderFactory) {
            ((CachingMetadataReaderFactory) metadataReaderFactory)
                .setCacheLimit(resolvedResources.length);
          }
    ...
    for (Resource resource : resources) {
      ...
      // we call isConcrete() here first since the metadataReader always initializes the class
      // metadata,
