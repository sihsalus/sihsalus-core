...
public class ParameterUtil {
   ...
   *
   * <p>This supports the scenario where a REST client submits a minimal representation of an object
   * by looking up the real object, and not just converting the submitted object's properties, e.g.
   * you could submit an encounter type that looks like {"display": "Emergency", "uuid":
   * "07000be2-26b6-4cce-8b40-866d8435b613"} or even just "07000be2-26b6-4cce-8b40-866d8435b613"
   *
   * @param parameters
   * @param parameterValues
   * @return
   */
  public static Map<String, Object> convertParameterValues(
      List<Parameter> parameters, Map<String, Object> parameterValues) throws Exception {
      ...
      if (parameter != null) {
        ...
        if (parameter.getCollectionType() != null) {
          ...
          if (resource != null) {
            ...
            // no resource for this type, so it's probably a simple type (date, etc), and we use the
            // REST
            // module's ConversionUtil to convert it

            // this looks hacky, but there's no good way to construct a ParameterizedType at
            // run-time; the only
            // way I know to programmatically construct something like a collection with type
            // information is by
            // using an array
  ...
  /**
   * Expects you to pass in a String UUID, or else an object with a String "uuid" property. Also
   * assumes that resource is a conventional OpenMRS implementation, with a "getByUniqueId" method
   *
   * @param resource
   * @param item
   * @return
  ...
  private static Object getRealInstanceByUuid(Resource resource, Object item) {
    try {
      ...
      String uuid =
          (String) (item instanceof String ? item : PropertyUtils.getProperty(item, "uuid"));
      return method.invoke(resource, uuid);
    } catch (Exception e) {
      throw new IllegalArgumentException(
          "Failed to find item=" + item + " using resource=" + resource, e);
    }
  }
  ...
  /**
   * If any parameters on mapped.parameterizable are not explicitly mapped, they will be mapped to
   * ${paramName}
   *
   * @param mapped
   */
