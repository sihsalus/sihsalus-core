...
@Resource(
    name = RestConstants.VERSION_1 + "/bedTag",
    supportedClass = BedTag.class,
...
public class BedTagResource extends DelegatingCrudResource<BedTag> {
  ...
  @Override
  protected void delete(BedTag bedTag, String reason, RequestContext context)
      throws ResponseException {
  ...
  private BedTag constructBedTag(String uuid, SimpleObject properties) {
    ...
    if (uuid != null) {
      ...
      if (bedTag == null) throw new IllegalPropertyException("Bed Tag not exist");

      if (properties.get("name") != null) bedTag.setName((String) properties.get("name"));
    } else {
      bedTag = new BedTag();
