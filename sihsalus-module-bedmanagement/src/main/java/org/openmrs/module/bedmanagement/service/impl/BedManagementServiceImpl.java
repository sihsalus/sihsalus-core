...
public class BedManagementServiceImpl extends BaseOpenmrsService implements BedManagementService {
  ...
  public List<AdmissionLocation> getAdmissionLocations() {
    LocationTag admissionLocationTag =
        locationService.getLocationTagByName(
            BedManagementApiConstants.LOCATION_TAG_SUPPORTS_ADMISSION);
  ...
  @Override
  @Transactional
  public AdmissionLocation setBedLayoutForAdmissionLocation(
      AdmissionLocation admissionLocation, Integer row, Integer column) {
    Location location = admissionLocation.getWard();
    for (int i = 1; i <= row; i++) {
      for (int j = 1; j <= column; j++) {
        BedLocationMapping bedLocationMapping =
            bedManagementDao.getBedLocationMappingByLocationAndRowAndColumn(location, i, j);
    ...
    List<BedLocationMapping> bedLocationMappings =
        bedManagementDao.getBedLocationMappingsByLocation(admissionLocation.getWard());
      ...
      if (bedlocationMapping.getRow() > row || bedlocationMapping.getColumn() > column) {
        if (bedlocationMapping.getBed() == null) {
          ...
          throw new APIException(
              "Cannot downsize bed layout with existing bed in the way at row "
                  + bedlocationMapping.getRow()
                  + " and column "
                  + bedlocationMapping.getColumn());
        }
      }
    }
  ...
  private BedDetails unassignBed(Bed bed, Patient patient, Date bedAssignmentEndDatetime) {
    ...
    if (bed != null) {
      List<BedPatientAssignment> currentAssignments =
  ...
  public List<BedDetails> unAssignBedsInEndedVisit(Visit visit) {
    ...
    List<BedPatientAssignment> bedassignments =
        bedManagementDao.getBedPatientAssignmentByVisit(visit.getUuid(), false);
  ...
  public List<Bed> getBeds(
      String locationUuid, String bedTypeName, BedStatus status, Integer limit, Integer offset) {
    Location location =
        locationUuid != null ? locationService.getLocationByUuid(locationUuid) : null;
    BedType bedType = null;
    if (bedTypeName != null) {
      List<BedType> bedTypes = bedManagementDao.getBedTypes(bedTypeName, 1, 0);
      if (bedTypes.size() == 0) throw new APIException("Invalid bed type name");
  ...
  public BedLocationMapping saveBedLocationMapping(BedLocationMapping bedLocationMapping) {
    ...
    BedLocationMapping existingBedLocationMapping =
        bedManagementDao.getBedLocationMappingByLocationAndRowAndColumn(
    ...
    if (existingBedLocationMapping != null && existingBedLocationMapping.getBed() == null) {
      ...
      bedLocationMapping = existingBedLocationMapping;
    } else if (existingBedLocationMapping != null
        && existingBedLocationMapping.getBed() != null
        && !existingBedLocationMapping
            .getBed()
            .getId()
            .equals(bedLocationMapping.getBed().getId())) {
  ...
  @Override
  @Transactional(readOnly = true)
  public BedLocationMapping getBedLocationMappingByLocationUuidAndRowColumn(
      String locationUuid, Integer row, Integer column) {
