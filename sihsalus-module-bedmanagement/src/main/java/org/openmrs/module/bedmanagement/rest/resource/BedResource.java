...
@Resource(
    name = RestConstants.VERSION_1 + "/bed",
    supportedClass = Bed.class,
...
public class BedResource extends DelegatingCrudResource<Bed> {
  ...
  public Model getGETModel(Representation rep) {
    ...
    if (rep instanceof DefaultRepresentation || rep instanceof RefRepresentation) {
      modelImpl
          ...
          .property("row", new IntegerProperty())
          .property("column", new IntegerProperty())
          .property("bedNumber", new StringProperty())
  ...
  public Integer getRow(Bed bed) {
    BedLocationMapping bedLocationMapping =
        Context.getService(BedManagementService.class).getBedLocationMappingByBedId(bed.getId());
  ...
  protected PageableResult doSearch(RequestContext context) {
    ...
    List<Bed> bedList =
        getBedManagementService().getBeds(locationUuid, bedType, bedStatus, null, null);
    return new NeedsPaging<>(bedList, context);
  }
  ...
  @Override
  protected void delete(Bed bed, String reason, RequestContext requestContext)
  ...
  public Object create(SimpleObject propertiesToCreate, RequestContext context)
    ...
    if (propertiesToCreate.get("bedNumber") == null || propertiesToCreate.get("bedType") == null)
      throw new ConversionException(
          "Required parameters:  bedNumber, bedType, row, column, locationUuid");

    Bed bed = this.constructBed(null, propertiesToCreate);
    Context.getService(BedManagementService.class).saveBed(bed);
    if (propertiesToCreate.get("row") != null
        && propertiesToCreate.get("column") != null
      ...
      BedLocationMapping bedLocationMapping =
          this.constructBedLocationMapping(bed, locationUuid, row, column);
  ...
  public Object update(String uuid, SimpleObject propertiesToUpdate, RequestContext context)
    ...
    Context.getService(BedManagementService.class).saveBed(bed);
    if (propertiesToUpdate.get("row") != null
        && propertiesToUpdate.get("column") != null
  ...
  private Bed constructBed(String uuid, SimpleObject properties) {
    ...
    if (properties.get("bedType") != null) {
      ...
      List<BedType> bedTypes =
          Context.getService(BedManagementService.class).getBedTypes(bedTypeName, 1, 0);
      if (bedTypes.size() == 0) throw new IllegalPropertyException("Invalid bed type name");
      bedType = bedTypes.get(0);
    }

    if (uuid != null) {
      bed = Context.getService(BedManagementService.class).getBedByUuid(uuid);
      if (bed == null) throw new IllegalPropertyException("Bed not exist");
      ...
      bed = new Bed();
      if (properties.get("bedNumber") == null
          || properties.get("bedType") == null
          || properties.get("row") == null
      ...
      bed.setBedNumber((String) properties.get("bedNumber"));
      bed.setStatus(
          properties.get("status") != null
              ? parseBedStatus(properties.get("status").toString()).toString()
  ...
  private BedLocationMapping constructBedLocationMapping(
      Bed bed, String locationUuid, Integer row, Integer column) {
    BedLocationMapping bedLocationMapping =
        Context.getService(BedManagementService.class).getBedLocationMappingByBedId(bed.getId());
    if (bedLocationMapping != null) bedLocationMapping.setBed(null);
