 ...
 *
 * <p>Could replace this with a generic validator? though isAddressStructured may be more
 * appropriate as a service method?
 */
@Deprecated
  ...
  public boolean isAddressStructured(PersonAddress pa) {
    ...
    if ((matchingLocationId = getMatchingLocation(country, hierarchyList)) != -1) {
      if ((matchingLocationId =
              getMatchingLocation(pa.getStateProvince(), ahs.getNextComponent(matchingLocationId)))
          != -1) {
        if ((matchingLocationId =
                getMatchingLocation(
                    pa.getCountyDistrict(), ahs.getNextComponent(matchingLocationId)))
            != -1) {
          if ((matchingLocationId =
                  getMatchingLocation(
                      pa.getCityVillage(), ahs.getNextComponent(matchingLocationId)))
              != -1) {
            if ((matchingLocationId =
                    getMatchingLocation(pa.getAddress3(), ahs.getNextComponent(matchingLocationId)))
                != -1) {
              if ((matchingLocationId =
                      getMatchingLocation(
                          pa.getAddress1(), ahs.getNextComponent(matchingLocationId)))
                  != -1) {
                structured = true;
              }
            }
