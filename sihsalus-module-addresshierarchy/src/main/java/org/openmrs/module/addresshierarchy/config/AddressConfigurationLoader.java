...
public class AddressConfigurationLoader {
  ...
  public static String getChecksumsPath() {
    return Paths.get(OpenmrsUtil.getApplicationDataDirectory(), "configuration_checksums")
        .toString();
  }

  public static Map<String, String> loadAddressConfiguration(
      Path configPath, Map<String, String> previousChecksums) {
    final ConfigDirUtil configUtil =
        new ConfigDirUtil(
            configPath.toString(), AddressHierarchyConstants.ADDRESS_HIERARCHY_DOMAIN);
    ...
    if (!domainDir.exists()) {
      log.info(
          "Address hierarchy domain folder appears not present, skipping the loading process: "
              + domainDir.getPath());
    ...
    final String xmlConfigChecksumPath =
        AddressHierarchyConstants.ADDRESS_HIERARCHY_DOMAIN + "/" + xmlConfigFileName;
    ...
    if (checksum.equals(lastChecksum)) {
      log.info(
          "Address hierarchy configuration file is unchanged, skipping it: " + xmlConfigFileName);
    } else {

      log.info(
          "Address hierarchy configuration file has changed, reloading it: " + xmlConfigFileName);
      ...
      forceReloadEntries =
          true; // if anything upstream is changed, we force reload the address entries from CSV
    }
    ...
    final String csvEntriesChecksumPath =
        AddressHierarchyConstants.ADDRESS_HIERARCHY_DOMAIN + "/" + csvEntriesFileName;
    ...
    if (checksum.equals(lastChecksum) && !forceReloadEntries) {
      log.info(
          "Address hierarchy entries CSV file is unchanged, skipping it: " + csvEntriesFileName);
    } else {
      log.info(
          "Address hierarchy entries CSV file has changed, reloading it: " + csvEntriesFileName);
      installAddressHierarchyEntries(
          configUtil,
          addressConfiguration.getAddressHierarchyFile(),
  ...
  public static void installAddressTemplate(Object addressTemplate) {
    try {
      ...
      String xml =
          Context.getSerializationService().getDefaultSerializer().serialize(addressTemplate);
    ...
    if (getService().getAddressHierarchyLevelsCount() == 0) {
      ...
      for (AddressComponent component : addressComponents) {
        AddressHierarchyLevel level =
            getService().getAddressHierarchyLevelByAddressField(component.getField());
        level.setRequired(component.isRequiredInHierarchy());
        getService().saveAddressHierarchyLevel(level);
      }
    }
  ...
  public static void installAddressHierarchyEntries(
      ConfigDirUtil configDirUtil, AddressHierarchyFile file, boolean deleteEntries) {
    ...
    try {
      ...
      AddressHierarchyImportUtil.importAddressHierarchyFile(
          is, file.getEntryDelimiter(), file.getIdentifierDelimiter());
    } catch (Exception e) {
